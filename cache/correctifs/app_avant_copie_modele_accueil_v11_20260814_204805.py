...
class PageMaitreV2(tk.Tk):
    ...
    def _render_header_canvas(self, _event=None) -> None:
        ...
        # Rebord bas du ruban : plus visible, comme sur l’Accueil.
        relief_high = self._mix(theme.WINDOW, "#F2EFE8", 0.18)
        relief = self._mix(theme.WINDOW, "#726C64", 0.24)
        canvas.create_line(
            0, height - 5,
            width, height - 5,
            fill=relief_high,
    ...
    def _build_visualisation_vierge(self, parent: tk.Frame) -> None:
        ...
        def draw_global_panel(_event=None):
            ...
            global_panel.create_line(
                14, h - 4, w - 22, h - 4,
                fill=self._mix(PANEL, "#F2EFE8", 0.18),
                width=1,
                tags=("panel_decor",),
            )
            global_panel.create_line(
                12, h - 2, w - 20, h - 2,
                fill=self._mix(PANEL, "#726C64", 0.22),
                width=4,
                tags=("panel_decor",),
            )
    ...
    def _get_notebook_spiral_photo(
        ...
        *,
        ...
        # RELIEF_ACCUEIL_GLOBAL_CORRIGE_V8B
# REBORD_RELIEF_VISIBLE_GLOBAL_V9B
# RELIEF_ADOUCI_NON_NOIR_V10
    ...
    def _attach_notebook_spiral_frame(self, frame, color: str) -> None:
        ...
        spring.place(x=0, y=2, width=24, relheight=1.0)
    ...
    def _card(
        ...
        *,
        ...
        # Rebord TomeLinea visible :
        # fin liseré clair + bande basse plus marquée.
        tk.Frame(
            card,
            bg=self._mix(theme.PANEL, "#726C64", 0.22),
            height=4,
        ).pack(side="bottom", fill="x", padx=10, pady=(0, 1))
        tk.Frame(
            card,
            bg=self._mix(theme.PANEL, "#F2EFE8", 0.18),
            height=1,
        ...
        def render(_event=None):
            ...
            for index, (title, value, accent) in enumerate(stats):
                ...
                canvas.create_line(
                    x1 + 16, y2 - 4,
                    x2 - 20, y2 - 4,
                    fill=self._mix(theme.PANEL, "#FFFFFF", 0.16),
                    width=1,
                )
                canvas.create_line(
                    x1 + 14, y2 - 2,
                    x2 - 18, y2 - 2,
                    fill=self._mix(theme.PANEL, "#000000", 0.20),
                    width=3,
                )
                canvas.create_rectangle(
                    x1, y1, x2, y1 + 5,
            ...
            canvas.create_line(
                left + 16, book_y2 - 4,
                right - 24, book_y2 - 4,
                fill=self._mix(theme.PANEL, "#F2EFE8", 0.18),
                width=1,
            )
            canvas.create_line(
                left + 14, book_y2 - 2,
                right - 22, book_y2 - 2,
                fill=self._mix(theme.PANEL, "#726C64", 0.22),
                width=4,
            )
            canvas.create_rectangle(
