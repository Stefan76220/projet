...
class TomeLineaV4(tk.Tk):
    ...
    def _composition_refresh_after_structure_change(
        ...
        if count_var is not None:
            count_var.set(f"{len(book.page_order)} pages")
    ...
    def _composition_cover_selector_body_page_ids(self) -> list[str]:
        ...
        return [
            ...
            if page_id in self.session.book.pages
            and not is_cover_face(self.session.book.pages[page_id])
        ]
    ...
    def _composition_apply_format_change(
        ...
        try:
            ...
            def action(project):
                ...
                project.book.metadata["format_reflow_final_page_count"] = len(
                    project.book.page_order
                )
