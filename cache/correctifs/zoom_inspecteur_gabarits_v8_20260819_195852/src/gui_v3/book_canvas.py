...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Gabarits — poste de zoom historique de Structure, remis à gauche.
        # Il reste très discret dans B. Dès qu'il est utilisé, la visionneuse
        # globale prend le relais afin que l'agrandissement ne soit pas coupé
        # par les limites de B.
        ...
        self.gabarit_zoom_nav.configure(width=108, height=44, bg=theme.WINDOW_DEEP)
        self.gabarit_zoom_nav.set_caption("Zoom")
        self.gabarit_zoom_nav.pack(fill="x")
        self.gabarit_zoom_fit_btn = tk.Button(
            self.gabarit_zoom_controls,
            text="Adapter", command=self.gabarit_reset_zoom,
            bg="#202D36", fg="#9FB8BC",
            activebackground="#314752", activeforeground="#FFFFFF",
            relief="flat", bd=0, highlightthickness=0,
            padx=4, pady=2, font=(theme.FONT_UI, 6, "bold"),
            cursor="hand2", takefocus=False,
        )
        self.gabarit_zoom_fit_btn.pack(fill="x", pady=(2, 0))
    ...
    def _gabarit_sync_zoom_widget(self) -> None:
        """Synchronise les deux postes de zoom avec la visionneuse globale."""
        value = int(getattr(self, "_gabarit_overlay_zoom", 100) if getattr(self, "_gabarit_overlay_active", False) else 100)
        self._gabarit_zoom = 100
        try:
    ...
    def open_gabarit_overlay(self, *, zoom: int = 100) -> None:
        """Ouvre le zoom global Gabarits, hors des limites de B."""
        if getattr(self, "_work_mode", "") != "gabarits":
            return
        self._gabarit_overlay_active = True
        self._gabarit_overlay_zoom = max(45, min(600, int(zoom)))
        self._gabarit_overlay_pan_x = 0.0
        self._gabarit_overlay_pan_y = 0.0
        self._gabarit_overlay_page_box = None
        self._gabarit_zoom = 100
        ...
        self.gabarit_overlay_frame.place(x=0, y=0, relwidth=1, relheight=1)
        try:
            self.gabarit_overlay_frame.tk.call("raise", self.gabarit_overlay_frame._w)
        except Exception:
            self.gabarit_overlay_frame.lift()
        try:
            self.gabarit_overlay.focus_set()
        except Exception:
            pass
    ...
    def close_gabarit_overlay(self) -> None:
        ...
        self._gabarit_overlay_pan_y = 0.0
        self._gabarit_overlay_page_box = None
        if self._gabarit_overlay_render_job is not None:
            try:
                self.after_cancel(self._gabarit_overlay_render_job)
            except Exception:
                pass
            self._gabarit_overlay_render_job = None
        try:
    ...
    def _gabarit_zoom_navigate(self, velocity: float) -> None:
        """Ancien geste Structure : utiliser Zoom ouvre la visionneuse globale."""
        velocity = float(velocity)
        if abs(velocity) < 0.001:
            return
        if not getattr(self, "_gabarit_overlay_active", False):
            self.open_gabarit_overlay(zoom=100)
        self._gabarit_overlay_zoom_navigate(velocity)
    ...
    def gabarit_zoom_in(self) -> None:
        if not getattr(self, "_gabarit_overlay_active", False):
            self.open_gabarit_overlay(zoom=112)
        else:
            self._gabarit_overlay_set_zoom(int(self._gabarit_overlay_zoom) + 12)

    def gabarit_zoom_out(self) -> None:
        if not getattr(self, "_gabarit_overlay_active", False):
            self.open_gabarit_overlay(zoom=88)
        else:
            self._gabarit_overlay_set_zoom(int(self._gabarit_overlay_zoom) - 12)

    def gabarit_reset_zoom(self) -> None:
        """Adapter = page entière dans la visionneuse globale si elle est ouverte."""
        if getattr(self, "_gabarit_overlay_active", False):
            self._gabarit_overlay_zoom = 100
            self._gabarit_overlay_pan_x = 0.0
            self._gabarit_overlay_pan_y = 0.0
            self._gabarit_sync_zoom_widget()
    ...
    def gabarit_step_zoom(self, delta: int, *, anchor: tuple[float, float] | None = None) -> None:
        """Compatibilité interne : tout zoom Gabarits passe désormais hors de B."""
        if not getattr(self, "_gabarit_overlay_active", False):
            self.open_gabarit_overlay(zoom=112 if int(delta) > 0 else 88)
            return
        step = 12 if int(getattr(self, "_gabarit_overlay_zoom", 100)) < 220 else 20
        self._gabarit_overlay_set_zoom(
            int(self._gabarit_overlay_zoom) + (step if int(delta) > 0 else -step),
            anchor=anchor,
        )
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        if zoom_controls is not None:
            try:
                zoom_controls.place(x=10, rely=0.52, anchor="w")
                zoom_controls.lift()
            except Exception:
                pass
    ...
    def _on_ctrl_mousewheel(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            # Même logique que la molette simple : le zoom quitte B.
            value = 112 if event.delta > 0 else 88
            self.open_gabarit_overlay(zoom=value)
            return "break"
    ...
    def _on_mousewheel(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            # Reprise de l'ancienne visionneuse Structure : le premier cran de
            # molette ouvre la page hors de B, puis la molette reste ancrée
            # précisément sous le pointeur dans la visionneuse globale.
            if self._gabarit_point_in(getattr(self, "_gabarit_page_box", None), float(event.x), float(event.y)):
                self.open_gabarit_overlay(zoom=112 if event.delta > 0 else 88)
            return "break"
        delta = -1 if event.delta > 0 else 1
