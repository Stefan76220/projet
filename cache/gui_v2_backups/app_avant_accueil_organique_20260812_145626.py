...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def get_project_icon(key: str, icon_path: Path, size=72):
            # Icônes TomeLinea réalistes : même famille visuelle dans Créer et Ouvrir.
            render_size = max(72, int(size))
            cache_key = ("accueil_realiste", key, render_size)
        ...
        def get_open_project_photo():
            cache_key = "_open_project_types_photo_realiste"
            ...
            try:
                path = (
                    ...
                    / "assets"
                    / "gui_v2"
                    / "accueil_realistic_icons"
                    / "trio.png"
                )
                image = Image.open(path).convert("RGBA")
                image.thumbnail((170, 96), Image.Resampling.LANCZOS)
                photo = ImageTk.PhotoImage(image)
                setattr(self, cache_key, photo)
                return photo
            except Exception:
                return None
