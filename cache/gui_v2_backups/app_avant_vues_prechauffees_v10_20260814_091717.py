...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V9_REVELATION_ATOMIQUE
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # Fenêtre TomeLinea sans barre native Windows :
        # la croix interne est la seule commande de fermeture.
        try:
        ...
        # ----------------------------------------------------------
        # Contrôleur de changement de vue V9 — révélation atomique.
        #
        # Même méthode que pour les fenêtres TomeLinea déjà stabilisées :
        #   1. la Toplevel reste créée et mappée ;
        #   2. elle devient totalement transparente ;
        #   3. géométrie + contenu sont reconstruits hors perception ;
        #   4. update_idletasks finalise le dessin ;
        #   5. alpha revient directement à 1.
        #
        # Aucun withdraw/deiconify : Windows ne recrée pas la fenêtre.
        # ----------------------------------------------------------
        switch_state = {
            "active": False,
        }
            ...
            if key == "global":
                ...
                subtitle_label.configure(
                    text=(
                        ...
                        "et des parties. Cliquez-glissez pour parcourir le livre."
                    )
                )
            else:
                book_host.tkraise()
        ...
        def show_tab(key, *, animate=True):
            ...
            try:
                # Même principe que DEMARRAGE_ATOMIQUE :
                # la fenêtre reste mappée mais devient invisible.
                try:
                    win.attributes("-alpha", 0.0)
                except tk.TclError:
                ...
                # Laisse Windows enregistrer l'état invisible AVANT
                # tout changement de géométrie.
                win.update_idletasks()

                apply_mode_content(key)

                # Un seul changement de taille et de position.
                win.geometry(
                    f"{final_w}x{final_h}"
                    f"+{final_x}+{final_y}"
                )

                # Force la nouvelle géométrie avant tout redessin final.
                win.update_idletasks()
                ...
                # Tous les widgets/canvas sont maintenant terminés
                # alors que la fenêtre est encore invisible.
                win.update_idletasks()
                ...
                # Sécurité : ne jamais laisser la fenêtre transparente
                # à la suite d'une exception mineure de dessin.
                try:
                    if alpha_supported:
                        win.attributes("-alpha", 1.0)
                    elif not win.winfo_viewable():
                        win.deiconify()
                except Exception:
                    pass
