...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_build_station_button_image(self, state: str, size: int):
        ...
        gold="#D5AD63" if state=="active" else "#9B8C6E"
        # Ombre
        d.rounded_rectangle((17,18,81,82),radius=17,fill=(0,0,0,78))
        # Plaquette sombre externe
        d.rounded_rectangle((13,13,79,79),radius=16,fill="#0F1519",outline="#556168",width=2)
        # Corps interne avec léger relief
        d.rounded_rectangle((18,18,74,74),radius=14,fill="#1E282D",outline="#2C3A40",width=2)
        d.rounded_rectangle((21,21,71,41),radius=10,fill="#2A353A",outline=None)
        # Pastille centrale plus premium qu'un simple cercle plat
        d.rounded_rectangle((28,28,64,64),radius=12,fill="#12191D",outline=accent if state!="normal" else "#607077",width=2)
        d.line((36,46,56,46),fill=accent,width=4)
        d.ellipse((44,42,52,50),fill=gold if state=="active" else ("#8ABAB7" if state=="hover" else "#899699"))
        if state in {"hover","active","danger"}:
            d.rounded_rectangle((11,11,81,81),radius=18,outline=accent,width=2 if state=="hover" else 3)
        if state=="disabled":
            image=Image.alpha_composite(image,Image.new("RGBA",(S,S),(13,18,21,130)))
        try:
