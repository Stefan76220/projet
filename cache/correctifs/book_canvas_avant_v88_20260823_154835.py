# TOMELINEA_GABARITS_ICONES_PREMIUM_V87 — corrections déroulés + icônes Texte/Annuler/Rétablir
...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_build_premium_icon_image(self, kind: str, state: str, size: int):
        ...
        if kind == "settings":
            ...
            self._gabarit_premium_metal_rect(image,(22,22,74,34),5)
            self._gabarit_premium_metal_rect(image,(43,30,53,72),5)
            d=ImageDraw.Draw(image)
            d.line((28,20,68,20),fill="#E9EFF1",width=2)
            d.line((29,76,67,76),fill=accent,width=3)
            d.ellipse((33,72,41,80),fill=gold)
        elif kind == "image":
            ...
            d=ImageDraw.Draw(image)
            if kind=="undo":
                shadow_pts=[(69,36),(60,29),(47,27),(34,30),(25,40),(23,52),(28,63),(40,70),(56,70)]
                metal_pts=[(68,36),(59,30),(47,28),(35,31),(27,40),(26,51),(30,61),(41,67),(55,67)]
                accent_pts=[(55,67),(42,67),(31,61),(27,51)]
                d.line(shadow_pts,fill="#2B3338",width=10,joint="curve")
                d.line(metal_pts,fill=silver,width=6,joint="curve")
                d.line(accent_pts,fill=accent,width=3,joint="curve")
                d.polygon([(29,26),(14,33),(28,41)],fill=silver)
            else:
                shadow_pts=[(27,36),(36,29),(49,27),(62,30),(71,40),(73,52),(68,63),(56,70),(40,70)]
                metal_pts=[(28,36),(37,30),(49,28),(61,31),(69,40),(70,51),(66,61),(55,67),(41,67)]
                accent_pts=[(41,67),(54,67),(65,61),(69,51)]
                d.line(shadow_pts,fill="#2B3338",width=10,joint="curve")
                d.line(metal_pts,fill=silver,width=6,joint="curve")
                d.line(accent_pts,fill=accent,width=3,joint="curve")
                d.polygon([(67,26),(82,33),(68,41)],fill=silver)
        elif kind == "trash":
    ...
    def _gabarit_inspector_station_button(self, target, key: str, box, label: str, *, selected=False, danger=False, disabled=False, tooltip=""):
        ...
        button_size=max(22,min(30,int(round(min((x2-x1)-2.0,(y2-y1)-16.0)))))
        cy=y1+16.0+button_size/2.0
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        def icon_strip(items, *, button=41.0, gap=8.0, y_offset=0.0, center=True, start_x=None):
            # V87 — les commandes du déroulé utilisent toujours le même bouton,
            # mais la grille est réellement répartie et laisse plus d'air entre
            # le texte, le bouton et les lignes voisines.
            yy=content_top+y_offset
            cell_h = button + 18.0
            count = len(items)
            usable_left = fly_left + 11.0
            usable_right = fly_right - 11.0
            usable_w = max(0.0, usable_right-usable_left)
            used_gap = float(gap)
            total_w = count*button + max(0,count-1)*used_gap
            if start_x is not None:
                sx=float(start_x)
            elif center and count > 0:
                if total_w <= usable_w:
                    sx = usable_left + (usable_w-total_w)/2.0
                else:
                    if count > 1:
                        used_gap = max(4.0, (usable_w-count*button)/(count-1))
                    sx = usable_left
            else:
                sx = usable_left
            for i,item in enumerate(items):
                ...
                x1=sx+i*(button+used_gap)
                label=self._gabarit_station_label(key,selected=selected,tooltip=tip)
                self._gabarit_inspector_station_button(target,key,(x1,yy,x1+button,yy+cell_h),label,selected=selected,danger=danger,disabled=disabled,tooltip=tip)
            return cell_h
        ...
        if active_band == "allowed":
            ...
            section_y = content_top + row_h + 10.0
            target.create_text(fly_left+11.0,section_y,text="VERROUILLAGE",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            ...
            ),button=39.0,gap=12.0,y_offset=(section_y-content_top)+11.0,center=False,start_x=fly_left+11.0)
            ...
            if frame not in {"margins","page","bleed"}:
                frame="bleed"
            section_y = content_top + row_h + 10.0
            target.create_text(fly_left+11.0,section_y,text="CADRAGE AUTO",anchor="w",fill="#9FB0B3",font=(theme.FONT_UI,8,"bold"))
            row_h2 = icon_strip((
            ...
            ),button=37.0,gap=11.0,y_offset=(section_y-content_top)+12.0)
            ...
            label_y=section_y + 12.0 + row_h2 + 10.0
            target.create_text(
            ...
            bar_x1=fly_left+18.0; bar_x2=fly_right-18.0; bar_y=label_y+28.0
            ...
            ),button=31.0,gap=4.0,y_offset=yy+11.0)
            yy += 11.0 + row_h + 11.0
            ...
            ),button=34.0,gap=12.0,y_offset=yy+11.0)
            yy += 11.0 + row_h + 11.0
            ...
            ),button=34.0,gap=10.0,y_offset=yy+11.0)
            yy += 11.0 + row_h + 11.0
            ...
            ),button=34.0,gap=7.0,y_offset=yy+11.0)
