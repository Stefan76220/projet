...
def main() -> int:
    ...
    report: dict = {
        ...
        "visual_fixes": [
            ...
            "composition_shell_left_explorer_and_canvas_center",
            "composition_explorer_page_selection_drives_canvas",
            "composition_canvas_scroll_updates_active_page",
            "composition_previous_next_page_sync",
        ],
    ...
    app = tk.Tk()
    app.title("TomeLinea — Phase 2.36")
    app.geometry("1450x900")
    app.minsize(1100, 700)
    app.configure(bg="#252A30")

    composition_shell = tk.Frame(app, bg="#252A30", bd=0, highlightthickness=0)
    composition_shell.pack(fill="both", expand=True)
    composition_shell.grid_rowconfigure(0, weight=1)
    composition_shell.grid_columnconfigure(0, minsize=245, weight=0)
    composition_shell.grid_columnconfigure(1, weight=1)

    explorer = tk.Frame(composition_shell, bg="#20252A", bd=0, highlightthickness=0, width=245)
    explorer.grid(row=0, column=0, sticky="nsew")
    explorer.grid_propagate(False)

    tk.Label(
        explorer, text="LIVRE", bg="#20252A", fg="#E8EBEE",
        font=("Segoe UI", 10, "bold"), anchor="w", padx=14, pady=12,
    ).pack(fill="x")

    explorer_style = ttk.Style(app)
    try:
        explorer_style.theme_use("clam")
    except tk.TclError:
        pass
    explorer_style.configure(
        "TL.Treeview", background="#20252A", fieldbackground="#20252A",
        foreground="#DDE2E6", rowheight=27, borderwidth=0, relief="flat",
        font=("Segoe UI", 9),
    )
    explorer_style.map(
        "TL.Treeview", background=[("selected", "#59464A")],
        foreground=[("selected", "#FFFFFF")],
    )
    explorer_style.configure(
        "TL.Treeview.Heading", background="#20252A", foreground="#AEB6BD",
        borderwidth=0, relief="flat", font=("Segoe UI", 9, "bold"),
    )

    book_tree = ttk.Treeview(explorer, show="tree", style="TL.Treeview", selectmode="browse")
    book_tree.pack(fill="both", expand=True, padx=(8, 4), pady=(0, 8))
    tree_nodes = {
        "begin": book_tree.insert("", "end", text="Début", open=False),
        "body": book_tree.insert("", "end", text="Corps", open=True),
        "end": book_tree.insert("", "end", text="Fin", open=False),
    }

    center = tk.Frame(composition_shell, bg="#252A30", bd=0, highlightthickness=0)
    center.grid(row=0, column=1, sticky="nsew")
    center.grid_rowconfigure(1, weight=1)
    center.grid_columnconfigure(0, weight=1)

    nav_bar = tk.Frame(center, bg="#2D333A", height=44, bd=0, highlightthickness=0)
    nav_bar.grid(row=0, column=0, sticky="ew")
    nav_bar.grid_propagate(False)

    active_page_label = tk.Label(
        nav_bar, text="Page — / —", bg="#2D333A", fg="#E8EBEE",
        font=("Segoe UI", 10, "bold"),
    )
    active_page_label.pack(side="left", padx=14)

    host_holder = tk.Frame(center, bg="#252A30", bd=0, highlightthickness=0)
    host_holder.grid(row=1, column=0, sticky="nsew")

    page_state = {"active": 1, "count": 0, "syncing": False}
    ...
    def sync_page_ui(page_no: int, page_count: int | None = None) -> None:
        try:
        ...
        if page_count is not None:
            try:
                page_state["count"] = max(0, int(page_count))
            except (TypeError, ValueError):
                pass
        count = int(page_state.get("count") or 0)
        if count:
            page_no = min(page_no, count)
        page_state["active"] = page_no
        active_page_label.config(text=f"Page {page_no} / {count if count else '—'}")
        item = f"page:{page_no}"
        if book_tree.exists(item):
            page_state["syncing"] = True
            try:
                book_tree.selection_set(item)
                book_tree.see(item)
            finally:
                page_state["syncing"] = False

    def populate_explorer(page_count: int) -> None:
        body = tree_nodes["body"]
        for child in book_tree.get_children(body):
            book_tree.delete(child)
        page_state["count"] = max(0, int(page_count or 0))
        for page_no in range(1, page_state["count"] + 1):
            book_tree.insert(body, "end", iid=f"page:{page_no}", text=f"Page {page_no}")
        book_tree.item(body, open=True)
        if page_state["count"]:
            sync_page_ui(1, page_state["count"])

    def go_to_page(page_no: int) -> None:
        count = int(page_state.get("count") or 0)
        if not count:
            return
        page_no = max(1, min(int(page_no), count))
        host = host_ref.get("host")
        if host is not None:
            host.go_to_page(page_no)
        sync_page_ui(page_no, count)

    def on_tree_select(_event=None) -> None:
        if page_state.get("syncing"):
            return
        ...
        if item.startswith("page:"):
            try:
                go_to_page(int(item.split(":", 1)[1]))
            except (TypeError, ValueError):
                pass

    def previous_page() -> None:
        go_to_page(int(page_state.get("active") or 1) - 1)

    def next_page() -> None:
        go_to_page(int(page_state.get("active") or 1) + 1)

    tk.Button(
        nav_bar, text="Précédente", command=previous_page, bg="#343B42", fg="#DDE1E4",
        activebackground="#444D56", activeforeground="#FFFFFF", relief="flat", bd=0,
        padx=12, pady=6, cursor="hand2",
    ).pack(side="right", padx=(4, 12), pady=7)
    tk.Button(
        nav_bar, text="Suivante", command=next_page, bg="#343B42", fg="#DDE1E4",
        activebackground="#444D56", activeforeground="#FFFFFF", relief="flat", bd=0,
        padx=12, pady=6, cursor="hand2",
    ).pack(side="right", padx=4, pady=7)
    book_tree.bind("<<TreeviewSelect>>", on_tree_select)
    ...
    def on_page_changed(event: dict) -> None:
        page_no = event.get("pageNo")
        page_count = event.get("pageCount")
        sync_page_ui(page_no, page_count)
        report["active_page"] = page_state.get("active")
        write_report()
    ...
    def on_ready(session_snapshot: dict) -> None:
        ...
        populate_explorer(int(session_snapshot.get("page_count") or 0))
        app.title("TomeLinea — Phase 2.36 — Composition")
        host = host_ref.get("host")
        if host is not None:
            try:
