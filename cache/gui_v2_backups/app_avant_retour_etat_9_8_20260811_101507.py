    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton de commande : personnalité retrouvée, sans soulignement."""
            ...
            face = self._mix(color, "#FCF9F4", 0.84 if hover else 0.88)
            inner_face = self._mix(color, "#FFFDFC", 0.92 if hover else 0.95)
            shadow = self._mix(color, "#857B70", 0.66 if hover else 0.71)
            outline = self._mix(color, "#D6CBC0", 0.16 if hover else 0.26)
            accent = self._mix(color, "#FFFFFF", 0.18 if hover else 0.08)
            accent_soft = self._mix(color, "#FFFFFF", 0.58)
            text_color = "#2C342F"
            ...
            home_round_rect(
                ...
                10,
                fill=inner_face,
                outline=self._mix(face, "#FFFFFF", 0.42),
                width=1,
                tags=(tag, "home_button"),
            )

            # Rehausse de personnalité : accent doux à gauche.
            home_round_rect(
                canvas,
                x1 + 8, y1 + 8,
                x1 + 15, y2 - 8,
            ...
            canvas.create_line(
                x1 + 16, y1 + 5,
                x2 - 16, y1 + 5,
                fill=self._mix(face, "#FFFFFF", 0.68),
                width=2,
                tags=(tag, "home_button"),
            )
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Zone de choix : carte papier satinée, ton sur ton, sans soulignement."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FBF8F3", 0.87 if hover else 0.90)
            inner_face = self._mix(color, "#FFFDFC", 0.94 if hover else 0.96)
            shadow = self._mix(color, "#877D72", 0.72 if hover else 0.76)
            outline = self._mix(color, "#D3C9BE", 0.14 if selected else 0.26)
            accent = self._mix(color, "#FFFFFF", 0.10 if selected else 0.18)
            accent_soft = self._mix(color, "#FFFFFF", 0.56)
            texture = self._mix(color, "#E9E0D4", 0.66)
            ...
            # Petite languette colorée et rappel haut.
            home_round_rect(
                canvas,
                x1 + 10, y1 + 10,
                x1 + 30, y1 + 18,
            ...
            # Texture très légère pour garder l'effet papier.
            mid = (y1 + y2) / 2
            for dy in (-12, 0, 12):
                canvas.create_line(
                    x1 + 18, mid + dy,
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule informative ton sur ton, sans effet Excel."""
            width = max(66, int(len(text) * 5.0) + 29)
            height = 20

            face = self._mix(bg, "#FFFDFC", 0.40)
            outline = self._mix(fg, "#D7CDC2", 0.18)
            accent = self._mix(fg, "#FFFFFF", 0.18)
            accent_soft = self._mix(fg, "#FFFFFF", 0.56)
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Grand bouton raccourci : plus vivant, sans aplat massif ni soulignement."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FBF8F3", 0.88 if hover else 0.91)
            inner_face = self._mix(color, "#FFFDFC", 0.94 if hover else 0.96)
            shadow = self._mix(color, "#877D73", 0.73 if hover else 0.77)
            outline = self._mix(color, "#D2C8BD", 0.18 if hover else 0.28)
            accent = self._mix(color, "#FFFFFF", 0.12 if hover else 0.20)
            accent_soft = self._mix(color, "#FFFFFF", 0.56)
            ...
            # Accent vertical + petite capsule haute.
            home_round_rect(
                canvas,
                x1 + 8, y1 + 10,
                x1 + 14, y2 - 10,
