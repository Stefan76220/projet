...
class TomeLineaV3(tk.Tk):
    ...
    def _build_workspace(self):
        ...
        def layout(event=None):
            ...
            gap = 10
            a_h = max(96, min(116, int(h * 0.118)))
    ...
    def _build_zone_a(self):
        """Zone A : identité TomeLinea, projet, travail actuel et commandes globales."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=4, minsize=360)
        body.grid_columnconfigure(4, weight=3, minsize=280)

        # 1. Signature TomeLinea — cohérente avec l'Accueil.
        brand = tk.Frame(body, bg=theme.PANEL)
        brand.grid(row=0, column=0, sticky="nsw", padx=(0, 14), pady=(2, 1))

        brand_icon = self._home_brand_icon_photo(56) if hasattr(self, "_home_brand_icon_photo") else None
        if brand_icon is not None:
            icon_label = tk.Label(brand, image=brand_icon, bg=theme.PANEL, bd=0)
            icon_label.image = brand_icon
            icon_label.pack(side="left", padx=(0, 10))

        brand_text = tk.Frame(brand, bg=theme.PANEL)
        brand_text.pack(side="left", anchor="n")
        brand_title = self._home_brand_title_photo(205) if hasattr(self, "_home_brand_title_photo") else None
        if brand_title is not None:
            title_label = tk.Label(brand_text, image=brand_title, bg=theme.PANEL, bd=0)
            title_label.image = brand_title
            title_label.pack(anchor="w")
        else:
            tk.Label(
                brand_text, text="TomeLinea", bg=theme.PANEL, fg=theme.INK,
                font=(theme.FONT_TITLE, 16, "bold"),
            ).pack(anchor="w")

        signature = tk.Canvas(
            brand_text, width=206, height=12, bg=theme.PANEL,
            bd=0, highlightthickness=0,
        )
        signature.pack(anchor="w", pady=(1, 0))
        signature.create_text(
            0, 6, anchor="w",
            text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE",
            fill=theme.ACCENT_BRIGHT,
            font=(theme.FONT_UI, 6, "bold"),
        )
        signature.create_line(176, 9, 184, 3, fill="#E7C37A", width=2)
        signature.create_line(184, 9, 191, 2, fill="#E47A45", width=2)
        signature.create_line(191, 9, 198, 4, fill="#86A98C", width=2)

        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=1, sticky="ns", padx=(4, 16)
        )

        # 2. Projet + type de livre.
        project = tk.Frame(body, bg=theme.PANEL)
        project.grid(row=0, column=2, sticky="nsew", padx=(0, 18), pady=(1, 0))
        tk.Label(
            project, text="PROJET", bg=theme.PANEL, fg=theme.ACCENT_BRIGHT,
            font=(theme.FONT_UI, 8, "bold"),
        ).pack(anchor="w")
        ...
        tk.Label(
            project, textvariable=self.project_name_var, bg=theme.PANEL,
            fg=theme.INK, font=(theme.FONT_TITLE, 14, "bold"),
        ).pack(anchor="w", pady=(1, 2))

        project_type = tk.Frame(project, bg=theme.PANEL)
        project_type.pack(anchor="w", pady=(0, 1))
        self.zone_a_type_icon = tk.Label(project_type, bg=theme.PANEL, bd=0)
        self.zone_a_type_icon.pack(side="left", padx=(0, 7))
        tk.Label(
            project_type, textvariable=self.project_meta_var, bg=theme.PANEL,
            fg=theme.MUTED, font=(theme.FONT_UI, 10, "bold"),
        ).pack(side="left")

        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=3, sticky="ns", padx=(0, 18)
        )

        # 3. Contexte de travail courant.
        current = tk.Frame(body, bg=theme.PANEL)
        current.grid(row=0, column=4, sticky="nsew", padx=(0, 14), pady=(1, 0))
        tk.Label(
            current, text="TRAVAIL ACTUEL", bg=theme.PANEL,
            fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 8, "bold"),
        ).pack(anchor="w")

        self.workspace_current_var = tk.StringVar(value="STRUCTURE")
        tk.Label(
            current, textvariable=self.workspace_current_var, bg=theme.PANEL,
            fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 11, "bold"),
        ).pack(anchor="w", pady=(2, 2))

        tk.Label(
            current,
            textvariable=self.book_canvas.status_var,
            bg=theme.PANEL,
            fg=theme.MUTED,
            font=(theme.FONT_UI, 8),
            justify="left",
        ...
        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=5, sticky="ns", padx=(0, 12)
        )

        # 4. Commandes permanentes. La réduction actuelle est conservée telle quelle.
        actions = tk.Frame(body, bg=theme.PANEL)
        actions.grid(row=0, column=6, sticky="nse", pady=(2, 0))

        info = tk.Label(
            actions, text="ⓘ", bg=theme.PANEL, fg=theme.MUTED,
            font=(theme.FONT_UI, 12, "bold"), cursor="hand2", padx=6,
        )
        info.pack(side="left", padx=(0, 4))
        info.bind("<Button-1>", lambda _e: self._show_zone_info("a"))

        home_btn = V3Button(actions, "Accueil", self.show_home, compact=True)
        home_btn.configure(font=(theme.FONT_UI, 9, "bold"), padx=11, pady=6)
        home_btn.pack(side="left", padx=3)

        help_btn = V3Button(actions, "Aide", self._show_general_help, compact=True)
        help_btn.configure(font=(theme.FONT_UI, 9, "bold"), padx=11, pady=6)
        help_btn.pack(side="left", padx=3)

        sep = tk.Frame(actions, bg=theme.BORDER_SOFT, width=1, height=34)
        sep.pack(side="left", padx=7)

        V3Button(actions, "Réduire", self._minimize_to_taskbar, compact=True).pack(
            side="left", padx=3
        )

        close = V3Button(actions, "✕", self.destroy, compact=True)
        close.configure(
            fg="#F08A5A",
    ...
    def _build_structure_tab(self, parent):
        """Structure V5 : C fournit les outils ; B est l'unique zone de construction."""
        ...
        def show_structure_modal(
            ...
            *,
            ...
            """Fenêtre TomeLinea intégrée : aucune fenêtre Windows pour Structure."""
            close_structure_modal()

            host = getattr(self, "stack", self)
            overlay = tk.Frame(host, bg=theme.WINDOW_DEEP, bd=0, highlightthickness=0)
            ...
            body.grid(row=2, column=0, sticky="nsew", padx=20, pady=14)
            body.grid_columnconfigure(0, weight=1)

            if message:
                tk.Label(
                    body, text=message, bg=theme.PANEL_ALT, fg=theme.INK,
                    justify="left", anchor="w", wraplength=max(260, width - 80),
                    font=(theme.FONT_UI, 9),
            ...
            foot.grid(row=3, column=0, sticky="ew", padx=20, pady=(8, 16))
        ...
        # Bandeau contextuel : agit toujours sur la sélection courante de B.
        ...
        def create_custom_type():
            ...
            def build_body(body):
                ...
                tk.Label(
                    form, text="Aperçu", bg=theme.PANEL_ALT, fg=theme.MUTED,
                    font=(theme.FONT_UI, 8, "bold"),
                ).grid(row=2, column=0, sticky="nw", padx=(0, 12), pady=(10, 5))
                visual_row = tk.Frame(form, bg=theme.PANEL_ALT)
                visual_row.grid(row=2, column=1, sticky="w", pady=(10, 5))
                for label, value in (
                    ("Libre", "custom"),
                    ("Texte", "text"),
                    ("Chapitre", "chapter"),
                    ...
                    def choose(v=value):
                        state["visual"].set(v)
                        for key, button in visual_buttons.items():
                            active = key == v
                            button.configure(
                    ...
                    button = tk.Button(
                        visual_row, text=label, command=choose,
                        bg=theme.ACCENT_DARK if value == "custom" else theme.PANEL_SOFT,
                        fg=theme.WHITE if value == "custom" else theme.INK,
                        activebackground=theme.ACCENT_SOFT, activeforeground=theme.WHITE,
                        relief="flat", bd=0, padx=8, pady=5,
                        font=(theme.FONT_UI, 7, "bold"), cursor="hand2",
                    )
                    button.pack(side="left", padx=(0, 4), pady=2)
                    visual_buttons[value] = button

                tk.Label(
                    form, text="Comportement", bg=theme.PANEL_ALT, fg=theme.MUTED,
                    font=(theme.FONT_UI, 8, "bold"),
                ).grid(row=3, column=0, sticky="nw", padx=(0, 12), pady=(10, 5))
                options = tk.Frame(form, bg=theme.PANEL_ALT)
                options.grid(row=3, column=1, sticky="w", pady=(10, 5))
                tk.Checkbutton(
                    options, text="Ce type peut être dupliqué",
                    variable=state["duplicable"],
                    bg=theme.PANEL_ALT, fg=theme.INK,
                    activebackground=theme.PANEL_ALT, activeforeground=theme.INK,
                    selectcolor=theme.WINDOW_DEEP,
                    font=(theme.FONT_UI, 8), cursor="hand2",
                ).pack(side="left")

                tk.Label(
                    form,
                    text="Le nom et l’aperçu définissent la nouvelle brique. Les règles recto-verso seront gérées dans Règles.",
                    bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                    font=(theme.FONT_UI, 7), anchor="w",
                ).grid(row=4, column=1, sticky="w", pady=(7, 0))
            ...
            def confirm_new_type():
                ...
                if not name:
                    # Message dans la fenêtre TomeLinea elle-même, pas de messagebox.
        ...
        def select_family(name: str):
            ...
            if name == "Pages":
                make_brick(
                    row, "Page sans type", "page",
                    {"type": "", "label": "Sans type", "untyped": True},
                    accent=True,
                ).pack(side="left", padx=(0, 7), pady=(0, 6))
                for type_key, label, _custom in page_types():
                    make_brick(
                        row, label, "page", {"type": type_key, "label": label}
                    ).pack(side="left", padx=(0, 6), pady=(0, 6))
                palette_hint_var.set(
                    "Clic : choisissez une brique, puis cliquez sur un + dans B  •  glissez sur une page pour remplacer son type"
                )

            elif name == "Parties":
                make_brick(
                    row, "+ Partie", "group", {"type": "partie", "label": "+ Partie"},
                    accent=True,
                ).pack(side="left", padx=(0, 7), pady=(0, 6))
                small_action(row, "Renommer la sélection", rename_selected).pack(
                    side="left", padx=(0, 6), pady=(0, 6)
                )
                small_action(row, "Supprimer la partie vide", delete_selected, danger=True).pack(
                    side="left", padx=(0, 6), pady=(0, 6)
                )
                palette_hint_var.set(
                    "Cliquez + Partie puis sur un + entre deux parties dans B. Déplacement ensuite par glisser-déposer."
                )

            elif name == "Blancs":
                make_brick(
                    row, "Page blanche", "page",
                    {"type": "page_blanche", "label": "Page blanche"},
                ).pack(side="left", padx=(0, 7), pady=(0, 6))
                small_action(row, "Blanc avant", lambda: insert_blank_relative("before")).pack(
                    side="left", padx=(0, 6), pady=(0, 6)
                )
                small_action(row, "Blanc après", lambda: insert_blank_relative("after")).pack(
                    side="left", padx=(0, 6), pady=(0, 6)
                )
                palette_hint_var.set(
                    "Page blanche : clic puis + dans B, ou insertion directe avant/après la page sélectionnée."
                )

            elif name == "Types":
                small_action(row, "+ Nouveau type", create_custom_type).pack(
                    side="left", padx=(0, 8), pady=(0, 6)
                )
                custom = [(key, label) for key, label, is_custom in page_types() if is_custom]
                for type_key, label in custom:
                    make_brick(
                        row, label, "page", {"type": type_key, "label": label}, accent=True
                    ).pack(side="left", padx=(0, 6), pady=(0, 6))
                if not custom:
                    tk.Label(
                        row, text="Aucun type personnalisé.", bg=theme.PANEL_ALT,
                        fg=theme.MUTED_DARK, font=(theme.FONT_UI, 8),
                    ).pack(side="left", padx=4, pady=7)
                palette_hint_var.set(
                    "Le nouveau type devient immédiatement une brique réutilisable dans Pages."
                )
                ...
                tk.Label(
                    row, text="Recto / verso complet : étape suivante.",
                    bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                    font=(theme.FONT_UI, 8),
    ...
    def select_tab(self, key: str):
        if key not in self.tab_frames:
            return
        self.active_tab = key
        if hasattr(self, "workspace_current_var"):
            self.workspace_current_var.set(dict(theme.TAB_NAMES).get(key, key.title()).upper())
    ...
    def _refresh_workspace_state(self):
        ...
        if hasattr(self, "zone_a_type_icon"):
            photo = self._type_icon(self.context.project_type, 26)
            if photo is not None:
                self.zone_a_type_icon.configure(image=photo)
                self.zone_a_type_icon.image = photo
            else:
                self.zone_a_type_icon.configure(image="")
                self.zone_a_type_icon.image = None

        if hasattr(self, "workspace_current_var"):
            self.workspace_current_var.set(
                dict(theme.TAB_NAMES).get(self.active_tab, self.active_tab.title()).upper()
            )
