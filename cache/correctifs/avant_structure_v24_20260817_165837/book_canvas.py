...
class BookCanvas(tk.Frame):
    ...
    def _structure_action_button(self, mode: str):
        # V20_SELECTION_DIRECTE_COMMANDES
        # V21_LIGNE_STRUCTURE_IMMOBILE
        mode = str(mode or "")
        if mode not in {"duplicate", "delete"}:
            return

        # Deuxième clic sur la même commande : on confirme directement.
        if self._structure_action_mode == mode and self._structure_action_target_kind:
            self._structure_execute_action()
            return

        # La cible est désormais la sélection déjà visible dans B.
        # Sélection -> commande -> Confirmer. Aucun second clic sur la page.
    ...
    def _structure_page_auto_type(self, item: dict, position: str) -> str:
        key = "page_auto_before_type" if str(position) == "before" else "page_auto_after_type"
    ...
    def structure_set_selected_page_auto(self, position: str, target_type: str = "") -> bool:
        # Associe immédiatement un type automatique Avant ou Après à la page sélectionnée.
        ...
        if not self.structure_auto_source_allowed(item):
            self.status_var.set("Cette page ne peut pas recevoir une association automatique.")
            return False
        ...
        field = "page_auto_before_type" if position == "before" else "page_auto_after_type"
        if target:
            item[field] = target
        else:
            item.pop(field, None)
        ...
        for i, candidate in enumerate(self.items):
            if str(candidate.get("id") or "") == source_id:
                ...
                self._selected_group_id = self._item_group_id(candidate)
                self._structure_selection_kind = "page"
                break
        self.render()
        ...
        if target:
            definition = self._structure_type_definition(target)
            label = definition.get("short_label") or definition.get("label") or target.replace("_", " ").strip().capitalize()
            self.status_var.set(f"Page auto {side.lower()}  •  {label}")
        else:
            self.status_var.set(f"Page auto {side.lower()} retirée")
        return True
    ...
    def _structure_open_auto_menu(self, position: str, widget) -> None:
        ...
        if not self.structure_auto_source_allowed(source):
            self.status_var.set("Sélectionnez une page principale, pas une page automatique.")
            return
        ...
        current = self._structure_page_auto_type(source, position)
        menu = tk.Menu(
            self, tearoff=False,
            bg="#102633", fg="#F2F3F1",
        ...
        def choose(type_key: str):
            self._structure_last_auto_type = type_key
            self.structure_set_selected_page_auto(position, type_key)

        for type_key, label in options:
            visible = f"✓  {label}" if type_key == current else str(label)
            menu.add_command(label=visible, command=lambda key=type_key: choose(key))
        if current:
            menu.add_separator()
            menu.add_command(
                label="Retirer l'association",
                command=lambda: self.structure_set_selected_page_auto(position, ""),
            )
        try:
    ...
    def structure_duplicate_selected(self) -> bool:
        ...
        self._sync_page_auto_for_source(duplicate)
        self._save_order()
        self.render()
        return True
    ...
    def _on_press(self, event):
        if getattr(self, "_work_mode", "structure") == "structure" and getattr(self, "_structure_action_mode", None):
            # Un clic dans B pendant une confirmation annule la commande en attente,
            # puis ce même clic redevient une sélection normale.
            self._structure_reset_action()
        if getattr(self, "_work_mode", "structure") == "structure" and getattr(self, "_structure_pending_kind", None):
