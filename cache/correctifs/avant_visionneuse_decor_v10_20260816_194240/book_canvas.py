...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Commandes flottantes de la visionneuse. Elles utilisent la même
        # variable de zoom que les commandes de B, mais restent accessibles
        # lorsque la page recouvre A/B/C.
        overlay_controls = tk.Frame(
            self.page_overlay_frame,
            bg=theme.PANEL,
        ...
        overlay_controls.place(relx=0.5, rely=1.0, y=-18, anchor="s")

        self._tool_button(overlay_controls, "−", lambda: self.step_zoom(-10), width=2).pack(side="left", padx=(0, 4))
        self.overlay_zoom_scale = tk.Scale(
            overlay_controls,
            from_=100,
            to=500,
            orient="horizontal",
            variable=self.viewer_zoom_var,
            command=self._on_viewer_scale,
            showvalue=False,
            length=150,
            resolution=1,
            bg=theme.PANEL,
        ...
        self.overlay_zoom_scale.pack(side="left")
        self.overlay_zoom_scale.bind("<ButtonRelease-1>", self._overlay_zoom_release, add="+")
        self._tool_button(overlay_controls, "+", lambda: self.step_zoom(10), width=2).pack(side="left", padx=(4, 6))
        tk.Label(
            overlay_controls,
        ...
        self._tool_button(overlay_controls, "Page entière", self.fit_selected, width=10).pack(side="left", padx=(0, 4))
        self._tool_button(overlay_controls, "Fermer", self.close_page_overlay, width=6).pack(side="left")
        ...
        # Navigation et zoom forment un même poste de commande. L'ensemble est
        # centré dans la zone basse de B pour éviter les grands déplacements
        # de souris ; le zoom reste juste à droite du navigateur.
        nav_dock = tk.Frame(nav_row, bg=theme.WINDOW_DEEP)
        nav_dock.place(relx=0.52, rely=0.5, anchor="center")

        self.h_nav = TLBookNavigator(nav_dock, command=self._navigate_horizontal)
        self.h_nav.pack(side="left")

        tk.Frame(nav_dock, bg=theme.WINDOW_DEEP, width=18).pack(side="left")

        zoom_controls = tk.Frame(nav_dock, bg=theme.WINDOW_DEEP)
        zoom_controls.pack(side="left")
        self._tool_button(zoom_controls, "−", lambda: self.step_zoom(-10), width=2).pack(side="left", padx=(0, 3))
        ...
        self._tool_button(zoom_controls, "+", lambda: self.step_zoom(10), width=2).pack(side="left", padx=(3, 4))
        tk.Label(
            zoom_controls,
            textvariable=self.zoom_text_var,
            width=5,
        ...
        self._tool_button(zoom_controls, "Livre", self.close_page_overlay, width=5).pack(side="left", padx=(0, 3))
        self._tool_button(zoom_controls, "Page", self.fit_selected, width=5).pack(side="left")
        self.zoom_scale.bind("<ButtonRelease-1>", self._overlay_zoom_release, add="+")
    ...
    def _render_page_overlay(self):
        ...
        # Fond calme, sans nouvelle fenêtre : la page peut maintenant recouvrir
        # tout l'espace TomeLinea et non plus seulement B.
        ...
        # À 100–110 %, la page entière reste visible. Au-delà, le curseur main
        # indique qu'on peut déplacer la feuille sans bouger le livre dessous.
        if int(self.viewer_zoom_var.get()) > 112:
