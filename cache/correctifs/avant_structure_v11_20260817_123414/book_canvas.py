...
class BookCanvas(tk.Frame):
    ...
    def _structure_bar_button(self, parent, text, command, *, danger=False):
        """Bouton du bandeau Structure : commande, jamais brique."""
        return tk.Button(
    ...
    def _structure_update_action_buttons(self):
        ...
        if duplicate is not None and duplicate.winfo_exists():
            duplicate.configure(
                ...
                bg=theme.ACCENT_DARK if mode == "duplicate" else theme.PANEL_SOFT,
                fg=theme.WHITE if mode == "duplicate" else theme.INK,
            )
        if delete is not None and delete.winfo_exists():
            delete.configure(
                text="Confirmer" if mode == "delete" and target else "Supprimer",
                bg="#6A3029" if mode == "delete" else theme.PANEL_SOFT,
                fg=theme.WHITE if mode == "delete" else "#E4876E",
            )
    ...
    def _build_canvas(self):
        ...
        # Bandeau Structure : discret, immédiatement identifiable, placé entre
        # la navigation et la ligne. Il ne contient que les actions d'organisation.
        self.structure_command_bar = tk.Frame(
            viewer, bg=theme.PANEL_ALT, height=42,
            highlightthickness=1, highlightbackground=theme.ACCENT_DARK,
        )
        self.structure_command_bar.grid(row=1, column=0, sticky="ew", pady=(1, 1))
        self.structure_command_bar.grid_propagate(False)
        accent = tk.Frame(self.structure_command_bar, bg=theme.ACCENT_BRIGHT, width=3)
        accent.pack(side="left", fill="y")
        tk.Label(
            self.structure_command_bar, text="STRUCTURE",
            bg=theme.PANEL_ALT, fg=theme.ACCENT_BRIGHT,
            font=(theme.FONT_UI, 7, "bold"),
        ).pack(side="left", padx=(9, 12))

        self.structure_new_part_btn = self._structure_bar_button(
            self.structure_command_bar, "+ Nouvelle partie", self._structure_new_part_command
        )
        self.structure_new_part_btn.pack(side="left", padx=(0, 6), pady=6)
        ...
        self.structure_duplicate_btn.pack(side="left", padx=(0, 6), pady=6)
        ...
        self.structure_delete_btn.pack(side="left", padx=(0, 6), pady=6)

        tk.Label(
            self.structure_command_bar,
            text="Nom et position : directement sur la ligne",
