...
class BookCanvas(tk.Frame):
    ...
    def gabarit_associate_selected_zones(self) -> bool:
        ...
        if len(zones)<2:
            self.status_var.set("Associer : sélectionnez au moins 2 zones — Maj+clic ou Ctrl+clic pour ajouter une zone.")
            return False
    ...
    def gabarit_distribute_selected_zones(self, axis: str) -> bool:
        zones=self._gabarit_selected_zone_dicts()
        if len(zones) < 3:
            self.status_var.set("Distribuer : sélectionnez au moins 3 zones — Maj+clic ou Ctrl+clic pour ajouter des zones.")
            return False
        if not self._gabarit_multi_same_surface(zones):
    ...
    def _gabarit_press(self, event):
        ...
        for zid, box in reversed(list(self._gabarit_zone_hitboxes.items())):
            ...
            if inside_zone:
                ...
                additive = bool(state & 0x0001) or bool(state & 0x0004)  # Maj ou Ctrl
                if additive:
