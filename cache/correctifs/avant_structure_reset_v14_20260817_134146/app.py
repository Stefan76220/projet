...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V13 : atelier C visuel, briques sur deux lignes, règle auto simple source -> cible."""
        WORK_BG = "#162B37"
        WORK_BG_2 = "#132630"
        WORK_TILE = "#233B49"
        WORK_TILE_HOVER = "#2C4A5A"
        WORK_TILE_ACTIVE = "#294957"
        WORK_BORDER = "#3A5665"
        WORK_GOLD = "#C7A24E"
        WORK_TEAL = "#74B6AA"
        WORK_BLUE = "#7FA7BE"
        ...
        atelier.grid_rowconfigure(1, weight=1, minsize=108)
        ...
        def command_chip(parent_, text, command, *, accent=WORK_TEAL, danger=False, compact=False):
            ...
            lbl = tk.Label(
                ...
                font=(theme.FONT_UI, 8 if compact else 9, "bold"),
                padx=9 if compact else 11, pady=4 if compact else 5,
        ...
        tk.Label(
            head, text="choisir puis cliquer dans la ligne", bg=WORK_BG, fg=WORK_MUTED,
        ...
        def make_brick(parent_, label, type_key, *, custom=False):
            # Brique TomeLinea V13 : une seule forme, une seule matière, une seule couleur.
            # Seuls l'icône et le nom changent.
            width, height = 122, 42
            card = tk.Canvas(
                parent_, width=width, height=height, bg=WORK_BG_2,
                bd=0, highlightthickness=0, cursor="hand2",
            )
            display = str(label or "Sans type")
            cut = 7
            ...
            def draw(fill=WORK_TILE, outline=WORK_BORDER, active=False):
                card.delete("all")
                card.create_polygon(
                    *polygon_points(), fill=fill, outline=outline,
                    width=2 if active else 1,
                )
                card.create_rectangle(
                    8, 10, 29, 31,
                    fill=WORK_BG_2, outline=WORK_GOLD, width=1,
                )
                card.create_text(
                    18.5, 20.5, text=brick_icon(type_key), fill=WORK_GOLD,
                    font=(theme.FONT_UI, 9, "bold"),
                )
                # Le nom tient sur une ligne : pas de texte coupé ni de micro-écriture.
                card.create_text(
                    36, height/2, text=display, anchor="w",
                    fill=WORK_TEXT, font=(theme.FONT_UI, 8, "bold"),
                )
            ...
            def enter(_e=None):
                if active_brick.get("widget") is not card:
                    draw(WORK_TILE_HOVER, WORK_GOLD)
            def leave(_e=None):
                if active_brick.get("widget") is not card:
                    normal()
            def press(_e=None):
                draw("#1A303B", WORK_GOLD)
                return "break"
            def release(_e=None):
                reset_active_brick()
        ...
        def rebuild_catalog():
            ...
            cv = tk.Canvas(
                holder, bg=WORK_BG_2, bd=0, highlightthickness=0,
                yscrollincrement=48,
            )
            ...
            # Neuf briques par ligne : le catalogue courant occupe au moins
            # deux lignes. Les lignes supplémentaires défilent à la molette.
            columns = 9
            ...
            for key, label, custom in page_types():
                if key not in seen:
                    bricks.append((key, label, custom))
                    seen.add(key)
        ...
        # --------------------------------------------------------------
        # RÈGLES : page choisie dans B -> type associé -> portée -> Appliquer.
        # Une sélection seule ne crée rien et ne modifie rien.
        # --------------------------------------------------------------
        rules = tk.Frame(atelier, bg=WORK_BG, pady=3)
        rules.grid(row=2, column=0, sticky="ew", pady=(6, 0))
        rules.grid_columnconfigure(1, weight=1)

        tk.Label(
            rules, text="Règles générales", bg=WORK_BG, fg=WORK_GOLD,
            font=(theme.FONT_TITLE, 10, "bold"),
        ).grid(row=0, column=0, sticky="w", padx=(0, 14), pady=(2, 3))

        rule_area = tk.Frame(rules, bg=WORK_BG)
        rule_area.grid(row=0, column=1, sticky="ew")
        rule_area.grid_columnconfigure(10, weight=1)

        source_var = tk.StringVar(value="Sélectionnez une page dans B")
        target_var = tk.StringVar(value="Choisir…")
        info_var = tk.StringVar(value="")
        scope = {"value": "local"}
        target_key = {"value": None}
        scope_labels = {}
        detach_ref = {"widget": None}
        last_source = {"index": None}

        tk.Label(
            rule_area, text="Page choisie", bg=WORK_BG, fg=WORK_MUTED,
            font=(theme.FONT_UI, 9, "bold"),
        ).grid(row=0, column=0, sticky="w", padx=(0, 6))
        source_box = tk.Label(
            rule_area, textvariable=source_var,
            bg=WORK_BG_2, fg=WORK_TEXT,
            font=(theme.FONT_UI, 9, "bold"), padx=11, pady=6,
            highlightthickness=1, highlightbackground=WORK_BORDER,
            width=25, anchor="w",
        )
        source_box.grid(row=0, column=1, sticky="w", padx=(0, 10))

        tk.Label(
            rule_area, text="→", bg=WORK_BG, fg=WORK_GOLD,
            font=(theme.FONT_UI, 12, "bold"),
        ).grid(row=0, column=2, padx=(0, 10))

        tk.Label(
            rule_area, text="Page automatique", bg=WORK_BG, fg=WORK_MUTED,
            font=(theme.FONT_UI, 9, "bold"),
        ).grid(row=0, column=3, sticky="w", padx=(0, 6))

        target_menu = tk.Menu(rule_area, tearoff=False)
        target_mb = tk.Menubutton(
            rule_area, textvariable=target_var, menu=target_menu,
        ...
        target_mb.grid(row=0, column=4, sticky="w", padx=(0, 12))

        tk.Label(
            rule_area, text="Appliquer à", bg=WORK_BG, fg=WORK_MUTED,
            font=(theme.FONT_UI, 9, "bold"),
        ...
        def choose_target(key, label):
            ...
            target_var.set(str(label or "Aucune"))

        def rebuild_target_menu():
            target_menu.delete(0, "end")
            target_menu.add_command(
                label="Aucune page automatique",
                command=lambda: choose_target("", "Aucune"),
            )
            ...
            for key, label in options:
                target_menu.add_command(
                    label=label,
                    command=lambda k=key, l=label: choose_target(k, l),
                )
        ...
        def set_scope(value):
            ...
            for key, lbl in scope_labels.items():
                active = key == value
                lbl.configure(
                    bg=WORK_TILE_ACTIVE if active else WORK_TILE,
                    fg=WORK_TEXT,
                    highlightbackground=WORK_GOLD if active else WORK_BORDER,
                )
        ...
        def scope_chip(value, text):
            lbl = tk.Label(
                rule_area, text=text,
                bg=WORK_TILE, fg=WORK_TEXT,
                font=(theme.FONT_UI, 9, "bold"), padx=10, pady=6,
                highlightthickness=1, highlightbackground=WORK_BORDER,
                cursor="hand2",
            )
        ...
        scope_chip("local", "Cette page").grid(row=0, column=6, padx=(0, 5))
        scope_chip("general", "Toutes les pages de ce type").grid(row=0, column=7, padx=(0, 9))

        def refresh_info(ctx=None):
            if ctx is None:
            ...
            if not ctx.get("allowed"):
                info_var.set("Cette page ne peut pas déclencher de page automatique.")
                return
            labels = labels_by_key()
            source_name = str(ctx.get("page_label") or ctx.get("source_type") or "Page")
            general = str(ctx.get("general_target") or "")
            mode = str(ctx.get("mode") or "inherit")
            if general:
                if mode == "none":
                    info_var.set(f"{source_name} suit normalement : {labels.get(general, general)}  •  cette page est désolidarisée")
                elif mode == "local":
                    local = str(ctx.get("local_target") or "")
                    info_var.set(f"Règle du type : {labels.get(general, general)}  •  cette page : {labels.get(local, local)}")
                else:
                    info_var.set(f"Règle du type : {source_name} → {labels.get(general, general)}")
            elif mode == "local":
                local = str(ctx.get("local_target") or "")
                info_var.set(f"Cette page → {labels.get(local, local)}")
            else:
                info_var.set("Aucune règle appliquée.")

        def refresh_detach(ctx=None):
            if ctx is None:
                ctx = selected_context()
            widget = detach_ref.get("widget")
            if widget is None:
                return
            general = str(ctx.get("general_target") or "") if ctx.get("selected") else ""
            mode = str(ctx.get("mode") or "inherit")
            if not ctx.get("selected") or not ctx.get("allowed") or not general:
                widget.grid_remove()
                return
            widget.configure(text="Suivre la règle générale" if mode in {"none", "local"} else "Désolidariser cette page")
            widget.grid()
        ...
        def refresh_rules():
            ...
            ctx = selected_context()
            if not ctx.get("selected"):
                source_var.set("Sélectionnez une page dans B")
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = None
                target_var.set("Choisir…")
                scope_labels["general"].configure(text="Toutes les pages de ce type")
                refresh_info(ctx)
                refresh_detach(ctx)
                return

            if not ctx.get("allowed"):
                source_var.set(str(ctx.get("page_label") or "Page protégée"))
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = None
                target_var.set("Non applicable")
                refresh_info(ctx)
                refresh_detach(ctx)
                return
            ...
            index = ctx.get("index")
            source_name = str(ctx.get("page_label") or ctx.get("source_type") or "Page")
            source_var.set(source_name)
            target_mb.configure(state="normal", cursor="hand2")
            scope_labels["general"].configure(text=f"Toutes les « {source_name} »")

            # Le clic dans B renseigne seulement l'interface. Rien n'est enregistré ici.
            if last_source["index"] != index:
                last_source["index"] = index
                set_scope("local")
                labels = labels_by_key()
                mode = str(ctx.get("mode") or "inherit")
                if mode == "local":
                    current = str(ctx.get("local_target") or "")
                    target_key["value"] = current
                    target_var.set(labels.get(current, current) if current else "Aucune")
                elif mode == "none":
                    target_key["value"] = ""
                    target_var.set("Aucune")
                else:
                    current = str(ctx.get("general_target") or "")
                    target_key["value"] = current if current else None
                    target_var.set(labels.get(current, current) if current else "Choisir…")
        ...
        def apply_rule():
            ...
            if canvas is None or not ctx.get("selected") or not ctx.get("allowed"):
                return
            if target_key["value"] is None:
                try:
                    canvas.status_var.set("Choisissez la page automatique à associer.")
                except Exception:
                    pass
                return
        ...
        def detach_or_rejoin():
            canvas = book()
            ctx = selected_context()
            if canvas is None or not ctx.get("selected") or not ctx.get("allowed"):
                return
            if not str(ctx.get("general_target") or ""):
                return
            if str(ctx.get("mode") or "inherit") in {"none", "local"}:
        ...
        command_chip(
            rule_area, "Appliquer", apply_rule,
            accent=WORK_GOLD, compact=False,
        ).grid(row=0, column=8, padx=(0, 8))

        detach_ref["widget"] = command_chip(
            rule_area, "Désolidariser cette page", detach_or_rejoin,
            accent=WORK_BLUE, compact=True,
        )
        detach_ref["widget"].grid(row=1, column=6, columnspan=3, sticky="w", pady=(6, 0))

        tk.Label(
            rule_area, textvariable=info_var,
            bg=WORK_BG, fg=WORK_MUTED,
