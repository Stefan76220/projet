...
class PageMaitreV2(tk.Tk):
    ...
    def _tomelinea_open_page_type_window(self, group_name="Partie"):
        ...
        # Même cycle d'apparition atomique déjà validé.
        # Le fond est celui du Maquettage, pas celui de l'Accueil.
        self._prepare_accueil_dialog(win)
        ...
        try:
            if bg_path.exists():
                ...
                win._tomelinea_type_bg_photo = ImageTk.PhotoImage(bg_image)
                canvas.create_image(
                    0,
                    0,
        ...
        # Titre posé directement sur le fond.
        canvas.create_text(
            34,
            27,
        ...
        # Simple séparation fonctionnelle : pas de grand cadre dans un cadre.
        canvas.create_line(
            390,
            102,
            390,
        ...
        # Bibliothèque standard provisoire : noms + images déjà présentes.
        ...
        tk.Label(
            ...
            text=(
                ...
                "uniquement pour le type choisi."
            ),
            bg=base_bg,
            fg=muted,
            font=("Segoe UI", 8),
        ...
        list_row = tk.Frame(left, bg=base_bg)
        ...
        preview_row = tk.Frame(left, bg=base_bg)
        ...
        preview_box.pack(side="left", fill="y")
        ...
        win._tomelinea_existing_type_photo = None

        use_existing_button = self._accueil_make_dialog_button(
            selected_info,
        ...
        use_existing_button.configure(state="disabled")
        use_existing_button.pack(anchor="w", pady=(18, 0))

        def _load_preview(path_text, target_label, attr_name, empty_text):
            photo = None
        ...
        use_existing_button.configure(command=_use_existing)
        ...
        def _select_existing(_event=None):
            ...
            if image_path and Path(image_path).exists():
                ...
                empty_text = "Aucune image\nassociée"

            _load_preview(
                image_path,
                preview_label,
                "_tomelinea_existing_type_photo",
                empty_text,
            )

            use_existing_button.configure(state="normal")
        ...
        image_row = tk.Frame(right, bg=base_bg)
        image_row.pack(fill="x", pady=(5, 9))

        new_preview_box = tk.Frame(
            image_row,
            bg="#FFFEFC",
            width=92,
            height=124,
        ...
        new_preview_box.pack(side="left")
        new_preview_box.pack_propagate(False)

        new_preview_label = tk.Label(
            new_preview_box,
            bg="#FFFEFC",
            fg="#8B949C",
            text="Aucune\nimage",
        ...
        new_preview_label.pack(fill="both", expand=True, padx=4, pady=4)
        ...
        win._tomelinea_new_type_photo = None
        ...
        def _choose_image():
            ...
            _load_preview(
                path,
                new_preview_label,
                "_tomelinea_new_type_photo",
                "Image\nillisible",
            )

        choose_image_button = self._accueil_make_dialog_button(
            image_tools,
        ...
        create_button = self._accueil_make_dialog_button(
            right,
            text="Créer le type   ›",
            command=lambda: None,
            primary=True,
        ...
        create_button.configure(command=_create_type)

        def _update_create_state(*_args):
            create_button.configure(
                state="normal" if name_var.get().strip() else "disabled"
            )
        ...
        close_button = self._accueil_make_dialog_button(
            close_holder,
            text="Fermer",
            command=win.destroy,
            primary=False,
