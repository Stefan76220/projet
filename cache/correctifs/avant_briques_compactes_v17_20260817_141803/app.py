...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        ...
        def make_brick(parent_, label, type_key, *, custom=False):
            # Même encombrement que V11 ; seul le langage visuel change.
            width, height = 122, 42
            card = tk.Canvas(
            ...
            def draw(fill=WORK_TILE, outline=WORK_BORDER, active=False):
                card.delete("all")
                # Corps uniforme : rectangle sombre, filet bleu discret.
                card.create_rectangle(
                    1, 1, width-1, height-1,
                    fill=fill, outline=outline, width=2 if active else 1,
                )
                # Mini-icône dans son logement carré or, comme la référence.
                card.create_rectangle(
                    8, 10, 29, 31,
