...
class BookCanvas(tk.Frame):
    ...
    def _ensure_minimum_structure(self, source: dict) -> tuple[dict, bool]:
        ...
        for item in items:
            # B affiche deux informations distinctes pour chaque page :
            # une famille de page et son attribut éditorial. Les anciens projets
            # sont enrichis sans supprimer leurs champs historiques.
            if not str(item.get("page_family", "")).strip():
                if self._is_cover(item) or self._is_second_cover(item) or self._is_third_cover(item) or self._is_back_cover(item):
                    item["page_family"] = "Couverture"
                else:
                    item["page_family"] = "Page intérieure"
                changed = True
    ...
    def _page_type_label(self, item: dict) -> str:
        for key in ("page_family", "page_type_label", "family", "famille"):
            value = item.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
        if self._is_cover(item) or self._is_second_cover(item) or self._is_third_cover(item) or self._is_back_cover(item):
            return "Couverture"
        return "Page intérieure"

    def _page_attribute_label(self, item: dict, index: int) -> str:
        for key in ("attribute", "attribut", "role", "fonction"):
            value = item.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
        return self._legacy_page_attribute(item) or self._item_label(item, index)
    ...
    def _draw_page(self, index: int, x: float, y: float, page_w: float, page_h: float, scale: float):
        ...
        font_size = max(6, min(22, int(7 + self.zoom / 50)))
        attribute_size = max(6, min(20, int(6 + self.zoom / 58)))
        number_size = max(6, min(22, int(7 + self.zoom / 55)))
        page_type = self._page_type_label(item)
        attribute = self._page_attribute_label(item, index)
        if page_w >= 55 and page_h >= 80:
            center_y = y + page_h / 2
            self.canvas.create_text(
                x + page_w / 2,
                center_y - max(5, 14 * scale),
                text=page_type,
                fill="#303740",
            ...
            self.canvas.create_text(
                x + page_w / 2,
                center_y + max(6, 15 * scale),
                text=attribute,
                fill=theme.ACCENT_DARK if not self._is_locked_page(item) else "#7A5C28",
                width=max(30, page_w - max(18, 32 * scale)),
                font=(theme.FONT_UI, attribute_size, "italic"),
                justify="center",
            )
        if page_w >= 34:
            self.canvas.create_text(
    ...
    def render(self):
        ...
        if self._page_focus:
            ...
            for group_pos, group in enumerate(self.groups):
                ...
                end_x = x
                min_header = max(138, min(250, 160 + max(len(self._group_name(group)), len(self._group_part_title(group))) * 2))
