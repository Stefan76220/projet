...
class BookCanvas(tk.Frame):
    ...
    def set_work_mode(self, mode: str):
        ...
        if zoom_dock is not None:
            if mode == "gabarits":
                # Poste discret à gauche de B. Comme vrai widget, il reste toujours
                # cliquable, indépendamment des rendus/zoom du Canvas.
                zoom_dock.place(x=12, rely=0.52, anchor="w")
                try:
                    zoom_dock.lift()
                except Exception:
                    pass
    ...
    def _gabarit_view_center(self) -> tuple[float, float]:
        """Centre réel de la surface B utilisée par la page."""
        ...
        return width / 2.0, (work_top + work_bottom) / 2.0
    ...
    def _gabarit_draw_context_overlay(self, units: list[dict], active_pos: int, viewport_w: float) -> None:
        """Ligne de contexte transparente, toujours au-dessus de la page agrandie."""
        self._gabarit_context_hitboxes = {}
        positions = [viewport_w * ratio for ratio in (0.17, 0.335, 0.50, 0.665, 0.83)]
    ...
    def gabarit_current_settings(self) -> dict:
        ...
        for key in ("margins_mm", "bleed_mm"):
            ...
            if isinstance(source, dict):
                result[key].update({k: float(v) for k, v in source.items() if k in result[key]})
    ...
    def gabarit_set_margins(self, top: float, bottom: float, inside: float, outside: float) -> bool:
        ...
        if any(v < 0 for v in values.values()) or values["top"] + values["bottom"] >= 297 or values["inside"] + values["outside"] >= 210:
            return False
        return self._gabarit_apply_settings_patch({"margins_mm": values}, "Marges mises à jour")
    ...
    def gabarit_set_bleed(self, top: float, right: float, bottom: float, left: float) -> bool:
        ...
        if any(v < 0 or v > 30 for v in values.values()):
            return False
        return self._gabarit_apply_settings_patch({"bleed_mm": values}, "Fond perdu mis à jour")
    ...
    def _gabarit_fit_scale(self, viewport_w: float, viewport_h: float) -> float:
        ...
        # La page doit rester centrée dans B. On réserve donc la même largeur
        # à gauche qu'à droite : zoom à gauche, fiche de page à droite.
        info_w = 220.0 if viewport_w >= 1000 else 178.0
        side_reserve = info_w + 26.0
        top_h = 82.0
        bottom_h = 16.0
        usable_w = max(180.0, viewport_w - side_reserve * 2.0 - 24.0)
        ...
        if isinstance(raw, dict):
            for key in ("margins_mm", "bleed_mm"):
                if isinstance(raw.get(key), dict):
                    settings[key].update(raw[key])
            for key in ("show_guides", "background"):
                if key in raw:
                    settings[key] = raw[key]
        bg = str(settings.get("background") or "#F1F1EE")
        target.create_rectangle(x, y, x + w, y + h, fill=bg, outline="#B6B9B5", width=1)
        if bool(settings.get("show_guides", True)):
            bleed = settings["bleed_mm"]
            margins = settings["margins_mm"]
            bx1 = x + max(2.0, w * float(bleed.get("left", 3.0)) / 210.0)
            bx2 = x + w - max(2.0, w * float(bleed.get("right", 3.0)) / 210.0)
            by1 = y + max(2.0, h * float(bleed.get("top", 3.0)) / 297.0)
            by2 = y + h - max(2.0, h * float(bleed.get("bottom", 3.0)) / 297.0)
            target.create_rectangle(bx1, by1, bx2, by2, outline="#C97B93", dash=(4, 3), width=1)
            ...
            target.create_rectangle(mx1, my1, mx2, my2, outline="#7FA2C4", dash=(5, 3), width=1)
            safe = max(4.0, min(14.0, w * 0.025))
            target.create_rectangle(mx1 + safe, my1 + safe, mx2 - safe, my2 - safe, outline="#8BAB93", dash=(5, 3), width=1)
        if fold_side == "right":
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float) -> None:
        ...
        for zone in zones:
            ...
            self.canvas.create_rectangle(zx1, zy1, zx2, zy2, outline=color, width=3 if selected else 2, dash=() if selected else (5, 3))
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        info_w = 220.0 if viewport_w >= 1000 else 178.0
        side_reserve = info_w + 26.0
        work_left = side_reserve
        work_right = viewport_w - side_reserve
        work_top = 88.0
        work_bottom = viewport_h - 14.0
        # La page reste au centre géométrique de B, indépendamment de la fiche à droite.
        work_center_x = viewport_w / 2.0
        ...
        info_x1_for_arrows = viewport_w - info_w
        ...
        left_ok = vertical_ok and left_cx - arrow_half >= 74.0 and left_cx + arrow_half <= viewport_w - 8.0
        right_ok = vertical_ok and right_cx - arrow_half >= 8.0 and right_cx + arrow_half <= info_x1_for_arrows - 8.0
        ...
        # ----- fiche contextuelle de la page active -----
        info_x1 = viewport_w - info_w
        info_y1 = work_top + 10
        info_x2 = viewport_w - 10
        info_y2 = min(work_bottom - 8, info_y1 + 260)
        self.canvas.create_rectangle(info_x1, info_y1, info_x2, info_y2, fill="#26313B", outline="#46545D")
        tx = info_x1 + 16
        self.canvas.create_text(tx, info_y1 + 24, text="PAGE ACTIVE", anchor="w", fill="#9DB0B5", font=(theme.FONT_UI, 7, "bold"))
        self.canvas.create_text(tx, info_y1 + 47, text=f"{number} — {active_type}", anchor="w", fill="#F0F2F0", font=(theme.FONT_TITLE, 11, "bold"), width=max(120, info_w - 32))
        self.canvas.create_line(tx, info_y1 + 64, info_x2 - 16, info_y1 + 64, fill="#4F817A")
        self.canvas.create_text(tx, info_y1 + 87, text="PORTÉE", anchor="w", fill="#9DB0B5", font=(theme.FONT_UI, 7, "bold"))
        page_scope = str(getattr(self, "_gabarit_scope", "page")) != "type"
        type_scope = not page_scope
        self.canvas.create_text(tx + 4, info_y1 + 109, text="Cette page", anchor="w", fill="#FFFFFF" if page_scope else "#9BA5A8", font=(theme.FONT_UI, 8, "bold" if page_scope else "normal"))
        self.canvas.create_text(tx + 4, info_y1 + 131, text="Toutes les pages de ce type", anchor="w", fill="#FFFFFF" if type_scope else "#9BA5A8", font=(theme.FONT_UI, 8, "bold" if type_scope else "normal"), width=max(130, info_w - 34))
        self._gabarit_hitboxes["scope_page"] = (tx, info_y1 + 96, info_x2 - 14, info_y1 + 118)
        self._gabarit_hitboxes["scope_type"] = (tx, info_y1 + 119, info_x2 - 14, info_y1 + 148)
        ...
        self.canvas.create_text(tx, info_y1 + 166, text="STATUT DU GABARIT", anchor="w", fill="#9DB0B5", font=(theme.FONT_UI, 7, "bold"))
        self.canvas.create_oval(tx, info_y1 + 184, tx + 10, info_y1 + 194, fill=status_color, outline="")
        self.canvas.create_text(tx + 18, info_y1 + 189, text=status_text, anchor="w", fill="#E4E8E6", font=(theme.FONT_UI, 8, "bold"))
        self._gabarit_hitboxes["toggle_status"] = (tx, info_y1 + 174, info_x2 - 14, info_y1 + 202)
        self.canvas.create_text(tx, info_y1 + 220, text="PARTIE", anchor="w", fill="#9DB0B5", font=(theme.FONT_UI, 7, "bold"))
        self.canvas.create_text(tx, info_y1 + 240, text=part_text, anchor="w", fill="#C9D0CE", font=(theme.FONT_UI, 8), width=max(120, info_w - 32))

        # Toujours en dernier : la ligne reste lisible par-dessus une page très agrandie.
        self._gabarit_draw_context_overlay(units, active_pos, viewport_w)
        self._gabarit_sync_zoom_widget()
    ...
    def _on_hover_motion(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            x, y = float(event.x), float(event.y)
            hovered = ""
            for key in ("prev", "next", "scope_page", "scope_type", "toggle_status"):
