    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton de commande : relief verre + peau papier perlée discrète."""
            ...
            shadow = self._mix(
                color,
                "#56615A",
                0.26 if state == "hover" else 0.32,
            )
            rim = self._mix(
                color,
                "#F4EEE5",
                0.28 if state != "pressed" else 0.38,
            )
            face_mix = {
                "normal": 0.78,
                "hover": 0.72,
            ...
            face = self._mix(color, "#FFF9F2", face_mix)
            inner = self._mix(color, "#FFFDFC", 0.90)
            shine = self._mix(color, "#FFFFFF", 0.97)
            pearl = self._mix(color, "#FFFFFF", 0.78)
            grain = self._mix(color, "#E9E0D5", 0.76)
            ink = self._mix(color, "#273229", 0.54)
            ...
            # Deux reflets perlés courts, volontairement hors du texte.
            canvas.create_line(
                x2 - 24, y1 + 8,
                x2 - 16, y1 + 13,
                fill=pearl,
            ...
            canvas.create_line(
                x2 - 30, y1 + 7,
                x2 - 24, y1 + 11,
                fill=self._mix(pearl, "#FFFFFF", 0.45),
                width=1,
                tags=(tag, "home_button"),
            )

            # Grain papier très localisé dans les marges.
            for gx, gy in (
                (x1 + 10, y1 + 11),
                (x1 + 14, y2 - 10),
                (x2 - 12, y2 - 9),
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Carte de choix : verre-papier perlé, ton sur ton, sans aplat massif."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            # Garde la couleur visible, mais la dilue dans un ivoire chaud.
            base_mix = 0.82
            if hover:
                base_mix = 0.77
            if pressed:
            ...
            face = self._mix(color, "#FFF9F1", max(0.68, base_mix))
            shadow = self._mix(color, "#777168", 0.63 if hover else 0.69)
            outline = (
                self._mix(color, "#FFF7EE", 0.18)
                if selected
                else self._mix(color, "#FFF7EE", 0.42)
            )
            inner = self._mix(color, "#FFFFFF", 0.80)
            satin = self._mix(color, "#FFFFFF", 0.96)
            pearl = self._mix(color, "#FFFFFF", 0.72)
            grain = self._mix(color, "#E8DED1", 0.73)
            ...
            # Reflets perlés dans le coin supérieur droit.
            canvas.create_line(
                x2 - 32, y1 + 13,
                x2 - 20, y1 + 21,
                fill=pearl,
            ...
            # Fibres et micro-grain placés uniquement dans les marges.
            # Rien ne traverse les zones de texte.
            fibers = (
                (x1 + 13, y1 + 31, x1 + 31, y1 + 28),
                (x1 + 14, y1 + 43, x1 + 28, y1 + 46),
                (x2 - 31, y2 - 28, x2 - 15, y2 - 31),
                (x2 - 27, y2 - 18, x2 - 14, y2 - 16),
            )
            for xa, ya, xb, yb in fibers:
                canvas.create_line(
                    xa, ya, xb, yb,
                    fill=grain,
                    width=1,
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule informative ton sur ton avec texture verre-papier."""
            width = max(64, int(len(text) * 5.0) + 31)
            height = 20

            # La couleur du texte devient aussi la couleur de matière.
            face = self._mix(fg, "#FFF9F2", 0.88)
            outline = self._mix(fg, "#FFF7EE", 0.44)
            inner = self._mix(fg, "#FFFFFF", 0.83)
            pearl = self._mix(fg, "#FFFFFF", 0.72)
            grain = self._mix(fg, "#EEE5DA", 0.74)
            ...
            # Reflet supérieur et petite perle.
            canvas.create_line(
                x + 13, y + 3,
                x + width - 11, y + 3,
                fill=self._mix(face, "#FFFFFF", 0.76),
                width=1,
                tags=tag,
            )
            canvas.create_oval(
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Raccourci : bouton verre-papier coloré, sans aplat massif."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FFF9F2", face_mix)
            shadow = self._mix(color, "#777168", 0.64 if state == "hover" else 0.69)
            outline = self._mix(color, "#FFF7EE", 0.34)
            rim = self._mix(color, "#FFFFFF", 0.78)
            pearl = self._mix(color, "#FFFFFF", 0.72)
            grain = self._mix(color, "#E8DED1", 0.73)
            ...
            # Reflet perlé dans le coin droit.
            canvas.create_line(
                x2 - 27, y1 + 10,
                x2 - 18, y1 + 16,
                fill=pearl,
            ...
            # Grain localisé dans les marges.
            for gx, gy in (
                (x1 + 9, y1 + 20),
                (x1 + 11, y2 - 13),
                (x2 - 12, y2 - 11),
