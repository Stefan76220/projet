...
class PageMaitreV2(tk.Tk):
    ...
    def _tomelinea_open_page_type_window(self, group_name="Partie"):
        """Fenêtre Maquettage — style TomeLinea, logique métier inchangée."""
        win = tk.Toplevel(self)
        win.title("TomeLinea — Créer / choisir un type")

        # Même cycle d'apparition atomique que les fenêtres validées de l'Accueil.
        # On réutilise le mécanisme existant sans modifier son architecture.
        self._prepare_accueil_dialog(win)
        self._accueil_center_base_window(win, 840, 520)

        body = self._accueil_base_shell(
            win,
            title="Créer / choisir un type de page",
            subtitle=f"Partie concernée : {group_name}",
            accent="#8564B1",
        )
        ...
        use_button = self._accueil_make_dialog_button(
            actions,
            text="Utiliser / créer   ›",
            command=lambda: None,
            primary=True,
        ...
        use_button.configure(
            ...
            bg="#DDE2E7",
            fg="#89939D",
            disabledforeground="#89939D",
            activebackground="#DDE2E7",
            relief="flat",
        )
        use_button.pack(side="right", padx=(0, 10))

        content = tk.Frame(body, bg="#FFFEFC")
        content.pack(fill="both", expand=True)

        # ------------------------------------------------------
        # COLONNE GAUCHE — type existant
        # ------------------------------------------------------
        existing = tk.Frame(
            content,
            bg="#FAF8F4",
        ...
        existing.pack(side="left", fill="y", padx=(0, 14))
        ...
        existing_body = tk.Frame(
            existing,
            bg="#FAF8F4",
            padx=15,
            pady=14,
        )
        existing_body.pack(side="left", fill="both", expand=True)
        ...
        tk.Label(
            existing_body,
            text="Réutilisez un type déjà défini dans ce projet.",
            bg="#FAF8F4",
            fg="#68717A",
            font=("Segoe UI", 8),
            justify="left",
        ...
        type_list.insert("end", "Aucun type défini pour le moment")
        type_list.configure(state="disabled")

        # ------------------------------------------------------
        # COLONNE DROITE — création d'un type
        # ------------------------------------------------------
        create = tk.Frame(
            content,
            bg="#FAF8F4",
        ...
        tk.Label(
            create,
            text="Définissez les informations qui identifieront ce type de page.",
            bg="#FAF8F4",
            fg="#68717A",
            font=("Segoe UI", 8),
            anchor="w",
        ...
        field_label("Nom du type").pack(fill="x")
        name_entry = entry_field()
        name_entry.pack(fill="x", ipady=5, pady=(4, 10))

        field_label("Nom court").pack(fill="x")
        short_entry = entry_field()
        short_entry.pack(fill="x", ipady=5, pady=(4, 10))
        ...
        field_label("Description").pack(fill="x")
        description = tk.Text(
            create,
            height=5,
            wrap="word",
        ...
        description.pack(fill="both", expand=True, pady=(4, 0))

        # Aucun branchement métier n'est ajouté : le bouton principal reste
        # désactivé comme dans la fenêtre de base actuelle.
        self._reveal_accueil_dialog(win)
        return win
