# TOMELINEA_GABARITS_ICONES_PREMIUM_V86 — 14 icônes premium + bouton station commun dans les déroulés
...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_build_premium_icon_image(self, kind: str, state: str, size: int):
        ...
        if kind == "settings":
            ...
            self._gabarit_premium_metal_rect(image,(24,24,72,34),4); self._gabarit_premium_metal_rect(image,(44,31,54,70),4)
            d=ImageDraw.Draw(image); d.line((30,75,66,75),fill=accent,width=4); d.ellipse((65,72,71,78),fill=gold)
        elif kind == "image":
            self._gabarit_premium_metal_rect(image,(20,22,76,72),7); d=ImageDraw.Draw(image)
            d.rounded_rectangle((25,27,71,67),radius=5,outline=accent,width=3)
            d.polygon([(30,62),(43,46),(52,55),(61,42),(69,62)],fill="#7D868B",outline="#D4DBDD")
            d.ellipse((55,32,64,41),fill=gold)
        elif kind == "document":
            self._gabarit_premium_metal_rect(image,(25,18,70,77),6); d=ImageDraw.Draw(image)
            d.polygon([(56,19),(69,19),(69,33)],fill=gold); d.line((33,39,59,39),fill=accent,width=4)
            for yy,ll in ((49,27),(58,23),(67,18)):
                d.line((33,yy,33+ll,yy),fill="#AEB7BB",width=3)
        elif kind in {"undo","redo"}:
            box=(23,24,72,72); start=35 if kind=="redo" else 145; extent=250
            d.arc(box,start=start,end=start+extent,fill="#2B3338",width=10)
            d.arc(box,start=start,end=start+extent,fill=silver,width=6)
            d.arc(box,start=start+145,end=start+220,fill=accent,width=3)
            if kind=="undo": d.polygon([(26,34),(14,43),(29,48)],fill=silver)
            else: d.polygon([(70,34),(82,43),(67,48)],fill=silver)
        elif kind == "trash":
            ...
            self._gabarit_premium_metal_rect(image,(22,22,74,67),6); d=ImageDraw.Draw(image)
            d.rounded_rectangle((27,27,69,62),radius=4,outline=accent,width=2)
            for xx in range(23,77,10): d.line((xx,74,min(xx+5,77),74),fill=accent,width=3)
            d.ellipse((58,31,66,39),fill=gold)
        elif kind == "frame":
            self._gabarit_premium_metal_rect(image,(23,20,73,76),6); d=ImageDraw.Draw(image)
            d.rounded_rectangle((30,27,66,69),radius=4,outline=accent,width=3)
            d.rounded_rectangle((17,14,79,82),radius=8,outline=coral,width=2)
            d.ellipse((44,11,52,19),fill=gold)
        elif kind == "snap":
    ...
    def _gabarit_inspector_draw_premium_icon(self, target, kind: str, box, *, selected=False, disabled=False, hovered=False):
        ...
        size=max(18,min(44,int(round(min(x2-x1,y2-y1)))))
    ...
    def _gabarit_build_station_button_image(self, state: str, size: int):
        if Image is None or ImageDraw is None:
            return None
        S=96; image=Image.new("RGBA",(S,S),(0,0,0,0)); d=ImageDraw.Draw(image)
        accent="#43B8B5" if state in {"hover","active"} else "#6A9295"
        if state == "danger": accent="#C97669"
        d.ellipse((17,20,83,86),fill=(0,0,0,78))
        d.ellipse((15,15,81,81),fill="#11171B",outline="#56646B",width=3)
        d.ellipse((20,20,76,76),fill="#20292E",outline="#2D383E",width=2)
        d.ellipse((25,25,71,71),fill="#151C20",outline=accent,width=2 if state!="normal" else 1)
        d.line((34,48,62,48),fill=accent,width=4)
        center="#D5AD63" if state=="active" else ("#C97669" if state=="danger" else ("#82B8B5" if state=="hover" else "#899699"))
        d.ellipse((44,44,52,52),fill=center)
        if state in {"hover","active","danger"}:
            d.ellipse((13,13,83,83),outline=accent,width=2 if state=="hover" else 3)
        if state=="disabled":
            image=Image.alpha_composite(image,Image.new("RGBA",(S,S),(13,18,21,120)))
        try:
            return image.resize((max(20,int(size)),max(20,int(size))),Image.Resampling.LANCZOS)
        except Exception:
            return image.resize((max(20,int(size)),max(20,int(size))))
    ...
    def _gabarit_inspector_station_button(self, target, key: str, box, label: str, *, selected=False, danger=False, disabled=False, tooltip=""):
        ...
        target.create_text(cx,y1,text=str(label),anchor="n",fill=label_color,font=(theme.FONT_UI,6,"bold" if selected else "normal"))
        button_size=max(22,min(32,int(round(min(x2-x1,(y2-y1)-10.0)))))
        cy=y2-button_size/2.0
        photo=self._gabarit_station_button_photo(state,button_size)
        if photo is not None and hasattr(target,"create_image"):
            target.create_image(cx,cy,image=photo)
        else:
            r=button_size/2.0
            outline="#51BEB5" if state in {"hover","active"} else ("#C97669" if state=="danger" else "#526168")
            target.create_oval(cx-r,cy-r,cx+r,cy+r,fill="#172027",outline=outline,width=2)
            target.create_line(cx-r*0.42,cy,cx+r*0.42,cy,fill=outline,width=2)
        if not disabled:
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        def icon_strip(items, *, button=41.0, gap=8.0, y_offset=0.0, center=True, start_x=None):
            # V86 — dans les déroulés, plus de dictionnaire d'icônes :
            # le nom de la fonction est au-dessus et le même bouton-station
            # premium est répété sous chaque fonction.
            ...
            for i,item in enumerate(items):
                ...
                self._gabarit_inspector_station_button(target,key,(x1,yy,x1+button,yy+button+9.0),label,selected=selected,danger=danger,disabled=disabled,tooltip=tip)
        ...
        if active_band == "allowed":
            ...
            if pending in {"margins","page","bleed"} and pending != current:
                yy=content_top+50.0; gap=16.0; button=46.0; x1=fly_left+11.0
                self._gabarit_inspector_station_button(target,"align_scope_page",(x1,yy,x1+button,yy+43.0),"Cette page",tooltip="Appliquer ce cadre à cette page")
                x2=x1+button+gap
                self._gabarit_inspector_station_button(target,"align_scope_type",(x2,yy,x2+button,yy+43.0),"Tout le type",tooltip="Appliquer ce cadre à toutes les pages du type")
            ...
            pending_scope=str(getattr(self,"_gabarit_scope_pending","") or "")
            yy=content_top+3.0; button=54.0; gap=18.0; x1=fly_left+11.0
            ...
            if count==1 and bool(info.get("spread")):
                yy=content_top+47.0; current=str(info.get("spread_position") or "left"); button=47.0; gap=8.0; sx=fly_left+11.0
