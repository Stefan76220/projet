...
class BookCanvas(tk.Frame):
    ...
    def _structure_bar_button(self, parent, text, command, *, danger=False):
        """Outil Structure V13 : même matière bleu-nuit/or que l’atelier C."""
        base = "#162B37"
        hover = "#233E4B"
        pressed = "#10232D"
        edge = "#C7A24E"
        fg = "#F3DEDA" if danger else "#EEF2F1"
        label = tk.Label(
            parent, text=text, bg=base, fg=fg,
            font=(theme.FONT_UI, 9, "bold"), padx=13, pady=5,
            highlightthickness=1, highlightbackground=edge,
            cursor="hand2",
        )
        def enter(_e=None):
            label.configure(bg=hover, highlightbackground="#C7A24E")
        def leave(_e=None):
    ...
    def _structure_update_action_buttons(self):
        ...
        if duplicate is not None and duplicate.winfo_exists():
            duplicate.configure(
                ...
                bg="#294957" if mode == "duplicate" else "#162B37",
                fg="#EEF2F1",
                highlightbackground="#C7A24E",
            )
        if delete is not None and delete.winfo_exists():
            delete.configure(
                text="Confirmer" if mode == "delete" and target else "Supprimer",
                bg="#5A3937" if mode == "delete" else "#162B37",
                fg="#F3DEDA",
                highlightbackground="#C56B5B" if mode == "delete" and target else "#C7A24E",
            )
    ...
    def _build_canvas(self):
        ...
        # Bandeau Structure V13 : petit dock centré, uniquement de la longueur
        # de son contenu, avec la même matière bleu-nuit/or que l'atelier C.
        self.structure_command_bar_host = tk.Frame(
            viewer, bg=theme.WINDOW_DEEP, height=48,
        ...
        self.structure_command_bar = tk.Frame(
            self.structure_command_bar_host, bg="#162B37",
            highlightthickness=1, highlightbackground="#C7A24E",
            padx=7, pady=5,
        )
        self.structure_command_bar.pack(anchor="center", pady=3)
