...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        ...
        WORK_BG_2 = "#243438"
        WORK_TILE = "#31474B"
        WORK_TILE_HOVER = "#3A5559"
        WORK_TILE_ACTIVE = "#385E60"
        WORK_BORDER = "#48656A"
        WORK_GOLD = "#C2A36A"
        WORK_TEAL = "#74B6AA"
        WORK_BLUE = "#7FA7BE"
        ...
        def command_chip(parent_, text, command, *, accent=WORK_TEAL, danger=False, compact=False):
            ...
            hover = "#3B5558"
            pressed = "#26383B"
            fg = "#F0D8CF" if danger else WORK_TEXT
            edge = "#A95D50" if danger else accent
            lbl = tk.Label(
        ...
        tk.Label(
            head, text="BRIQUES DE PAGES", bg=WORK_BG, fg=WORK_TEXT,
            font=(theme.FONT_TITLE, 10, "bold"),
        ).grid(row=0, column=0, sticky="w")
        tk.Label(
            head, text="choisir → cliquer dans la ligne", bg=WORK_BG, fg=WORK_MUTED,
            font=(theme.FONT_UI, 7),
        ).grid(row=0, column=1, sticky="w", padx=(9, 0))
        new_type_slot = tk.Frame(head, bg=WORK_BG)
        new_type_slot.grid(row=0, column=3, sticky="e")
        ...
        def brick_style(type_key):
            key = str(type_key or "")
            if key in {"illustration", "fiche", "annexe"}:
                return WORK_ORANGE
            if key in {"chapitre", "tete_partie", "transition"}:
                return WORK_TEAL
            if key in {"texte", "avant_propos", "conclusion", "remerciements"}:
                return WORK_BLUE
            return WORK_GOLD

        def brick_icon(type_key):
            return {
                "": "□", "page_blanche": "▫", "page_titre": "T",
                "sommaire": "≡", "avant_propos": "¶", "tete_partie": "◆",
                "chapitre": "A", "texte": "☰", "illustration": "▧",
                "fiche": "▤", "transition": "—", "annexe": "↗",
        ...
        def make_brick(parent_, label, type_key, *, custom=False):
            ...
            def draw(fill=WORK_TILE, outline=WORK_BORDER, active=False):
                card.delete("all")
                card.create_rectangle(1, 1, width-1, height-1, fill=fill, outline=outline, width=2 if active else 1)
                card.create_rectangle(1, 1, width-1, 4, fill=accent, outline="")
                card.create_rectangle(8, 10, 29, 31, fill=WORK_BG_2, outline=accent, width=1)
                card.create_text(18.5, 20.5, text=brick_icon(type_key), fill=accent, font=(theme.FONT_UI, 9, "bold"))
                card.create_text(36, 16, text=display, anchor="w", fill=WORK_TEXT, font=(theme.FONT_UI, 7, "bold"), width=45)
                if custom:
                    card.create_text(width-7, height-8, text="●", anchor="e", fill=WORK_TEAL, font=(theme.FONT_UI, 5))
            ...
            def release(_e=None):
                ...
                draw(WORK_TILE_ACTIVE, accent, active=True)
                arm("page", {"type": str(type_key or ""), "label": str(label or "Sans type")})
                return "break"
        ...
        def rebuild_catalog():
            ...
            if old is not None:
                try:
                    ...
                    pass
            reset_active_brick()
            holder = tk.Frame(catalog_host, bg=WORK_BG_2)
            holder.grid(row=0, column=0, sticky="nsew")
            catalog_refs["holder"] = holder
            ...
            bricks = [("", "Sans type", False), ("page_blanche", "Page blanche", False)]
            seen = {"", "page_blanche"}
            for key, label, custom in page_types():
                if key not in seen:
        ...
        command_chip(new_type_slot, "+ Nouveau type", create_custom_type, accent=WORK_GOLD, compact=True).pack(side="right")

        # --------------------------------------------------------------
        # RÈGLES : on désigne d'abord la PAGE SOURCE dans B, puis ce qui suit.
        # Local et général vivent dans le même outil, sans autre fenêtre.
        # --------------------------------------------------------------
        rules = tk.Frame(atelier, bg=WORK_BG, pady=5)
        rules.grid(row=2, column=0, sticky="ew", pady=(5, 0))
        rules.grid_columnconfigure(1, weight=1)

        tk.Label(
            rules, text="RÈGLES", bg=WORK_BG, fg=WORK_GOLD,
            font=(theme.FONT_UI, 7, "bold"),
        ).grid(row=0, column=0, sticky="nw", padx=(0, 10), pady=(2, 0))

        rule_line = tk.Frame(rules, bg=WORK_BG)
        rule_line.grid(row=0, column=1, sticky="ew")
        rule_line.grid_columnconfigure(7, weight=1)

        source_var = tk.StringVar(value="1  Source : sélectionnez une page dans B")
        target_var = tk.StringVar(value="2  Page automatique associée : —")
        info_var = tk.StringVar(value="")
        scope = {"value": "local"}
        target_key = {"value": ""}
        scope_labels = {}
        target_mb_ref = {"widget": None}
        general_chips = tk.Frame(rules, bg=WORK_BG)
        general_chips.grid(row=1, column=1, sticky="ew", pady=(5, 0))

        source_label = tk.Label(
            rule_line, textvariable=source_var, bg=WORK_BG_2, fg=WORK_TEXT,
            font=(theme.FONT_UI, 7, "bold"), padx=8, pady=4,
            highlightthickness=1, highlightbackground=WORK_BORDER,
        )
        source_label.grid(row=0, column=0, sticky="w", padx=(0, 6))

        tk.Label(rule_line, text="→", bg=WORK_BG, fg=WORK_GOLD, font=(theme.FONT_UI, 9, "bold")).grid(row=0, column=1, padx=(0, 6))

        target_mb = tk.Menubutton(
            rule_line, textvariable=target_var,
            bg=WORK_TILE, fg=WORK_TEXT,
            activebackground=WORK_TILE_HOVER, activeforeground=WORK_TEXT,
            relief="flat", bd=0, padx=8, pady=4,
            font=(theme.FONT_UI, 7, "bold"), cursor="hand2",
            highlightthickness=1, highlightbackground=WORK_TEAL,
        )
        target_mb.grid(row=0, column=2, sticky="w", padx=(0, 8))
        target_menu = tk.Menu(target_mb, tearoff=False, bg=WORK_BG_2, fg=WORK_TEXT, activebackground=WORK_TILE_HOVER, activeforeground=WORK_TEXT)
        target_mb.configure(menu=target_menu)
        target_mb_ref["widget"] = target_mb

        def set_scope(value):
            scope["value"] = value
            for key, lbl in scope_labels.items():
                lbl.configure(
                    bg=WORK_TILE_ACTIVE if key == value else WORK_TILE,
                    fg=WORK_TEXT,
                    highlightbackground=WORK_GOLD if key == value else WORK_BORDER,
                )
            refresh_rule_info()

        def scope_chip(value, text):
            lbl = tk.Label(
                rule_line, text=text, bg=WORK_TILE, fg=WORK_TEXT,
                font=(theme.FONT_UI, 7, "bold"), padx=7, pady=4,
                highlightthickness=1, highlightbackground=WORK_BORDER,
                cursor="hand2",
            )
            lbl.bind("<Button-1>", lambda _e, v=value: set_scope(v))
            scope_labels[value] = lbl
            return lbl

        scope_chip("local", "Cette page").grid(row=0, column=3, padx=(0, 4))
        scope_chip("general", "Toutes celles de ce type").grid(row=0, column=4, padx=(0, 8))

        def selected_context():
            canvas = book()
            if canvas is None or not hasattr(canvas, "structure_selected_auto_context"):
                return {"selected": False}
            return canvas.structure_selected_auto_context()
        ...
        def choose_target(key, label):
            target_key["value"] = str(key or "")
            target_var.set(f"2  Associer : {label if key else 'Aucune page'}")
            refresh_rule_info()

        def rebuild_target_menu():
            target_menu.delete(0, "end")
            target_menu.add_command(label="Aucune page automatique", command=lambda: choose_target("", "Aucune page"))
            target_menu.add_separator()
            canvas = book()
            options = canvas.structure_auto_target_options() if canvas is not None and hasattr(canvas, "structure_auto_target_options") else []
            for key, label in options:
                target_menu.add_command(label=label, command=lambda k=key, l=label: choose_target(k, l))

        def refresh_rule_info():
            ctx = selected_context()
            if not ctx.get("selected"):
                info_var.set("Sélectionnez d’abord la page qui doit déclencher l’ajout.")
                return
            if not ctx.get("allowed"):
                info_var.set("Cette page ne peut pas être une source de page automatique.")
                return
            labels = labels_by_key()
            if scope["value"] == "general":
                general = str(ctx.get("general_target") or "")
                if general:
                    info_var.set(f"Règle générale actuelle : {labels.get(general, general)}")
                else:
                    info_var.set("Aucune règle générale pour ce type de page.")
            else:
                mode = ctx.get("mode")
                if mode == "local":
                    value = str(ctx.get("local_target") or "")
                    info_var.set(f"Exception locale : {labels.get(value, value)}")
                elif mode == "none":
                    info_var.set("Exception locale : aucune page automatique.")
                else:
                    general = str(ctx.get("general_target") or "")
                    info_var.set("Cette page suit la règle générale." if general else "Aucune page automatique sur cette page.")

        def refresh_general_chips():
            for child in general_chips.winfo_children():
                child.destroy()
            canvas = book()
            rules_map = canvas.structure_general_auto_rules() if canvas is not None and hasattr(canvas, "structure_general_auto_rules") else {}
            if not rules_map:
                tk.Label(general_chips, text="Règles générales : aucune", bg=WORK_BG, fg=WORK_MUTED, font=(theme.FONT_UI, 6)).pack(side="left")
                return
            labels = labels_by_key()
            tk.Label(general_chips, text="Générales :", bg=WORK_BG, fg=WORK_MUTED, font=(theme.FONT_UI, 6, "bold")).pack(side="left", padx=(0, 5))
            for source, target in rules_map.items():
                source_label_ = str(book()._structure_type_definition(source).get("short_label") or book()._structure_type_definition(source).get("label") or source)
                target_label_ = labels.get(target, target)
                chip = tk.Label(
                    general_chips, text=f"{source_label_} → {target_label_}",
                    bg=WORK_BG_2, fg=WORK_TEXT, font=(theme.FONT_UI, 6, "bold"),
                    padx=6, pady=2, highlightthickness=1, highlightbackground=WORK_BORDER,
                )
        ...
        def refresh_rules():
            ...
            ctx = selected_context()
            if not ctx.get("selected"):
                source_var.set("1  Source : sélectionnez une page dans B")
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = ""
                target_var.set("2  Page automatique associée : —")
            elif not ctx.get("allowed"):
                source_var.set(f"1  Source : {ctx.get('page_label', 'page')} — non applicable")
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = ""
                target_var.set("2  Page automatique associée : —")
            else:
                source_type = str(ctx.get("source_type") or "")
                source_name = str(ctx.get("page_label") or source_type or "Page")
                source_var.set(f"1  Source : {source_name}")
                target_mb.configure(state="normal", cursor="hand2")
                labels = labels_by_key()
                if scope["value"] == "general":
                    current_target = str(ctx.get("general_target") or "")
                else:
                    mode = ctx.get("mode")
                    if mode == "local": current_target = str(ctx.get("local_target") or "")
                    elif mode == "none": current_target = ""
                    else: current_target = str(ctx.get("effective_target") or "")
                target_key["value"] = current_target
                target_var.set(f"2  Associer : {labels.get(current_target, current_target) if current_target else 'Aucune page'}")
            refresh_rule_info()
            refresh_general_chips()

        def apply_rule():
            canvas = book()
            ctx = selected_context()
            if canvas is None or not ctx.get("selected") or not ctx.get("allowed"):
                return
            target = str(target_key["value"] or "")
            if scope["value"] == "general":
                source_type = str(ctx.get("source_type") or "")
                if target:
                    canvas.structure_set_general_auto_rule(source_type, target)
                else:
                    canvas.structure_remove_general_auto_rule(source_type)
            else:
                if target:
                    canvas.structure_set_selected_auto_override("local", target)
                else:
                    canvas.structure_set_selected_auto_override("none")
            refresh_rules()

        def inherit_rule():
            canvas = book()
            ctx = selected_context()
            if canvas is not None and ctx.get("selected") and ctx.get("allowed"):
                canvas.structure_set_selected_auto_override("inherit")
                set_scope("local")
                refresh_rules()

        command_chip(rule_line, "Appliquer", apply_rule, accent=WORK_TEAL, compact=True).grid(row=0, column=5, padx=(0, 4))
        command_chip(rule_line, "Suivre la règle générale", inherit_rule, accent=WORK_BLUE, compact=True).grid(row=0, column=6, padx=(0, 6))
        tk.Label(rule_line, textvariable=info_var, bg=WORK_BG, fg=WORK_MUTED, font=(theme.FONT_UI, 6), anchor="e").grid(row=0, column=7, sticky="e")
        set_scope("local")

        # --------------------------------------------------------------
        # Synchronisation avec B.
        # --------------------------------------------------------------
        def refresh_from_selection(_event=None):
            self.after_idle(refresh_rules)

        try:
            self.book_canvas.canvas.bind("<ButtonRelease-1>", refresh_from_selection, add="+")
        except Exception:
            pass
