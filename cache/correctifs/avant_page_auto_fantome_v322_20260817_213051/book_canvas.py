...
class BookCanvas(tk.Frame):
    ...
    def _draw_page_auto_rule_markers(
        ...
        """V32.1 : la page fantôme reste au premier plan, sans être masquée par la page suivante."""
        ...
        for position in ("before", "after"):
            ...
            ghost_h = max(34, min(float(page_h) * 0.72, 50 * max(0.85, scale)))
            ghost_w = ghost_h * 0.70
            gy = y + max(5.0, (float(page_h) - ghost_h) * 0.30)

            overlap = ghost_w * 0.22
            if position == "before":
            ...
            try:
                ...
                if preview:
                    ...
                    if preview_path.is_file():
                        ...
                        cache_key = (str(preview_path), pw, ph)
                        ...
                        if photo is None:
                            ...
                            alpha = Image.new("L", image.size, 78)
        ...
        if ghost_drawn:
            # Le rendu des pages voisines peut avoir lieu après celui du fantôme.
            # On le remonte une fois le rendu courant terminé : il reste donc
            # entièrement visible sans modifier la géométrie de la ligne.
            self.canvas.after_idle(
                lambda: self.canvas.tag_raise("structure_auto_ghost")
            )
