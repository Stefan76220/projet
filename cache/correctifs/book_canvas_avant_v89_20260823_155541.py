...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_build_premium_icon_image(self, kind: str, state: str, size: int):
        ...
        if kind == "settings":
            ...
            # Support de travail : une image fantôme clairement placée derrière une page.
            d=ImageDraw.Draw(image)
            d.rounded_rectangle((18,29,67,70),radius=7,fill="#253138",outline="#77878D",width=2)
            d.polygon([(23,63),(35,48),(44,57),(53,43),(63,63)],fill="#59666B",outline="#ADB7BA")
            d.ellipse((50,36,59,45),fill=gold)
            # Page avant, légèrement décalée, uniquement en contour pour laisser voir le guide.
            d.rounded_rectangle((30,20,77,76),radius=7,fill=(20,27,31,120),outline="#C8D0D3",width=3)
            d.rounded_rectangle((35,26,72,70),radius=5,outline=accent,width=3)
            # Ligne pointillée de guide sous la page.
            for xx in range(20,79,9):
                d.line((xx,81,min(xx+5,79),81),fill=accent,width=3)
        elif kind == "frame":
            # Trois référentiels visibles d'un coup : fond perdu, page, marges.
            d=ImageDraw.Draw(image)
            # Fond perdu : repères saumon aux quatre coins.
            for (x,y,sx,sy) in ((17,17,1,1),(79,17,-1,1),(17,79,1,-1),(79,79,-1,-1)):
                d.line((x,y,x+sx*12,y),fill=coral,width=3)
                d.line((x,y,x,y+sy*12),fill=coral,width=3)
            # Page finie : plaque métallique claire.
            self._gabarit_premium_metal_rect(image,(25,21,71,75),6)
            d=ImageDraw.Draw(image)
            # Marges : cadre turquoise clairement intérieur.
            d.rounded_rectangle((33,30,63,66),radius=4,outline=accent,width=3)
            # Station centrale = référentiel de placement.
            d.line((43,48,53,48),fill=accent,width=2)
            d.ellipse((46,44,54,52),fill=gold)
        elif kind == "snap":
    ...
    def _gabarit_inspector_station_button(self, target, key: str, box, label: str, *, selected=False, danger=False, disabled=False, tooltip=""):
        ...
        button_size=max(22,min(28,int(round(min((x2-x1)-4.0,(y2-y1)-18.0)))))
