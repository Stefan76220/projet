...
class TomeLineaV3(tk.Tk):
    ...
    def _build_gabarits_tab(self, parent):
        ...
        def _panel_fade_in(win, alpha=0.0):
            """Fait apparaître le panneau comme une couche légère au-dessus de la page.

            On s'arrête volontairement avant l'opacité totale : la page reste perceptible
            sous le panneau, comme sous l'inspecteur droit.
            """
            try:
                if not win.winfo_exists():
                    return
                target_alpha = 0.94
                alpha = min(target_alpha, alpha + 0.16)
        ...
        def open_tool_panel(opener, title, builder, *, min_width=320):
            ...
            # Architecture verticale : le panneau se déploie vers la page, à droite
            # de la bande d'outils. Il recouvre temporairement la surface de travail
            # sans la repousser et laisse la page légèrement visible par transparence.
            root_x = int(self.winfo_rootx())
            root_y = int(self.winfo_rooty())
            root_w = int(self.winfo_width())
            root_h = int(self.winfo_height())
            x = int(max(root_x + 10, min(
                ox + ow + 8,
                root_x + root_w - w - 10,
            )))
            y = int(max(root_y + 58, min(
                oy - 8,
                root_y + root_h - h - 12,
            )))
        ...
        def family_card(key, number, title, subtitle):
            card = tk.Frame(
                ...
                highlightbackground=CARD_BORDER, padx=9, pady=8,
            )
