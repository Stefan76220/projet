...
class PageMaitreV2(tk.Tk):
    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        ...
        def render(_event=None):
            ...
            # Variante A — squelette du livre avec ligne continue.
            # Icônes photoréalistes pour rester cohérent avec le reste de TomeLinea.
            ...
            book_sources = [
                PROJECT_ROOT / "assets" / "gui_v2" / "accueil_realistic_icons" / "ouvrage_structure.png",
                PROJECT_ROOT / "assets" / "gui_v2" / "accueil_realistic_icons" / "livre_textuel.png",
                PROJECT_ROOT / "assets" / "gui_v2" / "accueil_realistic_icons" / "bande_dessinee.png",
            ]
            flag_source = (
                PROJECT_ROOT
                / "assets" / "gui_v2" / "navigation_icons_photorealistes"
                / "10_finalisation" / "10_finalisation_64px.png"
            )

            def structure_photo(source_path, size, *, tint=None):
                cache_key = (str(source_path), size, tint)
                cached = self._maquettage_structure_photos.get(cache_key)
                if cached is not None:
                    return cached
                ...
                try:
                    ...
                    if tint is not None:
                        tint_layer = Image.new(
                            "RGBA",
                            image.size,
                            (tint[0], tint[1], tint[2], 255),
                        )
                        image = Image.blend(image, tint_layer, 0.58)
                        image.putalpha(Image.open(source_path).convert("RGBA").getchannel("A"))
                    image.thumbnail((size, size), Image.Resampling.LANCZOS)
                    photo = ImageTk.PhotoImage(image)
                    self._maquettage_structure_photos[cache_key] = photo
                    return photo
                except Exception:
                    return None

            # Ligne continue dessinée d'abord, derrière les nœuds.
            for index in range(node_count - 1):
                left_group = structure_model[index]
                right_group = structure_model[index + 1]
                ...
                canvas.create_line(
                    node_centers[index],
                    track_y,
                    node_centers[index + 1],
                    track_y,
                    fill=self._mix(line_color, "#FFFFFF", 0.20),
                    width=2,
                    capstyle="round",
                    tags="maquettage_ui",
                )
            ...
            for index, group in enumerate(structure_model):
                ...
                canvas.create_oval(
                    ...
                    cx + 19,
                    cy + 19,
                    fill=self._mix(color, "#FFFFFF", 0.95),
                    outline=color,
                    width=2,
                    tags="maquettage_ui",
                )
                ...
                photo = None
                if index == 0:
                    photo = structure_photo(flag_source, 28, tint=(102, 176, 138))
                elif index == node_count - 1:
                    photo = structure_photo(flag_source, 28, tint=(122, 184, 155))
                else:
                    source = book_sources[(index - 1) % len(book_sources)]
                    photo = structure_photo(source, 28)
                ...
                if photo is not None:
                    ...
                    # filet minimal si l'image manque
                    canvas.create_text(
                        cx,
                        cy,
                        text="•",
