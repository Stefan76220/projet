...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_inspector_draw_premium_icon(self, target, kind: str, box, *, selected=False, disabled=False, hovered=False):
        ...
        size=max(16,min(40,int(round(min(x2-x1,y2-y1)*0.88))))
    ...
    def _gabarit_build_station_button_image(self, state: str, size: int):
        ...
        gold="#D5AD63" if state=="active" else "#9B8C6E"
        # ombre douce
        d.ellipse((15,18,87,90),fill=(0,0,0,68))
        # bouton circulaire premium
        d.ellipse((11,11,83,83),fill="#0F1519",outline="#556168",width=2)
        d.ellipse((16,16,78,78),fill="#1D272C",outline="#2A363C",width=2)
        d.ellipse((19,19,75,45),fill="#2A353A")
        d.arc((18,18,76,76),start=215,end=330,fill="#6E7A80",width=2)
        d.arc((18,18,76,76),start=30,end=145,fill="#11181C",width=2)
        # coeur du bouton
        d.ellipse((29,29,65,65),fill="#12191D",outline=accent if state!="normal" else "#607077",width=2)
        d.line((38,47,56,47),fill=accent,width=4)
        d.ellipse((44,43,52,51),fill=gold if state=="active" else ("#8ABAB7" if state=="hover" else "#899699"))
        # repère externe de réaction
        if state in {"hover","active","danger"}:
            d.ellipse((8,8,86,86),outline=accent,width=2 if state=="hover" else 3)
        if state=="disabled":
            image=Image.alpha_composite(image,Image.new("RGBA",(S,S),(13,18,21,132)))
        try:
    ...
    def _gabarit_inspector_station_button(self, target, key: str, box, label: str, *, selected=False, danger=False, disabled=False, tooltip=""):
        ...
        button_size=max(20,min(26,int(round(min((x2-x1)-6.0,(y2-y1)-18.0)))))
