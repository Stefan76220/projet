...
"""Structure physique du Livre composé par TomeLinea.
...
- Canvas Editor compose et pagine le texte, chapitre par chapitre ;
...
class CompositionStructureSyncResult:
    changed: bool
    body_page_count: int
...
def _ordered_composition_ids(book: Any) -> list[str]:
    result: list[str] = []
    for page_id in list(getattr(book, "page_order", ())):
        page = book.pages.get(page_id)
        metadata = getattr(page, "metadata", None) if page is not None else None
        if isinstance(metadata, dict) and metadata.get("composition_page"):
            result.append(str(page_id))
    return result


def _insert_composition_page_at_chapter_end(
    book: Any,
...
def synchronize_composition_structure(
    ...
    composition_ids = _ordered_composition_ids(book)
    tagged = any(
        isinstance(book.pages[page_id].metadata.get("canvas_chapter_index"), int)
        for page_id in composition_ids
    )
    ...
    if tagged:
        ...
        for chapter_index, wanted in enumerate(counts):
            take = min(wanted, max(0, len(composition_ids) - cursor))
            groups[chapter_index].extend(composition_ids[cursor:cursor + take])
            cursor += take
        unassigned.extend(composition_ids[cursor:])
    ...
    for chapter_index, wanted in enumerate(counts):
        while len(groups[chapter_index]) < wanted:
            _insert_composition_page_at_chapter_end(book, groups, chapter_index)
            created_pages += 1
            changed = True
    ...
    for chapter_index, group in enumerate(groups):
        ...
        for local_index, page_id in enumerate(group):
            ...
            before = (
                ...
                page.title,
                page.page_type,
                metadata.get("composition_page_index"),
            ...
            metadata["composition_page"] = True
            metadata["composition_page_index"] = global_index
            metadata["composition_page_start"] = global_index
    ...
    return CompositionStructureSyncResult(
        changed=changed,
