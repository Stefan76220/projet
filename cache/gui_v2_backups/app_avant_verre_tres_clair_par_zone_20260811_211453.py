...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def _home_glass_photo(kind: str, width: int, height: int, color: str, state: str):
            ...
            # La couleur est maintenant un voile, pas un aplat.
            back_rgb = _blend_rgb(color_rgb, warm_white, 0.73)
            glass_rgb = _blend_rgb(color_rgb, warm_white, 0.90)
            edge_rgb = _blend_rgb(color_rgb, warm_white, 0.53)

            grain_light = _blend_rgb(warm_white, glass_rgb, 0.18)
            grain_dark = _blend_rgb((222, 217, 210), color_rgb, 0.10)

            img = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            draw = ImageDraw.Draw(img)

            # Couleur très claire derrière le verre.
            ...
            draw.rounded_rectangle(
                ...
                outline=(255, 253, 249, 92),
                width=1,
            )

            # Lumière diffuse venant du haut : très faible, sans trait lumineux.
            top = inset + 1
            bottom = height - inset - 2
            usable_h = max(1, bottom - top + 1)
            for yy in range(top, bottom + 1):
                p = (yy - top) / usable_h
                alpha = int(10 * (1.0 - p) + 2)
                wash_rgb = _blend_rgb(warm_white, glass_rgb, 0.20 + 0.25 * p)
                draw.line(
            ...
            # Grain dépoli fin, mat et peu contrasté.
            for gx in range(10, width - 10, grain_step_x):
                for gy in range(8, height - 8, grain_step_y):
                    seed = ((gx * 17 + gy * 23 + width * 3 + height) % 9)
                    if seed in (0, 4, 7):
                        dot_fill = grain_light + (24 if seed == 0 else 18,)
                    else:
                        dot_fill = grain_dark + (12 if seed in (2, 6) else 8,)
                    draw.ellipse(
                        (gx, gy, gx + 1, gy + 1),
                        fill=dot_fill,
                        outline=None,
            ...
            # Liseré intérieur quasi invisible.
            draw.rounded_rectangle(
        ...
        def home_command_button(
            ...
            *,
            ...
            shadow = self._mix(color, "#D0CBC4", 0.82 if state == "hover" else 0.86)
        ...
        def home_choice_panel(
            ...
            *,
            ...
            shadow = self._mix(color, "#D0CBC4", 0.82 if hover else 0.86)
            if selected:
                shadow = self._mix(color, "#C9C4BD", 0.78 if hover else 0.82)
        ...
        def render_shortcuts_content(_event=None):
            ...
            shortcut_data = (
                ("Maquettage", "Organiser les pages", "#1E3651", "shortcut_maquettage"),
                ("Atelier", "Préparer les gabarits", "#8AB1B1", "shortcut_atelier"),
                ("Conception", "Créer les pages", "#7B6D84", "shortcut_conception"),
                ("Centre du projet", "Vue d’ensemble", "#E17755", "shortcut_centre"),
            )
            ...
            for index, (title, value, color) in enumerate(columns):
                ...
                if index < 2:
                    ...
                    canvas.create_line(
                        sep_x, status_y - 2,
                        sep_x, status_y + 30,
                        fill=self._mix("#7B6D84", "#FFFFFF", 0.62),
                        tags="home_shortcuts",
                    )
