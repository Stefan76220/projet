...
class BookV4:
    ...
    def remove_page(
        ...
        if is_cover_face(self.pages[page_id]):
            raise ValueError(
                "Les faces de couverture font partie de la structure physique du livre et ne peuvent pas être supprimées."
            )
    ...
    def move_page(
        ...
        lower, upper = interior_insertion_bounds(self)
        if int(new_index) < lower or int(new_index) > upper:
            raise ValueError(
