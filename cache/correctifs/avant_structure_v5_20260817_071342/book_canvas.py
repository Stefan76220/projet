...
class BookCanvas(tk.Frame):
    ...
    # ------------------------------------------------------------------
    # Structure V4 : C = briques / B = zone de travail
    # ------------------------------------------------------------------
        ...
        if isinstance(definitions, list):
            for definition in definitions:
                ...
                if type_key.lower() in self.COVER_TYPES | self.BACK_COVER_TYPES | self.SECOND_COVER_TYPES | self.THIRD_COVER_TYPES:
                    continue
                label = str(
                    definition.get("label") or definition.get("title") or definition.get("name") or type_key
                ).strip() or type_key
                result.append((type_key, label, bool(definition.get("custom", True))))
                seen.add(type_key)
        return result

    def structure_create_custom_type(self, name: str) -> tuple[str, str] | None:
        ...
        if self.on_change is not None:
            self.on_change()
        self.status_var.set(f"Nouveau type  •  {name}")
    ...
    def structure_arm_tool(self, kind: str, payload) -> None:
        ...
        if isinstance(payload, (tuple, list)) and len(payload) > 1:
            label = str(payload[1])
        self.status_var.set(f"{label}  •  cliquez dans B pour déposer  •  Échap/Annuler pour quitter")
        try:
    ...
    def _structure_insert_page(self, page_type: str, label: str, group_id: str, local_pos: int) -> int | None:
        if self.project is None:
            return None
        page_type = str(page_type or "personnalisee").strip() or "personnalisee"
        label = str(label or "Page").strip() or "Page"
        item = {
    ...
    def _structure_insert_group(self, canvas_x: float) -> str | None:
        ...
        target = self._target_movable_group_index(canvas_x)
        ...
        start_group = next((dict(g) for g in self.groups if str(g.get("id", "")) == self.START_GROUP_ID), dict(self.DEFAULT_GROUPS[0]))
        end_group = next((dict(g) for g in self.groups if str(g.get("id", "")) == self.END_GROUP_ID), dict(self.DEFAULT_GROUPS[-1]))
    ...
    def _structure_apply_at(self, kind: str, payload, x: float, y: float) -> bool:
        ...
        if kind == "page":
            ...
            if index is not None:
                return self._structure_change_page_type(index, str(page_type), str(label))
            group_id = self._group_id_at_x(canvas_x)
            local_pos = self._target_local_pos_from_x(canvas_x, group_id)
            return self._structure_insert_page(str(page_type), str(label), group_id, local_pos) is not None
        return False
    ...
    def _structure_apply_pending_event(self, event):
        ...
        if not kind:
            return None
        success = self._structure_apply_at(kind, payload, event.x, event.y)
        if success:
    ...
    def structure_drop_from_root(self, kind: str, payload, root_x: int, root_y: int) -> bool:
        ...
        if x < 0 or y < 0 or x > width or y > height:
            return False
        success = self._structure_apply_at(str(kind), payload, x, y)
        if success:
            self.structure_cancel_tool()
        return bool(success)
    ...
    def structure_duplicate_selected(self) -> bool:
        if getattr(self, "_structure_selection_kind", "page") != "page":
            self.status_var.set("La duplication de partie sera ajoutée plus tard. Pour l'instant, déposez + Partie dans B.")
            return False
        ...
        for key in ("automatic_recto_verso", "automatic", "auto_generated", "recto_target_id", "linked_to", "parent_id", "source_page_id"):
    ...
    def structure_delete_selected(self) -> bool:
        ...
        if kind == "group" and self._selected_group_id:
            ...
            if self._group_items(group_id):
                ...
                return False
            self.groups = [group for group in self.groups if str(group.get("id", "")) != group_id]
