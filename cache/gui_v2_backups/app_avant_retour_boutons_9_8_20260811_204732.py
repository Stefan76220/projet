    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton : verre dépoli mat, grain fin, reflet très léger venant du haut."""
            ...
            shadow = self._mix(color, "#6A6A68", 0.34 if state == "hover" else 0.40)
            face = self._mix(color, "#FFF8F1", 0.82 if state == "hover" else 0.86)
            veil = self._mix(color, "#FBF7F1", 0.80)
            outline = self._mix(color, "#F8F2EA", 0.34)
            inner = self._mix(color, "#FFFFFF", 0.88)
            top_light = self._mix(color, "#FFFFFF", 0.97)
            grain_light = self._mix(color, "#FFFFFF", 0.76)
            grain_dark = self._mix(color, "#E7DED3", 0.76)
            text_color = self._mix(color, "#2B342F", 0.56)
            ...
            # Reflet très léger venant du haut.
            canvas.create_line(
                x1 + 14, y1 + 5,
                x2 - 14, y1 + 5,
                fill=top_light,
            ...
            canvas.create_line(
                x1 + 18, y1 + 8,
                x2 - 18, y1 + 8,
                fill=self._mix(face, "#FFFFFF", 0.55),
                width=1,
                tags=(tag, "home_button"),
            )

            # Grain fin de verre dépoli.
            dots = [
                (x1 + 12, y1 + 10), (x1 + 20, y1 + 15), (x1 + 16, y2 - 11),
                (x2 - 22, y1 + 11), (x2 - 14, y1 + 16), (x2 - 18, y2 - 10),
                (cx - 24, cy - 4), (cx + 26, cy + 3),
            ]
            for i, (gx, gy) in enumerate(dots):
                canvas.create_oval(
                    gx, gy, gx + 1.2, gy + 1.2,
                    fill=grain_light if i % 2 == 0 else grain_dark,
                    outline="",
                    tags=(tag, "home_button"),
                )
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Carte de choix : verre dépoli mat, grain discret, lumière venant du haut."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            base_mix = 0.84 if not selected else 0.78
            if hover:
                base_mix -= 0.04
            if pressed:
                base_mix += 0.01

            face = self._mix(color, "#FFF8F1", max(0.70, base_mix))
            veil = self._mix(color, "#FBF7F1", 0.80)
            shadow = self._mix(color, "#726E68", 0.35 if hover else 0.41)
            outline = self._mix(color, "#F8F2EA", 0.24 if selected else 0.40)
            inner = self._mix(color, "#FFFFFF", 0.86)
            top_light = self._mix(color, "#FFFFFF", 0.97)
            grain_light = self._mix(color, "#FFFFFF", 0.78)
            grain_dark = self._mix(color, "#E7DED3", 0.78)
            ...
            canvas.create_line(
                x1 + 26, y1 + 10,
                x2 - 26, y1 + 10,
                fill=self._mix(face, "#FFFFFF", 0.54),
                width=1,
                tags=(tag, "home_choice"),
            )

            # Grain fin réparti, discret.
            dots = [
                (x1 + 16, y1 + 18), (x1 + 24, y1 + 26), (x1 + 20, y2 - 19),
                (x1 + 32, y2 - 14), (x2 - 22, y1 + 20), (x2 - 15, y1 + 28),
                (x2 - 26, y2 - 18), (x2 - 18, y2 - 12), (x1 + 46, y1 + 15),
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule informative : verre dépoli fin."""
            width = max(64, int(len(text) * 5.0) + 31)
            height = 20

            face = self._mix(fg, "#FFF8F1", 0.86)
            veil = self._mix(fg, "#FBF7F1", 0.82)
            outline = self._mix(fg, "#F8F2EA", 0.40)
            inner = self._mix(fg, "#FFFFFF", 0.86)
            top_light = self._mix(fg, "#FFFFFF", 0.97)
            grain = self._mix(fg, "#E9E0D5", 0.78)
            ...
            for gx, gy in ((x + 9, y + 9), (x + width - 14, y + 8), (x + width - 19, y + 12)):
                canvas.create_oval(
                    gx, gy, gx + 1.2, gy + 1.2,
                    fill=grain,
                    outline="",
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Raccourci : verre dépoli fin, cohérent avec les autres éléments."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FFF8F1", face_mix)
            veil = self._mix(color, "#FBF7F1", 0.82)
            shadow = self._mix(color, "#746F68", 0.36 if state == "hover" else 0.42)
            outline = self._mix(color, "#F8F2EA", 0.40)
            inner = self._mix(color, "#FFFFFF", 0.86)
            top_light = self._mix(color, "#FFFFFF", 0.97)
            grain_light = self._mix(color, "#FFFFFF", 0.78)
            grain_dark = self._mix(color, "#E7DED3", 0.78)
