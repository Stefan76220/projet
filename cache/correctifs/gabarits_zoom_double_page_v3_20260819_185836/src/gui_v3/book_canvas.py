...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Gabarits — poste de zoom REEL.
        # Ce ne sont plus des dessins/hitboxes dans le Canvas : ce sont de vrais
        # boutons Tk reliés directement au même moteur que la molette.
        self.gabarit_zoom_dock = tk.Frame(
            viewer,
            bg=theme.WINDOW_DEEP,
            bd=0,
            highlightthickness=0,
        ...
        def make_gabarit_zoom_button(text: str, action, *, width: int, font_size: int):
            # Vrai bouton Tk, sans hitbox Canvas. L'action est reliée directement
            # au ButtonPress pour ne dépendre d'aucune couche de rendu de B.
            btn = tk.Button(
                self.gabarit_zoom_dock,
                text=text,
                command=action,  # conserve aussi .invoke() pour les tests/accessibilité
                width=width,
                bg="#223744",
                fg="#F2F3F1",
                activebackground="#314752",
            ...
            def press(_event):
                btn.configure(bg="#314752", relief="sunken")
                action()
                return "break"

            def release(_event):
                btn.configure(bg="#314752" if str(btn.cget("cursor")) == "hand2" else "#223744", relief="flat")
                return "break"

            btn.bind("<ButtonPress-1>", press)
            btn.bind("<ButtonRelease-1>", release)
            btn.bind("<Enter>", lambda _e: btn.configure(bg="#314752"))
            btn.bind("<Leave>", lambda _e: btn.configure(bg="#223744", relief="flat"))
            return btn

        self.gabarit_zoom_plus_btn = make_gabarit_zoom_button(
            "+", self.gabarit_zoom_in, width=3, font_size=13
        )
        self.gabarit_zoom_plus_btn.pack(fill="x", pady=(0, 4))
        self.gabarit_zoom_value_label = tk.Label(
            self.gabarit_zoom_dock,
            textvariable=self._gabarit_zoom_text_var,
            bg=theme.WINDOW_DEEP,
            fg="#A6BBC0",
            font=(theme.FONT_UI, 8, "bold"),
            width=7,
        )
        self.gabarit_zoom_value_label.pack(fill="x", pady=2)
        self.gabarit_zoom_minus_btn = make_gabarit_zoom_button(
            "−", self.gabarit_zoom_out, width=3, font_size=13
        )
        self.gabarit_zoom_minus_btn.pack(fill="x", pady=4)
        self.gabarit_zoom_fit_btn = make_gabarit_zoom_button(
            "Adapter", self.gabarit_reset_zoom, width=7, font_size=8
        )
        self.gabarit_zoom_fit_btn.pack(fill="x", pady=(4, 0))
        self.gabarit_zoom_dock.place_forget()
    ...
    def set_work_mode(self, mode: str):
        ...
        zoom_dock = getattr(self, "gabarit_zoom_dock", None)
        if zoom_dock is not None:
            # Le zoom à la molette est le geste principal. Les commandes de secours
            # sont maintenant intégrées tout en bas de l'inspecteur de B.
            zoom_dock.place_forget()
    ...
    def gabarit_set_selected_zone_rect_mm(self, x_mm: float, y_mm: float, w_mm: float, h_mm: float) -> bool:
        ...
        if w_mm <= 0 or h_mm <= 0:
            return False
        # Une zone libre peut volontairement dépasser la page ; on garde toutefois
        # des limites raisonnables pour éviter une saisie accidentelle aberrante.
        if not (-30.0 <= x_mm <= 240.0 and -30.0 <= y_mm <= 327.0 and w_mm <= 270.0 and h_mm <= 357.0):
            return False
        zone["occupation"] = "free"
        return self._gabarit_commit_zone_rect(
            zone,
            (x_mm / 210.0, y_mm / 297.0, w_mm / 210.0, h_mm / 297.0),
    ...
    def gabarit_set_selected_zone_occupation(self, mode: str) -> bool:
        ...
        if mode == "margins":
            ...
            left = float(margins.get("inside", 15.0)) / 210.0
            right = float(margins.get("outside", 15.0)) / 210.0
            top = float(margins.get("top", 15.0)) / 297.0
            bottom = float(margins.get("bottom", 15.0)) / 297.0
            ...
            left = float(bleed.get("left", 3.0)) / 210.0
            right = float(bleed.get("right", 3.0)) / 210.0
            top = float(bleed.get("top", 3.0)) / 297.0
            bottom = float(bleed.get("bottom", 3.0)) / 297.0
    ...
    def _gabarit_sync_zoom_widget(self) -> None:
        """Synchronise l'affichage du vrai poste de zoom avec le moteur."""
        var = getattr(self, "_gabarit_zoom_text_var", None)
        if var is not None:
            try:
                var.set(f"{int(getattr(self, '_gabarit_zoom', 100))} %")
            except Exception:
                pass
        dock = getattr(self, "gabarit_zoom_dock", None)
        if dock is not None and getattr(self, "_work_mode", "") == "gabarits":
            try:
                dock.lift()
            except Exception:
                pass
    ...
    def gabarit_zoom_in(self) -> None:
        """Vrai bouton + : appelle exactement le moteur utilisé par la molette."""
        self.gabarit_step_zoom(1, anchor=self._gabarit_button_zoom_anchor())
    ...
    def gabarit_reset_zoom(self) -> None:
        """Réinitialise au plus grand affichage entièrement contenu dans B."""
        self._gabarit_zoom = 100
        self._gabarit_pan_x = 0.0
        self._gabarit_pan_y = 0.0
        self._gabarit_zoom_anchor = None
    ...
    def _gabarit_view_center(self) -> tuple[float, float]:
        """Centre réel de la surface de travail, hors inspecteur latéral."""
        width = max(1.0, float(self.canvas.winfo_width()))
        height = max(1.0, float(self.canvas.winfo_height()))
        info_w = 190.0 if width >= 1000 else 168.0
        work_left = 12.0
        work_right = max(work_left + 1.0, width - info_w - 18.0)
        work_top = 82.0
    ...
    def _gabarit_dimensions_for_zoom(self, value: int | None = None) -> tuple[float, float, float]:
        ...
        single_w = self.BASE_PAGE_W * scale
        return single_w * (2.0 if spread else 1.0), self.BASE_PAGE_H * scale, scale
    ...
    def _gabarit_button_zoom_anchor(self) -> tuple[float, float]:
        ...
        if box is None or not hasattr(self, "canvas"):
            return cx, cy
        vw = max(1.0, float(self.canvas.winfo_width()))
        vh = max(1.0, float(self.canvas.winfo_height()))
        x1, y1, x2, y2 = box
        ix1, ix2 = max(0.0, x1), min(vw, x2)
        iy1, iy2 = max(78.0, y1), min(vh, y2)
    ...
    def gabarit_step_zoom(self, delta: int, *, anchor: tuple[float, float] | None = None) -> None:
        ...
        if current < 160:
            ...
            step = 40
        self.gabarit_set_zoom(current + (step if int(delta) > 0 else -step), anchor=anchor)
    ...
    def _gabarit_effective_book_layout(self) -> dict:
        """Marges et fond perdu : un réglage unique au niveau du livre.

        Les anciens projets peuvent encore porter ces valeurs sur les pages. Dans ce
        cas on reprend silencieusement la première valeur disponible comme héritage,
        jusqu'à la première validation explicite dans l'inspecteur.
        """
        ...
        if not isinstance(raw, dict):
            # Migration douce : première page manuelle ayant des réglages historiques.
            raw = None
    ...
    def _gabarit_save_book_layout(self, key: str, values: dict, status: str) -> bool:
        ...
        # Conserve le comportement historique pour un BookCanvas détaché (tests,
        # outils anciens) qui n'a pas de projet persistant.
    ...
    def gabarit_set_margins(self, top: float, bottom: float, inside: float, outside: float) -> bool:
        ...
        if any(v < 0 for v in values.values()) or values["top"] + values["bottom"] >= 297 or values["inside"] + values["outside"] >= 210:
            return False
    ...
    def gabarit_align_selected_zone(self, mode: str, reference: str = "page") -> bool:
        ...
        if reference == "margins":
            settings = self.gabarit_current_settings() or {}
            margins = dict(settings.get("margins_mm") or {})
            rx = float(margins.get("inside", 15.0)) / 210.0
            ry = float(margins.get("top", 15.0)) / 297.0
            rw = max(0.001, 1.0 - (float(margins.get("inside", 15.0)) + float(margins.get("outside", 15.0))) / 210.0)
            rh = max(0.001, 1.0 - (float(margins.get("top", 15.0)) + float(margins.get("bottom", 15.0))) / 297.0)
        else:
    ...
    def _gabarit_fit_scale(self, viewport_w: float, viewport_h: float) -> float:
        spread = self._gabarit_active_is_spread()
        logical_w = self.BASE_PAGE_W * (2.0 if spread else 1.0)
        ...
        info_w = 190.0 if viewport_w >= 1000 else 168.0
        work_left = 12.0
        work_right = max(work_left + 1.0, viewport_w - info_w - 18.0)
        top_h = 82.0
        bottom_h = 16.0
    ...
    def _gabarit_clamp_pan(self, viewport_w: float, viewport_h: float, page_w: float, page_h: float) -> None:
        """Limite seulement le déplacement ; ne réduit jamais le zoom."""
        if int(getattr(self, "_gabarit_zoom", 100)) <= 100:
            self._gabarit_pan_x = 0.0
            self._gabarit_pan_y = 0.0
            return
        # Même principe que l'ancienne visionneuse Structure : tant qu'une
        # dimension tient dans B elle reste centrée ; si elle déborde, on peut
        # amener chacun de ses bords dans la fenêtre d'observation.
        max_x = max(0.0, (float(page_w) - float(viewport_w) * 0.94) / 2.0)
        max_y = max(0.0, (float(page_h) - float(viewport_h) * 0.94) / 2.0)
        if max_x <= 0.0:
            self._gabarit_pan_x = 0.0
        else:
    ...
    @staticmethod
    def _gabarit_bleed_pixels(w: float, h: float, settings: dict) -> tuple[float, float, float, float]:
        ...
        left = px(bleed.get("left", 3.0), w, 210.0)
        right = px(bleed.get("right", 3.0), w, 210.0)
        top = px(bleed.get("top", 3.0), h, 297.0)
        bottom = px(bleed.get("bottom", 3.0), h, 297.0)
    ...
    def _gabarit_draw_sheet_guides(self, target, x: float, y: float, w: float, h: float, *, item: dict | None = None, fold_side: str = "") -> None:
        ...
        if bool(settings.get("show_guides", True)):
            ...
            mx1 = x + w * float(margins.get("inside", 15.0)) / 210.0
            mx2 = x + w - w * float(margins.get("outside", 15.0)) / 210.0
            my1 = y + h * float(margins.get("top", 15.0)) / 297.0
            my2 = y + h - h * float(margins.get("bottom", 15.0)) / 297.0
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float) -> None:
        ...
        for zone in zones:
            ...
            kind = str(zone.get("kind") or "document")
            zx1 = x1 + max(0.0, min(1.0, float(zone.get("x", 0.1)))) * total_w
            zy1 = y1 + max(0.0, min(1.0, float(zone.get("y", 0.1)))) * page_h
            zw = max(0.04, min(1.0, float(zone.get("w", 0.3)))) * total_w
            zh = max(0.04, min(1.0, float(zone.get("h", 0.2)))) * page_h
            zx2 = min(x1 + total_w, zx1 + zw)
            zy2 = min(y1 + page_h, zy1 + zh)
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # Le poste de zoom n'est plus dessiné dans le Canvas.
        # Les vrais boutons Tk sont créés une seule fois dans _build_canvas().

        # ----- inspecteur contextuel : toute la hauteur de B -----
        info_y1 = 10.0
        info_x2 = viewport_w - 10.0
        info_y2 = viewport_h - 10.0
        self.canvas.create_rectangle(
            info_x1, info_y1, info_x2, info_y2,
            fill="#26313B", outline="#46545D", width=1,
        )
        tx = info_x1 + 13.0
        inner_right = info_x2 - 13.0
        inner_w = max(100.0, inner_right - tx)

        def section_label(y: float, text: str) -> float:
            self.canvas.create_text(
                tx, y, text=text, anchor="w", fill="#9DB0B5",
        ...
        def control_box(key: str, y: float, h: float, *, selected: bool = False) -> tuple[float, float, float, float]:
            hovered = str(getattr(self, "_gabarit_hover_control", "")) == key
            pressed = str(getattr(self, "_gabarit_pressed_control", "")) == key
            ...
            self.canvas.create_rectangle(
                tx, y, inner_right, y + h, fill=fill, outline=outline,
                width=2 if hovered or selected else 1,
            )
            self._gabarit_hitboxes[key] = (tx, y, inner_right, y + h)
            return tx, y, inner_right, y + h

        y = info_y1 + 22.0
        y = section_label(y, "PAGE ACTIVE")
        title_id = self.canvas.create_text(
            tx, y, text=f"{number} — {active_type}", anchor="nw",
            fill="#F0F2F0", font=(theme.FONT_TITLE, 9, "bold"),
            width=inner_w,
        )
        bbox = self.canvas.bbox(title_id)
        y = float((bbox[3] if bbox else y + 20.0) + 8.0)
        self.canvas.create_text(
            tx, y, text=part_text, anchor="nw", fill="#BFC9C7",
            font=(theme.FONT_UI, 7), width=inner_w,
        )
        y += 31.0
        y = separator(y)

        y = section_label(y, "GABARIT")
        self.canvas.create_text(tx, y, text="Portée", anchor="w", fill="#C3CDCB", font=(theme.FONT_UI, 7, "bold"))
        y += 10.0
        page_scope = str(getattr(self, "_gabarit_scope", "page")) != "type"
        type_scope = not page_scope
        control_box("scope_page", y, 25.0, selected=page_scope)
        self.canvas.create_text(tx + 9, y + 12.5, text="Cette page", anchor="w", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if page_scope else "normal"))
        y += 30.0
        control_box("scope_type", y, 25.0, selected=type_scope)
        self.canvas.create_text(tx + 9, y + 12.5, text="Toutes les pages du type", anchor="w", fill="#F1F3F2", font=(theme.FONT_UI, 7, "bold" if type_scope else "normal"))
        y += 37.0

        self.canvas.create_text(tx, y, text="Statut", anchor="w", fill="#C3CDCB", font=(theme.FONT_UI, 7, "bold"))
        y += 10.0
        ...
        control_box("toggle_status", y, 28.0)
        self.canvas.create_oval(tx + 9, y + 9, tx + 19, y + 19, fill=status_color, outline="")
        self.canvas.create_text(tx + 26, y + 14, text=status_text, anchor="w", fill="#E8ECEA", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(inner_right - 9, y + 14, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += 41.0
        y = separator(y)

        y = section_label(y, "LIVRE")
        self.canvas.create_text(tx, y, text="Format fini", anchor="w", fill="#B9C5C3", font=(theme.FONT_UI, 7))
        self.canvas.create_text(inner_right, y, text="210 × 297 mm", anchor="e", fill="#E2E7E5", font=(theme.FONT_UI, 7, "bold"))
        y += 18.0
        ...
        control_box("edit_margins", y, 39.0)
        self.canvas.create_text(tx + 9, y + 11, text="Marges", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(tx + 9, y + 27, text=compact_values(margins, ("top", "bottom", "inside", "outside"), ("H", "B", "I", "E")), anchor="w", fill="#AEBDBA", font=(theme.FONT_UI, 7))
        self.canvas.create_text(inner_right - 9, y + 20, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += 45.0
        control_box("edit_bleed", y, 39.0)
        self.canvas.create_text(tx + 9, y + 11, text="Fond perdu", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(tx + 9, y + 27, text=compact_values(bleed, ("top", "bottom", "left", "right"), ("H", "B", "G", "D")), anchor="w", fill="#C49A9A", font=(theme.FONT_UI, 7))
        self.canvas.create_text(inner_right - 9, y + 20, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))

        # Zoom secondaire, volontairement compact et collé au bas de l'inspecteur.
        zoom_y = info_y2 - 48.0
        self.canvas.create_line(tx, zoom_y - 12, inner_right, zoom_y - 12, fill="#40525B", width=1)
        self.canvas.create_text(tx, zoom_y - 1, text="AFFICHAGE", anchor="sw", fill="#9DB0B5", font=(theme.FONT_UI, 7, "bold"))
        zy = zoom_y + 4.0
        gap = 4.0
        minus_w, value_w, plus_w = 28.0, 48.0, 28.0
        fit_w = max(48.0, inner_w - minus_w - value_w - plus_w - gap * 3)
        zx = tx
        # -
        hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_minus"
        self.canvas.create_rectangle(zx, zy, zx + minus_w, zy + 25, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
        self.canvas.create_text(zx + minus_w/2, zy + 12.5, text="−", fill="#E6EBE9", font=(theme.FONT_UI, 11, "bold"))
        self._gabarit_hitboxes["zoom_minus"] = (zx, zy, zx + minus_w, zy + 25)
        zx += minus_w + gap
        self.canvas.create_rectangle(zx, zy, zx + value_w, zy + 25, fill="#26313B", outline="#40525B")
        self.canvas.create_text(zx + value_w/2, zy + 12.5, text=f"{int(getattr(self, '_gabarit_zoom', 100))}%", fill="#B9C6C4", font=(theme.FONT_UI, 7, "bold"))
        zx += value_w + gap
        hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_plus"
        self.canvas.create_rectangle(zx, zy, zx + plus_w, zy + 25, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
        self.canvas.create_text(zx + plus_w/2, zy + 12.5, text="+", fill="#E6EBE9", font=(theme.FONT_UI, 10, "bold"))
        self._gabarit_hitboxes["zoom_plus"] = (zx, zy, zx + plus_w, zy + 25)
        zx += plus_w + gap
        hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_fit"
        self.canvas.create_rectangle(zx, zy, inner_right, zy + 25, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
        self.canvas.create_text((zx + inner_right)/2, zy + 12.5, text="Adapter", fill="#E6EBE9", font=(theme.FONT_UI, 7, "bold"))
        self._gabarit_hitboxes["zoom_fit"] = (zx, zy, inner_right, zy + 25)
    ...
    def _gabarit_press(self, event):
        ...
        for key, box in self._gabarit_hitboxes.items():
            if self._gabarit_point_in(box, x, y):
                if key in {"prev", "next"}:
                    # Les commandes de navigation/zoom s'exécutent dès l'appui.
                    # L'ancienne activation au relâchement devenait aléatoire car
                    # le rendu complet reconstruit les hitboxes entre les deux événements.
                    ...
                    if key == "prev":
                        ...
                        self.gabarit_navigate(1)
                    # Le rendu déclenché par l'action conserve brièvement l'état pressé.
                    try:
    ...
    def _on_ctrl_mousewheel(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            self.gabarit_step_zoom(
                1 if event.delta > 0 else -1,
                anchor=(float(getattr(event, "x", 0.0)), float(getattr(event, "y", 0.0))),
            )
            return "break"
        # Structure : aucun zoom de squelette.
        return "break"
    ...
    def _on_mousewheel(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            # Dans Gabarits la molette sert au zoom : c'est l'usage naturel
            # quand l'utilisateur observe un détail de la vraie page.
            if self._gabarit_point_in(getattr(self, "_gabarit_page_box", None), float(event.x), float(event.y)):
                self.gabarit_step_zoom(
                    1 if event.delta > 0 else -1,
