...
# TOMELINEA_GABARITS_PALETTE_RAIL_V63
...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_inspector_width(self, viewport_w: float | None = None) -> int:
        """Largeur du calque d'inspecteur.

        Sans zone sélectionnée, l'inspecteur général garde sa largeur historique.
        Avec une zone active, V63 réserve une grande surface transparente à gauche
        du rail : seul le rail de 50 px est visible au repos, et les palettes peuvent
        se déployer vers la gauche sans tasser leurs icônes.
        """
        width = float(viewport_w if viewport_w is not None else self.canvas.winfo_width())
        try:
            contextual = len(self.gabarit_selected_zone_infos()) == 1
        except Exception:
            contextual = False
        if contextual:
    ...
    GABARIT_INSPECTOR_BLOCK_ORDER_KEY = "gabarit_inspector_band_order_v63"
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        """Palette contextuelle V63 : vrai rail vertical à droite.

        Au repos, seule une colonne d'icônes de taille confortable reste visible.
        Le groupe survolé se déploie vers la gauche dans la partie transparente du
        calque, sans déplacer la page ni les autres outils. Aucun texte explicatif
        permanent : seulement le titre court du panneau ouvert et, après attente,
        une infobulle discrète sur l'icône réellement survolée.
        """
        ...
        # Rail : une icône par ligne, suffisamment grande pour rester lisible.
        right = width - 7.0
        rail_w = 50.0
        rail_x1 = right - rail_w
        gap_y = 4.0
        available = max(330.0, height - 20.0)
        row_h = min(52.0, max(42.0, (available - gap_y * (len(order)-1)) / max(1, len(order))))
        total_h = len(order)*row_h + max(0,len(order)-1)*gap_y
        y = max(8.0, min(14.0, (height-total_h)/2.0))
        target.create_rectangle(rail_x1-2.0, y-4.0, right+1.0, min(height-6.0,y+total_h+4.0), fill="#141E24", outline="#34464F", width=1)
        ...
        band_icons = {
            "create":"band_create",
            "allowed":{"margins":"frame_margins","page":"frame_page","bleed":"frame_bleed"}.get(current_frame,"frame_page"),
            "snap":{"margins":"snap_margins","page":"snap_page","center":"snap_center","zones":"snap_zones","none":"snap_none"}.get(current_snap,"snap_none"),
            "align":"align_center",
            "layer":"layer_forward",
            "actions":"unlock" if locked else "lock",
            "scope":"scope",
            "model":"duplicate",
            "geometry":"geometry",
        }
        ...
        for name in order:
            ...
            target.create_rectangle(rail_x1, y1, right, y2, fill=fill, outline=outline, width=2 if is_active else 1)
            icon_box = (rail_x1+5.0, y1+4.0, right-5.0, y2-4.0)
            self._gabarit_inspector_draw_icon(target, band_icons[name], icon_box, selected=is_active, hovered=hovered_band)
            # Petite marque latérale : état ouvert sans ajouter de texte.
            if is_active:
                target.create_line(rail_x1+1.0,y1+6.0,rail_x1+1.0,y2-6.0,fill="#57C2B8",width=3)
            key=f"dragblock_{name}"
            self._gabarit_inspector_hitboxes[key]=(rail_x1,y1,right,y2)
            # Pas d'infobulle sur le rail : le panneau porte son titre.
            y += row_h + gap_y
        ...
        fy1=max(7.0,min(height-panel_h-7.0,fy1))
        fy2=fy1+panel_h
        ...
        content_top=fy1+title_h+5.0
        content_bottom=fy2-5.0

        def icon_strip(items, *, button=39.0, gap=7.0, y_offset=0.0):
            count=len(items)
            total=count*button+max(0,count-1)*gap
            start_x=fly_left+(fly_w-total)/2.0
        ...
        if active_band == "create":
            ...
            icon_strip(tuple(items),button=41.0,gap=12.0)
            if pending in {"margins","page","bleed"} and pending != current:
                yy=content_top+47.0
                gap=7.0; bw=(fly_w-gap-20.0)/2.0
                x1=fly_left+10.0
                self._gabarit_inspector_control_box(target,"align_scope_page",(x1,yy,x1+bw,yy+28))
                target.create_text(x1+bw/2.0,yy+14.0,text="Cette page",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
                x2=x1+bw+gap
                self._gabarit_inspector_control_box(target,"align_scope_type",(x2,yy,x2+bw,yy+28))
                target.create_text(x2+bw/2.0,yy+14.0,text="Toutes du type",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
            ...
            pending_scope=str(getattr(self,"_gabarit_scope_pending","") or "")
            gap=8.0; bw=(fly_w-gap-20.0)/2.0; yy=content_top+5.0
            for i,(key,label,selected) in enumerate((
                ...
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+31),selected=selected)
                target.create_text(x1+bw/2.0,yy+15.5,text=label,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold" if selected else "normal"))
            ...
            for i,(key,label,disabled) in enumerate((("copy_template","Copier",False),("apply_template","Appliquer",not has_copy))):
                x1=fly_left+10.0+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+31),disabled=disabled)
                target.create_text(x1+bw/2.0,yy+15.5,text=label,anchor="center",fill="#D8E0E1" if not disabled else "#65737A",font=(theme.FONT_UI,8,"bold"))

        elif active_band == "geometry":
            fields=(("X","x","geom_x"),("Y","y","geom_y"),("L","w","geom_w"),("H","h","geom_h"))
            gap=5.0; bw=(fly_w-gap*3.0-20.0)/4.0; yy=content_top+3.0
            for i,(lab,field,key) in enumerate(fields):
                x1=fly_left+10.0+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+32))
                target.create_text(x1+6.0,yy+16.0,text=lab,anchor="w",fill="#7F959A",font=(theme.FONT_UI,8,"bold"))
                target.create_text(x1+bw-6.0,yy+16.0,text=f"{float(rect.get(field,0)):g}",anchor="e",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
            if bool(info.get("spread")):
                ...
                for i,(lab,key,pos) in enumerate((("G","spread_left","left"),("C","spread_center","center"),("D","spread_right","right"))):
                    x1=fly_left+10.0+i*(bw2+3.0)
                    self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw2,yy+27),selected=current==pos)
                    target.create_text(x1+bw2/2.0,yy+13.5,text=lab,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
