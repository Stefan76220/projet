...
class PageMaitreV2(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # DEMARRAGE_ACCUEIL_SANS_SURSAUT_V1
        # La fenêtre reste masquée pendant la construction complète.
        # Cela évite d'afficher successivement la taille initiale,
        # la disparition du ruban puis l'agrandissement Windows.
        self.withdraw()
        try:
        ...
        # La fenêtre n'est rendue visible qu'une fois l'Accueil totalement
        # construit et la taille Windows définitive appliquée.
    ...
    def _build_shell(self) -> None:
        ...
        # Sur l'Accueil le ruban n'existe pas : on ne le mappe donc
        # pas au démarrage. show_screen() l'affichera seulement lorsqu'un
        # bureau de projet sera réellement ouvert.
        ...
        # Aucun rendu anticipé du ruban tant que l'Accueil est actif.
    ...
    def _header_tool(
        ...
        button = self._tomelinea_rounded_button(
            box,
            text=icon,
            command=command,
    ...
    def show_screen(self, name: str) -> None:
        ...
        if name == "accueil" and previous != "accueil":
            self._reset_accueil_project_selection()

        self._active = name

        # ACCUEIL_DERNIER_PROJET_CENTRE_V1
        # L'Accueil est volontairement hors projet :
        # le ruban de navigation n'y est jamais affiché.
        if name == "accueil":
    ...
    def open_manage_window(self) -> None:
        ...
        self._tomelinea_rounded_button(
            shell,
            text="Fermer",
            command=win.destroy,
            relief="flat",
    ...
    # ACCUEIL_FENETRES_COMPACTES_V3
    # TRAITS_NOIRS_ET_BOUTONS_ARRONDIS_GLOBAUX_V6
    def _accueil_base_shell(
        ...
        *,
        ...
        # Même fond éditorial que l'Accueil, visible autour de la zone.
        ...
        # Titre + trait noir : même construction dans les deux fenêtres.
        title_row = tk.Frame(body, bg="#FFFEFC")
        ...
        def draw_title_line(event=None):
            title_line.delete("all")
            width = max(
                20,
                int(event.width) if event is not None else title_line.winfo_width(),
            )
            y = 9
            end_x = max(8, width - 8)
        ...
        title_line.after_idle(draw_title_line)
    ...
    def _tomelinea_rounded_button(
        ...
        **_ignored,
        ...
        """Bouton TomeLinea unique : arrondi, relief, survol et pression."""
        disabled = str(state) == "disabled" or command is None

        bg_text = str(bg or "").lower()
        fg_text = str(fg or "").lower()
        if primary is None:
            primary = (
                fg_text in ("#ffffff", "white")
                and bg_text not in ("#ffffff", "white", "#fffdfc", "")
            )
        ...
        show_chevron = (
            ...
            and raw_text.strip().lower() not in ("fermer", "annuler")
            and len(raw_text.strip()) > 1
        )

        char_width = 8
        calculated = max(88, len(raw_text) * char_width + 42)
        if show_chevron:
            calculated += 18
        ...
        if width is not None:
            try:
                wv = int(width)
                if wv <= 60:
                    calculated = max(calculated, wv * 8 + 24)
                else:
                    calculated = max(calculated, wv)
            except Exception:
                pass
        ...
        canvas = tk.Canvas(
            ...
            highlightthickness=0,
            bd=0,
            cursor=("arrow" if disabled else (cursor or "hand2")),
            takefocus=0,
        )

        canvas._tomelinea_button_role = "primary" if primary else "secondary"
        canvas._tomelinea_button_command = command
        canvas._tomelinea_button_disabled = disabled
        canvas._tomelinea_button_state = "normal"
        ...
        def draw():
            canvas.delete("all")
            current = canvas._tomelinea_button_state
            yoff = 1 if current == "pressed" and not disabled else 0
            ...
            if disabled:
                fill = "#E9E7E3"
                outline = "#D2CEC8"
                text_color = disabledforeground or "#92979B"
                shadow = "#DEDAD4"
            elif primary:
                fill = "#173E70"
                if current == "hover":
                    fill = "#24558A"
                elif current == "pressed":
                    fill = "#12345F"
                outline = fill
                text_color = "#FFFFFF"
                shadow = "#D2CEC8"
            ...
            round_rect(
                ...
                fill=fill,
                outline=outline,
                width=0 if primary and not disabled else 1,
            )
            ...
            if show_chevron:
                canvas.create_text(
                    ...
                    (y1 + y2) / 2 + yoff,
                    text="›",
                    fill="#FFFFFF" if primary else "#56708A",
                    font=("Segoe UI", 16),
                    anchor="center",
                )
        ...
        def on_enter(_event=None):
            if disabled:
                return
            canvas._tomelinea_button_state = "hover"
            draw()
        ...
        def on_press(_event=None):
            if disabled:
                return
            canvas._tomelinea_button_state = "pressed"
            draw()
        ...
        def on_release(event):
            ...
            canvas._tomelinea_button_state = "hover" if inside else "normal"
            draw()
    ...
    def _accueil_make_dialog_button(
        ...
        *,
        ...
        return self._tomelinea_rounded_button(
            parent,
            text=text,
            command=command,
            primary=primary,
            width=max(0, int(min_width)),
            font=("Segoe UI", 9, "bold"),
        )
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def render(_event=None):
            ...
            # Partie gauche
            # -------------------------
            last_title_id = canvas.create_text(
                X(145), Y(470),
            ...
            last_title_box = canvas.bbox(last_title_id)
            last_line_x1 = (
                last_title_box[2] + X(18)
                if last_title_box is not None
                else X(320)
            )
            last_line_x2 = X(770)
            canvas.create_line(
                last_line_x1, Y(482), last_line_x2, Y(482),
                fill="#111111",
                width=1,
            )
            canvas.create_oval(
                last_line_x2 - 3, Y(479),
                last_line_x2 + 3, Y(485),
                fill="#FFFDFC",
                outline="#111111",
                width=1,
            )
            ...
            # Partie droite
            # -------------------------
            shortcuts_title_id = canvas.create_text(
                X(855), Y(470),
            ...
            shortcuts_title_box = canvas.bbox(shortcuts_title_id)
            shortcuts_line_x1 = (
                shortcuts_title_box[2] + X(18)
                if shortcuts_title_box is not None
                else X(1055)
            )
            shortcuts_line_x2 = X(1445)
            canvas.create_line(
                shortcuts_line_x1, Y(482), shortcuts_line_x2, Y(482),
                fill="#111111",
                width=1,
            )
            canvas.create_oval(
                shortcuts_line_x2 - 3, Y(479),
                shortcuts_line_x2 + 3, Y(485),
                fill="#FFFDFC",
                outline="#111111",
                width=1,
            )
