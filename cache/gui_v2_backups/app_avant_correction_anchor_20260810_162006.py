...
class PageMaitreV2(tk.Tk):
    ...
    def _build_shell(self) -> None:
        ...
        # Zone gauche : nom du bureau + outils de bureau.
        ...
        self.header_title.pack(anchor="w", pady=(0, 8))

        quick = tk.Frame(left, bg="#FBFAF6")
        quick.pack(anchor="w")

        self._header_visualisation = self._header_tool(
            quick,
            "◉",
            "Visualisation",
            self.open_visualisation_window,
        )
        self._header_visualisation.pack(side="left", padx=(0, 20))

        self._header_follow = self._header_tool(
            quick,
            "⌁",
            "Suivi du livre",
            None,
        ...
        # Navigation centrale persistante.
        ...
        for key, icon, label, accent in theme.NAV_ITEMS:
            ...
            btn.pack(side="left", fill="y", expand=True, padx=1)
            self._nav_buttons[key] = btn

        # Droite.
        right = tk.Frame(self.header, bg="#FBFAF6")
        right.place(relx=1.0, x=-202, y=10, width=190, height=105)

        self.project_label = tk.Label(
            right,
            text="projet x  ·  Accueil",
            bg="#FBFAF6",
            fg=theme.MUTED,
            font=("Segoe UI", 8),
            anchor="e",
        )
        self.project_label.pack(fill="x", pady=(0, 10))
        ...
        tk.Frame(
            ...
            height=1,
            bg=theme.BORDER,
        ).place(relx=0, rely=1, relwidth=1, y=-1)
    ...
    def show_screen(self, name: str) -> None:
        ...
        self.header_title.configure(text=labels.get(name, name.title()))
        self.project_label.configure(
            text=f"projet x  ·  {labels.get(name, name.title())}"
        )

        # Outils gauche : actifs/inactifs selon le bureau.
        on_accueil = name == "accueil"
        self._set_header_tool_state(
            self._header_visualisation,
            enabled=not on_accueil,
        )
        self._set_header_tool_state(
            self._header_follow,
            enabled=False,
        ...
        for key, btn in self._nav_buttons.items():
            ...
            if key == name:
                btn.configure(
                    bg=self._mix(accent, "#FFFFFF", 0.70),
                    fg=theme.INK,
        ...
        *,
        ...
        for child in box.winfo_children():
            if isinstance(child, tk.Button):
                ...
                child.configure(
                    fg=theme.MUTED if enabled else "#9AA0A6",
                )
