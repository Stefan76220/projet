...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V7 : C contient les briques ; B devient la surface de pose."""
        ...
        def show_structure_modal(
            ...
            *,
            ...
            """Surimpression TomeLinea avec le vrai fond général et le vrai logo."""
            ...
            # Vrai habillage Accueil : grand monogramme + vrai titre couleur.
            icon = self._home_brand_icon_photo(92)
            title_photo = self._home_brand_title_photo(320)
            if icon is not None:
                self._structure_modal_brand_refs.append(icon)
                tk.Label(overlay, image=icon, bg=theme.WINDOW_DEEP, bd=0).place(x=30, y=18)
            if title_photo is not None:
                self._structure_modal_brand_refs.append(title_photo)
                tk.Label(overlay, image=title_photo, bg=theme.WINDOW_DEEP, bd=0).place(relx=0.5, y=20, anchor="n")
            tk.Label(
                overlay, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE",
                bg=theme.WINDOW_DEEP, fg=theme.MUTED_DARK,
                font=(theme.FONT_UI, 8, "bold"),
            ).place(relx=0.5, y=62, anchor="n")

            card = tk.Frame(
                overlay, bg=theme.PANEL_ALT,
                highlightthickness=1, highlightbackground=theme.BORDER,
            )
            card.place(relx=0.5, rely=0.54, anchor="center", width=width, height=height)
            card.grid_columnconfigure(0, weight=1)
            ...
            tk.Frame(card, bg=theme.ACCENT_DARK, height=1).grid(row=1, column=0, sticky="ew", padx=24)

            body = tk.Frame(card, bg=theme.PANEL_ALT)
            body.grid(row=2, column=0, sticky="nsew", padx=24, pady=14)
            body.grid_columnconfigure(0, weight=1)
            body.grid_rowconfigure(0, weight=1)
            if message:
                tk.Label(
                    body, text=message, bg=theme.PANEL_ALT, fg=theme.INK,
                    justify="left", anchor="w", wraplength=max(300, width - 100),
                    font=(theme.FONT_UI, 9),
            ...
            foot.grid(row=3, column=0, sticky="ew", padx=24, pady=(8, 16))
        ...
        for text_, command_ in (
            ("Nommer", rename_selected),
            ("Dupliquer", duplicate_selected),
            ("Supprimer", delete_selected),
            ("Annuler outil", cancel_tool),
        ):
            command_button(action_bar, text_, command_, danger=(text_ == "Supprimer")).pack(side="left", padx=(0, 5))
        ...
        palette_hint_var = tk.StringVar(value="Choisissez une brique puis placez-la directement dans B")
        tk.Label(
        ...
        def make_brick(parent_, label: str, kind: str, payload, *, locked=False, subtitle="brique de structure"):
            """Carte-matière TomeLinea : objet à prendre, visuellement distinct d'un bouton."""
            base = "#203831"
            hover = "#294A40"
            active = "#31594B"
            locked_bg = "#2A3035"
            ...
            card = tk.Frame(
                ...
                highlightthickness=1, highlightbackground="#46505A" if locked else border,
                width=132, height=62,
            )
            card.pack_propagate(False)
            edge = tk.Frame(card, bg="#70767A" if locked else gold, width=4)
            edge.pack(side="left", fill="y")
            content = tk.Frame(card, bg=locked_bg if locked else base, padx=8, pady=6)
            content.pack(side="left", fill="both", expand=True)
            icon = tk.Label(
                content, text="◇" if not locked else "▣",
                bg=locked_bg if locked else base,
                fg=theme.MUTED if locked else gold,
                font=(theme.FONT_UI, 11, "bold"),
            )
            icon.pack(side="left", padx=(0, 7))
            texts = tk.Frame(content, bg=locked_bg if locked else base)
            texts.pack(side="left", fill="both", expand=True)
            name_lbl = tk.Label(
                texts, text=label, bg=locked_bg if locked else base,
                fg=theme.MUTED if locked else theme.INK,
                font=(theme.FONT_UI, 8, "bold"), anchor="w",
                wraplength=92, justify="left",
            )
            name_lbl.pack(fill="x")
            sub_lbl = tk.Label(
                texts, text="protégé" if locked else subtitle,
                bg=locked_bg if locked else base,
                fg=theme.MUTED_DARK if locked else "#8CB4A4",
                font=(theme.FONT_UI, 6), anchor="w",
            )
            sub_lbl.pack(fill="x", pady=(2, 0))

            children = (card, content, icon, texts, name_lbl, sub_lbl)
            if locked:
            ...
            def enter(_e=None):
                if active_brick.get("widget") is card:
                    return
                paint(hover, theme.ACCENT_BRIGHT)
        ...
        def all_type_entries():
            # Types réellement présents dans TomeLinea. Les couvertures existent mais restent structurellement protégées.
            protected = [
                ("couverture", "Couverture", True),
                ("deuxieme_couverture", "2e de couverture", True),
                ("troisieme_couverture", "3e de couverture", True),
                ("quatrieme_couverture", "4e de couverture", True),
            ]
            seen = {key for key, _label, _locked in protected}
            result = list(protected)
            result.append(("page_blanche", "Page blanche", False))
            seen.add("page_blanche")
            for key, label, _custom in page_types():
                if key not in seen:
                    result.append((key, label, False))
                    seen.add(key)
            return result
        ...
        def create_custom_type():
            ...
            def make_photo(path: Path, size=(180, 250)):
                try:
            ...
            def build_body(body):
                ...
                def field(row, label, key):
                    tk.Label(
                        form, text=label, bg=theme.PANEL_ALT, fg=theme.MUTED,
                        font=(theme.FONT_UI, 8, "bold"),
                    ).grid(row=row, column=0, sticky="w", padx=(0, 12), pady=6)
                    entry = tk.Entry(
                        form, bg=theme.WINDOW_DEEP, fg=theme.INK,
                        insertbackground=theme.ACCENT_BRIGHT,
                        relief="flat", bd=0, font=(theme.FONT_UI, 9),
                    )
                ...
                tk.Label(
                    form, text="Image de base", bg=theme.PANEL_ALT, fg=theme.MUTED,
                    font=(theme.FONT_UI, 8, "bold"),
                ).grid(row=2, column=0, sticky="nw", padx=(0, 12), pady=(12, 6))
                ...
                def choose_library():
                    if library_holder.winfo_manager():
                        close_library()
                        return
                    library_holder.pack(fill="x", pady=(8, 0))

                for label, command in (
                    ("Bibliothèque", choose_library),
                    ("Image extérieure", choose_external),
                    ("Aucune image", choose_none),
                ):
                    command_button(controls, label, command).pack(side="left", padx=(0, 5))
                tk.Label(
                    image_area, textvariable=selected_image_var,
                    bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                    font=(theme.FONT_UI, 8),
                ).pack(anchor="w", pady=(6, 0))

                tk.Label(
                    library_holder, text="Bibliothèque TomeLinea",
                    bg=theme.WINDOW_DEEP, fg=theme.ACCENT_BRIGHT,
                    font=(theme.FONT_UI, 8, "bold"),
                ).pack(anchor="w", padx=8, pady=(8, 4))
                lib_grid = tk.Frame(library_holder, bg=theme.WINDOW_DEEP)
                lib_grid.pack(fill="x", padx=8, pady=(0, 8))
                row_i = col_i = 0
                for lib_label, path in library_candidates():
                    tile = tk.Frame(lib_grid, bg=theme.PANEL_SOFT, padx=6, pady=5, highlightthickness=1, highlightbackground=theme.BORDER)
                    tile.grid(row=row_i, column=col_i, padx=4, pady=4, sticky="nsew")
                    photo = make_photo(path, (54, 76))
                    if photo is not None:
                        img_lbl = tk.Label(tile, image=photo, bg=theme.PANEL_SOFT)
                        img_lbl.image = photo
                        img_lbl.pack(side="left")
                    txt_lbl = tk.Label(tile, text=lib_label, bg=theme.PANEL_SOFT, fg=theme.INK, font=(theme.FONT_UI, 7, "bold"))
                    txt_lbl.pack(side="left", padx=(5, 0))
                    def select_lib(_e=None, p=path, title_=lib_label):
                        state["preview_image"] = p
                        selected_image_var.set(title_)
                        close_library()
                        refresh_preview()
                ...
                tk.Label(
                    form, text="Propriétés", bg=theme.PANEL_ALT, fg=theme.MUTED,
                    font=(theme.FONT_UI, 8, "bold"),
                ).grid(row=3, column=0, sticky="nw", padx=(0, 12), pady=(12, 6))
                options = tk.Frame(form, bg=theme.PANEL_ALT)
                options.grid(row=3, column=1, sticky="w", pady=(12, 6))
                tk.Checkbutton(
                    options, text="Page automatique",
                    variable=state["automatic"], bg=theme.PANEL_ALT, fg=theme.INK,
                ...
                tk.Label(
                    form,
                    text="Automatique peut être activé pour tous les types sauf les quatre couvertures.",
                    bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                    font=(theme.FONT_UI, 7), anchor="w",
            ...
            def confirm_new_type():
                ...
                created = canvas.structure_create_custom_type({
                    ...
                    "automatic": bool(state["automatic"].get()),
        ...
        def make_flow(parent_, entries, *, columns=7):
            ...
            for entry in entries:
                widget = entry(holder)
                widget.grid(row=r, column=c, padx=5, pady=5, sticky="nw")
                c += 1
                if c >= columns:
                    c = 0
                    r += 1
        ...
        def make_scroll_flow(parent_, entries, *, columns=7):
            holder = tk.Frame(parent_, bg=theme.PANEL_ALT)
            holder.pack(fill="both", expand=True)
        ...
        def build_rules():
            ...
            tk.Label(
                section, text="Pages automatiques",
                bg=theme.PANEL_ALT, fg=theme.ACCENT_BRIGHT,
                font=(theme.FONT_UI, 8, "bold"),
            ).pack(anchor="w", pady=(2, 7))
            tk.Label(
                section,
                text="Activez Automatique pour les types qui doivent pouvoir être générés par une règle. Les quatre couvertures restent exclues.",
                bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                font=(theme.FONT_UI, 7), anchor="w",
            ).pack(anchor="w", pady=(0, 8))
            grid = tk.Frame(section, bg=theme.PANEL_ALT)
            grid.pack(fill="x")
            col = row = 0
            for type_key, label, locked in all_type_entries():
                if locked:
                    continue
                allowed = True
                if canvas is not None and hasattr(canvas, "structure_type_automatic_allowed"):
                    allowed = bool(canvas.structure_type_automatic_allowed(type_key))
                if not allowed:
                    continue
                enabled = bool(canvas.structure_type_is_automatic(type_key)) if canvas is not None and hasattr(canvas, "structure_type_is_automatic") else False
                ...
                toggle = tk.Checkbutton(
                    ...
                    padx=8, pady=5, font=(theme.FONT_UI, 7, "bold"), cursor="hand2",
                )
                def changed(key=type_key, variable=var):
                    current = book()
                    if current is not None and hasattr(current, "structure_set_type_automatic"):
                        current.structure_set_type_automatic(key, bool(variable.get()))
                toggle.configure(command=changed)
                toggle.grid(row=row, column=col, padx=4, pady=4, sticky="w")
                col += 1
                if col >= 5:
                    col = 0
                    row += 1
        ...
        def select_family(name: str):
            ...
            if name == "Pages":
                entries = []
                entries.append(lambda parent_: make_brick(parent_, "Page sans type", "page", {"type": "", "label": "Sans type"}, subtitle="page libre"))
                entries.append(lambda parent_: make_brick(parent_, "Page blanche", "page", {"type": "page_blanche", "label": "Page blanche"}, subtitle="page volontaire"))
                ...
                make_flow(palette_body, entries, columns=7)
                palette_hint_var.set("Une brique armée suit la souris dans B ; cliquez pour la poser. Un clic ailleurs annule.")

            elif name == "Parties":
                row = tk.Frame(palette_body, bg=theme.PANEL_ALT)
                row.pack(anchor="nw", fill="x")
                make_brick(row, "Nouvelle partie", "group", {"type": "partie", "label": "Nouvelle partie"}, subtitle="brique de partie").pack(side="left", padx=(0, 8))
                command_button(row, "Renommer la sélection", rename_selected).pack(side="left", padx=(0, 6), pady=4)
                command_button(row, "Supprimer la partie vide", delete_selected, danger=True).pack(side="left", padx=(0, 6), pady=4)
                palette_hint_var.set("La future partie suit la souris entre les parties de B ; cliquez à l’endroit voulu.")
                ...
                for type_key, label, locked in all_type_entries():
                    entries.append(
                        lambda parent_, k=type_key, l=label, locked_=locked: make_brick(
                            parent_, l, "page", {"type": k, "label": l},
                            locked=locked_, subtitle="type de page"
                        )
                    )
                make_scroll_flow(palette_body, entries, columns=7)
                palette_hint_var.set("Types réellement disponibles. Les couvertures sont visibles mais protégées. Molette : défilement vertical.")

            elif name == "Règles":
                build_rules()
                palette_hint_var.set("Structure : propriétés automatiques et règles du squelette.")
        ...
        # Lorsque B consomme ou annule une brique, son état visuel revient à zéro.
