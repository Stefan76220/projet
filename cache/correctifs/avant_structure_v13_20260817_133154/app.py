...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V12 : atelier C compact, briques sur deux lignes, page auto source -> cible."""
        WORK_BG = "#293A3D"
        WORK_BG_2 = "#243438"
        WORK_TILE = "#31474B"
        ...
        def make_brick(parent_, label, type_key, *, custom=False):
            # Une seule brique TomeLinea : même forme, même matière, même couleur.
            # Seuls l'icône et le nom du type changent.
            width, height = 132, 44
            card = tk.Canvas(
            ...
            def draw(fill=WORK_TILE, outline=WORK_BORDER, active=False):
                ...
                # Petit logement d'icône, identique pour chaque type.
                card.create_rectangle(
                    9, 10, 31, 32,
                    fill=WORK_BG_2, outline=WORK_GOLD, width=1,
                )
                card.create_text(
                    20, 21, text=brick_icon(type_key), fill=WORK_GOLD,
                    font=(theme.FONT_UI, 9, "bold"),
                )
                card.create_text(
                    39, height/2, text=display, anchor="w",
            ...
            def enter(_e=None):
                if active_brick.get("widget") is not card:
                    draw(WORK_TILE_HOVER, WORK_TEAL)
            def leave(_e=None):
                if active_brick.get("widget") is not card:
                    normal()
            def press(_e=None):
                draw("#223236", WORK_TEAL)
                return "break"
            def release(_e=None):
                reset_active_brick()
        ...
        def rebuild_catalog():
            ...
            # Neuf briques par ligne : avec le catalogue actuel, deux lignes
            # sont réellement utilisées et restent visibles. Les suivantes
            # défilent verticalement à la molette.
            columns = 9
        ...
        # --------------------------------------------------------------
        # RÈGLES : sélectionner une page dans B ne fait que renseigner sa
        # source. Aucun changement n'est enregistré avant « Appliquer ».
        # --------------------------------------------------------------
        rules = tk.Frame(atelier, bg=WORK_BG, pady=4)
        ...
        tk.Label(
            ...
            font=(theme.FONT_UI, 8, "bold"),
        ).grid(row=0, column=0, sticky="nw", padx=(0, 10), pady=(5, 0))

        rule_line = tk.Frame(rules, bg=WORK_BG)
        rule_line.grid(row=0, column=1, sticky="ew")
        rule_line.grid_columnconfigure(8, weight=1)

        source_var = tk.StringVar(value="Page choisie : sélectionnez une page dans B")
        target_var = tk.StringVar(value="Page auto : choisir…")
        info_var = tk.StringVar(value="")
        scope = {"value": "local"}
        target_key = {"value": None}  # None = aucun choix encore effectué
        scope_labels = {}
        detach_ref = {"widget": None}
        last_source = {"index": None}

        source_label = tk.Label(
            rule_line, textvariable=source_var,
            bg=WORK_BG_2, fg=WORK_TEXT,
            font=(theme.FONT_UI, 8, "bold"), padx=9, pady=5,
            highlightthickness=1, highlightbackground=WORK_BORDER,
        )
        source_label.grid(row=0, column=0, sticky="w", padx=(0, 7))

        tk.Label(
            rule_line, text="→", bg=WORK_BG, fg=WORK_GOLD,
            font=(theme.FONT_UI, 10, "bold"),
        ).grid(row=0, column=1, padx=(0, 7))
        ...
        target_mb.grid(row=0, column=2, sticky="w", padx=(0, 10))
        target_menu = tk.Menu(
            target_mb, tearoff=False,
            bg=WORK_BG_2, fg=WORK_TEXT,
            activebackground=WORK_TILE_HOVER, activeforeground=WORK_TEXT,
        )
        ...
        def choose_target(key, label):
            ...
            target_var.set(f"Page auto : {label if key else 'Aucune'}")
        ...
        scope_chip("local", "Cette page").grid(row=0, column=3, padx=(0, 5))
        scope_chip("general", "Toutes les pages de ce type").grid(row=0, column=4, padx=(0, 9))

        def refresh_info(ctx=None):
            if ctx is None:
                ctx = selected_context()
            if not ctx.get("selected"):
                info_var.set("Sélectionnez une page dans la ligne.")
                return
            ...
            if general:
                text_ = f"Règle générale : {source_name} → {labels.get(general, general)}"
                if mode == "none":
                    text_ += "  •  cette page est désolidarisée"
                elif mode == "local":
                    local = str(ctx.get("local_target") or "")
                    text_ += f"  •  exception : {labels.get(local, local)}"
                info_var.set(text_)
            elif mode == "local":
                local = str(ctx.get("local_target") or "")
                info_var.set(f"Association locale : {source_name} → {labels.get(local, local)}")
            else:
        ...
        def refresh_detach(ctx=None):
            ...
            if not ctx.get("selected") or not ctx.get("allowed") or not general:
                widget.grid_remove()
                return
            widget.configure(text="Réintégrer cette page" if mode == "none" else "Désolidariser cette page")
            widget.grid()
        ...
        def refresh_rules():
            ...
            ctx = selected_context()
            if not ctx.get("selected"):
                source_var.set("Page choisie : sélectionnez une page dans B")
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = None
                target_var.set("Page auto : choisir…")
                refresh_info(ctx)
                refresh_detach(ctx)
                return

            if not ctx.get("allowed"):
                source_var.set(f"Page choisie : {ctx.get('page_label', 'page')} — non applicable")
                target_mb.configure(state="disabled", cursor="arrow")
                target_key["value"] = None
                target_var.set("Page auto : non applicable")
                refresh_info(ctx)
                refresh_detach(ctx)
                return
            ...
            index = ctx.get("index")
            source_name = str(ctx.get("page_label") or ctx.get("source_type") or "Page")
            source_var.set(f"Page choisie : {source_name}")
            target_mb.configure(state="normal", cursor="hand2")
            scope_labels["general"].configure(text=f"Toutes les pages « {source_name} »")

            # À chaque nouvelle page choisie, on repart de l'état réellement
            # enregistré. Une sélection seule ne crée et ne modifie rien.
            if last_source["index"] != index:
                ...
                if mode == "local":
                    ...
                    target_var.set(f"Page auto : {labels.get(current, current)}" if current else "Page auto : Aucune")
                elif mode == "none":
                    target_key["value"] = ""
                    target_var.set("Page auto : Aucune")
                else:
                    current = str(ctx.get("general_target") or "")
                    if current:
                        target_key["value"] = current
                        target_var.set(f"Page auto : {labels.get(current, current)}")
                    else:
                        target_key["value"] = None
                        target_var.set("Page auto : choisir…")
        ...
        def apply_rule():
            ...
            if target_key["value"] is None:
                try:
                    canvas.status_var.set("Choisissez d’abord la page automatique à associer.")
                except Exception:
                    pass
                return
        ...
        def detach_or_rejoin():
            ...
            if not str(ctx.get("general_target") or ""):
                return
            if str(ctx.get("mode") or "inherit") == "none":
        ...
        command_chip(
            rule_line, "Appliquer", apply_rule,
            accent=WORK_TEAL, compact=False,
        ).grid(row=0, column=5, padx=(0, 6))

        detach_ref["widget"] = command_chip(
            rule_line, "Désolidariser cette page", detach_or_rejoin,
            accent=WORK_BLUE, compact=True,
        )
        detach_ref["widget"].grid(row=0, column=6, padx=(0, 8))

        tk.Label(
            rule_line, textvariable=info_var,
            bg=WORK_BG, fg=WORK_MUTED,
            font=(theme.FONT_UI, 7), anchor="e",
