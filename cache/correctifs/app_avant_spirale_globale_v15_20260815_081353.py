...
# SPIRALE_FINE_PROPORTIONS_V14
    ...
    def _draw_notebook_spiral_canvas(
        ...
        x_draw = int(round(x - (image.width() * 0.58)))
        ...
        if not hasattr(canvas, "_tl_spiral_refs_v14"):
            canvas._tl_spiral_refs_v14 = []
        canvas._tl_spiral_refs_v14.append(image)
        return item


    def _attach_notebook_spiral_frame(self, frame, accent=None):
        if getattr(frame, "_tl_spiral_bound_v14", False):
            return
        ...
        frame._tl_spiral_holder_v14 = holder
        ...
        def refresh(_event=None):
            ...
            x_pos = -int(round(image.width() * 0.58))
        ...
        frame._tl_spiral_bound_v14 = True

    def _get_tomelinea_spiral_image(self, target_height, zone_bg="#333845"):
        if not hasattr(self, "_tl_spiral_cache_v14"):
            self._tl_spiral_cache_v14 = {}
        ...
        cached = self._tl_spiral_cache_v14.get(cache_key)
        if cached is not None:
            return cached
        ...
        source_path = globals().get("TOMELINEA_SPIRAL_MODEL_V13", globals().get("TOMELINEA_SPIRAL_MODEL"))
        if source_path is None:
            raise RuntimeError("Image source de la spirale introuvable.")

        source = Image.open(source_path).convert("RGBA")
        src_w, src_h = source.size

        # Retour à une proportion fine, proche du rendu du matin.
        ratio = src_w / float(src_h)
        target_width = max(18, int(round(target_height * ratio * 0.92)))
        resized = source.resize((target_width, target_height), Image.LANCZOS)

        zone_rgb = _hex_to_rgb(zone_bg)
        band_left = int(round(resized.width * 0.43))
        band_right = int(round(resized.width * 0.78))
        px = resized.load()

        for yy in range(resized.height):
            for xx in range(band_left, min(resized.width, band_right + 1)):
                r, g, b, a = px[xx, yy]
                if a == 0:
                    continue
        ...
        self._tl_spiral_cache_v14[cache_key] = tk_image
        return tk_image
