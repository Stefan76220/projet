# TOMELINEA_GABARITS_LIBERTE_V68
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "create":
            ...
            target.create_text(fly_left+11.0,content_top+53.0,text="VERROUILLAGE",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            locked=all(bool(z.get("locked",False)) for z in self._gabarit_selected_zone_dicts()) if self._gabarit_selected_zone_dicts() else bool(info.get("locked",False))
            icon_strip((("zone_lock","unlock" if locked else "lock",locked,"Déverrouiller la sélection" if locked else "Verrouiller la sélection",False),),button=43.0,gap=10.0,y_offset=63.0,center=False,start_x=fly_left+11.0)
    ...
    def _gabarit_inspector_band_for_key(self, key: str) -> str:
        ...
        mapping = {
            ...
            "zone_lock":"snap", "zone_duplicate":"actions", "zone_delete":"actions",
