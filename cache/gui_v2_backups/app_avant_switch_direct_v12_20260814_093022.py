...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V11_REDRAW_WIN32_BLOQUE
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # Deux géométries finales, sans animation entre elles.
        # Vue globale reste panoramique ; Feuilleter retrouve son format compact.
        global_size = (
        ...
        # ----------------------------------------------------------
        # Changement de vue V11.
        #
        # Les deux vues restent préconstruites. Sous Windows, on bloque
        # temporairement le redessin natif de la Toplevel pendant l'unique
        # changement de géométrie. La nouvelle vue est terminée avant de
        # réautoriser Windows à peindre.
        # ----------------------------------------------------------
        switch_state = {
            "active": False,
        }

        def _freeze_native_redraw():
            """Bloque le repaint Win32 de la Toplevel si disponible."""
            try:
                import ctypes
        ...
        def _resume_native_redraw(token):
            """Réactive le repaint et force un seul rendu complet."""
            if token is None:
                return
        ...
        def show_tab(key, *, animate=True):
            ...
            redraw_token = _freeze_native_redraw()
            ...
            try:
                ...
                # Une seule modification de géométrie.
                win.geometry(
                    f"{final_w}x{final_h}"
                    f"+{final_x}+{final_y}"
                )

                # Tk calcule la mise en page pendant que Windows
                # n'a pas le droit de repeindre.
                win.update_idletasks()
                ...
                _resume_native_redraw(redraw_token)
        ...
        # Les deux vues sont déjà construites. Lors des clics,
        # une seule géométrie est appliquée sous verrou de redraw Win32.
        if state["tab"] == "global":
            global_host.tkraise()
        else:
            book_host.tkraise()
