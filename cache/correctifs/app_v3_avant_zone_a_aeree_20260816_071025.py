...
class TomeLineaV3(tk.Tk):
    ...
    def _build_zone_a(self):
        """Bandeau de chantier : identité, projet, travail actuel et commandes globales."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=1)

        # 1. Identité TomeLinea — même signature que l'Accueil, réduite.
        brand = tk.Frame(body, bg=theme.PANEL)
        brand.grid(row=0, column=0, sticky="nsw", padx=(0, 14))

        brand_icon = self._home_brand_icon_photo(52) if hasattr(self, "_home_brand_icon_photo") else None
        ...
        brand_title = self._home_brand_title_photo(192) if hasattr(self, "_home_brand_title_photo") else None
        ...
        signature = tk.Canvas(
            brand_text, width=194, height=11, bg=theme.PANEL,
            bd=0, highlightthickness=0,
        )
        signature.pack(anchor="w")
        ...
        signature.create_line(168, 9, 176, 3, fill="#E7C37A", width=2)
        signature.create_line(176, 9, 183, 2, fill="#E47A45", width=2)
        signature.create_line(183, 9, 190, 4, fill="#86A98C", width=2)

        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=1, sticky="ns", padx=(4, 16)
        )

        # 2. Où suis-je ? Projet + type et icône.
        project = tk.Frame(body, bg=theme.PANEL)
        project.grid(row=0, column=2, sticky="nsw", padx=(0, 22))
        tk.Label(
        ...
        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=3, sticky="ns", padx=(0, 20)
        )

        # 3. Travail réel du moment, sans faux pourcentage d'avancement.
        current = tk.Frame(body, bg=theme.PANEL)
        current.grid(row=0, column=4, sticky="nsew", padx=(0, 18))
        current.grid_columnconfigure(0, weight=1)
        tk.Label(
            current, text="TRAVAIL ACTUEL", bg=theme.PANEL,
            fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 7, "bold"),
        ).grid(row=0, column=0, sticky="w")

        self.workspace_current_var = tk.StringVar(value="STRUCTURE")
        tk.Label(
            current, textvariable=self.workspace_current_var, bg=theme.PANEL,
            fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 10, "bold"),
        ).grid(row=1, column=0, sticky="w", pady=(1, 0))

        tk.Label(
            current, textvariable=self.book_canvas.status_var, bg=theme.PANEL,
        ...
        # 4. Commandes permanentes et commandes de fenêtre TomeLinea.
        actions = tk.Frame(body, bg=theme.PANEL)
        actions.grid(row=0, column=6, sticky="e")

        info = tk.Label(
            actions, text="ⓘ", bg=theme.PANEL, fg=theme.MUTED,
            font=(theme.FONT_UI, 11, "bold"), cursor="hand2", padx=5,
        )
        info.pack(side="left", padx=(0, 2))
        info.bind("<Button-1>", lambda _e: self._show_zone_info("a"))

        V3Button(actions, "Accueil", self.show_home, compact=True).pack(side="left", padx=2)
        V3Button(actions, "Aide", self._show_general_help, compact=True).pack(side="left", padx=2)

        sep = tk.Frame(actions, bg=theme.BORDER_SOFT, width=1, height=30)
        sep.pack(side="left", padx=6)

        minimize = V3Button(actions, "—", self._window_minimize, compact=True)
        minimize.configure(fg=theme.ACCENT_BRIGHT, width=2)
        minimize.pack(side="left", padx=2)

        self.zone_a_size_button = V3Button(actions, "❐", self._window_toggle_size, compact=True)
        self.zone_a_size_button.configure(fg=theme.ACCENT_BRIGHT, width=2)
        self.zone_a_size_button.pack(side="left", padx=2)

        close = V3Button(actions, "✕", self.destroy, compact=True)
        close.configure(fg="#E47A45", activeforeground=theme.WHITE, width=2)
        close._hover_bg = "#654039"
