...
class BookCanvas(tk.Frame):
    ...
    def __init__(
        ...
        *,
        ...
        # Navigation de la page : Espace + glisser ou clic-molette + glisser.
        # Le déplacement visuel est fait directement sur les éléments déjà dessinés
        # pour éviter le clignotement provoqué par un render() complet à chaque pixel.
        self._gabarit_space_pan: bool = False
        self._gabarit_pan_visual_offset: tuple[float, float] = (0.0, 0.0)
        self._gabarit_drag_render_job = None
    ...
    def _build_canvas(self):
        ...
        # Gabarits : le rail gauche reste visuellement transparent, mais il n'est
        # plus une fenêtre Toplevel indépendante. Sous Windows, cette ancienne
        # fenêtre native à clé chromatique devait être recomposée par le DWM chaque
        # fois que la page bougeait dessous : c'était la source du clignotement qui
        # persistait malgré la suppression des render() pendant le panoramique.
        #
        # V31 : le rail devient un enfant réel de B. On applique la clé de couleur
        # directement aux petits widgets enfants qui utilisent le fond du rail.
        # La page et les outils appartiennent donc à la même hiérarchie de fenêtre.
        self._gabarit_tools_transparent_key = "#102633"
        self._gabarit_tools_overlay = False
        self._gabarit_tools_layered_child = False
        try:
        ...
        host = tk.Frame(
            viewer, bg=self._gabarit_tools_transparent_key,
        ...
        # Windows 8+ autorise WS_EX_LAYERED sur les fenêtres enfants. Après que
        # _build_gabarits_tab a créé les labels/frames, on rend transparents les
        # seuls widgets dont le fond est exactement la clé du rail. Boutons/cartes
        # gardent leur propre fond opaque et restent parfaitement interactifs.
        self._gabarit_tools_sync_job = None
        if windowing == "win32":
            self._gabarit_tools_layered_child = True
            try:
                self.after_idle(self._apply_gabarit_child_transparency)
            except Exception:
                self._gabarit_tools_layered_child = False
    ...
    def _windows_colorkey(self, color: str) -> int:
        """Convertit #RRGGBB vers COLORREF (0x00BBGGRR)."""
    ...
    def _make_widget_colorkey_transparent(self, widget) -> bool:
        """Applique LWA_COLORKEY à un widget enfant Windows, sans Toplevel."""
        try:
            ...
            setter(hwnd, GWL_EXSTYLE, style | WS_EX_LAYERED)
            ok = bool(user32.SetLayeredWindowAttributes(
                hwnd, self._windows_colorkey(self._gabarit_tools_transparent_key),
                255, LWA_COLORKEY,
            ))
            user32.SetWindowPos(
                hwnd, 0, 0, 0, 0, 0,
    ...
    def _apply_gabarit_child_transparency(self) -> None:
        """Rend transparents les fonds du rail dans la même fenêtre que le Canvas.

        On ne touche qu'aux widgets dont le bg correspond à la clé. Le traitement
        est récursif car l'atelier contient plusieurs Frames et Labels imbriqués.
        """
        if not bool(getattr(self, "_gabarit_tools_layered_child", False)):
            return
        host = getattr(self, "gabarit_tools_host", None)
        if host is None:
            return
        key = str(getattr(self, "_gabarit_tools_transparent_key", "#102633")).lower()
        try:
        ...
        while stack:
            ...
            try:
                ...
                bg = ""
            if bg == key and self._make_widget_colorkey_transparent(widget):
                applied += 1
        # Si l'API n'est pas disponible, on garde simplement le rail enfant opaque :
        # mieux vaut perdre l'effet que réintroduire la fenêtre Toplevel clignotante.
        if applied <= 0:
            self._gabarit_tools_layered_child = False

    def _schedule_gabarit_tools_overlay_sync(self, _event=None):
        # Compatibilité avec les appels historiques : V31 n'utilise plus de Toplevel.
        return None
    ...
    def _show_gabarit_tools_host(self):
        ...
        try:
            ...
            host.lift()
            if bool(getattr(self, "_gabarit_tools_layered_child", False)):
                self.after_idle(self._apply_gabarit_child_transparency)
        except Exception:
            pass
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # Tous les éléments créés depuis la feuille (ombre, guides, zones et
        # flèches accolées) forment un seul groupe visuel déplaçable. Pendant un
        # panoramique on translate ce groupe sans effacer/reconstruire le Canvas.
        try:
    ...
    def _gabarit_cancel_drag_render(self) -> None:
        job = getattr(self, "_gabarit_drag_render_job", None)
        self._gabarit_drag_render_job = None
        if job is not None:
            try:
                self.after_cancel(job)
    ...
    def _gabarit_schedule_drag_render(self) -> None:
        """Regroupe les rafraîchissements de zone pendant un glisser.

        Les souris modernes peuvent envoyer bien plus de 60 événements/seconde.
        Redessiner tout B sur chacun d'eux crée un clignotement perceptible.
        """
        if getattr(self, "_gabarit_drag_render_job", None) is not None:
            return
        def run():
            self._gabarit_drag_render_job = None
            if getattr(self, "_work_mode", "") == "gabarits":
                self.render()
        try:
            self._gabarit_drag_render_job = self.after(16, run)
        except Exception:
            self.render()
    ...
    def _gabarit_space_press(self, _event=None):
        ...
        if self._gabarit_zone_drag is None:
            try:
                self.canvas.configure(cursor="fleur")
            except Exception:
                pass
    ...
    def _gabarit_middle_release(self, event=None):
        if getattr(self, "_work_mode", "") != "gabarits":
            return None
        return self._gabarit_finish_pan(event)
    ...
    def _gabarit_pan_drag(self, event):
        ...
        old_dx, old_dy = getattr(self, "_gabarit_pan_visual_offset", (0.0, 0.0))
        ...
        if step_x or step_y:
            try:
                ...
                self._gabarit_schedule_drag_render()
            if self._gabarit_page_box is not None:
                x1, y1, x2, y2 = self._gabarit_page_box
                self._gabarit_page_box = (x1 + step_x, y1 + step_y, x2 + step_x, y2 + step_y)
        self._gabarit_pan_visual_offset = (visual_dx, visual_dy)
        return "break"

    def _gabarit_finish_pan(self, _event=None):
        if self._gabarit_drag_origin is None and self._gabarit_pan_origin is None:
            return "break"
        self._gabarit_drag_origin = None
        self._gabarit_pan_origin = None
        self._gabarit_pan_visual_offset = (0.0, 0.0)
        self._gabarit_cancel_drag_render()
        # Un seul rendu final recalcule hitboxes, flèches et limites après le geste.
        self.render()
        try:
    ...
    def _gabarit_press(self, event):
        try:
            ...
            pass
        x, y = float(event.x), float(event.y)
        # Espace + clic-glisser a priorité sur les zones : on déplace la vue,
        # même lorsque le pointeur est exactement au-dessus d'une zone Texte/Image.
    ...
    def _gabarit_release(self, _event=None):
        ...
        if self._gabarit_drag_origin is not None or self._gabarit_pan_origin is not None:
            return self._gabarit_finish_pan(_event)
        self.canvas.configure(cursor="fleur" if self._gabarit_space_pan else "arrow")
        return "break"
    ...
    def _on_hover_motion(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            # Pendant un glisser, aucun survol ne doit déclencher un render() :
            # cela annulait le déplacement visuel direct et produisait le flash
            # observé sur la vidéo.
            if self._gabarit_drag_origin is not None or self._gabarit_zone_drag is not None:
                try:
                    self.canvas.configure(cursor="fleur" if self._gabarit_drag_origin is not None else "arrow")
                except Exception:
                    pass
                return
            x, y = float(event.x), float(event.y)
