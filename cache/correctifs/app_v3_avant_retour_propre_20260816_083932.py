...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # COMMANDES_FENETRE_NATIVES_SEULES_V3_V1
        # La fenêtre principale reste gérée par Windows. Le cadre visuel sera
        # retiré sans utiliser overrideredirect.
        ...
        # Couche fenêtre isolée : A/B/C ne sont ni reconstruits ni modifiés.
        self._install_native_window_shell()
        self._set_native_restore_geometry()

        # Ouverture directement dans le vrai mode maximisé Windows.
        self.deiconify()
        self._window_native_maximize()
    ...
    def _native_window_api(self):
        """API Win32 typée utilisée uniquement par les trois commandes de fenêtre."""
        import ctypes
    ...
    def _native_hwnd(self):
        """Handle de la fenêtre de niveau supérieur gérée par Windows."""
        try:
            import sys
            if sys.platform != "win32":
                return None
            _ctypes, wintypes, user32 = self._native_window_api()
            self.update_idletasks()
            child = wintypes.HWND(int(self.winfo_id()))
            parent = user32.GetParent(child)
            return int(parent or child.value or 0)
        except Exception:
            return None

    def _install_native_window_shell(self):
        """Masque seulement la légende, sans sortir la fenêtre du gestionnaire Windows."""
        try:
            import sys
            if sys.platform != "win32":
                return
    ...
    def _set_native_restore_geometry(self):
        """Prépare une vraie taille restaurée avant l'ouverture maximisée."""
        try:
            import sys
            if sys.platform != "win32":
                return

            ctypes, _wintypes, user32 = self._native_window_api()
            ...
            work_w = max(1180, rect.right - rect.left)
            work_h = max(720, rect.bottom - rect.top)

            width = min(work_w, max(1180, int(work_w * 0.82)))
            height = min(work_h, max(720, int(work_h * 0.84)))
    ...
    def _native_system_command(self, command: int, *, immediate: bool = False):
        """Envoie la même commande système que les boutons de légende Windows."""
        try:
            import sys
            if sys.platform == "win32":
                _ctypes, wintypes, user32 = self._native_window_api()
                ...
                if hwnd_value:
                    ...
                    if immediate:
                        user32.SendMessageW(hwnd, WM_SYSCOMMAND, command, 0)
                    else:
    ...
    def _window_native_minimize(self):
        self._native_system_command(0xF020)

    def _window_native_maximize(self):
        self._native_system_command(0xF030, immediate=True)
        self.after(80, self._sync_native_size_button)

    def _window_native_toggle_size(self):
        try:
            _ctypes, wintypes, user32 = self._native_window_api()
            hwnd_value = self._native_hwnd()
            maximized = bool(
                hwnd_value and user32.IsZoomed(wintypes.HWND(hwnd_value))
            )
        except Exception:
            maximized = False

        if maximized:
            self._native_system_command(0xF120)
        else:
            self._native_system_command(0xF030)

        self.after(100, self._sync_native_size_button)

    def _window_native_close(self):
        self._native_system_command(0xF060)

    def _sync_native_size_button(self):
        button = getattr(self, "native_size_button", None)
        if button is None:
            return
    ...
    def _build_zone_a(self):
        ...
        # Seule modification de A : les trois commandes de fenêtre.
        minimize = V3Button(
            actions, "—", self._window_native_minimize, compact=True
        )
        self.native_size_button = V3Button(
            actions, "❐", self._window_native_toggle_size, compact=True
        )
        close = V3Button(
            actions, "✕", self._window_native_close, compact=True
        )

        for index, button in enumerate(
            (minimize, self.native_size_button, close)
        ):
            button.configure(
                width=3,
                padx=0,
            ...
            button.pack(
                side="left",
                padx=((10 if index == 0 else 3), 0),
            )

        self.after(120, self._sync_native_size_button)
