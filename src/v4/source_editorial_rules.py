...
def _looks_like_part_opening(
    ...
    # Marqueurs structurels directement écrits dans le livre.
    if re.match(
        r"^(partie|part|section|livre|chapitre|chapter|interlude)"
        r"(?:\b|\s|[-–—:])",
        first,
        flags=re.IGNORECASE,
    ):
    ...
    # Formes naturelles : « Première partie », « Deuxième partie »...
    if re.match(
        ...
        r"dixi(?:e|è)me)\s+(partie|part|section|livre)\b",
        first,
        flags=re.IGNORECASE,
    ):
        return True

    # Forme éditoriale fréquente : numéro romain seul, puis titre.
    # Exemple : « II » / « Les cartes noyées ».
    if (
        len(lines) >= 2
        and word_count <= 30
        and re.fullmatch(
    ...
    def propose(
        ...
        last_page_number = max(
            (
                ...
                for _target_id, facts
                in ordered
            ),
            default=0,
        )

        facts_by_page_number = {
            int(
                _value(
                    page_facts,
                    "page.number",
        ...
        for (
            ...
            # ==================================================
            # Quatrième de couverture probable
            # ==================================================
                ...
                result.append(
                    _proposal(
                        ...
                        rationale=(
                            ...
                            "avec illustration et texte court à moyen : "
                            "quatrième de couverture probable."
                        ),
                        facts=facts,
                        signals=signals,
            ...
            next_facts = facts_by_page_number.get(
                page_number + 1
            )
            ...
            # Un titre très court repris immédiatement comme en-tête
            # sur une page de texte longue est une forte signature
            # de chapitre, même sans le mot « Chapitre ».
            repeated_heading_opening = (
                1 <= word_count <= 30
                and image_count == 0
                and table_count == 0
                and 1 <= len(lines) <= 3
                ...
                and next_word_count >= 80
            )

            if (
                _looks_like_part_opening(
                    text,
                    word_count,
                )
                or repeated_heading_opening
            ):
                result.append(
                    _proposal(
                        context=context,
                        ...
                        rationale=(
                            "Marqueur structurel explicite ou titre "
                            "court repris comme en-tête de la page "
                            "de texte suivante."
                        ),
                        facts=facts,
                        signals=signals,
