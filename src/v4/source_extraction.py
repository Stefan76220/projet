...
def _pdf_text_placements(
    page,
    text_blocks,
    list_structures,
) -> list[dict[str, Any]]:
    """Construit des zones texte factuelles à partir des blocs PDF.

    PyMuPDF peut réunir dans un même bloc plusieurs petits libellés placés
    sur la même ligne mais très éloignés horizontalement. Dans ce cas,
    TomeLinea conserve des zones distinctes afin que l'inventaire et la
    sélection correspondent réellement à ce qui est visible.

    Les lignes appartenant à une structure de liste déjà reconnue (par
    exemple le sommaire titre + numéro de page) restent volontairement
    groupées : la structure Source décrit déjà leur relation.
    """

    list_block_indexes: set[int] = set()
    for structure in list_structures if isinstance(list_structures, list) else []:
        if not isinstance(structure, dict):
            continue
        for item in structure.get("items", []) if isinstance(structure.get("items", []), list) else []:
            if not isinstance(item, dict):
                continue
            try:
                list_block_indexes.add(int(item.get("block_index")))
    ...
    for block_index, block in enumerate(text_blocks):
        ...
        for line_index, line in enumerate(block.get("lines", [])):
            ...
            for span_index, span in enumerate(line.get("spans", [])):
                ...
                sx0, sy0, sx1, sy1 = tuple(
                    float(value) for value in span.get("bbox", (0, 0, 0, 0))
                )
                spans_data.append(
                    {
                        ...
                        "bbox_norm": [
                            round(sx0 / page.rect.width, 6),
                            round(sy0 / page.rect.height, 6),
                            round(sx1 / page.rect.width, 6),
                            round(sy1 / page.rect.height, 6),
                        ],
                    }
                )

            line_text = _clean_text("".join(line_parts))
            if not line_text:
                continue
        ...
        split_same_row = False
        if block_index not in list_block_indexes and len(line_records) == 2:
            centers = [
                ((record["bbox"][1] + record["bbox"][3]) / 2.0) / page.rect.height
                for record in line_records
            ]
            same_horizontal_band = max(centers) - min(centers) <= 0.006
            ordered = sorted(line_records, key=lambda record: record["bbox"][0])
            gaps = [
                (ordered[index + 1]["bbox"][0] - ordered[index]["bbox"][2]) / page.rect.width
                for index in range(len(ordered) - 1)
            ]
            clearly_separated = bool(gaps) and min(gaps) >= 0.04
            split_same_row = same_horizontal_band and clearly_separated

        if split_same_row:
            for sub_index, record in enumerate(
                sorted(line_records, key=lambda item: item["bbox"][0])
            ):
                placements.append(
                    {
                        "block_index": block_index,
                        "block_sub_index": sub_index,
        ...
        placements.append(
            {
                ...
                "text": _clean_text(" ".join(record["text"] for record in line_records)),
                "spans": [
                    span
                    for record in line_records
...
def _detect_pdf_list_structures(
    page,
    raw_blocks,
) -> list[dict[str, Any]]:

    """
    Detecte des structures de liste observables.

    Important :
    - une petite liste locale est conservee comme fait ;
    - elle ne signifie pas que la PAGE est de type Liste ;
    - le caractere dominant sera decide plus tard.
    """
        ...
        structures.append(
            {
                "kind":
                    (
                        "checklist"
                        if bullet_rows
                        and all(bool(row.get("is_checkbox")) for row in bullet_rows)
                        else "bulleted_or_numbered"
                    ),
    ...
    #
    # On cherche :
    # - beaucoup de blocs ayant exactement deux lignes ;
    # - lignes sur le meme axe vertical ;
    # - premiere colonne alignee a gauche ;
    # - seconde colonne alignee a droite ;
    # - seconde valeur courte.
    # --------------------------------------------------------
    ...
    for block_index, block in enumerate(
        ...
        if (
            same_row
            and separated_columns
            and short_right_value
        ):

            two_column_rows.append(
                {
                    "block_index":
                        block_index,
...
def _extract_pdf(
    ...
    try:
        ...
        for index in range(page_count):
            ...
            # --------------------------------------------------
            # Blocs texte réellement placés.
            # --------------------------------------------------

            text_placements = _pdf_text_placements(
                page,
                text_blocks,
                list_structures,
            )
            ...
            # --------------------------------------------------
            # Tableaux reels detectes par PyMuPDF.
            #
            # On ne deduit jamais un tableau de simples
            # alignements de texte : find_tables() doit avoir
            # effectivement reconnu une structure tabulaire.
            # --------------------------------------------------

            table_placements: list[
                dict[str, Any]
            ] = []
            ...
            recorder.add(
                **common,
                key="layout.text_blocks",
                value=text_placements,
            )

            recorder.add(
                **common,
                key="layout.list_count",
                value=len(
                    list_structures
                ),
            )
            ...
            recorder.add(
                **common,
                key="layout.lists",
                value=list_structures,
            )

            recorder.add(
                **common,
                key="layout.table_count",
                value=len(
                    table_placements
                ),
            )
