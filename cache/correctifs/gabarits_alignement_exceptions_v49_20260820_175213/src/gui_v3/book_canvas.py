...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        target.create_text(tx, y, text="ALIGNEMENT", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 17
    ...
    def gabarit_snap_preset(self) -> str:
        if not bool(getattr(self, "_gabarit_snap_enabled", True)):
            return "none"
        preset = str(self.gabarit_snap_profile().get("preset") or "page").strip().lower()
        return preset if preset in {"page", "margins", "center", "zones"} else "page"
    ...
    def gabarit_align_selected_zones(self, mode: str) -> bool:
        ...
        if any(bool(z.get("locked", False)) for z in zones):
            ...
            return False
        min_x=min(float(z.get("x",0)) for z in zones); min_y=min(float(z.get("y",0)) for z in zones)
        max_x=max(float(z.get("x",0))+float(z.get("w",0)) for z in zones)
        max_y=max(float(z.get("y",0))+float(z.get("h",0)) for z in zones)
        cx=(min_x+max_x)/2.0; cy=(min_y+max_y)/2.0
        mode=str(mode or "").lower()
        for z in zones:
            w=float(z.get("w",.2)); h=float(z.get("h",.2))
            if mode=="left": z["x"]=min_x
            elif mode in {"center","hcenter"}: z["x"]=cx-w/2.0
            elif mode=="right": z["x"]=max_x-w
            elif mode=="top": z["y"]=min_y
            elif mode in {"middle","vcenter"}: z["y"]=cy-h/2.0
            elif mode=="bottom": z["y"]=max_y-h
            else: return False
            self._gabarit_mark_edited(self._gabarit_active_item())
        self._save_order(); self.render(); self._emit_gabarit_selection_changed()
        self.status_var.set(f"{len(zones)} zones alignées")
        return True
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            threshold_x = max(0.003, min(0.035, 8.0 / pw))
            threshold_y = max(0.003, min(0.035, 8.0 / ph))
            ...
            if state["mode"] == "resize":
                ...
                right = snap_edge(ox + nw, x_refs, threshold_x)
                bottom = snap_edge(oy + nh, y_refs, threshold_y)
                ...
                nx = snap_origin(nx, ow, x_refs, threshold_x)
                ny = snap_origin(ny, oh, y_refs, threshold_y)
