...
class BookCanvas(tk.Frame):
    ...
    def _production_draw_page_state_halo(
        ...
        """Halo d'avancement autour d'une page Production.

        Vierge : aucun halo. En cours : orange. Validée : vert.
        Le halo est dessiné avant la feuille : l'intérieur reste donc propre.
        """
        if str(getattr(self, "_work_mode", "") or "") != "production":
            return
        ...
        if (
            Image is not None and ImageDraw is not None and ImageFilter is not None
            and hasattr(target, "image") and getattr(target, "image", None) is not None
        ):
            try:
                overlay = Image.new("RGBA", target.image.size, (0, 0, 0, 0))
                draw = ImageDraw.Draw(overlay)
                pad = 5
                draw.rectangle(
                    (int(round(x1-pad)), int(round(y1-pad)), int(round(x2+pad)), int(round(y2+pad))),
                    outline=rgba, width=7,
                )
                glow = overlay.filter(ImageFilter.GaussianBlur(radius=10))
                base = Image.alpha_composite(target.image.convert("RGBA"), glow)
                target.image.paste(base.convert("RGB"))
                target.create_rectangle(x1-3, y1-3, x2+3, y2+3, fill="", outline=edge, width=2)
                return
            except Exception:
                pass
        target.create_rectangle(x1-9, y1-9, x2+9, y2+9, fill="", outline=edge, width=2)
        target.create_rectangle(x1-5, y1-5, x2+5, y2+5, fill="", outline=edge, width=2)
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float, *, update_inspector: bool = True) -> None:
        ...
        # Statut Production sur la grande page : halo indépendant de la ligne.
        if str(getattr(self, "_work_mode", "") or "") == "production":
            if spread:
