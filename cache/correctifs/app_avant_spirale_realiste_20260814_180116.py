...
class PageMaitreV2(tk.Tk):
    ...
    def _draw_notebook_spiral_canvas(
        ...
        *,
        ...
        if y2 <= y1 + 14:
            return
        shadow = self._mix(color, '#000000', 0.42)
        highlight = self._mix(color, '#FFFFFF', 0.14)
        y = y1 + 10
        while y <= y2 - 8:
            canvas.create_oval(
                x - rx + 1,
    ...
    def _attach_notebook_spiral_frame(self, frame, color: str) -> None:
        ...
        spring.place(x=0, y=7, width=14, relheight=1.0)

        def redraw(_event=None):
            spring.delete('all')
            h = max(24, spring.winfo_height())
            self._draw_notebook_spiral_canvas(
                spring,
                5,
                6,
        ...
        # CORRECTION_RESSORT_CANVAS_RAISE_TCL_V2
        # Canvas redéfinit tkraise() pour ses objets internes ;
        # on appelle donc directement la commande Tk de fenêtre.
