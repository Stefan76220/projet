...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_inspector_draw_icon(self, target, icon: str, box, *, selected=False, danger=False, disabled=False, hovered=False):
        ...
        if icon in {"undo", "redo"}:
            # Même grammaire visuelle : trait 2 px, flèche ouverte, taille optique commune.
            c = teal if hovered and not disabled else fg
            direction = -1 if icon == "undo" else 1
            target.create_arc(cx-10, cy-9, cx+10, cy+9, start=35 if direction < 0 else 145, extent=250, style="arc", outline=c, width=2)
            hx = cx-9 if direction < 0 else cx+9
            target.create_line(hx, cy-5, hx-5*direction, cy-5, fill=c, width=2)
            target.create_line(hx, cy-5, hx, cy-10, fill=c, width=2)
            return
