...
def write_phase1_report(
    ...
    *,
    ...
    summary_lines = [
        ...
        f"Polices manquantes : {', '.join(fonts.get('missing_families', [])) or 'aucune'}",
        "",
    ...
    font_rows = []
    for item in fonts.get("availability", []):
        font_rows.append(
            "<tr>"
            f"<td>{_e(item.get('family'))}</td>"
            f"<td>{_e(item.get('status'))}</td>"
            f"<td>{_e(item.get('installed_family'))}</td>"
    ...
    captured = model.get("future_editorial_support", {}).get("captured_for_later", [])
...
<div class="card"><b>Polices manquantes</b><br>{escape(', '.join(fonts.get('missing_families', [])) or 'aucune')}</div>
</div>
...
<table><thead><tr><th>Section</th><th>Paragraphes</th><th>Format</th><th>Orientation</th><th>Marges</th><th>Rupture</th></tr></thead><tbody>{''.join(section_rows)}</tbody></table>
<h2>Polices</h2>
<table><thead><tr><th>Police demandée</th><th>État</th><th>Police trouvée</th><th>Origine</th></tr></thead><tbody>{''.join(font_rows)}</tbody></table>
...
<h2>Faits conservés pour les futures règles éditoriales</h2><ul>{captured_html}</ul>
