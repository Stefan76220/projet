...
class TomeLineaV4Survol(
    ...
    def _survol_buffer_page_changed(self, host, event: dict) -> None:
        ...
        # Canvas Editor publie page_changed juste après avoir lancé le recentrage
        # embarqué. On garde encore l'ancienne page visible quelques millisecondes
        # afin que la transformation de centrage soit effectivement posée avant
        # l'échange des WebView.
        self._survol_schedule_buffer_reveal(delay_ms=90)
    ...
    def _survol_use_buffer(
        ...
        try:
            ...
            # Même en cas de retour JS muet, le fallback temporisé permet de ne pas
            # bloquer le Survol.
            pass

        # Fallback : sur certaines configurations WebView2 l'événement page_changed
        # peut être regroupé. 180 ms laisse deux à plusieurs frames au recentrage.
        self._survol_schedule_buffer_reveal(delay_ms=180)
