...
"""
    ...
    def _composition_render_book_state_panel(self, host) -> None:
        ...
        tk.Label(
            host,
            text="Format, marges et texte courant sont appliqués ensemble.",
        ...
        def compact_field(parent, label, key, suffix="", width=7):
            ...
            entry = tk.Entry(
                ...
                relief="flat", bd=0, justify="right", font=(theme.FONT_UI, 8),
            )
        ...
        # FORMAT — menu hiérarchique dans le panneau, jamais de Toplevel.
        section("FORMAT")
        format_label = tk.StringVar()
        self._global_settings_format_label = format_label
        def refresh_format_label():
            item = STANDARD_FORMATS_BY_KEY.get(vars_map["format_key"].get())
            if item is None:
                format_label.set("Choisir un format")
            else:
                format_label.set(item.display_label)
        def select_format(key):
            vars_map["format_key"].set(str(key))
            refresh_format_label()
        selector = tk.Menubutton(
            host, textvariable=format_label, anchor="w",
            bg=theme.PANEL_SOFT, fg=theme.INK,
            activebackground=theme.PANEL_ALT, activeforeground=theme.WHITE,
            relief="flat", bd=0, padx=9, pady=7, wraplength=285, justify="left",
