# TOMELINEA_GABARITS_ICONES_ASSETS_V95 — icônes légèrement réduites + bouton capsule disque plein
...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_draw_context_marker_tooltip(self, target, x: float, label: str) -> None:
        ...
        if not label:
            return
        half=max(64.0,min(105.0,22.0+len(label)*3.7))
        y1=73.0; y2=96.0
        target.create_rectangle(x-half,y1,x+half,y2,fill="#111B21",outline="#5D747B",width=1)
        target.create_text(x,(y1+y2)/2.0,text=label,anchor="center",fill="#DCE5E5",font=(theme.FONT_UI,7,"bold"))
        ...
        *, work_left: float = 12.0, work_right: float | None = None, target=None,
        ...
        for slot, x in enumerate(positions):
            ...
            if automatic:
                ...
                marker_label="Page automatique"
                self._gabarit_context_marker_hitboxes[marker_key]=((x-8,42,x+8,58),marker_label)
                if str(getattr(self,"_gabarit_hover_context_marker","") or "")==marker_key:
                    self._gabarit_draw_context_marker_tooltip(target,x,marker_label)
            else:
                if status == "termine":
                ...
                # V95 — chaque station de page possède désormais son propre commentaire
                # au survol, même lorsqu'elle n'a aucune exception particulière.
                if status == "termine":
                    status_label = "Page terminée"
                elif status == "en_cours":
                    status_label = "Page en cours"
                else:
                    status_label = "Page à faire"
                status_marker_key=f"{idx}:status"
                self._gabarit_context_marker_hitboxes[status_marker_key]=((x-radius-5,44-radius,x+radius+5,56+radius),status_label)
                if str(getattr(self,"_gabarit_hover_context_marker","") or "")==status_marker_key:
                    self._gabarit_draw_context_marker_tooltip(target,x,status_label)
                if self._gabarit_is_local_exception(item, idx):
                    ...
                    marker_label="Exception · Gabarit local"
                    self._gabarit_context_marker_hitboxes[marker_key]=((x-14,36,x+14,64),marker_label)
                    if str(getattr(self,"_gabarit_hover_context_marker","") or "")==marker_key:
                        self._gabarit_draw_context_marker_tooltip(target,x,marker_label)
                if self._gabarit_alignment_is_exception(item):
                    ...
                    marker_label="Exception · Cadre de placement"
                    self._gabarit_context_marker_hitboxes[marker_key]=((x-10,57,x+10,66),marker_label)
                    if str(getattr(self,"_gabarit_hover_context_marker","") or "")==marker_key:
                        self._gabarit_draw_context_marker_tooltip(target,x,marker_label)
            if active:
