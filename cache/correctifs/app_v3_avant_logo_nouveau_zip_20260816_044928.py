...
class TomeLineaV3(tk.Tk):
    ...
    def _build_home(self):
        ...
        brand_wrap.place(relx=0.00, rely=0.00, relwidth=0.62, relheight=1.00)

        logo_photo = self._logo_photo(392)
        if logo_photo is not None:
            brand_logo = tk.Label(brand_wrap, image=logo_photo, bg=theme.WINDOW_DEEP, bd=0)
            brand_logo.image = logo_photo
            brand_logo.place(x=0, y=3)
        else:
            tk.Label(
                brand_wrap, text="TL", bg=theme.WINDOW_DEEP, fg=theme.INK,
                font=(theme.FONT_TITLE, 38, "bold"),
            ).place(relx=0.00, rely=0.00)
            tk.Label(
                brand_wrap, text="TomeLinea", bg=theme.WINDOW_DEEP, fg=theme.INK,
                font=(theme.FONT_TITLE, 34),
            ).place(relx=0.085, rely=0.00)
            tk.Label(
                brand_wrap, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE",
        ...
        ).place(relx=0.405, rely=0.18)

        tools = tk.Frame(header, bg=theme.WINDOW_DEEP)
        tools.place(relx=0.72, rely=0.14, relwidth=0.28, relheight=0.52)
        help_label = tk.Label(
