...
class TomeLineaV3(tk.Tk):
    ...
    def _build_home(self):
        ...
        content.place(relx=0.05, rely=0.05, relwidth=0.90, relheight=0.90)
        content.grid_columnconfigure(0, weight=1)
        content.grid_rowconfigure(1, weight=1)

        # En-tête volontairement calme : identité + rôle de l'Accueil.
        ...
        tk.Label(
            header,
            text="Choisir son travail",
            bg=theme.WINDOW_DEEP,
            fg=theme.MUTED,
            font=(theme.FONT_UI, 9, "bold"),
        ).grid(row=0, column=1, sticky="e", pady=(4, 0))

        main = tk.Frame(content, bg=theme.WINDOW_DEEP)
        ...
        projects_panel.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        projects_body = projects_panel.body
        projects_body.grid_columnconfigure(0, weight=1)
        projects_body.grid_rowconfigure(1, weight=1)

        projects_header = tk.Frame(projects_body, bg=theme.PANEL)
        projects_header.grid(row=0, column=0, sticky="ew", pady=(0, 10))
        ...
        self.home_recent_list = tk.Frame(projects_body, bg=theme.PANEL)
        self.home_recent_list.grid(row=1, column=0, sticky="nsew")
        self.home_recent_list.grid_columnconfigure(0, weight=1)

        # --- Démarrer / Nouveau projet ----------------------------------
        start_panel = CutPanel(
            main,
            fill=theme.PANEL,
            border=theme.BORDER,
            cut=18,
        ...
        start_panel.grid(row=0, column=1, sticky="nsew", padx=(8, 0))
        start_body = start_panel.body
        start_body.grid_rowconfigure(0, weight=1)
        start_body.grid_columnconfigure(0, weight=1)

        self.home_start_frame = tk.Frame(start_body, bg=theme.PANEL)
        self.home_start_frame.grid(row=0, column=0, sticky="nsew")
        self.home_start_frame.grid_columnconfigure(0, weight=1)
        self.home_start_frame.grid_rowconfigure(1, weight=1)

        tk.Label(
            self.home_start_frame,
            text="Démarrer",
            bg=theme.PANEL,
            fg=theme.INK,
            font=(theme.FONT_TITLE, 16, "bold"),
        ...
        actions = tk.Frame(self.home_start_frame, bg=theme.PANEL)
        actions.grid(row=1, column=0, sticky="ew", pady=(20, 0))
        ...
        new_box.grid(row=0, column=0, sticky="ew", pady=(0, 8))
        new_box.grid_columnconfigure(0, weight=1)
        new_title = tk.Label(
            new_box,
            text="＋  Nouveau projet",
            bg=theme.ACCENT_DARK,
        ...
        new_title.grid(row=0, column=0, sticky="ew")
        new_sub = tk.Label(
            new_box,
            text="Créer un livre et recevoir le travail de l’auteur",
            bg=theme.ACCENT_DARK,
            fg="#DCEBE7",
        ...
        new_sub.grid(row=1, column=0, sticky="ew", pady=(4, 0))
        for widget in (new_box, new_title, new_sub):
        ...
        open_box.grid(row=1, column=0, sticky="ew", pady=(8, 0))
        open_box.grid_columnconfigure(0, weight=1)
        open_title = tk.Label(
            open_box,
            text="Ouvrir un projet TomeLinea",
            bg=theme.PANEL_ALT,
            fg=theme.INK,
            font=(theme.FONT_UI, 10, "bold"),
            anchor="w",
        )
        open_title.grid(row=0, column=0, sticky="ew")
        open_sub = tk.Label(
            open_box,
            text="Retrouver un projet existant",
            bg=theme.PANEL_ALT,
            fg=theme.MUTED,
            font=(theme.FONT_UI, 8),
            anchor="w",
        )
        open_sub.grid(row=1, column=0, sticky="ew", pady=(4, 0))
        for widget in (open_box, open_title, open_sub):
            widget.bind("<Button-1>", lambda _e: self.open_existing_project())

        # Le formulaire de création existe déjà dans l'Accueil ; aucune
        # fenêtre supplémentaire n'est créée.
        self.home_creator_frame = tk.Frame(start_body, bg=theme.PANEL)
        self.home_creator_frame.grid(row=0, column=0, sticky="nsew")
        self.home_creator_frame.grid_remove()
        self.home_creator_frame.grid_columnconfigure(0, weight=1)
        self.home_creator_frame.grid_rowconfigure(5, weight=1)

        creator_head = tk.Frame(self.home_creator_frame, bg=theme.PANEL)
        creator_head.grid(row=0, column=0, sticky="ew")
        creator_head.grid_columnconfigure(1, weight=1)
        V3Button(
            creator_head,
            "← Retour",
        ...
        type_box = tk.Frame(self.home_creator_frame, bg=theme.PANEL)
        type_box.grid(row=1, column=0, sticky="ew", pady=(14, 10))
        type_box.grid_columnconfigure(0, weight=1)

        type_labels = {
            "ouvrage_structure": ("Livre de fiches", "Fiches, recettes, catalogue…"),
            "livre_textuel": ("Livre textuel", "Roman, récit, essai…"),
            "bande_dessinee": ("Bande dessinée", "Planches, illustrations…"),
        }
        for row, key in enumerate(("ouvrage_structure", "livre_textuel", "bande_dessinee")):
            ...
            card.grid(row=row, column=0, sticky="ew", pady=(0 if row == 0 else 3, 0))
            card.grid_columnconfigure(1, weight=1)
            photo = self._type_icon(key, 38)
            icon = tk.Label(card, bg=theme.PANEL_ALT)
            if photo is not None:
                icon.configure(image=photo)
                icon.image = photo
            icon.grid(row=0, column=0, rowspan=2, padx=(0, 9))
            label = tk.Label(
                card,
                text=title,
                bg=theme.PANEL_ALT,
            ...
            sub.grid(row=1, column=1, sticky="ew", pady=(2, 0))
            for widget in (card, icon, label, sub):
                widget.bind("<Button-1>", lambda _e, k=key: self._home_select_type(k))
            self._home_type_frames[key] = card

        fields = tk.Frame(self.home_creator_frame, bg=theme.PANEL)
        fields.grid(row=2, column=0, sticky="ew", pady=(4, 0))
        ...
        self._home_source_count_var = tk.StringVar(value="Aucun fichier — vous pourrez le faire plus tard")
        ...
        tk.Entry(
            fields,
            textvariable=self._home_new_name_var,
            bg=theme.PANEL_ALT,
            fg=theme.INK,
        ...
        tk.Entry(
            fields,
            textvariable=self._home_new_location_var,
            bg=theme.PANEL_ALT,
            fg=theme.INK,
            insertbackground=theme.INK,
            relief="flat",
            bd=0,
            font=(theme.FONT_UI, 8),
        ).grid(row=1, column=1, sticky="ew", padx=(10, 4), ipady=5)
        V3Button(
            fields,
            "…",
            lambda: self._choose_location(self._home_new_location_var),
            compact=True,
        ).grid(row=1, column=2)
        ...
        source.grid(row=3, column=0, sticky="ew", pady=(12, 0))
        source.grid_columnconfigure(0, weight=1)
        tk.Label(
            source,
            text="Travail de l’auteur",
            bg=theme.PANEL_ALT,
            fg=theme.INK,
            font=(theme.FONT_UI, 8, "bold"),
            anchor="w",
        ...
        tk.Label(
            source,
            textvariable=self._home_source_count_var,
            bg=theme.PANEL_ALT,
            fg=theme.MUTED,
            font=(theme.FONT_UI, 7),
            anchor="w",
        ...
        V3Button(
            self.home_creator_frame,
            "Créer et ouvrir",
            self._home_create_project_inline,
            primary=True,
    ...
    def open_create_dialog(self):
        # V3 : aucune fenêtre modale. Le formulaire apparaît dans le panneau
        # Démarrer de l'Accueil et l'utilisateur garde son contexte visuel.
        self._home_new_name_var.set("Nouveau projet")
        self._home_new_location_var.set(str(Path.home() / "Documents"))
        self._home_new_type_var.set("ouvrage_structure")
        self._home_new_sources.clear()
        self._home_source_count_var.set("Aucun fichier — vous pourrez le faire plus tard")
        self._home_refresh_type_cards()
        self.home_start_frame.grid_remove()
        self.home_creator_frame.grid()
    ...
    def _refresh_home_recent(self):
        ...
        if not recent:
            ...
            empty.grid(row=0, column=0, sticky="ew", pady=(4, 0))
            tk.Label(
                empty,
            ...
            tk.Label(
                empty,
                text="Créez un projet ou ouvrez un projet TomeLinea existant.",
                bg=theme.PANEL_ALT,
                fg=theme.MUTED,
                font=(theme.FONT_UI, 8),
        ...
        for row, item in enumerate(recent):
            ...
            title = tk.Label(
                card,
                text=item.get("name", "Projet TomeLinea"),
                bg=theme.PANEL_ALT,
                fg=theme.INK,
                font=(theme.FONT_UI, 10, "bold"),
                anchor="w",
            ...
            kind = type_labels.get(item.get("type"), theme.PROJECT_TYPES.get(item.get("type"), "Projet TomeLinea"))
            kind_label = tk.Label(
                card,
                text=kind,
                bg=theme.PANEL_ALT,
            ...
            kind_label.grid(row=1, column=0, sticky="ew", pady=(3, 0))
            ...
            button = V3Button(
                card,
                "Reprendre",
                lambda p=path: self._open_recent_path(p),
                primary=True,
                compact=True,
            )
            button.grid(row=0, column=1, rowspan=3, sticky="e", padx=(12, 0))

            for widget in (card, title, kind_label, path_label):
                widget.bind("<Double-Button-1>", lambda _e, p=path: self._open_recent_path(p))
