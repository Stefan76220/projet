...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def render_recent_content(_event=None):
            ...
            top_y = 82
            text_x = 67
            row_tag = "recent_project_row"
            row_state = getattr(self, "_home_interaction_state", {}).get(
                row_tag, "normal"
            )

            if row_state in ("hover", "pressed"):
                home_round_rect(
                    canvas,
                    18, top_y - 28,
                    width - 135, top_y + 55,
                    ...
                    outline=self._mix(
                        "#7DBBA4",
                        "#FFFFFF",
                        0.35 if row_state == "pressed" else 0.55,
                    ),
                    width=2 if row_state == "pressed" else 1,
                    tags=("home_recent", row_tag),
                )
            ...
            secondary_tag = "recent_secondary_row"
            secondary_state = getattr(
                self, "_home_interaction_state", {}
            ).get(secondary_tag, "normal")

            if secondary_state in ("hover", "pressed"):
                canvas.create_line(
                    25, separator_y + 8,
                    25, separator_y + 29,
                    fill="#7DBBA4",
                    width=3 if secondary_state == "pressed" else 2,
                    tags=("home_recent", secondary_tag),
                )
