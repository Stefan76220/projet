...
class PageMaitreV2(tk.Tk):
    ...
    def _accueil_project_confirmation_window(
        ...
        *,
        ...
        # CONFIRMATION_ACCUEIL_BOUTONS_VISIBLES_V5
        win = tk.Toplevel(self)
        win.title(window_title)
        win.transient(self)
        win.resizable(False, False)
        self._accueil_center_base_window(win, 730, 570)

        body = self._accueil_base_shell(
            win,
            title="Confirmer le projet",
            subtitle=(
                "Ce choix définit le parcours du projet. "
                "Il ne sera plus redemandé ensuite."
            ),
            accent=accent,
        )

        # IMPORTANT : le bandeau d'actions est créé EN PREMIER et réservé
        # en bas de fenêtre. Ainsi il reste toujours visible, même avec
        # l'agrandissement Windows ou des polices plus grandes.
        actions = tk.Frame(
            body,
            bg="#FFFEFC",
            height=48,
        ...
        def validate_and_continue():
            # Le choix est mémorisé pour orienter plus tard automatiquement
            # vers la bonne Conception.
        ...
        tk.Button(
            ...
            bg="#FFFEFC",
            fg="#26313A",
            activebackground="#F2F4F6",
            activeforeground="#26313A",
            relief="solid",
            bd=1,
            padx=15,
            pady=8,
        ...
        choice.pack(fill="x", pady=(0, 11))

        tk.Label(
            choice,
            text="TYPE DE PROJET CHOISI",
            bg="#FAF8F4",
            fg="#6A737C",
            font=("Segoe UI", 8, "bold"),
        ).pack(anchor="w")

        title_row = tk.Frame(choice, bg="#FAF8F4")
        ...
        for point in points:
            line = tk.Frame(detail, bg="#F7F4EF")
        ...
        route_rows = (
            ("Maquettage", "Commun aux trois types de projet"),
            ("Conception", conception_name),
            (
                "Puis",
                "Assemblage → Vérification → Finalisation",
            ),
        )

        for label, value in route_rows:
            row = tk.Frame(route, bg="#FCFAF6")
    ...
    def _open_accueil_open_project_window(self):
        ...
        win.transient(self)
        win.resizable(False, False)
        self._accueil_center_base_window(win, 720, 500)
        ...
        body = self._accueil_base_shell(
            ...
            subtitle=(
                ...
                "Les commandes seront branchées ultérieurement."
            ),
            accent="#4A98D0",
        )
        ...
        path_row = tk.Frame(body, bg="#FFFEFC")
        ...
        self._accueil_base_secondary_label(
            path_row,
            "Parcourir…",
        ...
        for title, kind, date in (
            ("projet x", "Ouvrage structuré", "10/08/2026 15:23"),
            ("nouveau 1", "Ouvrage structuré", "07/08/2026 16:09"),
        ):
            row = tk.Frame(
                recent,
                bg="#FAF8F4",
        ...
        self._accueil_base_action_label(
            actions,
            "Ouvrir le projet",
    ...
    def _open_accueil_resume_project_window(self):
        ...
        win.transient(self)
        win.resizable(False, False)
        self._accueil_center_base_window(win, 680, 445)

        body = self._accueil_base_shell(
            win,
            title="Reprendre votre travail",
            subtitle=(
                "Retrouvez le dernier projet actif et le bureau "
                "sur lequel le travail s’est arrêté."
            ),
            accent="#8B6AB8",
        )
        ...
        tk.Label(
            project,
            text="Ouvrage structuré  •  22 pages",
            bg="#FAF8F4",
            fg="#5F6973",
            font=("Segoe UI", 9),
        ).pack(anchor="w", pady=(5, 10))

        details = tk.Frame(project, bg="#FAF8F4")
        details.pack(fill="x")
        ...
        for label, value in (
            ...
            line = tk.Frame(details, bg="#FAF8F4")
        ...
        tk.Label(
            body,
            text=(
                "La reprise directe au bon bureau sera connectée "
                "plus tard."
            ),
            bg="#FFFEFC",
            fg="#68717A",
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def render(_event=None):
            ...
            # ACCUEIL_DERNIER_PROJET_CENTRE_V1
            # Une seule zone remplace "Reprendre votre travail"
            # et "Repères & accès directs".
            cut_panel(
            ...
            canvas.create_text(
                X(145), Y(500),
                text=(
                    "Retrouvez le dernier ouvrage ouvert, son état "
                    "et les accès utiles pour poursuivre le travail."
                ),
                fill="#68717A",
                font=("Segoe UI", F(8)),
                anchor="nw",
            ...
            recent_photo = None
            try:
                cache_key = ("recent_last_project", 50)
            ...
            # Action principale : le Centre remplace l'ancien bouton Reprendre.
            draw_primary_button(
                "last_project_centre_button",
                "Centre du projet",
                X(505), Y(558),
                width=X(205),
            )
            bind_interaction(
                "last_project_centre_button",
                command=lambda: self.show_screen("centre"),
            )

            canvas.create_line(
                X(645), Y(520), X(645), Y(616),
                fill="#E1DDD7",
                width=1,
            )
            ...
            canvas.create_text(
                X(685), Y(535),
                text="Ouvrez directement un bureau du dernier projet actif.",
                fill="#68717A",
                font=("Segoe UI", F(7)),
                anchor="nw",
            )
            ...
            shortcuts = (
                ("maquettage", "Maquettage", "#579B73", 735),
                ("atelier", "Atelier", "#4C9ED0", 860),
                ("conception", "Conception", "#8A69BE", 985),
                ("assemblage", "Assemblage", "#C19A52", 1110),
                ("verification", "Vérification", "#4F8C7B", 1235),
                ("finalisation", "Finalisation", "#E06B4E", 1360),
            )

            for key, title, accent, cx0 in shortcuts:
                tag = f"shortcut_{key}"
                state = self._accueil_prof_state.get(tag, "normal")
                hover = state == "hover"
                pressed = state == "pressed"

                cy0 = 579 + (-2 if hover else (1 if pressed else 0))
                radius = 28 if hover else 26

                if hover:
                    canvas.create_oval(
                        X(cx0 - radius - 6), Y(cy0 - radius - 6),
                        X(cx0 + radius + 6), Y(cy0 + radius + 6),
                ...
                photo = self._get_nav_photo(key, state_name, 40)
                if photo is None:
                    photo = self._get_nav_photo(key, "normal", 40)
                if photo is not None:
                    canvas.create_image(
                        X(cx0), Y(cy0),
                        image=photo,
