...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self, *, startup_progress: Callable[[str, str], None] | None = None) -> None:
        ...
        # PRECHAUFFAGE_VISIONNEUR_INTERNE_V3_V1
        # Le vrai WebView du Visionneur est créé et chargé AVANT l'affichage
        # de TomeLinea. La fenêtre principale est mappée hors écran quelques
        # instants afin que WebView2 dispose d'un HWND et d'une vraie taille.
        # Au premier clic, on réutilise exactement ce moteur déjà chaud.
    ...
    def _viewer_pages_info(self) -> list[dict]:
        """Informations éditoriales légères affichées de chaque côté du livre.

        Le Visionneur reste une vue lecteur : on ne lui transmet ici que les
        informations utiles pour identifier immédiatement les deux pages
        visibles, sans y recopier tout l'inspecteur Structure/Gabarits.
        """
        canvas = getattr(self, "book_canvas", None)
        items = list(getattr(canvas, "items", []) or []) if canvas is not None else []
        ...
        for index, item in enumerate(items):
            ...
            if effective_side == "recto":
                badges.append("R")
            elif effective_side == "verso":
                badges.append("V")

            try:
                if bool(canvas._double_page_pair_id(item)):
                    badges.append("2P")
            except Exception:
                if bool(item.get("double_page") or item.get("double_page_pair_id")):
                    badges.append("2P")

            automatic_kind = str(item.get("automatic_kind") or "").strip().lower()
            automatic_position = str(item.get("automatic_position") or "").strip().lower()
            try:
            ...
            if is_auto:
                if automatic_kind == "page_auto_before" or automatic_position == "before":
                    badges.append("AUTO AV")
                elif automatic_kind == "page_auto_after" or automatic_position == "after":
                    badges.append("AUTO AP")
                else:
                    badges.append("AUTO")

            # Évite les doublons lorsqu'une page automatique satisfait plusieurs règles.
