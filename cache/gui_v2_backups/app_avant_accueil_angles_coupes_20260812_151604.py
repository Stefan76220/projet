...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        """Accueil TomeLinea — atelier éditorial organique, sans grille de cartes."""
        ...
        self._accueil_organic_state = {}
        self._accueil_organic_photos = {}
        ...
        def source_for(key: str) -> Path | None:
        ...
        def photo_for(key: str, max_w: int, max_h: int):
            ...
            cached = self._accueil_organic_photos.get(cache_key)
            if cached is not None:
                return cached

            if key == "trio":
                path = icon_paths.get("trio")
                if path is not None and path.exists():
                    try:
                        image = Image.open(path).convert("RGBA")
                        image.thumbnail((max_w, max_h), Image.Resampling.LANCZOS)
                        photo = ImageTk.PhotoImage(image)
                        self._accueil_organic_photos[cache_key] = photo
                        return photo
                    except Exception:
                        pass

                # Fallback : recomposer le trio avec les mêmes icônes.
                try:
                    composite = Image.new("RGBA", (240, 150), (0, 0, 0, 0))
                    specs = (
                        ("ouvrage_structure", (6, 36), 96),
                        ("livre_textuel", (70, 4), 112),
                        ("bande_dessinee", (142, 40), 94),
                    )
                    for subkey, (x, y), size in specs:
                        subpath = source_for(subkey)
                        if subpath is None:
                            continue
                        icon = Image.open(subpath).convert("RGBA")
        ...
        def organic_points(x1, y1, x2, y2, variant=0, dy=0):
            w = x2 - x1
            h = y2 - y1
            presets = (
                (
                    (0.06, 0.03), (0.36, 0.00), (0.72, 0.02), (0.96, 0.12),
                    (1.00, 0.42), (0.96, 0.82), (0.78, 0.98), (0.42, 1.00),
                    (0.12, 0.94), (0.00, 0.70), (0.02, 0.28),
                ),
                (
                    (0.10, 0.00), (0.48, 0.03), (0.86, 0.00), (0.99, 0.22),
                    (0.96, 0.60), (0.88, 0.92), (0.55, 1.00), (0.18, 0.94),
                    (0.00, 0.74), (0.03, 0.34),
                ),
                (
                    (0.02, 0.10), (0.28, 0.00), (0.72, 0.04), (0.98, 0.18),
                    (1.00, 0.66), (0.88, 0.94), (0.54, 0.98), (0.18, 1.00),
                    (0.00, 0.78), (0.04, 0.36),
                ),
            )
            ...
            pts = []
            for px, py in preset:
                pts.extend((x1 + px * w, y1 + py * h + dy))
            return pts

        def organic_blob(
            x1, y1, x2, y2,
            *, fill, outline="", width=1,
            variant=0, tags=(), shadow=True,
        ):
            if shadow:
                canvas.create_polygon(
                    organic_points(x1 + 2, y1 + 5, x2 + 2, y2 + 5, variant),
                    smooth=True,
                    splinesteps=40,
                    fill="#DED8CE",
                    outline="",
            ...
            return canvas.create_polygon(
                organic_points(x1, y1, x2, y2, variant),
                smooth=True,
                splinesteps=40,
                fill=fill,
                outline=outline,
        ...
        def round_rect(x1, y1, x2, y2, radius, **kwargs):
            ...
            return canvas.create_polygon(
                pts, smooth=True, splinesteps=28, **kwargs
            )

        def section_header(number, title, x, y, accent):
            canvas.create_oval(
                x, y-18, x+38, y+20,
        ...
        def draw_primary_button(tag, text, cx, cy, width=190):
            state = self._accueil_organic_state.get(tag, "normal")
            hover = state == "hover"
            pressed = state == "pressed"
            h = 42
            yoff = 1 if pressed else 0
            fill = "#173E70" if not hover else "#24558E"
            if pressed:
                fill = "#12345F"
            round_rect(
                cx-width/2+1, cy-h/2+4,
                cx+width/2+1, cy+h/2+4,
                22,
                fill="#CBC9C5", outline="",
                tags=(tag,),
        ...
        def draw_secondary_button(tag, text, cx, cy, width=132):
            state = self._accueil_organic_state.get(tag, "normal")
            hover = state == "hover"
            pressed = state == "pressed"
            h = 40
            yoff = 1 if pressed else 0
            fill = "#FFFDFC" if not hover else "#F1F6FB"
            border = "#BCC6D1" if not hover else "#456B95"
            round_rect(
                cx-width/2+1, cy-h/2+4,
                cx+width/2+1, cy+h/2+4,
                18,
        ...
        def draw_chip(x, y, text, fill, fg, w):
            round_rect(
                x, y, x+w, y+22, 11,
                fill=fill,
                outline=mix(fg, "#FFFFFF", 0.72),
                width=1,
            )
            canvas.create_text(
                x+w/2, y+11,
        ...
        def bind_interaction(tag, *, command=None, redraw=True):
            def schedule():
                if redraw:
                    canvas.after_idle(render)

            def enter(_event=None):
                self._accueil_organic_state[tag] = "hover"
                canvas.configure(cursor="hand2")
                schedule()

            def leave(_event=None):
                self._accueil_organic_state[tag] = "normal"
                canvas.configure(cursor="arrow")
                schedule()

            def press(_event=None):
                self._accueil_organic_state[tag] = "pressed"
                schedule()
        ...
        def render(_event=None):
            ...
            sx = width / 1600.0
            sy = height / 760.0
            scale = max(0.80, min(1.10, min(sx, sy)))

            X = lambda v: v * sx
            Y = lambda v: v * sy
            F = lambda v: max(7, int(round(v * scale)))

            canvas.delete("all")

            # Fond éditorial général — volontairement très présent.
            if self._accueil_bg_source is not None:
                try:
                    bg = cover(self._accueil_bg_source, width, height)
                    self._accueil_organic_bg_photo = ImageTk.PhotoImage(bg)
                    canvas.create_image(
                        0, 0,
                        image=self._accueil_organic_bg_photo,
                        anchor="nw",
                    )
                except Exception:
                    canvas.configure(bg="#F7F2E9")

            # Voile crème léger uniquement au centre pour maintenir la lecture.
            # Pas de panneau général : le décor reste visible autour des fonctions.
            ...
            # ---------------- 01 Créer ----------------
            section_header("01", "Créer un nouveau projet", X(135), Y(160), "#559875")

            # Feuille principale très douce, sans cadre régulier.
            organic_blob(
                X(115), Y(185), X(815), Y(415),
                fill="#FFFDFC",
                outline="#E8E1D7",
                width=1,
                variant=0,
            ...
            for index, (key, title, subtitle, accent, variant) in enumerate(project_types):
                bx1, by1, bx2, by2 = choice_boxes[index]
                tag = f"choice_{key}"
                state = self._accueil_organic_state.get(tag, "normal")
                selected = self._selected_project_type == key
                hover = state == "hover"
                pressed = state == "pressed"
                lift = -3 if hover else (1 if pressed else 0)

                face = mix(accent, "#FFFFFF", 0.925)
                if hover:
                    face = mix(accent, "#FFFFFF", 0.88)
                if selected:
                    face = mix(accent, "#FFFFFF", 0.84)

                organic_blob(
                    X(bx1), Y(by1+lift), X(bx2), Y(by2+lift),
                    fill=face,
                    outline=mix(accent, "#FFFFFF", 0.48 if selected else 0.68),
                    width=2 if selected else 1,
                    variant=variant,
                    shadow=hover or selected,
                    tags=(tag,),
                )

                # Sélecteur : petit repère discret, pas bouton radio massif.
                cx = X(bx1+19)
                cy = Y(by1+18+lift)
                canvas.create_oval(
                    cx-X(7), cy-Y(7), cx+X(7), cy+Y(7),
                    fill="#FFFDFC",
                    outline=accent,
                    width=1,
                    tags=(tag,),
                ...
                if selected:
                    canvas.create_oval(
                        cx-X(3.2), cy-Y(3.2), cx+X(3.2), cy+Y(3.2),
                        fill=accent,
                        outline="",
                        tags=(tag,),
                    )

                photo = photo_for(key, int(X(82)), int(Y(76)))
                if photo is not None:
                    canvas.create_image(
                        X((bx1+bx2)/2), Y(by1+55+lift),
                        image=photo,
                        anchor="center",
                        tags=(tag,),
                    )
                ...
                bind_interaction(
                    tag,
                    command=lambda chosen=key: setattr(
            ...
            # ---------------- 02 Ouvrir ----------------
            section_header("02", "Ouvrir un projet", X(885), Y(160), "#4B82B7")

            organic_blob(
                X(850), Y(185), X(1485), Y(402),
                fill="#FFFDFC",
                outline="#E7E2DB",
                width=1,
                variant=1,
            ...
            trio = photo_for("trio", int(X(190)), int(Y(125)))
            if trio is not None:
                canvas.create_image(
                    X(1025), Y(285),
                    image=trio,
            ...
            # ---------------- 03 Reprendre ----------------
            section_header("03", "Reprendre votre travail", X(115), Y(455), "#8968B4")
            canvas.create_text(
                X(172), Y(482),
            ...
            try:
                cache_key = ("recent", 46)
                recent_photo = self._accueil_organic_photos.get(cache_key)
                if recent_photo is None and recent_photo_path.exists():
                    ...
                    self._accueil_organic_photos[cache_key] = recent_photo
            ...
            # Ouvrir et Reprendre : exactement le même composant secondaire.
            draw_secondary_button(
                "resume_project_button", "Reprendre",
                X(700), Y(548),
                width=X(142),
            )
            ...
            draw_chip(X(265), Y(596), "Ouvrage structuré", "#F5FAF7", "#4D9870", X(122))
            draw_chip(X(398), Y(596), "En cours", "#F6FAF7", "#4D9870", X(78))
            draw_chip(X(486), Y(596), "Dernier bureau : Maquettage", "#F8F5FB", "#8063A4", X(170))
            canvas.create_text(
                X(762), Y(607),
            ...
            # ---------------- 04 Accès directs ----------------
            section_header("04", "Repères & accès directs", X(850), Y(455), "#D96C4F")
            canvas.create_text(
                X(907), Y(482),
            ...
            # Grande feuille discrète : aucun sous-cadre autour de chaque accès.
            organic_blob(
                X(835), Y(505), X(1495), Y(635),
                fill="#FFFDFC",
                outline="#E6E0D8",
                width=1,
                variant=0,
            ...
            shortcuts = (
                ("maquettage", "Maquettage", "Organiser les pages", "#579B73", 950),
                ("atelier", "Atelier", "Préparer les gabarits", "#4C9ED0", 1110),
                ("conception", "Conception", "Créer les pages", "#8A69BE", 1270),
                ("centre", "Centre du projet", "Vue d’ensemble", "#E06B4E", 1430),
            )
            ...
            for key, title, subtitle, accent, cx0 in shortcuts:
                ...
                hover = state == "hover"
                pressed = state == "pressed"
                cy0 = 550 + (-3 if hover else (1 if pressed else 0))
                radius = 37 if hover else 34

                # Halo réactif visible au survol, sans case.
                if hover:
                    canvas.create_oval(
                        X(cx0-radius-7), Y(cy0-radius-7),
                        X(cx0+radius+7), Y(cy0+radius+7),
                        fill=mix(accent, "#FFFFFF", 0.90),
                        outline="",
                        tags=(tag,),
                    )
                canvas.create_oval(
            ...
            # ---------------- Statut direct sur le fond ----------------
            status_y = 682
            canvas.create_line(
                X(760), Y(status_y-22), X(760), Y(status_y+22),
                fill="#DFDBD4",
            )
            canvas.create_line(
                X(980), Y(status_y-22), X(980), Y(status_y+22),
                fill="#DFDBD4",
            )
            canvas.create_line(
                X(1200), Y(status_y-22), X(1200), Y(status_y+22),
                fill="#DFDBD4",
            )

            status_items = (
                (790, "État", "●  En cours", "#4C936D"),
                (1010, "Étape actuelle", "●  Maquettage", "#8564B1"),
                (1230, "Dernière activité", "◷  10/08/2026 15:23", "#3E607E"),
            )
            for x0, label, value, color in status_items:
                canvas.create_text(
                    X(x0), Y(status_y-13),
                    text=label,
                    fill="#4A5560",
