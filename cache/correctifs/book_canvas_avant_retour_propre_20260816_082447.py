...
class BookCanvas(tk.Frame):
    # SURVOL_PAGES_V3_V1
    # ZOOM_PAGE_SELECTION_SEULE_V3_V5
    ...
    def _build_canvas(self):
        ...
        # SURVOL_CANEVAS_PAGE_SEULE_V3_V2
        # B possède son propre survol page par page. Le gestionnaire global
        # ne doit donc pas éclairer toute la zone du livre.
    ...
    # ------------------------------------------------------------------
    # Zoom continu — une seule surface, deux cibles naturelles
    # ------------------------------------------------------------------
    ...
    def _book_height_cap(self) -> int:
        """Zoom maximum du livre : une page ne dépasse jamais la hauteur de B."""
        if not hasattr(self, "canvas"):
            return 100
        viewport_h = max(180, self.canvas.winfo_height() - 34)
        value = int(viewport_h / float(self.BASE_PAGE_H) * 100)
    ...
    def _set_page_focus(self, active: bool):
        ...
        if not active:
            # Mémorise le seuil obtenu dans la géométrie normale de B.
            self._book_zoom_cap = max(self.MIN_ZOOM, int(self._book_zoom_cap))
    ...
    def set_zoom(self, value: int, *, center_selected=True):
        ...
        if not self._page_focus:
            ...
            if requested > cap and self._selected_index is not None:
                # Pas de nouvelle vue : le même B prend simplement l'espace
                # disponible et le zoom continue sur la page sélectionnée.
                self._set_page_focus(True)
                value = requested
            else:
                value = min(requested, cap)
        else:
            # En revenant sous le seuil du livre, B retrouve automatiquement
            # sa place normale et C réapparaît, sans reconstruction.
    ...
    def fit_selected(self):
        ...
        if not self._page_focus:
            self._book_zoom_cap = self._book_height_cap()
            self._set_page_focus(True)
            # Attend que le même B ait reçu sa nouvelle géométrie.
    ...
    def _schedule_render(self, _event=None):
        if self._render_pending is not None:
            try:
                ...
                pass
        # Pendant un redimensionnement de la fenêtre, conserver le rendu
        # existant et ne recalculer le livre qu'une fois la taille stabilisée.
        self._render_pending = self.after(120, self.render)
    ...
    def render(self):
        ...
        if self._page_focus:
            self.status_var.set(
                f"Page {selected_page_number} sur {len(self.items)}  •  travail détaillé"
            )
        else:
            self.status_var.set(
                f"{len(self.items)} page{'s' if len(self.items) > 1 else ''}  •  "
                f"page {selected_page_number} sélectionnée"
            )
        ...
        # En travail détaillé, le même canevas B n'affiche plus que la page
        # sélectionnée. Le livre n'est ni reconstruit ni remplacé : on change
        # simplement ce que le canevas dessine pendant le zoom de page.
        if self._page_focus:
        ...
        for index in indexes:
            ...
            if selected:
                fill = "#E1ECE8"
                outline = theme.ACCENT_BRIGHT if hovered else theme.ACCENT
        ...
        if self._dragging and self._drag_target_index is not None and not self._page_focus:
