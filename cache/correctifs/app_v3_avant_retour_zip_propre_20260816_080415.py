...
from src.gui_v3.windows_native_frame import WindowsNativeFrame
...
class WindowControl(tk.Frame):
    """Commande de fenêtre TomeLinea à gabarit pixel fixe."""

    def __init__(self, parent, symbol: str, command, *, accent: str, hover: str):
        super().__init__(
            parent,
            width=44,
            height=34,
        ...
        self.button = V3Button(self, symbol, command, compact=True)
        ...
        self.button._base_bg = theme.PANEL_SOFT
        self.button._hover_bg = hover
        self.button.configure(bg=self.button._base_bg)
        self.button.pack(fill="both", expand=True)

    def set_symbol(self, symbol: str) -> None:
        self.button.configure(text=symbol)
...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Fenêtre Windows native : le cadre sera supprimé par WM_NCCALCSIZE,
        # mais Windows conserve réduction, restauration, Alt+Tab et barre des tâches.
        self.native_frame = WindowsNativeFrame(self)
        ...
        # Cadre natif installé pendant que la fenêtre est encore cachée :
        # aucune barre de titre Windows ne doit apparaître au premier affichage.
        native_ok = self.native_frame.install()
        self.native_frame.set_button_state_callback(self._sync_window_control_state)
        if native_ok:
            self.native_frame.prepare_restore_geometry()
            self.deiconify()
            self.native_frame.maximize()
        else:
            # Repli de sécurité : l'application reste utilisable même si
            # l'intégration Win32 n'est pas disponible.
            try:
        ...
        # Force le premier vrai calcul de taille, Configure et fond.
        self.update_idletasks()
        self.update()
    ...
    def _build_workspace(self):
        ...
        def layout(event=None):
            ...
            gap = 10
            a_h = max(104, min(118, int(h * 0.115)))
    ...
    def _build_zone_a(self):
        """Bandeau chantier : identité, projet, travail actuel et commandes globales."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=4, minsize=285)
        body.grid_columnconfigure(4, weight=3, minsize=235)

        # 1. Identité TomeLinea — même signature que l'Accueil, en plus petit.
        brand = tk.Frame(body, bg=theme.PANEL)
        brand.grid(row=0, column=0, sticky="nsw", padx=(0, 14), pady=(2, 1))

        brand_icon = self._home_brand_icon_photo(56)
        if brand_icon is not None:
            icon_label = tk.Label(brand, image=brand_icon, bg=theme.PANEL, bd=0, cursor="fleur")
            icon_label.image = brand_icon
            icon_label.pack(side="left", padx=(0, 10))
            icon_label.bind("<ButtonPress-1>", self.native_frame.begin_drag, add="+")

        brand_text = tk.Frame(brand, bg=theme.PANEL, cursor="fleur")
        brand_text.pack(side="left", anchor="n")
        brand_text.bind("<ButtonPress-1>", self.native_frame.begin_drag, add="+")
        brand_text.bind("<Double-Button-1>", lambda _e: self.native_frame.toggle_maximize(), add="+")

        brand_title = self._home_brand_title_photo(205)
        if brand_title is not None:
            title_label = tk.Label(brand_text, image=brand_title, bg=theme.PANEL, bd=0, cursor="fleur")
            title_label.image = brand_title
            title_label.pack(anchor="w")
            title_label.bind("<ButtonPress-1>", self.native_frame.begin_drag, add="+")
            title_label.bind("<Double-Button-1>", lambda _e: self.native_frame.toggle_maximize(), add="+")
        else:
            title_label = tk.Label(
                brand_text, text="TomeLinea", bg=theme.PANEL, fg=theme.INK,
                font=(theme.FONT_TITLE, 16, "bold"), cursor="fleur",
            )
            title_label.pack(anchor="w")
            title_label.bind("<ButtonPress-1>", self.native_frame.begin_drag, add="+")
        ...
        signature.bind("<ButtonPress-1>", self.native_frame.begin_drag, add="+")

        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=1, sticky="ns", padx=(4, 16)
        )

        # 2. Où suis-je ? Nom du projet + type + icône.
        ...
        self.zone_a_type_icon.pack(side="left", padx=(0, 8))
        tk.Label(
        ...
        # 3. Ce que je fais maintenant — pas de faux pourcentage d'avancement.
        ...
        tk.Label(
            current, textvariable=self.book_canvas.status_var,
        ...
        # 4. Commandes globales + vraies commandes Windows, habillées TomeLinea.
        ...
        tk.Frame(actions, bg=theme.BORDER_SOFT, width=1, height=34).pack(
            side="left", padx=7
        )

        self.zone_a_min_button = WindowControl(
            actions, "—", self.native_frame.minimize,
            accent=theme.ACCENT_BRIGHT, hover=theme.ACCENT_SOFT,
        )
        self.zone_a_min_button.pack(side="left", padx=3)

        self.zone_a_size_button = WindowControl(
            actions, "❐", self.native_frame.toggle_maximize,
            accent="#9CD8D0", hover="#304B57",
        )
        self.zone_a_size_button.pack(side="left", padx=3)

        self.zone_a_close_button = WindowControl(
            actions, "✕", self.native_frame.close,
            accent="#F08A5A", hover="#624038",
        )
        self.zone_a_close_button.pack(side="left", padx=(3, 0))

    def _sync_window_control_state(self, maximized: bool) -> None:
        button = getattr(self, "zone_a_size_button", None)
        if button is not None:
            button.set_symbol("❐" if maximized else "□")
    ...
    def select_tab(self, key: str):
        ...
        if hasattr(self, "workspace_current_var"):
            self.workspace_current_var.set(
                dict(theme.TAB_NAMES).get(key, key.title()).upper()
            )
    ...
    def _refresh_workspace_state(self):
        ...
        if hasattr(self, "zone_a_type_icon"):
            photo = self._type_icon(self.context.project_type, 28)
            if photo is not None:
