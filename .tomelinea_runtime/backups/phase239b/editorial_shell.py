...
class TomeLineaV4Editorial(
    ...
    def _phase239_resolve_structure_questions(self, analysis: dict) -> dict:
        """Ne demande l'utilisateur que lorsque le type éditorial reste inconnu."""
        ...
        for unit in list(result.get("units") or []):
            if not isinstance(unit, dict) or not unit.get("ask_user"):
                continue
            ...
            create = messagebox.askyesno(
                ...
                (
                    ...
                    f"« {title} »\n\n"
                    "Faut-il créer une section distincte ?"
                ),
                parent=self,
            )
            ...
            if create:
                name = simpledialog.askstring(
                    "Nom de la section",
                    "Comment doit-on appeler cette section ?",
                    initialvalue=title,
                    parent=self,
                )
                name = str(name or title).strip() or title
                result = apply_structure_choice(
                    result,
                    key,
                    create_section=True,
                    name=name,
    ...
    def _phase2_poll(self, canvas) -> None:
        ...
        while True:
            ...
            if kind == "ready":
                ...
                structure_analysis = self._phase239_resolve_structure_questions(
                    structure_analysis
                )
    ...
    def _draw_phase2_canvas_page(self, canvas) -> None:
        ...
        if self._phase2_canvas_state == "idle":
            ...
            self._phase2_show_overlay(
                canvas,
                "Préparation de Composition",
                "Analyse du Livre avant affichage…",
            )
            return

        if self._phase2_canvas_state in {"preparing", "preflight"}:
            return
        if self._phase2_canvas_state in {"blocked", "failed"}:
            return
        ...
        *,
        ...
        if self._phase2_can_render():
            ...
            if (
                ...
                try:
                    self.after_idle(
                        lambda c=canvas, i=int(index): self._phase2_request_global_page(c, i)
                    )
                except Exception:
                    pass
            return
