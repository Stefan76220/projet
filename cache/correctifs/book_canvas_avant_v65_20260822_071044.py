...
# TOMELINEA_GABARITS_PALETTE_ORGANISER_GUIDE_V64
...
class BookCanvas(tk.Frame):
    ...
    GABARIT_INSPECTOR_DEFAULT_BLOCK_ORDER = (
        "create", "allowed", "snap", "guide", "organize",
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        multi_infos = list(multi_infos or [info])
        ...
        # Rail : neuf lignes confortables, contenu intégralement dans la hauteur disponible.
        right = width - 7.0
        rail_w = 54.0
        rail_x1 = right - rail_w
        gap_y = 5.0
        available = max(360.0, height - 18.0)
        row_h = min(56.0, max(42.0, (available - gap_y * (len(order)-1)) / max(1, len(order))))
        total_h = len(order)*row_h + max(0,len(order)-1)*gap_y
        y = max(7.0, (height-total_h)/2.0)
        target.create_rectangle(rail_x1-2.0, y-4.0, right+1.0, min(height-5.0,y+total_h+4.0), fill="#141E24", outline="#34464F", width=1)
        ...
        for name in order:
            ...
            icon_box = (rail_x1+6.0, y1+5.0, right-6.0, y2-5.0)
            self._gabarit_inspector_draw_icon(target, band_icons[name], icon_box, selected=is_active or (name=="guide" and bool(guide)), hovered=hovered_band)
            if is_active:
                target.create_line(rail_x1+1.0,y1+7.0,rail_x1+1.0,y2-7.0,fill="#57C2B8",width=3)
            key=f"dragblock_{name}"
        ...
        if active_band == "allowed":
            ...
            content_h = 154.0 if guide else 60.0
        ...
        def icon_strip(items, *, button=41.0, gap=8.0, y_offset=0.0, center=True, start_x=None):
            count_items=len(items)
            total=count_items*button+max(0,count_items-1)*gap
            sx=(fly_left+(fly_w-total)/2.0) if center else (start_x if start_x is not None else fly_left+10.0)
            yy=content_top+y_offset
            for i,item in enumerate(items):
                key,icon,selected,tip,danger = item[:5]
                disabled = bool(item[5]) if len(item) > 5 else False
                x1=sx+i*(button+gap)
                self._gabarit_inspector_icon_button(target,key,(x1,yy,x1+button,yy+button),icon,selected=selected,danger=danger,disabled=disabled,tooltip=tip)
        ...
        if active_band == "create":
            ...
            tips={"margins":"Limiter la zone aux marges","page":"Autoriser jusqu’au bord de page","bleed":"Autoriser jusqu’au fond perdu"}
            ...
            icon_strip(tuple(items),button=43.0,gap=14.0)
            if pending in {"margins","page","bleed"} and pending != current:
                yy=content_top+50.0; gap=7.0; bw=(fly_w-gap-20.0)/2.0; x1=fly_left+10.0
                self._gabarit_inspector_control_box(target,"align_scope_page",(x1,yy,x1+bw,yy+30))
                target.create_text(x1+bw/2.0,yy+15.0,text="Cette page",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
                x2=x1+bw+gap
                self._gabarit_inspector_control_box(target,"align_scope_type",(x2,yy,x2+bw,yy+30))
                target.create_text(x2+bw/2.0,yy+15.0,text="Toutes du type",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
            ...
            if not guide:
                icon_strip((("guide_import","guide_import",False,"Importer un fond guide",False),),button=45.0,gap=10.0)
            else:
                visible=bool(guide.get("visible",True))
                icon_strip((
                    ("guide_import","guide_import",False,"Remplacer le fond guide",False),
                    ("guide_toggle","guide_show" if visible else "guide_hide",visible,"Masquer le fond guide" if visible else "Afficher le fond guide",False),
                    ("guide_delete","delete",False,"Supprimer le fond guide",True),
                ),button=42.0,gap=12.0,y_offset=0.0)
                frame=str(guide.get("frame") or "page")
                target.create_text(fly_left+11.0,content_top+52.0,text="CADRAGE",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
                icon_strip((
                ...
                ),button=37.0,gap=11.0,y_offset=65.0)
                opacity=max(0.10,min(1.0,float(guide.get("opacity",0.45) or 0.45)))
                transparency=1.0-opacity
                label_y=content_top+111.0
                target.create_text(fly_left+11.0,label_y,text=f"TRANSPARENCE  {int(round(transparency*100))} %",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
                bar_x1=fly_left+116.0; bar_x2=fly_right-14.0; bar_y=label_y
                target.create_line(bar_x1,bar_y,bar_x2,bar_y,fill="#405760",width=4)
                knob_x=bar_x1+(bar_x2-bar_x1)*(transparency/0.90)
                knob_x=max(bar_x1,min(bar_x2,knob_x))
                target.create_oval(knob_x-6,bar_y-6,knob_x+6,bar_y+6,fill="#51BEB5",outline="#C8D9D8",width=1)
                self._gabarit_inspector_hitboxes["guide_opacity_bar"]=(bar_x1-8,bar_y-12,bar_x2+8,bar_y+12)
            ...
            label_x=fly_left+10.0
            target.create_text(label_x,content_top+18,text="ALIGNER",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=31.0,gap=4.0,y_offset=28.0)
            target.create_text(label_x,content_top+68,text="DISTRIBUER",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=34.0,gap=12.0,y_offset=77.0)
            target.create_text(label_x,content_top+117,text="ASSOCIER",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
                ("associate_zones","associate",False,"Associer les zones sélectionnées",False,count<2),
                ("dissociate_zones","dissociate",False,"Dissocier le groupe",False,not has_group),
            ),button=34.0,gap=12.0,y_offset=126.0)
            target.create_text(label_x,content_top+166,text="SUPERPOSITION",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=34.0,gap=7.0,y_offset=175.0)

        elif active_band == "actions":
            locked=bool(info.get("locked",False))
            icon_strip((
                ("zone_lock","unlock" if locked else "lock",locked,"Déverrouiller" if locked else "Verrouiller",False),
                ("zone_duplicate","duplicate",False,"Dupliquer",False,count>1),
                ("zone_delete","delete",False,"Supprimer la sélection" if count>1 else "Supprimer la zone",True),
            ),button=43.0,gap=13.0)
            ...
            pending_scope=str(getattr(self,"_gabarit_scope_pending","") or "")
            gap=8.0; bw=(fly_w-gap-20.0)/2.0; yy=content_top+6.0
            for i,(key,label,selected) in enumerate((
                ...
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+32),selected=selected)
                target.create_text(x1+bw/2.0,yy+16.0,text=label,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold" if selected else "normal"))
            ...
            for i,(key,label,disabled) in enumerate((("copy_template","Copier",False),("apply_template","Appliquer",not has_copy))):
                x1=fly_left+10.0+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+32),disabled=disabled)
                target.create_text(x1+bw/2.0,yy+16.0,text=label,anchor="center",fill="#D8E0E1" if not disabled else "#65737A",font=(theme.FONT_UI,8,"bold"))
            ...
            if count==1 and bool(info.get("spread")):
                yy=content_top+47.0; current=str(info.get("spread_position") or "left"); bw2=(fly_w-26.0)/3.0
                for i,(lab,key,pos) in enumerate((("G","spread_left","left"),("C","spread_center","center"),("D","spread_right","right"))):
                    x1=fly_left+10.0+i*(bw2+3.0)
                    self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw2,yy+28),selected=current==pos)
                    target.create_text(x1+bw2/2.0,yy+14.0,text=lab,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
    ...
    def _gabarit_inspector_press(self, event):
        ...
        if key == "create_text":
            ...
            box=self._gabarit_inspector_hitboxes.get(key)
            if box:
                ratio=(float(event.x)-float(box[0]))/max(1.0,float(box[2])-float(box[0]))
                self.gabarit_set_guide_transparency(max(0.0,min(0.90,ratio*0.90)))
    ...
    def _gabarit_guide_data(self, item: dict | None = None) -> dict:
        item = item if isinstance(item, dict) else self._gabarit_active_item()
        raw = item.get("gabarit_guide") if isinstance(item, dict) else None
        return raw if isinstance(raw, dict) else {}
    ...
    def gabarit_import_guide(self) -> bool:
        ...
        item["gabarit_guide"]={
            ...
            "visible":True,
            "frame":str(old.get("frame") or "page"),
            "opacity":float(old.get("opacity",0.45) or 0.45),
        }
        ...
        self.status_var.set("Fond guide chargé — il ne sera ni imprimé ni exporté.")
        return True
    ...
    def gabarit_set_guide_transparency(self, transparency: float) -> bool:
        item=self._gabarit_active_item(); guide=self._gabarit_guide_data(item)
        if not guide: return False
    ...
    def gabarit_associate_selected_zones(self) -> bool:
        ...
        if len(zones)<2:
            self.status_var.set("Sélectionnez au moins deux zones à associer.")
            return False
        ...
        for zid, box in reversed(list(self._gabarit_zone_hitboxes.items())):
            ...
            if inside_zone:
                ...
                if shift:
                    ...
                    self._gabarit_set_selected_ids(associated, primary=zid)
                    if zone is not None and not bool(zone.get("locked", False)):
                        ...
                        if len(associated) > 1:
                            members=[]
                            for azid in associated:
                                member=self._gabarit_find_zone(active, zone_id=azid)
                                if isinstance(member,dict):
                                    members.append((member,float(member.get("x",0)),float(member.get("y",0)),float(member.get("w",.2)),float(member.get("h",.2))))
                            drag_state["members"]=members
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            if state["mode"] == "resize":
                ...
                if members:
                    ddx=nx-ox; ddy=ny-oy
                    # Le déplacement du groupe est limité par le membre qui atteint
                    # le premier la zone autorisée.
                    min_dx=max(frame_x-mx for _m,mx,my,mw,mh in members)
                    max_dx=min(frame_x+frame_w-(mx+mw) for _m,mx,my,mw,mh in members)
                    min_dy=max(frame_y-my for _m,mx,my,mw,mh in members)
                    max_dy=min(frame_y+frame_h-(my+mh) for _m,mx,my,mw,mh in members)
                    ddx=max(min_dx,min(max_dx,ddx)); ddy=max(min_dy,min(max_dy,ddy))
