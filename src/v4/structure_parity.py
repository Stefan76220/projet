...
def _strip_old_parity_state(
    ...
    for page in list(
        ...
        # Une page automatique déjà membre d'une 2P est conservée dans le
        # Livre pendant ce recalcul. Elle ne peut donc pas être simultanément
        # proposée comme ancienne compensation réutilisable : sinon
        # _insert_compensation tenterait de réajouter le même UUID déjà présent.
        ...
        elif parity and page.spread_id:
            # Une page automatique devenue membre explicite d'une 2P est
            # désormais utile par la décision éditoriale elle-même. Elle ne
            # peut pas être retirée comme une simple ancienne compensation.
            _set_roles(page, [])
...
def _share_spread_correction_with_previous_after_rule(
    book: BookV4,
    left: PageV4,
    correction: PageV4 | None,
) -> None:
    """Réutilise la correction 2P pour une contrainte « blanc après ».

    Cas important : un même blanc pouvait auparavant servir à la fois d'AP de
    la page précédente et d'AV de la future moitié droite. Si ce blanc devient
    ensuite la moitié gauche d'une 2P, une correction de parité doit être
    insérée avant lui. Cette nouvelle page est exactement au bon endroit pour
    satisfaire l'ancien AP ; on transfère donc le rôle au lieu de laisser une
    relation devenue non contiguë ou de créer un blanc supplémentaire.
    """
    if correction is None or not is_structural_auto_page(left):
        return

    try:
        correction_index = book.page_order.index(correction.id)
    except ValueError:
        return
    if correction_index <= 0:
        return

    previous_id = book.page_order[correction_index - 1]
    previous = book.pages.get(previous_id)
    if previous is None:
        return

    left_roles = _normalized_roles(left)
    moved = [
        role for role in left_roles
        if role.get("code") == "AP" and role.get("source_id") == previous_id
    ]
    if not moved:
        return

    _set_roles(left, [role for role in left_roles if role not in moved])

    # La nouvelle correction est immédiatement après la page concernée : elle
    # peut satisfaire simultanément AP + la correction de parité de la 2P.
    for role in moved:
        _add_role(
            correction,
            code=str(role.get("code") or "AP"),
            source_id=previous_id,
            target_type=str(role.get("target_type") or "Page blanche"),
        )

    previous.auto_after = [
        correction.id if page_id == left.id else page_id
        for page_id in previous.auto_after
    ]
    if correction.id not in previous.auto_after:
        previous.auto_after.append(correction.id)
...
def sync_structure_parity(
    ...
    # Les pages automatiques seules ne portent normalement pas de règle
    # R/V propre. En revanche, une double page peut être constituée de
    # deux pages automatiques explicitement choisies par l'utilisateur.
    # Il faut alors traiter au moins un membre de cette 2P, sinon sa parité
    # gauche=verso / droite=recto ne serait jamais recalculée.
    base_ids = [
        page_id
        for page_id in book.page_order
        if (
            ...
            or bool(book.pages[page_id].spread_id)
        )
    ]
    ...
    for page_id in base_ids:
        ...
        if page.spread_id:
            ...
            if correction is not None:
                ...
                _share_spread_correction_with_previous_after_rule(
                    book,
                    left,
                    correction,
                )

            # Une page automatique qui est elle-même moitié de la double page
            # porte explicitement le rôle DP. Elle reste ainsi une vraie page
            # automatique utile et n'est jamais considérée comme orpheline.
            if is_structural_auto_page(left):
                _add_role(
                    left,
                    code="DP",
                    source_id=right.id,
            ...
            if is_structural_auto_page(right):
                _add_role(
                    right,
                    code="DP",
                    source_id=left.id,
