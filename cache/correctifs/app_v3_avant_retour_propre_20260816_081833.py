...
BRAND_VISIBLE_ROOT = PROJECT_ROOT / "assets" / "branding" / "tomelinea" / "logo_pack_visibilite"
BRAND_ICON = BRAND_VISIBLE_ROOT / "TomeLinea.ico"
BRAND_ICON_PNG = BRAND_VISIBLE_ROOT / "TomeLinea_windows_64x64.png"
...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Prépare une taille restaurée saine puis confie réellement la fenêtre à Windows.
        self._prepare_native_restore_geometry()
        self.deiconify()
        try:
            self.state("zoomed")
        except tk.TclError:
        ...
        # Force le premier vrai calcul Windows puis applique uniquement l'habillage DWM.
        self.update_idletasks()
        self.update()
        self._apply_native_titlebar_style()
        self.after(120, self._apply_native_titlebar_style)
    ...
    # FENETRE_NATIVE_STABLE_TOMELINEA_V3_V1
    ...
    def _apply_native_titlebar_style(self):
        """Habille la vraie barre Windows sans modifier son comportement natif."""
        try:
            import ctypes
            import sys
            if sys.platform != "win32":
            ...
            # Windows 11 : couleurs DWM de la légende, du texte et de la bordure.
            attrs = (
                (20, ctypes.c_int(1)),                      # mode sombre
                (35, ctypes.c_uint32(self._colorref("#222831"))),  # légende
                (36, ctypes.c_uint32(self._colorref("#F4E9C7"))),  # texte
                (34, ctypes.c_uint32(self._colorref("#497C74"))),  # bordure
            )
    ...
    def _prepare_native_restore_geometry(self):
        """Définit une taille restaurée confortable avant la maximisation native."""
        try:
            import ctypes
            ...
            if ctypes.windll.user32.SystemParametersInfoW(
                ...
                width = min(work_w, max(1180, int(work_w * 0.86)))
        ...
        width = max(1180, int(self.winfo_screenwidth() * 0.86))
        height = max(720, int(self.winfo_screenheight() * 0.82))
    ...
    def _build_workspace(self):
        ...
        def layout(event=None):
            ...
            gap = 10
            a_h = max(98, min(112, int(h * 0.112)))
    ...
    def _build_zone_a(self):
        """Bandeau chantier : identité, projet, travail actuel et accès permanents."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=5, minsize=350)
        body.grid_columnconfigure(4, weight=4, minsize=300)
        ...
        signature = tk.Canvas(
            brand_text, width=206, height=12, bg=theme.PANEL, bd=0, highlightthickness=0
        )
        signature.pack(anchor="w", pady=(1, 0))
        signature.create_text(
            0, 6, anchor="w", text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE",
            fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 6, "bold")
        )
        ...
        current.grid(row=0, column=4, sticky="nsew", padx=(0, 16), pady=(1, 0))
        tk.Label(
            current, text="TRAVAIL ACTUEL", bg=theme.PANEL, fg=theme.ACCENT_BRIGHT,
            font=(theme.FONT_UI, 8, "bold")
        ).pack(anchor="w")
        ...
        tk.Label(
            ...
            fg=theme.MUTED, font=(theme.FONT_UI, 8), justify="left",
        ...
        # A informe ; il ne remplace pas les commandes système Windows.
    ...
    def _refresh_workspace_state(self):
        ...
        if hasattr(self, "zone_a_type_icon"):
            photo = self._type_icon(self.context.project_type, 30)
            if photo is not None:
