...
class TomeLineaV4Survol(
    ...
    def _composition_render_book_state_panel(self, host) -> None:
        """Format = format physique + règles générales du texte.

        On ne demande pas à l'utilisateur d'ouvrir un second onglet pour des
        instructions qui concernent tout le Livre. Tant que le format n'est
        pas validé, les règles texte restent simplement annoncées. Une fois le
        format confirmé, on réutilise le vrai moteur de règles générales déjà
        présent dans TomeLinea.
        """
        ...
        format_reviewed = bool(
            isinstance(metadata, dict)
            and metadata.get("book_import_state_reviewed", False)
        )
        ...
        tk.Frame(
            ...
            height=1,
        ).pack(fill="x", padx=12, pady=(10, 9))

        tk.Label(
            host,
            text="TEXTE — RÈGLES GÉNÉRALES",
            bg=theme.PANEL,
        ...
        if not format_reviewed:
            tk.Label(
                host,
                text=(
                    "Validez d'abord le format du Livre. Les règles globales "
                    "du texte seront ensuite réglées ici, avant le Survol."
                ),
                bg=theme.PANEL,
                fg=theme.MUTED,
                wraplength=215,
        ...
        try:
            self._composition_render_text_general_rules(host)
        ...
        tk.Frame(
            ...
            bg=theme.PAGE_BORDER,
            height=1,
        ).pack(fill="x", padx=18, pady=(10, 9))

        tk.Label(
            host,
            text=(
                "Format et règles globales prêts. Le Survol peut maintenant "
                "apprendre les règles particulières du livre sur les vrais cas."
            ),
            bg=theme.PANEL,
            fg=theme.MUTED,
            wraplength=215,
    ...
    def _composition_update_inspector_context(
        ...
        if current_tool != "survol":
            ...
            if (
                current_tool in {"text", "layout", "constraints"}
                and getattr(self, "_survol_context_kind", None)
            ):
                ...
                if host is not None:
                    ...
                    label = {
                        ...
                        "table": "Tableau sélectionné",
                    }.get(
                        str(getattr(self, "_survol_context_kind", "")),
                        "Élément sélectionné",
                    )
                    tk.Label(
                        host,
                        text=(
                            f"{label}. Le Survol est en pause pendant cette "
...
"""
    ...
    def _survol_prerequisites_ready(self) -> tuple[bool, str]:
        ...
        if not (
            isinstance(metadata, dict)
            and bool(metadata.get("book_import_state_reviewed", False))
        ):
            return False, "Le format du Livre doit d'abord être validé."
        ...
        try:
            ...
            rules = {}
        if not bool((rules or {}).get("confirmed", False)):
            return False, "Les règles générales du texte doivent d'abord être confirmées dans Format."
