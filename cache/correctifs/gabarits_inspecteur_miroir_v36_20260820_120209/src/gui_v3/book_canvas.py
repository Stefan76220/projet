...
class BookCanvas(tk.Frame):
    ...
    def __init__(
        ...
        *,
        ...
        # La molette Windows peut produire une rafale d'événements pour un seul
        # geste. On les regroupe avant d'effectuer le rendu : les boutons +/−, eux,
        # ne produisent qu'une commande à la fois et ne scintillent pas.
        self._gabarit_wheel_zoom_job = None
        self._gabarit_wheel_zoom_target: int | None = None
        self._gabarit_wheel_zoom_anchor: tuple[float, float] | None = None
        ...
        # Pendant un panoramique, la page vectorielle est remplacée visuellement
        # par une seule image raster temporaire. Cela évite à Tk/Windows de
        # repeindre des dizaines d'items Canvas à chaque mouvement de souris.
        self._gabarit_pan_proxy_id = None
        self._gabarit_pan_proxy_image = None
        self._gabarit_pan_proxy_items: list[int] = []
        self._gabarit_pan_proxy_origin: tuple[float, float] | None = None
    ...
    def _gabarit_draw_exception_ring(
        self, cx: float, cy: float, radius: float, *, width: int = 2, color: str = "#D7DEDC"
    ) -> None:
        ...
        self.canvas.create_arc(*box, start=118, extent=124, style="arc", outline=color, width=width)
        self.canvas.create_arc(*box, start=298, extent=124, style="arc", outline=color, width=width)
    ...
    def _gabarit_work_rect(self) -> tuple[float, float, float, float]:
        """Surface réellement disponible pour la page, hors outils, ligne et inspecteur."""
            ...
            if automatic:
                ...
                if self._gabarit_is_local_exception(item, idx):
                    ...
                    self._gabarit_draw_exception_ring(
                        ...
                        width=2 if active else 2, color="#E0E5E3" if active else "#BEC8C5",
                    )
            if active:
    ...
    def gabarit_set_zoom(self, value: int, *, anchor: tuple[float, float] | None = None) -> None:
        ...
        if value <= 100:
            ...
            # Au dézoomage, ne recalcule pas la position à partir du pointeur à
            # chaque cran : cela faisait dériver la feuille latéralement puis la
            # ramenait au centre par petits sauts. On conserve le déplacement
            # courant, mais on le résorbe progressivement vers le centre à mesure
            # que l'on revient vers Adapter (100 %). Le zoom avant reste, lui,
            # strictement ancré sous la souris.
            old_excess = max(1.0, float(current - 100))
            new_excess = max(0.0, float(value - 100))
            recenter = new_excess / old_excess
            self._gabarit_pan_x = float(getattr(self, "_gabarit_pan_x", 0.0)) * recenter
            self._gabarit_pan_y = float(getattr(self, "_gabarit_pan_y", 0.0)) * recenter
    ...
    def gabarit_step_zoom(self, delta: int, *, anchor: tuple[float, float] | None = None) -> None:
        """Un cran de zoom immédiat (boutons/clavier), sans temporisation."""
        current = int(getattr(self, "_gabarit_zoom", 100))
        step = 12 if current < 220 else 20
        value = current + (step if int(delta) > 0 else -step)
        self.gabarit_set_zoom(max(100, value), anchor=anchor)

    def _gabarit_queue_wheel_zoom(self, delta: int, *, anchor: tuple[float, float]) -> None:
        """Regroupe une rafale de molette en un seul rendu de zoom.

        Sous Windows, une rotation rapide génère plusieurs <MouseWheel> presque
        simultanés. Jusqu'ici chacun appelait gabarit_set_zoom() -> render(), ce
        qui expliquait le clignotement absent avec les boutons +/−. Le niveau
        final est calculé immédiatement mais le Canvas n'est reconstruit qu'une
        fois, juste après la fin de la rafale.
        """
        pending = getattr(self, "_gabarit_wheel_zoom_target", None)
        current = int(pending if pending is not None else getattr(self, "_gabarit_zoom", 100))
        step = 12 if current < 220 else 20
        target = max(100, min(900, current + (step if int(delta) > 0 else -step)))
        self._gabarit_wheel_zoom_target = target
        self._gabarit_wheel_zoom_anchor = (float(anchor[0]), float(anchor[1]))

        job = getattr(self, "_gabarit_wheel_zoom_job", None)
        if job is not None:
            try:
                self.after_cancel(job)
            except Exception:
                pass
        # 45 ms est assez court pour sembler immédiat, mais suffisamment long
        # pour absorber la rafale d'un cran de molette Windows/touchpad.
        self._gabarit_wheel_zoom_job = self.after(45, self._gabarit_commit_wheel_zoom)

    def _gabarit_commit_wheel_zoom(self) -> None:
        self._gabarit_wheel_zoom_job = None
        target = getattr(self, "_gabarit_wheel_zoom_target", None)
        anchor = getattr(self, "_gabarit_wheel_zoom_anchor", None)
        self._gabarit_wheel_zoom_target = None
        self._gabarit_wheel_zoom_anchor = None
        if target is None or anchor is None or getattr(self, "_work_mode", "") != "gabarits":
            return
        self.gabarit_set_zoom(int(target), anchor=anchor)
    ...
    def _gabarit_delete_key(self, _event=None):
        ...
        if str(getattr(self, "_work_mode", "")) != "gabarits":
            return None
        if not str(getattr(self, "_gabarit_selected_zone_id", "") or ""):
            return "break"
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        try:
            page_visual_before = set(self.canvas.find_all())
        except Exception:
            page_visual_before = set()
        ...
        # La feuille, ses guides, ses zones et ses flèches forment un groupe
        # persistant. Pendant un panoramique on déplace ce groupe sans jamais
        # effacer puis reconstruire le Canvas.
        try:
            for item_id in self.canvas.find_all():
                if item_id not in page_visual_before:
                    self.canvas.addtag_withtag("gabarit_page_visual", item_id)
        except Exception:
            pass

        # ----- zoom historique, discret à gauche de B -----
        zoom_controls = getattr(self, "gabarit_zoom_controls", None)
        left_separator_x = work_left - 8.0
        if zoom_controls is not None:
            try:
                zoom_controls.place(x=tools_w + 8.0, rely=0.54, anchor="w")
                zoom_controls.lift()
                zoom_controls.update_idletasks()
                zoom_w = float(zoom_controls.winfo_reqwidth())
                left_separator_x = max(left_separator_x, tools_w + 8.0 + zoom_w + 10.0)
            except Exception:
                pass

        # Trait fin à gauche : même rôle qu'à droite, mais il délimite toute la
        # zone OUTILS (familles + zoom) avant la surface de travail. Il n'est plus
        # dessiné à l'intérieur du rail.
        self.canvas.create_line(left_separator_x, 8.0, left_separator_x, max(8.0, viewport_h - 8.0), fill="#465A63", width=1)
        ...
        def control_box(
            key: str, y: float, h: float, *, selected: bool = False, pending: bool = False,
            ...
            pressed = str(getattr(self, "_gabarit_pressed_control", "")) == key
            if pending:
                fill = "#55442F" if not hovered and not pressed else "#655239"
                outline = "#D0A458"
            ...
            self.canvas.create_rectangle(
                ...
                width=2 if hovered or selected or pending else 1,
            )
        ...
        if self._gabarit_is_local_exception(active_item, int(self._selected_index)):
            # Repère explicite dans l'inspecteur, sur la même ligne que le titre de
            # section : aucune hauteur supplémentaire et aucun décalage des espaces
            # validés de l'inspecteur.
            badge_right = inner_right
            badge_icon_x = badge_right - 65.0
            self._gabarit_draw_exception_ring(badge_icon_x, page_active_header_y, 6.0, width=2, color="#D7DEDC")
            self.canvas.create_text(
                badge_right, page_active_header_y, text="EXCEPTION", anchor="e",
                fill="#D7DEDC", font=(theme.FONT_UI, 7, "bold"),
            )
        ...
        page_label = "Confirmer" if page_pending else "Cette page"
        self.canvas.create_text(
            (tx + scope_mid) / 2.0, y + scope_h / 2.0,
            text=page_label, anchor="center",
            fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if page_scope or page_pending else "normal"),
        )
        ...
        type_label = "Confirmer" if type_pending else "Toutes du type"
        self.canvas.create_text(
            (type_x1 + inner_right) / 2.0, y + scope_h / 2.0,
            text=type_label, anchor="center",
            fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if type_scope or type_pending else "normal"),
        )
        ...
        # Statut = information automatique, pas une commande.
        self.canvas.create_rectangle(
            tx, y, inner_right, y + status_h,
            fill="#202D36", outline="#465963", width=1,
        )
    ...
    def _gabarit_color_rgba(self, value: str, alpha: int = 255):
        value = str(value or "").strip()
        if not value:
            return None
        try:
            r16, g16, b16 = self.winfo_rgb(value)
            return (r16 // 257, g16 // 257, b16 // 257, int(alpha))
        except Exception:
            return None

    def _gabarit_build_pan_proxy(self) -> bool:
        """Rasterise une fois le groupe page dans une image transparente.

        La zone OUTILS de gauche est stable parce qu'elle est composée de vrais
        widgets Windows, pas d'une scène Canvas redessinée. Pour rapprocher le
        comportement de la page de ce modèle, le panoramique manipule désormais
        un seul bitmap temporaire au lieu de dizaines d'items vectoriels.
        """
        ...
        try:
            items = [int(i) for i in self.canvas.find_withtag("gabarit_page_visual")]
        except Exception:
            items = []
        if not items:
            return False
        try:
            bbox = self.canvas.bbox("gabarit_page_visual")
        except Exception:
            bbox = None
        if not bbox:
            return False
        ...
        pad = 4
        width = max(2, int(round(x2 - x1)) + pad * 2)
        height = max(2, int(round(y2 - y1)) + pad * 2)
        # Garde-fou mémoire pour un zoom extrême accidentel.
        ...
        def rel(coords):
            ...
            for idx, value in enumerate(coords):
                result.append(float(value) - (ox if idx % 2 == 0 else oy))
            return result

        def item_width(item_id, default=1):
            try:
                return max(1, int(round(float(self.canvas.itemcget(item_id, "width") or default))))
            except Exception:
                return default

        for item_id in items:
            try:
                kind = str(self.canvas.type(item_id) or "")
                coords = rel(self.canvas.coords(item_id))
                state = str(self.canvas.itemcget(item_id, "state") or "normal")
                if state == "hidden":
                    continue
            except Exception:
                continue
            try:
                if kind == "rectangle" and len(coords) >= 4:
                    fill = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "fill"))
                    outline = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "outline"))
                    if fill is not None:
                        draw.rectangle(coords[:4], fill=fill)
                    if outline is not None:
                        draw.rectangle(coords[:4], outline=outline, width=item_width(item_id))
                elif kind == "oval" and len(coords) >= 4:
                    fill = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "fill"))
                    outline = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "outline"))
                    if fill is not None:
                        draw.ellipse(coords[:4], fill=fill)
                    if outline is not None:
                        draw.ellipse(coords[:4], outline=outline, width=item_width(item_id))
                elif kind == "line" and len(coords) >= 4:
                    fill = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "fill"))
                    if fill is not None:
                        draw.line(coords, fill=fill, width=item_width(item_id), joint="curve")
                    ...
                    if outline is not None:
                        draw.line(points + [points[0]], fill=outline, width=item_width(item_id), joint="curve")
                elif kind == "text" and len(coords) >= 2:
                    label = str(self.canvas.itemcget(item_id, "text") or "")
                    fill = self._gabarit_color_rgba(self.canvas.itemcget(item_id, "fill"))
                    if label and fill is not None:
                        size = 10
                        try:
                            spec = self.canvas.itemcget(item_id, "font")
                            actual = tkfont.Font(font=spec).actual()
                            raw_size = int(actual.get("size", 10) or 10)
                            size = max(7, abs(raw_size))
                        except Exception:
                            pass
                        font = None
                        try:
                            win_font = Path(r"C:\Windows\Fonts\arial.ttf")
                            if win_font.exists():
                                font = ImageFont.truetype(str(win_font), size=size)
                        except Exception:
                            font = None
                        if font is None:
                            try:
                                ...
                                font = None
                        anchor = str(self.canvas.itemcget(item_id, "anchor") or "center")
                        pil_anchor = {
                            "center": "mm", "n": "ma", "s": "md",
                            "w": "lm", "e": "rm", "nw": "la", "ne": "ra",
                            "sw": "ld", "se": "rd",
                        }.get(anchor, "mm")
                        try:
                            draw.text((coords[0], coords[1]), label, fill=fill, font=font, anchor=pil_anchor)
                        except Exception:
                            draw.text((coords[0], coords[1]), label, fill=fill, font=font)
            except Exception:
                # Un item décoratif non pris en charge ne doit jamais bloquer le pan.
                continue

        try:
            tk_image = ImageTk.PhotoImage(image)
            proxy_id = self.canvas.create_image(ox, oy, image=tk_image, anchor="nw", tags=("gabarit_pan_proxy",))
            # Le proxy doit rester sous l'interface fixe de droite ; le rail gauche
            # est de toute façon une fenêtre Tk distincte et reste au-dessus.
            self.canvas.tag_lower(proxy_id)
            # Cache la scène vectorielle sans la supprimer : aucune reconstruction.
            for item_id in items:
                try:
                    self.canvas.itemconfigure(item_id, state="hidden")
                except Exception:
                    pass
            self._gabarit_pan_proxy_id = proxy_id
            self._gabarit_pan_proxy_image = tk_image
            self._gabarit_pan_proxy_items = items
            self._gabarit_pan_proxy_origin = (float(ox), float(oy))
            # Force la première image stable avant que les mouvements rapides commencent.
            self.canvas.update_idletasks()
            return True
        except Exception:
            try:
                if 'proxy_id' in locals():
                    self.canvas.delete(proxy_id)
            except Exception:
                pass
            self._gabarit_pan_proxy_id = None
            self._gabarit_pan_proxy_image = None
            self._gabarit_pan_proxy_items = []
            self._gabarit_pan_proxy_origin = None
            return False

    def _gabarit_finish_pan_proxy(self, dx: float, dy: float) -> None:
        proxy_id = getattr(self, "_gabarit_pan_proxy_id", None)
        items = list(getattr(self, "_gabarit_pan_proxy_items", []) or [])
        if proxy_id is None:
            return
        # Les vecteurs sont déplacés alors qu'ils sont cachés, puis réaffichés au
        # même emplacement que l'image. Ainsi aucune phase intermédiaire n'est visible.
        try:
            if dx or dy:
                self.canvas.move("gabarit_page_visual", float(dx), float(dy))
            for item_id in items:
                try:
    ...
    def _gabarit_begin_pan(self, x: float, y: float) -> str:
        ...
        # Un seul bitmap est déplacé pendant le geste. Si Pillow est absent ou
        # qu'un proxy ne peut être créé, on conserve le repli vectoriel historique.
        self._gabarit_build_pan_proxy()
        try:
    ...
    def _gabarit_pan_drag(self, event):
        ...
        visual_dx = float(self._gabarit_pan_x) - float(self._gabarit_pan_origin[0])
        visual_dy = float(self._gabarit_pan_y) - float(self._gabarit_pan_origin[1])
        old_dx, old_dy = self._gabarit_pan_visual_offset
        step_x = visual_dx - float(old_dx)
        step_y = visual_dy - float(old_dy)

        if step_x or step_y:
            proxy_id = getattr(self, "_gabarit_pan_proxy_id", None)
            if proxy_id is not None:
                self.canvas.move(proxy_id, step_x, step_y)
            else:
                self.canvas.move("gabarit_page_visual", step_x, step_y)
            self._gabarit_shift_page_hitboxes(step_x, step_y)
            self._gabarit_pan_visual_offset = (visual_dx, visual_dy)
            # Sous Windows, laisser Tk vider immédiatement sa file de redisplay
            # évite l'accumulation de zones sales lors d'un drag rapide.
            try:
                self.canvas.update_idletasks()
            except Exception:
                pass
    ...
    def _gabarit_finish_pan(self):
        ...
        Le proxy raster est remplacé par les vecteurs déjà existants, déplacés
        pendant qu'ils sont cachés. Il n'y a donc ni delete('all'), ni création
        d'une nouvelle scène au relâchement.
        """
        visual_dx, visual_dy = self._gabarit_pan_visual_offset
        self._gabarit_finish_pan_proxy(float(visual_dx), float(visual_dy))
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            self._gabarit_update_zone_canvas(str(zone.get("id") or ""), zone)
            return "break"
        return self._gabarit_pan_drag(event)
    ...
    def _on_ctrl_mousewheel(self, event):
        if getattr(self, "_work_mode", "structure") == "gabarits":
            ...
            if work_left <= x <= work_right and work_top <= y <= work_bottom:
                self._gabarit_queue_wheel_zoom(1 if event.delta > 0 else -1, anchor=(x, y))
            return "break"
        return "break"
