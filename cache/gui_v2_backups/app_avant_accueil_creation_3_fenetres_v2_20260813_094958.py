...
class PageMaitreV2(tk.Tk):
    ...
    def _open_accueil_create_project_window(self):
        ...
        self._accueil_center_base_window(win, 720, 510)

        body = self._accueil_base_shell(
            win,
            title="Créer un nouveau projet",
            subtitle=(
                "Définissez les premières informations du projet. "
                "Cette fenêtre est une base visuelle."
            ),
            accent="#579B73",
        )
        ...
        types = tk.Frame(body, bg="#FFFEFC")
        types.pack(fill="x", pady=(0, 20))

        for title, subtitle, accent in (
            ("Ouvrage structuré", "Fiches, guides, catalogues", "#579B73"),
            ("Livre textuel", "Roman, récit, essai", "#8564B1"),
            ("Bande dessinée", "Planches, cases, narration", "#E06B4E"),
        ):
            item = tk.Frame(
                types,
                bg="#FAF8F4",
