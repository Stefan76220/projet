# TOMELINEA_GABARITS_GROUPES_V69
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "create":
            ...
            ),button=39.0,gap=12.0,y_offset=63.0,center=False,start_x=fly_left+11.0)
            ...
            icon_strip((
                ("zone_duplicate","duplicate",False,"Dupliquer la sélection" if count>1 else "Dupliquer",False),
    ...
    def _gabarit_inspector_band_for_key(self, key: str) -> str:
        ...
        mapping = {
            ...
            "zone_lock":"snap", "zone_unlock_all":"snap", "zone_duplicate":"actions", "zone_delete":"actions",
    ...
    def gabarit_align_selected_zones(self, mode: str) -> bool:
        ...
        for unit in units:
            sample = unit[0]
            rx, ry, rw, rh = self._gabarit_alignment_reference_box(sample, reference)
        ...
        if changed:
            ...
            self.status_var.set(f"{unit_count} ensemble{'s' if unit_count > 1 else ''} aligné{'s' if unit_count > 1 else ''} sur {article} {label}")
        return changed
