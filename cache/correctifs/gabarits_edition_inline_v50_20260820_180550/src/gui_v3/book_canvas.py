...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        h = 58.0
        self._gabarit_inspector_control_box(target, "zone_geometry", (tx,y,right,y+h))
        target.create_text(tx+10, y+17, text=f"X  {float(rect.get('x',0)):g} mm     Y  {float(rect.get('y',0)):g} mm", anchor="w", fill="#E7ECEA", font=(theme.FONT_UI, 8, "bold"))
        target.create_text(tx+10, y+40, text=f"L  {float(rect.get('w',0)):g} mm     H  {float(rect.get('h',0)):g} mm", anchor="w", fill="#C7D1CF", font=(theme.FONT_UI, 8))
        target.create_text(right-8, y+h/2, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += h + 14
        ...
        # Changer de référentiel reste un geste à 3 clics maximum :
        # sélection zone -> Marges/Page -> Cette page/Toutes du type.
        gap=4.0; bw=(inner-gap)/2.0
        self._gabarit_inspector_control_box(target,"align_ref_margins",(tx,y,tx+bw,y+29),selected=(alignment_ref=="margins" and not pending_ref) or pending_ref=="margins")
        target.create_text(tx+bw/2,y+14.5,text="Marges",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold" if alignment_ref=="margins" else "normal"))
        self._gabarit_inspector_control_box(target,"align_ref_page",(tx+bw+gap,y,right,y+29),selected=(alignment_ref=="page" and not pending_ref) or pending_ref=="page")
        target.create_text(tx+bw+gap+bw/2,y+14.5,text="Page",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold" if alignment_ref=="page" else "normal"))
        y += 35
