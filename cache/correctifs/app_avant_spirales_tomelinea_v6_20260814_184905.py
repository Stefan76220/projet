...
class PageMaitreV2(tk.Tk):
    ...
    def _get_notebook_spiral_photo(
        ...
        *,
        ...
        # Prototype réaliste affiné : plus fin et un peu éclairci.
        proto_h = max(110, int(base.height * (target_w / base.width)))
        proto = base.resize((target_w, proto_h), Image.Resampling.LANCZOS)
        proto_rgb = ImageEnhance.Brightness(proto.convert("RGB")).enhance(1.12)
        proto_rgb = ImageEnhance.Contrast(proto_rgb).enhance(1.04)
        proto = proto_rgb.convert("RGBA")
        proto.putalpha(base.resize((target_w, proto_h), Image.Resampling.LANCZOS).getchannel("A"))

        # On extrait une spire courte au centre du prototype.
        # Chaque spire est ensuite posée librement, avec un pas régulier,
        # pour éviter l'effet de colonne grise continue.
        head_h = 10
        ...
        # Teinte très légère et discrète.
        try:
        ...
        tint_alpha = alpha.point(lambda v: int(v * 0.04))
        tint.putalpha(tint_alpha)
        final = Image.alpha_composite(final, tint)

        final_alpha = final.getchannel("A").point(lambda v: int(v * opacity))
        final.putalpha(final_alpha)

        # Petits trous plus fins et plus discrets à droite.
    ...
    def _draw_notebook_spiral_canvas(
        ...
        *,
        ...
        # Centrage plus propre sur la ligne gauche.
    ...
    def _attach_notebook_spiral_frame(self, frame, color: str) -> None:
        ...
        spring.place(x=0, y=2, width=22, relheight=1.0)

        def redraw(_event=None):
            spring.configure(bg=frame.cget("bg"))
            spring.delete("all")
            photo = self._get_notebook_spiral_photo(
                max(34, spring.winfo_height() - 4),
                color,
                opacity=0.78,
            )
            if photo is None:
                return
            x = int(-(photo.width() / 2) + 1)
