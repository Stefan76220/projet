...
def _count_canvas_type(items: list[dict[str, Any]], item_type: str) -> int:
    """Compte un type Canvas, y compris dans les valueList de listes."""
    total = 0
    wanted = str(item_type).casefold()
    for item in items:
        if not isinstance(item, dict):
            continue
        if str(item.get("type") or "").casefold() == wanted:
            total += 1
        nested = item.get("valueList")
        if isinstance(nested, list):
            total += _count_canvas_type(nested, item_type)
    return total
...
def _character_element(run: dict[str, Any], text: str) -> dict[str, Any]:
    ...
    source_family = str(fmt.get("font_family") or "").strip()
    item["font"] = WORKING_FONT_FAMILY
    tl_meta = item["extension"]["tomelinea"]
    if source_family:
        tl_meta["sourceFontFamily"] = source_family
    tl_meta["sourceCharacterFormat"] = deepcopy(fmt)
...
def _image_element(
    ...
    *,
    ...
    # Au premier import, l'image reste un repère logique dans le flux. Les
    # coordonnées/habillages Word sont conservés dans l'extension Source, mais
    # ne deviennent pas des contraintes de composition TomeLinea.
    item["imgDisplay"] = "inline"
    tl_extension["sourcePlacement"] = {
        "mode": image.get("mode"),
        "paragraph_index": image.get("paragraph_index"),
        "run_index": image.get("run_index"),
        "anchor_text": str(anchor_text or ""),
        "position_h_relative_to": image.get("position_h_relative_to"),
        "position_h_offset_mm": image.get("position_h_offset_mm"),
        "position_v_relative_to": image.get("position_v_relative_to"),
        "position_v_offset_mm": image.get("position_v_offset_mm"),
        "wrap": image.get("wrap"),
    }
    tl_extension["placementStatus"] = "source_marker_pending_tomelinea_decision"
    return item
...
def _default_text_style(contract: dict[str, Any]) -> tuple[str, float]:
    for block in contract.get("content", []):
        ...
        for run in block.get("runs", []):
            ...
            size = pt_to_px(fmt.get("size_pt"))
            if size:
                return WORKING_FONT_FAMILY, size
    return WORKING_FONT_FAMILY, 16.0
    ...
    for index, section in enumerate(sections):
        ...
        for block in content_by_section[index]:
            if block.get("kind") == "paragraph":
                ...
                translated_counts["images"] += _count_canvas_type(items, "image")
                translated_counts["page_breaks"] += _count_canvas_type(items, "pageBreak")
                translated_counts["lists"] += _count_canvas_type(items, "list")
