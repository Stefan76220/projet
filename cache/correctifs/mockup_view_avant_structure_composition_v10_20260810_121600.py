...
class MockupView:
    ...
    def _create_group_block(
        ...
        """Un groupe montre ses pages prévues, jamais sa bibliothèque de types."""
        group_id = str(group_definition.get("id", ""))
        ...
        compact_width = max(110, min(160, width))
        ...
        title_bar = ctk.CTkFrame(
            block, height=22, fg_color=title_soft, corner_radius=6,
        )
        ...
        # Pages réelles appartenant au groupe.
        group_items = []
        for item in self._items():
            definition = self._definition_for(str(item.get("type", "")))
            if str(definition.get("group", "")) == group_id:
                group_items.append((item, definition))

        pages_frame = ctk.CTkScrollableFrame(
            block,
        ...
        for item, definition in group_items:
            ...
            for occurrence in range(1, count + 1):
                ...
                row = ctk.CTkButton(
                    pages_frame,
                    text=f"{page_number:02d}  {page_name}",
                    height=22,
                    corner_radius=4,
                    fg_color=self._mix_color_with_white(accent, 0.95),
                    hover_color=self._mix_color_with_white(accent, 0.84),
                    text_color=self.INK,
                    border_width=0,
                    font=(Fonts.FAMILY, 8),
                    anchor="w",
        ...
        # Un seul geste de création, clairement séparé des pages existantes.
        add_button = ctk.CTkButton(
            ...
            corner_radius=5,
            fg_color=title_soft,
            hover_color=self._mix_color_with_white(accent, 0.72),
            text_color=accent,
            border_width=1,
            border_color=self._mix_color_with_white(accent, 0.60),
            font=(Fonts.FAMILY, 8, "bold"),
            command=lambda gid=group_id: self._show_add_page_menu(gid),
        )
        add_button.grid(row=2, column=0, sticky="ew", padx=4, pady=(1, 4))
    ...
    def _refresh_sequence(self) -> None:
        """Centre = pages réelles du groupe sélectionné, une vignette par page."""
        ...
        active_ids = {str(item.get("id", "")) for item in source_items}
        ...
        for item in source_items:
            ...
            for occurrence_index in range(1, count + 1):
                pages.append((item, occurrence_index, count))

        if not pages:
            self._sequence_empty_label = ctk.CTkLabel(
                self._sequence_frame,
                text="Aucune page prévue dans ce groupe.\nUtilisez un bouton + Type ci-dessus pour en créer une.",
                font=Fonts.NORMAL,
                text_color=self.TEXT_LIGHT,
                justify="center",
            )
            self._sequence_empty_label.grid(
                row=0, column=0, sticky="nsew", padx=20, pady=28
            )
        else:
            self._sequence_empty_label = None
            try:
                available_width = max(520, int(self._sequence_frame.winfo_width()))
            except Exception:
                available_width = 760
            ...
            for column in range(columns):
                self._sequence_frame.grid_columnconfigure(
                    column, weight=1, uniform="real_pages"
                )

            for display_index, (item, occurrence_index, occurrence_total) in enumerate(pages):
