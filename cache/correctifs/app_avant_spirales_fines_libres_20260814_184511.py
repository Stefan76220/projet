...
class PageMaitreV2(tk.Tk):
    ...
    def _get_notebook_spiral_photo(
        ...
        *,
        ...
        # SPIRALE_CENTREE_LONGUE_TROUS_OMBRE_V4
        ...
        if base is None:
            ...
            try:
                ...
                # les trous du papier sont à droite.
        ...
        # Largeur fixe pour toutes les zones.
        target_w = 26
        target_h = max(34, int(height))
        key = (target_w, target_h, accent, round(opacity, 2))
        photo = cache.get(key)
        if photo is not None:
            return photo

        # On garde l'aspect actuel du ressort :
        # largeur fixe, proportions préservées,
        # puis on le prolonge verticalement avec répétition du corps central
        # pour qu'il soit plus long sans être déformé.
        base_h = max(1, int(base.height * (target_w / base.width)))
        prototype = base.resize((target_w, base_h), Image.Resampling.LANCZOS)

        if target_h <= prototype.height:
            final = prototype.resize((target_w, target_h), Image.Resampling.LANCZOS)
        else:
            cap = min(18, prototype.height // 4)
            top = prototype.crop((0, 0, target_w, cap))
            body = prototype.crop((0, cap, target_w, prototype.height - cap))
            bottom = prototype.crop((0, prototype.height - cap, target_w, prototype.height))

            final = Image.new("RGBA", (target_w, target_h), (0, 0, 0, 0))
            final.paste(top, (0, 0))
            y = cap
            body_h = max(1, body.height)
            limit = target_h - cap
            while y < limit:
                take_h = min(body_h, limit - y)
                if take_h == body_h:
                    tile = body
                else:
                    tile = body.crop((0, 0, target_w, take_h))
                final.paste(tile, (0, y))
                y += take_h
            final.paste(bottom, (0, target_h - cap))
        # Légère mise en lumière pour mieux la voir.
        rgb = final.convert("RGB")
        rgb = ImageEnhance.Brightness(rgb).enhance(1.10)
        rgb = ImageEnhance.Contrast(rgb).enhance(1.05)
        final = rgb.convert("RGBA").copy()
        final.putalpha(final.getchannel("A"))

        try:
            rgb16 = self.winfo_rgb(accent)
            accent_rgb = tuple(v // 256 for v in rgb16)
        except Exception:
            accent_rgb = (143, 160, 174)

        # Teinte d'accent très légère.
        alpha = final.getchannel("A")
        tint = Image.new("RGBA", final.size, accent_rgb + (0,))
        tint_alpha = alpha.point(lambda v: int(v * 0.05))
        tint.putalpha(tint_alpha)
        final = Image.alpha_composite(final, tint)

        # Opacité du ressort.
        spring_alpha = final.getchannel("A").point(lambda v: int(v * opacity))
        final.putalpha(spring_alpha)

        # Trous du papier à droite :
        # ombre plus marquée + cœur très sombre,
        # pour voir que le ressort plonge dans le papier.
        ...
        while y < target_h - 8:
            draw.ellipse(
                (hole_x1 - 1, y - 4, hole_x2 + 1, y + 5),
                fill=(0, 0, 0, 70),
            )
            draw.ellipse(
                (hole_x1, y - 3, hole_x2, y + 4),
                fill=(0, 0, 0, 155),
            )
            draw.ellipse(
                (hole_x1 + 1, y - 2, hole_x2 - 1, y + 3),
                fill=(10, 11, 14, 225),
            )
            y += hole_step
    ...
    def _draw_notebook_spiral_canvas(
        ...
        *,
        ...
        photo = self._get_notebook_spiral_photo(
            max(34, int(y2 - y1) - 2),
            color,
            opacity=0.80,
        )
        ...
        # Centrage sur la ligne gauche de la zone :
        # le ressort chevauche la ligne pour un rendu réaliste.
        img_x = int(x - (photo.width() / 2))
        img_y = int(y1 + max(0, ((y2 - y1) - photo.height()) / 2))
        canvas.create_image(
            img_x,
            img_y,
            image=photo,
    ...
    def _attach_notebook_spiral_frame(self, frame, color: str) -> None:
        spring = tk.Canvas(
            frame,
            bg=frame.cget("bg"),
            width=30,
        ...
        spring.place(x=0, y=1, width=30, relheight=1.0)

        def redraw(_event=None):
            spring.configure(bg=frame.cget("bg"))
            spring.delete("all")
            photo = self._get_notebook_spiral_photo(
                max(34, spring.winfo_height() - 2),
                color,
                opacity=0.82,
            )
            if photo is None:
                return
            # Le centre du ressort vient chevaucher la ligne gauche.
            x = int(-(photo.width() / 2) + 2)
            y = max(0, (spring.winfo_height() - photo.height()) // 2)
            spring.create_image(
                x,
                y,
                image=photo,
        ...
        spring.bind("<Configure>", redraw, add="+")
        spring.tk.call("raise", spring._w)
        redraw()
