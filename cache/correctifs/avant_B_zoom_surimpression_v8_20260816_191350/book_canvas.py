...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        self._tool_button(zoom_controls, "Livre", self.fit_book, width=5).pack(side="left", padx=(0, 3))
    ...
    def set_zoom(self, value: int, *, center_selected=True):
        """Zoome directement dans la ligne du livre, sans changer de vue."""
        value = max(self.MIN_ZOOM, min(self.MAX_ZOOM, int(value)))
        # La vue détaillée séparée n'est plus utilisée : le zoom reste toujours
        # dans B et conserve parties, pages et navigateur visibles.
        if self._page_focus:
            self._set_page_focus(False)
    ...
    def fit_selected(self):
        """Agrandit la page sélectionnée dans la ligne, sans ouvrir d'autre vue."""
        ...
        factor = self.PAGE_SIZE_SELECTED
        viewport_w = max(300, self.canvas.winfo_width() - 90)
        available_h = self._normal_page_available_height()
        # On garde un peu de contexte horizontal et toute la hauteur de page.
        zoom_x = int((viewport_w * 0.72) / (self.BASE_PAGE_W * factor) * 100)
        zoom_y = int(available_h / (self.BASE_PAGE_H * factor) * 100)
        value = max(self.MIN_ZOOM, min(self.MAX_ZOOM, zoom_x, zoom_y))
        self.set_zoom(value, center_selected=True)
    ...
    def render(self):
        ...
        # Le zoom reste dans la même ligne du livre. Il n'est donc plus
        # plafonné artificiellement pour basculer vers une vue détaillée.
    ...
    def _on_double_click(self, event):
        # Le double-clic n'ouvre plus une fenêtre de visionnage : le zoom de B
        # remplit déjà ce rôle. Il reste réservé à l'édition directe d'une partie.
        index = self._index_at(event)
        if index is not None:
            self._selected_index = index
            self._selected_group_id = self._item_group_id(self.items[index])
