...
"""
    ...
    # ==========================================================
    # Phase 2.40C — ETAPE 1 : REGLES GENERALES UNIQUEMENT
    # ==========================================================

    def _composition_build_tool_rail(self, parent):
        """Pendant l'étape 2.40C, seul Format est exposé.

        Le Survol et les outils contextuels reviendront seulement après
        validation du fonctionnement réel des règles générales.
        """
        rail = tk.Frame(parent, bg=theme.PANEL_ALT, width=78)
        rail.pack_propagate(False)
        self._composition_tool_tabs = {}
        tab = tk.Canvas(
            rail, width=76, height=44, bg=theme.ACCENT_DARK,
            highlightthickness=0, bd=0, cursor="arrow", takefocus=False,
        )
        tab.pack(fill="x", padx=1, pady=1)
        text_id = tab.create_text(
            38, 22, text="Format", width=70, justify="center",
            fill=theme.WHITE, font=(theme.FONT_UI, 9, "bold"),
        )
        self._composition_tool_tabs["book"] = (tab, text_id)
        self._composition_active_tool = "book"
        return rail
    ...
    def _composition_activate_tool(self, tool: str) -> None:
        # 2.40C : aucun changement de domaine n'est possible.
        self._composition_active_tool = "book"
        self._composition_book_state_open = True
        try:
            self.session.clear_selection()
    ...
    def _general_rules_source(self) -> dict:
        ...
        try:
            ...
            existing = {}
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        inferred = infer_rules_from_plans(plans) if plans else normalize_general_text_rules(existing)
        merged = dict(inferred)
        for key, value in existing.items():
            if value not in (None, ""):
                merged[key] = value
        # L'ancienne règle stockait seulement un pas de ligne en mm.
        if "line_spacing_multiple" not in merged:
            try:
                ...
                base = size * 25.4 / 72.0
                merged["line_spacing_multiple"] = round(pitch / base, 2) if pitch > 0 and base > 0 else inferred.get("line_spacing_multiple", 1.15)
            except Exception:
    ...
    @staticmethod
    def _general_rules_number(raw, label: str, minimum: float, maximum: float) -> float:
    ...
    def _general_rules_apply_book_content(self, book, rules: dict) -> int:
        """Met à jour le modèle Livre pour que la règle soit persistante.

        Seul le corps de texte reconnu est modifié ; les titres restent hors
    ...
    def _general_rules_reload_active_canvas(self) -> None:
        """Recharge uniquement l'unité visible avec les plans déjà modifiés."""
        host = getattr(self, "_phase2_canvas_host", None)
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        ...
        try:
            ...
            fresh = CanvasLoadSession(contract)
            host.load_document(plans[int(index)], fresh)
            # Le rappel différé est volontaire : Canvas doit recalculer ses lignes
            # et sa pagination avant de repositionner la page locale.
            def restore_page():
                try:
                    if bool(getattr(host, "ready", False)):
                        host.go_to_page(previous_local_page, smooth=False)
                    else:
                        self.after(120, restore_page)
                except Exception:
                    pass
            self.after(150, restore_page)
        except Exception:
            pass

    def _general_rules_apply(self) -> None:
        from src.v4.general_text_rules_runtime import (
            apply_rules_to_plan,
            normalize_general_text_rules,
        )
        vars_map = getattr(self, "_general_rules_vars", None)
        ...
        try:
            ...
            size = self._general_rules_number(vars_map["font_size_pt"].get(), "Taille", 6, 72)
            indent = self._general_rules_number(vars_map["first_line_indent_mm"].get(), "Retrait", 0, 30)
            line = self._general_rules_number(vars_map["line_spacing_multiple"].get(), "Interligne", 0.8, 3)
            gap = self._general_rules_number(vars_map["paragraph_gap_mm"].get(), "Espace après", 0, 20)
            ...
            self._general_rules_notice = str(exc)
            self._composition_update_inspector_context()
            return
        ...
        def action(project):
            ...
            project.book.metadata["text_general_rules"] = saved
            project.book.metadata["text_reflow_requested"] = True
            changed = self._general_rules_apply_book_content(project.book, rules)
            project.touch()
            return changed

        try:
            changed_book = int(self.session.execute("Règles générales du texte", action) or 0)
        except Exception as exc:
            self._general_rules_notice = str(exc)
            self._composition_update_inspector_context()
            return
        ...
        for plan in plans:
            try:
                report = apply_rules_to_plan(plan, rules)
                paragraphs += int(report.get("paragraphs", 0))
                elements += int(report.get("elements", 0))
            except Exception:
                pass

        self._general_rules_notice = (
            f"Règles appliquées : {paragraphs or changed_book} paragraphe(s) de texte courant. "
            "Les titres restent inchangés."
        )
        self._general_rules_last_saved = dict(rules)
        self._general_rules_reload_active_canvas()
        try:
    ...
    def _general_rules_open_margins(self) -> None:
        """Marges dans une petite fenêtre : elles ne tassent plus le panneau."""
        fmt = self.session.book.format
        win = tk.Toplevel(self)
        win.title("Marges du livre")
        ...
        box = tk.Frame(win, bg=theme.PANEL, padx=18, pady=16)
        box.pack(fill="both", expand=True)
        tk.Label(box, text="Marges générales", bg=theme.PANEL, fg=theme.INK,
                 font=(theme.FONT_UI, 11, "bold")).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0,10))
        labels = (("top","Haut"),("bottom","Bas"),("inside","Intérieure"),("outside","Extérieure"))
        for row,(key,label) in enumerate(labels,1):
            tk.Label(box,text=label,bg=theme.PANEL,fg=theme.INK,font=(theme.FONT_UI,9)).grid(row=row,column=0,sticky="w",pady=3)
            tk.Entry(box,textvariable=values[key],width=8,bg=theme.PANEL_SOFT,fg=theme.INK,insertbackground=theme.INK,
                     relief="flat",justify="right",font=(theme.FONT_UI,9)).grid(row=row,column=1,padx=(12,4),pady=3)
            tk.Label(box,text="mm",bg=theme.PANEL,fg=theme.MUTED,font=(theme.FONT_UI,8)).grid(row=row,column=2,sticky="w")
        def apply():
            try:
                parsed = {k: float(v.get().strip().replace(",", ".")) for k,v in values.items()}
            except Exception:
                messagebox.showerror("Marges", "Entrez quatre valeurs numériques.", parent=win)
                return
            if self._composition_apply_general_settings(
                margin_top_mm=parsed["top"], margin_bottom_mm=parsed["bottom"],
                margin_inside_mm=parsed["inside"], margin_outside_mm=parsed["outside"],
            ):
                win.destroy()
        self._button(box,"Appliquer",apply,compact=True,accent=True).grid(row=6,column=0,columnspan=3,sticky="ew",pady=(12,0))
        try:
    ...
    def _composition_render_book_state_panel(self, host) -> None:
        """2.40C : panneau aéré et limité aux réglages globaux testés."""
        from src.v4.format_catalog import find_standard_format
        ...
        current_label = (
            f"{current.label} · {float(fmt.width_mm):.1f} × {float(fmt.height_mm):.1f} mm"
            if current is not None
            else f"{float(fmt.width_mm):.1f} × {float(fmt.height_mm):.1f} mm"
        )

        # Format : une seule ligne + une action.
        tk.Label(host, text="FORMAT DU LIVRE", bg=theme.PANEL, fg=theme.ACCENT_BRIGHT,
                 font=(theme.FONT_UI, 8, "bold"), anchor="w").pack(fill="x", padx=16, pady=(12,4))
        tk.Label(host, text=current_label, bg=theme.PANEL_SOFT, fg=theme.INK,
                 font=(theme.FONT_UI, 10, "bold"), anchor="w").pack(fill="x", padx=16, ipady=8)
        self._button(host,"Changer le format",self._composition_open_format_chooser,compact=True).pack(fill="x",padx=16,pady=(6,3))

        # Marges : résumé seulement. Les quatre champs s'ouvrent à la demande.
        margins = (
            f"H {float(fmt.margin_top_mm):g} · B {float(fmt.margin_bottom_mm):g} · "
            f"Int {float(fmt.margin_inside_mm):g} · Ext {float(fmt.margin_outside_mm):g} mm"
        )
        tk.Label(host, text="MARGES", bg=theme.PANEL, fg=theme.MUTED,
                 font=(theme.FONT_UI, 8, "bold"), anchor="w").pack(fill="x", padx=16, pady=(10,2))
        tk.Label(host, text=margins, bg=theme.PANEL, fg=theme.INK,
                 font=(theme.FONT_UI, 8), anchor="w", wraplength=220, justify="left").pack(fill="x", padx=16)
        self._button(host,"Modifier les marges",self._general_rules_open_margins,compact=True).pack(fill="x",padx=16,pady=(5,5))

        tk.Frame(host,bg=theme.PAGE_BORDER,height=1).pack(fill="x",padx=14,pady=(8,8))
        tk.Label(host,text="RÈGLES GÉNÉRALES DU TEXTE",bg=theme.PANEL,fg=theme.ACCENT_BRIGHT,
                 font=(theme.FONT_UI,9,"bold"),anchor="w").pack(fill="x",padx=16,pady=(0,2))
        tk.Label(host,text="Texte courant uniquement. Les titres restent indépendants.",bg=theme.PANEL,fg=theme.MUTED,
                 font=(theme.FONT_UI,7),anchor="w",wraplength=220,justify="left").pack(fill="x",padx=16,pady=(0,7))

        rules = self._general_rules_source()
        saved = getattr(self, "_general_rules_last_saved", None)
        ...
        self._general_rules_vars = vars_map

        def field(label, key, suffix=""):
            row = tk.Frame(host,bg=theme.PANEL)
            row.pack(fill="x",padx=16,pady=2)
            tk.Label(row,text=label,bg=theme.PANEL,fg=theme.INK,font=(theme.FONT_UI,8),anchor="w").pack(side="left",fill="x",expand=True)
            if suffix:
                tk.Label(row,text=suffix,bg=theme.PANEL,fg=theme.MUTED,font=(theme.FONT_UI,7)).pack(side="right",padx=(5,0))
            tk.Entry(row,textvariable=vars_map[key],width=9,bg=theme.PANEL_SOFT,fg=theme.INK,insertbackground=theme.INK,
                     relief="flat",bd=0,justify="right",font=(theme.FONT_UI,8)).pack(side="right",ipady=3)

        # Police en largeur entière pour ne pas tasser le nom.
        tk.Label(host,text="Police",bg=theme.PANEL,fg=theme.INK,font=(theme.FONT_UI,8),anchor="w").pack(fill="x",padx=16,pady=(2,2))
        tk.Entry(host,textvariable=vars_map["font_family"],bg=theme.PANEL_SOFT,fg=theme.INK,insertbackground=theme.INK,
                 relief="flat",bd=0,font=(theme.FONT_UI,8)).pack(fill="x",padx=16,ipady=4,pady=(0,3))
        field("Taille", "font_size_pt", "pt")
        field("Retrait 1re ligne", "first_line_indent_mm", "mm")
        field("Interligne", "line_spacing_multiple", "×")
        field("Espace après", "paragraph_gap_mm", "mm")

        def choice_row(title, variable, options):
            tk.Label(host,text=title,bg=theme.PANEL,fg=theme.INK,font=(theme.FONT_UI,8,"bold"),anchor="w").pack(fill="x",padx=16,pady=(6,2))
            row = tk.Frame(host,bg=theme.PANEL)
            ...
            def refresh():
                for button,value in buttons:
                    ...
                    button.configure(bg=theme.ACCENT_DARK if selected else theme.PANEL_SOFT,
                                     fg=theme.WHITE if selected else theme.INK)
            for label,value in options:
                b=tk.Button(row,text=label,relief="flat",bd=0,cursor="hand2",font=(theme.FONT_UI,7,"bold"),
                            command=lambda v=value:(variable.set(v),refresh()))
                b.pack(side="left",fill="x",expand=True,padx=(0,3) if buttons==[] else (3,0),ipady=4)
                buttons.append((b,value))
            refresh()

        choice_row("Alignement", vars_map["alignment"], (("Justifié","justify"),("À gauche","left")))
        choice_row("Césures", vars_map["hyphenation"], (("Sans césures","forbid"),("Contrôlées","controlled")))

        notice = str(getattr(self, "_general_rules_notice", "") or "").strip()
        if notice:
            tk.Label(host,text=notice,bg=theme.PANEL,fg=theme.ACCENT_BRIGHT,wraplength=220,justify="left",
                     font=(theme.FONT_UI,7,"bold")).pack(fill="x",padx=16,pady=(8,2))

        self._button(host,"Appliquer les règles générales",self._general_rules_apply,compact=True,accent=True).pack(fill="x",padx=16,pady=(8,12))
