...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V6_TRANSITION_COMPOSEE
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # ----------------------------------------------------------
        # Contrôleur de transition V6.
        #
        # Principe :
        #   1. le contenu s'efface très légèrement ;
        #   2. UNE SEULE géométrie Windows est appliquée ;
        #   3. la destination est dessinée à sa taille définitive ;
        #   4. la fenêtre revient immédiatement à pleine présence.
        #
        # On ne tente plus d'animer le cadre natif de Windows.
        # ----------------------------------------------------------
        transition_fx = {
            "active": False,
            "after": None,
        ...
        def apply_mode_content(key):
            """Affiche la destination à sa géométrie définitive."""
            state["tab"] = key
        ...
        def transition_to_mode(key):
            """Transition de mode sans animation du cadre Windows."""
            if key not in ("global", "feuilleter"):
                return

            if (
                transition_fx["active"]
            ...
            if transition_fx["after"] is not None:
                try:
                    win.after_cancel(
                        transition_fx["after"]
                    )
                    ...
                    pass
                transition_fx["after"] = None

            transition_fx["active"] = True
            ...
            transition_fx["base_alpha"] = base_alpha

            fade_floor = max(
                0.72,
                min(0.80, base_alpha * 0.76),
            )
            fade_out_frames = 5
            fade_in_frames = 8
            frame_ms = 16
            ...
            def fade_out(frame_index=0):
                if not win.winfo_exists():
                    return

                t = smooth(
                    frame_index / fade_out_frames
                )
                alpha = (
                    base_alpha
                    + (
                        fade_floor - base_alpha
                    ) * t
                )
                set_alpha(alpha)

                if frame_index < fade_out_frames:
                    transition_fx["after"] = win.after(
                        frame_ms,
                        lambda: fade_out(
                            frame_index + 1
                        ),
            ...
            def commit_layout_change():
                if not win.winfo_exists():
                    return

                # Le contenu change pendant que la fenêtre est visuellement
                # atténuée. Aucun redraw coûteux n'est demandé avant que
                # Windows ait terminé l'unique redimensionnement.
                apply_mode_content(key)
                ...
                # Une seule reconstruction finale, à la bonne taille.
                ...
                # Laisse le gestionnaire de fenêtres stabiliser la nouvelle
                # géométrie avant la réapparition.
                transition_fx["after"] = win.after(
                    24,
                    lambda: fade_in(0),
                )

            def fade_in(frame_index=0):
                if not win.winfo_exists():
                    return

                t = smooth(
                    frame_index / fade_in_frames
                )
                alpha = (
                    fade_floor
                    + (
                        base_alpha - fade_floor
                    ) * t
                )
                set_alpha(alpha)
