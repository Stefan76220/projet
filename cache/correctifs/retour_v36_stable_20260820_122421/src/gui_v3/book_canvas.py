...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_button_zoom_anchor(self) -> tuple[float, float]:
        ...
        cx += float(getattr(self, "_gabarit_display_center_dx", 0.0))
    ...
    def gabarit_set_zoom(self, value: int, *, anchor: tuple[float, float] | None = None) -> None:
        ...
        viewport_h = max(1.0, float(self.canvas.winfo_height()))
        cx, cy = self._gabarit_view_center()
        # Le rendu stable v36 reste inchangé. La correction de centrage est une
        # translation visuelle finale ; le calcul d'ancrage souris doit simplement
        # connaître cette même translation pour que le point sous le pointeur reste fixe.
        cx += float(getattr(self, "_gabarit_display_center_dx", 0.0))
        box = getattr(self, "_gabarit_page_box", None)
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # Centrage visuel sans toucher au moteur stable v36 : la page a déjà été
        # entièrement dessinée. On translate simplement son groupe Canvas pour que
        # son centre réel tombe exactement entre les deux traits visibles. Canvas.move
        # est le même mécanisme que le panoramique clic-molette, déjà validé sans flash.
        visual_center_x = (float(left_separator_x) + float(info_x1)) / 2.0
        center_dx = visual_center_x - float(work_center_x)
        self._gabarit_display_center_dx = center_dx
        if abs(center_dx) > 0.01:
            try:
                self.canvas.move("gabarit_page_visual", center_dx, 0.0)
                self._gabarit_shift_page_hitboxes(center_dx, 0.0)
            except Exception:
                pass
        ...
        self._gabarit_draw_context_overlay(
            ...
            work_left=float(left_separator_x) + 12.0,
            work_right=float(info_x1) - 12.0,
        )
        self._gabarit_sync_zoom_widget()
