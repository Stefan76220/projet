...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.update_idletasks()
        self.update()
        self._configure_windows_taskbar_style()
    ...
    # ZONE_A_CHANTIER_V3_V2
    # FENETRE_COMMANDES_STABLES_V3_V3
    # FENETRE_TRANSITIONS_NATIVES_V3_V4
    # FENETRE_TRANSITIONS_NATIVES_V3_V4
    def _native_window_handle(self):
        """Retourne le HWND de la fenêtre racine Tk sous Windows."""
        try:
            import ctypes
            import sys
    ...
    def _configure_windows_taskbar_style(self):
        """Conserve une vraie entrée dans la barre des tâches sans remettre le cadre Windows."""
        try:
            import ctypes
            import sys
            if sys.platform != "win32":
                self._native_shell_window_ready = False
                return
            ...
            self._native_shell_window_ready = True
        except Exception:
            self._native_shell_window_ready = False

    def _window_alpha(self, value: float):
        try:
            self.attributes("-alpha", max(0.05, min(1.0, float(value))))
        except tk.TclError:
            pass

    def _window_fade_sequence(self, values, callback=None, delay=12):
        """Petit fondu non bloquant utilisé uniquement pour masquer le recalcul de géométrie."""
        values = list(values)
        ...
        def step(index=0):
            if index >= len(values):
                if callback is not None:
                    callback()
                return
            self._window_alpha(values[index])
            self.after(delay, lambda: step(index + 1))

        step()

    def _window_work_area(self):
        """Retourne la zone de travail Windows (barre des tâches exclue)."""
        try:
            import ctypes
            ...
            if ctypes.windll.user32.SystemParametersInfoW(
                ...
                return (
                    rect.left,
                    rect.top,
                    max(1180, rect.right - rect.left),
                    max(720, rect.bottom - rect.top),
                )
        except Exception:
            pass
        ...
        return (
            0,
            0,
            max(1180, self.winfo_screenwidth()),
            max(720, self.winfo_screenheight() - 48),
        )

    def _window_minimize(self):
        """Réduction Windows native quand elle est disponible."""
        try:
            import ctypes
            import sys
            if sys.platform == "win32":
                hwnd = self._native_window_handle()
                if hwnd:
                    # SW_MINIMIZE : Windows gère lui-même la réduction et le retour.
                    ctypes.windll.user32.ShowWindow(hwnd, 6)
                    return
        except Exception:
            pass

        # Repli Tk uniquement hors Windows ou si l'API native échoue.
        try:
            self._restore_frameless_on_map = True
            self.overrideredirect(False)
            self.iconify()
        except tk.TclError:
            pass

    def _window_restore_frameless(self, _event=None):
        """Repli uniquement pour le mode Tk non natif."""
        if getattr(self, "_native_shell_window_ready", False):
            return
        if not getattr(self, "_restore_frameless_on_map", False):
            return
        if str(self.state()) == "iconic":
            return

        self._restore_frameless_on_map = False
    ...
    def _window_toggle_size(self):
        """Change de taille avec un fondu très court pour éviter le repli brutal du contenu."""
        if getattr(self, "_window_transitioning", False):
            return

        left, top, work_w, work_h = self._window_work_area()
        maximized = getattr(self, "_tomelinea_window_maximized", True)

        if maximized:
            width = min(work_w, max(1360, int(work_w * 0.84)))
            height = min(work_h, max(720, int(work_h * 0.86)))
            x = left + max(0, (work_w - width) // 2)
            y = top + max(0, (work_h - height) // 2)
            target = (width, height, x, y)
            target_maximized = False
            button_text = "□"
        else:
            target = (work_w, work_h, left, top)
            target_maximized = True
            button_text = "❐"

        self._window_transitioning = True
        ...
        def apply_target():
            ...
            try:
                ...
                self.update_idletasks()
                # On laisse Tk recalculer A/B/C presque invisible au lieu
                # d'exposer le redessin progressif à l'utilisateur.
                self.update()
            except tk.TclError:
                pass

            self._tomelinea_window_maximized = target_maximized
            if hasattr(self, "zone_a_size_button"):
                self.zone_a_size_button.configure(text=button_text)

            self.after(
                18,
                lambda: self._window_fade_sequence(
                    (0.28, 0.44, 0.62, 0.78, 0.90, 1.0),
                    callback=finish,
                    delay=12,
                ),
            )
        ...
        def finish():
            self._window_alpha(1.0)
            self._window_transitioning = False
            self.lift()

        self._window_fade_sequence(
            (0.88, 0.70, 0.52, 0.36, 0.24),
    ...
    def _build_zone_a(self):
        """Zone A chantier : plus aérée, plus lisible, commandes renforcées."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=4, minsize=240)
        body.grid_columnconfigure(4, weight=3, minsize=205)

        # 1. Signature TomeLinea réduite mais cohérente avec l'Accueil.
        ...
        # 2. Projet : plus de largeur et une présentation moins tassée.
        ...
        # 3. Travail actuel : deux lignes bien visibles, sans coupure.
        ...
        # 4. Commandes permanentes et commandes de fenêtre plus marquées.
        ...
        minimize = V3Button(actions, "—", self._window_minimize, compact=True)
        minimize.configure(
        ...
        minimize._hover_bg = theme.ACCENT_SOFT
        minimize.pack(side="left", padx=3)

        self.zone_a_size_button = V3Button(actions, "❐", self._window_toggle_size, compact=True)
        self.zone_a_size_button.configure(
            fg="#9CD8D0",
            font=(theme.FONT_UI, 12, "bold"),
            padx=10,
            pady=6,
        ...
        self.zone_a_size_button._hover_bg = "#304B57"
        self.zone_a_size_button.pack(side="left", padx=3)
        ...
        self.bind("<Map>", self._window_restore_frameless, add="+")
