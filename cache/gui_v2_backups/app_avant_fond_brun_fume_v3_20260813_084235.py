...
class PageMaitreV2(tk.Tk):
    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        ...
        def render(_event=None):
            ...
            # ------------------------------------------------------
            # FOND_MAQUETTAGE_GRIS_FUME_V2
            # Fond gris neutre sans dessin, avec de larges volutes floues.
            # Il est dessiné ici APRÈS l'ancien décor et AVANT les panneaux :
            # il le masque donc complètement sans modifier la structure.
            from PIL import ImageDraw, ImageFilter

            bg_size = (width, height)
            if getattr(self, "_maquettage_smoke_v2_size", None) != bg_size:
                smoke_bg = Image.new("RGBA", bg_size, "#C5C8CC")
                smoke_layer = Image.new("RGBA", bg_size, (0, 0, 0, 0))
                smoke_draw = ImageDraw.Draw(smoke_layer, "RGBA")

                light_clouds = (
                    (-int(width * 0.10), -int(height * 0.04),
                     int(width * 0.42), int(height * 0.38),
                     (255, 255, 255, 58)),
                    (int(width * 0.18), int(height * 0.12),
                     int(width * 0.70), int(height * 0.50),
                     (255, 255, 255, 42)),
                    (int(width * 0.60), -int(height * 0.02),
                     int(width * 1.08), int(height * 0.42),
                     (255, 255, 255, 48)),
                    (-int(width * 0.04), int(height * 0.54),
                     int(width * 0.48), int(height * 1.02),
                     (255, 255, 255, 38)),
                    (int(width * 0.52), int(height * 0.50),
                     int(width * 1.08), int(height * 1.04),
                     (255, 255, 255, 44)),
                )
                ...
                dark_clouds = (
                    (int(width * 0.04), int(height * 0.24),
                     int(width * 0.42), int(height * 0.70),
                     (83, 88, 94, 32)),
                    (int(width * 0.38), -int(height * 0.10),
                     int(width * 0.82), int(height * 0.30),
                     (86, 91, 97, 26)),
                    (int(width * 0.70), int(height * 0.30),
                     int(width * 1.10), int(height * 0.82),
                     (81, 86, 92, 30)),
                )
                ...
                self._maquettage_smoke_v2_photo = ImageTk.PhotoImage(
                    smoke_bg
                )
                self._maquettage_smoke_v2_size = bg_size

            canvas.create_image(
                0,
                0,
                image=self._maquettage_smoke_v2_photo,
                anchor="nw",
                tags="maquettage_smoke_v2",
            )
            ...
            # Ajustement V9 :
            # - le drapeau de fin réutilise exactement le dessin du drapeau de départ
            # - seule sa couleur change (rouge sourd TomeLinea)
            # - la borne Fin est rapprochée pour équilibrer l'espace avec la zone Outils
            ...
            start_icon_path = structure_icon_dir / "debut_final.png"
            book_icon_paths = [
            ...
            def structure_photo(path, max_w, max_h, tint_rgba=None):
                key = (str(path), max_w, max_h, tint_rgba)
                ...
                try:
                    ...
                    if tint_rgba is not None:
                        ...
                        tint_layer = Image.new("RGBA", image.size, tint_rgba)
                        image = Image.blend(image, tint_layer, 0.72)
                        image.putalpha(src_alpha)
                    image.thumbnail((max_w, max_h), Image.Resampling.LANCZOS)
                    photo = ImageTk.PhotoImage(image)
            ...
            for index, group in enumerate(structure_model):
                ...
                photo = structure_photo(icon_path, icon_w, icon_h, tint)
                if photo is not None:
                    canvas.create_image(
                        cx,
                        icon_anchor_y,
            ...
            def draw_page_preview(px1, py1, pw, ph, kind, index):
                px2 = px1 + pw
                py2 = py1 + ph
            ...
            for row in range(visible_rows):
                ...
                for local_col, index in enumerate(
                    range(row_start, row_end)
                ):
                    kind = visual_page_kind(index)
