...
class TomeLineaV3(tk.Tk):
    ...
    def _build_gabarits_tab(self, parent):
        ...
        def _book_settings_opener():
            canvas = book()
            if canvas is not None:
                opener = getattr(canvas, "gabarit_inspector_canvas", None)
        ...
        def open_margins():
            ...
            numeric_panel(
                _book_settings_opener(), "Marges du livre — millimètres",
        ...
        def open_bleed():
            ...
            numeric_panel(
                _book_settings_opener(), "Fond perdu du livre — millimètres",
        ...
        try:
            ...
            # Les commandes LIVRE de l'inspecteur droit vivent désormais dans un
            # Toplevel indépendant. Elles passent par des événements virtuels :
            # elles ne dépendent plus du parent Tk du panneau ni d'un attribut callback.
            self.book_canvas.bind("<<GabaritEditFormat>>", lambda _e: open_format(), add="+")
            self.book_canvas.bind("<<GabaritEditMargins>>", lambda _e: open_margins(), add="+")
            self.book_canvas.bind("<<GabaritEditBleed>>", lambda _e: open_bleed(), add="+")
        except Exception:
            pass
