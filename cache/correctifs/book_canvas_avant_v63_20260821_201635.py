...
# TOMELINEA_GABARITS_PALETTE_BANDEAUX_V62
...
class BookCanvas(tk.Frame):
    ...
    GABARIT_INSPECTOR_BLOCK_ORDER_KEY = "gabarit_inspector_band_order_v62"
    GABARIT_INSPECTOR_DEFAULT_BLOCK_ORDER = (
        "create", "allowed", "snap", "align", "layer",
        "actions", "scope", "model", "geometry",
    )
    ...
    def _gabarit_inspector_draw_icon(self, target, icon: str, box, *, selected=False, danger=False, disabled=False, hovered=False):
        ...
        if icon == "create_text":
            # T éditorial : convention universelle du bloc texte.
            target.create_text(cx, cy+1, text="T", anchor="center", fill=teal if hovered else fg, font=(theme.FONT_TITLE, 16, "bold"))
            return
        if icon == "create_image":
            c = teal if hovered else fg
            target.create_rectangle(cx-11, cy-9, cx+11, cy+9, outline=c, width=2)
            target.create_oval(cx+4, cy-6, cx+8, cy-2, fill=gold, outline="")
            target.create_line(cx-9, cy+6, cx-3, cy, cx+1, cy+4, cx+6, cy-2, cx+10, cy+5, fill=c, width=2, smooth=True)
            return
        if icon == "create_document":
            c = teal if hovered else fg
            self._gabarit_inspector_draw_page_shape(target, cx, cy, 19, 25, outline=c)
            target.create_line(cx-5, cy-2, cx+5, cy-2, fill=c, width=1)
            target.create_line(cx-5, cy+2, cx+5, cy+2, fill=c, width=1)
            target.create_line(cx-5, cy+6, cx+2, cy+6, fill=c, width=1)
            return
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        """Palette contextuelle V62 : une ligne par groupe, déploiement au survol.

        Au repos la hauteur reste minimale. Le groupe survolé se déploie dans la
        largeur de l'inspecteur, vers la gauche, sans pousser les autres lignes.
        Les commandes historiques conservent leurs clés et leur comportement.
        """
        ...
        # Les neuf bandeaux restent toujours visibles et ne prennent qu'une ligne.
        for name in order:
            y1 = y
            y2 = y1 + row_h - 2.0
            rows[name] = (y1, y2)
            self._gabarit_inspector_block_bounds[name] = (y1, y1 + row_h)
            is_active = name == active_band
            fill = "#21323A" if is_active else "#19242B"
            outline = "#5F8D8A" if is_active else "#34464F"
            target.create_rectangle(tx, y1, right, y2, fill=fill, outline=outline, width=1)
            target.create_text(tx+8, (y1+y2)/2.0, text=titles[name], anchor="w", fill="#D0D9DA" if is_active else "#9EADB1", font=(theme.FONT_UI, 7, "bold"))
            tab_x1 = right - tab_w
            target.create_rectangle(tab_x1, y1, right, y2, fill="#23343C" if is_active else "#202D34", outline=outline, width=1)
            target.create_text((tab_x1+right)/2.0, (y1+y2)/2.0, text="›" if is_active else "‹", anchor="center", fill="#73BBB5" if is_active else "#71858B", font=(theme.FONT_UI, 11, "bold"))
            key = f"dragblock_{name}"
            self._gabarit_inspector_hitboxes[key] = (tx, y1, right, y2)
            self._gabarit_inspector_tooltips[key] = "Survol : ouvrir · Glisser verticalement : déplacer"
            y += row_h
        ...
        # Le contenu actif est dessiné en dernier : il passe visuellement au-dessus
        # des bandeaux voisins et s'étend vers la gauche sans modifier leur position.
        ry1, ry2 = rows[active_band]
        fly_right = right - tab_w - 4.0
        fly_left = tx + 2.0
        fly_w = max(120.0, fly_right - fly_left)
        base_h = 32.0
        extra_h = 0.0
        if active_band == "geometry" and bool(info.get("spread")):
            extra_h = 30.0
        if active_band == "allowed":
            ...
            if pending in {"margins","page","bleed"} and pending != current:
                extra_h = 31.0
        fy1 = ry1 + 2.0
        fy2 = fy1 + base_h + extra_h
        # Si le flyout risquait de dépasser en bas, il monte au lieu de pousser la palette.
        if fy2 > height - 7.0:
            shift = fy2 - (height - 7.0)
            fy1 -= shift
            fy2 -= shift
        target.create_rectangle(fly_left, fy1, fly_right, fy2, fill="#111B21", outline="#5A767D", width=1)
        self._gabarit_inspector_hitboxes[f"flyout_{active_band}"] = (fly_left, fy1, fly_right, fy2)

        def icon_strip(items, *, button=30.0, gap=3.0):
            count = len(items)
            total = count*button + max(0,count-1)*gap
            start_x = fly_right - total - 3.0
            yy = fy1 + 1.0
            for i, item in enumerate(items):
                key, icon, selected, tip, danger = item
                x1 = start_x + i*(button+gap)
                self._gabarit_inspector_icon_button(target, key, (x1, yy, x1+button, yy+button), icon, selected=selected, danger=danger, tooltip=tip)

        if active_band == "create":
            icon_strip((
                ("create_text","create_text",False,"Créer une zone Texte",False),
                ("create_image","create_image",False,"Créer une zone Image",False),
                ("create_document","create_document",False,"Créer une zone Document",False),
            ), button=31.0, gap=5.0)

        elif active_band == "allowed":
            active = self._gabarit_active_item()
            ...
            icon_strip(tuple(items),button=31.0,gap=6.0)
            state = self._gabarit_frame_reference_label(current)
            suffix = "hérité" if current == book_ref else "exception"
            target.create_text(fly_left+7, fy1+16, text=f"{state} · {suffix}", anchor="w", fill="#759197" if current==book_ref else "#D29A77", font=(theme.FONT_UI,7,"bold"))
            if pending in {"margins","page","bleed"} and pending != current:
                yy = fy1 + 35.0
                gap=4.0; bw=(fly_w-gap-6.0)/2.0
                self._gabarit_inspector_control_box(target,"align_scope_page",(fly_left+3,yy,fly_left+3+bw,yy+25))
                target.create_text(fly_left+3+bw/2,yy+12.5,text="Cette page",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
                x2=fly_left+3+bw+gap
                self._gabarit_inspector_control_box(target,"align_scope_type",(x2,yy,fly_right-3,yy+25))
                target.create_text((x2+fly_right-3)/2,yy+12.5,text="Toutes du type",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
            ...
            scope_active = self._gabarit_scope_is_active(active) if isinstance(active, dict) else False
            pending_scope = str(getattr(self, "_gabarit_scope_pending", "") or "")
            gap=4.0; bw=(fly_w-gap-6.0)/2.0; yy=fy1+3.0
            for i,(key,label,selected) in enumerate((
                ("scope_page","Confirmer" if pending_scope=="page" else "Cette page", (scope_active and last_scope=="page") or pending_scope=="page"),
                ("scope_type","Confirmer" if pending_scope=="type" else "Toutes du type", (scope_active and last_scope=="type") or pending_scope=="type"),
            )):
                x1=fly_left+3+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+25),selected=selected)
                target.create_text(x1+bw/2,yy+12.5,text=label,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold" if selected else "normal"))

        elif active_band == "model":
            has_copy = self.gabarit_has_copied_template()
            gap=4.0; bw=(fly_w-gap-6.0)/2.0; yy=fy1+3.0
            for i,(key,label,disabled) in enumerate((("copy_template","Copier",False),("apply_template","Appliquer",not has_copy))):
                x1=fly_left+3+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+25),disabled=disabled)
                target.create_text(x1+bw/2,yy+12.5,text=label,anchor="center",fill="#D8E0E1" if not disabled else "#65737A",font=(theme.FONT_UI,7,"bold"))

        elif active_band == "geometry":
            fields=(('X','x','geom_x'),('Y','y','geom_y'),('L','w','geom_w'),('H','h','geom_h'))
            gap=3.0; bw=(fly_w-gap*3.0-6.0)/4.0; yy=fy1+3.0
            for i,(lab,field,key) in enumerate(fields):
                x1=fly_left+3+i*(bw+gap)
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+25))
                target.create_text(x1+4,yy+12.5,text=lab,anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
                target.create_text(x1+bw-4,yy+12.5,text=f"{float(rect.get(field,0)):g}",anchor="e",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
            if bool(info.get("spread")):
                ...
                for i,(lab,key,pos) in enumerate((("G","spread_left","left"),("C","spread_center","center"),("D","spread_right","right"))):
                    x1=fly_left+3+i*(bw2+3)
                    self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw2,yy+23),selected=current==pos)
                    target.create_text(x1+bw2/2,yy+11.5,text=lab,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
    ...
    def _gabarit_inspector_band_for_key(self, key: str) -> str:
        ...
        mapping = {
            "create_text":"create", "create_image":"create", "create_document":"create",
            ...
            "align_left":"align", "align_center":"align", "align_right":"align", "align_top":"align", "align_middle":"align", "align_bottom":"align",
            "layer_back":"layer", "layer_backward":"layer", "layer_forward":"layer", "layer_front":"layer",
            "zone_lock":"actions", "zone_duplicate":"actions", "zone_delete":"actions",
