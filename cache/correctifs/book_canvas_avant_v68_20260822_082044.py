# TOMELINEA_GABARITS_SELECTION_V67
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "allowed":
            ...
            content_h = 178.0 if guide else 60.0
        ...
        if active_band == "create":
            ...
            if not guide:
                ...
                # Curseur volontairement large : clic ou glisser, sans petite cible difficile à saisir.
                bar_x1=fly_left+18.0; bar_x2=fly_right-18.0; bar_y=content_top+139.0
    ...
    def gabarit_add_zone(self, kind: str) -> bool:
        ...
        for idx in targets:
            ...
            constrained = self._gabarit_constrain_rect_to_reference(zone, initial_rect, self._gabarit_alignment_reference(item))
    ...
    def gabarit_distribute_selected_zones(self, axis: str) -> bool:
        zones=self._gabarit_selected_zone_dicts()
        if len(zones) < 3:
            self.status_var.set("Distribuer : sélectionnez au moins 3 zones — Maj+clic pour ajouter ou retirer des zones.")
            return False
        if not self._gabarit_multi_same_surface(zones):
    ...
    def _gabarit_press(self, event):
        ...
        if self._gabarit_point_in(self._gabarit_page_box, x, y):
            ...
            # Le bouton gauche dans une zone vide est réservé au cadre de
            # sélection. Le panoramique reste disponible avec Espace+glisser
            # ou la molette, même lorsque le zoom dépasse 100 %.
            state = int(getattr(event, "state", 0) or 0)
        ...
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            if str(state.get("mode") or "") in {"multi_resize", "multi_rotate"}:
                ...
                if mode == "multi_rotate":
                    ...
                    if candidates_inside(candidates):
                        apply_candidates(candidates)
                    else:
                        lo,hi=0.0,1.0; best=None
                        for _ in range(20):
                            t=(lo+hi)/2.0; d=delta*t; rr=math.radians(d); cc,ss=math.cos(rr),math.sin(rr)
                            probe=[]
                            for member,mx,my,mw,mh,mang,mcx,mcy in members:
                                dx0,dy0=mcx-gcx,mcy-gcy
                                ncx=gcx+dx0*cc-dy0*ss; ncy=gcy+dx0*ss+dy0*cc
                                nx=(ncx-surface_x)/pw-mw/2.0; ny=(ncy-surface_y)/ph-mh/2.0
                                probe.append((member,nx,ny,mw,mh,mang+d))
                            if candidates_inside(probe): best=probe; lo=t
                            else: hi=t
                        if best: apply_candidates(best)
                ...
                if candidates_inside(candidates):
                    ...
                    # Approche progressive de la limite : le groupe bute comme
                    # une zone simple au lieu de déformer certains membres.
                    lo,hi=0.0,1.0; best=None
                    starts=[(m,mx,my,mw,mh,ma) for m,mx,my,mw,mh,ma,_cx,_cy in members]
                    for _ in range(20):
                        t=(lo+hi)/2.0; probe=[]
                        for start,cand in zip(starts,candidates):
                            m,mx,my,mw,mh,ma=start; _m,nx,ny,nw,nh,na=cand
                            probe.append((m,mx+(nx-mx)*t,my+(ny-my)*t,mw+(nw-mw)*t,mh+(nh-mh)*t,ma+(na-ma)*t))
                        if candidates_inside(probe): best=probe; lo=t
                        else: hi=t
                    if best: apply_candidates(best)
                self._gabarit_refresh_raster_only(); return "break"
