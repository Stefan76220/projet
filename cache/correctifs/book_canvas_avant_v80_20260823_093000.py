...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_inspector_draw_icon(self, target, icon: str, box, *, selected=False, danger=False, disabled=False, hovered=False):
        ...
        if icon in {"undo", "redo"}:
            # V79 — symbole volontairement très lisible : grande flèche courbe pleine,
            # convention Annuler/Rétablir immédiatement reconnaissable, même sans texte.
            c = teal if hovered and not disabled else fg
            if icon == "undo":
                # Courbe revenant franchement vers la gauche + grosse pointe triangulaire.
                target.create_arc(cx-8, cy-8, cx+10, cy+10, start=315, extent=225,
                                  style="arc", outline=c, width=3)
                target.create_line(cx-7, cy-5, cx-12, cy-5, fill=c, width=3)
                target.create_polygon(
                    cx-12, cy-5,
                    cx-5, cy-11,
                    cx-5, cy+1,
                    fill=c, outline=c,
                ...
                # Miroir exact : courbe repartant vers la droite.
                target.create_arc(cx-10, cy-8, cx+8, cy+10, start=0, extent=225,
                                  style="arc", outline=c, width=3)
                target.create_line(cx+7, cy-5, cx+12, cy-5, fill=c, width=3)
                target.create_polygon(
                    cx+12, cy-5,
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "allowed":
            ...
            # V77 : cadrage et transparence sont visibles avant comme après import.
            content_h = 198.0
        ...
        if active_band == "create":
            ...
            # Annuler/Rétablir ont un pendant souris sans créer un nouveau bandeau.
            icon_strip((
                ("history_undo","undo",False,"Annuler · Ctrl+Z",False,not self.can_undo()),
                ("history_redo","redo",False,"Rétablir · Ctrl+Y / Ctrl+Maj+Z",False,not self.can_redo()),
    ...
    def _gabarit_inspector_band_for_key(self, key: str) -> str:
        ...
        mapping = {
            ...
            "history_undo":"actions", "history_redo":"actions", "zone_duplicate":"actions", "zone_delete":"actions",
