    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            # Largeur adaptée au texte, sans panneau autour du bouton.
            width = max(min_width, int(len(text) * 7.0) + 34)
            x1 = cx - width / 2
            x2 = cx + width / 2
            y1 = cy - height / 2
            y2 = cy + height / 2
            shadow = self._mix(color, "#68726A", 0.34)
            rim = self._mix(color, "#657361", 0.24)
            face = self._mix(color, "#FFFFFF", 0.82)
            shine = self._mix(color, "#FFFFFF", 0.95)
            ink = self._mix(color, "#253226", 0.60)
        ...
        def home_hover_cursor(canvas, tag):
            canvas.tag_bind(
                tag,
                "<Enter>",
                lambda _e, c=canvas: c.configure(cursor="hand2"),
            )
            canvas.tag_bind(
                tag,
                "<Leave>",
                lambda _e, c=canvas: c.configure(cursor="arrow"),
            )
        ...
        def render_create_content(_event=None):
            ...
            for index, (key, title, sub, color, icon_path) in enumerate(project_types):
                ...
                face = self._mix(
                    color,
                    "#FFFFFF",
                    0.80 if selected else 0.91,
                )
                shadow = self._mix(color, "#7B817A", 0.62)
                ...
                home_round_rect(
                    ...
                    13,
                    fill=face,
                    outline=color if selected else self._mix(color, "#FFFFFF", 0.36),
                    width=2 if selected else 1,
                    tags=("home_create", tag),
                )

                canvas.create_oval(
                    x1 + 10, y1 + 9, x1 + 21, y1 + 20,
                    outline=color,
                    width=1.4,
                    fill=color if selected else "",
                    tags=("home_create", tag),
                )

                photo = get_project_type_photo(key, icon_path)
                icon_y = y1 + min(39, max(30, (y2 - y1) * 0.33))
                if photo is not None:
                    canvas.create_image(
                        (x1 + x2) / 2,
                        icon_y,
            ...
            home_hover_cursor(canvas, "create_project_button")
        ...
        def render_open_content(_event=None):
            ...
            # Icône dossier dessinée directement sur le décor.
            icon_x = max(110, width * 0.30)
            icon_y = height * 0.53
            folder_color = "#8C6DC5"
            folder_light = self._mix(folder_color, "#FFFFFF", 0.78)
            home_round_rect(
                canvas,
                icon_x - 43, icon_y - 27,
                icon_x + 43, icon_y + 29,
            ...
            canvas.create_line(
                icon_x - 30, icon_y - 4,
                icon_x + 30, icon_y - 4,
                fill=self._mix(folder_color, "#FFFFFF", 0.35),
                width=1,
                tags="home_open",
            )
            ...
            home_hover_cursor(canvas, "open_project_button")
        ...
        def render_recent_content(_event=None):
            ...
            # Projet principal : informations directement sur le décor.
            icon_x = 34
            top_y = 82
            canvas.create_rectangle(
                icon_x - 11, top_y - 13,
                icon_x + 11, top_y + 17,
                fill=self._mix("#69AEE0", "#FFFFFF", 0.82),
                outline="#69AEE0",
                width=1.2,
                tags="home_recent",
            )
            ...
            home_hover_cursor(canvas, "resume_project_button")

            # Séparation discrète : elle structure sans créer de panneau.
            separator_y = min(height - 55, top_y + 67)
            canvas.create_line(
                22, separator_y,
                width - 22, separator_y,
                fill=self._mix("#7DBBA4", "#FFFFFF", 0.55),
                width=1,
                tags="home_recent",
            )
        ...
        def render_shortcuts_content(_event=None):
            ...
            for index, (title, sub, color, tag) in enumerate(shortcut_data):
                x1 = margin + index * (button_w + gap)
                x2 = x1 + button_w
                shadow = self._mix(color, "#7B817A", 0.68)
                face = self._mix(color, "#FFFFFF", 0.91)
            ...
            # Les trois indicateurs sont simplement du texte sur le décor.
            ...
            for index, (title, value, color) in enumerate(columns):
                ...
                if index < 2:
                    ...
                    canvas.create_line(
                        sep_x, status_y - 2,
                        sep_x, status_y + 30,
                        fill=self._mix("#9B7BCB", "#FFFFFF", 0.72),
                        tags="home_shortcuts",
                    )
