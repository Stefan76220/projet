...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Visionneuse locale : même cellule que le livre, mais dans un Canvas
        # indépendant. Elle peut donc recouvrir B sans recalculer ni déplacer
        # une seule page de la ligne.
        ...
        self.page_overlay.grid(row=0, column=0, sticky="nsew")
        self.page_overlay.grid_remove()
    ...
    def open_page_overlay(self, index: int | None = None, *, reset_zoom: bool = True):
        ...
        self.page_overlay.grid()
        # Canvas redéfinit tkraise() pour ses items ; on appelle donc le
        # gestionnaire de fenêtres Tcl directement pour empiler le widget.
    ...
    def _overlay_fit_scale(self) -> float:
        width = max(260, self.page_overlay.winfo_width())
        height = max(220, self.page_overlay.winfo_height())
        # 88 % de la hauteur : la page est déjà grande, mais les premiers crans
        # de zoom la gardent encore entière avant qu'elle dépasse la zone.
        return min((width * 0.90) / self.BASE_PAGE_W, (height * 0.88) / self.BASE_PAGE_H)
    ...
    def _render_page_overlay(self):
        ...
        cy = height / 2.0 + self._overlay_pan_y
        x = cx - page_w / 2.0
        y = cy - page_h / 2.0
        ...
        # Fond calme, sans nouvelle fenêtre : la page peut recouvrir tout B.
