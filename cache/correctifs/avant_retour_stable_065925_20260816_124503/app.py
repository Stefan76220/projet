...
from src.gui_v3.windows_safe_ops import WindowsSafeOps
...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # FENETRE_SANS_CALLBACK_WIN32_V3_V9
        # Aucune interception WndProc en Python : uniquement des appels Win32
        # synchrones initiés depuis les callbacks Tk.
        self._window_ops = WindowsSafeOps(self)
        self._tomelinea_window_maximized = True
        self._window_transition = False
        ...
        # Le wrapper Tk/Windows est préparé pendant que la fenêtre est encore
        # cachée. Tk attribue WS_EX_TOOLWINDOW aux override-redirect ; TomeLinea
        # le remplace par WS_EX_APPWINDOW pour garder une vraie entrée de barre
        # des tâches, sans ajouter de barre de titre Windows.
        self._window_ops.resolve()
        self._window_ops.ensure_taskbar_button()
    ...
    # FENETRE_ATOMIQUE_SANS_WNDPROC_V3_V9
    def _window_work_area(self):
        ...
        try:
            ...
            if ctypes.windll.user32.SystemParametersInfoW(
                ...
                return (
                    int(rect.left),
                    int(rect.top),
                    max(1180, int(rect.right - rect.left)),
                    max(720, int(rect.bottom - rect.top)),
                )
        except Exception:
            pass
        ...
        return (
            0,
            0,
            max(1180, int(self.winfo_screenwidth())),
            max(720, int(self.winfo_screenheight()) - 48),
        )

    def _window_apply_geometry_atomically(self, x: int, y: int, width: int, height: int) -> None:
        """Recalcule toute la V3 pendant que Windows garde l'image précédente.

        Contrairement à V8, aucun message Windows n'appelle du code Python.
        Le redraw est simplement suspendu pendant que Tk traite la nouvelle
        géométrie et ses <Configure>, puis la fenêtre complète est invalidée
        en une seule fois.
        """
        if self._window_transition:
            return
        self._window_transition = True
        try:
            with self._window_ops.redraw_suspended():
                self.geometry(f"{int(width)}x{int(height)}+{int(x)}+{int(y)}")
                # update() est volontaire ici : le redraw natif est bloqué, mais
                # les Configure Tk, les places/grids et les fonds PIL arrivent à
                # leur taille finale avant la prochaine peinture Windows.
                self.update_idletasks()
                self.update()
        except tk.TclError:
            pass
    ...
    def _window_minimize(self) -> None:
        """Minimise via le HWND existant, sans changer overrideredirect."""
        if self._window_ops.minimize():
            return
        try:
            self.iconify()
        except tk.TclError:
    ...
    def _window_toggle_size(self) -> None:
        """Bascule grande vue / fenêtre restaurée sans état intermédiaire peint."""
        left, top, work_w, work_h = self._window_work_area()
        maximized = bool(getattr(self, "_tomelinea_window_maximized", True))
        ...
        if maximized:
            ...
            self._window_apply_geometry_atomically(x, y, width, height)
            self._tomelinea_window_maximized = False
            symbol = "□"
        else:
            self._window_apply_geometry_atomically(left, top, work_w, work_h)
            self._tomelinea_window_maximized = True
            symbol = "❐"

        for name in ("home_size_label", "zone_a_size_button"):
            widget = getattr(self, name, None)
            if widget is not None:
                try:
                    widget.configure(text=symbol)
                except tk.TclError:
                    pass

    def _window_drag_start(self, event) -> None:
        if getattr(self, "_tomelinea_window_maximized", True):
            return
    ...
    def _window_drag_move(self, event) -> None:
        origin = self._window_drag_origin
        if not origin or getattr(self, "_tomelinea_window_maximized", True):
            return
        sx, sy, wx, wy = origin
    ...
    def _window_drag_end(self, _event=None) -> None:
        self._window_drag_origin = None
    ...
    def _build_home(self):
        ...
        min_label = tk.Label(
            header, text="—", bg=theme.WINDOW_DEEP, fg=theme.MUTED,
            font=(theme.FONT_UI, 14, "bold"), cursor="hand2", padx=8, pady=2,
        )
        min_label.place(relx=0.925, y=10, anchor="ne")
        min_label.bind("<Button-1>", lambda _e: self._window_minimize())
        min_label.bind("<Enter>", lambda _e: min_label.configure(fg=theme.INK))
        min_label.bind("<Leave>", lambda _e: min_label.configure(fg=theme.MUTED))

        self.home_size_label = tk.Label(
            header, text="❐", bg=theme.WINDOW_DEEP, fg=theme.MUTED,
            font=(theme.FONT_UI, 13, "bold"), cursor="hand2", padx=8, pady=2,
        )
        self.home_size_label.place(relx=0.955, y=10, anchor="ne")
        self.home_size_label.bind("<Button-1>", lambda _e: self._window_toggle_size())
        self.home_size_label.bind(
            "<Enter>", lambda _e: self.home_size_label.configure(fg=theme.INK)
        )
        self.home_size_label.bind(
            "<Leave>", lambda _e: self.home_size_label.configure(fg=theme.MUTED)
        )
        ...
        # En vue restaurée, la zone vide du bandeau sert de poignée de déplacement.
        header.bind("<ButtonPress-1>", self._window_drag_start, add="+")
        header.bind("<B1-Motion>", self._window_drag_move, add="+")
        header.bind("<ButtonRelease-1>", self._window_drag_end, add="+")
    ...
    def _build_zone_a(self):
        ...
        tk.Frame(actions, bg=theme.BORDER_SOFT, width=1, height=30).pack(
            side="left", padx=(8, 5)
        )
        minimize = V3Button(actions, "—", self._window_minimize, compact=True)
        minimize.configure(font=(theme.FONT_UI, 11, "bold"), padx=8, pady=5, width=3)
        minimize.pack(side="left", padx=2)
        self.zone_a_size_button = V3Button(
            actions, "❐", self._window_toggle_size, compact=True
        )
        self.zone_a_size_button.configure(
            font=(theme.FONT_UI, 11, "bold"), padx=8, pady=5, width=3
        )
        self.zone_a_size_button.pack(side="left", padx=2)
        close = V3Button(actions, "✕", self.destroy, compact=True)
        close.configure(
            fg="#F08A5A", font=(theme.FONT_UI, 11, "bold"), padx=8, pady=5, width=3
        )
