...
class PageMaitreV2(tk.Tk):
    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        ...
        zone_background_paths = {
            "structure": (
                PROJECT_ROOT
                / "assets"
                / "gui_v2"
                / "maquettage_zone_backgrounds"
                / "01_structure_du_livre.png"
            ),
            "detail": (
                PROJECT_ROOT
                / "assets"
            ...
            "composition": (
                ...
                / "gui_v2"
                / "maquettage_zone_backgrounds"
                / "03_composition_de_la_partie.png"
            ),
            "properties": (
                PROJECT_ROOT
                / "assets"
                / "gui_v2"
                / "maquettage_zone_backgrounds"
                / "04_proprietes_de_la_page.png"
            ),
        }

        zone_bg_sources = {}
        self._maquettage_zone_bg_photos = []

        for key, path in zone_background_paths.items():
            if path.exists():
                try:
                    zone_bg_sources[key] = Image.open(path).convert("RGBA")
                except Exception:
                    zone_bg_sources[key] = None
            else:
                zone_bg_sources[key] = None
        ...
        def zone(
            ...
            *,
            ...
            # Zones de travail très claires, avec un décor discret lié à la fonction.
            ...
            zone_source = (
                zone_bg_sources.get(background_key)
                if background_key
                else None
            )

            if zone_source is not None:
                width = max(8, int(round(x2 - x1)))
                height = max(8, int(round(y2 - y1)))

                image = cover_crop(zone_source, width, height)
                ...
                panel.paste(image, (0, 0), mask)

                veil = Image.new(
                    "RGBA",
                    (width, height),
                    hex_to_rgba(face, 172),
                )
                ...
                self._maquettage_zone_bg_photos.append(photo)
                canvas.create_image(
                    x1,
                    y1,
                    image=photo,
