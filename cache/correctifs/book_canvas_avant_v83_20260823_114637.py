...
class BookCanvas(tk.Frame):
    ...
    def _build_toolbar(self):
        ...
        self.gabarit_tools_side_btn.grid(row=0, column=2, sticky="e", padx=(8, 0))
    ...
    def gabarit_toggle_tools_side(self) -> None:
        ...
        self.render()
        self.status_var.set("Outils placés à gauche." if new_side == "left" else "Outils placés à droite.")
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        target.create_line(permanent_left, add_y+add_button+8.0, permanent_right, add_y+add_button+8.0, fill="#34464F", width=1)
        target.create_line(permanent_left, quick_y-8.0, permanent_right, quick_y-8.0, fill="#34464F", width=1)
    ...
    def _gabarit_left_separator_x(self, viewport_w: float | None = None) -> float:
        """Bord intérieur de la palette, quel que soit son côté."""
        ...
        return info_w + 8.0 if self._gabarit_tools_side() == "left" else width - info_w - 8.0
    ...
    def _gabarit_work_rect_for_size(self, width: float, height: float) -> tuple[float, float, float, float]:
        ...
        edge = 8.0
        if self._gabarit_tools_side() == "left":
            work_left = info_w + edge + 12.0
            work_right = width - 20.0
        else:
            work_left = 20.0
            work_right = width - info_w - edge - 12.0
    ...
    def _place_gabarit_zoom_controls(self) -> None:
        ...
        try:
            ...
            info_w = float(self._gabarit_inspector_width(cw))
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float, *, update_inspector: bool = True) -> None:
        ...
        # Bord intérieur de la palette, à gauche ou à droite selon la préférence.
