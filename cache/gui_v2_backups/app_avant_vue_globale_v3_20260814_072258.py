...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_GLOBALE_PANORAMIQUE_BORNES_V1
    # VUE_GLOBALE_PANORAMIQUE_BORNES_V2_RECENTREE
    def _tomelinea_open_global_view(self):
        ...
        def draw_title_line(_event=None):
            ...
            line_w = max(40, title_line.winfo_width())
            yy = 13
            end_x = max(10, line_w - 8)
        ...
        # Bouton Fermer discret, sans introduire de nouveau vocabulaire.
        close = tk.Canvas(
            header,
            width=42,
            height=38,
        ...
        close.place(relx=1.0, x=-48, y=4, anchor="ne")
        ...
        def draw_close():
            close.delete("all")
            pressed = close_state["pressed"]
            hover = close_state["hover"]
            dy = 1 if pressed else 0

            face = "#E9F0F7" if pressed else (
                "#F3F7FB" if hover else "#FFFDFC"
            )
            border = "#355C85" if pressed else (
                "#496D94" if hover else "#BFC7D0"
            )
        ...
        # ----------------------------------------------------------
        # Fond de borne : mini planche de maquettage.
        # Une surface papier technique très légère met en valeur le livre
        # ou le drapeau sans créer un nouveau vocabulaire graphique.
        # ----------------------------------------------------------
        def draw_structure_tile(cx, icon_anchor_y, *, book=False):
