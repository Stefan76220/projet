...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V4_SUPPORT_LIBRAIRIE
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        def draw_book_background(geometry):
            """Même table de montage que la Vue globale."""
            w = geometry["w"]
            h = geometry["h"]
            left_x = geometry["left_x"]
            right_x = geometry["right_x"]
            y1 = geometry["y1"]
            total_w = geometry["total_w"]
            ...
            # Plateau clair identique à la frise globale.
            book_canvas.create_polygon(
                polygon_points(
                    x1,
                    y_top,
            ...
            # Grille éditoriale très discrète.
            gx = x1 + 38
            while gx < x2 - 20:
            ...
            # Support d'exposition de librairie : clair, moderne et discret.
            # Il soutient le livre sans former un cadre sombre autour de lui.
            support_left = left_x - 26
            support_right = left_x + total_w + 26
            support_base_y = y1 + page_h + 13

            # Ombre flottante très douce sous l'ensemble.
            ...
            # Tablette basse, esprit présentoir contemporain en librairie.
            book_canvas.create_polygon(
                support_left,
                support_base_y,
                support_right,
            ...
            # Deux appuis transparents simulés derrière le bas des pages.
            support_fill = "#E5E8E6"
            support_edge = "#C9CFCC"
            for sx in (
                left_x + page_w * 0.34,
        ...
        # ----------------------------------------------------------
        # Changement d'onglet ET transformation de la fenêtre.
        # ----------------------------------------------------------
        animation = {
            "after": None,
        }

        def centered_geometry(width, height):
            x = max(
                0,
                int((screen_w - width) / 2),
            )
            y = max(
                0,
                int((screen_h - height) / 2),
            )
            return int(width), int(height), x, y

        def animate_window_to(target_size):
            # Animation pilotée par le temps plutôt que par un nombre fixe
            # d'images : si Windows saute une image, la trajectoire reste fluide
            # et la fenêtre arrive malgré tout exactement au bon endroit.
            ...
            try:
                current_w = max(1, win.winfo_width())
                current_h = max(1, win.winfo_height())
                current_x = win.winfo_x()
                current_y = win.winfo_y()
            except Exception:
                current_w, current_h = target_w, target_h
                current_x = int((screen_w - target_w) / 2)
                current_y = int((screen_h - target_h) / 2)
            ...
            (
                ...
                final_x,
                final_y,
            ) = centered_geometry(target_w, target_h)

            if animation["after"] is not None:
                try:
                    win.after_cancel(animation["after"])
                except Exception:
                    pass
                animation["after"] = None

            resize_fx["active"] = True
            ...
            def frame():
                ...
                # Ease-in-out sinusoïdal : très régulier visuellement
                # sur une transformation simultanée taille + position.
                eased = 0.5 - 0.5 * __import__("math").cos(3.141592653589793 * t)

                nw = int(round(
                    current_w
                    + (final_w - current_w) * eased
                ))
                nh = int(round(
                    current_h
                    + (final_h - current_h) * eased
                ))
                nx = int(round(
                    current_x
                    + (final_x - current_x) * eased
                ))
                ny = int(round(
                    current_y
                    + (final_y - current_y) * eased
                ))

                geometry = f"{nw}x{nh}+{nx}+{ny}"
                if geometry != last_geometry[0]:
                    win.geometry(geometry)
                    last_geometry[0] = geometry

                if t < 1.0:
                    animation["after"] = win.after(
                        frame_ms,
                        frame,
                    )
                    return
                ...
                animation["after"] = None
                resize_fx["active"] = False
                win.geometry(
                    f"{final_w}x{final_h}+{final_x}+{final_y}"
                )
                win.update_idletasks()
                redraw_shell()
        ...
        def show_tab(key, *, animate=True):
            ...
            if key == "global":
                ...
                subtitle_label.configure(
                    text=(
                        "Vue panoramique de l’ordre des pages "
                        "et des parties."
                    )
                )
                # La destination est déjà visible pendant le morphing.
                draw_global()
                draw_global_scroll()
                if animate:
                    animate_window_to(global_size)
            else:
                book_host.tkraise()
                subtitle_label.configure(
                    text=(
                ...
                # Le livre apparaît immédiatement puis la fenêtre
                # se transforme autour de lui.
                render_book()
                if animate:
                    animate_window_to(book_size)
