...
class PageMaitreV2(tk.Tk):
    ...
    def _build_shell(self) -> None:
        """Structure stable : Accueil, Projet et Visualisation sont séparés."""
        ...
        # Visualisation : page entière, sans ruban, appelée depuis les bureaux.
        self.visualisation_shell = tk.Frame(self.app_stack, bg=theme.WINDOW)
        self.visualisation_shell.grid(row=0, column=0, sticky="nsew")
        self.visualisation_shell.grid_rowconfigure(0, weight=1)
        self.visualisation_shell.grid_columnconfigure(0, weight=1)
    ...
    def open_visualisation_window(self) -> None:
        """Ouvre la page Visualisation plein écran, sans ruban."""
        if not hasattr(self, "_visualisation_screen"):
            screen = tk.Frame(self.visualisation_shell, bg=theme.WINDOW)
            screen.grid(row=0, column=0, sticky="nsew")
            self._visualisation_screen = screen
            self._build_visualisation(screen)

        source = getattr(self, "_active", "maquettage") or "maquettage"
        if source == "visualisation":
            source = getattr(self, "_visualisation_return_screen", "maquettage")
        self._visualisation_return_screen = source
        self._active = "visualisation"

        render = getattr(self, "_visualisation_render", None)
        if callable(render):
            render()
            self.update_idletasks()

        self.visualisation_shell.tkraise()

    def _close_visualisation_page(self) -> None:
        target = getattr(self, "_visualisation_return_screen", "maquettage") or "maquettage"
        if target == "accueil":
            self.show_screen("accueil")
        else:
            self.show_screen(target)

    def _build_visualisation(self, parent: tk.Frame) -> None:
        canvas = tk.Canvas(parent, bg=theme.WINDOW, highlightthickness=0, bd=0)
        canvas.pack(fill="both", expand=True)

        parent._visualisation_canvas = canvas
        self._visualisation_canvas = canvas

        if not hasattr(self, "_visualisation_bg_source"):
            try:
                self._visualisation_bg_source = Image.open(ACCUEIL_BG).convert("RGB")
            except Exception:
                self._visualisation_bg_source = None
        self._visualisation_bg_photo = None
        self._visualisation_thumb_cache = {}
        ...
        def cut_points(x1, y1, x2, y2, cut=18):
            cut = max(8, min(cut, int((x2 - x1) / 6), int((y2 - y1) / 6)))
            return [
                x1, y1,
                x2 - cut, y1,
                x2, y1 + cut,
        ...
        def safe_type_filename(kind: str) -> str:
            ...
            mapping = {
                ...
                "page auto": "type_page_texte.png",
                "tête de partie": "type_page_chapitre.png",
                "tete de partie": "type_page_chapitre.png",
            }
            return mapping.get(low, "type_page_texte.png")

        def get_type_thumb(kind: str, size: tuple[int, int]):
            key = (kind, size)
            cached = self._visualisation_thumb_cache.get(key)
            if cached is not None:
                return cached
            thumb_path = PROJECT_ROOT / "assets" / "page_thumbnails" / safe_type_filename(kind)
            try:
                ...
                photo = None
            self._visualisation_thumb_cache[key] = photo
            return photo

        def draw_background(width: int, height: int):
            if self._visualisation_bg_source is not None:
                source = self._visualisation_bg_source
                sw, sh = source.size
                scale = max(width / sw, height / sh)
                new_size = (max(1, int(sw * scale)), max(1, int(sh * scale)))
                image = source.resize(new_size, Image.Resampling.LANCZOS)
                left = max(0, (image.size[0] - width) // 2)
                top = max(0, (image.size[1] - height) // 2)
                image = image.crop((left, top, left + width, top + height))
                veil = Image.new("RGB", (width, height), "#FBF8F2")
                image = Image.blend(image.convert("RGB"), veil, 0.22)
                self._visualisation_bg_photo = ImageTk.PhotoImage(image)
                canvas.create_image(0, 0, image=self._visualisation_bg_photo, anchor="nw")
            else:
                canvas.create_rectangle(0, 0, width, height, fill="#F5F1EA", outline="")

            canvas.create_rectangle(0, 0, width, height, fill="#FFFDFC", stipple="gray75", outline="")

        def draw_shell(x1, y1, x2, y2):
            canvas.create_polygon(cut_points(x1 + 2, y1 + 6, x2 + 2, y2 + 6, 20), fill="#DCD7D0", outline="")
            canvas.create_polygon(cut_points(x1, y1, x2, y2, 20), fill="#FFFEFD", outline="#DDD8D1", width=1)
            canvas.create_line(x1 + 10, y1 + 18, x1 + 10, y2 - 18, fill="#8D70C7", width=3)

        def draw_title_line(x1, x2, y, title):
            canvas.create_text(x1, y, text=title, fill="#173B6C", font=("Georgia", 22, "bold"), anchor="nw")
            line_x1 = x1 + 180
            line_y = y + 14
            circle_x = x2 - 8
            canvas.create_line(line_x1, line_y, circle_x - 16, line_y, fill="#111111", width=1)
            canvas.create_oval(circle_x - 5, line_y - 5, circle_x + 5, line_y + 5, outline="#111111", width=1)

        def draw_panel(x1, y1, x2, y2, accent, title, subtitle):
            canvas.create_polygon(cut_points(x1 + 2, y1 + 5, x2 + 2, y2 + 5, 18), fill="#DCD7D0", outline="")
            canvas.create_polygon(cut_points(x1, y1, x2, y2, 18), fill="#FFFEFD", outline="#DDD8D1", width=1)
            canvas.create_line(x1 + 10, y1 + 18, x1 + 10, y2 - 18, fill=accent, width=3)
            canvas.create_text(x1 + 24, y1 + 18, text=title, fill="#173B6C", font=("Georgia", 16, "bold"), anchor="nw")
            if subtitle:
                canvas.create_text(x1 + 24, y1 + 50, text=subtitle, fill="#6B747C", font=("Segoe UI", 9), anchor="nw")
            line_y = y1 + 26
            line_x1 = x1 + 205
            line_x2 = x2 - 22
            if line_x2 > line_x1 + 18:
                canvas.create_line(line_x1, line_y, line_x2 - 12, line_y, fill="#111111", width=1)
                canvas.create_oval(line_x2 - 5, line_y - 5, line_x2 + 5, line_y + 5, outline="#111111", width=1)
            return (x1 + 18, y1 + 78, x2 - 18, y2 - 18)

        def return_label() -> str:
            target = getattr(self, "_visualisation_return_screen", "maquettage")
            labels = {
                "centre": "Retour au Centre",
                "maquettage": "Retour au Maquettage",
                "atelier": "Retour à l’Atelier",
                "conception": "Retour à la Conception",
                "assemblage": "Retour à l’Assemblage",
                "verification": "Retour à la Vérification",
                "finalisation": "Retour à la Finalisation",
                "accueil": "Retour à l’Accueil",
            }
            return labels.get(target, "Retour")

        def draw_return_button(cx, cy):
            button = TLCommandButton(
                tag="visual_return",
                text=return_label(),
                cx=cx,
                cy=cy,
            ...
            x1, y1, x2, y2 = button.draw(canvas, "normal")
            regions["return"] = (x1, y1, x2, y2)

        def clamp_spread(pages):
            max_left = max(0, len(pages) - 1)
            left = int(state["spread_left"])
            if left < 0:
                left = 0
            if left > max_left:
                left = max_left
            if left % 2 == 1:
                left -= 1
            state["spread_left"] = max(0, left)

        def draw_page_thumb(x1, y1, kind, number, size):
            pw, ph = size
            x2 = x1 + pw
            y2 = y1 + ph
            rounded(x1 + 1, y1 + 3, x2 + 1, y2 + 3, 7, fill="#D9D4CD", outline="")
            rounded(x1, y1, x2, y2, 7, fill="#FFFEFB", outline="#D8D2CA", width=1)
            canvas.create_text(x1 + 7, y1 + 7, text=str(number), fill="#61707A", font=("Segoe UI", 7, "bold"), anchor="nw")
            if kind == "Couverture":
                fill = "#F1EEE7"
                label = "Couverture"
                label_fill = "#EEF3EA"
                text = "COUVERTURE"
            elif kind == "4e de couverture":
                fill = "#F1EEE7"
                label = "4e de couverture"
                label_fill = "#F5EEE6"
                text = "4E DE\nCOUVERTURE"
            elif kind == "Tête de partie":
                fill = "#F1EEE7"
            ...
            canvas.create_rectangle(x1 + 9, y1 + 12, x2 - 9, y2 - 12, fill=fill, outline="")
            if kind in ("Couverture", "4e de couverture", "Tête de partie"):
                canvas.create_text((x1 + x2) / 2, y1 + ph * 0.40, text=text, fill="#343434", font=("Georgia", 8, "bold"), justify="center")
                canvas.create_polygon(x1 + 9, y2 - 32, x1 + pw * 0.34, y2 - 46, x1 + pw * 0.58, y2 - 34, x2 - 9, y2 - 48, x2 - 9, y2 - 12, x1 + 9, y2 - 12, fill="#E4E7DD", outline="")
            elif kind == "Page auto":
                canvas.create_text((x1 + x2) / 2, y1 + 22, text="AUTO", fill="#6B737A", font=("Segoe UI", 6, "bold"))
                for index in range(5):
                    yy = y1 + 34 + index * max(5, int(ph * 0.07))
                    canvas.create_line(x1 + 12, yy, x2 - 12, yy, fill="#D9D5CE", width=1)
            else:
                canvas.create_rectangle(x1 + 10, y1 + 18, x2 - 10, y1 + ph * 0.42, fill="#E9ECE7", outline="")
                for index in range(5):
                    yy = y1 + ph * 0.52 + index * max(4, int(ph * 0.055))
                    right = x2 - 16 if index < 4 else x1 + pw * 0.68
                    canvas.create_line(x1 + 11, yy, right, yy, fill="#D2CEC7", width=1)

            label_y = y2 + 10
            label_w = max(46, min(90, 10 + len(label) * 5))
            rounded((x1 + x2) / 2 - label_w / 2, label_y - 8, (x1 + x2) / 2 + label_w / 2, label_y + 10, 6, fill=label_fill, outline="#DDD6CD", width=1)
            canvas.create_text((x1 + x2) / 2, label_y + 1, text=label, fill="#61707A", font=("Segoe UI", 7), anchor="center")

        def draw_global_view(rect, pages, model):
            x1, y1, x2, y2 = rect
            rounded(x1, y1, x2, y2, 14, fill="#F7F4ED", outline="#E2DDD5", width=1)
            inner_x1 = x1 + 18
            inner_y1 = y1 + 18
            inner_x2 = x2 - 18
            inner_y2 = y2 - 18
            canvas.create_rectangle(inner_x1, inner_y1, inner_x2, inner_y2, fill="#FBF9F4", outline="")
            for gx in range(int(inner_x1) + 40, int(inner_x2), 120):
                canvas.create_line(gx, inner_y1 + 14, gx, inner_y2 - 14, fill="#EEE8E0", width=1)
            for gy in range(int(inner_y1) + 18, int(inner_y2), 48):
                canvas.create_line(inner_x1 + 16, gy, inner_x2 - 16, gy, fill="#F0EAE3", width=1)

            regions["global_drag"] = (inner_x1, inner_y1, inner_x2, inner_y2)
            state["global_view_width"] = max(1, inner_x2 - inner_x1)

            group_colors = {
                str(group.get("name", "")): str(group.get("color", "#8D70C7"))
                for group in model
            }

            offset = int(state["global_offset"])
            left = inner_x1 + 24 - offset
            current_group = None
            group_start = None
            group_boxes = []
            page_step = 84
            bottom_y = inner_y2 - 92

            content_end = inner_x1 + 24

            for page in pages:
                kind = page.get("kind", "Page courante")
                if kind == "Tête de partie":
                    size = (56, 82)
                elif kind == "Page auto":
                    size = (44, 66)
                else:
                ...
                page_x = left
                page_y = bottom_y - ph
                page_group = str(page.get("group", "Partie"))

                if current_group != page_group:
                    if current_group is not None and group_start is not None:
                        group_boxes.append((current_group, group_start, page_x - 20))
                    current_group = page_group
                    group_start = page_x - 10

                if page_x + pw >= inner_x1 - 80 and page_x <= inner_x2 + 80:
                    draw_page_thumb(page_x, page_y, kind, page.get("global", 0), size)

                left += page_step
                content_end = page_x + pw

            if current_group is not None and group_start is not None:
                group_boxes.append((current_group, group_start, content_end + 10))

            for group_name, gx1, gx2 in group_boxes:
                color = group_colors.get(group_name, "#8D70C7")
                top = inner_y1 + 24
                folder_x1 = max(inner_x1 + 16, gx1)
                folder_x2 = min(inner_x2 - 16, gx2)
                if folder_x2 - folder_x1 < 48:
                    continue
                rounded(folder_x1, top, folder_x2, top + 30, 10, fill=mix_hex(color, "#FFFFFF", 0.88), outline=mix_hex(color, "#FFFFFF", 0.42), width=1)
                canvas.create_text(folder_x1 + 14, top + 15, text=group_name, fill="#173B6C", font=("Segoe UI", 9, "bold"), anchor="w")
                canvas.create_line(folder_x1 + 10, top + 38, folder_x2 - 10, top + 38, fill=mix_hex(color, "#FFFFFF", 0.24), width=2)

            state["global_total_width"] = max(int(content_end - inner_x1 + 60), int(inner_x2 - inner_x1))
            max_offset = max(0, int(state["global_total_width"] - state["global_view_width"]))
            if state["global_offset"] > max_offset:
                state["global_offset"] = max_offset

        def draw_reader_page(x1, y1, x2, y2, page, side):
            canvas.create_rectangle(x1 + 8, y1 + 10, x2 + 8, y2 + 10, fill="#D6D0C7", outline="")
            canvas.create_rectangle(x1, y1, x2, y2, fill="#FFFEFC", outline="#CFC8BF", width=1)

            kind = str(page.get("kind", "Page courante"))
            group = str(page.get("group", ""))
            if group:
                canvas.create_text(x1 + 20, y1 + 18, text=group, fill="#173B6C", font=("Georgia", 15, "bold"), anchor="nw")
                canvas.create_line(x1 + 20, y1 + 50, x2 - 20, y1 + 50, fill="#D6D0C7", width=1)

            photo = get_type_thumb(kind, (int(x2 - x1 - 56), int(y2 - y1 - 110)))
            if photo is not None:
                canvas.create_image((x1 + x2) / 2, (y1 + y2) / 2 + 8, image=photo, anchor="center")
            else:
                canvas.create_rectangle(x1 + 36, y1 + 78, x2 - 36, y2 - 78, fill="#F0ECE4", outline="")
                canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, text=kind.upper(), fill="#2E2B28", font=("Georgia", 18, "bold"), justify="center")

            canvas.create_text((x1 + x2) / 2, y2 - 26, text=kind, fill="#6A655F", font=("Segoe UI", 9), anchor="center")
            if side == "left":
                canvas.create_line(x2 - 1, y1 + 8, x2 - 1, y2 - 8, fill="#BFB8AF", width=2)
            else:
                canvas.create_line(x1 + 1, y1 + 8, x1 + 1, y2 - 8, fill="#BFB8AF", width=2)

        def draw_book_view(rect, pages):
            x1, y1, x2, y2 = rect
            rounded(x1, y1, x2, y2, 14, fill="#F7F4ED", outline="#E2DDD5", width=1)
            inner_x1 = x1 + 22
            inner_y1 = y1 + 16
            inner_x2 = x2 - 22
            inner_y2 = y2 - 18
            canvas.create_rectangle(inner_x1, inner_y1, inner_x2, inner_y2, fill="#F9F6F0", outline="")
            for gx in range(int(inner_x1) + 34, int(inner_x2), 110):
                canvas.create_line(gx, inner_y1 + 12, gx, inner_y2 - 12, fill="#ECE6DE", width=1)
            for gy in range(int(inner_y1) + 20, int(inner_y2), 56):
                canvas.create_line(inner_x1 + 14, gy, inner_x2 - 14, gy, fill="#EFE8E1", width=1)

            base_w = min(610, max(430, inner_x2 - inner_x1 - 120))
            base_h = min(260, max(180, inner_y2 - inner_y1 - 110))
            base_x1 = (inner_x1 + inner_x2 - base_w) / 2
            base_x2 = base_x1 + base_w
            base_y2 = inner_y2 - 26
            base_y1 = base_y2 - base_h

            # Support discret vu du dessus.
            rounded(base_x1, base_y1 + 42, base_x2, base_y2, 18, fill="#E8E1D7", outline="#D7CEC2", width=1)
            canvas.create_oval(base_x1 + 34, base_y2 - 28, base_x2 - 34, base_y2 + 10, fill="#D7CCBF", outline="")
            canvas.create_oval(base_x1 + 58, base_y2 - 18, base_x2 - 58, base_y2 + 2, fill="#EEE8DF", outline="")

            book_w = min(520, max(360, inner_x2 - inner_x1 - 190))
            page_w = int(book_w / 2)
            page_h = min(360, max(240, inner_y2 - inner_y1 - 78))
            book_x1 = (inner_x1 + inner_x2 - book_w) / 2
            gutter = book_x1 + page_w
            page_top = inner_y1 + 34
            page_bottom = page_top + page_h

            canvas.create_rectangle(book_x1 - 14, page_top - 14, book_x1 + book_w + 14, page_bottom + 14, fill="#D4CCC1", outline="")
            canvas.create_rectangle(book_x1 - 6, page_top - 6, gutter - 4, page_bottom + 6, fill="#FDFBF7", outline="#CFC8BF", width=1)
            canvas.create_rectangle(gutter + 4, page_top - 6, book_x1 + book_w + 6, page_bottom + 6, fill="#FDFBF7", outline="#CFC8BF", width=1)
            canvas.create_line(gutter, page_top - 10, gutter, page_bottom + 10, fill="#8F857A", width=4)
            canvas.create_line(gutter - 4, page_top - 8, gutter - 4, page_bottom + 8, fill="#B7AEA3", width=1)
            canvas.create_line(gutter + 4, page_top - 8, gutter + 4, page_bottom + 8, fill="#B7AEA3", width=1)

            clamp_spread(pages)
            left_index = state["spread_left"]
            left_page = pages[left_index] if left_index < len(pages) else None
            right_page = pages[left_index + 1] if left_index + 1 < len(pages) else None

            if left_page is not None:
                draw_reader_page(book_x1, page_top, gutter - 8, page_bottom, left_page, "left")
            if right_page is not None:
                draw_reader_page(gutter + 8, page_top, book_x1 + book_w, page_bottom, right_page, "right")

            canvas.create_text((inner_x1 + inner_x2) / 2, inner_y1 + 4, text="Cliquez à gauche ou à droite sur le livre pour feuilleter.", fill="#6B747C", font=("Segoe UI", 9), anchor="n")
            regions["book_click"] = (book_x1, page_top, book_x1 + book_w, page_bottom)

        def draw_tools(rect):
            x1, y1, x2, y2 = rect
            rounded(x1, y1, x2, y2, 14, fill="#F7F4ED", outline="#E2DDD5", width=1)
            canvas.create_text(x1 + 18, y1 + 18, text="Outils communs", fill="#173B6C", font=("Georgia", 15, "bold"), anchor="nw")
            canvas.create_text(x1 + 18, y1 + 48, text="Zone réservée aux futures commandes de contrôle.", fill="#6B747C", font=("Segoe UI", 9), anchor="nw")
            canvas.create_line(x1 + 18, y1 + 78, x2 - 18, y1 + 78, fill="#DDD7CF", width=1)
            box_y = y1 + 100
            hints = [
                "• outils transversaux",
                "• filtres d’affichage",
            ...
            for hint in hints:
                rounded(x1 + 18, box_y, x2 - 18, box_y + 42, 10, fill="#FFFEFD", outline="#E1DBD3", width=1)
                canvas.create_text(x1 + 32, box_y + 21, text=hint, fill="#64707A", font=("Segoe UI", 9), anchor="w")
                box_y += 56

        def render(_event=None):
            if render_guard["busy"]:
                return
            render_guard["busy"] = True
            try:
                width = max(canvas.winfo_width(), 1220)
                ...
                shell_x2 = width - 18
                shell_y2 = height - 24
                draw_shell(shell_x1, shell_y1, shell_x2, shell_y2)

                title_x = shell_x1 + 28
                title_y = shell_y1 + 30
                draw_title_line(title_x, shell_x2 - 28, title_y, "Visualisation")
                canvas.create_text(title_x, title_y + 42, text="Vue globale en haut, vue feuilletée en dessous. Même base visuelle pour le moment.", fill="#6B747C", font=("Segoe UI", 10), anchor="nw")
                draw_return_button(shell_x2 - 126, shell_y1 + 38)

                content_top = shell_y1 + 86
                content_bottom = shell_y2 - 18
                tools_w = 286
                gap = 18

                left_x1 = shell_x1 + 18
                left_x2 = shell_x2 - tools_w - gap
                tools_x1 = left_x2 + gap
                tools_x2 = shell_x2 - 18

                global_y1 = content_top
                global_y2 = global_y1 + 250
                book_y1 = global_y2 + gap
                book_y2 = content_bottom

                global_rect = draw_panel(left_x1, global_y1, left_x2, global_y2, "#7EB59C", "Vue globale", "Panorama du livre, lecture de gauche à droite.")
                book_rect = draw_panel(left_x1, book_y1, left_x2, book_y2, "#8D70C7", "Vue feuilletée", "Feuilletage simple du livre à partir des vignettes connues.")
                tools_rect = draw_panel(tools_x1, content_top, tools_x2, content_bottom, "#E28A6D", "Outils", "Réservé aux outils communs de Visualisation.")
                ...
                draw_global_view(global_rect, pages, model)
                draw_book_view(book_rect, pages)
                draw_tools(tools_rect)
            finally:
                render_guard["busy"] = False
        ...
        def on_press(event):
            if inside(regions.get("return"), event.x, event.y):
                self._close_visualisation_page()
                return
            if inside(regions.get("global_drag"), event.x, event.y):
                state["drag_active"] = True
                state["drag_start_x"] = event.x
                state["drag_origin"] = state["global_offset"]
                canvas.configure(cursor="fleur")
                return
            if inside(regions.get("book_click"), event.x, event.y):
                bounds = regions.get("book_click")
                if bounds:
        ...
        def on_motion(event):
            if state["drag_active"]:
                delta = event.x - state["drag_start_x"]
                max_offset = max(0, int(state["global_total_width"] - state["global_view_width"]))
                state["global_offset"] = max(0, min(max_offset, int(state["drag_origin"] - delta)))
                render()
                return
            if inside(regions.get("return"), event.x, event.y):
                canvas.configure(cursor="hand2")
            elif inside(regions.get("global_drag"), event.x, event.y):
                canvas.configure(cursor="hand2")
            elif inside(regions.get("book_click"), event.x, event.y):
                canvas.configure(cursor="hand2")
            else:
        ...
        canvas.bind("<Leave>", lambda _e: canvas.configure(cursor="arrow"))

        self._visualisation_render = render
