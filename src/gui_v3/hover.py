...
class GlobalHoverManager:
    ...
    def _resolve_target(self, widget: tk.Misc | None) -> tk.Misc | None:
        ...
        # SURVOL_CIBLE_SEULE_V3_V2
        # Les canevas qui gèrent eux-mêmes leurs objets interactifs
        # (notamment le livre B) ne doivent jamais faire réagir tout leur fond.
        if isinstance(widget, tk.Canvas) and getattr(widget, "_tl_item_hover_only", False):
            return None
    ...
    def _apply(self, target: tk.Misc, *, pressed: bool = False) -> None:
        ...
        amount = 0.11 if not pressed else 0.22
