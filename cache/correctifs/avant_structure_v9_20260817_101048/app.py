...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V8 : briques compactes + règles générales et exceptions locales."""
        ...
        # --------------------------------------------------------------
        # Niveau local : exception de page automatique à la sélection
        # --------------------------------------------------------------
        selection_row = tk.Frame(palette, bg="#18251F", highlightthickness=1, highlightbackground="#30463D")
        selection_row.grid(row=1, column=0, sticky="ew", pady=(0, 3))
        selection_row.grid_columnconfigure(3, weight=1)
        selection_row.grid_remove()
        tk.Label(
            selection_row, text="Après cette page :", bg="#18251F", fg=theme.MUTED,
            font=(theme.FONT_UI, 7, "bold"),
        ).grid(row=0, column=0, padx=(7, 5), pady=3, sticky="w")
        local_auto_var = tk.StringVar(value="")
        local_auto_button = tk.Menubutton(
            selection_row, textvariable=local_auto_var,
            bg="#254239", fg=theme.WHITE,
            activebackground="#31594B", activeforeground=theme.WHITE,
            relief="flat", bd=0, padx=8, pady=3,
            font=(theme.FONT_UI, 7, "bold"), cursor="hand2",
        )
        local_auto_menu = tk.Menu(local_auto_button, tearoff=False, bg=theme.PANEL_ALT, fg=theme.INK, activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE)
        local_auto_button.configure(menu=local_auto_menu)
        local_auto_button.grid(row=0, column=1, padx=(0, 7), pady=2, sticky="w")
        general_info_var = tk.StringVar(value="")
        tk.Label(
            selection_row, textvariable=general_info_var,
            bg="#18251F", fg="#8CB4A4",
            font=(theme.FONT_UI, 7),
        ).grid(row=0, column=2, padx=(0, 7), pady=3, sticky="w")

        def auto_option_labels():
            canvas = book()
            options = canvas.structure_auto_target_options() if canvas is not None and hasattr(canvas, "structure_auto_target_options") else []
            return options, {key: label for key, label in options}

        def apply_local_auto(mode, target=""):
            canvas = book()
            if canvas is not None and hasattr(canvas, "structure_set_selected_auto_override"):
                canvas.structure_set_selected_auto_override(mode, target)
            self.after_idle(refresh_selection_context)

        def refresh_selection_context():
            reset_delete_state()
            canvas = book()
            if canvas is None or not hasattr(canvas, "structure_selected_auto_context"):
                selection_row.grid_remove()
                return
            ctx = canvas.structure_selected_auto_context()
            if not ctx.get("selected") or not ctx.get("allowed"):
                selection_row.grid_remove()
                return
            selection_row.grid()
            options, labels = auto_option_labels()
            general = str(ctx.get("general_target") or "")
            general_label = labels.get(general, "Aucune") if general else "Aucune"
            general_info_var.set(f"Règle générale : {general_label}")
            local_auto_menu.delete(0, "end")
            inherit_label = f"Selon règle générale — {general_label.lower()}"
            local_auto_menu.add_command(label=inherit_label, command=lambda: apply_local_auto("inherit"))
            local_auto_menu.add_command(label="Aucune pour cette page", command=lambda: apply_local_auto("none"))
            local_auto_menu.add_separator()
            for key, label in options:
                local_auto_menu.add_command(label=label, command=lambda k=key: apply_local_auto("local", k))
            mode = ctx.get("mode")
            if mode == "none":
                local_auto_var.set("Aucune pour cette page")
            elif mode == "local":
                local_auto_var.set(labels.get(str(ctx.get("local_target") or ""), "Page automatique"))
            else:
                local_auto_var.set(inherit_label)

        # --------------------------------------------------------------
        # Tête / palette
        # --------------------------------------------------------------
        palette_head = tk.Frame(palette, bg=theme.PANEL_ALT)
        palette_head.grid(row=2, column=0, sticky="ew", pady=(0, 3))
        ...
        palette_body.grid(row=3, column=0, sticky="nsew")
        ...
        def make_brick(parent_, label, kind, payload, *, locked=False):
            """Brique TomeLinea compacte : objet éditorial, pas bouton de commande."""
            base = "#213B33"
            hover = "#2A4A40"
            pressed = "#31584A"
            armed = "#294E42"
            ...
            locked_bg = "#2A3035"
            card = tk.Frame(
                parent_, bg=locked_bg if locked else base,
                highlightthickness=1, highlightbackground="#495159" if locked else border,
                width=106, height=42,
            )
            card.grid_propagate(False)
            edge = tk.Frame(card, bg="#70767A" if locked else gold, width=3)
            edge.grid(row=0, column=0, sticky="ns")
            card.grid_rowconfigure(0, weight=1)
            card.grid_columnconfigure(1, weight=1)
            content = tk.Frame(card, bg=locked_bg if locked else base, padx=6, pady=4)
            content.grid(row=0, column=1, sticky="nsew")
            symbol = tk.Label(
                content, text="◆" if not locked else "▣",
                bg=locked_bg if locked else base,
                fg=gold if not locked else theme.MUTED_DARK,
                font=(theme.FONT_UI, 7, "bold"),
            )
            symbol.pack(side="left", padx=(0, 5))
            name = tk.Label(
                content, text=label, bg=locked_bg if locked else base,
                fg=theme.INK if not locked else theme.MUTED,
                font=(theme.FONT_UI, 7, "bold"), anchor="w",
                wraplength=78, justify="left",
            )
            name.pack(side="left", fill="both", expand=True)
            children = (card, edge, content, symbol, name)
            if locked:
            ...
            def paint(bg, line):
                card.configure(bg=bg, highlightbackground=line)
                content.configure(bg=bg)
                symbol.configure(bg=bg)
        ...
        def install_scroll_bindings(root_widget, cv):
            ...
            def wheel(event):
                ...
                return "break"
            self.bind_class(tag, "<MouseWheel>", wheel)
            self.bind_class(tag, "<Button-4>", wheel)
            self.bind_class(tag, "<Button-5>", wheel)
            def apply(widget):
        ...
        def make_scroll_flow(parent_, entries, *, columns=9):
            ...
            sb = tk.Scrollbar(holder, orient="vertical", command=cv.yview, width=10)
        ...
        def build_rules():
            ...
            if not options:
                tk.Label(section, text="Aucun type disponible.", bg=theme.PANEL_ALT, fg=theme.MUTED).pack(anchor="w")
                return
            source_var = tk.StringVar(value=options[0][1])
            target_var = tk.StringVar(value=options[0][1])
            source_key = {"value": options[0][0]}
            target_key = {"value": options[0][0]}
            editor = tk.Frame(section, bg="#18251F", highlightthickness=1, highlightbackground="#30463D")
            editor.pack(fill="x", pady=(1, 5))
            tk.Label(editor, text="Quand", bg="#18251F", fg=theme.MUTED, font=(theme.FONT_UI,7,"bold")).pack(side="left", padx=(7,4), pady=4)

            def menu_button(parent_, var, holder):
                mb=tk.Menubutton(parent_, textvariable=var, bg="#254239", fg=theme.WHITE, activebackground="#31594B", activeforeground=theme.WHITE, relief="flat", bd=0, padx=8, pady=3, font=(theme.FONT_UI,7,"bold"), cursor="hand2")
                menu=tk.Menu(mb,tearoff=False,bg=theme.PANEL_ALT,fg=theme.INK,activebackground=theme.ACCENT_DARK,activeforeground=theme.WHITE)
                mb.configure(menu=menu)
                for key,label in options:
                    menu.add_command(label=label, command=lambda k=key,l=label: (holder.__setitem__("value",k), var.set(l)))
                return mb
            source_mb=menu_button(editor,source_var,source_key); source_mb.pack(side="left",padx=(0,5),pady=3)
            tk.Label(editor,text="est présente  →  ajouter",bg="#18251F",fg=theme.MUTED,font=(theme.FONT_UI,7,"bold")).pack(side="left",padx=(0,5),pady=4)
            target_mb=menu_button(editor,target_var,target_key); target_mb.pack(side="left",padx=(0,6),pady=3)

            rules_holder=tk.Frame(section,bg=theme.PANEL_ALT); rules_holder.pack(fill="both",expand=True)
            def refresh_rules_list():
                for child in rules_holder.winfo_children(): child.destroy()
                current=book()
                rules=current.structure_general_auto_rules() if current is not None and hasattr(current,"structure_general_auto_rules") else {}
                tk.Label(rules_holder,text="Règles générales du livre",bg=theme.PANEL_ALT,fg=theme.ACCENT_BRIGHT,font=(theme.FONT_UI,7,"bold")).pack(anchor="w",pady=(0,3))
                if not rules:
                    tk.Label(rules_holder,text="Aucune règle générale. Les choix locaux restent possibles page par page.",bg=theme.PANEL_ALT,fg=theme.MUTED_DARK,font=(theme.FONT_UI,7)).pack(anchor="w")
                    return
                for source,target in rules.items():
                    row=tk.Frame(rules_holder,bg=theme.PANEL_SOFT,highlightthickness=1,highlightbackground=theme.BORDER)
                    row.pack(fill="x",pady=2)
                    s_label=labels.get(source, source.replace("_"," ").capitalize())
                    t_label=labels.get(target, target.replace("_"," ").capitalize())
                    select=tk.Label(row,text=f"{s_label}   →   {t_label}",bg=theme.PANEL_SOFT,fg=theme.INK,font=(theme.FONT_UI,7,"bold"),anchor="w",cursor="hand2")
                    select.pack(side="left",fill="x",expand=True,padx=8,pady=4)
                    def load(_e=None,s=source,t=target,sl=s_label,tl=t_label):
                        source_key["value"]=s; target_key["value"]=t; source_var.set(sl); target_var.set(tl)
                    select.bind("<Button-1>",load)
                    remove=tk.Label(row,text="×",bg=theme.PANEL_SOFT,fg="#E4876E",font=(theme.FONT_UI,10,"bold"),cursor="hand2")
                    remove.pack(side="right",padx=7)
                    def remove_rule(_e=None,s=source):
                        ...
                        refresh_rules_list(); self.after_idle(refresh_selection_context)
                    remove.bind("<Button-1>",remove_rule)

            def save_rule():
                current=book()
                if current is not None and hasattr(current,"structure_set_general_auto_rule"):
                    current.structure_set_general_auto_rule(source_key["value"],target_key["value"])
                refresh_rules_list(); self.after_idle(refresh_selection_context)
            command_button(editor,"Enregistrer la règle",save_rule).pack(side="left",padx=(0,7),pady=3)
            tk.Label(section,text="Une page peut ensuite déroger à cette règle directement lorsqu’elle est sélectionnée dans B.",bg=theme.PANEL_ALT,fg=theme.MUTED_DARK,font=(theme.FONT_UI,7)).pack(anchor="w",pady=(0,5))
            refresh_rules_list()
        ...
        def select_family(name):
            ...
            if name=="Pages":
                ...
                make_scroll_flow(palette_body,entries,columns=9)
                ...
                make_scroll_flow(palette_body,entries,columns=9)
                palette_hint_var.set("Briques de types disponibles. Molette pour défiler.")
            elif name=="Règles":
                build_rules()
                palette_hint_var.set("Règle générale : page source → page automatique associée.")
        ...
        # Synchronisation légère : une sélection dans B met à jour le niveau local.
        try:
