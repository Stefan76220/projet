...
# TOMELINEA_GABARITS_REGLAGES_LIVRE_V54
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        ...
        # 3 — Cadre : Marges par défaut, Page uniquement pour l'exception rare.
        ...
        target.create_text(
            right, y,
            text=self._gabarit_frame_reference_label(frame_ref).upper() + (" · EXCEPTION" if exception else ""),
            anchor="e", fill="#78B8B1" if exception else "#9FB0B3",
            font=(theme.FONT_UI, 7, "bold" if exception else "normal"),
        )
        y += 18.0
        gap = 5.0
        bw = (inner - gap) / 2.0
        for i, (label, ref) in enumerate((("Marges", "margins"), ("Page", "page"))):
            x1 = tx + i * (bw + gap)
            self._gabarit_inspector_control_box(
                target, f"frame_{ref}", (x1, y, x1+bw, y+30),
                selected=(frame_ref == ref or pending_ref == ref),
            )
            target.create_text(
                x1+bw/2.0, y+15.0, text=label, anchor="center", fill="#E7ECEA",
                font=(theme.FONT_UI, 8, "bold" if frame_ref == ref else "normal"),
            )
        y += 37.0
        if pending_ref in {"margins", "page"} and pending_ref != frame_ref:
            target.create_text(tx, y, text="APPLIQUER CE CADRE", anchor="w", fill="#9FB0B3", font=(theme.FONT_UI, 7, "bold"))
            y += 15.0
            bw2 = (inner - gap) / 2.0
            self._gabarit_inspector_control_box(target, "align_scope_page", (tx, y, tx+bw2, y+28))
            target.create_text(tx+bw2/2.0, y+14.0, text="Cette page", anchor="center", fill="#E7ECEA", font=(theme.FONT_UI, 7, "bold"))
            x2 = tx + bw2 + gap
            self._gabarit_inspector_control_box(target, "align_scope_type", (x2, y, right, y+28))
            target.create_text(x2+bw2/2.0, y+14.0, text="Toutes du type", anchor="center", fill="#E7ECEA", font=(theme.FONT_UI, 7, "bold"))
            y += 36.0

        target.create_line(tx, y, right, y, fill="#40525B", width=1)
        y += 13.0

        # 4 — Première action de construction. Rien d'autre avant sélection.
