...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V10_VUES_PRECHAUFFEES_FIXES
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # Architecture stable : une seule géométrie pour les deux vues.
        # Comme Accueil / Espace Projet, les deux contenus sont construits
        # une fois puis simplement remontés avec tkraise().
        fixed_size = (
        ...
        # ----------------------------------------------------------
        # ----------------------------------------------------------
        # Changement de vue : méthode éprouvée TomeLinea.
        # Les deux vues restent construites dans le même conteneur,
        # à la même taille. Le clic ne fait qu'un tkraise().
        # ----------------------------------------------------------
        def show_tab(key, *, animate=True):
            if key not in ("global", "feuilleter"):
                return
            if flip_state["active"] or key == state["tab"]:
                return
        ...
        # Seul l'empilement change ; aucune géométrie, aucun alpha,
        # aucun withdraw/deiconify lors des clics d'onglet.
        if state["tab"] == "global":
            global_host.tkraise()
        else:
            book_host.tkraise()
