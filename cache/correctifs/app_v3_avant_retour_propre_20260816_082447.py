...
class TomeLineaV3(tk.Tk):
    # FENETRE_WINDOWS_NATIVE_FLUIDE_V3_V1
    # ZONE_A_CHANTIER_NATIVE_V3_V1
    # REDESSIN_FENETRE_DEBOUNCE_V3_V1
    ...
    def __init__(self) -> None:
        ...
        # La V3 est construite hors écran. La vraie fenêtre Windows est
        # ensuite affichée directement dans son état maximisé.
        self.withdraw()
        try:
        ...
        # Prépare une taille de restauration utile, puis confie réellement
        # maximisation/réduction/restauration à Windows.
        self._prepare_native_window_geometry()
        self.deiconify()
        try:
        ...
        # Une seule mise en page complète avant de rendre la fenêtre visible.
        self.update_idletasks()
        self.update()
    ...
    def _windows_work_area(self):
        """Zone de travail Windows, barre des tâches exclue."""
        try:
            import ctypes
    ...
    def _prepare_native_window_geometry(self):
        """Fixe seulement la taille mémorisée par Windows pour « Restaurer »."""
        left, top, work_w, work_h = self._windows_work_area()
        width = min(work_w - 40, max(1280, int(work_w * 0.84)))
        height = min(work_h - 40, max(740, int(work_h * 0.84)))
        width = max(1100, width)
    ...
    def _fit_to_windows_work_area(self):
        """Repli hors Windows ; sous Windows le vrai mode zoomed est utilisé."""
        if sys.platform == "win32":
            try:
                self.state("zoomed")
                return
        ...
        left, top, work_w, work_h = self._windows_work_area()
        self.state("normal")
        self.geometry(f"{work_w}x{work_h}+{left}+{top}")

    def _native_toplevel_hwnd(self):
        """HWND réel du toplevel Tk sous Windows."""
        if sys.platform != "win32":
            return None
        try:
            import ctypes
    ...
    def _apply_native_titlebar_theme(self):
        """Habille la vraie barre Windows sans modifier son comportement."""
        if sys.platform != "win32":
            return
        ...
        try:
            ...
            def set_dword(attribute: int, value: int):
            ...
            # Windows 11 : barre native sombre, couleurs TomeLinea.
            set_dword(20, 1)  # DWMWA_USE_IMMERSIVE_DARK_MODE
            set_dword(35, self._colorref(theme.WINDOW_DEEP))  # CAPTION_COLOR
            set_dword(36, self._colorref(theme.INK))          # TEXT_COLOR
            set_dword(34, self._colorref(theme.WINDOW_DEEP))  # BORDER_COLOR
        except Exception:
            # Le comportement natif reste correct même si une version de
            # Windows refuse une option purement esthétique.
            pass
    ...
    def _background_photo(self, width: int, height: int, key: str):
        ...
        image = source.resize((new_w, new_h), Image.Resampling.BICUBIC)
    ...
    def _screen_with_background(self, key: str):
        ...
        screen._bg_label = bg
        screen._bg_resize_job = None
        screen._bg_has_image = False

        def apply_background(width: int, height: int):
            screen._bg_resize_job = None
            if not screen.winfo_exists():
                return
            photo = self._background_photo(width, height, key)
            if photo is not None:
                bg.configure(image=photo)
                bg.image = photo
                screen._bg_has_image = True

        def refresh(event):
            width = max(400, int(event.width))
            height = max(300, int(event.height))

            if screen._bg_resize_job is not None:
                try:
                    self.after_cancel(screen._bg_resize_job)
                except Exception:
                    pass

            # Au premier affichage, produire rapidement une image. Ensuite,
            # attendre que le changement de taille soit stabilisé : aucune
            # interpolation PIL lourde au milieu de l'animation Windows.
            delay = 20 if not screen._bg_has_image else 170
            screen._bg_resize_job = self.after(
                delay,
                lambda w=width, h=height: apply_background(w, h),
            )
    ...
    def _build_zone_a(self):
        """Bandeau permanent : identité, projet, contexte de travail."""
        body = self.zone_a.body
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(2, weight=4, minsize=240)
        body.grid_columnconfigure(4, weight=3, minsize=220)

        # 1. Signature TomeLinea.
        brand = tk.Frame(body, bg=theme.PANEL)
        brand.grid(row=0, column=0, sticky="nsw", padx=(0, 12), pady=(2, 1))
        ...
        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=1, sticky="ns", padx=(4, 15)
        )

        # 2. Projet et type.
        project = tk.Frame(body, bg=theme.PANEL)
        project.grid(row=0, column=2, sticky="nsew", padx=(0, 17), pady=(1, 0))
        tk.Label(
            project,
            text="PROJET",
            bg=theme.PANEL,
        ...
        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=3, sticky="ns", padx=(0, 17)
        )

        # 3. Travail actuel : pas de pourcentage arbitraire.
        current = tk.Frame(body, bg=theme.PANEL)
        current.grid(row=0, column=4, sticky="nsew", padx=(0, 12), pady=(1, 0))
        tk.Label(
            current,
            text="TRAVAIL ACTUEL",
            bg=theme.PANEL,
        ...
        tk.Label(
            current,
            textvariable=self.workspace_current_var,
            bg=theme.PANEL,
            fg=theme.ACCENT_BRIGHT,
        ...
        tk.Frame(body, bg=theme.BORDER_SOFT, width=1).grid(
            row=0, column=5, sticky="ns", padx=(0, 11)
        )

        # 4. Commandes TomeLinea. Les commandes de fenêtre restent dans la
        # vraie barre Windows, donc aucun doublon et aucune simulation ici.
