...
class MockupView:
    ...
    def _create_group_block(
        ...
        if deleted:
            ...
            block.grid_rowconfigure((0, 2), weight=1)
            ...
            trace.grid(row=1, column=0, sticky="ew", padx=4)
            trace.grid_propagate(False)
            trace.grid_columnconfigure(0, weight=1)

            part_number = group_definition.get("part_number")
            trace_text = (
                f"Partie {part_number} — Supprimée"
                if part_number not in (None, "")
                else "Partie supprimée"
            )
            ctk.CTkLabel(
                trace,
                text=trace_text,
