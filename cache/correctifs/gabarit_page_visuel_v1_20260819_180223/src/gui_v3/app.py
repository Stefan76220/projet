...
class TomeLineaV3(tk.Tk):
    ...
    def _build_gabarits_tab(self, parent):
        ...
        page_card, page_box = family_card(0, "1", "PAGE", "définir les limites")
        ...
        # 1 — PAGE
        # ----------------------------------------------------------
        page_box.grid_columnconfigure(0, weight=1)
        margins_var = tk.StringVar(value="Marges\n—")
        bleed_var = tk.StringVar(value="Fond perdu\n—")

        margins_btn = V3Button(page_box, "", compact=True, primary=True)
        margins_btn.configure(textvariable=margins_var, pady=7)
        margins_btn.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        bleed_btn = V3Button(page_box, "", compact=True)
        bleed_btn.configure(textvariable=bleed_var, pady=7)
        bleed_btn.grid(row=1, column=0, sticky="ew")
        ...
        def refresh_gabarit_tools(_event=None):
            ...
            margins_var.set(
                ...
                f"I {float(margins.get('inside', 0)):g} · E {float(margins.get('outside', 0)):g} mm"
            )
            bleed_var.set(
                "Fond perdu\n"
                f"H {float(bleed.get('top', 0)):g} · D {float(bleed.get('right', 0)):g} · "
                f"B {float(bleed.get('bottom', 0)):g} · G {float(bleed.get('left', 0)):g} mm"
            )
            try:
