...
# TOMELINEA_GABARITS_DOUBLEPAGE_V101
...
# STRUCTURE_2P_V102 : paire locale immédiate, extension par suite exacte, Scinder global, R/V suspendu en 2P.
...
class BookCanvas(tk.Frame):
    ...
    def __init__(
        ...
        *,
        ...
        # Ancre de sélection Structure : un clic simple (ou Ctrl+clic ajouté)
        # fixe le point de départ ; Maj+clic sélectionne ensuite toute la plage
        # de pages principales comprise entre cette ancre et la page cliquée.
        self._structure_selection_anchor_index: int | None = None
        ...
        # PRODUCTION_V22 — contenu réel attaché aux zones du gabarit.
    ...
    def _structure_selection_unit_indices(self, index: int | None) -> list[int]:
        """Retourne l'unité Structure sélectionnable contenant ``index``.

        Une page normale constitue une unité d'une page. Une vraie double page
        soudée (paire 2P) constitue une seule unité de sélection : cliquer sur
        l'une de ses deux moitiés doit toujours sélectionner les deux.
        """
        ...
        pair_id = self._double_page_pair_id(source)
        if pair_id:
            members = self._valid_double_page_pair_members(pair_id)
            if members:
                member_ids = {str(item.get("id") or "").strip() for item in members}
                return [
                    i for i, item in enumerate(self.items)
                    if not self._is_automatic_page(item)
                    and str(item.get("id") or "").strip() in member_ids
                ]
        return [source_index]
    ...
    def _set_single_page_selection(self, index: int | None) -> None:
        ...
        if source_index is None:
            ...
            self._structure_selection_anchor_index = None
        ...
        unit_ids = {
            str(self.items[i].get("id") or "").strip()
            for i in unit_indices
            if 0 <= i < len(self.items) and str(self.items[i].get("id") or "").strip()
        }
        self._selected_index = source_index
        self._structure_selection_anchor_index = source_index
        source = self.items[source_index]
        self._selected_page_ids = unit_ids
        self._selected_group_id = self._item_group_id(source)
        self._structure_selection_kind = "page"
        # Une 2P est une unité atomique : les commandes R/V d'une page simple
        # ne sont pas armées lorsqu'on sélectionne l'une de ses deux moitiés.
        self._structure_recto_verso_armed = (
            len(unit_indices) == 1 and self.structure_recto_verso_source_allowed(source)
        )
    ...
    def _set_multi_page_selection(self, indices) -> None:
        ...
        for raw in indices or []:
            for source_index in self._structure_selection_unit_indices(raw):
        ...
        self._structure_selection_anchor_index = ordered[0]
    ...
    def _select_page_range(self, index: int | None) -> None:
        """Maj+clic : sélectionne toutes les pages principales entre l'ancre et la cible."""
        target_index = self._source_index_for_index(index)
        if target_index is None:
            return

        anchor_index = self._source_index_for_index(
            getattr(self, "_structure_selection_anchor_index", None)
        )
        if anchor_index is None:
            anchor_index = self._source_index_for_index(self._selected_index)
        if anchor_index is None:
            self._set_single_page_selection(target_index)
            return

        lo, hi = sorted((anchor_index, target_index))
        indices = [
            i for i in range(lo, hi + 1)
            if 0 <= i < len(self.items) and not self._is_automatic_page(self.items[i])
        ]
        preserved_anchor = anchor_index
        self._set_multi_page_selection(indices)
        # La cible devient la page de référence visuelle/fonctionnelle, tandis
        # que l'ancre reste fixe pour permettre plusieurs Maj+clic successifs.
        self._structure_selection_anchor_index = preserved_anchor
        if target_index in indices:
            self._selected_index = target_index
            self._selected_group_id = self._item_group_id(self.items[target_index])

    def _toggle_page_selection(self, index: int | None) -> None:
        """Ajoute/retire une page de la sélection Structure (Maj/Ctrl+clic)."""
        self._structure_double_pair_pending = None
        self._structure_rule_target = ""
        source_index = self._source_index_for_index(index)
        if source_index is None:
        ...
        if str(getattr(self, "_structure_selection_kind", "page") or "page") != "page":
            current_ids.clear()

        # Ctrl+clic sur une moitié d'une 2P ajoute/retire la 2P entière.
        if unit_ids.issubset(current_ids):
            current_ids.difference_update(unit_ids)
        else:
            current_ids.update(unit_ids)

        self._selected_page_ids = current_ids
        self._structure_selection_kind = "page"

        selected_indices = [
            i for i, item in enumerate(self.items)
            if not self._is_automatic_page(item)
            and str(item.get("id") or "").strip() in current_ids
        ]
        ...
        # La dernière page ajoutée devient la page de référence. Si elle vient
        # d'être retirée, on conserve comme référence la première page restante.
        if unit_ids.intersection(current_ids):
            primary_index = source_index
            self._structure_selection_anchor_index = source_index
        else:
            primary_index = selected_indices[0]
            if getattr(self, "_structure_selection_anchor_index", None) in unit_indices:
                self._structure_selection_anchor_index = primary_index
        self._selected_index = primary_index
        self._selected_group_id = self._item_group_id(self.items[primary_index])
        self._structure_recto_verso_armed = (
            len(selected_indices) == 1
            and self.structure_recto_verso_source_allowed(self.items[selected_indices[0]])
        )
    ...
    def _selected_source_indices(self) -> list[int]:
        ...
        if not ids and self._selected_index is not None:
            ...
            if source_index is not None:
                ...
                if source_id:
                    ids.add(source_id)
        # Une sélection héritée ou restaurée ne doit jamais laisser une moitié
        # de 2P isolée : dès qu'un membre est présent, son pair l'est aussi.
        ...
        for index, item in enumerate(self.items):
            ...
            if item_id not in ids:
                continue
            for unit_index in self._structure_selection_unit_indices(index):
                unit_id = str(self.items[unit_index].get("id") or "").strip()
                if unit_id:
                    expanded_ids.add(unit_id)
        ids = expanded_ids
    ...
    def _structure_page_auto_selection_source(self, position: str) -> tuple[dict | None, bool]:
        """Source logique AV/AP de la sélection courante.

        - page simple sélectionnée : la page elle-même ;
        - vraie 2P sélectionnée : AV s'attache à la moitié gauche et AP à la
          moitié droite afin que la page automatique reste à l'extérieur du
          bloc indivisible ;
        - toute autre multi-sélection : aucune source.

        Le booléen retourné indique qu'il s'agit d'une 2P sélectionnée comme
        bloc.
        """
        ...
        if len(indices) == 1:
            ...
            return (item, False) if self.structure_auto_source_allowed(item) else (None, False)
        if len(indices) != 2:
            return None, False
        ...
        if len(selected) != 2:
            return None, False
        pair_id = self._double_page_pair_id(selected[0])
        if not pair_id or pair_id != self._double_page_pair_id(selected[1]):
            return None, False
        members = self._valid_double_page_pair_members(pair_id)
        if not members:
            return None, False
        ...
        member_ids = {str(left.get("id") or "").strip(), str(right.get("id") or "").strip()}
        if selected_ids != member_ids:
            return None, False
        source = left if position == "before" else right
        return (source, True) if self.structure_auto_source_allowed(source) else (None, True)
    ...
    def _is_page_index_selected(self, index: int) -> bool:
        ...
        if selected_ids:
            if source_id in selected_ids:
                return True
            # Tolère un ancien état partiel : une moitié sélectionnée rend la
            # double page entière visuellement sélectionnée.
            unit_ids = {
                str(self.items[i].get("id") or "").strip()
                for i in self._structure_selection_unit_indices(source_index)
                if str(self.items[i].get("id") or "").strip()
            }
            return bool(unit_ids.intersection(selected_ids))
        if source_index == self._selected_index:
            return True
        return any(i == self._selected_index for i in self._structure_selection_unit_indices(source_index))
    ...
    def _structure_restore_selection_snapshot(self, snapshot: dict | None) -> None:
        ...
        if primary_id and primary_id in source_by_id:
            kept.add(primary_id)
        expanded_kept = set(kept)
        for page_id in list(kept):
            for unit_index in self._structure_selection_unit_indices(source_by_id[page_id]):
                unit_id = str(self.items[unit_index].get("id") or "").strip()
                if unit_id:
                    expanded_kept.add(unit_id)
        kept = expanded_kept
    ...
    def _build_canvas(self):
        ...
        self.structure_rule_extend_btn = self._structure_bar_button(
            self.structure_rule_action_panel, "Étendre", self.structure_extend_or_scind_context_rule,
        )
        self.structure_rule_extend_btn.configure(width=8, state="disabled")
        self.structure_rule_extend_btn.pack(side="left", padx=1, pady=1)
        ...
        self.structure_rule_exception_btn.configure(width=9, state="disabled")
        self.structure_rule_exception_btn.pack(side="left", padx=1, pady=1)
        ...
        self.structure_rule_remove_btn.configure(width=11, state="disabled")
        self.structure_rule_remove_btn.pack(side="left", padx=1, pady=1)
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "allowed":
            ...
            if self._gabarit_active_is_spread() and count <= 1:
                yy=content_top+50.0
                current=(str(info.get("spread_position") or self.gabarit_spread_target()) if count==1 else self.gabarit_spread_target())
                button=48.0; gap=6.0; sx=fly_left+12.0
                target.create_text(sx,yy-7.0,text="CIBLE 2P" if count==1 else "PROCHAINE ZONE 2P",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
                yy += 6.0
                for i,(lab,key,pos) in enumerate((("Gauche","spread_left","left"),("Double","spread_center","center"),("Droite","spread_right","right"))):
                    x1=sx+i*(button+gap)
                    tip=(f"Placer la zone sur la page {lab.lower()}" if pos != "center" else "Étendre la zone sur l'ensemble de la double page")
                    if count==0:
                        tip=(f"Créer la prochaine zone sur la page {lab.lower()}" if pos != "center" else "Créer la prochaine zone sur l'ensemble de la double page")
                    self._gabarit_inspector_station_button(target,key,(x1,yy,x1+button,yy+41.0),lab,selected=current==pos,tooltip=tip)
    ...
    def _production_inspector_signature_value(self):
        ...
        try:
            ...
            return (
                ...
                self._production_status(item),self.production_validation_exception(item),selected,
    ...
    def _render_production_tools_overlay(self, *, force: bool = False):
        ...
        # Le panneau d'état reste neutre : le code couleur d'avancement est
        ...
        exception_y1=96.0; exception_y2=119.0
        exception_on=self.production_validation_exception(item)
        self._gabarit_inspector_control_box(
            target,"production_validation_exception",
            (permanent_left,exception_y1,permanent_right,exception_y2),
            selected=exception_on,disabled=False,
        )
        target.create_text(
            permanent_left+8,(exception_y1+exception_y2)/2.0,
            text="EXCEPTION VALIDATION" + ("  ✓" if exception_on else ""),
            anchor="w",fill="#D8E0DF" if exception_on else "#829297",
            font=(theme.FONT_UI,6,"bold"),
        )
        self._gabarit_inspector_register_tooltip(
            "production_validation_exception",
            "Conserver le statut propre de cette page lors d'une validation de groupe",
        )
        ...
        rail_top=129.0; rail_bottom=quick_y-15.0
        gap_y=8.0
        available=max(1.0,rail_bottom-rail_top)
    ...
    def production_validation_exception(self, item: dict | None = None) -> bool:
        """Exception d'avancement, indépendante des exceptions de Gabarit."""
        if item is None:
            item = self._gabarit_active_item()
        return bool(isinstance(item, dict) and item.get("production_validation_exception", False))

    def production_toggle_validation_exception(self) -> bool:
        item = self._gabarit_active_item()
        if not isinstance(item, dict):
            return False
        enabled = not bool(item.get("production_validation_exception", False))
        item["production_validation_exception"] = enabled
        self._save_order()
        self._gabarit_inspector_signature = None
        self.render()
    ...
    def _production_exception_validation_choice(self, item: dict) -> str:
        ...
        try:
            ...
            x = max(rx + 20, rx + (rw - ww) // 2)
            y = max(ry + 40, ry + (rh - wh) // 2)
            win.geometry(f"+{x}+{y}")
            win.grab_set()
            win.focus_force()
    ...
    def production_toggle_validation(self) -> bool:
        ...
        if current == "validee":
            ...
            self.status_var.set("Page déjà validée — modifiez-la pour la repasser en cours.")
            return False
        else:
            ...
            # Une exception de validation ne doit jamais devenir un raccourci.
            # Le gabarit a déjà été contrôlé ci-dessus ; ici l'utilisateur doit
            # confirmer explicitement s'il conserve l'exception ou la retire.
    ...
    def set_work_mode(self, mode: str):
        ...
        if mode in {"gabarits", "production"}:
            ...
            if previous == "gabarits":
                ...
                # Le Canvas Structure va supprimer l'item Gabarits, mais on garde
                # volontairement le dernier PhotoImage en mémoire. Au retour dans
                # Gabarits il est réaffiché immédiatement, puis actualisé en place.
                # Cela évite le bref fond vide/reconstruction visible.
    ...
    def gabarit_copy_current_template(self) -> bool:
        """Copie la mise en page courante ; une 2P est copiée en totalité."""
        active = self._gabarit_active_item()
        if active is None:
            self.status_var.set("Aucune page à copier.")
            return False
        snapshot = self._gabarit_unit_snapshot()
        if not snapshot:
            return False
        self._gabarit_clipboard = {
            "unit_snapshot": deepcopy(snapshot),
            # Compatibilité avec les anciens presse-papiers.
            "zones": deepcopy(snapshot.get("zones") or []),
            "page_settings": deepcopy((snapshot.get("member_page_settings") or [{}])[0]),
            "source_type": self._gabarit_type_identity(active),
            "source_label": self._page_type_label(active, int(self._selected_index or 0)),
            "surface": str(snapshot.get("surface") or "single"),
        }
        if str(snapshot.get("surface")) == "spread":
            self.status_var.set("Double page copiée intégralement — choisissez une autre double page puis Appliquer.")
        else:
    ...
    def gabarit_apply_copied_template(self) -> bool:
        """Applique le modèle copié ; une double page remplace ses deux pages."""
        active = self._gabarit_active_item()
        clipboard = getattr(self, "_gabarit_clipboard", None)
        if active is None or self._selected_index is None or not isinstance(clipboard, dict):
            self.status_var.set("Copiez d'abord un gabarit.")
            return False

        snapshot = clipboard.get("unit_snapshot")
        if not isinstance(snapshot, dict):
            # Ancien presse-papiers : reconstruction minimale.
            snapshot = {
                "surface": str(clipboard.get("surface") or "single"),
                "zones": deepcopy(clipboard.get("zones") or []),
                "member_page_settings": [deepcopy(clipboard.get("page_settings") or {})],
            }

        source_spread = str(snapshot.get("surface") or "single") == "spread"
        target_spread = self._gabarit_unit_is_spread(int(self._selected_index))
        if source_spread != target_spread:
            self.status_var.set(
                "Un modèle double page s'applique à une autre double page."
                if source_spread else
                "Un modèle de page simple s'applique à une page simple."
            )
            return False
        if not self._gabarit_apply_unit_snapshot(snapshot, int(self._selected_index)):
            self.status_var.set("Impossible d'appliquer ce modèle à cette surface.")
            return False

        unit_indices = self._gabarit_active_unit_indices() or [int(self._selected_index)]
        for idx in unit_indices:
            if not (0 <= int(idx) < len(self.items)):
                continue
            item = self.items[int(idx)]
            was_done = self._gabarit_status(item) == "termine"
        ...
        self.status_var.set(
            f"Double page « {src} » appliquée intégralement — choisissez la portée."
            if source_spread else
            f"Gabarit « {src} » appliqué localement — choisissez la portée."
        )
        return True
    ...
    def _gabarit_finish_all_same_type_from_active(self) -> int:
        """Étend réellement la mise en page active aux surfaces du même type."""
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return 0
        ...
        for primary in targets:
            if not (0 <= int(primary) < len(self.items)):
                continue
            if int(primary) != active_index and not self._gabarit_apply_unit_snapshot(snapshot, int(primary)):
                continue
            unit_indices = self._gabarit_unit_indices_for_index(int(primary)) or [int(primary)]
            for idx in unit_indices:
                item = self.items[int(idx)]
    ...
    def _gabarit_unit_indices_for_index(self, index: int | None = None) -> list[int]:
        """Retourne les indices métier d'une surface Gabarits (page ou double page)."""
        units, pos = self._gabarit_unit_position(index)
        if not units:
            return []
        return [int(value) for value in units[pos].get("indices", ())]

    def _gabarit_unit_is_spread(self, index: int | None = None) -> bool:
        indices = self._gabarit_unit_indices_for_index(index)
        if len(indices) == 2:
            return True
        if not indices:
            return False
        try:
            return bool(self._effective_double_page_rule(self.items[indices[0]]))
        except Exception:
            return False

    def _gabarit_unit_snapshot(self, index: int | None = None) -> dict:
        """Photographie complète d'une surface, y compris les deux pages d'une 2P.

        Sur une paire 2P, les zones historiques éventuellement stockées sur la
        seconde page sont réintégrées dans la composition commune et affectées à
        la page droite. Les zones modernes gardent leur cible gauche/double/droite.
        """
        indices = self._gabarit_unit_indices_for_index(index)
        if not indices:
            return {}
        spread = self._gabarit_unit_is_spread(indices[0])
        primary_index = int(indices[0])
        zones: list[dict] = []

        member_indices = indices if len(indices) == 2 else [primary_index]
        for member_pos, member_index in enumerate(member_indices):
            if not (0 <= member_index < len(self.items)):
                continue
            item = self.items[member_index]
            raw_zones = item.get("gabarit_zones")
            if not isinstance(raw_zones, list):
                continue
            for source in raw_zones:
                if not isinstance(source, dict):
                    continue
                clone = deepcopy(source)
                if spread:
                    raw_position = str(clone.get("spread_position") or "").strip().lower()
                    if raw_position not in {"left", "center", "right"}:
                        if len(member_indices) == 2:
                            clone["spread_position"] = "left" if member_pos == 0 else "right"
                        else:
                            # Une ancienne zone d'une vraie surface 2P occupait le
                            # référentiel complet : on préserve ce rendu.
                            clone["spread_position"] = "center"
                zones.append(clone)
        ...
        for member_index in member_indices:
            if not (0 <= member_index < len(self.items)):
                continue
            item = self.items[member_index]
            raw_settings = item.get("gabarit_page_settings")
            settings.append(deepcopy(raw_settings) if isinstance(raw_settings, dict) else {})
            member_types.append(self._gabarit_type_identity(item))

        return {
            "surface": "spread" if spread else "single",
            "zones": self._gabarit_template_zone_snapshot(zones),
            "member_page_settings": settings,
            "member_types": member_types,
            "primary_type": self._gabarit_type_identity(self.items[primary_index]),
        }

    def _gabarit_apply_unit_snapshot(self, snapshot: dict, target_index: int) -> bool:
        """Remplace intégralement une page ou une double page par une photographie."""
        if not isinstance(snapshot, dict):
            return False
        indices = self._gabarit_unit_indices_for_index(target_index)
        if not indices:
            return False
        target_spread = self._gabarit_unit_is_spread(indices[0])
        source_spread = str(snapshot.get("surface") or "single") == "spread"
        if target_spread != source_spread:
            return False

        primary_index = int(indices[0])
        primary = self.items[primary_index]
        primary["gabarit_zones"] = self._gabarit_template_zones_for_page(snapshot.get("zones"))

        # Une paire 2P est éditée comme UNE surface : toutes ses zones résident
        # sur la page primaire et portent left/center/right. La seconde page ne
        # conserve donc aucun ancien gabarit invisible après remplacement.
        for member_index in indices[1:]:
            self.items[int(member_index)]["gabarit_zones"] = []

        source_settings = snapshot.get("member_page_settings")
        source_settings = source_settings if isinstance(source_settings, list) else []
        for pos, member_index in enumerate(indices):
            item = self.items[int(member_index)]
            if source_settings:
                source = source_settings[pos] if pos < len(source_settings) else source_settings[-1]
                if isinstance(source, dict) and source:
                    item["gabarit_page_settings"] = deepcopy(source)
                else:
    ...
    def _gabarit_same_type_target_indices(self, *, include_exceptions: bool = True) -> list[int]:
        """Retourne les SURFACES du même type et de même nature simple/2P.

        Une double page compte comme une seule cible. Une composition 2P ne peut
        jamais être injectée dans une page simple du même type, ni l'inverse.
        """
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return []
        type_key = self._gabarit_type_identity(active)
        active_index = int(self._selected_index)
        active_spread = self._gabarit_unit_is_spread(active_index)
        if not type_key:
            return [active_index]

        targets: list[int] = []
        for unit in self._gabarit_navigation_units():
            indices = [int(value) for value in unit.get("indices", ())]
            if not indices:
                continue
            primary = int(unit.get("primary", indices[0]))
            if any(self._is_automatic_page(self.items[idx]) for idx in indices if 0 <= idx < len(self.items)):
                continue
            if self._gabarit_type_identity(self.items[primary]) != type_key:
                continue
            if bool(unit.get("double")) != bool(active_spread):
                continue
            if not include_exceptions and primary != active_index:
                if any(bool(self.items[idx].get("gabarit_local_override")) for idx in indices if 0 <= idx < len(self.items)):
                    continue
    ...
    def _gabarit_apply_template_record_to_item(self, item: dict, type_key: str, record: dict) -> None:
        ...
        settings = None
        if isinstance(record, dict):
            members = record.get("member_page_settings")
            if isinstance(members, list) and members and isinstance(members[0], dict):
                settings = members[0]
            else:
                settings = record.get("page_settings")
    ...
    def _gabarit_publish_active_template_to_type(self) -> int:
        """Publie la mise en page active à toutes les surfaces compatibles du type.

        Pour une double page, la photographie comprend les zones gauche/droite/
        double ainsi que les réglages propres aux deux pages. Les exceptions des
        autres surfaces restent protégées ; l'exception active peut, elle, devenir
        le nouveau gabarit commun — c'est précisément le sens de « Tout le type ».
        """
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return 0
        ...
        record = {
            ...
            "zones": deepcopy(snapshot.get("zones") or []),
            "page_settings": deepcopy((snapshot.get("member_page_settings") or [{}])[0]),
            "member_page_settings": deepcopy(snapshot.get("member_page_settings") or []),
            "member_types": deepcopy(snapshot.get("member_types") or []),
        }
        ...
        touched_units = 0
        for primary in sorted(set(int(v) for v in targets)):
            if not (0 <= primary < len(self.items)):
                continue
            if primary != active_index and not self._gabarit_apply_unit_snapshot(snapshot, primary):
                continue
            unit_indices = self._gabarit_unit_indices_for_index(primary) or [primary]
            for idx in unit_indices:
                item = self.items[int(idx)]
                item["gabarit_scope_last"] = "type"
                item["gabarit_scope_active"] = False
    ...
    def _gabarit_reintegrate_active_exception(self) -> bool:
        """Réapplique le gabarit commun à l'exception active, page ou 2P."""
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return False
        type_key = self._gabarit_type_identity(active)
        ...
        if not isinstance(record, dict):
            return False
        snapshot = {
            "surface": str(record.get("surface") or "single"),
            "zones": deepcopy(record.get("zones") or []),
            "member_page_settings": deepcopy(record.get("member_page_settings") or [record.get("page_settings") or {}]),
            "member_types": deepcopy(record.get("member_types") or []),
        }
        if not self._gabarit_apply_unit_snapshot(snapshot, int(self._selected_index)):
            return False
        for idx in self._gabarit_active_unit_indices() or [int(self._selected_index)]:
            ...
            item["gabarit_done"] = True
            item["gabarit_template_type"] = type_key
            item["gabarit_template_version"] = int(record.get("version", 1) or 1)
            item["gabarit_local_override"] = False
    ...
    def _gabarit_validate_scope(self, requested: str) -> bool:
        ...
        if requested == "page":
            for idx in (self._gabarit_active_unit_indices() or [int(self._selected_index)]):
                ...
                item.pop("gabarit_template_version", None)
            self._save_order()
            self.status_var.set("Gabarit validé pour cette double page" if self._gabarit_active_is_spread() else "Gabarit validé pour cette page")
        else:
            count = self._gabarit_publish_active_template_to_type()
            if count <= 0:
                self.status_var.set("Aucune surface du même type n'a pu être mise à jour.")
                return False
            noun = "double(s) page(s)" if self._gabarit_active_is_spread() else "page(s)"
            self.status_var.set(f"Gabarit étendu à {count} {noun} du même type")
    ...
    def gabarit_set_scope(self, scope: str) -> bool:
        """Portée en deux clics : le second clic exécute réellement l'extension."""
        requested = "type" if str(scope or "page").strip().lower() == "type" else "page"
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return False
        ...
        if str(getattr(self, "_gabarit_scope_pending", "")) != requested:
            ...
            if requested == "type":
                normal_targets = self._gabarit_same_type_target_indices(include_exceptions=False)
                all_targets = self._gabarit_same_type_target_indices(include_exceptions=True)
                count = max(1, len(normal_targets))
                exceptions = max(0, len(all_targets) - len(normal_targets))
    ...
    def gabarit_select_index(self, index: int, *, preserve_zoom: bool = True) -> bool:
        ...
        should_remind = bool(
            str(getattr(self, "_work_mode", "") or "") == "gabarits"
    ...
    def _gabarit_draw_context_overlay(
        ...
        *, work_left: float = 12.0, work_right: float | None = None, target=None,
        ...
        # PRODUCTION_V21 — Production reprend EXACTEMENT la ligne de contexte
        # de Gabarits : mêmes stations, mêmes codes AV/AP/R/V/2P, exceptions
        # et Fond guide. L'avancement Production est porté par la grande page.
        self._gabarit_context_hitboxes = {}
        self._gabarit_context_marker_hitboxes = {}
        target = target or self.canvas
        if work_right is None:
    ...
    def gabarit_set_selected_zone_spread_position(self, position: str) -> bool:
        ...
        if changed:
            ...
            self._emit_gabarit_page_changed()
            self._emit_gabarit_selection_changed()
            self.status_var.set(f"Zone placée sur la page {labels[position]}" if position != "center" else "Zone étendue sur toute la double page")
        return changed
    ...
    def _production_element_for_zone(self, item: dict | None, zone: dict | None) -> dict | None:
        ...
        if not slot:
            return None
        for key in ("production_elements", "content_elements", "page_elements"):
            values = item.get(key)
    ...
    def _production_load_zone_image(self, source: str):
        if Image is None:
            return None
        path = self._production_resolve_content_path(source)
        if path is None:
            return None
        key = str(path)
        cached = self._production_zone_content_cache.get(key)
        if cached is not None:
            try:
                return cached.copy()
            except Exception:
        ...
        try:
            ...
            self._production_zone_content_cache[key] = image.copy()
            # Limite simple pour éviter un cache infini sur de très gros ouvrages.
            if len(self._production_zone_content_cache) > 180:
                for old_key in list(self._production_zone_content_cache.keys())[:80]:
                    self._production_zone_content_cache.pop(old_key, None)
            return image
        except Exception:
            return None
    ...
    def _production_draw_zone_content(
        ...
        x: float, y: float, w: float, h: float, angle: float = 0.0,
    ) -> bool:
        """Dessine le contenu dans la zone, et non derrière la page.

        V2.2 introduit le lien fonctionnel indispensable à Production : le
        contenu suit la géométrie du gabarit. Le test Rosier/Pâquerette utilise
        des blocs raster séparés ; le futur import ODP pourra fournir du texte,
        des images originales ou des documents au même point d'entrée.
        """
        if str(getattr(self, "_work_mode", "") or "") != "production":
            return False
        if Image is None or not hasattr(target, "image"):
            return False
        element = self._production_element_for_zone(item, zone)
        if not isinstance(element, dict):
            return False
        ...
        if image is None:
            # Repli texte minimal pour les futurs imports structurés.
            text = str(element.get("text") or element.get("value") or "").strip()
            if not text:
                return False
            try:
                font_size = max(7, int(float(element.get("font_size_pt", 10) or 10)))
            except Exception:
                font_size = 10
            pad = max(3.0, min(10.0, min(w, h) * 0.05))
            # Le moteur typographique complet viendra ensuite ; ici on prouve
            # seulement que le texte appartient réellement à sa zone.
            target.create_text(
                x + pad, y + pad, text=text, anchor="nw",
                fill=str(element.get("color") or "#1D2724"),
                font=(str(element.get("font_family") or theme.FONT_UI), font_size),
            )
            return True
        ...
        try:
            ...
            if fit == "cover" and (rw > tw or rh > th):
                left = max(0, (rw - tw) // 2)
                top = max(0, (rh - th) // 2)
                resized = resized.crop((left, top, left + tw, top + th))
                rw, rh = resized.size
            if abs(float(angle or 0.0)) > 0.01:
                resized = resized.rotate(-float(angle), expand=True, resample=Image.Resampling.BICUBIC)
                rw, rh = resized.size
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float, *, target=None) -> None:
        ...
        for zone in zones:
            ...
            color = colors.get(kind, "#C39A4A")
            production_item = self._gabarit_active_item() if str(getattr(self, "_work_mode", "") or "") == "production" else None
            has_production_content = self._production_draw_zone_content(
                target, zone, production_item, zx1, zy1, zw, zh, angle
            )
            ...
            text_id = target.create_text(
                ...
                state=("hidden" if has_production_content else ("normal" if zw > 74 and zh > 28 else "hidden")),
            )
    ...
    def _gabarit_prepare_raster_surface(self, viewport_w: float, viewport_h: float) -> bool:
        ...
        if first:
            ...
            # Si une image Gabarits a déjà été calculée, on la remet immédiatement
            # à l'écran au lieu d'afficher un fond vierge pendant le recalcul.
            self.canvas.delete("all")
            cached = getattr(self, "_gabarit_raster_photo", None)
            cached_size = tuple(getattr(self, "_gabarit_raster_size", (0, 0)))
            if cached is not None and cached_size == size:
                photo = cached
            else:
    ...
    def _production_draw_page_state_halo(
        ...
        """Halo d'avancement nettement visible, exclusivement hors de la feuille.

        Vierge : aucun halo. En cours : orange. Validée : vert.
        Le halo est volontairement dessiné après la page et les zones afin de ne
        jamais être masqué par le fond de feuille, tout en restant à l'extérieur.
        """
        if str(getattr(self, "_work_mode", "") or "") != "production":
            return
        status = self._production_status(item if isinstance(item, dict) else None)
        if status == "vierge":
            return
        if status == "validee":
            colors = ("#315C49", "#478568", "#68C293")
        else:
            colors = ("#6A4720", "#A96E28", "#E8A342")

        # Trois anneaux externes donnent un vrai halo lisible sur le fond sombre
        # sans jamais recouvrir le contenu de la page.
        for pad, color, width in ((13.0, colors[0], 3), (8.0, colors[1], 3), (4.0, colors[2], 3)):
            target.create_rectangle(
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float, *, update_inspector: bool = True) -> None:
        ...
        # Le halo est dessiné en dernier autour de la feuille : il reste donc
        # visible même lorsque la page, son contenu ou son fond guide sont opaques.
        if str(getattr(self, "_work_mode", "") or "") == "production":
            if spread:
                _prod_indices = list(active_unit.get("indices", (active_index,)))
                _prod_left = self.items[_prod_indices[0]] if _prod_indices else active_item
                _prod_right = self.items[_prod_indices[1]] if len(_prod_indices) > 1 else active_item
                self._production_draw_page_state_halo(target, x1, y1, x1 + single_w, y2, _prod_left)
    ...
    def _structure_type_usage(self, type_key: str, data: dict | None = None) -> tuple[int, int]:
        ...
        if isinstance(double_rules, dict) and bool(double_rules.get(type_key)):
            rules += 1
        pair_rules = data.get("double_page_pair_rules", {})
        if isinstance(pair_rules, dict):
            for record in pair_rules.values():
                if isinstance(record, dict) and type_key in {
                    str(record.get("left_type") or "").strip().lower(),
                    str(record.get("right_type") or "").strip().lower(),
                }:
                    rules += 1
        return pages, rules
    ...
    def _structure_target_rule_context(self) -> dict:
        ...
        indices = self._selected_source_indices()
        pair_page_auto = False
        if code in {"AV", "AP"}:
            position = "before" if code == "AV" else "after"
            source, pair_page_auto = self._structure_page_auto_selection_source(position)
        else:
        ...
        result = {
            ...
            "can_reintegrate": False,
            "excluded": False,
        }

        if code in {"AV", "AP"}:
            position = "before" if code == "AV" else "after"
            general_target = self.structure_get_page_auto_type_rule(source_type, position)
            local_override = self._page_auto_override_value(source, position)
            effective_target = self._structure_page_auto_type(source, position)
            general_exists = bool(general_target)
            local_rule = local_override not in (None, "", "__none__") and bool(effective_target)
            local_variant = bool(local_rule and (not general_exists or str(local_override) != general_target))
            excluded = bool(general_exists and local_override == "__none__")
            result.update({
                "position": position,
                "general_target": general_target,
                "local_override": local_override,
                "effective_target": effective_target,
                "has_rule": general_exists,
                "general_exists": general_exists,
                "general_code": code,
                "local_rule": local_rule,
                "local_variant": local_variant,
                "excluded": excluded,
                # Une décision locale différente de la règle générale peut elle aussi
                # être propagée volontairement à tout le type.
                # Sur une 2P, AV/AP décrit le bloc entier. Sa portée reste locale
                # tant que la sémantique d'une extension de paire n'est pas définie.
                "pair_page_auto": pair_page_auto,
                "can_extend": bool((not pair_page_auto) and local_rule and (not general_exists or effective_target != general_target)),
                "can_scind": bool((not pair_page_auto) and general_exists and not local_variant),
                "can_remove_local": bool(local_rule and not general_exists),
                "can_reintegrate": bool(general_exists and local_override is not None),
            })
            return result

        if code in {"R", "V"}:
            side = "recto" if code == "R" else "verso"
            general_side = self.structure_get_recto_verso_type_rule(source_type)
            local_override = self._recto_verso_override_value(source)
            effective_side = self._effective_recto_verso_rule(source)
            general_exists = general_side in {"recto", "verso"}
            local_rule = local_override in {"recto", "verso"}
            local_variant = bool(local_rule and (not general_exists or local_override != general_side))
            excluded = bool(general_exists and local_override == "__none__")
            result.update({
                "side": side,
                "general_side": general_side,
                "local_override": local_override,
                "effective_side": effective_side,
                "has_rule": general_exists,
                "general_exists": general_exists,
                "general_code": "R" if general_side == "recto" else "V" if general_side == "verso" else code,
                "local_rule": local_rule,
                "local_variant": local_variant,
                "excluded": excluded,
                "can_extend": bool(local_rule and (not general_exists or effective_side != general_side)),
                "can_scind": bool(general_exists and not local_variant),
                "can_remove_local": bool(local_rule and not general_exists),
                "can_reintegrate": bool(general_exists and local_override is not None),
            })
            return result

        # Ancienne règle 2P par type : conservée uniquement pour compatibilité.
    ...
    def _structure_update_common_rule_actions(self) -> None:
        ...
        extend_btn = getattr(self, "structure_rule_extend_btn", None)
        ...
        pair_context = self._structure_pair_extension_context()
        code = str(context.get("code") or "")
        display_code = code or ("2P" if pair_context.get("active") else "")
        active = bool(display_code)
        general_exists = bool(context.get("general_exists", context.get("has_rule")))
        local_variant = bool(context.get("local_variant"))
        can_remove_local = bool(context.get("can_remove_local"))
        can_reintegrate = bool(context.get("can_reintegrate"))
        try:
            panel.configure(
            ...
            badge.configure(
                text=display_code or "—",
                bg="#6E5B32" if active else "#162A38",
                fg="#FFF0C4" if active else "#7F9099",
            )
            if extend_btn is not None:
                scoped_extend = code in {"AV", "AP", "R", "V"} and bool(context.get("can_extend"))
                scoped_scind = code in {"AV", "AP", "R", "V"} and bool(context.get("can_scind"))
                pair_action_active = bool(pair_context.get("active") and code not in {"AV", "AP", "R", "V"})
                extend_active = bool(pair_action_active or scoped_extend or scoped_scind)
                scind_mode = bool(pair_action_active and pair_context.get("extended")) or scoped_scind
                extend_btn.configure(
                    state="normal" if extend_active else "disabled",
                    text="Scinder" if scind_mode else "Étendre",
                    bg="#4A2C2A" if scind_mode else "#162A38",
                    fg="#FFF0F0" if scind_mode else "#F2F3F1",
                )

            # Une règle générale accepte une exception locale. Si la page possède
            # déjà n'importe quel override local (exclusion ou variante), le même
            # bouton devient Réintégrer.
            exception_btn.configure(
                state="normal" if general_exists else "disabled",
                text="Réintégrer" if can_reintegrate else "Exception",
            )

            # Une règle locale seule doit pouvoir être retirée. Pour une variante
            # locale posée sur une règle générale, on évite l'ambiguïté : Étendre
            # ou Réintégrer sont proposés, mais pas la suppression globale.
            can_remove_general = bool(general_exists and not local_variant)
            remove_enabled = bool(can_remove_general or can_remove_local)
            pending_key = str(getattr(self, "_structure_rule_remove_pending", "") or "")
            general_code = str(context.get("general_code") or code)
            current_key = f"{general_code}:{context.get('source_type', '')}" if can_remove_general else ""
            pending = bool(can_remove_general and pending_key == current_key)
            if can_remove_local and not general_exists:
                remove_text = "Retirer local"
            else:
                remove_text = "Confirmer" if pending else "Retirer règle"
    ...
    def structure_select_recto_verso_rule(self, side: str) -> bool:
        ...
        if source is None:
            return False
        self._structure_set_rule_target(code)
        current = self._effective_recto_verso_rule(source)
        # Si cette occurrence est déjà du côté demandé, le clic cible simplement
        # la règle pour donner accès à Étendre/Scinder/Exception sans la recréer.
        if current == side:
            self.status_var.set(f"Règle {code} ciblée  •  actions disponibles")
    ...
    def structure_select_double_page_rule(self) -> bool:
        ...
        # Compatibilité avec une ancienne règle 2P « par type » : on peut encore
        # la cibler pour la retirer, mais on n'en crée plus. La nouvelle 2P part
        # toujours de deux vraies pages voisines.
        source = self.items[indices[0]] if len(indices) == 1 else None
        if source is not None and self.structure_get_double_page_type_rule(self._type_of(source)):
            self._structure_set_rule_target("2P")
            self.status_var.set("Ancienne règle 2P par type ciblée  •  Retirer règle disponible")
            self._structure_update_double_page_visuals()
            return True

        self.status_var.set("2P : sélectionnez exactement deux pages voisines.")
        self._structure_update_double_page_visuals()
        return False
    ...
    def structure_toggle_target_rule_exception(self) -> bool:
        ...
        self._structure_rule_remove_pending = ""
        context = self._structure_target_rule_context()
        if not bool(context.get("general_exists", context.get("has_rule"))):
            return False
        code = str(context.get("code") or "")
        if code in {"AV", "AP"}:
            position = "before" if code == "AV" else "after"
            # Toute variante locale sur une règle générale se réintègre en
            # supprimant l'override ; sans override on crée une vraie exception.
            if context.get("local_override") is not None:
    ...
    def structure_remove_target_rule(self) -> bool:
        ...
        # Portée locale : depuis que AV/AP et R/V naissent localement, leur
        # suppression doit être possible sans les avoir d'abord étendus.
        if not general_exists and can_remove_local:
            self._structure_rule_remove_pending = ""
            if code in {"AV", "AP"}:
                position = "before" if code == "AV" else "after"
                ok = self.structure_set_selected_page_auto(position, "", scope="local")
            elif code in {"R", "V"}:
                ok = self.structure_remove_selected_local_recto_verso_rule()
            else:
                ok = False
            if ok:
                self.status_var.set(f"Règle locale {code} retirée de cette page.")
            self._structure_update_common_rule_actions()
            return bool(ok)

        # Une variante locale sur une règle générale a deux gestes non ambigus :
        # Étendre cette variante ou Réintégrer la règle générale. On ne supprime
        # pas silencieusement la règle de tout le type depuis cet état.
        if not general_exists or local_variant:
            self._structure_rule_remove_pending = ""
            self._structure_update_common_rule_actions()
            return False
        ...
        general_code = str(context.get("general_code") or code)
        pending_key = f"{general_code}:{source_type}"

        # Suppression d'une règle générale : confirmation en deux clics car la
        # portée dépasse la page courante.
        if str(getattr(self, "_structure_rule_remove_pending", "") or "") != pending_key:
            self._structure_rule_remove_pending = pending_key
            self._structure_update_common_rule_actions()
            self.status_var.set(
                f"Retirer {general_code} pour tous les {source_label}  •  cliquez une seconde fois pour confirmer"
            )
            return False
        ...
        if ok:
            self.status_var.set(f"Règle {general_code} retirée pour tous les {source_label}.")
        self._structure_update_common_rule_actions()
        return bool(ok)
    ...
    def structure_double_page_source_allowed(self, item: dict | None) -> bool:
        if not isinstance(item, dict):
            return False
        return (
            not self._is_locked_page(item)
            and not self._is_automatic_page(item)
            and not self._double_page_pair_id(item)
            and bool(self._type_of(item))
        )
    ...
    def _clear_double_page_pair_metadata(item: dict | None) -> None:
        ...
        for key in (
            ...
            "double_page_pair_rule_key", "double_page_pair_conflict",
        ):
            item.pop(key, None)
    ...
    @staticmethod
    def _double_page_pair_rule_key(left_type: str, right_type: str) -> str:
        left = str(left_type or "").strip().lower()
        right = str(right_type or "").strip().lower()
        return f"{left}||{right}" if left and right else ""

    @staticmethod
    def _double_page_pair_managed_rule(item: dict | None) -> str:
        return str(item.get("double_page_pair_rule_key") or "").strip() if isinstance(item, dict) else ""

    def structure_double_page_pair_rules(self) -> dict[str, dict]:
        data = self._data if isinstance(self._data, dict) else {}
        raw = data.get("double_page_pair_rules", {})
        result: dict[str, dict] = {}
        if isinstance(raw, dict):
            for key, record in raw.items():
                if not isinstance(record, dict):
                    continue
                left = str(record.get("left_type") or "").strip().lower()
                right = str(record.get("right_type") or "").strip().lower()
                canonical = self._double_page_pair_rule_key(left, right)
                if canonical:
                    result[canonical] = {"left_type": left, "right_type": right}
        return result

    def _assign_double_page_pair(self, left: dict, right: dict, *, rule_key: str = "") -> str:
        left_id = str(left.get("id") or "").strip()
        right_id = str(right.get("id") or "").strip()
        if not left_id:
            left_id = f"MAQUETTE-{uuid4().hex[:12].upper()}"
            left["id"] = left_id
        if not right_id:
            right_id = f"MAQUETTE-{uuid4().hex[:12].upper()}"
            right["id"] = right_id
        ...
        if rule_key:
            left["double_page_pair_rule_key"] = rule_key
            right["double_page_pair_rule_key"] = rule_key
        else:
            left.pop("double_page_pair_rule_key", None)
            right.pop("double_page_pair_rule_key", None)
        left.pop("double_page_pair_conflict", None)
        right.pop("double_page_pair_conflict", None)
        return pair_id

    def _apply_double_page_pair_rules(self, base_items: list[dict] | None = None) -> tuple[bool, int]:
        """Applique les règles 2P aux seules suites de types strictement identiques.

        Exemple : une règle Titre + Illustration ne touche ni Titre + Texte,
        ni Illustration + Titre. Chaque règle est ordonnée et non chevauchante.
        """
        rules = self.structure_double_page_pair_rules()
        if not rules:
            return False, 0
        ...
        for gid in group_order:
            group_pages = by_group[gid]
            pos = 0
            while pos + 1 < len(group_pages):
                left = group_pages[pos]
                right = group_pages[pos + 1]
                key = self._double_page_pair_rule_key(self._type_of(left), self._type_of(right))
                if key not in rules or any(self._is_locked_page(item) for item in (left, right)):
                    pos += 1
                    continue
                left_pair = self._double_page_pair_id(left)
                right_pair = self._double_page_pair_id(right)
                if left_pair and left_pair == right_pair:
                    if self._double_page_pair_managed_rule(left) != key or self._double_page_pair_managed_rule(right) != key:
                        left["double_page_pair_rule_key"] = key
                        right["double_page_pair_rule_key"] = key
                        changed = True
                    pos += 2
                    continue
                if left_pair or right_pair:
                    pos += 1
                    continue
                self._assign_double_page_pair(left, right, rule_key=key)
                changed = True
                created += 1
                pos += 2
        return changed, created

    def _structure_pair_extension_context(self) -> dict:
        indices = sorted(self._selected_source_indices())
        if len(indices) != 2:
            return {"active": False, "extended": False}
        left, right = self.items[indices[0]], self.items[indices[1]]
        pair_id = self._double_page_pair_id(left)
        if not pair_id or pair_id != self._double_page_pair_id(right):
            return {"active": False, "extended": False}
        members = self._valid_double_page_pair_members(pair_id)
        if not members:
            return {"active": False, "extended": False}
        ...
        key = self._double_page_pair_rule_key(self._type_of(left), self._type_of(right))
        rules = self.structure_double_page_pair_rules()
        managed = self._double_page_pair_managed_rule(left)
        extended = bool(key and key in rules and managed == key)
        return {
            "active": True,
            "extended": extended,
            "left": left,
    ...
    def structure_extend_or_scind_context_rule(self) -> bool:
        # Une vraie double page garde la priorité lorsque deux pages sont sélectionnées.
        pair_context = self._structure_pair_extension_context()
        current_code = str(getattr(self, "_structure_rule_target", "") or "").upper()
        if pair_context.get("active") and current_code not in {"AV", "AP", "R", "V"}:
            if pair_context.get("extended"):
                return self.structure_scind_extended_double_page_pair()
            return self.structure_extend_selected_double_page_pair()

        # AV/AP et R/V : la règle naît localement sur une occurrence. Étendre la
        # transforme volontairement en règle de type ; Scinder fait l'inverse en
        # conservant la règle seulement sur la page actuellement sélectionnée.
        context = self._structure_target_rule_context()
        code = str(context.get("code") or "")
        if code in {"AV", "AP"}:
            if context.get("can_scind"):
                return self.structure_scind_selected_page_auto_rule()
            if context.get("can_extend"):
                return self.structure_extend_selected_page_auto_rule()
            self.status_var.set(f"{code} : aucune association locale à étendre.")
            self._structure_update_common_rule_actions()
            return False
        if code in {"R", "V"}:
            if context.get("can_scind"):
                return self.structure_scind_selected_recto_verso_rule()
            if context.get("can_extend"):
                return self.structure_extend_selected_recto_verso_rule()
            self.status_var.set(f"{code} : aucune règle locale à étendre.")
            self._structure_update_common_rule_actions()
            return False

        self.status_var.set("Étendre : sélectionnez une règle locale AV/AP, R/V ou une double page.")
        self._structure_update_common_rule_actions()
        return False

    def structure_extend_selected_page_auto_rule(self) -> bool:
        context = self._structure_target_rule_context()
        code = str(context.get("code") or "")
        if code not in {"AV", "AP"} or not context.get("can_extend"):
            return False
        position = str(context.get("position") or "")
        target = str(context.get("effective_target") or "")
        source_type = str(context.get("source_type") or "")
        if not position or not target or not source_type:
            return False

        ok = self.structure_set_selected_page_auto(position, target, scope="type")
        if not ok:
            return False
        self._structure_set_rule_target(code)
        definition = self._structure_type_definition(source_type)
        source_label = str(definition.get("short_label") or definition.get("label") or source_type.replace("_", " ").capitalize())
        self.status_var.set(f"{code} étendue à toutes les pages {source_label}.")
        self._structure_update_common_rule_actions()
        return True

    def structure_scind_selected_page_auto_rule(self) -> bool:
        context = self._structure_target_rule_context()
        code = str(context.get("code") or "")
        if code not in {"AV", "AP"} or not context.get("can_scind"):
            return False
        position = str(context.get("position") or "")
        source_type = str(context.get("source_type") or "")
        source = context.get("source")
        if not isinstance(source, dict) or not position or not source_type:
            return False

        source_id = str(source.get("id") or "")
        selected_target = str(context.get("effective_target") or "")
        if not selected_target and not context.get("excluded"):
            selected_target = str(context.get("general_target") or "")

        ok = self.structure_remove_page_auto_type_rule(source_type, position)
        if not ok:
            return False

        # Après suppression de la règle générale, on restaure uniquement
        # l'occurrence sélectionnée comme association locale.
        source = next((item for item in self.items if str(item.get("id") or "") == source_id), None)
        if source is not None and selected_target:
            source[f"page_auto_{position}_override"] = selected_target
            source.pop(f"page_auto_{position}_type", None)
            self._sync_page_auto_for_source(source)
            self._save_order()
            source_index = next((i for i, item in enumerate(self.items) if str(item.get("id") or "") == source_id), None)
            if source_index is not None:
                self._set_single_page_selection(source_index)
                self.render()

        self._structure_set_rule_target(code)
        self.status_var.set(f"{code} scindée : seule la page sélectionnée conserve l'association.")
        self._structure_update_common_rule_actions()
        return True

    def structure_extend_selected_recto_verso_rule(self) -> bool:
        context = self._structure_target_rule_context()
        code = str(context.get("code") or "")
        if code not in {"R", "V"} or not context.get("can_extend"):
            return False
        side = str(context.get("side") or "")
        source_type = str(context.get("source_type") or "")
        if side not in {"recto", "verso"} or not source_type:
            return False
        ...
        # Étendre signifie réellement « toutes les pages du type » : on retire les
        # anciennes exceptions locales de ce type pour partir d'un état homogène.
        for item in self.items:
            if self._is_automatic_page(item) or self._type_of(item) != source_type:
                continue
            item.pop("recto_verso_override", None)
            item["structure_side"] = side
        self._data = data
        ...
        self._save_order()
        self._structure_set_rule_target(code)
        self.status_var.set(f"{code} étendue à toutes les pages du même type.")
        self.render()
        return True

    def structure_scind_selected_recto_verso_rule(self) -> bool:
        context = self._structure_target_rule_context()
        code = str(context.get("code") or "")
        if code not in {"R", "V"} or not context.get("can_scind"):
            return False
        side = str(context.get("side") or "")
        source_type = str(context.get("source_type") or "")
        source = context.get("source")
        if not isinstance(source, dict) or side not in {"recto", "verso"} or not source_type:
            return False
        source_id = str(source.get("id") or "")
        ...
        # Une fois la règle générale retirée, l'occurrence sélectionnée garde R/V.
        source = next((item for item in self.items if str(item.get("id") or "") == source_id), None)
        if source is not None:
            source["recto_verso_override"] = side
            source["structure_side"] = side
        self._sync_structural_automatic_pages()
        self._save_order()
        restored = next((i for i, item in enumerate(self.items) if str(item.get("id") or "") == source_id), None)
        if restored is not None:
            self._set_single_page_selection(restored)
        self._structure_set_rule_target(code)
        self.status_var.set(f"{code} scindée : seule la page sélectionnée conserve la règle.")
        self.render()
        return True

    def structure_extend_selected_double_page_pair(self) -> bool:
        context = self._structure_pair_extension_context()
        if not context.get("active"):
            return False
        left = context["left"]
        right = context["right"]
        left_type = str(context.get("left_type") or "")
        right_type = str(context.get("right_type") or "")
        key = str(context.get("rule_key") or "")
        if not key:
            return False

        rules = self.structure_double_page_pair_rules()
        rules[key] = {"left_type": left_type, "right_type": right_type}
        if not isinstance(self._data, dict):
            self._data = {}
        self._data["double_page_pair_rules"] = rules

        # La paire d'origine devient elle-même membre de la règle.
        left["double_page_pair_rule_key"] = key
        right["double_page_pair_rule_key"] = key
        selection_snapshot = self._structure_selection_snapshot()
        _changed, created = self._apply_double_page_pair_rules([
            item for item in self.items if not self._is_automatic_page(item)
        ])
        self._sync_structural_automatic_pages()
        self._save_order()
        ...
        self.render()
        total = sum(
            1 for _pid in {self._double_page_pair_id(item) for item in self.items}
            if _pid and any(
                self._double_page_pair_id(candidate) == _pid
                and self._double_page_pair_managed_rule(candidate) == key
                for candidate in self.items
            )
        )
        self.status_var.set(
            f"2P étendue : {left_type} + {right_type}  •  {total} double(s) page(s)"
            + (f"  •  {created} créée(s)" if created else "")
        )
        return True

    def structure_scind_extended_double_page_pair(self) -> bool:
        context = self._structure_pair_extension_context()
        if not context.get("active") or not context.get("extended"):
            return False
        key = str(context.get("rule_key") or "")
        rules = self.structure_double_page_pair_rules()
        record = rules.pop(key, None)
        if not record:
            return False
        if not isinstance(self._data, dict):
            self._data = {}
        ...
        left_type = str(record.get("left_type") or "")
        right_type = str(record.get("right_type") or "")
        ...
        for pair_id in list(pair_ids):
            ...
            if not members:
                continue
            left, right = members
            exact = self._type_of(left) == left_type and self._type_of(right) == right_type
            managed = self._double_page_pair_managed_rule(left) == key or self._double_page_pair_managed_rule(right) == key
            if exact and managed:
        ...
        self.render()
        self.status_var.set(
            f"2P scindée : {left_type} + {right_type}  •  {removed} double(s) page(s) séparée(s)"
        )
        return True
    ...
    def _selected_double_page_pair_context(self) -> dict:
        ...
        # Une paire 2P doit rester physiquement contiguë. Les règles R/V peuvent
        # être suspendues pendant l'association, mais pas une page auto AV/AP :
        # le bloc [AV + source + AP] reste indivisible. Si une auto est exigée
        # exactement entre les deux pages, la paire ne peut donc pas être créée.
        if self._structure_page_auto_type(left, "after") or self._structure_page_auto_type(right, "before"):
            return {
                "valid": False,
                "reason": "Une règle AV/AP exige une page entre ces deux pages. Créez une exception avant de les souder.",
            }
    ...
    def structure_toggle_selected_double_page_pair(self) -> bool:
        ...
        if action == "unpair":
            rule_key = self._double_page_pair_managed_rule(left)
            if rule_key and rule_key in self.structure_double_page_pair_rules():
                self.status_var.set("Cette 2P appartient à une règle étendue  •  utilisez Scinder dans le cadre Règle.")
                self._structure_update_common_rule_actions()
                return False
            self._clear_double_page_pair_metadata(left)
            self._clear_double_page_pair_metadata(right)
            message = "Double page scindée : les contraintes propres à chaque page redeviennent actives."
        else:
            self._assign_double_page_pair(left, right)
            message = "Double page créée : gauche = Verso, droite = Recto  •  Étendre est disponible."
    ...
    def _structure_update_double_page_visuals(self) -> None:
        ...
        if len(indices) == 2:
            ...
            if action == "unpair":
                pair_context = self._structure_pair_extension_context()
                extended = bool(pair_context.get("extended"))
            text = "2P ✓" if extended else ("Scinder" if action == "unpair" else "2P")
            try:
                button.configure(
                    text=text,
                    width=8 if text == "Scinder" else 5,
                    state="disabled" if extended else ("normal" if allowed else "disabled"),
                )
            except Exception:
                pass
            self._structure_update_common_rule_actions()
        ...
        source = self.items[indices[0]] if len(indices) == 1 else None
        legacy = self.structure_get_double_page_type_rule(self._type_of(source)) if source is not None else False
        try:
            button.configure(
                text="2P ✓" if legacy else "2P",
                width=5,
                state="normal" if legacy else "disabled",
            )
        except Exception:
            pass
        self._structure_update_common_rule_actions()
    ...
    def structure_recto_verso_source_allowed(self, item: dict | None) -> bool:
        if not isinstance(item, dict):
            return False
        # Une 2P impose déjà gauche=Verso / droite=Recto. Les règles R/V
        # éventuelles restent mémorisées, mais les commandes sont inactives
        # jusqu'à une future scission de la paire.
        return (
            not self._is_locked_page(item)
            and not self._is_automatic_page(item)
            and not self._double_page_pair_id(item)
    ...
    def structure_toggle_recto_verso_rule(self, side: str) -> bool:
        """Applique Recto/Verso localement à la page sélectionnée.

        Comme AV/AP, R/V naît sur l'occurrence courante. L'utilisateur choisit
        ensuite explicitement Étendre s'il veut transformer cette règle locale
        en règle commune à toutes les pages du même type.
        """
        side = str(side or "").strip().lower()
        ...
        definition = self._structure_type_definition(source_type)
        source_label = str(definition.get("short_label") or definition.get("label") or source_type.replace("_", " ").capitalize())
        self.status_var.set(f"{source_label}  •  {side.capitalize()} sur cette page  •  Étendre disponible")
        self.render()
        return True
    ...
    def _effective_recto_verso_rule(self, item: dict | None) -> str:
        if not isinstance(item, dict) or self._is_automatic_page(item):
            return ""
        override = self._recto_verso_override_value(item)
        if override in {"recto", "verso"}:
            return str(override)
        if override == "__none__":
            return ""
        return self.structure_get_recto_verso_type_rule(self._type_of(item))

    def structure_remove_selected_local_recto_verso_rule(self) -> bool:
        """Retire uniquement la règle R/V locale de la page sélectionnée."""
        indices = self._selected_source_indices()
        if len(indices) != 1:
            return False
        source = self.items[indices[0]]
        override = self._recto_verso_override_value(source)
        if override not in {"recto", "verso"}:
            return False
        source_id = str(source.get("id") or "")
        source.pop("recto_verso_override", None)
        source.pop("recto_verso_conflict", None)
        source["structure_side"] = self.structure_get_recto_verso_type_rule(self._type_of(source)) or "libre"
        self._sync_structural_automatic_pages()
        self._save_order()
        restored = next((i for i, item in enumerate(self.items) if str(item.get("id") or "") == source_id), None)
        if restored is not None:
    ...
    def _structure_update_recto_verso_visuals(self) -> None:
        ...
        try:
            ...
            pass
        current = self._effective_recto_verso_rule(source) if source is not None else ""
        general = self.structure_get_recto_verso_type_rule(self._type_of(source)) if source is not None else ""
        excluded = general in {"recto", "verso"} and self._recto_verso_override_value(source) == "__none__" if source is not None else False
        try:
    ...
    def structure_set_page_auto_type_rule(self, source_type: str, position: str, target_type: str) -> bool:
        ...
        # Une règle de type signifie réellement « toutes les pages du type ».
        # Lors d'une création comme d'un remplacement, les anciens overrides et
        # exclusions de cette position sont donc nettoyés avant application.
        self._clear_page_auto_position_state(
            source, position, data=data, keep_exclusions=False
        )
    ...
    def _sync_structural_automatic_pages(self) -> bool:
        ...
        try:
            ...
            pair_rule_changed, _pair_rule_created = self._apply_double_page_pair_rules(base_items)
            pair_sanitized = self._sanitize_double_page_pairs(base_items) or pair_rule_changed
            if not base_items:
            ...
            for gid in group_order:
                ...
                for pos, source in enumerate(pages):
                    ...
                    # Une AV/AP ne peut jamais être insérée ENTRE les deux moitiés
                    # d'une paire 2P. Si une règle est ajoutée après la création de la
                    # paire, on conserve la paire contiguë mais on la marque en conflit
                    # pour qu'une décision explicite puisse être prise dans Structure.
                    if before_type:
                        if internal_before:
                            source["double_page_pair_conflict"] = True
                            peer = self._double_page_pair_peer(source)
                            if peer is not None:
            ...
            for kind, payload in expanded:
                ...
                if double_page and rule == "recto":
                    ...
                    rule = ""
                # Une vraie 2P porte déjà sa contrainte physique : gauche = Verso,
                # droite = Recto. Les anciennes règles R/V restent stockées sur les
                # pages mais sont suspendues tant que la paire existe.
            ...
            for token in active_tokens:
                ...
                built_for_token[token] = self._prepare_structural_auto_item(
                    best,
                    roles=roles,
                    group_id=auto_groups.get(token, self._structure_fallback_content_group_id()),
                )
    ...
    def structure_set_selected_page_auto(self, position: str, target_type: str = "", scope: str = "local") -> bool:
        ...
        pair_page_auto = False
        boundary_source, pair_page_auto = self._structure_page_auto_selection_source(position)
        if pair_page_auto:
            if boundary_source is None:
                self.status_var.set("Page auto : la double page sélectionnée n'est pas exploitable.")
                return False
            # AV/AP sur une 2P vise uniquement son bord extérieur : gauche pour
            # AV, droite pour AP. On ne crée jamais de règle entre les moitiés.
            indices = [self.items.index(boundary_source)]
            if scope == "type":
                self.status_var.set("AV/AP sur une double page reste local au bloc.")
                return False
    ...
    # ------------------------------------------------------------------
    # Page auto — comportement local par défaut.
    # Une association AV/AP est d'abord attachée à la page sélectionnée.
    # L'extension à toutes les pages du même type doit rester volontaire.
    # ------------------------------------------------------------------
    ...
    def structure_page_auto_context(self) -> dict:
        ...
        general_target = self.structure_get_page_auto_type_rule(source_type, position)
        if override is not None:
            target_type = "" if str(override or "").strip() == "__none__" else str(override or "").strip()
        else:
            target_type = general_target
        target_label = ""
        if target_type:
        ...
        return {
            ...
            "has_rule": bool(general_target),
            "has_local": bool(override not in (None, "", "__none__")),
    ...
    def structure_begin_page_auto_rule(self, position: str) -> bool:
        ...
        if position not in {"before", "after"}:
            return False
        item, pair_page_auto = self._structure_page_auto_selection_source(position)
        if item is None:
            self.status_var.set("Page auto : sélectionnez une page simple ou une double page 2P dans B.")
            return False
        if not self.structure_auto_source_allowed(item):
            self.status_var.set("Cette sélection ne peut pas servir de source à une règle Page auto.")
            return False
        source_type = self._type_of(item)
        if not source_type:
        ...
        self._structure_page_auto_mode = {
            ...
            "source_label": source_label,
            "source_id": source_id,
            "double_page_block": bool(pair_page_auto),
        }
        self._structure_update_page_auto_visuals()
        side = "avant" if position == "before" else "après"
        if pair_page_auto:
            source_label = "la double page"
        current_target = self._structure_page_auto_type(item, position)
        if current_target:
            target_def = self._structure_type_definition(current_target)
            target_label = str(target_def.get("short_label") or target_def.get("label") or current_target)
            self.status_var.set(
                f"Page auto  •  cette page  •  {side} : {target_label}  •  choisissez une brique dans C pour remplacer"
            )
        else:
            self.status_var.set(
                f"Page auto  •  cette page  •  choisissez dans C la page à placer {side}"
            )
    ...
    def structure_consume_page_auto_choice(self, target_type: str) -> bool:
        ...
        # Portée locale par défaut : seule la page source reçoit AV/AP.
        # Une éventuelle règle générale du type reste intacte ; l'override local
        # prend simplement la priorité pour cette occurrence.
        source[f"page_auto_{position}_override"] = target
        source.pop(f"page_auto_{position}_type", None)
        self._sync_page_auto_for_source(source)
        self._save_order()
        ...
        self._structure_update_page_auto_visuals()
        self._structure_emit_page_auto_mode()
        self.status_var.set(f"Page auto  •  cette page  •  {side} → {target_label}")
        return True
    ...
    def _structure_change_page_type(self, index: int, page_type: str, label: str) -> bool:
        ...
        # Le changement de type ne doit pas effacer les décisions LOCALES
        # attachées à cette occurrence (AV/AP/R/V). Les règles générales de
        # l'ancien type cessent naturellement de s'appliquer une fois le type
        # changé ; les overrides locaux, eux, suivent la page.
        #
        # L'exception 2P par type reste spécifique à l'ancien type et est donc
        # retirée. Les marqueurs de conflit sont toujours recalculés.
        for key in (
            "recto_verso_conflict",
    ...
    def structure_replace_selected_page_type(self, type_key: str) -> bool:
        ...
        for item in sources:
            ...
            # conflit sont nettoyées avant recalcul.
            for key in (
                "recto_verso_conflict",
                "double_page_override", "double_page_conflict",
            ):
    ...
    def _structure_valid_local_positions(self, group_id: str) -> list[int]:
        """Retourne uniquement les frontières entre blocs structurels atomiques.

        Une insertion depuis la bibliothèque doit respecter exactement les mêmes
        invariants qu'un déplacement : elle ne peut jamais couper une 2P soudée,
        ni se glisser entre une page principale et ses pages automatiques AV/AP.
        Les positions sont exprimées dans la séquence physique du groupe car
        ``_move_page_to_group_position`` insère ensuite dans cette même séquence.
        """
        indexes = self._group_items(group_id)
        if not indexes:
            positions = [0]
        else:
            local_by_index = {index: pos for pos, index in enumerate(indexes)}
            blocks = self._page_blocks_in_group(group_id)
            # Frontière avant le premier bloc, puis après chaque bloc complet.
            positions = [0]
            for block in blocks:
                local_members = [local_by_index[index] for index in block if index in local_by_index]
                if local_members:
                    positions.append(max(local_members) + 1)
            positions = sorted(set(max(0, min(len(indexes), int(pos))) for pos in positions))
    ...
    def structure_delete_selected(self) -> bool:
        ...
        if kind == "group" and self._selected_group_id:
            ...
            # Après suppression, sélectionner une autre partie ordinaire si elle
            # existe. Ne pas basculer artificiellement sur Début/Fin.
            remaining_middle = [
                str(group.get("id", ""))
                for group in self.groups
                if str(group.get("id", "")) not in {self.START_GROUP_ID, self.END_GROUP_ID}
            ]
            self._selected_group_id = remaining_middle[0] if remaining_middle else None
            self._selected_index = None
            self._selected_page_ids.clear()
            self._structure_selection_kind = "group"
            self._save_order()
    ...
    def set_project(self, project):
        ...
        if self.items:
            # TRANSITIONS_PRECHAUFFEES_V1
            # L'espace Projet existe déjà derrière l'Accueil : on finalise sa vue
            # immédiatement, avant de le révéler, au lieu de programmer un second
            # rendu visible juste après l'ouverture du projet.
    ...
    def _ensure_minimum_structure(self, source: dict) -> tuple[dict, bool]:
        ...
        # `pages_interieures` est l'identifiant historique de la première Partie 1,
        # pas une partie structurelle obligatoire. Une Partie 1 supprimée ne doit
        # donc pas réapparaître vide à la sauvegarde. On ne recrée la partie par
        # défaut que pour une structure réellement neuve, ou si des pages de
        # contenu orphelines ont besoin d'un groupe d'accueil.
        persisted_group_ids = {
            str(raw.get("id", "")).strip()
            for raw in raw_groups
            if isinstance(raw, dict) and not bool(raw.get("deleted", False))
            and str(raw.get("id", "")).strip()
        } if isinstance(raw_groups, list) else set()
        raw_items_for_group_need = data.get("items", [])
        needs_content_group = any(
            isinstance(item, dict)
            and not self._is_automatic_page(item)
            ...
            for item in (raw_items_for_group_need if isinstance(raw_items_for_group_need, list) else [])
        )
        if not middle and (not persisted_group_ids or needs_content_group):
        ...
        content_group_ids = [
            group_id for group_id in valid_group_ids
            if group_id not in {self.START_GROUP_ID, self.END_GROUP_ID}
        ]
        fallback_content_group_id = (
            self.DEFAULT_GROUP_ID if self.DEFAULT_GROUP_ID in valid_group_set
            else (content_group_ids[0] if content_group_ids else self.DEFAULT_GROUP_ID)
        )
        ...
        for item in items:
            ...
            if self._is_cover(item) or self._is_second_cover(item):
                ...
                group_id = page_type_group.get(page_type, fallback_content_group_id)
                if group_id not in valid_group_set:
                    group_id = fallback_content_group_id
    ...
    def _save_order(self):
        ...
        if hasattr(self, "GABARIT_ALIGNMENT_TYPE_RULES_KEY"):
            data[self.GABARIT_ALIGNMENT_TYPE_RULES_KEY] = deepcopy(self._gabarit_alignment_type_rules())
        # V102 — les règles 2P étendues portent sur une PAIRE ORDONNÉE de types.
        # Elles vivent au niveau du livre et doivent survivre à toute sauvegarde.
        if isinstance(getattr(self, "_data", None), dict):
            data["double_page_pair_rules"] = deepcopy(self._data.get("double_page_pair_rules", {}))
        data, _ = self._ensure_minimum_structure(data)
        self.groups = [dict(group) for group in data.get("groups", []) if isinstance(group, dict)]
        self.items = [dict(item) for item in data.get("items", []) if isinstance(item, dict)]
        saved = self.project.save_mockup(data)
    ...
    def _structure_fallback_content_group_id(self) -> str:
        """Retourne une partie de contenu réellement présente, sans recréer Partie 1."""
        valid = [str(group.get("id", "")) for group in self.groups if isinstance(group, dict)]
        if self.DEFAULT_GROUP_ID in valid:
            return self.DEFAULT_GROUP_ID
        return next(
            (group_id for group_id in valid if group_id not in {self.START_GROUP_ID, self.END_GROUP_ID}),
    ...
    def _item_group_id(self, item: dict) -> str:
        ...
        return group_id if group_id in valid else self._structure_fallback_content_group_id()
    ...
    def _open_default_view(self):
        ...
        # plaquer toute la structure contre le bord gauche. Ces deux opérations
        # restent synchrones : l'Accueil masque encore l'espace Projet.
        self.center_selected()
        self._update_visible_part_marker()
    ...
    def _page_size_factor(self, index: int, active_group_id: str | None = None) -> float:
        """Trois tailles utiles : pages sélectionnées, page normale et page automatique rétractée.

        En multi-sélection, toutes les pages sélectionnées conservent la grande
        taille visuelle. ``_selected_index`` reste seulement la page de référence
        pour les commandes qui en ont besoin.
        """
        if not 0 <= index < len(self.items):
            return self.PAGE_SIZE_NORMAL
        item = self.items[index]
        if self._is_page_index_selected(index):
    ...
    def _schedule_render(self, _event=None):
        # TRANSITIONS_PRECHAUFFEES_V1
        # Pendant un changement de bureau, App fixe d'abord la géométrie finale
        # puis demande UN rendu synchrone. Les <Configure> intermédiaires ne
        # doivent donc pas programmer un second rendu visible 35 ms plus tard.
        if bool(getattr(self, "_transition_render_suspended", False)):
            return
    ...
    def _draw_group_header(self, group: dict, x1: float, x2: float, y1: float, y2: float):
        group_id = str(group.get("id", ""))
        # Une partie n'est visuellement « sélectionnée » que lorsqu'elle est
        # réellement la cible structurelle. Une page sélectionnée conserve son
        # contexte de partie via _selected_group_id, mais ne doit pas donner le
        # même signal visuel que la sélection de la partie entière.
        selected = (
            str(getattr(self, "_structure_selection_kind", "page") or "page") == "group"
            and group_id == self._selected_group_id
        )
    ...
    def _source_rule_tokens(self, item: dict) -> list[tuple[str, bool]]:
        """Repères cumulés de toutes les règles réellement portées par la page.

        Une règle locale et une règle étendue utilisent le même code visuel ; une
        exception à une règle générale reste visible mais barrée. Le but est que
        la page montre toujours AV + AP + R/V + 2P simultanément lorsqu'ils existent.
        """
        ...
        for position, code in (("before", "AV"), ("after", "AP")):
            general = self.structure_get_page_auto_type_rule(source_type, position)
            override = self._page_auto_override_value(item, position)
            effective = self._structure_page_auto_type(item, position)
            if effective:
                tokens.append((code, False))
            elif general and override == "__none__":
                tokens.append((code, True))

        general_side = self.structure_get_recto_verso_type_rule(source_type)
        override_side = self._recto_verso_override_value(item)
        effective_side = self._effective_recto_verso_rule(item)
        if effective_side in {"recto", "verso"}:
            tokens.append(("R" if effective_side == "recto" else "V", False))
        elif general_side in {"recto", "verso"} and override_side == "__none__":
            tokens.append(("R" if general_side == "recto" else "V", True))
    ...
    def _structure_refresh_page_auto_buttons(self) -> None:
        ...
        if len(indices) in {1, 2}:
            for position, base in (("before", "AV"), ("after", "AP")):
                source, _pair_page_auto = self._structure_page_auto_selection_source(position)
                if source is None:
                    continue
                source_type = self._type_of(source)
                general = self.structure_get_page_auto_type_rule(source_type, position) if source_type else ""
                override = self._page_auto_override_value(source, position)
                effective = self._structure_page_auto_type(source, position)

                # L'indicateur du bouton décrit l'état EFFECTIF de la page
                # sélectionnée, qu'il provienne d'une règle locale ou étendue.
                # Une exception explicite à une règle générale reste barrée.
                if effective:
                    label = f"{base} ✓"
                elif general and override == "__none__":
                    label = f"{base} —"
    ...
    def render(self):
        ...
        # Structure autorise explicitement deux natures de sélection :
        # une ou plusieurs pages, OU une partie entière. Ne jamais effacer une
        # sélection de partie simplement parce qu'aucune page n'est sélectionnée.
        # C'était la cause du clic sur une partie qui semblait ne rien faire :
        # _on_press posait bien _selected_group_id, puis render() l'annulait.
        selection_kind = str(getattr(self, "_structure_selection_kind", "page") or "page")
        if selection_kind == "group":
            valid_group_ids = {str(group.get("id", "")) for group in self.groups}
            group_selection_valid = bool(
                self._selected_group_id
                and str(self._selected_group_id) in valid_group_ids
            )
            self._selected_index = None
            self._selected_page_ids.clear()
            ...
            selection_valid = (
                ...
                and 0 <= int(self._selected_index) < len(self.items)
            )
        ...
        if self._page_focus and self._selected_index is not None:
    ...
    def _group_id_at_x(self, canvas_x: float) -> str:
        ...
        if not self._group_page_bounds:
            return self._structure_fallback_content_group_id()
        ordered = []
        for group in self.groups:
            group_id = str(group.get("id", ""))
            bounds = self._group_page_bounds.get(group_id)
    ...
    def _target_local_pos_from_x(self, canvas_x: float, group_id: str, *, dragged_index: int | None = None) -> int:
        """Position d'insertion parmi les pages sources, sans jamais couper une 2P.

        Les pages automatiques restent invisibles pour le calcul de position. Une
        double page soudée, en revanche, constitue une cible atomique : le pointeur
        peut viser avant le bloc ou après le bloc, jamais entre ses deux moitiés.
        """
        ...
        local_by_index = {index: pos for pos, index in enumerate(source_indexes)}
        consumed: set[int] = set()
        ...
        for index in source_indexes:
            ...
            item = self.items[index]
            atomic_sources = [index]
            pair_id = self._double_page_pair_id(item)
            if pair_id:
                members = [
                    member_index for member_index, _member in self._double_page_pair_members(pair_id)
                    if member_index in local_by_index and member_index not in excluded
                ]
                if len(members) == 2:
                    atomic_sources = sorted(members)

            consumed.update(atomic_sources)

            visual_indices: set[int] = set(atomic_sources)
            source_ids = {
                str(self.items[source_index].get("id") or "").strip()
                for source_index in atomic_sources
            }
            source_ids.discard("")
            if source_ids:
                for candidate_index in self._group_items(group_id):
                    if candidate_index in excluded or not self._is_automatic_page(self.items[candidate_index]):
                        continue
                    if source_ids.intersection(self._automatic_source_ids(self.items[candidate_index])):
                        visual_indices.add(candidate_index)

            boxes = [
                self._page_hitboxes[candidate_index]
                for candidate_index in visual_indices
                if candidate_index in self._page_hitboxes
            ]
            if not boxes:
                continue
            ...
            first_pos = min(local_by_index[source_index] for source_index in atomic_sources)
            after_pos = max(local_by_index[source_index] for source_index in atomic_sources) + 1

            # Une 2P n'offre que deux cibles : avant ou après son bloc complet.
            if canvas_x < (x1 + x2) / 2.0:
                return first_pos
            if len(atomic_sources) > 1 and canvas_x <= x2:
                return after_pos
    ...
    def _on_press(self, event):
        ...
        # Un clic simple sur le titre d’une partie doit sélectionner la partie,
        # exactement comme un clic sur le reste de son bandeau. Le renommage
        # reste réservé au double-clic (_on_double_click). Cela permet ensuite
        # Dupliquer / Supprimer sans provoquer l’édition du titre par erreur.
        ...
        if shift_pressed or ctrl_pressed:
            ...
            # - Ctrl+clic ajoute/retire une page isolée ;
            # - Maj+clic sélectionne toute la plage depuis l'ancre.
            # Maj est prioritaire si les deux touches sont maintenues.
            self._drag_kind = None
            self._drag_start_index = None
            self._drag_selected_page_ids.clear()
            if shift_pressed:
                self._select_page_range(source_index)
            else:
                self._toggle_page_selection(source_index)
            count = len(self._selected_source_indices())
            self.status_var.set(
