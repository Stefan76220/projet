...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V15 : répétition fidèle de la forme de référence dans C."""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=1)

        surface = tk.Frame(parent, bg=theme.PANEL)
        surface.grid(row=0, column=0, sticky="nsew")
        surface.grid_rowconfigure(0, weight=1)
        surface.grid_columnconfigure(0, weight=1)
        self._structure_v15_surface = surface

        strip = tk.Frame(surface, bg=theme.PANEL, height=62)
        strip.grid(row=0, column=0, sticky="new", padx=8, pady=(8, 0))
        strip.grid_propagate(False)
        self._structure_v15_strip = strip

        tile_path = (
            PROJECT_ROOT / "assets" / "gui_v3" / "structure" /
            "structure_tile_reference_v15.png"
        )
        try:
            with Image.open(tile_path) as src:
                tile_image = src.convert("RGBA")
                self._structure_v15_tile_photo = ImageTk.PhotoImage(tile_image)
        except Exception:
            self._structure_v15_tile_photo = None

        state = {"count": 0, "job": None}

        def rebuild_tiles():
            state["job"] = None
            photo = self._structure_v15_tile_photo
            if photo is None:
                return
            width = max(1, strip.winfo_width())
            tile_w = max(1, int(photo.width()))
            min_gap = 8
            count = max(1, int((width - 8 + min_gap) // (tile_w + min_gap)))
            if count == state["count"] and strip.winfo_children():
                return
            state["count"] = count
            for child in strip.winfo_children():
                child.destroy()
            for col in range(count):
                strip.grid_columnconfigure(col, weight=1, uniform="structure_v15_tiles")
                zone = tk.Label(
                    strip, image=photo, bg=theme.PANEL, bd=0,
                    highlightthickness=0, cursor="hand2",
                )
                zone.grid(row=0, column=col, sticky="n", padx=3, pady=0)

        def schedule_rebuild(_event=None):
            if state["job"] is not None:
                try:
                    strip.after_cancel(state["job"])
                except Exception:
                    pass
            state["job"] = strip.after(40, rebuild_tiles)

        strip.bind("<Configure>", schedule_rebuild)
        strip.after_idle(rebuild_tiles)
