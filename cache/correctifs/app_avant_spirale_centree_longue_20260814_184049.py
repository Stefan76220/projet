...
class PageMaitreV2(tk.Tk):
    ...
    def _get_notebook_spiral_photo(
        ...
        *,
        ...
        # SPIRALE_REALISTE_DISCRETE_UNIFORME_V2
        # SPIRALE_VISIBLE_ET_ZONES_MAQUETTAGE_SOMBRES_V3
        ...
        if base is None:
            ...
            try:
                ...
                if bbox:
                    image = image.crop(bbox)
                # Orientation corrigée : trous vers la droite.
        ...
        # Taille uniforme sur les zones :
        # on vise une hauteur standard discrète,
        # seulement réduite si une zone est trop petite.
        standard_h = 110
        target_h = min(standard_h, max(26, int(height)))
        key = (target_h, accent, round(opacity, 2))
        photo = cache.get(key)
        if photo is not None:
            return photo

        # Largeur volontairement un peu renforcée pour garder
        # le réalisme du métal sans redevenir envahissante.
        target_w = 26
        image = base.resize((target_w, target_h), Image.Resampling.LANCZOS)
        ...
        # Teinte très légère seulement.
        alpha = image.getchannel("A")
        tint = Image.new("RGBA", image.size, accent_rgb + (0,))
        tint_alpha = alpha.point(lambda v: int(v * 0.06))
        tint.putalpha(tint_alpha)
        image = Image.alpha_composite(image, tint)

        # Opacité réduite pour plus de discrétion.
        final_alpha = image.getchannel("A").point(lambda v: int(v * opacity))
    ...
    def _draw_notebook_spiral_canvas(
        ...
        *,
        ...
        available_h = max(26, int(y2 - y1) - 10)
        ...
        # La spirale reste entièrement dans SA zone,
        # accolée au bord gauche, sans déborder vers la zone voisine.
        img_x = int(x + 2)
    ...
    def _attach_notebook_spiral_frame(self, frame, color: str) -> None:
        ...
        spring.place(x=2, y=7, width=30, relheight=1.0)

        def redraw(_event=None):
            spring.configure(bg=frame.cget("bg"))
            spring.delete("all")
            available_h = max(26, spring.winfo_height() - 18)
            photo = self._get_notebook_spiral_photo(
                available_h,
                color,
                opacity=0.72,
            ...
            if photo is None:
                return
            x = 1
            y = max(6, (spring.winfo_height() - photo.height()) // 2)
            spring.create_image(
                x,
                y,
                image=photo,
