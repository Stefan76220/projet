...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        for i,(lab,ref) in enumerate((("Marges","margins"),("Page","page"),("Fond perdu","bleed"))):
            ...
            target.create_text(x1+bw3/2,y+14.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold" if frame_ref==ref else "normal"))
        y += 35
        ...
        target.create_text(tx, y, text="ACCROCHAGE", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        off_w = min(78.0, max(66.0, inner * 0.38))
        self._gabarit_inspector_control_box(
            target, "snap_none", (right-off_w, y-5, right, y+19),
            selected=current=="none",
        )
        target.create_text(
            right-off_w/2.0, y+7.0,
            text="Désactivé" if current=="none" else "Désactiver",
            anchor="center", fill="#E7ECEA",
            font=(theme.FONT_UI, 7, "bold" if current=="none" else "normal"),
        )
        y += 27
        snaps=(("Page","snap_page"),("Marges","snap_margins"),
               ("Centre","snap_center"),("Zones","snap_zones"))
        gap=5.0; bws=(inner-gap)/2.0
        for i,(lab,key) in enumerate(snaps):
            row=i//2; col=i%2
            x1=tx+col*(bws+gap); yy=y+row*34.0
            preset=key.replace("snap_","")
            self._gabarit_inspector_control_box(
                target,key,(x1,yy,x1+bws,yy+29),selected=current==preset
            )
            target.create_text(
                x1+bws/2.0,yy+14.5,text=lab,anchor="center",
                fill="#E7ECEA",font=(theme.FONT_UI,8,"bold" if current==preset else "normal")
            )
        y += 76
        ...
        for i,(lab,key) in enumerate((("Fond","layer_back"),("−","layer_backward"),("+","layer_forward"),("Devant","layer_front"))):
            ...
            target.create_text(x1+bw4/2,y+14.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7 if lab in {"Fond","Devant"} else 9,"bold"))
        y += 40
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        ...
        for i,(lab,ref) in enumerate((("Marges","margins"),("Page","page"),("Fond perdu","bleed"))):
            ...
            target.create_text(x1+frame_bw/2.0, y+mix(13.5,15.0), text=lab, anchor="center", fill="#F1F3F2", font=(theme.FONT_UI,7,"bold" if frame_ref==ref else "normal"))
        y += mix(33.0,36.0)
        if pending_ref in {"margins","page","bleed"} and pending_ref != frame_ref:
