    ...
    def _draw_overlay_branding(self, target: tk.Canvas, width: float, height: float):
        ...
        if Image is not None and ImageTk is not None:
            try:
                ...
                target.create_image(width / 2.0 - 178, 22, image=photo, anchor="center")
            except Exception:
                pass
            try:
                with Image.open(title_path) as src:
                    ...
                    photo = ImageTk.PhotoImage(image)
                self._overlay_brand_refs.append(photo)
                target.create_image(width / 2.0 + 10, 24, image=photo, anchor="center")
            except Exception:
                pass
        ...
        target.create_text(width / 2.0, 70, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE", fill=theme.MUTED_DARK, font=(theme.FONT_UI, 8, "bold"))
