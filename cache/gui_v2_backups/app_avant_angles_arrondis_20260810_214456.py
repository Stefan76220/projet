...
class PageMaitreV2(tk.Tk):
    ...
    def _project_type_card(
        ...
        *,
        ...
        """Grand bouton de choix du type de projet."""

        base_bg = self._mix(color, "#FFFFFF", 0.92)
        hover_bg = self._mix(color, "#FFFFFF", 0.87)
        selected_bg = self._mix(color, "#FFFFFF", 0.82)
        shadow_color = self._mix(color, "#838A82", 0.72)
        ...
        shadow = tk.Frame(shell, bg=shadow_color, bd=0, highlightthickness=0)
        shadow.place(x=4, y=113, relwidth=1.0, width=-8, height=3)

        card = tk.Frame(
            shell,
            bg=base_bg,
            highlightthickness=1,
            highlightbackground=self._mix(color, "#FFFFFF", 0.34),
            bd=0,
            cursor="hand2",
        )
        card.place(x=0, y=0, relwidth=1.0, height=113)
        ...
        widgets = (card, indicator, icon_label, title_label, subtitle_label)
        ...
        def draw_indicator(selected: bool) -> None:
        ...
        def refresh() -> None:
            ...
            if selected:
                ...
                shadow.place_configure(y=114, height=2)
            else:
                paint(base_bg)
                card.configure(
                    highlightbackground=self._mix(color, "#FFFFFF", 0.34),
                ...
                shadow.place_configure(y=113, height=3)
            draw_indicator(selected)
