...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        frame_ref = self._gabarit_alignment_reference(self._gabarit_active_item())
        pending_ref = str(getattr(self, "_gabarit_alignment_pending_reference", "") or "")
        exception = frame_ref != "margins"
        target.create_text(tx, y, text="CADRE DE TRAVAIL", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right, y, text=self._gabarit_frame_reference_label(frame_ref).upper() + (" · EXCEPTION" if exception else " · LIVRE"), anchor="e", fill="#78B8B1" if exception else "#9FB0B3", font=(theme.FONT_UI, 7, "bold" if exception else "normal")); y += 17
        gap=4.0; bw3=(inner-gap*2)/3.0
        for i,(lab,ref) in enumerate((("Marges","margins"),("Page","page"),("Fond perdu","bleed"))):
            x1=tx+i*(bw3+gap)
            self._gabarit_inspector_control_box(target,f"frame_{ref}",(x1,y,x1+bw3,y+29),selected=(frame_ref==ref or pending_ref==ref))
            target.create_text(x1+bw3/2,y+14.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,6 if ref=="bleed" else 7,"bold" if frame_ref==ref else "normal"))
        y += 35
        if pending_ref in {"margins", "page", "bleed"} and pending_ref != frame_ref:
            target.create_text(tx, y, text="PORTÉE", anchor="w", fill="#9FB0B3", font=(theme.FONT_UI, 7, "bold")); y += 14
            bw=(inner-gap)/2.0
            self._gabarit_inspector_control_box(target,"align_scope_page",(tx,y,tx+bw,y+27))
            target.create_text(tx+bw/2,y+13.5,text="Cette page",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
            self._gabarit_inspector_control_box(target,"align_scope_type",(tx+bw+gap,y,right,y+27))
            target.create_text(tx+bw+gap+bw/2,y+13.5,text="Toutes du type",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold")); y += 34
        target.create_text(tx, y, text="ALIGNER", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 17
        ...
        snaps=(("Off","snap_none"),("Page","snap_page"),("Marges","snap_margins"),("Centre","snap_center"),("Zones","snap_zones"))
        gap=3.0; bws=(inner-gap*4.0)/5.0
        for i,(lab,key) in enumerate(snaps):
            x1=tx+i*(bws+gap); preset=key.replace("snap_","")
            self._gabarit_inspector_control_box(target,key,(x1,y,x1+bws,y+27),selected=current==preset)
            target.create_text(x1+bws/2,y+13.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,5 if lab=="Marges" else 6,"bold" if current==preset else "normal"))
        y += 37
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        ...
        exception = frame_ref != "margins"
        target.create_text(tx, y, text="CADRE DE TRAVAIL", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        target.create_text(inner_right, y, text=self._gabarit_frame_reference_label(frame_ref).upper() + (" · EXCEPTION" if exception else " · LIVRE"), anchor="e", fill="#78B8B1" if exception else "#9FB0B3", font=(theme.FONT_UI, 7, "bold" if exception else "normal"))
        y += mix(16.0, 18.0)
        frame_gap=4.0; frame_bw=(inner_w-frame_gap*2.0)/3.0
        for i,(lab,ref) in enumerate((("Marges","margins"),("Page","page"),("Fond perdu","bleed"))):
            x1=tx+i*(frame_bw+frame_gap)
            control_box(f"frame_{ref}", y, mix(27.0,30.0), selected=(frame_ref==ref or pending_ref==ref), x1=x1, x2=x1+frame_bw)
            target.create_text(x1+frame_bw/2.0, y+mix(13.5,15.0), text=lab, anchor="center", fill="#F1F3F2", font=(theme.FONT_UI,6 if ref=="bleed" else 7,"bold" if frame_ref==ref else "normal"))
        y += mix(33.0,36.0)
        if pending_ref in {"margins","page","bleed"} and pending_ref != frame_ref:
            scope_gap2=4.0; scope_bw2=(inner_w-scope_gap2)/2.0
            control_box("align_scope_page", y, mix(26.0,29.0), x1=tx, x2=tx+scope_bw2)
            target.create_text(tx+scope_bw2/2.0, y+mix(13.0,14.5), text="Cette page", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI,7,"bold"))
            sx2=tx+scope_bw2+scope_gap2
            control_box("align_scope_type", y, mix(26.0,29.0), x1=sx2, x2=inner_right)
            target.create_text((sx2+inner_right)/2.0, y+mix(13.0,14.5), text="Toutes du type", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI,7,"bold"))
            y += mix(32.0,35.0)
        y = separator(y)
    ...
    def gabarit_set_selected_zone_rect_mm(self, x_mm: float, y_mm: float, w_mm: float, h_mm: float) -> bool:
        ...
        constrained = self._gabarit_constrain_rect_to_reference(zone, requested, self._gabarit_alignment_reference(active))
        limited = any(abs(a-b) > 1e-9 for a,b in zip(requested, constrained))
        return self._gabarit_commit_zone_rect(zone, constrained, "Valeur limitée au cadre de travail" if limited else "Position et dimensions mises à jour")
    ...
    def gabarit_apply_alignment_reference(self, scope: str) -> bool:
        ...
        for item in touched:
            self._gabarit_constrain_item_zones_to_reference(item, self._gabarit_alignment_reference(item))
    ...
    def gabarit_align_selected_zones(self, mode: str) -> bool:
        ...
        if changed:
            ...
            self.status_var.set(f"{len(zones)} zones alignées sur {article} {label}")
        return changed
    ...
    def _gabarit_constrain_rect_to_reference(self, zone: dict, rect: tuple[float, float, float, float], reference: str | None = None) -> tuple[float, float, float, float]:
        reference = self._gabarit_normalize_frame_reference(reference if reference is not None else self._gabarit_alignment_reference())
        rx, ry, rw, rh = self._gabarit_alignment_reference_box(zone, reference)
        x, y, w, h = (float(v) for v in rect)
        min_w = min(0.04, rw); min_h = min(0.04, rh)
        w = max(min_w, min(w, rw)); h = max(min_h, min(h, rh))
        x = max(rx, min(rx + rw - w, x)); y = max(ry, min(ry + rh - h, y))
        return x, y, w, h
    ...
    def _gabarit_constrain_item_zones_to_reference(self, item: dict, reference: str | None = None) -> bool:
        ...
        if not isinstance(zones, list):
            return False
        reference = self._gabarit_normalize_frame_reference(reference if reference is not None else self._gabarit_alignment_reference(item))
        changed = False
        for zone in zones:
            if not isinstance(zone, dict):
                continue
            old = (float(zone.get("x", 0.1)), float(zone.get("y", 0.1)), float(zone.get("w", 0.3)), float(zone.get("h", 0.2)))
            new = self._gabarit_constrain_rect_to_reference(zone, old, reference)
            if any(abs(a-b) > 1e-9 for a,b in zip(old,new)):
                zone.update({"x":new[0],"y":new[1],"w":new[2],"h":new[3]}); changed = True
        return changed
    ...
    def _gabarit_constrain_active_zones_to_frame(self, *, save: bool = False) -> bool:
        ...
        if not isinstance(item, dict):
            return False
        changed = self._gabarit_constrain_item_zones_to_reference(item, self._gabarit_alignment_reference(item))
        if changed and save:
            self._save_order()
        return changed
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            if state["mode"] == "resize":
                min_w = min(0.05, frame_w); min_h = min(0.05, frame_h)
                max_w = max(min_w, frame_x + frame_w - ox); max_h = max(min_h, frame_y + frame_h - oy)
                nw = max(min_w, min(max_w, ow + dx)); nh = max(min_h, min(max_h, oh + dy))
                ...
                nw = max(min_w, min(max_w, right - ox)); nh = max(min_h, min(max_h, bottom - oy))
                zone.update({"x": ox, "y": oy, "w": nw, "h": nh})
            else:
                min_x = frame_x; max_x = frame_x + frame_w - ow
                min_y = frame_y; max_y = frame_y + frame_h - oh
                nx = max(min_x, min(max_x, ox + dx)); ny = max(min_y, min(max_y, oy + dy))
                ...
                nx = max(min_x, min(max_x, nx)); ny = max(min_y, min(max_y, ny))
