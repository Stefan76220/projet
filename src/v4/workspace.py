...
class WorkspaceSessionV4:
    ...
    def __init__(
        ...
        *,
        ...
        # Migration/garde-fou des premiers états V4 : si une ancienne version
        # a laissé une page avant la 1re ou après la 4e, on conserve cette page
        # mais on la replace à l'intérieur avant de valider la session.
        if project.book is not None:
            from src.v4.structure_covers import (
                normalize_cover_structure,
                prepare_inside_cover_confirmation,
            )
            prepare_inside_cover_confirmation(project.book)
            normalize_cover_structure(project.book)
        ...
        # Raccord léger vers l'interface : le noyau reste indépendant,
        # mais peut signaler immédiatement qu'Annuler/Rétablir a changé.
    ...
    def refresh_context(
        ...
        """
        R?concilie explicitement le contexte transitoire
        apr?s une mutation Projet qui ne rel?ve pas de Undo/Redo.

        Utilis? notamment apr?s la cr?ation initiale du Livre
        depuis une proposition d'Analyse.
        """
    ...
    def set_history_change_callback(
        ...
        """Branche/débranche le rafraîchissement visuel Undo/Redo."""
    ...
    def _notify_history_change(
        ...
        callback = self.on_history_change
        ...
        try:
            ...
            # Une interface fermée ne doit jamais casser une transaction.
            pass
