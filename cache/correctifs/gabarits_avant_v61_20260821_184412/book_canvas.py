...
# TOMELINEA_GABARITS_PALETTE_VISUELLE_V60
...
class BookCanvas(tk.Frame):
    ...
    GABARIT_INSPECTOR_BLOCK_ORDER_KEY = "gabarit_inspector_block_order"
    GABARIT_INSPECTOR_DEFAULT_BLOCK_ORDER = (
        "geometry", "allowed", "align", "snap", "layer", "actions",
    )
        ...
        if isinstance(preview, list) and preview:
            ...
            if isinstance(getattr(self, "_data", None), dict):
                ...
                if isinstance(saved, list):
                    raw = saved
        clean = [str(v) for v in raw if str(v) in default]
    ...
    def _gabarit_inspector_save_block_order(self, order) -> None:
        clean = [v for v in list(order or ()) if v in self.GABARIT_INSPECTOR_DEFAULT_BLOCK_ORDER]
    ...
    def _gabarit_inspector_draw_icon(self, target, icon: str, box, *, selected=False, danger=False, disabled=False, hovered=False):
        ...
        if icon.startswith("snap_"):
            if icon == "snap_margins":
                px1,py1,px2,py2 = self._gabarit_inspector_draw_page_shape(target,cx-3,cy-1,15,21,outline=fg)
                target.create_rectangle(px1+3,py1+4,px2-3,py2-4,outline=teal,width=1,dash=(2,2))
                self._gabarit_inspector_draw_magnet(target,cx+8,cy+7,11,teal)
            elif icon == "snap_page":
                self._gabarit_inspector_draw_page_shape(target,cx-3,cy-1,15,21,outline=blue)
                self._gabarit_inspector_draw_magnet(target,cx+8,cy+7,11,blue)
            elif icon == "snap_center":
                target.create_line(cx-9,cy,cx+5,cy,fill=teal,width=1)
                target.create_line(cx-2,cy-7,cx-2,cy+7,fill=teal,width=1)
                target.create_oval(cx-4,cy-2,cx,cy+2,outline=fg,width=1)
                self._gabarit_inspector_draw_magnet(target,cx+8,cy+7,11,teal)
            elif icon == "snap_zones":
                target.create_rectangle(cx-10,cy-7,cx+1,cy+5,outline=fg,width=1)
                target.create_rectangle(cx-5,cy-3,cx+6,cy+9,outline=teal,width=1)
                self._gabarit_inspector_draw_magnet(target,cx+9,cy+7,10,teal)
            else:  # none
                self._gabarit_inspector_draw_magnet(target,cx,cy,17,"#819198")
    ...
    def _gabarit_inspector_draw_hover_tooltip(self, target, width: float, height: float) -> None:
        key = str(getattr(self, "_gabarit_inspector_hover", "") or "")
        text = str(getattr(self, "_gabarit_inspector_tooltips", {}).get(key) or "")
        box = getattr(self, "_gabarit_inspector_hitboxes", {}).get(key)
        if not text or not box or key.startswith("geom_"):
            return
        tip_w = min(178.0, max(138.0, width - 24.0))
        tip_h = 42.0
        x2 = width - 10.0
        x1 = x2 - tip_w
        y1 = float(box[3]) + 5.0
        if y1 + tip_h > height - 8.0:
            y1 = max(8.0, float(box[1]) - tip_h - 5.0)
        y2 = y1 + tip_h
        target.create_rectangle(x1,y1,x2,y2,fill="#111A20",outline="#58727A",width=1)
        target.create_text(x1+8,y1+7,text=text,anchor="nw",fill="#DDE5E6",font=(theme.FONT_UI,7),width=tip_w-16)
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        """Palette visuelle contextuelle d'une zone sélectionnée.

        Les blocs validés sont compacts, iconographiques et réordonnables par leur
        en-tête. Les clés d'action historiques sont conservées pour ne pas changer
        le comportement métier pendant cette passe visuelle.
        """
        target.create_line(0, 8, 0, max(8, height-8), fill="#465A63", width=1)
        tx = 14.0; right = width - 11.0; inner = max(100.0, right-tx)
        ...
        y = 17.0
        target.create_text(tx, y, text=kind, anchor="w", fill="#66BFB7", font=(theme.FONT_UI, 9, "bold"))
        target.create_text(right, y, text="sélection active", anchor="e", fill="#899DA2", font=(theme.FONT_UI, 7))
        y += 23.0
        target.create_line(tx, y, right, y, fill="#3D4D55", width=1)
        y += 10.0

        def render_geometry(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target, "geometry", "POSITION & DIMENSIONS", tx, right, y0)
            gap = 4.0; bw = (inner-gap)/2.0
            fields = (("X","x","geom_x"),("Y","y","geom_y"),("L","w","geom_w"),("H","h","geom_h"))
            for i,(lab,field,key) in enumerate(fields):
                row,col = divmod(i,2)
                x1=tx+col*(bw+gap); yy=y0+row*31.0
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+27))
                target.create_text(x1+7,yy+13.5,text=lab,anchor="w",fill="#81959B",font=(theme.FONT_UI,7,"bold"))
                target.create_text(x1+bw-7,yy+13.5,text=f"{float(rect.get(field,0)):g} mm",anchor="e",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
            y0 += 62.0
            if bool(info.get("spread")):
                target.create_text(tx,y0,text="2P",anchor="w",fill="#81959B",font=(theme.FONT_UI,7,"bold"))
                ...
                for i,(lab,key,pos) in enumerate((("G","spread_left","left"),("C","spread_center","center"),("D","spread_right","right"))):
                    x1=tx+18+i*(sw+4)
                    self._gabarit_inspector_control_box(target,key,(x1,y0-7,x1+sw,y0+18),selected=current==pos)
                    target.create_text(x1+sw/2,y0+5.5,text=lab,anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
                y0 += 25.0
            return y0 + 7.0

        def render_allowed(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target, "allowed", "ZONE AUTORISÉE", tx, right, y0)
            active = self._gabarit_active_item()
            current = self._gabarit_alignment_reference(active)
            ...
            target.create_text(right,y0-17,text=f"{state} · {suffix}",anchor="e",fill="#6F8E93" if current==book_ref else "#D29A77",font=(theme.FONT_UI,7))

            # Le cadre actif n'est pas un bouton : seuls les deux changements utiles sont proposés.
            refs=[r for r in ("margins","page","bleed") if r != current]
            gap=7.0; bw=(inner-gap)/2.0
            tips={
                "margins":"Limiter la zone à l’intérieur des marges",
                "page":"Autoriser la zone jusqu’au bord de la page",
                "bleed":"Autoriser la zone jusqu’au fond perdu",
            }
            ...
            for i,ref in enumerate(refs[:2]):
                ...
                self._gabarit_inspector_icon_button(target,f"frame_{ref}",(x1,y0,x1+bw,y0+42),icons[ref],selected=pending==ref,tooltip=tip)
            y0 += 48.0
            if pending in {"margins","page","bleed"} and pending != current:
                target.create_text(tx,y0,text="APPLIQUER",anchor="w",fill="#83989D",font=(theme.FONT_UI,7,"bold"))
                y0 += 13.0
                bw2=(inner-5.0)/2.0
                self._gabarit_inspector_control_box(target,"align_scope_page",(tx,y0,tx+bw2,y0+25))
                target.create_text(tx+bw2/2,y0+12.5,text="Cette page",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
                x2=tx+bw2+5.0
                self._gabarit_inspector_control_box(target,"align_scope_type",(x2,y0,right,y0+25))
                target.create_text(x2+bw2/2,y0+12.5,text="Toutes du type",anchor="center",fill="#D8E0E1",font=(theme.FONT_UI,7,"bold"))
                y0 += 31.0
            return y0 + 5.0

        def render_align(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target,"align","ALIGNER",tx,right,y0)
            actions=(
                ("align_left","align_left","Aligner à gauche"),
                ("align_center","align_center","Centrer horizontalement"),
                ("align_right","align_right","Aligner à droite"),
                ("align_top","align_top","Aligner en haut"),
                ("align_middle","align_middle","Centrer verticalement"),
                ("align_bottom","align_bottom","Aligner en bas"),
            )
            gap=5.0; bw=(inner-gap*2.0)/3.0
            for i,(key,icon,tip) in enumerate(actions):
                row,col=divmod(i,3); x1=tx+col*(bw+gap); yy=y0+row*42.0
                self._gabarit_inspector_icon_button(target,key,(x1,yy,x1+bw,yy+37),icon,tooltip=tip)
            return y0 + 84.0 + 5.0

        def render_snap(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target,"snap","ACCROCHAGE",tx,right,y0)
            current=self.gabarit_snap_preset() if self.gabarit_snap_enabled() else "none"
            items=(
                ("snap_margins","snap_margins","margins","Accrocher aux marges"),
                ("snap_page","snap_page","page","Accrocher aux bords de page"),
                ("snap_center","snap_center","center","Accrocher aux axes centraux"),
                ("snap_zones","snap_zones","zones","Accrocher aux autres zones"),
                ("snap_none","snap_none","none","Désactiver l’accrochage"),
            )
            gap=5.0; bw=(inner-gap*2.0)/3.0
            for i,(key,icon,preset,tip) in enumerate(items):
                row,col=divmod(i,3); x1=tx+col*(bw+gap); yy=y0+row*42.0
                self._gabarit_inspector_icon_button(target,key,(x1,yy,x1+bw,yy+37),icon,selected=current==preset,tooltip=tip)
            return y0 + 84.0 + 5.0

        def render_layer(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target,"layer","SUPERPOSITION",tx,right,y0)
            items=(
                ("layer_back","layer_back","Placer tout derrière"),
                ("layer_backward","layer_backward","Reculer d’un plan"),
                ("layer_forward","layer_forward","Avancer d’un plan"),
                ("layer_front","layer_front","Placer tout devant"),
            )
            gap=4.0; bw=(inner-gap*3.0)/4.0
            for i,(key,icon,tip) in enumerate(items):
                x1=tx+i*(bw+gap)
                self._gabarit_inspector_icon_button(target,key,(x1,y0,x1+bw,y0+39),icon,tooltip=tip)
            return y0 + 46.0

        def render_actions(y0: float) -> float:
            y0 = self._gabarit_inspector_block_header(target,"actions","ACTIONS",tx,right,y0)
            locked=bool(info.get("locked",False))
            gap=6.0; bw=(inner-gap*2.0)/3.0
            items=(
                ("zone_lock","unlock" if locked else "lock",locked,"Déverrouiller la zone" if locked else "Verrouiller la zone",False),
                ("zone_duplicate","duplicate",False,"Dupliquer la zone",False),
                ("zone_delete","delete",False,"Supprimer la zone",True),
            )
            for i,(key,icon,selected,tip,danger) in enumerate(items):
                x1=tx+i*(bw+gap)
                self._gabarit_inspector_icon_button(target,key,(x1,y0,x1+bw,y0+41),icon,selected=selected,danger=danger,tooltip=tip)
            return y0 + 48.0

        renderers={
            "geometry":render_geometry,
            "allowed":render_allowed,
            "align":render_align,
            "snap":render_snap,
            "layer":render_layer,
        ...
        for idx,name in enumerate(order):
            start_y=y
            y=renderers[name](y)
            self._gabarit_inspector_block_bounds[name]=(start_y,y)
            if idx < len(order)-1:
                target.create_line(tx,y,right,y,fill="#34434B",width=1)
                y += 8.0

        # Portée / Modèle ne sont pas encore redessinés : leur fonctionnement reste intact
        # pour permettre un test isolé de la palette validée.
        self._render_gabarit_finalize_controls(target,tx,right,y+2.0)
    ...
    def _render_gabarit_multi_inspector(self, target, width: float, height: float, infos: list[dict]) -> None:
        ...
        gap=5.0; bw3=(inner-gap)/2.0
        for i,(lab,ref) in enumerate((("Marges","margins"),("Page","page"))):
            x1=tx+i*(bw3+gap)
        ...
        y+=35
        if pending_ref in {"margins","page"} and pending_ref != frame_ref:
            bw=(inner-gap)/2.0
        ...
        y += 44
        self._render_gabarit_finalize_controls(target, tx, right, y)
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        """Inspecteur Gabarits : seulement l'action utile au moment utile.

        Sans sélection, le panneau suit l'ordre de travail validé : page active,
        accès unique aux réglages généraux du livre, cadre de travail, puis création.
        Les réglages eux-mêmes restent centralisés dans leur fenêtre dédiée.
        Dès qu'une zone est sélectionnée, il bascule vers l'inspecteur contextuel.
        """
        if str(getattr(self, "_work_mode", "")) != "gabarits":
            return
        target = getattr(self, "gabarit_inspector_canvas", None)
        ...
        # 1 — Identité. Le statut reste un simple point : aucun bloc redondant.
        target.create_text(tx, y, text="PAGE ACTIVE", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        status = self._gabarit_status(active_item)
        status_color = {"termine":"#79B49C", "en_cours":"#C39A4A", "non_commence":"#8A969B"}[status]
        target.create_oval(right-12, y-6, right, y+6, fill=status_color, outline="")
        if self._gabarit_is_local_exception(active_item, int(self._selected_index)):
            target.create_text(right-20, y, text="EXCEPTION", anchor="e", fill="#D7DEDC", font=(theme.FONT_UI, 7, "bold"))
        y += 20.0
        title_id = target.create_text(
            tx, y, text=f"{number} — {active_type}", anchor="nw",
            fill="#F0F2F0", font=(theme.FONT_TITLE, 10, "bold"), width=inner,
        )
        bbox = target.bbox(title_id)
        y = float((bbox[3] if bbox else y + 20.0) + 5.0)
        target.create_text(tx, y, text=part_text, anchor="nw", fill="#BFC9C7", font=(theme.FONT_UI, 8), width=inner)
        y += 26.0
        target.create_line(tx, y, right, y, fill="#40525B", width=1)
        y += 13.0

        # 2 — Accès unique aux réglages généraux du livre.
        # Les valeurs ne sont pas dupliquées dans Gabarits : ce bouton ouvre
        # simplement la fenêtre générale qui reste leur seul lieu de modification.
        target.create_text(tx, y, text="RÉGLAGES DU LIVRE", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        y += 18.0
        self._gabarit_inspector_control_box(target, "edit_book_settings", (tx, y, right, y+30.0))
        target.create_text(
            tx+10.0, y+15.0, text="Ouvrir les réglages", anchor="w",
            fill="#E7ECEA", font=(theme.FONT_UI, 8, "bold"),
        )
        target.create_text(
            right-9.0, y+15.0, text="›", anchor="e",
        ...
        # 3 — Lancement du travail : les réglages généraux restent secondaires.
        # Le cadre de travail général se règle désormais uniquement dans
        # « Réglages du livre » afin d'éviter tout doublon à l'ouverture.
        self._render_gabarit_create_row(target, tx, right, y)
    ...
    def _gabarit_inspector_motion(self, event):
        ...
        try:
            cursor = "sb_v_double_arrow" if key.startswith("dragblock_") else ("hand2" if key else "arrow")
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float, *, target=None) -> None:
        ...
        for zone in zones:
            ...
            rect_id = target.create_polygon(
                *flat, fill="", outline=color, width=3 if selected else 1,
            )
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            if state["mode"] == "rotate":
                ...
                if rect_inside_frame((ox, oy, ow, oh), new_angle):
                    zone["rotation"] = round(new_angle, 2)
                elif new_angle != free_angle and rect_inside_frame((ox, oy, ow, oh), free_angle):
                    # Si l'angle magnétique ferait sortir la zone de son cadre,
                    # on conserve la rotation libre plutôt que de bloquer le geste.
                    state.pop("rotation_snap", None)
                    zone["rotation"] = round(free_angle, 2)
                self._gabarit_refresh_raster_only()
                return "break"
            ...
            if state["mode"] == "resize":
                ...
                nx, ny, nw, nh = limit_from_start((nx, ny, nw, nh), angle)
                ...
                nx, ny, _, _ = limit_from_start((nx, ny, ow, oh), self._gabarit_zone_rotation(zone))
