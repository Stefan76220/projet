...
class MockupView:
    ...
    def show(self) -> None:
        preparing = bool(
            getattr(self, "_prepare_hidden_mode", False)
        )
        """Affiche le Maquettage sans reconstruire l'écran s'il existe déjà."""
        root = self._root
        if root is not None:
            try:
        ...
        # Le Centre reste construit derrière le Maquettage.
        # Le bureau est superposé et réutilisé à chaque retour.
        if not preparing:
            self._root.place(
                x=0,
                y=0,
        ...
        self._book_overview_frame = self._create_book_overview_panel(self._root)
        self._book_overview_frame.grid(
            row=2,
            column=0,
            sticky="ew",
            padx=12,
        ...
        self._sequence_panel_frame = self._create_sequence_panel(self._root)
        self._sequence_panel_frame.grid(
            row=3,
            column=0,
            sticky="nsew",
            padx=12,
    ...
    def prepare_hidden(self) -> None:
        """
        Construit entièrement le Maquettage sans l'afficher.

        Tant que _prepare_hidden_mode est actif, la racine n'est pas
        attachée au parent avec place/grid/pack.
        """
        root = getattr(self, "_root", None)
        if root is not None:
            try:
                if root.winfo_exists() and bool(
                    getattr(self, "_mockup_ready", False)
                ):
                    return
            except tk.TclError:
                self._root = None

        self._mockup_ready = False
        self._prepare_hidden_mode = True
        try:
            self.show()
            root = getattr(self, "_root", None)
            if root is not None and root.winfo_exists():
                root.update_idletasks()
                self._mockup_ready = True
        finally:
            self._prepare_hidden_mode = False

    def hide(self) -> None:
        """Replace le Maquettage derrière le Centre sans le détruire."""
    ...
    def _create_book_overview_panel(self, parent) -> ctk.CTkFrame:
        """Vue globale ; les widgets sont créés une fois puis seulement configurés."""
        shell = ctk.CTkFrame(
            parent,
            height=88,
        ...
        self._overview_station_buttons = {}
        ...
        for index, group in enumerate(active_groups):
            ...
            self._overview_station_buttons[group_id] = {
                "dot": dot,
                "label": label,
                "accent": accent,
            }
    ...
    def _canvas_group_page_count(self, group_id: str) -> int:
        """Nombre de pages prévues dans un groupe."""
        total = 0
        for item in self._items():
            ...
            if str(definition.get("group", "")) != group_id:
                continue
            try:
                total += max(1, int(item.get("count", 1) or 1))
            except (TypeError, ValueError):
                total += 1
        return total

    def _draw_canvas_group_card(
        self,
        canvas,
        group,
        x0: int,
        ...
        height: int = 64,
        *,
        tag_prefix: str = "group",
    ) -> tuple[int, int, int, int]:
        """Dessine une carte de groupe sans créer de widget CustomTkinter."""
        group_id = str(group.get("id", ""))
        title = str(group.get("title", "Groupe"))
        symbol = str(group.get("symbol", "▦"))
        ...
        fill = self._mix_color_with_white(accent, 0.92)
        title_fill = self._mix_color_with_white(accent, 0.82)
        border = accent if selected else self._mix_color_with_white(
            accent,
            0.62,
        )
        ...
        page_count = self._canvas_group_page_count(group_id)
        canvas.create_text(
            (x0 + x1) / 2,
            y0 + 43,
    ...
    def _draw_canvas_fixed_group(self, canvas, group) -> None:
        canvas.delete("all")
        self._draw_canvas_group_card(
            canvas,
            group,
            2,
            6,
    ...
    def _draw_canvas_ribbon(self) -> None:
        """Redessine seulement les formes du bandeau central."""
        canvas = getattr(self, "_canvas_ribbon_canvas", None)
        if canvas is None:
            return
        try:
        ...
        middle_groups = [
            ...
            for group in self._groups()
            if (
                not bool(group.get("deleted", False))
                and str(group.get("id", ""))
                not in {"debut_livre", "fin_livre"}
            )
        ]
        ...
        for group in middle_groups:
            group_id = str(group.get("id", ""))
            boxes[group_id] = self._draw_canvas_group_card(
                canvas,
                group,
                x,
                y,
        ...
        self._canvas_ribbon_boxes = boxes
        self._canvas_ribbon_middle_ids = [
            str(group.get("id", ""))
            for group in middle_groups
        ]
        canvas.configure(
            scrollregion=(0, 0, max(x, canvas.winfo_width()), 78)
        )

    def _refresh_canvas_ribbon(self) -> None:
        """Actualise le bandeau sans détruire aucun widget."""
        self._draw_canvas_ribbon()
        ...
        if start is not None:
            canvas = getattr(self, "_canvas_ribbon_start", None)
            if canvas is not None:
                self._draw_canvas_fixed_group(canvas, start)

        if end is not None:
            canvas = getattr(self, "_canvas_ribbon_end", None)
            if canvas is not None:
                self._draw_canvas_fixed_group(canvas, end)

    def _canvas_ribbon_wheel(self, event):
        canvas = getattr(self, "_canvas_ribbon_canvas", None)
        if canvas is None:
            return "break"
    ...
    def _canvas_ribbon_group_at(self, canvas_x: float, canvas_y: float):
        for group_id, box in self._canvas_ribbon_boxes.items():
            x0, y0, x1, y1 = box
            if x0 <= canvas_x <= x1 and y0 <= canvas_y <= y1:
                return group_id
        return None

    def _canvas_ribbon_press(self, event) -> None:
        canvas = self._canvas_ribbon_canvas
        x = canvas.canvasx(event.x)
        y = canvas.canvasy(event.y)
        group_id = self._canvas_ribbon_group_at(x, y)

        self._canvas_ribbon_drag_id = group_id
        self._canvas_ribbon_drag_start_x = x
        self._canvas_ribbon_drag_moved = False
        self._canvas_ribbon_drop_index = None

    def _canvas_ribbon_motion(self, event) -> None:
        group_id = getattr(self, "_canvas_ribbon_drag_id", None)
        if not group_id:
            return
        ...
        if abs(x - self._canvas_ribbon_drag_start_x) >= 5:
            self._canvas_ribbon_drag_moved = True

        if not self._canvas_ribbon_drag_moved:
            return

        ids = [
            gid
            for gid in self._canvas_ribbon_middle_ids
            if gid != group_id
        ]
        ...
        for index, gid in enumerate(ids):
            box = self._canvas_ribbon_boxes.get(gid)
            if box is None:
                continue
            x0, _y0, x1, _y1 = box
            middle = (x0 + x1) / 2
        ...
        if line_x is None:
            if ids:
                last = self._canvas_ribbon_boxes.get(ids[-1])
                line_x = last[2] + 4 if last else 8
            else:
                line_x = 8

        self._canvas_ribbon_drop_index = target
        canvas.delete("canvas_drop")
        canvas.create_line(
            line_x,
            5,
            line_x,
    ...
    def _canvas_ribbon_release(self, event) -> None:
        group_id = getattr(self, "_canvas_ribbon_drag_id", None)
        moved = bool(
            getattr(self, "_canvas_ribbon_drag_moved", False)
        )
        target = getattr(self, "_canvas_ribbon_drop_index", None)

        self._canvas_ribbon_drag_id = None
        self._canvas_ribbon_drag_moved = False
        self._canvas_ribbon_drop_index = None
        ...
        if moved and target is not None:
            self._canvas_reorder_group(group_id, target)
            return

        self._select_ribbon_group(group_id)

    def _canvas_reorder_group(self, group_id: str, target: int) -> None:
        """Réordonne les parties sans reconstruire le bandeau."""
        groups = self._groups()
        ...
        if source is None or bool(source.get("protected", False)):
            return

        old_ids = [str(group.get("id", "")) for group in middle]
        remaining = [
            group
            for group in middle
            if str(group.get("id", "")) != group_id
        ]
        target = max(0, min(int(target), len(remaining)))
        remaining.insert(target, source)
        new_ids = [str(group.get("id", "")) for group in remaining]
        ...
        for group in groups:
            ...
            is_middle_active = (
                not bool(group.get("deleted", False))
                and gid not in {"debut_livre", "fin_livre"}
            )
            rebuilt.append(next(iterator) if is_middle_active else group)

        groups[:] = rebuilt
        self._selected_ribbon_group_id = group_id
        self._save_data()
        self._refresh_canvas_ribbon()
        self._refresh_overview_selection()
        self._refresh_sequence()

    def _refresh_overview_selection(self) -> None:
        """Actualise uniquement l'état visuel des stations déjà créées."""
        records = getattr(self, "_overview_station_buttons", {})
        for group_id, record in records.items():
            selected = group_id == self._selected_ribbon_group_id
            dot = record.get("dot")
            label = record.get("label")
            accent = record.get("accent", self.SKY)
            try:
                dot.configure(
                    width=25 if selected else 22,
                    height=25 if selected else 22,
    ...
    def _create_ribbon(self, parent):
        """Bandeau structurel léger : groupes dessinés sur Canvas Tkinter."""
        self._page_type_buttons.clear()
        self._page_type_button_states.clear()
        ...
        self._canvas_ribbon_start = start_canvas

        # Zone centrale : un seul Canvas + une scrollbar Tk légère.
        middle = tk.Frame(
            ribbon,
            background=self.RIBBON_BG,
            borderwidth=0,
        ...
        scrollbar.grid(row=1, column=0, sticky="ew", pady=(1, 0))
        canvas.configure(xscrollcommand=scrollbar.set)

        self._canvas_ribbon_canvas = canvas
        self._canvas_ribbon_scrollbar = scrollbar
        self._canvas_ribbon_boxes = {}
        self._canvas_ribbon_drag_id = None
        self._canvas_ribbon_drag_start_x = 0
        self._canvas_ribbon_drag_moved = False
        self._canvas_ribbon_drop_index = None

        canvas.bind("<ButtonPress-1>", self._canvas_ribbon_press)
        canvas.bind("<B1-Motion>", self._canvas_ribbon_motion)
        canvas.bind("<ButtonRelease-1>", self._canvas_ribbon_release)
        canvas.bind(
            "<MouseWheel>",
            lambda event: self._canvas_ribbon_wheel(event),
        )
        canvas.bind(
            "<Shift-MouseWheel>",
            lambda event: self._canvas_ribbon_wheel(event),
        ...
        # Droite : Fin fixe + historiques + outils existants.
        right = tk.Frame(
            ribbon,
            background=self.RIBBON_BG,
            borderwidth=0,
        ...
        end_canvas.pack(side="left", padx=(0, 4))
        self._canvas_ribbon_end = end_canvas
        ...
        tools.pack(side="left", padx=(3, 0))

        self._draw_canvas_ribbon()

        if start_group is not None:
            self._draw_canvas_fixed_group(
                start_canvas,
                start_group,
            )
            start_canvas.bind(
                "<Button-1>",
                lambda _event, gid=str(start_group.get("id", "")):
                    self._select_ribbon_group(gid),
            )
        ...
        if end_group is not None:
            ...
            end_canvas.bind(
                "<Button-1>",
                lambda _event, gid=str(end_group.get("id", "")):
                    self._select_ribbon_group(gid),
            )

        self._ribbon_groups_panel = canvas
    ...
    def _create_group_block(
        ...
        """Niveau 1 : uniquement le squelette du livre."""
        ...
        if deleted:
            block = ctk.CTkFrame(parent, width=52, height=72, fg_color="transparent")
            block.pack_propagate(False)
            return block

        compact_width = max(112, min(165, width))
        soft = self._mix_color_with_white(accent, 0.92)
        title_soft = self._mix_color_with_white(accent, 0.82)

        block = ctk.CTkFrame(
            parent, width=compact_width, height=72,
            fg_color=soft, corner_radius=7,
            border_width=2 if group_id == self._selected_ribbon_group_id else 1,
            border_color=accent if group_id == self._selected_ribbon_group_id else title_soft,
        )
        block.pack_propagate(False)
        block.grid_propagate(False)
        ...
        title_label = ctk.CTkLabel(
            block, text=f"{symbol}  {title}",
            font=(Fonts.FAMILY, 10, "bold"),
            text_color=accent, anchor="center",
        )
        title_label.grid(row=0, column=0, sticky="ew", padx=6, pady=(9, 2))

        # Le volume est une information de structure, pas une liste de pages.
        page_count = 0
        for item in self._items():
            definition = self._definition_for(str(item.get("type", "")))
            if str(definition.get("group", "")) == group_id:
                page_count += max(1, int(item.get("count", 1) or 1))
        ...
        ctk.CTkLabel(
            ...
            text_color=self.TEXT_MUTED,
            anchor="center",
        ).grid(row=1, column=0, sticky="ew", padx=5, pady=(0, 7))

        for widget in (block, title_label):
            widget.bind(
                "<Button-1>",
        ...
        if not protected:
            self._bind_custom_group_drag(block, title_label, group_id)
        return block


    def _show_add_page_menu(self, group_id: str) -> None:
        """Choix ponctuel du type lors de la création d'une page."""
        definitions = [
            definition
            for definition in self._page_types()
            if (
                str(definition.get("group", "")) == str(group_id)
        ...
        for definition in definitions:
            title = str(definition.get("title", "Page"))
            symbol = str(definition.get("symbol", ""))
            menu.add_command(
                label=f"{symbol}  {title}".strip(),
                command=lambda d=definition: self._add_item(d),
            )
    ...
    def _create_page_type_button(
        ...
        """Palette de création : un type sert à créer une page, pas à la représenter."""
        ...
        definitions = [
            item for item in self._page_types()
            if str(item.get("group", "")) == group_id
        ]
        try:
            position = next(
                index for index, item in enumerate(definitions)
                if str(item.get("type", "")) == page_type
            )
        except StopIteration:
            position = 0

        factor = (0.84, 0.79, 0.74, 0.87)[position % 4]
        button = ctk.CTkButton(
            ...
            height=39,
            corner_radius=5,
            fg_color=self._mix_color_with_white(group_accent, factor),
            hover_color=self._mix_color_with_white(
                group_accent,
        ...
        self._attach_tooltip(
            button,
            f"Créer une nouvelle page de type « {full_title} »",
        )
        if page_type:
    ...
    def _select_ribbon_group(self, group_id: str) -> None:
        """Sélection locale : aucun show() et aucune reconstruction globale."""
        if not any(
    ...
    def _refresh_book_overview_panel(self) -> None:
        """Reconstruit uniquement la vue globale du livre."""
        root = self._root
        if root is None or not root.winfo_exists():
            return

        old = getattr(self, "_book_overview_frame", None)
        new = self._create_book_overview_panel(root)
        self._book_overview_frame = new
        new.grid(
            row=2,
            column=0,
            sticky="ew",
    ...
    def _refresh_sequence_panel(self) -> None:
        """Reconstruit uniquement la zone basse liée au groupe sélectionné."""
        root = self._root
        if root is None or not root.winfo_exists():
            return

        old = getattr(self, "_sequence_panel_frame", None)
        new = self._create_sequence_panel(root)
        self._sequence_panel_frame = new
        new.grid(
            row=3,
            column=0,
            sticky="nsew",
    ...
    def _bind_custom_group_drag(
        ...
        """Réactive le glisser-déposer des groupes non protégés."""
        group = self._group_for(group_id)
        if bool(group.get("protected", False)):
            return
        ...
        for widget in (group_widget, title_widget):
            widget.bind(
                "<ButtonPress-1>",
                lambda event, gid=group_id: self._start_group_drag(
                    event,
                    gid,
                ),
                add="+",
    ...
    def _create_sequence_panel(self, parent) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            title_row,
            text="Composition du groupe sélectionné",
            font=Fonts.H2,
            text_color=self.INK,
        ).grid(row=0, column=0, sticky="w")
    ...
    def _refresh_sequence(self) -> None:
        """Niveau 3 : composition du groupe par types, quantités et noms informatifs."""
        ...
        for item in self._items():
            ...
            if str(definition.get("group", "")) == selected_group_id:
                source_items.append((item, definition))

        active_ids = {str(item.get("id", "")) for item, _ in source_items}
        self._sanitize_page_selection(active_ids)

        # Entête lisible : Type prioritaire, puis quantité, suivi, nom informatif.
        headers = ("TYPE DE PAGE", "NOMBRE", "SUIVI", "NOM / REPÈRE (facultatif)")
        widths = (220, 78, 100, 230)
        for col, (label, width) in enumerate(zip(headers, widths)):
            self._sequence_frame.grid_columnconfigure(
                col, weight=1 if col in (0, 3) else 0, minsize=width
            )
            ctk.CTkLabel(
                self._sequence_frame,
                text=label,
                font=(Fonts.FAMILY, 8, "bold"),
                text_color=self.TEXT_MUTED,
                anchor="w" if col in (0, 3) else "center",
            ).grid(row=0, column=col, sticky="ew", padx=7, pady=(4, 7))

        if not source_items:
            ctk.CTkLabel(
                self._sequence_frame,
                text="Aucun type de page prévu dans ce groupe.",
                font=Fonts.NORMAL,
                text_color=self.TEXT_LIGHT,
            ).grid(row=1, column=0, columnspan=4, pady=28)
        else:
            for row_index, (item, definition) in enumerate(source_items, start=1):
                ...
                symbol = str(definition.get("symbol", ""))
                accent = str(definition.get("accent", self.INK))
                informative_name = str(item.get("title", "") or "").strip()
                # Si le titre est simplement le titre technique du type, ne le répète pas.
                if informative_name == title:
                    informative_name = ""
                ...
                row_frame.grid(
                    row=row_index, column=0, columnspan=4,
                    sticky="ew", padx=3, pady=3
                )
                row_frame.grid_columnconfigure(0, weight=1, minsize=220)
                row_frame.grid_columnconfigure(1, weight=0, minsize=78)
                row_frame.grid_columnconfigure(2, weight=0, minsize=100)
                row_frame.grid_columnconfigure(3, weight=1, minsize=230)
                ...
                type_label.grid(row=0, column=0, sticky="ew", padx=10, pady=9)
                ...
                follow = f"{completed}/{count} fait"
                ctk.CTkLabel(
                    row_frame,
                    text=follow,
                    font=(Fonts.FAMILY, 9, "bold"),
                    text_color=self.DONE if remaining == 0 else self.TEXT_MUTED,
                ).grid(row=0, column=2, padx=8)

                name_text = informative_name if informative_name else "—"
                name_label = ctk.CTkLabel(
                    row_frame,
                    text=name_text,
                    font=(Fonts.FAMILY, 9),
                    text_color=self.INK if informative_name else self.TEXT_LIGHT,
                    anchor="w",
                )
                name_label.grid(row=0, column=3, sticky="ew", padx=10)

                # Sélection du bloc conservée pour les commandes existantes.
                for widget in (row_frame, type_label, name_label):
                    widget.bind(
                        "<Button-1>",
                        lambda event, iid=item_id: self._update_page_selection_from_event(
                            iid, event
                        ),
                        add="+",
                    )
                ...
                self._sequence_row_widgets[item_id] = {
                    "row": row_frame,
                    "card": row_frame,
                    "group_strip": row_frame,
                    "thumbnail": row_frame,
                    "thumbnail_image_label": name_label,
                    "symbol_label": type_label,
                    "count_badge": name_label,
                    "info": row_frame,
                    "title_label": type_label,
                    "detail_label": name_label,
                    "auto_badge": name_label,
        ...
        # Palette de création du groupe sélectionné, placée sous la composition.
        palette_row = len(source_items) + 2
        ...
        palette.grid(
            row=palette_row, column=0, columnspan=4,
            sticky="ew", padx=3, pady=(12, 5)
        )
        palette.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(
            palette,
            text="+ Ajouter un type de page :",
            font=(Fonts.FAMILY, 9, "bold"),
            text_color=self.INK,
        ).grid(row=0, column=0, padx=(10, 6), pady=7)

        choices = ctk.CTkFrame(palette, fg_color="transparent")
        choices.grid(row=0, column=1, sticky="w", pady=5)

        definitions = [
            definition for definition in self._page_types()
            if (
                str(definition.get("group", "")) == selected_group_id
                and not bool(definition.get("deleted", False))
            )
        ...
        for index, definition in enumerate(definitions):
            title = str(definition.get("short", definition.get("title", "Page")))
            symbol = str(definition.get("symbol", ""))
            accent = str(definition.get("accent", self.INK))
            ctk.CTkButton(
                choices,
                ...
                height=26,
                corner_radius=5,
                fg_color=self._mix_color_with_white(accent, 0.88),
                hover_color=self._mix_color_with_white(accent, 0.74),
                text_color=accent,
                font=(Fonts.FAMILY, 8),
                command=lambda d=definition: self._add_item(d),
    ...
    def _create_sequence_occurrence_card(
        ...
        """Une vignette = une page réelle. Le nom de page est principal, le type secondaire."""
        source_id = str(item.get("id", ""))
        display_id = source_id if occurrence_index == 1 else f"{source_id}::occ::{occurrence_index}"
        ...
        page_color = self._plan_group_page_color(
            plan_group, str(definition.get("color", self.GROUP_BG))
        )

        outer = tk.Frame(
            parent, width=126, height=184, background="#FBFAF6",
        ...
        tk.Label(
            outer,
            text=f"Page {display_number}",
            font=(Fonts.FAMILY, 8, "bold"),
            foreground=self.TEXT_MUTED,
            background="#FBFAF6",
            borderwidth=0,
        ).grid(row=0, column=0, pady=(1, 2))

        card = tk.Frame(
            outer, width=106, height=142, background=self.CARD_BG,
        ...
        preview = tk.Frame(
            card, width=72, height=88, background=page_color,
            borderwidth=0, highlightthickness=1,
            highlightbackground=group_accent, highlightcolor=group_accent
        )
        preview.grid(row=0, column=0, pady=(7, 3))
        preview.grid_propagate(False)

        image_label = tk.Label(
            preview, image="", text="", background=page_color,
            borderwidth=0, highlightthickness=0
        )
        image_label.place(relx=0.5, rely=0.5, anchor="center")

        symbol_label = tk.Label(
            preview, text=str(definition.get("symbol", "?")),
            font=(Fonts.FAMILY, 15, "bold"), foreground=type_accent,
            background=page_color, borderwidth=0
        )
        symbol_label.place(relx=0.5, rely=0.5, anchor="center")

        auto_badge = tk.Label(
            preview,
            text="AUTO" if automatic_blank else "",
            font=(Fonts.FAMILY, 6, "bold"),
            foreground=self.INK,
            background="#E7EEF6" if automatic_blank else page_color,
            padx=3, pady=0, borderwidth=0
        )
        if automatic_blank:
            auto_badge.place(relx=0.04, rely=0.04, anchor="nw")

        base_name = str(item.get("title") or definition.get("title", "Page"))
        # Si un bloc représente plusieurs pages, chacune reçoit un nom lisible distinct.
        page_name = (
            f"{base_name} {occurrence_index}"
            if occurrence_total > 1
            else base_name
        )
        if len(page_name) > 18:
            page_name = page_name[:17].rstrip() + "…"

        type_name = str(definition.get("title", "Type"))
        if len(type_name) > 20:
            type_name = type_name[:19].rstrip() + "…"

        title_label = tk.Label(
            card, text=page_name,
            font=(Fonts.FAMILY, 8, "bold"), foreground=self.INK,
            background=self.CARD_BG, anchor="center", borderwidth=0
        )
        title_label.grid(row=1, column=0, sticky="ew", padx=4)

        detail_label = tk.Label(
            card, text=f"Type : {type_name}",
            font=(Fonts.FAMILY, 7), foreground=self.TEXT_MUTED,
            background=self.CARD_BG, anchor="center", borderwidth=0
        )
        detail_label.grid(row=2, column=0, sticky="ew", pady=(1, 4))

        photo = self._thumbnail_photo_for_definition(definition, subsample=5)
        if photo is not None:
            image_label.configure(image=photo)
            symbol_label.place_forget()

        record = {
            "row": outer, "card": card, "group_strip": preview,
            "thumbnail": preview, "thumbnail_image_label": image_label,
            "symbol_label": symbol_label, "count_badge": detail_label,
            "info": card, "title_label": title_label,
            "detail_label": detail_label, "auto_badge": auto_badge,
            "grid_index": display_number - 1, "display_state": {},
            "selection_state": None, "photo_ref": photo,
            "source_id": source_id, "occurrence_index": occurrence_index,
            "occurrence_total": occurrence_total,
        }
        self._sequence_row_widgets[display_id] = record

        for widget in (
            outer, card, preview, image_label,
            symbol_label, title_label, detail_label
        ):
            widget.bind(
                "<ButtonPress-1>",
                lambda event, sid=source_id: self._update_page_selection_from_event(sid, event),
                add="+",
            )

        self._bind_sequence_page_drag(
            source_id, card, preview, image_label,
            symbol_label, title_label, detail_label
        )
