# TOMELINEA_GABARITS_SELECTION_V66
...
class BookCanvas(tk.Frame):
    ...
    def gabarit_add_zone(self, kind: str) -> bool:
        ...
        for idx in targets:
            ...
            if kind == "document":
                # Point de départ : le document remplit exactement les marges,
                # quel que soit le cadre général. Il reste ensuite librement
                # déplaçable/redimensionnable selon la Zone autorisée choisie.
                initial_rect = self._gabarit_alignment_reference_box(zone, "margins")
            else:
