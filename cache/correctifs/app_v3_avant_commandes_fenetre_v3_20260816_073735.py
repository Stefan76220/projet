...
class TomeLineaV3(tk.Tk):
    ...
    def _window_minimize(self):
        """Réduit TomeLinea tout en restaurant le mode sans bandeau au retour."""
        try:
    ...
    def _window_restore_frameless(self, _event=None):
        ...
        self.after(40, restore)

    def _window_toggle_size(self):
        """Bascule entre zone de travail maximale et fenêtre restaurée."""
        left, top, work_w, work_h = self._window_work_area()
        maximized = getattr(self, "_tomelinea_window_maximized", True)

        if maximized:
            width = max(1180, int(work_w * 0.84))
            height = max(720, int(work_h * 0.86))
