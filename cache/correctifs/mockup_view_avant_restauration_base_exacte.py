...
class MockupView:
    ...
    def _create_ribbon(self, parent) -> ctk.CTkFrame:
        ...
        self._ribbon_groups_panel = self._create_soft_background_container(
            ribbon
        )
        ...
        for group_definition in groups:
            # Une partie supprimée reste dans les données, mais ne constitue
            # plus une colonne du ruban.
            if bool(group_definition.get("deleted", False)):
                continue
    ...
    def _create_group_block(
        ...
        if deleted:
            # Une partie supprimée reste visible comme repère historique,
            # mais n'occupe plus l'espace d'un vrai groupe.
            block = ctk.CTkFrame(
                parent,
                width=width,
                height=104,
            ...
            part_number = group_definition.get("part_number")
            short_text = (
                f"P{part_number}\nsuppr."
            ...
            trace.place(relx=0.5, rely=0.78, anchor="center")
            ...
            ctk.CTkLabel(
                ...
                justify="center",
                anchor="center",
            ).pack(expand=True, fill="both", padx=2, pady=1)
        ...
        # Affiche, accolée au numéro précédent, la trace des parties
        # supprimées consécutives sans leur réserver de colonne.
        if str(group_definition.get("group_kind", "")) == "partie":
            try:
                current_number = int(group_definition.get("part_number", 0) or 0)
            except (TypeError, ValueError):
                current_number = 0
            ...
            for candidate in self._groups():
                if str(candidate.get("group_kind", "")) != "partie":
                    continue
                try:
                    candidate_number = int(candidate.get("part_number", 0) or 0)
                except (TypeError, ValueError):
                    continue
                indexed_parts[candidate_number] = candidate

            while (
                expected in indexed_parts
                and bool(indexed_parts[expected].get("deleted", False))
            ):
                deleted_numbers.append(expected)
                expected += 1
            ...
            if deleted_numbers:
                trace = ctk.CTkLabel(
                    title_bar,
                    text=" · " + " · ".join(
                        f"P{number} supprimée" for number in deleted_numbers
                    ),
                    font=(Fonts.FAMILY, 7),
                    text_color="#969BA2",
                    anchor="e",
                )
                trace.grid(row=0, column=1, sticky="e", padx=(0, 3))
    ...
    def _refresh_ribbon(self) -> None:
        ...
        # Le nouveau ruban est construit sous l'ancien.
        # L'ancien reste donc visible pendant le rendu complet du nouveau :
        # aucun écran blanc pendant un déplacement de groupe.
        if old_ribbon is not None:
            try:
                new_ribbon.lower(old_ribbon)
            except Exception:
                pass

        def reveal_new_ribbon() -> None:
            try:
                if new_ribbon.winfo_exists():
                    new_ribbon.update_idletasks()
                    new_ribbon.lift()
            except Exception:
                pass
            ...
            if old_ribbon is not None:
                try:
                    if old_ribbon.winfo_exists():
                        old_ribbon.destroy()
                except Exception:
                    pass

        try:
            self._root.after(90, reveal_new_ribbon)
        except Exception:
            reveal_new_ribbon()
    ...
    def _start_group_drag(self, event, group_id: str) -> None:
        group = self._group_for(group_id)
        if bool(group.get("protected", False)) or bool(group.get("deleted", False)):
            return
    ...
    def _open_create_menu(self) -> None:
        window = self._new_dialog("Créer", "650x300")
    ...
    def _delete_custom_group(self, group_id: str, owner) -> None:
        ...
        if any(self._plan_group_id(item) == group_id for item in self._items()):
            return

        self._record_history()

        if str(target.get("group_kind", "")) == "partie":
            number = int(target.get("part_number", 0) or 0)
            target["deleted"] = True
            target["title"] = (
                f"Partie {number} — Supprimée"
                if number > 0
                else f"{target.get('title', 'Partie')} — Supprimée"
            )
            target["accent"] = "#A9ADB3"
        else:
            groups = self._groups()
            groups[:] = [
                group
    ...
    def _next_part_number(self) -> int:
        highest = 0
        for group in self._groups():
            if str(group.get("group_kind", "")) != "partie":
                continue
            try:
                highest = max(highest, int(group.get("part_number", 0) or 0))
            except (TypeError, ValueError):
                continue
    ...
    def _create_next_part(self, owner) -> None:
        ...
        # La nouvelle Partie N se place immédiatement après la Partie N-1.
        previous_index = None
        for index, group in enumerate(groups):
            if str(group.get("group_kind", "")) != "partie":
                continue
            try:
                group_number = int(group.get("part_number", 0) or 0)
            except (TypeError, ValueError):
                continue
            if group_number == number - 1:
        ...
        self._record_history()
        groups.insert(insert_index, new_group)
        self._save_data()
        self._close_dialog(owner)
        self._selected_ribbon_group_id = group_id
        self._refresh_ribbon()
    ...
    def _refresh_sequence(self) -> None:
        ...
        # Conserve exactement la position verticale du Plan du livre.
        # Le re-grillage des cartes ne doit pas faire sauter la vue après
        # un glisser-déposer.
        saved_yview = None
        try:
            canvas = getattr(self._sequence_frame, "_parent_canvas", None)
            if canvas is not None:
                saved_yview = canvas.yview()
        except Exception:
            saved_yview = None
        ...
        if saved_yview:
            try:
                ...
                if canvas is not None:
                    canvas.update_idletasks()
                    canvas.yview_moveto(saved_yview[0])
            except Exception:
                pass
            ...
            for raw in raw_groups:
                ...
                if group_id == "pages_interieures":
                    if not pages_interieures_seen:
                        migrated = dict(defaults["pages_interieures"])
                        migrated["title"] = "Partie 1"
                        migrated["group_kind"] = "partie"
                        migrated["part_number"] = 1
                        migrated["deleted"] = False
                        middle.append(migrated)
                ...
                kind = str(raw.get("group_kind", "") or "libre")
