...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_same_type_target_indices(self) -> list[int]:
        ...
        for unit in self._gabarit_navigation_units():
            ...
            item = self.items[idx]
            if self._is_automatic_page(item) or self._type_of(item) != type_key:
                continue
            if idx not in seen:
                seen.add(idx)
                targets.append(idx)
    ...
    def gabarit_set_scope(self, scope: str) -> bool:
        ...
        if requested == "type":
            ...
            self._gabarit_sync_active_template_to_type(targets)
            for idx in targets:
                item = self.items[idx]
            ...
            label = self._page_type_label(active, int(self._selected_index))
            self.status_var.set(
                f"En cours — gabarit appliqué aux {len(targets)} pages « {label} »."
                if len(targets) > 1
    ...
    def gabarit_add_zone(self, kind: str) -> bool:
        ...
        if str(getattr(self, "_gabarit_scope", "page")) == "type" and len(targets) > 1:
