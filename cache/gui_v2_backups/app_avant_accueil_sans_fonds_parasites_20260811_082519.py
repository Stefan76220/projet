...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        # Rangée des trois types de projet :
        # elle reprend le décor exact de la carte au lieu d'afficher
        # un grand panneau clair rectangulaire derrière les boutons.
        type_row = tk.Canvas(
            create_card,
            bg="#F5FBF8",
            height=119,
        ...
        type_row._pm_bg_photo = None

        def sync_type_row_background(_event=None):
            source = getattr(create_card, "_pm_composed_pil", None)
            if source is None:
                return

            x = max(type_row.winfo_x(), 0)
            y = max(type_row.winfo_y(), 0)
            width = max(type_row.winfo_width(), 1)
            height = max(type_row.winfo_height(), 1)

            patch = Image.new("RGBA", (width, height), "#F5FBF8")

            left = min(max(x, 0), source.width)
            top = min(max(y, 0), source.height)
            right = min(max(x + width, left), source.width)
            bottom = min(max(y + height, top), source.height)

            if right > left and bottom > top:
                crop = source.crop((left, top, right, bottom))
                patch.paste(crop, (0, 0))

            type_row._pm_bg_photo = ImageTk.PhotoImage(patch)
            type_row.delete("type_row_background")
            type_row.create_image(
                0,
                0,
                image=type_row._pm_bg_photo,
                anchor="nw",
                tags="type_row_background",
            )
            type_row.tag_lower("type_row_background")

        type_row.bind(
            "<Configure>",
            sync_type_row_background,
            add="+",
        )
        create_card.bind(
            "<Configure>",
            lambda _event: type_row.after_idle(sync_type_row_background),
            add="+",
        )
        type_row.after_idle(sync_type_row_background)
        ...
        def recent_rounded_box(
            ...
            *,
            ...
            content = tk.Frame(shell, bg=fill, bd=0, highlightthickness=0)
            window_id = shell.create_window(8, 5, anchor="nw", window=content)

            def draw(event=None):
                width = max(shell.winfo_width(), 20)
                box_height = height
                shell.delete("rounded_bg")
                r = min(radius, box_height // 2)
                ...
                points = [
                    ...
                    width - 2, 2,
                    width - 2, 2 + r,
                    width - 2, box_height - 2 - r,
                    width - 2, box_height - 2,
                    width - 2 - r, box_height - 2,
                    2 + r, box_height - 2,
                    2, box_height - 2,
                    2, box_height - 2 - r,
                    2, 2 + r,
                ...
                shell.tag_lower("rounded_bg")
                shell.coords(window_id, 8, 5)
                shell.itemconfigure(
                    window_id,
                    width=max(1, width - 16),
                    height=max(1, box_height - 10),
                )

            shell.bind("<Configure>", draw)
            shell.after_idle(draw)
            return shell, content

        def recent_chip(parent, text: str, bg: str, fg: str):
            width = max(58, int(len(text) * 4.8) + 18)
            height = 18
            chip = tk.Canvas(
                parent,
                width=width,
            ...
            chip.create_polygon(
                ...
                splinesteps=24,
                fill=bg,
                outline=self._mix(fg, "#FFFFFF", 0.55),
                width=1,
            )
            chip.create_text(
                width / 2,
        ...
        project_shell, project = recent_rounded_box(
            recent_card,
            fill="#FFFFFF",
            border="#83BFA9",
            height=88,
        ...
        project_shell.pack(fill="x", padx=14, pady=(0, 8))
        ...
        tk.Label(
            ...
            width=4,
        ).pack(side="left", padx=(7, 0), pady=6)

        details = tk.Frame(project, bg="#FFFFFF")
        details.pack(side="left", fill="both", expand=True, padx=7, pady=4)
        ...
        recent_chip(chips, "Ouvrage structuré", "#E5F3EC", "#4C9E7B")
        recent_chip(chips, "En cours", "#EDF5EF", "#4C9E7B")
        recent_chip(
            chips,
        ...
        PMCommandButton(
            ...
            width=94,
            command=None,
        ).pack(side="right", padx=(6, 4), pady=5)

        small_shell, small = recent_rounded_box(
            recent_card,
            fill="#FFFFFF",
            border=theme.BORDER,
            height=38,
        ...
        small_shell.pack(fill="x", padx=14, pady=(0, 10))
