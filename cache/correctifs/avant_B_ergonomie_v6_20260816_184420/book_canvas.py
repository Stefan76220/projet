...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        nav_row = tk.Frame(viewer, bg=theme.WINDOW_DEEP, height=52)
        ...
        self.h_nav.grid(row=0, column=0, pady=(1, 0))
    ...
    def _begin_page_name_edit(self, index: int):
        ...
        window = self.canvas.create_window(
            world_x, world_y, anchor="center", window=entry,
            width=width, height=22, tags=("page_name_editor",),
        )
        self._page_name_editor = entry
        self._page_name_editor_window = window
        self._editing_page_index = index
        entry.bind("<Return>", lambda _e: self._close_page_name_editor(commit=True))
        entry.bind("<Escape>", lambda _e: self._close_page_name_editor(commit=False))
