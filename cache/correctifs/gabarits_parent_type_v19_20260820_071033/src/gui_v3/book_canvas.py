...
class BookCanvas(tk.Frame):
    ...
    def __init__(
        ...
        *,
        ...
        # Portée Gabarits : la portée mémorisée d'une page reste distincte
        # de son état visuel. Les boutons ne s'allument que pendant un cycle
        # « En cours » validé ; un premier clic prépare la confirmation.
    ...
    def gabarit_toggle_finished(self) -> bool:
        """Valide le cycle courant en un clic.

        « Terminé » n'est accessible qu'après confirmation d'une portée.
        Une page terminée se rouvre automatiquement à sa prochaine modification ;
        cliquer sur son statut vert ne la rouvre donc pas accidentellement.
        """
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return False

        status = self._gabarit_status(active)
        if status == "termine":
            self.status_var.set("Gabarit terminé — modifiez la page pour le rouvrir.")
            return True

        scope = self._gabarit_item_last_scope(active)
        if not scope or not bool(active.get("gabarit_scope_active")):
            self.status_var.set("Choisissez puis confirmez d'abord la portée du gabarit.")
            return False

        if scope == "type":
            indices = self._gabarit_same_type_target_indices()
        else:
            indices = self._gabarit_active_unit_indices()
        if not indices:
            indices = [int(self._selected_index)]

        # « Toutes du type » est validé comme un vrai gabarit de type : au moment
        # précis où l'utilisateur passe à Terminé, la page active redevient la
        # référence et son contenu est recopié une dernière fois sur toutes les
        # pages homologues. On évite ainsi qu'une page restée différente pendant
        # le travail soit malgré tout marquée verte.
        if scope == "type":
            self._gabarit_sync_active_template_to_type(indices)

        for index in indices:
            item = self.items[index]
            item["gabarit_scope_last"] = scope
        ...
        self._emit_gabarit_page_changed()
        self.status_var.set(
            f"Gabarit terminé — {len(indices)} pages validées"
            if scope == "type" and len(indices) > 1
            else "Gabarit terminé"
        )
        return True

    def _gabarit_type_identity(self, item: dict | None) -> str:
        """Identité structurelle utilisée par « Toutes du type ».

        Dans Structure, le champ ``type`` est la véritable identité d'un type de page
        (y compris pour les types personnalisés). La propagation Gabarits doit donc
        utiliser cette même identité, et seulement se rabattre sur la clé visuelle
        canonique pour les très anciens éléments qui n'ont plus de type structurel.
        """
        if not isinstance(item, dict):
            return ""
        raw = str(self._type_of(item) or "").strip().lower()
        if raw:
            return raw
        try:
            return str(self._page_type_key(item) or "").strip().lower()
        except Exception:
            return ""

    def _gabarit_same_type_target_indices(self) -> list[int]:
        """Surfaces Gabarits manuelles correspondant au type de la page active.

        Une double page soudée reste une seule surface de travail : sa page primaire
        porte le gabarit de la paire. Les cibles sont dédupliquées et ordonnées comme
        le livre.
        """
        active = self._gabarit_active_item()
        if active is None:
        ...
        for unit in self._gabarit_navigation_units():
            ...
            if idx not in seen:
                seen.add(idx)
                targets.append(idx)
        if not targets and self._selected_index is not None:
            targets.append(int(self._selected_index))
        return targets

    def _gabarit_sync_active_template_to_type(self, targets: list[int]) -> int:
        """Copie le gabarit actif vers les autres pages ciblées.

        Les zones correspondantes partagent un ``slot_key`` mais conservent chacune
        un identifiant propre. Ainsi, les modifications suivantes en portée Type
        retrouvent réellement la zone homologue sur chaque page.
        """
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return 0
        ...
        if not isinstance(source_zones, list):
            source_zones = []
            active["gabarit_zones"] = source_zones

        # Les anciennes zones peuvent précéder le système de correspondance par slot.
        # On leur donne une clé stable avant de les dupliquer.
        for zone in source_zones:
            if not isinstance(zone, dict):
                continue
            if not str(zone.get("slot_key") or "").strip():
        ...
        for idx in targets:
            if idx == active_index:
                continue
            previous = self.items[idx].get("gabarit_zones")
            had_previous = isinstance(previous, list) and bool(previous)
            clones: list[dict] = []
            for source in source_zones:
                ...
                clones.append(clone)
            self.items[idx]["gabarit_zones"] = clones
            if clones or had_previous:
                self._gabarit_mark_edited(self.items[idx])
            changed += 1
        if source_zones:
            self._gabarit_mark_edited(active)
        return changed
    ...
    def gabarit_set_scope(self, scope: str) -> bool:
        """Cycle de portée en deux clics maximum.

        1er clic : le bouton choisi affiche « Confirmer » sans modifier le livre.
        2e clic : la portée est engagée, le bouton reste en surbrillance,
        le statut passe à « En cours » et la station devient orange.
        """
        ...
        last_scope = self._gabarit_item_last_scope(active)
        if self._gabarit_scope_is_active(active) and last_scope == requested:
            self._gabarit_scope_pending = ""
            self.status_var.set(
                "Portée déjà confirmée : toutes les pages du type."
                if requested == "type"
                else "Portée déjà confirmée : cette page."
            )
            self.render()
            return True

        # Premier clic : aucun changement métier, uniquement l'état « Confirmer ».
        if str(getattr(self, "_gabarit_scope_pending", "")) != requested:
            ...
            if requested == "type":
                ...
                label = self._page_type_label(active, int(self._selected_index))
                self.status_var.set(
                    f"Confirmer : appliquer ce gabarit aux {count} pages « {label} »."
                    if count > 1
                    else f"Confirmer : appliquer ce gabarit à la page « {label} »."
                )
            else:
                self.status_var.set("Confirmer : appliquer ce gabarit à cette page.")
            if getattr(self, "_work_mode", "") == "gabarits":
                self.render()
            return True

        # Deuxième clic sur le même bouton : engagement réel de la portée.
        self._gabarit_scope_pending = ""
        ...
        if requested == "type":
            ...
            synced_count = self._gabarit_sync_active_template_to_type(targets)
            for idx in targets:
                item = self.items[idx]
                item["gabarit_scope_last"] = "type"
                item["gabarit_scope_active"] = True
                item["gabarit_status"] = "en_cours"
                item["gabarit_done"] = False
            label = self._page_type_label(active, int(self._selected_index))
            self.status_var.set(
                f"En cours — gabarit synchronisé sur {len(targets)} pages « {label} »."
                if len(targets) > 1
                else f"En cours — gabarit appliqué à « {label} »."
            )
        else:
            targets = self._gabarit_active_unit_indices()
            if not targets:
                targets = [int(self._selected_index)]
            for idx in targets:
                item = self.items[idx]
                ...
                item["gabarit_status"] = "en_cours"
                item["gabarit_done"] = False
            self.status_var.set("En cours — gabarit appliqué à cette page.")
    ...
    def _gabarit_effective_scope(self, item: dict | None = None) -> str:
        """Portée réellement applicable à la prochaine modification.

        La donnée persistée sur la page est prioritaire sur la variable d'interface.
        Cela évite qu'un rafraîchissement de B ramène silencieusement une page liée au
        type en portée locale alors que le bouton/état indique toujours ce lien.
        """
        ...
        if last in {"page", "type"}:
            return last
        current = str(getattr(self, "_gabarit_scope", "page") or "page").strip().lower()
        return "type" if current == "type" else "page"
    ...
    def _gabarit_target_indices(self) -> list[int]:
        ...
        if active is None:
            return []
        if self._gabarit_effective_scope(active) != "type":
            indices = self._gabarit_active_unit_indices()
            return indices or [int(self._selected_index)]
        return self._gabarit_same_type_target_indices() or [int(self._selected_index)]

    def _gabarit_zone_target_indices(self) -> list[int]:
        """Surfaces réellement touchées par les actions de zone."""
        active = self._gabarit_active_item()
        if active is None:
            return []
        if self._gabarit_effective_scope(active) != "type":
            return [int(self._selected_index)]
        return self._gabarit_same_type_target_indices()
    ...
    def _gabarit_mark_edited(self, item: dict) -> None:
        """Invalide automatiquement une validation, sans engager une portée neuve.

        - avant la toute première confirmation de portée, la construction reste neutre ;
        - après une validation, la moindre modification rouvre le cycle précédent :
          En cours + portée mémorisée active + station orange.
        """
        last_scope = self._gabarit_item_last_scope(item)
        ...
        if last_scope:
            ...
            # Cas ancien sans métadonnée de portée : sécurité locale.
            item["gabarit_scope_last"] = "page"
            item["gabarit_scope_active"] = True
            item["gabarit_status"] = "en_cours"
        else:
