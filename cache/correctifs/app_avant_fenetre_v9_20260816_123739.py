...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Sous Windows on conserve une vraie fenêtre native : pas de
        # overrideredirect, donc Windows garde maximisation, restauration,
        # Snap, Alt+Tab et barre des tâches. Le non-client sera personnalisé
        # directement sur le HWND Tk après construction de l'interface.
        self._native_frame: WindowsNativeFrame | None = None
        ...
        native_ready = False
        if sys.platform == "win32":
            frame = WindowsNativeFrame(self, caption_height=34)
            native_ready = frame.install()
            if native_ready:
                self._native_frame = frame
                frame.prepare_restore_geometry()
                frame.set_state_callback(self._window_native_state_changed)

                # Révélation atomique : la fenêtre normale n'est jamais montrée
                # avant la première maximisation native.
                try:
        ...
        if not native_ready:
            # Repli historique (non-Windows ou API native indisponible).
            try:
                self.overrideredirect(True)
            except tk.TclError:
                pass
    ...
    def _window_native_state_changed(self, maximized: bool) -> None:
        """Synchronise uniquement le pictogramme avec l'état réel Windows."""
        label = getattr(self, "home_size_label", None)
        if label is not None:
            try:
                label.configure(text="❐" if maximized else "□")
            except tk.TclError:
                pass

    def _window_minimize(self) -> None:
        frame = self._native_frame
        if frame is not None and frame.installed:
            frame.minimize()
            return
        try:
            self.iconify()
    ...
    def _window_toggle_size(self) -> None:
        ...
        # Repli hors Windows : ancien comportement géométrique.
        try:
    ...
    def _build_home(self):
        ...
        # Commandes TomeLinea reliées à la vraie fenêtre Windows.
        # Elles restent dans le langage visuel de l'Accueil ; aucune seconde
        # coque ni barre de titre artificielle n'est créée.
        min_label = tk.Label(
        ...
        close_label.bind("<Button-1>", lambda _e: self._window_close())
    ...
    def _build_zone_a(self):
        ...
        V3Button(actions, "Fermer", self._window_close, compact=True).pack(side="left", padx=(10, 0))
