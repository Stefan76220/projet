...
class PageMaitreV2(tk.Tk):
    ...
    def _build_visualisation_vierge(self, parent: tk.Frame) -> None:
        ...
        # Vue globale : Canvas directement posé sur le fond.
        # Le dessin et le clic-glisser reprennent la vue Maquettage.
        panorama = tk.Canvas(
            parent,
            bg=PANEL,
            highlightthickness=0,
        ...
        # Outils : zone directe sur le fond, sous la Vue globale.
        ...
        def draw_tools(_event=None):
            ...
            tools_canvas.create_polygon(
                panel_points(0, 0, w - 3, h - 4, dx=2, dy=4),
                fill="#BDB7AE",
                outline="",
            )
            tools_canvas.create_polygon(
                panel_points(0, 0, w - 3, h - 4),
                fill="#FFFEFC",
                outline="#D5D1CA",
                width=1,
            )
        ...
        # Placement : Vue globale en haut sur TOUTE la largeur.
        # Outils commence seulement SOUS cette vue.
        def layout_visualisation(_event=None):
            ...
            global_h = min(
                470,
                max(405, int(height * 0.46)),
            )

            panorama.place(
                x=margin_x,
                y=top_y,
                width=max(900, width - margin_x * 2),
                height=global_h,
            )

            tools_w = 300
            tools_x = width - 44 - tools_w
            tools_y = top_y + global_h + 18
            tools_h = max(90, height - tools_y - 34)
