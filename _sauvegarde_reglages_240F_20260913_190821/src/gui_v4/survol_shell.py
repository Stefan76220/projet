...
"""
    ...
    def _phase2_show_overlay(self, canvas, title, message, *args, **kwargs):
        """Ne montre jamais une transition technique de navigation au lecteur."""
        ...
        if technical_transition:
            # Les unités Canvas sont une construction interne. Précédente /
            # Suivante doit donner l'impression d'un seul livre continu.
            try:
...
"""
    ...
    def _survol_preload_following_chapter(self) -> None:
        # Préchargement permanent : il sert aussi à Précédente/Suivante hors
        # Survol. Le nom historique de la méthode est conservé pour limiter le
        # risque de régression, mais sa portée est maintenant Composition.
        if bool(getattr(self, "_phase2_switch_in_progress", False)):
            return
        layout = getattr(self, "_phase2_chapter_layout", None)
    ...
    def _phase2_request_global_page(self, canvas, global_index: int, *, force: bool = False) -> None:
        """Échange les unités Canvas sans détruire la page visible."""
        ...
        # Le double-buffer n'appartient pas au Survol : c'est désormais le
        # comportement normal de navigation de Composition. Un changement de
        # page ne doit jamais réafficher une construction technique.
        host = getattr(self, "_phase2_canvas_host", None)
    ...
    def _global_settings_reload_active_canvas(self) -> None:
        """Recharge le vrai Canvas avec le plan qui vient d'être modifié."""
        host = getattr(self, "_phase2_canvas_host", None)
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        ...
        if host is None or index is None or not (0 <= int(index) < len(plans)):
            self._global_settings_notice = (
                "Réglages enregistrés. Le Canvas actif n'est pas encore disponible."
            )
            self._composition_update_inspector_context()
            return
        ...
        # Un changement global invalide le chapitre préchargé : ne jamais
        # réutiliser un buffer construit avec l'ancien format.
        try:
            self._survol_dispose_buffer()
        except Exception:
            pass
        ...
        def wait_ready():
            ...
            try:
                if bool(getattr(host, "ready", False)):
                    ...
                    self._global_settings_render_pending = False
                    self._global_settings_notice = (
                        "Réglages appliqués au livre et au rendu."
                        + (f" Pagination actuelle : {page_count} page(s)." if page_count else "")
                    )
                    self._composition_update_inspector_context()
                    try:
                        self.after_idle(self._survol_preload_following_chapter)
    ...
    def _global_settings_apply(self) -> None:
        """Une seule validation : format + marges + règles générales du texte."""
        ...
        try:
            ...
            if not font:
                raise ValueError("Police : indiquez une police.")
        ...
        # Une seule transaction pour l'état persistant du Livre.
        def action(project):
        ...
        try:
            changed_book = int(self.session.execute("Réglages globaux du Livre", action) or 0)
        except Exception as exc:
            self._global_settings_notice = str(exc)
            self._composition_update_inspector_context()
            return
        ...
        try:
            ...
            self._global_settings_notice = (
                "Le Livre a été enregistré, mais le plan de rendu n'a pas pu être mis à jour : "
                + str(exc)
            )
            self._composition_update_inspector_context()
            return
        ...
        self._global_settings_notice = (
            f"Application en cours : {int(report.get('paragraphs', 0) or changed_book)} paragraphe(s) de texte courant."
        )
        try:
    ...
    def _composition_render_book_state_panel(self, host) -> None:
        ...
        tk.Label(
            host,
            text="Réglez le livre ici, puis appliquez une seule fois.",
        ...
        tk.Entry(
            host, textvariable=vars_map["font_family"],
            bg=theme.PANEL_SOFT, fg=theme.INK, insertbackground=theme.INK,
            relief="flat", bd=0, font=(theme.FONT_UI, 8),
        ).pack(fill="x", padx=14, ipady=3, pady=(1, 3))
        ...
        self._button(
            host, "Appliquer les réglages globaux", self._global_settings_apply,
