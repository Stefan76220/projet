...
class BookCanvas(tk.Frame):
    ...
    def _draw_page_auto_rule_markers(self, item: dict, x: float, y: float, page_w: float, scale: float) -> None:
        """Badge Page auto très visible : maillon lié ou maillon cassé."""
        ...
        # Le badge doit rester lisible même lorsque la vignette est petite.
        radius = max(9.0, min(11.0, 10.0 * max(0.9, scale)))
        cy = y + max(15.0, 19.0 * scale)
        ...
        for position in ("before", "after"):
            ...
            # Le badge déborde légèrement du bord de la page : impossible de le
            # confondre avec le contenu de la vignette.
            cx = x + 2.0 if position == "before" else x + page_w - 2.0

            fill = "#26343E" if excluded else "#173B35"
            outline = "#C78366" if excluded else "#76C3A4"
            chain = "#E7D9B4" if not excluded else "#E2B6A6"
            ...
            # Deux maillons, dessinés directement pour ne pas dépendre d'un
            # caractère Unicode ou d'une police particulière.
            link_w = radius * 0.95
            link_h = radius * 0.48
            offset = radius * (0.33 if not excluded else 0.43)
            line_w = 2

            self.canvas.create_oval(
                cx - offset - link_w / 2, cy - link_h / 2,
                cx - offset + link_w / 2, cy + link_h / 2,
                outline=chain, width=line_w,
            )
            self.canvas.create_oval(
                cx + offset - link_w / 2, cy - link_h / 2,
                cx + offset + link_w / 2, cy + link_h / 2,
            ...
            if excluded:
                # Maillon cassé : coupure centrale très visible + barre diagonale.
                self.canvas.create_rectangle(
                ...
                self.canvas.create_line(
                    cx - radius * 0.32, cy + radius * 0.48,
                    cx + radius * 0.32, cy - radius * 0.48,
                    fill=outline, width=2,
                )
            else:
                # Maillon actif : pont central continu.
                self.canvas.create_line(
                    cx - offset + link_w * 0.34, cy,
                    cx + offset - link_w * 0.34, cy,
                    fill=chain, width=2,
                )
    ...
    def _draw_page(self, index: int, x: float, y: float, page_w: float, page_h: float, scale: float, *, show_label: bool = True):
        ...
        self._draw_page_auto_rule_markers(item, x, y, page_w, scale)
    ...
    def _on_press(self, event):
        ...
        # V30 : le bandeau d’une partie reste une vraie cible de sélection.
        # La moitié basse est déjà réservée au renommage ; ici, un clic sur la
        # moitié haute sélectionne/déplace la partie. Le lasso ne démarre que
        # dans un espace réellement vide hors du bandeau.
        ...
        # Glisser depuis un espace vide = cadre de sélection multiple.
        if index is None:
            cx = self.canvas.canvasx(event.x)
            cy = self.canvas.canvasy(event.y)
