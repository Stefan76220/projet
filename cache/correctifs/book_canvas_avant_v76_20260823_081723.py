# TOMELINEA_GABARITS_FOND_GUIDE_V75
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        if active_band == "allowed":
            ...
            content_h = 184.0 if guide else 60.0
        ...
        def icon_strip(items, *, button=41.0, gap=8.0, y_offset=0.0, center=True, start_x=None):
            # V74 — disposition compacte et prévisible : les outils commencent
            # à gauche du panneau et se suivent côte à côte, sans étirement.
            yy=content_top+y_offset
            sx=float(start_x) if start_x is not None else fly_left+11.0
        ...
        if active_band == "create":
            ...
            if not guide:
                ...
                target.create_text(fly_left+11.0,content_top+52.0,text="CADRAGE AUTO",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
                icon_strip((
    ...
    def gabarit_import_guide(self) -> bool:
        ...
        # Premier import : le fond perdu est le cadre de départ le plus large et
        # le plus sûr pour un support de construction. Un remplacement conserve
        # le cadrage et la transparence déjà choisis par l'utilisateur.
