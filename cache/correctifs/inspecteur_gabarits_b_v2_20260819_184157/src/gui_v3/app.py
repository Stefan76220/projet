...
class TomeLineaV3(tk.Tk):
    ...
    def _build_gabarits_tab(self, parent):
        """Gabarits : quatre familles stables, panneaux courts et contextuels."""
        WORK_BG = "#102633"
        CARD_BG = "#0E202B"
        CARD_BORDER = "#385264"
        TEAL = "#78B8B1"
        ...
        atelier = tk.Frame(
            parent, bg=WORK_BG, highlightthickness=1,
            highlightbackground=CARD_BORDER, padx=12, pady=8,
        )
        atelier.grid(row=0, column=0, sticky="nsew")
        atelier.grid_rowconfigure(1, weight=1)
        atelier.grid_columnconfigure(0, weight=1)
        ...
        tk.Label(
            head, text="GABARIT — OUTILS", bg=WORK_BG, fg=TEXT,
            font=(theme.FONT_TITLE, 10, "bold"),
        ).grid(row=0, column=0, sticky="w")
        ...
        families.grid(row=1, column=0, sticky="nsew")
        families.grid_rowconfigure(0, weight=1)
        # L'ordre visuel suit l'ordre réel de travail.
        weights = (3, 3, 4, 4)
        for col, weight in enumerate(weights):
            families.grid_columnconfigure(col, weight=weight, uniform="gab_family")
        ...
        def open_tool_panel(opener, title, builder, *, min_width=320):
            ...
            # Par défaut le panneau monte au-dessus de C et reste proche de sa famille.
        ...
        def family_card(col, number, title, subtitle):
            card = tk.Frame(
                families, bg=CARD_BG, highlightthickness=1,
                highlightbackground=CARD_BORDER, padx=9, pady=7,
            )
            card.grid(
        ...
        page_card, page_box = family_card(0, "1", "PAGE", "format fini · marges · fond perdu")
        content_card, content_box = family_card(1, "2", "CONTENU", "préparer Production")
        place_card, place_box = family_card(2, "3", "PLACEMENT", "positionner et dimensionner")
        align_card, align_box = family_card(3, "4", "ALIGNEMENT", "ordonner précisément")

        # ----------------------------------------------------------
        # 1 — PAGE
        # ----------------------------------------------------------
        page_box.grid_columnconfigure(1, weight=1)
        margins_var = tk.StringVar(value="Marges\n—")
        bleed_var = tk.StringVar(value="Fond perdu\n—")

        # Miniature pédagogique : même langage visuel que B. Le fond perdu
        # déborde réellement du format fini ; la marge est un unique repère
        # intérieur. Elle remplace la lecture abstraite de deux seuls boutons.
        page_mini = tk.Canvas(
            page_box, width=78, height=82, bg=CARD_BG,
            highlightthickness=0, bd=0, takefocus=0,
        )
        page_mini.grid(row=0, column=0, rowspan=2, sticky="nsw", padx=(0, 8))

        margins_btn = V3Button(page_box, "", compact=True)
        margins_btn.configure(textvariable=margins_var, pady=6)
        margins_btn.grid(row=0, column=1, sticky="ew", pady=(0, 5))
        bleed_btn = V3Button(page_box, "", compact=True)
        bleed_btn.configure(textvariable=bleed_var, pady=6)
        bleed_btn.grid(row=1, column=1, sticky="ew")

        def redraw_page_miniature(settings=None):
            if settings is None:
                canvas = book()
                try:
                    settings = canvas.gabarit_current_settings() if canvas is not None else {}
                except Exception:
                    settings = {}
            ...
            page_mini.delete("all")
            cw = max(78, int(page_mini.winfo_width() or 78))
            ch = max(82, int(page_mini.winfo_height() or 82))
            ph = min(66.0, ch - 12.0)
            pw = ph * 210.0 / 297.0
            x = (cw - pw) / 2.0
            y = (ch - ph) / 2.0

            def bleed_px(value, dimension, reference):
                value = max(0.0, float(value or 0.0))
                return 0.0 if value <= 0.0 else max(3.0, dimension * value / reference)

            left = bleed_px(bleed.get("left", 3.0), pw, 210.0)
            right = bleed_px(bleed.get("right", 3.0), pw, 210.0)
            top = bleed_px(bleed.get("top", 3.0), ph, 297.0)
            bottom = bleed_px(bleed.get("bottom", 3.0), ph, 297.0)
            halo_fill = "#9A6263"
            halo_outline = "#AA7273"
            if top > 0:
                page_mini.create_rectangle(x-left, y-top, x+pw+right, y, fill=halo_fill, outline="", stipple="gray25")
            if bottom > 0:
                page_mini.create_rectangle(x-left, y+ph, x+pw+right, y+ph+bottom, fill=halo_fill, outline="", stipple="gray25")
            if left > 0:
                page_mini.create_rectangle(x-left, y, x, y+ph, fill=halo_fill, outline="", stipple="gray25")
            if right > 0:
                page_mini.create_rectangle(x+pw, y, x+pw+right, y+ph, fill=halo_fill, outline="", stipple="gray25")
            if max(left, right, top, bottom) > 0:
                page_mini.create_rectangle(x-left, y-top, x+pw+right, y+ph+bottom, outline=halo_outline, width=1)

            page_mini.create_rectangle(x, y, x+pw, y+ph, fill="#F1F1EE", outline="#C8CECB", width=2)
            mx1 = x + pw * float(margins.get("inside", 15.0)) / 210.0
            mx2 = x + pw - pw * float(margins.get("outside", 15.0)) / 210.0
            my1 = y + ph * float(margins.get("top", 15.0)) / 297.0
            my2 = y + ph - ph * float(margins.get("bottom", 15.0)) / 297.0
            if mx2 > mx1 and my2 > my1:
                page_mini.create_rectangle(mx1, my1, mx2, my2, outline="#71928C", width=1)

        page_mini.bind("<Configure>", lambda _e: redraw_page_miniature())
        ...
        def open_margins():
            ...
            numeric_panel(
                margins_btn, "Marges — millimètres",
        ...
        def open_bleed():
            ...
            numeric_panel(
                bleed_btn, "Fond perdu — millimètres",
                [("top", "Haut"), ("right", "Droite"), ("bottom", "Bas"), ("left", "Gauche")],
                values,
                lambda v: canvas.gabarit_set_bleed(v["top"], v["right"], v["bottom"], v["left"]),
            )

        margins_btn.configure(command=open_margins)
        bleed_btn.configure(command=open_bleed)

        # ----------------------------------------------------------
        # 2 — CONTENU
        # ----------------------------------------------------------
        for col in range(3):
            content_box.grid_columnconfigure(col, weight=1, uniform="content")
        content_buttons = {}
        for col, (kind, label) in enumerate((("text", "Texte"), ("image", "Image"), ("document", "Document"))):
            ...
            btn.grid(row=0, column=col, sticky="ew", padx=(0 if col == 0 else 4, 0))
        ...
        # ----------------------------------------------------------
        for col in range(3):
            place_box.grid_columnconfigure(col, weight=1, uniform="place")
        ...
        dimensions_btn.grid(row=0, column=1, sticky="ew", padx=(4, 0))
        snap_btn.grid(row=0, column=2, sticky="ew", padx=(4, 0))
        ...
        align_box.grid_columnconfigure(1, weight=1)
        ...
        distribute_btn.grid(row=0, column=1, sticky="ew", padx=(4, 0))
        ...
        def refresh_gabarit_tools(_event=None):
            ...
            margins_var.set(
                "Marges\n"
                f"H {float(margins.get('top', 0)):g} · B {float(margins.get('bottom', 0)):g} · "
                f"Int. {float(margins.get('inside', 0)):g} · Ext. {float(margins.get('outside', 0)):g} mm"
            )
            bleed_var.set(
                "Fond perdu\n"
                f"H {float(bleed.get('top', 0)):g} · B {float(bleed.get('bottom', 0)):g} · "
                f"G {float(bleed.get('left', 0)):g} · D {float(bleed.get('right', 0)):g} mm"
            )
            redraw_page_miniature(settings)
            try:
            ...
            for btn in (occupation_btn, dimensions_btn, snap_btn, align_btn):
