...
class TomeLineaV3(tk.Tk):
    ...
    def _build_home(self):
        ...
        tk.Label(
            header, text="TL", bg=theme.WINDOW_DEEP, fg=theme.INK,
            font=(theme.FONT_TITLE, 38, "bold"),
        ).place(relx=0.00, rely=0.00)
        tk.Label(
            header, text="TomeLinea", bg=theme.WINDOW_DEEP, fg=theme.INK,
            font=(theme.FONT_TITLE, 34),
        ).place(relx=0.085, rely=0.00)
        tk.Label(
            header, text="V3", bg=theme.WINDOW_DEEP, fg=green,
            font=(theme.FONT_UI, 12, "bold"),
        ).place(relx=0.385, rely=0.12)
        tk.Label(
            header, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE",
            bg=theme.WINDOW_DEEP, fg=green, font=(theme.FONT_UI, 9, "bold"),
        ).place(relx=0.085, rely=0.67)

        tools = tk.Frame(header, bg=theme.WINDOW_DEEP)
        tools.place(relx=0.72, rely=0.12, relwidth=0.28, relheight=0.55)
        help_label = tk.Label(
        ...
        open_icon = tk.Label(open_other, text="⌕", bg=panel_alt, fg=green, font=(theme.FONT_UI, 17))
        ...
        for col, (key, title, accent) in enumerate((
            ...
            photo = self._home_type_icon_glow(key, 126, selected=False)
            if photo is not None:
                icon.configure(image=photo)
                icon.image = photo
            icon.pack(expand=True, pady=(8, 0))
            tk.Label(
                holder, text=title, bg=panel, fg=accent,
                font=(theme.FONT_UI, 9, "bold"),
            ).pack(pady=(0, 6))
        model_hint = tk.Frame(right, bg=panel_alt, highlightthickness=1, highlightbackground=border)
        model_hint.grid(row=3, column=0, sticky="ew", padx=30, pady=(0, 22), ipady=8)
        tk.Label(model_hint, text="▤", bg=panel_alt, fg=green, font=(theme.FONT_UI, 17)).pack(side="left", padx=(14, 10))
        tk.Label(
            model_hint, text="Ou repartir d’un modèle de projet déjà construit",
            bg=panel_alt, fg=theme.MUTED, font=(theme.FONT_UI, 8),
        ).pack(side="left")
        ...
        model_icon = tk.Label(model_card, text="▤", bg=panel_alt, fg=green, font=(theme.FONT_UI, 16))
