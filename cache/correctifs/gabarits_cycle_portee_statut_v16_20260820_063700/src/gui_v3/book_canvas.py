...
class BookCanvas(tk.Frame):
    ...
    def gabarit_set_scope(self, scope: str) -> bool:
        """Choisit la portée des modifications de gabarit.

        Passer à « Toutes du type » est une opération volontaire : TomeLinea confirme
        d'abord le nombre de pages concernées puis synchronise le gabarit de la page
        active vers ces pages. Après cette synchronisation, les zones homologues ont
        des ``slot_key`` communs et toutes les modifications suivantes se propagent.
        """
        scope = str(scope or "page").strip().lower()
        new_scope = "type" if scope == "type" else "page"
        current = str(getattr(self, "_gabarit_scope", "page"))
        if current == new_scope:
            return True
        ...
        if new_scope == "type":
            ...
            if len(targets) <= 1:
                try:
                    self.status_var.set("Aucune autre page de ce type dans le livre.")
                except Exception:
                    pass
                return False

            active = self._gabarit_active_item()
            label = self._page_type_label(active, int(self._selected_index or 0)) if active else "ce type"
            confirm = getattr(self, "on_gabarit_confirm_type_scope", None)
            if callable(confirm):
                try:
                    if not bool(confirm(str(label), len(targets))):
                        return False
                except Exception:
                    return False

            self._gabarit_scope = "type"
            self._gabarit_sync_active_template_to_type(targets)
            self._save_order()
            self._emit_gabarit_page_changed()
            self._emit_gabarit_selection_changed()
            try:
                self.status_var.set(f"Portée : {len(targets)} pages « {label} » synchronisées")
            except Exception:
                pass
        else:
            self._gabarit_scope = "page"
            try:
                self.status_var.set("Portée : cette page")
            except Exception:
                pass
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        self.canvas.create_text((type_x1 + inner_right) / 2.0, y + scope_h / 2.0, text="Toutes du type", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if type_scope else "normal"))
