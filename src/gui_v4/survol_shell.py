...
class TomeLineaV4Survol(
    ...
    def __init__(
        ...
        *,
        ...
        # Phase 2.40G — les choix éditoriaux généraux précèdent toute
        # navigation et toute intervention dans le texte. Le Livre reste
        # visible, mais le Canvas est forcé en lecture seule.
        self._global_settings_dirty = False
        self._global_settings_guard_job = None
        self._global_settings_plan_signature = None
        self._global_settings_last_lock_state = None
        try:
            self._global_settings_guard_job = self.after(250, self._global_settings_guard_tick)
        except Exception:
            self._global_settings_guard_job = None
...
"""
    ...
    def _survol_structure_page_ids(
        ...
        try:
            internal_ids = list(
                self._phase2_composition_page_ids()
            )
        except Exception:
            internal_ids = []
...
"""
    ...
    def _survol_page_ready(self, page_id: str) -> bool:
        ...
        try:
            internal_ids = set(str(value) for value in self._phase2_composition_page_ids())
        except Exception:
            internal_ids = set()
    ...
    def _activate_page(
        ...
        *,
        ...
        # Avant validation des choix généraux, la Structure est visible mais ne
        # sert pas encore à naviguer : la couverture reste le seul point affiché.
        if self._global_settings_navigation_locked() and current and target != current:
            self._global_settings_notice = (
                "Validez d'abord les réglages généraux du livre. "
                "La navigation sera ensuite disponible en lecture seule."
            )
            self._composition_update_inspector_context()
            self._composition_refresh_center_navigation()
            return None

        # Après validation, toute unité chargée reçoit à nouveau la configuration
        # canonique du Livre juste avant son affichage.
        try:
            order = list(getattr(self.session.book, "page_order", ()) or ())
            if target in order:
                self._global_settings_reapply_target_plan(order.index(target))
        except Exception:
            pass
...
"""
    ...
    def _phase2_request_global_page(self, canvas, global_index: int, *, force: bool = False) -> None:
        """Étape 1 : navigation verrouillée avant validation, plan canonique après."""
        layout = getattr(self, "_phase2_chapter_layout", None)
        if layout is None:
        ...
        if self._global_settings_navigation_locked() and not force:
            # La première page/couverture doit pouvoir être préparée à l'ouverture,
            # mais aucune autre page ne devient accessible avant validation.
            if global_index != 0:
                self._global_settings_notice = (
                    "Navigation verrouillée : validez d'abord les réglages généraux."
                )
                self._composition_update_inspector_context()
                self._composition_refresh_center_navigation()
                return None
            ...
            try:
                self._global_settings_reapply_target_plan(global_index)
            except Exception:
                pass
    ...
    # ==========================================================
    # Phase 2.40D — ETAPE 1 : REGLAGES GLOBAUX, UN SEUL ECRAN
    # ==========================================================

    def _composition_build_tool_rail(self, parent):
        """Étape 1 : aucun rail d'outils inutile.

        Format, marges et règles générales sont une seule décision globale du
        Livre. Le Survol et les outils contextuels reviendront seulement après
        validation de cette étape.
        """
        self._composition_tool_tabs = {}
        self._composition_active_tool = "book"
        rail = tk.Frame(parent, bg=theme.PANEL, width=1)
    ...
    def _global_settings_text_source(self) -> dict:
        from src.v4.general_text_rules_runtime import (
            infer_rules_from_plans,
            normalize_general_text_rules,
        )
        try:
            existing = dict(self._composition_text_rules() or {})
        except Exception:
            existing = {}
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        inferred = (
            infer_rules_from_plans(plans)
            if plans
            else normalize_general_text_rules(existing)
        )
        merged = dict(inferred)
        for key, value in existing.items():
            if value not in (None, ""):
                merged[key] = value
        if "line_spacing_multiple" not in merged:
            try:
                size = float(merged.get("font_size_pt") or inferred.get("font_size_pt") or 11.0)
                pitch = float(merged.get("line_pitch_mm") or 0.0)
                base = size * 25.4 / 72.0
                merged["line_spacing_multiple"] = (
                    round(pitch / base, 2)
                    if pitch > 0 and base > 0
                    else inferred.get("line_spacing_multiple", 1.15)
                )
            except Exception:
                merged["line_spacing_multiple"] = 1.15
        if str(merged.get("hyphenation") or "undecided") not in {"forbid", "controlled"}:
            merged["hyphenation"] = "forbid"
        return normalize_general_text_rules(merged)

    @staticmethod
    def _global_settings_number(raw, label: str, minimum: float, maximum: float) -> float:
        text = str(raw or "").strip().replace(",", ".")
        try:
            value = float(text)
        except (TypeError, ValueError):
            raise ValueError(f"{label} : entrez une valeur numérique.")
        if value < minimum or value > maximum:
            raise ValueError(
                f"{label} : valeur attendue entre {minimum:g} et {maximum:g}."
            )
        return value

    def _global_settings_apply_book_content(self, book, rules: dict) -> int:
        """Persiste la police/taille du corps dans le Livre, jamais dans les titres."""
        changed = 0
        try:
            from src.v4.page_element_roles import page_element_roles, ROLE_BODY
        except Exception:
            page_element_roles = None
        ...
        for page_id in list(getattr(book, "page_order", ()) or ()):
            ...
            if page is None:
                continue
            roles = {}
            if callable(page_element_roles):
                try:
                    roles = page_element_roles(book, page_id)
                except Exception:
                    roles = {}
            for element in list(getattr(page, "content", ()) or ()):
                if not isinstance(element, dict) or str(element.get("kind") or "").lower() != "text":
                    continue
                eid = str(element.get("id") or "")
                role = roles.get(eid)
                if role is not None and str(getattr(role, "role", "")) != str(ROLE_BODY):
                    continue
                ...
                if isinstance(spans, list):
                    for span in spans:
                        if not isinstance(span, dict):
                            continue
                        span["font"] = rules["font_family"]
                        span["size"] = rules["font_size_pt"]
                metadata = element.setdefault("metadata", {})
                if not isinstance(metadata, dict):
                    metadata = {}
                    element["metadata"] = metadata
                metadata["general_text_rules"] = {
                    "alignment": rules["alignment"],
                    "first_line_indent_mm": rules["first_line_indent_mm"],
                    "line_spacing_multiple": rules["line_spacing_multiple"],
                    "paragraph_gap_mm": rules["paragraph_gap_mm"],
                    "hyphenation": rules["hyphenation"],
                }
                changed += 1
        return changed

    def _global_settings_reload_active_canvas(self) -> None:
        """Recharge l'unité visible puis déverrouille la lecture du Livre."""
        host = getattr(self, "_phase2_canvas_host", None)
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        index = getattr(self, "_phase2_active_chapter_index", None)
        if host is None or index is None or not (0 <= int(index) < len(plans)):
            self._global_settings_notice = (
                "Réglages validés. Le rendu central sera actualisé dès qu'il sera disponible."
            )
            self._composition_update_inspector_context()
            self._composition_refresh_center_navigation()
            return
        ...
        contract = None
        try:
            contract = getattr(getattr(prepared, "canvas", None), "contract", None)
        except Exception:
            contract = None
        if not isinstance(contract, dict):
            try:
                contract = getattr(getattr(host, "_session", None), "contract", None)
            except Exception:
                contract = None
        if not isinstance(contract, dict):
            self._global_settings_notice = (
                "Réglages enregistrés, mais TomeLinea n'a pas pu relancer le rendu."
            )
            self._composition_update_inspector_context()
            return

        try:
            previous_local_page = int(getattr(host, "_active_page", 1) or 1)
        except Exception:
            previous_local_page = 1
        ...
        self._global_settings_render_pending = True
        self._global_settings_notice = "Recomposition du livre…"
        self._composition_update_inspector_context()

        try:
            host.load_document(plans[int(index)], CanvasLoadSession(contract))
        except Exception as exc:
            self._global_settings_render_pending = False
            self._global_settings_notice = f"Recomposition impossible : {exc}"
            self._composition_update_inspector_context()
            return
        ...
        def wait_ready():
            attempts["count"] += 1
            try:
                if bool(getattr(host, "ready", False)):
                    page_count = int(getattr(host, "page_count", 0) or 0)
                    wanted = previous_local_page
                    if page_count > 0:
                        wanted = max(1, min(previous_local_page, page_count))
                    try:
                        host.go_to_page(wanted, smooth=False)
                    except Exception:
                        pass
                    self._global_settings_set_live_canvas_mode("readonly")
                    self._global_settings_render_pending = False
                    self._global_settings_notice = (
                        "Réglages généraux validés pour tout le livre. "
                        "La navigation est maintenant disponible en lecture seule."
                    )
                    self._composition_update_inspector_context()
                    self._composition_refresh_center_navigation()
                    return
            ...
            if attempts["count"] < 160:
                try:
                    self.after(100, wait_ready)
                except Exception:
                    pass
            else:
                self._global_settings_render_pending = False
                self._global_settings_notice = (
                    "Les réglages sont enregistrés, mais le rendu n'a pas confirmé sa fin."
                )
                self._composition_update_inspector_context()

        try:
            self.after(120, wait_ready)
        except Exception:
            pass

    def _global_settings_apply(self) -> None:
        """Valide une seule fois format, marges et règles générales du Livre."""
        from src.v4.format_catalog import BY_KEY as STANDARD_FORMATS_BY_KEY
        from src.v4.general_text_rules_runtime import (
            apply_global_settings_to_plans,
            normalize_general_text_rules,
            set_plans_mode,
        )

        vars_map = getattr(self, "_global_settings_vars", None)
        if not isinstance(vars_map, dict):
            return

        try:
            format_key = str(vars_map["format_key"].get() or "").strip()
            standard = STANDARD_FORMATS_BY_KEY.get(format_key)
            if standard is None:
                raise ValueError("Format : choisissez un format du catalogue TomeLinea.")

            top = self._global_settings_number(vars_map["margin_top_mm"].get(), "Marge haute", 0, 80)
            bottom = self._global_settings_number(vars_map["margin_bottom_mm"].get(), "Marge basse", 0, 80)
            inside = self._global_settings_number(vars_map["margin_inside_mm"].get(), "Marge intérieure", 0, 80)
            outside = self._global_settings_number(vars_map["margin_outside_mm"].get(), "Marge extérieure", 0, 80)
            if inside + outside >= float(standard.width_mm):
                raise ValueError("Marges : intérieure + extérieure dépassent la largeur utile.")
            if top + bottom >= float(standard.height_mm):
                raise ValueError("Marges : haut + bas dépassent la hauteur utile.")

            font = str(vars_map["font_family"].get() or "").strip()
            if not font:
                raise ValueError("Police : choisissez une police disponible.")
            available = {name.casefold(): name for name in self._global_settings_available_fonts()}
            if available and font.casefold() not in available:
                raise ValueError(f"Police : {font} n'est pas disponible dans TomeLinea.")
            if available:
                font = available[font.casefold()]

            size = self._global_settings_number(vars_map["font_size_pt"].get(), "Taille", 6, 72)
            indent = self._global_settings_number(vars_map["first_line_indent_mm"].get(), "Retrait", 0, 30)
            line = self._global_settings_number(vars_map["line_spacing_multiple"].get(), "Interligne", 0.8, 3)
            gap = self._global_settings_number(vars_map["paragraph_gap_mm"].get(), "Espace après", 0, 20)
            alignment = str(vars_map["alignment"].get() or "justify")
            hyphenation = str(vars_map["hyphenation"].get() or "forbid")
            rules = normalize_general_text_rules({
                "font_family": font,
                "font_size_pt": size,
                "alignment": alignment,
            ...
            self._global_settings_notice = str(exc)
            self._composition_update_inspector_context()
            return

        config = {
            "schema": "tomelinea.global-editorial-settings.v1",
            "format_key": standard.key,
            "width_mm": float(standard.width_mm),
            "height_mm": float(standard.height_mm),
            "margin_top_mm": top,
            "margin_bottom_mm": bottom,
            "margin_inside_mm": inside,
            "margin_outside_mm": outside,
            "text_rules": dict(rules),
        }
        ...
        def action(project):
            ...
            book.metadata["format_catalog_key"] = standard.key
            book.metadata["format_catalog_label"] = standard.label
            book.metadata["format_platforms"] = list(standard.platforms)
            book.metadata["format_user_modified"] = True
            book.metadata["layout_reflow_check_required"] = True

            saved = update_text_general_rules(
                book,
                confirmed=True,
                ...
                hyphenation=rules["hyphenation"],
                first_line_indent_mm=rules["first_line_indent_mm"],
                line_pitch_mm=rules["line_pitch_mm"],
                paragraph_gap_mm=rules["paragraph_gap_mm"],
            )
            saved["line_spacing_multiple"] = rules["line_spacing_multiple"]
            book.metadata["text_general_rules"] = saved
            book.metadata["text_reflow_requested"] = True
            changed = self._global_settings_apply_book_content(book, rules)
            book.history.append({
                "action": "reglages_generaux_240g",
                "format_key": standard.key,
                "width_mm": config["width_mm"],
                "height_mm": config["height_mm"],
                "margins_mm": {
        ...
        try:
            changed_book = int(self.session.execute("Réglages généraux du Livre", action) or 0)
        except Exception as exc:
            self._global_settings_notice = str(exc)
            self._composition_update_inspector_context()
            return
        ...
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        try:
            report = apply_global_settings_to_plans(
                plans,
                width_mm=config["width_mm"],
                height_mm=config["height_mm"],
                margin_top_mm=top,
            ...
            self._global_settings_notice = (
                "Les réglages sont enregistrés dans le Livre, mais la préparation du rendu a échoué : "
                + str(exc)
            )
            self._composition_update_inspector_context()
            return

        self._global_settings_last_saved = {
            "format_key": standard.key,
            "margin_top_mm": top,
            "margin_bottom_mm": bottom,
            "margin_inside_mm": inside,
        ...
        self._global_settings_plan_signature = None
        self._global_settings_notice = (
            f"Validation en cours pour tout le livre : {int(report.get('plans', 0))} unité(s), "
            f"{int(report.get('paragraphs', 0) or changed_book)} paragraphe(s)."
        )
        try:
            self._refresh_history_buttons()
        except Exception:
            pass
        self._global_settings_reload_active_canvas()

    def _composition_render_book_state_panel(self, host) -> None:
        """Étape 1 : un seul écran, aucune fenêtre secondaire, une validation."""
        ...
        fmt = book.format
        current = find_standard_format(float(fmt.width_mm), float(fmt.height_mm))
        if current is None:
            current = nearest_standard_format(float(fmt.width_mm), float(fmt.height_mm))

        rules = self._global_settings_text_source()
        saved = getattr(self, "_global_settings_last_saved", None)
        if isinstance(saved, dict):
            rules.update({k: v for k, v in saved.items() if k in rules})

        # Les variables sont recréées depuis l'état enregistré à chaque vrai
        # rafraîchissement du panneau. Aucun bouton intermédiaire ne valide quoi
        # que ce soit.
        vars_map = {
            "format_key": tk.StringVar(value=(
                str(saved.get("format_key"))
                if isinstance(saved, dict) and saved.get("format_key") in STANDARD_FORMATS_BY_KEY
                else str(current.key)
            )),
            "margin_top_mm": tk.StringVar(value=str(
                saved.get("margin_top_mm", float(fmt.margin_top_mm)) if isinstance(saved, dict) else float(fmt.margin_top_mm)
            ).replace(".", ",")),
            "margin_bottom_mm": tk.StringVar(value=str(
                saved.get("margin_bottom_mm", float(fmt.margin_bottom_mm)) if isinstance(saved, dict) else float(fmt.margin_bottom_mm)
            ).replace(".", ",")),
            "margin_inside_mm": tk.StringVar(value=str(
                saved.get("margin_inside_mm", float(fmt.margin_inside_mm)) if isinstance(saved, dict) else float(fmt.margin_inside_mm)
            ).replace(".", ",")),
            "margin_outside_mm": tk.StringVar(value=str(
                saved.get("margin_outside_mm", float(fmt.margin_outside_mm)) if isinstance(saved, dict) else float(fmt.margin_outside_mm)
            ).replace(".", ",")),
            "font_family": tk.StringVar(value=str(rules.get("font_family") or "Arial")),
            "font_size_pt": tk.StringVar(value=str(rules.get("font_size_pt") or 11).replace(".", ",")),
            "alignment": tk.StringVar(value=str(rules.get("alignment") or "justify")),
            "first_line_indent_mm": tk.StringVar(value=str(rules.get("first_line_indent_mm") or 0).replace(".", ",")),
            "line_spacing_multiple": tk.StringVar(value=str(rules.get("line_spacing_multiple") or 1.15).replace(".", ",")),
            "paragraph_gap_mm": tk.StringVar(value=str(rules.get("paragraph_gap_mm") or 0).replace(".", ",")),
            "hyphenation": tk.StringVar(value=str(rules.get("hyphenation") or "forbid")),
        }
        self._global_settings_vars = vars_map

        tk.Label(
            host, text="RÉGLAGES DU LIVRE", bg=theme.PANEL, fg=theme.INK,
            font=(theme.FONT_UI, 11, "bold"), anchor="w",
        ).pack(fill="x", padx=14, pady=(12, 2))
        locked = self._global_settings_navigation_locked()
        tk.Label(
            host,
            text=(
                "Étape préalable · navigation verrouillée"
                if locked
                else "Réglages validés · navigation en lecture seule"
            ),
            bg=theme.PANEL,
            fg=(theme.WARNING if locked else theme.ACCENT_BRIGHT),
            font=(theme.FONT_UI, 8, "bold"),
        ...
        tk.Label(
            host,
            text=(
                "Définissez ici les règles du livre entier. Aucun texte de page "
                "n'est modifiable pendant cette étape."
            ),
        ...
        def section(title):
            ...
            tk.Label(
                host, text=title, bg=theme.PANEL, fg=theme.ACCENT_BRIGHT,
                font=(theme.FONT_UI, 9, "bold"), anchor="w",
            ).pack(fill="x", padx=14, pady=(0, 5))

        def compact_field(parent, label, key, suffix="", width=7):
            cell = tk.Frame(parent, bg=theme.PANEL)
            tk.Label(
                cell, text=label, bg=theme.PANEL, fg=theme.MUTED,
                font=(theme.FONT_UI, 8), anchor="w",
            ).pack(fill="x")
            line = tk.Frame(cell, bg=theme.PANEL)
            line.pack(fill="x", pady=(1, 0))
            entry = tk.Entry(
                line, textvariable=vars_map[key], width=width,
                bg=theme.PANEL_SOFT, fg=theme.INK, insertbackground=theme.INK,
                relief="flat", bd=0, justify="right", font=(theme.FONT_UI, 9),
            )
            entry.pack(side="left", fill="x", expand=True, ipady=3)
            if suffix:
                tk.Label(
                    line, text=suffix, bg=theme.PANEL, fg=theme.MUTED,
                    font=(theme.FONT_UI, 8),
                ).pack(side="left", padx=(4, 0))
            return cell

        def choice_row(title, variable, options):
            tk.Label(
                host, text=title, bg=theme.PANEL, fg=theme.MUTED,
                font=(theme.FONT_UI, 8), anchor="w",
            ...
            for index, (label, value) in enumerate(options):
                button = tk.Button(
                    row, text=label,
                    command=lambda v=value: (variable.set(v), refresh()),
                ...
                button.pack(
                    ...
                    padx=((0, 3) if index == 0 else (3, 0)), ipady=4,
                )
                buttons.append((button, value))
            refresh()

        # FORMAT — lecture immédiate : nom grand, dimensions et orientation.
        # Le menu sert seulement à CHANGER le format, pas à porter toute
        # l'information dans une ligne minuscule.
        section("FORMAT DU LIVRE")
        format_name = tk.StringVar()
        format_details = tk.StringVar()
        self._global_settings_format_label = format_name

        family_by_key = {}
        grouped_formats = formats_by_ui_family()
        for family, items in grouped_formats:
            for item in items:
                family_by_key[item.key] = family

        def refresh_format_label():
            item = STANDARD_FORMATS_BY_KEY.get(vars_map["format_key"].get())
            if item is None:
                format_name.set("Format à choisir")
                format_details.set("Sélectionnez un format éditorial TomeLinea")
                return
            format_name.set(str(item.label))
            if abs(float(item.width_mm) - float(item.height_mm)) < 0.15:
                orientation = "Carré"
            elif float(item.width_mm) > float(item.height_mm):
                orientation = "Paysage"
            else:
                orientation = "Portrait"
            family = str(family_by_key.get(item.key) or item.family or "")
            details = f"{item.dimensions_label}  ·  {orientation}"
            if family and family not in {orientation, item.label}:
                details += f"  ·  {family}"
            format_details.set(details)

        def select_format(key):
            vars_map["format_key"].set(str(key))
            refresh_format_label()

        format_card = tk.Frame(
            host, bg=theme.PANEL_ALT, bd=0, highlightthickness=1,
            highlightbackground=theme.PAGE_BORDER,
        )
        format_card.pack(fill="x", padx=14, pady=(0, 6))
        tk.Label(
            format_card, textvariable=format_name, bg=theme.PANEL_ALT, fg=theme.INK,
            font=(theme.FONT_UI, 13, "bold"), anchor="w",
        ).pack(fill="x", padx=10, pady=(8, 1))
        tk.Label(
            format_card, textvariable=format_details, bg=theme.PANEL_ALT, fg=theme.MUTED,
            font=(theme.FONT_UI, 8), anchor="w", justify="left", wraplength=285,
        ).pack(fill="x", padx=10, pady=(0, 8))

        selector = tk.Menubutton(
            host, text="Choisir un autre format  ▾", anchor="center",
            bg=theme.PANEL_SOFT, fg=theme.INK,
            activebackground=theme.PANEL_ALT, activeforeground=theme.WHITE,
            relief="flat", bd=0, padx=9, pady=7,
            font=(theme.FONT_UI, 9, "bold"), cursor="hand2",
        )
        selector.pack(fill="x", padx=14)
        menu = tk.Menu(
            selector, tearoff=False, bg=theme.PANEL_SOFT, fg=theme.INK,
            activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE,
            relief="flat", bd=0,
        )
        for family, items in grouped_formats:
            family_menu = tk.Menu(
                menu, tearoff=False, bg=theme.PANEL_SOFT, fg=theme.INK,
                activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE,
                relief="flat", bd=0,
            )
            for item in items:
                family_menu.add_command(
                    label=f"{item.label}   ·   {item.dimensions_label}",
                    command=lambda key=item.key: select_format(key),
                )
            menu.add_cascade(label=family, menu=family_menu)
        selector.configure(menu=menu)
        refresh_format_label()

        # MARGES — quatre champs visibles, mais compacts. Aucun second bouton.
        section("MARGES")
        margin_grid = tk.Frame(host, bg=theme.PANEL)
        margin_grid.pack(fill="x", padx=14)
        margin_grid.grid_columnconfigure(0, weight=1)
        margin_grid.grid_columnconfigure(1, weight=1)
        for cell, row, col in (
            (compact_field(margin_grid, "Haut", "margin_top_mm", "mm"), 0, 0),
            (compact_field(margin_grid, "Bas", "margin_bottom_mm", "mm"), 0, 1),
            (compact_field(margin_grid, "Intérieure", "margin_inside_mm", "mm"), 1, 0),
            (compact_field(margin_grid, "Extérieure", "margin_outside_mm", "mm"), 1, 1),
        ):
            cell.grid(row=row, column=col, sticky="ew", padx=((0, 6) if col == 0 else (6, 0)), pady=2)

        # TEXTE COURANT — titres exclus de cette règle.
        section("TEXTE COURANT — RÈGLE GÉNÉRALE")
        tk.Label(
            host, text="Police", bg=theme.PANEL, fg=theme.MUTED,
            font=(theme.FONT_UI, 7), anchor="w",
        ).pack(fill="x", padx=14)
        font_button = tk.Menubutton(
            host, textvariable=vars_map["font_family"], anchor="w",
            bg=theme.PANEL_SOFT, fg=theme.INK,
            activebackground=theme.PANEL_ALT, activeforeground=theme.WHITE,
            relief="flat", bd=0, padx=8, pady=6,
            font=(theme.FONT_UI, 9), cursor="hand2",
        )
        font_button.pack(fill="x", padx=14, pady=(1, 3))
        font_menu = tk.Menu(
            font_button, tearoff=False, bg=theme.PANEL_SOFT, fg=theme.INK,
            activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE,
            relief="flat", bd=0,
        )
        available_fonts = self._global_settings_available_fonts()
        groups = (("A–F", "A", "G"), ("G–L", "G", "M"), ("M–R", "M", "S"), ("S–Z", "S", "ZZZZ"))
        for label, start, end in groups:
            names = [name for name in available_fonts if start <= name.upper() < end]
            if not names:
                continue
            submenu = tk.Menu(
                font_menu, tearoff=False, bg=theme.PANEL_SOFT, fg=theme.INK,
                activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE,
                relief="flat", bd=0,
            )
            for name in names:
                submenu.add_command(
                    label=name,
                    command=lambda value=name: vars_map["font_family"].set(value),
                )
            font_menu.add_cascade(label=label, menu=submenu)
        font_button.configure(menu=font_menu)

        text_grid = tk.Frame(host, bg=theme.PANEL)
        text_grid.pack(fill="x", padx=14)
        text_grid.grid_columnconfigure(0, weight=1)
        text_grid.grid_columnconfigure(1, weight=1)
        for cell, row, col in (
            (compact_field(text_grid, "Taille", "font_size_pt", "pt"), 0, 0),
            (compact_field(text_grid, "Retrait 1re ligne", "first_line_indent_mm", "mm"), 0, 1),
            (compact_field(text_grid, "Interligne", "line_spacing_multiple", "×"), 1, 0),
            (compact_field(text_grid, "Espace après", "paragraph_gap_mm", "mm"), 1, 1),
        ):
            cell.grid(row=row, column=col, sticky="ew", padx=((0, 6) if col == 0 else (6, 0)), pady=2)

        choice_row(
            "Alignement", vars_map["alignment"],
            (("Justifié", "justify"), ("À gauche", "left")),
        )
        choice_row(
            "Césures", vars_map["hyphenation"],
            (("Sans césures", "forbid"), ("Contrôlées", "controlled")),
        )

        # Toute modification après une validation redevient un brouillon :
        # la navigation se reverrouille jusqu'à la prochaine validation.
        for variable in vars_map.values():
            try:
                variable.trace_add("write", self._global_settings_mark_dirty)
            except Exception:
                pass

        notice = str(getattr(self, "_global_settings_notice", "") or "").strip()
        if notice:
            tk.Label(
                host, text=notice, bg=theme.PANEL,
                fg=(theme.WARNING if "impossible" in notice.lower() or "entrez" in notice.lower() else theme.ACCENT_BRIGHT),
                wraplength=285, justify="left", anchor="w",
                font=(theme.FONT_UI, 7, "bold"),
            ).pack(fill="x", padx=14, pady=(8, 2))

        enabled = not bool(getattr(self, "_global_settings_render_pending", False))
        self._button(
            host, "Valider les réglages généraux", self._global_settings_apply,
            compact=True, accent=True, enabled=enabled,
        ).pack(fill="x", padx=14, pady=(10, 14))

    def _global_settings_available_fonts(self) -> list[str]:
        """Polices réellement accessibles au processus TomeLinea."""
        names = set()
        try:
            names.update(str(name).strip() for name in tkfont.families(self) if str(name).strip())
        except Exception:
            pass
        try:
            from src.v4.font_library import CORE_FONT_FAMILIES
            installed = {name.casefold() for name in names}
            for item in CORE_FONT_FAMILIES:
                if item.family.casefold() in installed:
                    names.add(item.family)
        except Exception:
            pass
        current = ""
        try:
            current = str(self._global_settings_vars.get("font_family").get() or "").strip()
        except Exception:
            current = ""
        if current:
            names.add(current)
    ...
    def _global_settings_project_state(self) -> dict:
        project = getattr(getattr(self, "session", None), "project", None)
        if project is None:
            return {}
        value = project.metadata.get("global_editorial_settings_v1")
        return dict(value) if isinstance(value, dict) else {}

    def _global_settings_is_confirmed(self) -> bool:
        state = self._global_settings_project_state()
        return bool(state.get("confirmed"))

    def _global_settings_navigation_locked(self) -> bool:
        if str(getattr(self, "current_workspace", "") or "") != "composition":
            return False
        return (
            not self._global_settings_is_confirmed()
            or bool(getattr(self, "_global_settings_dirty", False))
            or bool(getattr(self, "_global_settings_render_pending", False))
        )

    def _global_settings_mark_dirty(self, *_args) -> None:
        if bool(getattr(self, "_global_settings_render_pending", False)):
            return
        self._global_settings_dirty = True
        project = getattr(getattr(self, "session", None), "project", None)
        if project is not None:
            state = self._global_settings_project_state()
            if state.get("confirmed"):
                state["confirmed"] = False
                project.metadata["global_editorial_settings_v1"] = state
                try:
                    project.touch()
                except Exception:
                    pass
    ...
    def _global_settings_saved_config(self) -> dict:
        state = self._global_settings_project_state()
        config = state.get("settings")
        return dict(config) if isinstance(config, dict) else {}

    def _global_settings_apply_config_to_plans(self, plans) -> dict:
        from src.v4.general_text_rules_runtime import (
            apply_global_settings_to_plans,
            set_plans_mode,
        )
        config = self._global_settings_saved_config()
        plans = list(plans or ())
        if not config or not plans:
            set_plans_mode(plans, "readonly")
            return {"plans": 0}
        report = apply_global_settings_to_plans(
            plans,
            ...
            margin_top_mm=float(config["margin_top_mm"]),
            margin_bottom_mm=float(config["margin_bottom_mm"]),
            margin_inside_mm=float(config["margin_inside_mm"]),
            margin_outside_mm=float(config["margin_outside_mm"]),
            text_rules=dict(config.get("text_rules") or {}),
        )
        set_plans_mode(plans, "readonly")
        return report

    def _global_settings_reapply_target_plan(self, global_index: int) -> None:
        layout = getattr(self, "_phase2_chapter_layout", None)
        plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
        if layout is None or not plans or not self._global_settings_saved_config():
            return
        chapter_index, _local = layout.locate_global_index(int(global_index))
        chapter_index = int(chapter_index)
        if 0 <= chapter_index < len(plans):
            self._global_settings_apply_config_to_plans([plans[chapter_index]])

    def _global_settings_set_live_canvas_mode(self, mode: str = "readonly") -> None:
        host = getattr(self, "_phase2_canvas_host", None)
        web = getattr(host, "_web", None) if host is not None else None
        if web is None:
            return
        wanted = "readonly" if str(mode) != "edit" else "edit"
        script = (
            "window.tomeLineaCanvasSetMode && "
            f"window.tomeLineaCanvasSetMode({json.dumps(wanted)}); true;"
        )
        try:
            web.eval_js(script)
        except Exception:
    ...
    def _global_settings_guard_tick(self) -> None:
        """Maintient l'étape préalable cohérente sans modifier le Livre."""
        self._global_settings_guard_job = None
        try:
            if str(getattr(self, "current_workspace", "") or "") == "composition":
                plans = list(getattr(self, "_phase2_chapter_plans", ()) or ())
                signature = (
                    ...
                    str(self._global_settings_saved_config()),
                )
                if signature != getattr(self, "_global_settings_plan_signature", None):
                    if self._global_settings_saved_config():
                        self._global_settings_apply_config_to_plans(plans)
                    else:
                        from src.v4.general_text_rules_runtime import set_plans_mode
                        set_plans_mode(plans, "readonly")
                    self._global_settings_plan_signature = signature
                self._global_settings_set_live_canvas_mode("readonly")

                locked = self._global_settings_navigation_locked()
                if locked != getattr(self, "_global_settings_last_lock_state", None):
                    self._global_settings_last_lock_state = locked
                    try:
                        self._composition_refresh_center_navigation()
                    except Exception:
                        pass
            ...
            pass
        try:
            self._global_settings_guard_job = self.after(500, self._global_settings_guard_tick)
        except Exception:
            self._global_settings_guard_job = None

    def _composition_refresh_center_navigation(self) -> None:
        super()._composition_refresh_center_navigation()
        if not self._global_settings_navigation_locked():
            return
        for name in ("_composition_prev_button", "_composition_next_button"):
...
"""
    ...
    def _navigation_only_open_first_book_page(self) -> None:
        ...
        if current == first_page_id:
            try:
                self.after_idle(self._navigation_update_blank_cover_overlay)
            except Exception:
                pass
            return
    ...
    # ==========================================================
    # NAVIGATION PROPRE 02 — overrides de fin de classe
    # ----------------------------------------------------------
    # Cette couche ne touche jamais à l'espace Source. L'import DOCX reste
    # entièrement géré par la V4 courante ; la simplification à un seul onglet
    # ne s'applique qu'une fois arrivé dans Composition.
    # ==========================================================

    def _global_settings_navigation_locked(self) -> bool:
        """Aucun verrou de navigation dans la base de validation NAV02."""
        return False

    def _global_settings_guard_tick(self) -> None:
        """Désactive l'étape Réglages généraux pendant la validation navigation."""
        self._global_settings_guard_job = None
        return None

    def _composition_refresh_center_navigation(self) -> None:
        """Conserve les boutons Précédente/Suivante actifs et synchronise la couverture vide."""
        result = super()._composition_refresh_center_navigation()
        try:
            self.after_idle(self._navigation_update_blank_cover_overlay)
        except Exception:
            pass
    ...
    # ==========================================================
    # NAVIGATION PROPRE 04 — couvertures physiques sans contenu
    # ----------------------------------------------------------
    # Les faces de couverture ne font pas partie de la pagination locale
    # Canvas. Sans écran dédié, le Canvas laisse alors voir la première page
    # intérieure sous la couverture. Tant qu'aucun contenu n'a été attribué à
    # la face physique, TomeLinea affiche donc une vraie page blanche au-dessus
    # du Canvas, sans modifier le Livre ni consommer la page intérieure suivante.
    # ==========================================================

    def _navigation_active_cover_is_empty(self) -> bool:
        session = getattr(self, "session", None)
        book = getattr(session, "book", None)
        page_id = str(getattr(session, "active_page_id", "") or "")
        if book is None or not page_id:
            return False
        page = getattr(book, "pages", {}).get(page_id)
        if page is None or not is_cover_face(page):
            return False

        # Une couverture explicitement alimentée par l'utilisateur ou par une
        # Source doit rester visible. Le masque blanc n'existe que pour une
        # face physique encore vide.
        source = getattr(page, "source", None)
        content = list(getattr(page, "content", ()) or ())
        metadata = getattr(page, "metadata", {})
        if source is not None or content:
            return False
        if isinstance(metadata, dict):
            if metadata.get("cover_asset") or metadata.get("cover_image"):
                return False
            if str(metadata.get("cover_content_status") or "").strip().lower() in {
                "assigned", "content", "image", "filled"
            }:
                return False
        return True

    def _navigation_destroy_blank_cover_overlay(self) -> None:
        overlay = getattr(self, "_navigation_blank_cover_overlay", None)
        if overlay is not None:
            try:
                overlay.destroy()
            except Exception:
                pass
        self._navigation_blank_cover_overlay = None
        self._navigation_blank_cover_page = None
        self._navigation_blank_cover_parent = None

    def _navigation_layout_blank_cover_overlay(self, _event=None) -> None:
        overlay = getattr(self, "_navigation_blank_cover_overlay", None)
        page = getattr(self, "_navigation_blank_cover_page", None)
        parent = getattr(self, "_navigation_blank_cover_parent", None)
        if overlay is None or page is None or parent is None:
            return
        try:
            width = max(120, int(parent.winfo_width()))
            height = max(120, int(parent.winfo_height()))
            book = getattr(getattr(self, "session", None), "book", None)
            fmt = getattr(book, "format", None)
            page_w = float(getattr(fmt, "width_mm", 148.0) or 148.0)
            page_h = float(getattr(fmt, "height_mm", 210.0) or 210.0)
            ratio = max(0.15, page_w / max(1.0, page_h))
            usable_w = max(80, width - 44)
            usable_h = max(80, height - 44)
            draw_h = usable_h
            draw_w = int(round(draw_h * ratio))
            if draw_w > usable_w:
                draw_w = usable_w
                draw_h = int(round(draw_w / ratio))
            x = max(0, (width - draw_w) // 2)
            y = max(0, (height - draw_h) // 2)
            page.place(x=x, y=y, width=max(1, draw_w), height=max(1, draw_h))
            overlay.lift()
        except Exception:
            pass

    def _navigation_update_blank_cover_overlay(self) -> None:
        if not self._navigation_active_cover_is_empty():
            self._navigation_destroy_blank_cover_overlay()
            return

        host = getattr(self, "_phase2_canvas_host", None)
        parent = getattr(host, "master", None) if host is not None else None
        if parent is None:
            # Canvas peut finir sa création quelques millisecondes après la
            # sélection de la couverture. Un seul rappel suffit.
            try:
                self.after(60, self._navigation_update_blank_cover_overlay)
            except Exception:
                pass
            return

        overlay = getattr(self, "_navigation_blank_cover_overlay", None)
        if overlay is None or getattr(self, "_navigation_blank_cover_parent", None) is not parent:
            self._navigation_destroy_blank_cover_overlay()
            overlay = tk.Frame(parent, bg=theme.WINDOW_DEEP, bd=0, highlightthickness=0)
            ...
            self._navigation_blank_cover_overlay = overlay
            self._navigation_blank_cover_page = page
            self._navigation_blank_cover_parent = parent
            try:
                overlay.bind("<Configure>", self._navigation_layout_blank_cover_overlay, add="+")
            except Exception:
                pass

        self._navigation_layout_blank_cover_overlay()
