...
class DocumentView:
    ...
    def _create_main_workspace(self, parent) -> ctk.CTkFrame:
        ...
        # REGULATION_ALERTES_COMPACTES_V1
        ctk.CTkLabel(
            side,
        ...
        for alert_row, (severity, title, text) in enumerate(alerts[:4]):
            _soft_color, accent_color = alert_colors[severity]
            alert = ctk.CTkFrame(
                alerts_frame,
                height=30,
                fg_color="#FAFBF9",
                corner_radius=5,
                border_width=1,
                border_color=blend(accent_color, 0.66),
            )
            alert.grid(
                row=alert_row,
                column=0,
            ...
            alert.grid_propagate(False)
            alert.grid_columnconfigure(2, weight=1)
