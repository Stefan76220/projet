...
def _run_text(run: ET.Element) -> tuple[str, list[dict[str, Any]]]:
    ...
    for node in run.iter():
        if node.tag in {_q("w", "t"), _q("w", "delText")}:
            ...
            controls.append({"type": "tab", "offset": offset})
            parts.append("\t")
            offset += 1
        elif node.tag in {_q("w", "br"), _q("w", "cr")}:
            br_type = _attr(node, "w", "type") or "line"
            if br_type == "page":
                controls.append({"type": "page_break", "offset": offset})
            elif br_type == "column":
                controls.append({"type": "column_break", "offset": offset})
            else:
                controls.append({"type": "line_break", "offset": offset})
            parts.append("\n")
            offset += 1
        elif node.tag == _q("w", "noBreakHyphen"):
            controls.append({"type": "no_break_hyphen", "offset": offset})
            parts.append("‑")
            offset += 1
        elif node.tag == _q("w", "softHyphen"):
            controls.append({"type": "soft_hyphen", "offset": offset})
            parts.append("\u00ad")
            offset += 1
        elif node.tag == _q("w", "instrText"):
            controls.append({"type": "field_instruction", "value": node.text or "", "offset": offset})
        elif node.tag == _q("w", "fldChar"):
            controls.append({"type": "field_char", "value": _attr(node, "w", "fldCharType"), "offset": offset})
