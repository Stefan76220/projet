...
class BookCanvas(tk.Frame):
    ...
    def structure_create_custom_type(self, definition) -> tuple[str, str] | None:
        ...
        saved = {
            ...
            "automatic": bool(definition.get("automatic", False)),
        }
    ...
    def structure_type_automatic_allowed(self, type_key: str) -> bool:
        ...
        return bool(key) and key not in protected

    def structure_type_is_automatic(self, type_key: str) -> bool:
        key = str(type_key or "").strip()
        if not self.structure_type_automatic_allowed(key):
            return False
        definition = self._structure_type_definition(key)
        if "automatic" in definition:
            return bool(definition.get("automatic"))
        data = self._data if isinstance(self._data, dict) else {}
        values = data.get("automatic_page_types", [])
        return isinstance(values, list) and key in {str(v) for v in values}

    def structure_set_type_automatic(self, type_key: str, enabled: bool) -> bool:
        key = str(type_key or "").strip()
        if not self.structure_type_automatic_allowed(key):
            self.status_var.set("Les couvertures ne peuvent pas être automatiques.")
            return False
        if self.project is None:
            return False
        try:
        ...
        if not isinstance(data, dict):
            data = {}
        values = data.get("automatic_page_types", [])
        current = {str(v) for v in values} if isinstance(values, list) else set()
        if enabled:
            current.add(key)
        else:
            current.discard(key)
        data["automatic_page_types"] = sorted(current)
        definitions = data.get("page_types", [])
        if isinstance(definitions, list):
            for definition in definitions:
                if isinstance(definition, dict) and str(definition.get("type") or definition.get("id") or "").strip() == key:
                    definition["automatic"] = bool(enabled)
                    break
        self._data = data
        self.project.save_mockup(data)
        if self.on_change is not None:
            self.on_change()
        self.status_var.set(f"{self._structure_type_definition(key).get('label', key)}  •  automatique {'activé' if enabled else 'désactivé'}")
        return True
    ...
    def _structure_global_click(self, event):
        """Un clic hors de B annule immédiatement toute brique armée."""
        if not getattr(self, "_structure_pending_kind", None):
            return
        widget = getattr(event, "widget", None)
        try:
            inside_canvas = widget is self.canvas or str(widget).startswith(str(self.canvas))
        except Exception:
    ...
    def _structure_page_hover_target(self, event):
        ...
        for group in self.groups:
            ...
            for local_pos in self._structure_valid_local_positions(group_id):
                ...
                candidates.append((abs(cx - tx), group_id, int(local_pos), tx))
        if not candidates:
            return None
        _distance, group_id, local_pos, tx = min(candidates, key=lambda item: item[0])
