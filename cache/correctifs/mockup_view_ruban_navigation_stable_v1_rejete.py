...
class MockupView:
    ...
    def _create_header(self, parent) -> ctk.CTkFrame:
        """Ruban de navigation stable du Bureau de maquettage."""
        frame = ctk.CTkFrame(
            parent,
            height=92,
            fg_color=self.WINDOW_BG,
        ...
        bar.grid(row=0, column=0, sticky="nsew", padx=12, pady=(6, 4))
        ...
        shortcuts.pack(anchor="w", pady=(6, 0))

        for label, color in (
            ("Visualisation", self.LILAC),
            ("Suivi du livre", self.CELADON),
        ):
            item = ctk.CTkFrame(shortcuts, fg_color="transparent")
        ...
        steps = (
            ("Accueil", self.INK, False, None),
            ("Centre", self.INK, False, self._go_back),
            ("Maquettage", self.SKY, True, None),
            ("Atelier", self.CELADON, False, None),
            ("Conception", self.LILAC, False, None),
            ("Assemblage", self.YELLOW, False, None),
            ("Vérification", "#6FA6A5", False, None),
            ("Finalisation", self.CORAL, False, None),
        )

        for label, color, active, command in steps:
            cell = ctk.CTkFrame(
                nav,
                width=82 if label != "Maquettage" else 96,
                height=70,
                fg_color="#E4EDF5" if active else "transparent",
                corner_radius=24 if active else 0,
            )
            ...
            btn = ctk.CTkButton(
                ...
                corner_radius=0,
                fg_color="transparent",
                hover_color="#EDF2F6" if command else "transparent",
                text_color=color,
                font=(Fonts.FAMILY, 8, "bold" if active else "normal"),
                command=command,
                state="normal" if command else "disabled",
            )
            btn.pack()

        # Droite : projet + fermer
    ...
    def _create_group_block(
        ...
        if deleted:
            ...
            trace.place(relx=0.5, rely=0.80, anchor="center")
            trace.pack_propagate(False)

            ctk.CTkLabel(
                trace,
                text=f"P{number_text}\nsuppr.",
                font=(Fonts.FAMILY, 7),
                text_color="#8C9198",
                justify="center",
                anchor="center",
