...
class BookView(CutPanel):
    """Zone B : représentation permanente et unique du livre."""

    def __init__(self, parent, on_open_item: Callable[[dict, int], None]):
        super().__init__(parent, fill=theme.PANEL, border=theme.BORDER, cut=18, padding=(18, 12))
        ...
        header = tk.Frame(self.body, bg=theme.PANEL)
        header.grid(row=0, column=0, sticky="ew", pady=(0, 8))
        header.grid_columnconfigure(1, weight=1)

        tk.Label(
            header,
            text="Le livre — vue permanente",
            bg=theme.PANEL,
            fg=theme.INK,
        ...
        tk.Label(
            header,
            text="Défilement horizontal  •  double-clic : consulter",
            bg=theme.PANEL,
            fg=theme.MUTED,
            font=(theme.FONT_UI, 8),
        ).grid(row=0, column=2, sticky="e")

        viewer = tk.Frame(self.body, bg=theme.WINDOW_DEEP)
    ...
    def render(self):
        ...
        h = max(120, self.canvas.winfo_height())
        ...
        if not self.items:
            ...
            self.canvas.create_text(
                w / 2,
                h / 2 + 18,
                text="Structure → gabarits → pages réelles → contrôle final",
                fill=theme.MUTED,
        ...
        self.status_var.set(f"{len(self.items)} page{'s' if len(self.items) > 1 else ''} dans la structure")
        page_w = 72
        page_h = 104
        gap = 22
        x = 28
        y = max(12, (h - page_h) / 2 - 2)
        ...
        total_w = max(w, x + 20)
        self.canvas.configure(scrollregion=(0, 0, total_w, h))
...
class TomeLineaV3(tk.Tk):
    ...
    def _build_workspace(self):
        ...
        # A, B et C sont placés une seule fois. Changer d'onglet ne
        # reconstruit ni A ni B.
        self.zone_a = CutPanel(screen, fill=theme.PANEL, border=theme.BORDER, cut=14, padding=(18, 10))
        self.zone_b = BookView(screen, self.open_consultation)
        ...
        def layout(event):
            ...
            gap = 10
            a_h = max(74, int(h * 0.105))
            b_h = max(210, int(h * 0.30))
            c_y = margin_y + a_h + gap + b_h + gap
            c_h = max(260, h - c_y - margin_y)

            self.zone_a.place(x=margin_x, y=margin_y, width=inner_w, height=a_h)
            self.zone_b.place(x=margin_x, y=margin_y + a_h + gap, width=inner_w, height=b_h)
    ...
    def show_workspace(self, project: Project):
        ...
        self.zone_b.set_project(project)
        self._refresh_workspace_state()
    ...
    def _refresh_workspace_state(self):
        ...
        page_count = len(self.zone_b.items)
    ...
    def open_consultation(self, item: dict, index: int):
        ...
        label = self.zone_b._item_label(item, index)
        tk.Label(
            body,
