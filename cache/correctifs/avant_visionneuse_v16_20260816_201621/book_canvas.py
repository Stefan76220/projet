...
class BookCanvas(tk.Frame):
    ...
    def _render_page_overlay(self):
        ...
        # Position latérale des informations : elles restent fixes et centrées
        # verticalement dans la fenêtre, indépendamment de la croissance de la page.
        # Ainsi elles ne sont ni poussées ni masquées par le zoom.
        info_gap = 34.0
        info_w_left = min(220.0, max(170.0, width * 0.16))
        info_w_right = min(245.0, max(180.0, width * 0.18))
        left_anchor = max(26.0, min(x - info_gap, width * 0.21))
        right_anchor = min(width - 26.0, max(x + page_w + info_gap, width * 0.79))
        left_cy = height * 0.36
        right_cy = height * 0.36
        ...
        # Légers voiles transparents pour garder les textes lisibles sans cadre lourd.
        # Ils restent hors de la page.
        left_box = (left_anchor - info_w_left, left_cy - 72, left_anchor + 10, left_cy + 72)
        right_box = (right_anchor - 10, right_cy - 78, right_anchor + info_w_right, right_cy + 78)
        target.create_rectangle(*left_box, fill="#1D2731", stipple="gray50", outline="")
        target.create_rectangle(*right_box, fill="#1D2731", stipple="gray50", outline="")

        target.create_text(left_anchor, left_cy - 52, text="TITRE", anchor="e", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(left_anchor, left_cy - 28, text=page_title, anchor="e", fill=theme.INK, font=(theme.FONT_TITLE, 15, "bold"), width=info_w_left)
        target.create_text(left_anchor, left_cy + 18, text="TYPE", anchor="e", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(left_anchor, left_cy + 42, text=page_type, anchor="e", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 10, "bold"), width=info_w_left)

        part_value = part_name
        if part_title and part_title != "Titre à définir":
            part_value = f"{part_name} — {part_title}"
        target.create_text(right_anchor, right_cy - 58, text="PARTIE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right_anchor, right_cy - 30, text=part_value, anchor="w", fill=theme.INK, font=(theme.FONT_TITLE, 14, "bold"), width=info_w_right)
        target.create_text(right_anchor, right_cy + 24, text="AVANCEMENT", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right_anchor, right_cy + 48, text=stage_text, anchor="w", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 11, "bold"))
        target.create_text(right_anchor, right_cy + 72, text=f"Page {index + 1} sur {len(self.items)}", anchor="w", fill=theme.MUTED, font=(theme.FONT_UI, 9))

        # Fils graphiques discrets reliant les infos à la zone centrale.
        line_y = height * 0.50
        if left_anchor < x - 18:
            target.create_line(left_anchor - info_w_left + 12, line_y, x - 22, line_y, fill=theme.BORDER_SOFT, width=1)
            target.create_oval(x - 27, line_y - 3, x - 21, line_y + 3, fill=theme.ACCENT_DARK, outline="")
        if right_anchor > x + page_w + 18:
            target.create_line(x + page_w + 22, line_y, right_anchor + info_w_right - 12, line_y, fill=theme.BORDER_SOFT, width=1)
