...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V2 : C contient seulement les outils ; B est la zone de travail."""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=0, minsize=96)
        parent.grid_columnconfigure(1, weight=1)

        # Rail gauche : familles seulement. Il reste volontairement très étroit.
        rail = tk.Frame(parent, bg=theme.PANEL_ALT, width=96, padx=6, pady=7)
        rail.grid(row=0, column=0, sticky="nsw", padx=(0, 8))
        ...
        # Le reste de C est une palette. Il n'y a plus de zone de construction dans C.
        palette = tk.Frame(parent, bg=theme.PANEL_ALT, padx=10, pady=7)
        ...
        palette_head.grid(row=0, column=0, sticky="ew", pady=(0, 7))
        ...
        tk.Label(
            palette_head,
            text="Prenez une brique et déposez-la dans B",
            bg=theme.PANEL_ALT,
            fg=theme.MUTED_DARK,
        ...
        families = {
            "Pages": ["Chapitre", "Texte", "Illustration", "Transition", "Annexe"],
            "Parties": ["+ Partie"],
            "Blancs": ["Page blanche"],
            "Règles": ["Recto / verso", "Avant / après"],
        }
        ...
        def select_family(name: str):
            ...
            for label in families.get(name, []):
                brick = tk.Button(
                    row,
                    text=label,
                ...
                brick.pack(side="left", padx=(0, 7), pady=(0, 6))

        for family in ("Pages", "Parties", "Blancs", "Règles"):
            button = tk.Button(
                rail,
                text=family,
                command=lambda name=family: select_family(name),
                bg=theme.PANEL_SOFT,
                fg=theme.INK,
                activebackground=theme.ACCENT_SOFT,
                activeforeground=theme.WHITE,
