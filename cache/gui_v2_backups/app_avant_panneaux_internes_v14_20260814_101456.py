...
class PageMaitreV2(tk.Tk):
    ...
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # Deux tailles finales, sans animation.
        # Les deux vues restent préconstruites ; seul le cadre prend
        # directement la dimension correspondant à l'onglet choisi.
        global_size = (
        ...
        def redraw_shell(_event=None):
            ...
            # Pendant la transformation de la fenêtre on évite le
            # redimensionnement PIL du décor à chaque image. Le cadre
            # reste vivant, mais son redraw est volontairement léger.
            if resize_fx["active"]:
            ...
            if win._book_views_bg_source is not None:
                try:
                    ...
                    win._book_views_bg_photo = ImageTk.PhotoImage(
                        image
                    )
                    outer.create_image(
                        0,
                        0,
                        image=win._book_views_bg_photo,
                        anchor="nw",
                        tags=("bookviews_bg",),
                    )
                except Exception:
        ...
        shell_redraw_job = [None]

        def schedule_shell_redraw(_event=None):
            """Regroupe les <Configure> successifs en un seul redraw final."""
            if shell_redraw_job[0] is not None:
                try:
                    win.after_cancel(shell_redraw_job[0])
                except Exception:
                    pass

            def flush_shell_redraw():
                shell_redraw_job[0] = None
                if win.winfo_exists():
                    redraw_shell()

            shell_redraw_job[0] = win.after_idle(
                flush_shell_redraw
            )

        outer.bind("<Configure>", schedule_shell_redraw)
        ...
        global_hint = tk.Label(
            global_host,
            text="•  Cliquez-glissez sur la frise pour parcourir le livre",
            bg=PANEL,
            fg="#7A7F84",
            font=("Segoe UI", 7),
        )
        ...
        panorama.configure(xscrollcommand=update_scroll)

        def global_scroll_geometry():
            w = max(1, global_scroll.winfo_width())
            left = 8
            right = w - 8
            ...
            thumb_w = min(track, thumb_w)
            travel = max(0.0, track - thumb_w)
        ...
        def draw_global_scroll():
            global_scroll.delete("all")
            (
                left,
                right,
                x1,
                ...
                _travel,
                _thumb_w,
            ) = global_scroll_geometry()

            yy = 14
            global_scroll.create_line(
                left,
                yy,
                right,
                yy,
        ...
        global_scroll.bind(
            "<Configure>",
            lambda _e: draw_global_scroll(),
        )
        global_scroll.bind(
            "<Enter>",
            lambda _e: (
                scroll_state.update(hover=True),
                draw_global_scroll(),
            ),
        )
        global_scroll.bind(
            "<Leave>",
            lambda _e: (
                scroll_state.update(hover=False),
                draw_global_scroll(),
            ),
        )
        global_scroll.bind(
        ...
        def panorama_pan_drag(event):
            ...
            # gain=1 : déplacement naturel, sans effet d'accélération brutal.
            panorama.scan_dragto(event.x, event.y, gain=1)
            return "break"
        ...
        global_redraw_job = [None]

        def schedule_global_redraw(_event=None):
            """Ne redessine la frise que si elle est la vue active."""
            if state["tab"] != "global":
                return

            if global_redraw_job[0] is not None:
                try:
                    win.after_cancel(global_redraw_job[0])
                except Exception:
                    pass

            def flush_global_redraw():
                global_redraw_job[0] = None
                if (
                    win.winfo_exists()
                    and state["tab"] == "global"
                ):
                    draw_global()
                    draw_global_scroll()

            global_redraw_job[0] = win.after_idle(
                flush_global_redraw
            )

        panorama.bind(
            "<Configure>",
            schedule_global_redraw,
        )
        ...
        def draw_spread(
            ...
            *,
            ...
            if left_page is not None and left_number is not None:
                book_canvas.create_text(
                    left_x + page_w / 2,
                    y1 + page_h + 28,
                    text=f"p. {left_number}",
                    fill="#70777C",
                    font=("Segoe UI", 8),
                    anchor="center",
                )

            if right_page is not None and right_number is not None:
                book_canvas.create_text(
                    right_x + page_w / 2,
                    y1 + page_h + 28,
                    text=f"p. {right_number}",
                    fill="#70777C",
                    font=("Segoe UI", 8),
                    anchor="center",
                )
        ...
        book_redraw_job = [None]

        def schedule_book_redraw(_event=None):
            """Ne redessine le livre que si Feuilleter est actif."""
            if state["tab"] != "feuilleter":
                return

            if book_redraw_job[0] is not None:
                try:
                    win.after_cancel(book_redraw_job[0])
                except Exception:
                    pass

            def flush_book_redraw():
                book_redraw_job[0] = None
                if (
                    win.winfo_exists()
                    and state["tab"] == "feuilleter"
                    and not flip_state["active"]
                ):
                    render_book()

            book_redraw_job[0] = win.after_idle(
                flush_book_redraw
            )

        book_canvas.bind(
            "<Configure>",
            schedule_book_redraw,
        )
        book_canvas.bind("<Button-1>", book_click)

        # ----------------------------------------------------------
        # Changement de vue V13 — responsabilités séparées.
        #
        # 1. set_book_view_size() ne fait QUE la géométrie.
        # 2. show_book_view() ne fait QUE changer la vue.
        # 3. change_book_mode() orchestre les deux sans aucun redraw forcé.
        #
        # Les <Configure> sont coalescés avec after_idle et seule la vue
        # active est redessinée une fois la nouvelle taille réellement posée.
        # ----------------------------------------------------------
        switch_state = {
            "active": False,
        }

        def _target_geometry_for(key):
            target_w, target_h = (
                global_size
                if key == "global"
                else book_size
            )
            target_x = max(0, int((screen_w - target_w) / 2))
            target_y = max(0, int((screen_h - target_h) / 2))
            return (
                target_w,
                target_h,
                target_x,
        ...
        def set_book_view_size(key):
            """Responsabilité unique : taille et position de la fenêtre."""
            target_w, target_h, target_x, target_y = (
                _target_geometry_for(key)
            )
        ...
        def show_book_view(key):
            """Responsabilité unique : vue active et habillage d'onglet."""
            state["tab"] = key
        ...
        def change_book_mode(key):
            """Orchestre taille puis vue, sans piloter le rendu."""
            ...
            try:
                # La taille est demandée en premier. Aucun update() et aucun
                # redraw n'est forcé : Tk traite ensuite la nouvelle géométrie
                # en un cycle naturel de <Configure>.
                set_book_view_size(key)
                show_book_view(key)
            finally:
                switch_state["active"] = False

        def show_tab(key, *, animate=True):
            # Compatibilité avec les appels historiques de la visionneuse.
        ...
        # Pré-chauffage identique au principe de navigation stable de
        # TomeLinea : les DEUX vues sont terminées avant la révélation.
        win.update_idletasks()
        redraw_shell()
        draw_title_line()
        draw_global()
        ...
        # Les deux vues sont déjà construites. Le changement d'onglet
        # restera synchrone, avec une seule géométrie finale.
        if state["tab"] == "global":
            global_host.tkraise()
        else:
            book_host.tkraise()
