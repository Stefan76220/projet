...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        """Accueil inspiré du visuel TomeLinea validé."""
        ...
        # Zone bienvenue : aucun fond ajouté sur le décor.
        # Les textes sont dessinés directement sur le Canvas éditorial.
        content.create_text(
            150,
            12,
        ...
        grid.pack(fill="both", expand=True, padx=24, pady=(102, 8))
        ...
        # --- HOME_FROSTED_GLASS_HELPERS_START ---
        ...
        def _blend_rgb(a, b, t: float):
            t = max(0.0, min(1.0, float(t)))
            return tuple(
                int(round(a[i] * (1.0 - t) + b[i] * t))
                for i in range(3)
            )

        def _home_glass_photo(kind: str, width: int, height: int, color: str, state: str):
            """Plaque de verre dépoli très claire, teintée par la couleur réelle de sa zone."""
            if not hasattr(self, "_home_glass_cache"):
                self._home_glass_cache = {}

            width = max(8, int(round(width)))
            height = max(8, int(round(height)))
            key = ("zone_text_tone_v3", kind, width, height, color, state)
            cached = self._home_glass_cache.get(key)
            ...
            if kind == "choice":
                ...
                glass_alpha = 108
                stroke_alpha = 74
                grain_step_x, grain_step_y = 13, 10
            elif kind == "shortcut":
                radius = 12
                back_alpha = 32
                glass_alpha = 104
                stroke_alpha = 70
                grain_step_x, grain_step_y = 12, 10
            elif kind == "chip":
                radius = max(8, height // 2)
                back_alpha = 28
                glass_alpha = 98
                stroke_alpha = 66
                grain_step_x, grain_step_y = 12, 9
            else:
                radius = 12
                back_alpha = 30
                glass_alpha = 102
                stroke_alpha = 68
                grain_step_x, grain_step_y = 11, 9
            ...
            warm_white = (252, 249, 244)

            # La couleur réelle de la zone reste identifiable,
            # mais elle est très fortement diluée.
            back_rgb = _blend_rgb(color_rgb, warm_white, 0.84)
            glass_rgb = _blend_rgb(color_rgb, warm_white, 0.94)
            edge_rgb = _blend_rgb(color_rgb, warm_white, 0.66)

            grain_light = _blend_rgb(warm_white, glass_rgb, 0.12)
            grain_dark = _blend_rgb((226, 221, 214), color_rgb, 0.07)

            img = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            draw = ImageDraw.Draw(img)

            # Fond teinté très clair derrière la plaque.
            draw.rounded_rectangle(
                (1, 1, width - 2, height - 2),
                radius=radius,
                fill=back_rgb + (back_alpha,),
                outline=edge_rgb + (stroke_alpha,),
                width=1,
            )

            # Verre dépoli très clair.
            inset = 2
            draw.rounded_rectangle(
                (inset, inset, width - inset - 1, height - inset - 1),
                radius=max(4, radius - 2),
                fill=glass_rgb + (glass_alpha,),
                outline=(255, 253, 249, 72),
                width=1,
            )

            # Lumière diffuse ultra légère venant du haut.
            top = inset + 1
            bottom = height - inset - 2
            usable_h = max(1, bottom - top + 1)
            for yy in range(top, bottom + 1):
                p = (yy - top) / usable_h
                alpha = int(6 * (1.0 - p) + 1)
                wash_rgb = _blend_rgb(warm_white, glass_rgb, 0.15 + 0.18 * p)
                draw.line(
                    (inset + 5, yy, width - inset - 6, yy),
                    fill=wash_rgb + (alpha,),
                    width=1,
                )

            # Grain dépoli fin, presque mat.
            for gx in range(10, width - 10, grain_step_x):
                for gy in range(8, height - 8, grain_step_y):
                    seed = ((gx * 17 + gy * 23 + width * 3 + height) % 11)
                    if seed in (0, 4, 8):
                        dot_fill = grain_light + (17 if seed == 0 else 12,)
                    else:
                        dot_fill = grain_dark + (8 if seed in (2, 6) else 5,)
                    draw.ellipse(
                        (gx, gy, gx + 1, gy + 1),
                        fill=dot_fill,
                        outline=None,
            ...
            draw.rounded_rectangle(
                (inset + 1, inset + 1, width - inset - 2, height - inset - 2),
            ...
            self._home_glass_cache[key] = photo
            return photo



        def _draw_home_glass_plate(canvas, tag: str, x1: float, y1: float, x2: float, y2: float, color: str, state: str, kind: str):
            width = max(1, int(round(x2 - x1)))
            height = max(1, int(round(y2 - y1)))
            photo = _home_glass_photo(kind, width, height, color, state)
            canvas.create_image(
                x1,
                y1,
                image=photo,
        ...
        # --- HOME_FROSTED_GLASS_HELPERS_END ---
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton : plaque réaliste de verre dépoli sur fond coloré."""
            ...
            shadow = self._mix(color, "#E3DED7", 0.90 if state == "hover" else 0.92)
            text_color = self._mix(color, "#28322D", 0.58)
            ...
            _draw_home_glass_plate(
                canvas, tag, x1, y1, x2, y2, color, state, "button"
            )
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Carte de choix : plaque réaliste de verre dépoli sur fond coloré."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            shadow = self._mix(color, "#E3DED7", 0.90 if hover else 0.92)
            if selected:
                shadow = self._mix(color, "#DDD8D1", 0.88 if hover else 0.90)
            ...
            plate_state = "hover" if hover else ("pressed" if pressed else "normal")
            if selected and plate_state == "normal":
                plate_state = "hover"

            _draw_home_glass_plate(
                canvas, tag, x1, y1, x2, y2, color, plate_state, "choice"
            )
            ...
            if selected:
                canvas.create_line(
                    x1 + 20, y2 - 3,
                    x2 - 20, y2 - 3,
                    fill=self._mix(color, "#D9D2C9", 0.25),
                    width=2,
                    capstyle="round",
                    tags=(tag, "home_choice"),
                )
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule info : plaque de verre dépoli sur fond coloré."""
            width = max(64, int(len(text) * 5.0) + 31)
            height = 20

            _draw_home_glass_plate(
                canvas, tag, x, y, x + width, y + height, fg, "normal", "chip"
            )
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Raccourci : plaque réaliste de verre dépoli sur fond coloré."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            _draw_home_glass_plate(
                canvas, tag, x1, y1, x2, y2, color, state, "shortcut"
            )
        ...
        def home_hover_cursor(
            ...
            *,
            ...
            """Interaction fiable : aucun redraw pendant l'appui souris."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            def leave(_event=None):
                # Pendant un clic, ne détruit pas l'objet sous la souris.
                if self._home_interaction_state.get(tag) == "pressed":
                    return
            ...
            def press(_event=None):
                # Très important : pas de redraw ici.
                # Le même item reste vivant jusqu'au ButtonRelease.
    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        """Maquettage remis au niveau visuel TomeLinea."""
        ...
        self._maquettage_content = content
        content_window = page.create_window(0, 8, window=content, anchor="n")

        def paint_layer(target, key):
            if self._maquettage_bg_source is None:
                return
            width = max(target.winfo_width(), 1)
            height = max(target.winfo_height(), 1)
            ...
            photo = ImageTk.PhotoImage(image)
            self._maquettage_layer_photos[key] = photo
            target.delete(f"{key}_bg")
            target.create_image(0, 0, image=photo, anchor="nw", tags=f"{key}_bg")
            target.tag_lower(f"{key}_bg")
        ...
        def resize_page(event):
            if self._maquettage_bg_source is not None:
                ...
                self._maquettage_bg_photo = ImageTk.PhotoImage(image)
                page.itemconfigure("maquettage_bg", image=self._maquettage_bg_photo)
                page.tag_lower("maquettage_bg")

            content_width = min(max(event.width - 120, 1020), 1500)
            content_height = max(event.height - 16, 700)
            page.coords(content_window, event.width / 2, 8)
            page.itemconfigure(content_window, width=content_width, height=content_height)
            content.after_idle(lambda: paint_layer(content, "content"))

        page.bind("<Configure>", resize_page)
        content.bind(
            "<Configure>",
            lambda _event: paint_layer(content, "content"),
            add="+",
        ...
        def draw_soft_block(canvas, x1, y1, x2, y2, color, title, subtitle="", *, circle=False):
            face = self._mix(color, "#FFF8F1", 0.86)
            outline = self._mix(color, "#FFFFFF", 0.20)
            inner = self._mix(color, "#FFFFFF", 0.88)
            shadow = self._mix(color, "#6E6A66", 0.42)
            text_color = self._mix(color, theme.INK, 0.48)
            sub_color = self._mix(color, theme.INK, 0.72)
            top_light = self._mix(color, "#FFFFFF", 0.95)
            glass_line = self._mix(color, "#FFFFFF", 0.78)
            ...
            if circle:
                canvas.create_oval(
                    x1 + 10, y1 + 11, x1 + 22, y1 + 23,
                    outline=color,
                    width=1,
                    fill=self._mix(color, "#FFFFFF", 0.90),
                    tags="pm_zone",
                )
                tx = x1 + 28
                anchor = "w"
            ...
            if subtitle:
                canvas.create_text(
                    ...
                    font=("Segoe UI", 8),
                    anchor=anchor,
                    width=max(10, int(x2 - x1 - 26)),
                    tags="pm_zone",
                )

        def draw_command(canvas, x1, y1, x2, y2, color, text):
            face = self._mix(color, "#FFF8F1", 0.85)
            outline = self._mix(color, "#FFFFFF", 0.22)
            shadow = self._mix(color, "#706962", 0.40)
            text_color = self._mix(color, theme.INK, 0.54)
            rounded(canvas, x1 + 1, y1 + 3, x2 + 1, y2 + 3, 12, fill=shadow, outline="", tags="pm_button")
            rounded(canvas, x1, y1, x2, y2, 12, fill=face, outline=outline, width=1, tags="pm_button")
            canvas.create_line(x1 + 16, y1 + 8, x2 - 16, y1 + 8, fill=self._mix(color, "#FFFFFF", 0.94), width=1, tags="pm_button")
            canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2 - 1, text=text, fill=text_color, font=("Segoe UI", 9, "bold"), tags="pm_button")

        # En-tête
        header = tk.Frame(content, bg="#F8F4EC")
        header.pack(fill="x", padx=34, pady=(10, 8))

        texts = tk.Frame(header, bg="#F8F4EC")
        texts.pack(side="left")
        tk.Label(
            texts,
            text="Maquettage",
        ...
        steps = tk.Frame(texts, bg="#F8F4EC")
        steps.pack(anchor="w", pady=(4, 0))
        ...
        tk.Button(
            ...
            padx=18,
            pady=10,
            bg=self._mix(theme.SKY, "#FFF8F1", 0.88),
            fg=self._mix(theme.SKY, theme.INK, 0.56),
            activebackground=self._mix(theme.SKY, "#FFF8F1", 0.82),
            activeforeground=self._mix(theme.SKY, theme.INK, 0.52),
            highlightthickness=1,
            highlightbackground=self._mix(theme.SKY, "#FFFFFF", 0.24),
            font=("Segoe UI", 9, "bold"),
            cursor="hand2",
        ).pack(side="right", padx=4, pady=8)

        body = tk.Frame(content, bg="#F6F2EA")
        body.pack(fill="both", expand=True, padx=16, pady=(0, 10))
        body.grid_columnconfigure(0, weight=255)
        body.grid_columnconfigure(1, weight=560)
        body.grid_columnconfigure(2, weight=220)
        body.grid_rowconfigure(3, weight=1)

        # 1 — Structurer le livre
        skeleton = self._rounded_home_card(
            body,
            title="1 — Structurer le livre",
            accent=theme.LILAC,
            bg="#FBF8FD",
            title_align="left",
            subtitle="Début du livre, parties numérotées, groupes libres et fin du livre.",
        )
        skeleton.configure(height=172)
        skeleton.grid(row=0, column=0, columnspan=3, sticky="ew", padx=6, pady=(0, 10))

        def redraw_skeleton(_event=None):
            c = skeleton
            c.delete("pm_zone")
            width = max(c.winfo_width(), 880)
            margin = 22
            top = 76
            gap = 12
            names = [
                ("Début", theme.SKY),
                ("Partie 1", theme.LILAC),
                ("Annexe", theme.CELADON),
                ("Partie 2", theme.CORAL),
                ("Partie 3", theme.SKY),
                ("Fin", theme.LILAC),
            ]
            item_w = max(104, int((width - 2 * margin - gap * (len(names) - 1)) / len(names)))
            x = margin
            for name, color in names:
                draw_soft_block(c, x, top, x + item_w, top + 56, color, name)
                x += item_w + gap

        skeleton.bind("<Configure>", redraw_skeleton, add="+")
        skeleton.after_idle(redraw_skeleton)

        # 2 — Vue globale
        global_view = self._rounded_home_card(
            body,
            title="2 — Vue globale",
            accent=theme.CELADON,
            bg="#F7FBF8",
            title_align="left",
            subtitle="Vision linéaire de la structure et de l’ampleur du travail.",
        )
        global_view.configure(height=150)
        global_view.grid(row=1, column=0, columnspan=3, sticky="ew", padx=6, pady=(0, 10))

        def redraw_global(_event=None):
            c = global_view
            c.delete("pm_zone")
            width = max(c.winfo_width(), 880)
            left = 58
            right = width - 58
            y = 92
            c.create_line(left, y, right, y, fill="#9CCCB9", width=4, capstyle="round", tags="pm_zone")
            groups = [
                ("Début", 3, theme.SKY),
                ("P1", 20, theme.LILAC),
                ("Annexe", 4, theme.CELADON),
                ("P2", 14, theme.CORAL),
                ("P3", 8, theme.SKY),
                ("Fin", 3, theme.LILAC),
            ]
            step = (right - left) / (len(groups) - 1)
            for i, (label, count, color) in enumerate(groups):
                x = left + step * i
                c.create_oval(x - 8, y - 8, x + 8, y + 8, fill=color, outline="#FFFFFF", width=2, tags="pm_zone")
                c.create_text(x, y - 24, text=label, fill=self._mix(color, theme.INK, 0.48), font=("Segoe UI", 9, "bold"), tags="pm_zone")
                c.create_text(x, y + 18, text=f"{count} p.", fill="#4E5A66", font=("Segoe UI", 8), tags="pm_zone")

        global_view.bind("<Configure>", redraw_global, add="+")
        global_view.after_idle(redraw_global)

        # 3 colonnes du bas
        left = self._rounded_home_card(
            body,
            title="Types de page",
            accent=theme.SKY,
            bg="#F7FAFD",
            title_align="left",
            subtitle="Choix du type, quantité et informations de suivi.",
        )
        left.configure(height=320)
        left.grid(row=2, column=0, sticky="nsew", padx=(6, 4), pady=(0, 8))

        center = self._rounded_home_card(
            body,
            title="3 — Composition du groupe",
            accent=theme.INK,
            bg="#FBFAF7",
            title_align="left",
            subtitle="Séquence des pages du groupe sélectionné.",
        )
        center.configure(height=320)
        center.grid(row=2, column=1, sticky="nsew", padx=4, pady=(0, 8))

        right = self._rounded_home_card(
            body,
            title="Outils",
            accent=theme.CORAL,
            bg="#FDF8F5",
            title_align="left",
            subtitle="Commandes présentes mais encore inactives.",
        )
        right.configure(height=320)
        right.grid(row=2, column=2, sticky="nsew", padx=(4, 6), pady=(0, 8))

        def redraw_left(_event=None):
            c = left
            c.delete("pm_zone")
            width = max(c.winfo_width(), 240)
            labels = [
                "Couverture",
                "Page de titre",
                "Sommaire",
            ...
            y = 84
            for label in labels:
                draw_soft_block(c, 16, y, width - 16, y + 38, theme.SKY, label)
                y += 44

        def redraw_center(_event=None):
            c = center
            c.delete("pm_zone")
            ...
            for index in range(6):
                x1 = start_x + index * (page_w + gap)
                x2 = x1 + page_w
                y2 = y + page_h
                face = self._mix(theme.INK, "#FFF9F0", 0.92)
                rounded(c, x1 + 1, y + 3, x2 + 1, y2 + 3, 12, fill=self._mix(theme.INK, "#766F67", 0.42), outline="", tags="pm_zone")
                rounded(c, x1, y, x2, y2, 12, fill=face, outline=self._mix(theme.INK, "#FFFFFF", 0.24), width=1, tags="pm_zone")
                c.create_line(x1 + 12, y + 10, x2 - 12, y + 10, fill=self._mix(theme.INK, "#FFFFFF", 0.92), width=1, tags="pm_zone")
                c.create_rectangle(x1 + 17, y + 18, x2 - 17, y2 - 24, outline=self._mix(theme.INK, "#D5D0C8", 0.45), width=1, tags="pm_zone")
                c.create_text((x1 + x2) / 2, y2 - 12, text=str(index + 1), fill="#6A7480", font=("Segoe UI", 8, "bold"), tags="pm_zone")

        def redraw_right(_event=None):
            c = right
            c.delete("pm_zone")
            width = max(c.winfo_width(), 210)
            y = 86
            for label in ("Monter", "Descendre", "Dupliquer", "Supprimer", "Règles"):
                draw_command(c, 18, y, width - 18, y + 38, theme.CORAL, label)
                y += 47

        left.bind("<Configure>", redraw_left, add="+")
        center.bind("<Configure>", redraw_center, add="+")
        right.bind("<Configure>", redraw_right, add="+")
        left.after_idle(redraw_left)
        center.after_idle(redraw_center)
        right.after_idle(redraw_right)
