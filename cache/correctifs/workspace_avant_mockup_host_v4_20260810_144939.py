...
from src.gui.persistent_view_host import PersistentViewHost
...
class Workspace:
    ...
    def __init__(
        ...
        # SOCLE_AFFICHAGE_PERSISTANT_V1
        # Tous les grands écrans vivent désormais dans une même pile.
        self.host = PersistentViewHost(
            self.frame,
            fg_color=Colors.WINDOW,
        )
    ...
    def clear(self) -> None:
        ...
        Les transitions normales n'utilisent plus cette méthode. Elle ne
        détruit que le contenu de l'écran actif, jamais tous les bureaux.
        """
        active = self.host.active_name
        if active is not None:
            self.host.clear_surface(active)

    def show_dashboard(self) -> None:
        """
        Retour Accueil instantané.

        Si l'Accueil existe déjà, on le remet simplement au premier plan.
        Il n'est plus détruit/reconstruit lors du retour depuis le Centre.
        """
        self._destroy_cleanup_view()
        self._set_navigation_visible(False)

        if self.host.has("dashboard"):
            self.host.show("dashboard")
            return

        recent_projects = self._load_recent_projects()
        active_project = recent_projects[0] if recent_projects else None

        def build(surface) -> None:
            DashboardView(
                surface,
        ...
        self.host.rebuild("dashboard", build)
        self.host.show("dashboard")
    ...
    def show_documents(
        ...
        """
        Réagit immédiatement au clic puis construit le Centre sans exposer
        ...
        # Surface Centre persistante.
        surface = self.host.surface("documents")

        # Réponse visuelle immédiate : on ne laisse plus l'ancien écran
        # figé pendant toute la construction du Centre.
        ...
        loading = ctk.CTkLabel(
            shell,
            text="Ouverture du Centre…",
            font=("Segoe UI", 14, "bold"),
            text_color="#56606B",
        )
        loading.place(relx=0.5, rely=0.5, anchor="center")

        self.host.show("documents")
        surface.update_idletasks()

        def build_centre() -> None:
            try:
                for child in tuple(surface.winfo_children()):
                    child.destroy()
                ...
                if target_workspace:
                    self.frame.after_idle(
                        lambda key=target_workspace:
                        self._open_target_workspace(key)
                    )
            finally:
                self._documents_transitioning = False

        # Laisse Tk afficher le retour de clic avant le travail lourd.
        self.frame.after(1, build_centre)
    ...
    def show_document(
        ...
        """Ouvre l'éditeur sans vider le Centre déjà construit."""
        self._destroy_cleanup_view()
        self._set_navigation_visible(True)
        ...
        self.host.rebuild("document_editor", build)
        self.host.show("document_editor")

    def back_to_documents(self) -> None:
        """Retour instantané vers le Centre déjà construit."""
        if self.current_project is None:
            return

        if self.host.has("documents"):
            self._set_navigation_visible(True)
            self.host.show("documents")
            return
    ...
    def show_project_cleanup(self) -> None:
        """Affiche le nettoyage dans son propre écran, sans vider le Centre."""
        if (
        ...
        try:
            ...
            self.host.rebuild("cleanup", build)
            self.host.show("cleanup")
