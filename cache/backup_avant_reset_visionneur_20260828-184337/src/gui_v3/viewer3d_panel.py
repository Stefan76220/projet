...
class Viewer3DOverlay(tk.Frame):
    ...
    def _ensure_webview(self) -> bool:
        ...
        viewer_entry = WEB_ROOT / "index_v213.html"
        if not viewer_entry.is_file():
            self._failed = True
    ...
    def _prepare_pages_info(self, pages_info: list[dict] | None) -> list[dict]:
        """Place les rendus Production dans l'app locale du Visionneur.

        Le livre 3D reste le moteur validé. Seules les images des pages sont
        externalisées en PNG locaux versionnés par leur contenu ; aucune image
        Base64 ne traverse WebView2 et aucun canal RPC supplémentaire n'est
        nécessaire.
        """
        rows = [dict(value) for value in (pages_info or []) if isinstance(value, dict)]
        texture_dir = WEB_ROOT / "_page_textures"
        try:
        ...
        for row in rows:
            ...
            try:
                ...
                digest = hashlib.sha1(png).hexdigest()[:20]
                filename = f"page_{page_no:05d}_{digest}.png"
                target = texture_dir / filename
                if not target.is_file() or target.stat().st_size != len(png):
                    temp = target.with_suffix(".tmp")
                    temp.write_bytes(png)
                    temp.replace(target)
                row["textureUrl"] = f"_page_textures/{filename}"
            except Exception:
                pass
            finally:
                # Le gros Base64 ne doit jamais être injecté dans JavaScript.
                row.pop("textureData", None)

        try:
            files = sorted(
                (f for f in texture_dir.glob("page_*.png") if f.is_file()),
