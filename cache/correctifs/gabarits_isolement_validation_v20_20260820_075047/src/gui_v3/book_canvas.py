...
class BookCanvas(tk.Frame):
    ...
    def gabarit_toggle_finished(self) -> bool:
        ...
        La portée « Toutes du type » fonctionne comme un vrai gabarit parent :
        la page active reste un brouillon local pendant le travail, puis devient la
        référence canonique uniquement au passage à Terminé.
        """
        active = self._gabarit_active_item()
        ...
        if scope == "type":
            expected = max(1, len(self._gabarit_same_type_target_indices()))
            published = self._gabarit_publish_active_template_to_type()
            if published < expected:
                self.status_var.set(
                    f"Publication incomplète : {published}/{expected} pages. Le gabarit reste En cours."
                )
                # La page source doit rester clairement non validée en cas d'échec.
                active = self._gabarit_active_item()
                if isinstance(active, dict):
                    active["gabarit_scope_last"] = "type"
                    active["gabarit_scope_active"] = True
            ...
            self._emit_gabarit_selection_changed()
            self.status_var.set(
                f"Gabarit terminé — {published} pages du même type publiées"
                if published > 1 else "Gabarit terminé"
            )
            return True
    ...
    def _gabarit_same_type_target_indices(self) -> list[int]:
        """Retourne les surfaces manuelles qui portent exactement le même type.

        On ne dépend plus de l'état temporaire de la navigation Gabarits. Pour une
        double page soudée, seule sa page primaire porte le gabarit de la surface.
        """
        active = self._gabarit_active_item()
        if active is None:
            return []
    ...
    def _gabarit_publish_active_template_to_type(self) -> int:
        """Publie la page active comme gabarit canonique du type.

        C'est volontairement le passage à « Terminé » — et lui seul — qui publie.
        Le travail en cours reste local sur la page source jusqu'à cette validation.
        """
        active = self._gabarit_active_item()
        if active is None or self._selected_index is None:
            return 0
        ...
        # Transaction logique : si la sauvegarde ou sa vérification échoue, on
        # restaure l'état antérieur au lieu de laisser un demi-gabarit publié.
        before_items = deepcopy(self.items)
        before_registry = deepcopy(self._gabarit_type_templates())

        source_zones = active.get("gabarit_zones")
        if not isinstance(source_zones, list):
            source_zones = []
            active["gabarit_zones"] = source_zones
        # Donne des slot_key stables à la source avant la photographie canonique.
        ...
        if not self._gabarit_save_model_state():
            self.items = before_items
            if not isinstance(getattr(self, "_data", None), dict):
                self._data = {}
            self._data[self.GABARIT_TYPE_TEMPLATES_KEY] = before_registry
            return 0

        # Vérification après la vraie sauvegarde : chaque page ciblée doit exister,
        # être verte ET porter exactement la photographie publiée.
        by_id = {str(item.get("id") or "").strip(): item for item in self.items if isinstance(item, dict)}
        verified = 0
        for page_id in target_ids:
            item = by_id.get(page_id)
            if not isinstance(item, dict):
                continue
            if self._gabarit_status(item) != "termine":
                continue
            if int(item.get("gabarit_template_version", 0) or 0) != version:
                continue
            if self._gabarit_template_zone_snapshot(item.get("gabarit_zones")) != template_zones:
                continue
            current_settings = item.get("gabarit_page_settings")
            current_settings = current_settings if isinstance(current_settings, dict) else {}
            if current_settings != page_settings:
                continue
            verified += 1

        if verified != len(target_ids):
            self.items = before_items
            if not isinstance(getattr(self, "_data", None), dict):
                self._data = {}
            self._data[self.GABARIT_TYPE_TEMPLATES_KEY] = before_registry
    ...
    def gabarit_set_scope(self, scope: str) -> bool:
        """Engage la portée en deux clics, sans propager avant « Terminé ».

        1er clic : le bouton affiche « Confirmer ».
        2e clic : la page source passe En cours/orange et mémorise la portée.
        La publication vers toutes les pages du type n'arrive qu'au clic Terminé.
        """
        ...
        if self._gabarit_scope_is_active(active) and last_scope == requested:
            self._gabarit_scope_pending = ""
            self.status_var.set(
                "Portée déjà confirmée : toutes les pages du type seront publiées à Terminé."
                if requested == "type"
        ...
        if str(getattr(self, "_gabarit_scope_pending", "")) != requested:
            ...
            if requested == "type":
                count = max(1, len(self._gabarit_same_type_target_indices()))
                label = self._page_type_label(active, int(self._selected_index))
                self.status_var.set(
                    f"Confirmer : publier ce gabarit sur {count} pages « {label} » à Terminé."
                    if count > 1
                    else f"Confirmer : utiliser ce gabarit pour le type « {label} »."
                )
            else:
                self.status_var.set("Confirmer : valider ce gabarit uniquement pour cette page.")
        ...
        for idx in targets:
            item = self.items[idx]
            item["gabarit_scope_last"] = requested
        ...
        if requested == "type":
            count = max(1, len(self._gabarit_same_type_target_indices()))
            self.status_var.set(f"En cours — publication sur {count} pages au passage à Terminé.")
        else:
            self.status_var.set("En cours — gabarit limité à cette page.")
        if getattr(self, "_work_mode", "") == "gabarits":
            self.render()
        return True
    ...
    def _structure_insert_page(self, page_type: str, label: str, group_id: str, local_pos: int) -> int | None:
        ...
        # Si ce type possède déjà un gabarit validé « Toutes du type », une page
        # créée plus tard l'hérite immédiatement : c'est le lien de suivi du type.
