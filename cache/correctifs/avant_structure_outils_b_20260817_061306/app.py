...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V1 : trois zones compactes, sans logique métier."""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=0, minsize=128)
        parent.grid_columnconfigure(1, weight=1)
        parent.grid_columnconfigure(2, weight=0, minsize=238)

        # --- Gauche : familles de briques. Strict minimum de largeur.
        left = tk.Frame(parent, bg=theme.PANEL_ALT, width=128, padx=7, pady=8)
        ...
        # --- Centre : zone de construction. Elle prend tout l'espace restant.
        center = tk.Frame(parent, bg=theme.WINDOW_DEEP, padx=8, pady=8)
        center.grid(row=0, column=1, sticky="nsew")
        center.grid_rowconfigure(1, weight=1)
        center.grid_columnconfigure(0, weight=1)

        center_head = tk.Frame(center, bg=theme.WINDOW_DEEP)
        ...
        tk.Label(
            center_head,
            text="Partie sélectionnée dans B",
            bg=theme.WINDOW_DEEP,
            fg=theme.MUTED_DARK,
        ...
        def draw_work_area(_event=None):
            work.delete("all")
            w = max(320, work.winfo_width())
            h = max(120, work.winfo_height())
            cy = h / 2.0
            # Ligne très légère : futur emplacement des pages déposées.
            work.create_line(34, cy + 32, w - 34, cy + 32, fill=theme.BORDER_SOFT, width=1)
            for i, label in enumerate(("Page", "Page", "Page")):
                pw, ph = 48, 68
                gap = 18
                total = pw * 3 + gap * 2
                x = (w - total) / 2.0 + i * (pw + gap)
                y = cy - ph / 2.0
                work.create_rectangle(x, y, x + pw, y + ph, fill=theme.PANEL_SOFT, outline=theme.BORDER, width=1)
                work.create_text(x + pw / 2.0, y + ph + 14, text=label, fill=theme.MUTED_DARK, font=(theme.FONT_UI, 6))
            work.create_text(
                w / 2.0,
                max(18, cy - 58),
                text="Déposez ici les briques choisies à droite",
                fill=theme.MUTED,
                font=(theme.FONT_UI, 8),
            )

        work.bind("<Configure>", draw_work_area)

        # --- Droite : palette contextuelle. Largeur minimale utile.
        right = tk.Frame(parent, bg=theme.PANEL_ALT, width=238, padx=9, pady=8)
        right.grid(row=0, column=2, sticky="nse", padx=(7, 0))
        right.grid_propagate(False)

        palette_title_var = tk.StringVar(value="Ajouter")
        tk.Label(
            right,
            textvariable=palette_title_var,
            bg=theme.PANEL_ALT,
            fg=theme.ACCENT_BRIGHT,
            font=(theme.FONT_UI, 8, "bold"),
        ).pack(anchor="w", padx=2, pady=(0, 7))

        palette_body = tk.Frame(right, bg=theme.PANEL_ALT)
        palette_body.pack(fill="both", expand=True)

        palettes = {
            "Ajouter": ["+ Partie", "+ Page", "+ Blanc"],
            "Pages": ["Page de titre", "Sommaire", "Tête de partie", "Chapitre", "Texte", "Illustration", "Transition", "Annexe"],
            "Types": ["Créer un type", "Modifier un type", "Dupliquer un type", "Voir les types"],
            "Règles": ["Recto-verso", "Avant / après", "Verrouiller", "Voir les règles"],
            "Blancs": ["Blanc manuel", "Blanc automatique", "Règles de blanc"],
            "Catalogue": ["Tous les types", "Types personnalisés", "Types utilisés"],
        }
        ...
        def show_palette(name: str):
            ...
            for family, button in family_buttons.items():
                button.configure(
                    bg=theme.ACCENT_DARK if family == name else theme.PANEL_SOFT,
                    fg=theme.WHITE if family == name else theme.INK,
                )
            for label in palettes.get(name, []):
                V3Button(
                    palette_body,
                    label,
        ...
        for family in ("Ajouter", "Pages", "Types", "Règles", "Blancs", "Catalogue"):
            button = tk.Button(
                left,
                text=family,
                command=lambda name=family: show_palette(name),
                bg=theme.PANEL_SOFT,
                fg=theme.INK,
        ...
        tk.Label(
            left,
            text="B se mettra à jour\nautomatiquement",
            bg=theme.PANEL_ALT,
            fg=theme.MUTED_DARK,
            justify="left",
            font=(theme.FONT_UI, 6),
        ).pack(side="bottom", anchor="w", padx=3, pady=(8, 2))

        show_palette("Ajouter")
