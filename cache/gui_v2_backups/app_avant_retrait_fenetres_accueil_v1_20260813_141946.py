...
class PageMaitreV2(tk.Tk):
    ...
    # FENETRES_ACCUEIL_REVELATION_ATOMIQUE_V1
    def _prepare_accueil_dialog(self, win: tk.Toplevel) -> tk.Toplevel:
        """Prépare une fenêtre hors écran puis la révèle une fois complète."""
        ...
        # La Toplevel reste invisible pendant toute sa construction.
        win.withdraw()
        ...
        def _reveal_when_ready():
            try:
                if not win.winfo_exists():
                    return

                # À ce stade, fond, angles cassés, images et widgets ont déjà
                # terminé leur premier cycle de géométrie hors écran.
        ...
        def _wait_one_more_idle():
            try:
                if win.winfo_exists():
                    self.after_idle(_reveal_when_ready)
            except tk.TclError:
                pass

        # Deux passages idle : le premier termine les redraw différés propres
        # à la fenêtre ; le second la révèle dans son état final.
        self.after_idle(_wait_one_more_idle)
