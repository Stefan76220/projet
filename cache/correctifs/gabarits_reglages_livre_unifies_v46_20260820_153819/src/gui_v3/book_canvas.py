...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_request_book_setting(self, key: str) -> None:
        """Demande un réglage LIVRE à l'application via un événement Tk stable."""
        event_name = {
            "edit_format": "<<GabaritEditFormat>>",
            "edit_margins": "<<GabaritEditMargins>>",
            "edit_bleed": "<<GabaritEditBleed>>",
        }.get(str(key or ""))
        if not event_name:
            return
        try:
        ...
        # Repli pour un ancien hôte qui n'aurait pas encore les bindings virtuels.
        callback_name = {
            "edit_format": "on_gabarit_edit_format",
            "edit_margins": "on_gabarit_edit_margins",
            "edit_bleed": "on_gabarit_edit_bleed",
        }.get(str(key or ""), "")
    ...
    def _gabarit_inspector_press(self, event):
        ...
        elif key in {"edit_format", "edit_margins", "edit_bleed"}:
            self._gabarit_request_book_setting(key)
        self._gabarit_inspector_signature = None
        self._render_gabarit_inspector_overlay(force=True)
        return "break"
