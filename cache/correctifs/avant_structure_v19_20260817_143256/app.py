    ...
    def _build_structure_tab(self, parent):
        ...
        def make_brick(parent_, label, type_key, *, custom=False):
            ...
            def draw(fill=WORK_TILE, outline=WORK_BORDER, active=False):
                ...
                card.create_text(
                    width/2, 45, text=display, anchor="center", justify="center",
        ...
        # Emplacement réservé aux futures commandes de pages automatiques.
        # Aucun comportement n'est ajouté à ce stade.
        auto_line = tk.Frame(atelier, bg=WORK_BG, height=26)
        auto_line.grid(row=2, column=0, sticky="ew", pady=(5, 0))
        auto_line.grid_propagate(False)
        tk.Label(
            auto_line, text="Pages auto", bg=WORK_BG, fg=WORK_GOLD,
            font=(theme.FONT_TITLE, 9, "bold"),
        ).pack(side="left", padx=(2, 10))
        tk.Frame(auto_line, bg=WORK_BORDER, height=1).pack(
            side="left", fill="x", expand=True, padx=(0, 4)
        )

        # Règles retirées volontairement : elles seront réintroduites ultérieurement.
