...
class BookCanvas(tk.Frame):
    ...
    def structure_create_custom_type(self, definition) -> tuple[str, str] | None:
        ...
        short_label = str(definition.get("short_label") or name).strip() or name
    ...
    def structure_arm_tool(self, kind: str, payload) -> None:
        ...
        if isinstance(payload, dict):
            ...
            label = str(payload[1])
        self.status_var.set(
            f"{label}  •  cliquez sur un + dans B pour insérer  •  glissez sur une page pour remplacer"
        )
        try:
    ...
    def _draw_structure_tool_targets(self) -> None:
        self.canvas.delete("structure_tool_target")
        self._structure_tool_targets = []
        if self._page_focus or getattr(self, "_work_mode", "structure") != "structure":
            return
        ...
        if kind == "page":
            if self._page_hitboxes:
                y1 = min(box[1] for box in self._page_hitboxes.values())
                y2 = max(box[3] for box in self._page_hitboxes.values())
                cy = y1 + (y2 - y1) * 0.48
            else:
                cy = max(90, self.canvas.winfo_height() * 0.58)

            for group in self.groups:
                group_id = str(group.get("id", ""))
                for local_pos in self._structure_valid_local_positions(group_id):
                    cx = self._structure_page_target_x(group_id, local_pos)
                    radius = 10
                    self.canvas.create_oval(
                        cx - radius, cy - radius, cx + radius, cy + radius,
                        fill=theme.ACCENT_DARK, outline=self.GOLD, width=2,
                        tags=("structure_tool_target",),
                    )
                    self.canvas.create_text(
                        cx, cy - 1, text="+", fill=theme.WHITE,
                        font=(theme.FONT_UI, 11, "bold"),
                        tags=("structure_tool_target",),
                    )
                    self._structure_tool_targets.append(
                        ("page", group_id, int(local_pos), cx - 14, cy - 14, cx + 14, cy + 14)
                    )
        else:
            middle_count = sum(
            ...
            if header_boxes:
                cy = sum((box[1] + box[3]) / 2.0 for box in header_boxes) / len(header_boxes)
            else:
                cy = 40
            for target in range(middle_count + 1):
                cx = self._structure_group_target_x(target)
                radius = 11
                self.canvas.create_oval(
                    cx - radius, cy - radius, cx + radius, cy + radius,
                    fill=theme.ACCENT_DARK, outline=self.GOLD, width=2,
                ...
                self._structure_tool_targets.append(
                    ("group", int(target), None, cx - 15, cy - 15, cx + 15, cy + 15)
                )

    def _structure_target_at(self, event, kind: str):
        cx = self.canvas.canvasx(event.x)
        cy = self.canvas.canvasy(event.y)
        for target in getattr(self, "_structure_tool_targets", []):
            target_kind = target[0]
            if target_kind != kind:
                continue
            if target_kind == "page":
                _kind, group_id, local_pos, x1, y1, x2, y2 = target
                if x1 <= cx <= x2 and y1 <= cy <= y2:
                    return group_id, local_pos
            else:
                _kind, position, _unused, x1, y1, x2, y2 = target
                if x1 <= cx <= x2 and y1 <= cy <= y2:
                    return position
        return None

    def _structure_apply_at(self, kind: str, payload, x: float, y: float, *, direct_drop: bool = False) -> bool:
        if getattr(self, "_work_mode", "structure") != "structure" or self.project is None:
            return False
        event = self._structure_event(x, y)
        canvas_x = self.canvas.canvasx(x)

        if kind == "group":
            return self._structure_insert_group(canvas_x) is not None

        if kind == "page":
            page_type, label = self._structure_normalize_page_payload(payload)
            index = self._index_at(event)
            if direct_drop and index is not None:
                return self._structure_change_page_type(index, page_type, label)
            group_id = self._group_id_at_x(canvas_x)
            local_pos = self._target_local_pos_from_x(canvas_x, group_id)
            if local_pos not in self._structure_valid_local_positions(group_id):
                valid = self._structure_valid_local_positions(group_id)
                if not valid:
                    return False
                local_pos = min(valid, key=lambda value: abs(value - local_pos))
            return self._structure_insert_page(page_type, label, group_id, local_pos) is not None
        return False
    ...
    def _structure_apply_pending_event(self, event):
        ...
        target = self._structure_target_at(event, kind)
        success = False
        if kind == "page" and isinstance(target, tuple):
            group_id, local_pos = target
            page_type, label = self._structure_normalize_page_payload(payload)
            success = self._structure_insert_page(page_type, label, group_id, local_pos) is not None
        elif kind == "group" and isinstance(target, int):
            success = self._structure_insert_group_at_position(target) is not None
        else:
            self.status_var.set("Cliquez sur un bouton + dans B pour choisir l’emplacement.")
            return "break"
    ...
    def structure_duplicate_selected(self) -> bool:
        if getattr(self, "_structure_selection_kind", "page") != "page":
            self.status_var.set(
                "La duplication de partie sera ajoutée plus tard. Pour l'instant, déposez + Partie dans B."
            )
            return False
        index = self._selected_index
        if index is None or not 0 <= index < len(self.items):
            self.status_var.set("Sélectionnez une page à dupliquer.")
            return False
        item = self.items[index]
        if self._is_locked_page(item) or self._is_automatic_page(item):
            self.status_var.set("Cette page est protégée et ne peut pas être dupliquée.")
            return False
        if item.get("structure_duplicable", True) is False:
            self.status_var.set("Ce type de page a été défini comme non duplicable.")
            return False
        duplicate = deepcopy(item)
        duplicate["id"] = f"MAQUETTE-{uuid4().hex[:12].upper()}"
        for key in (
            "automatic_recto_verso", "automatic", "auto_generated",
            "recto_target_id", "linked_to", "parent_id", "source_page_id",
        ):
            duplicate.pop(key, None)
        name = self._page_display_name(item, index)
        if name:
            duplicate["page_name"] = f"{name} copie"
        group_id = self._item_group_id(item)
        group_indexes = self._group_items(group_id)
        local_pos = group_indexes.index(index) + 1 if index in group_indexes else len(group_indexes)
        self.items.append(duplicate)
        self._move_page_to_group_position(duplicate, group_id, local_pos)
        self._selected_index = self.items.index(duplicate)
        self._selected_group_id = group_id
        self._structure_selection_kind = "page"
        self._save_order()
        self.render()
    ...
    def structure_delete_selected(self) -> bool:
        ...
        index = self._selected_index
        if index is None or not 0 <= index < len(self.items):
            self.status_var.set("Sélectionnez une page à supprimer.")
            return False
        if self._is_locked_page(self.items[index]):
            self.status_var.set("Cette page structurelle est protégée.")
            return False
        block = set(self._drag_block_indices(index))
        self.items = [item for i, item in enumerate(self.items) if i not in block]
        self._selected_index = min(index, len(self.items) - 1) if self.items else None
    ...
    def _draw_overlay_branding(self, target: tk.Canvas, width: float, height: float):
        """Identité TomeLinea sobre : icône à gauche, titre centré, sans la ligne à stations."""
        self._overlay_brand_refs = []
        ...
        if Image is not None and ImageTk is not None:
            try:
                ...
                target.create_image(46, 52, image=photo, anchor="center")
            except Exception:
                pass

        target.create_text(width / 2.0, 34, text="TomeLinea", fill=theme.INK, font=(theme.FONT_TITLE, 22, "bold"), anchor="center")
        target.create_text(width / 2.0, 58, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE", fill=theme.MUTED_DARK, font=(theme.FONT_UI, 8, "bold"), anchor="center")
    ...
    def _render_page_overlay(self):
        ...
        # Même fond général que TomeLinea : aucune rupture visuelle en ouvrant le zoom.
        ...
        # La page reste la vedette. Les informations sont replacées sur les côtés,
        # centrées verticalement et indépendantes du zoom, afin de rester visibles
        # sans être poussées par l'agrandissement.
        ...
        panel_h = 88.0
        left_w = min(214.0, max(172.0, width * 0.18))
        right_w = min(226.0, max(184.0, width * 0.19))
        gap = 28.0
        left_x2 = max(38.0 + left_w, min(x - gap, width * 0.29))
        left_x1 = left_x2 - left_w
        right_x1 = min(width - 38.0 - right_w, max(x + page_w + gap, width * 0.71 - right_w))
        right_x2 = right_x1 + right_w
        panel_cy = height * 0.50
        panel_y1 = panel_cy - panel_h / 2.0
        panel_y2 = panel_cy + panel_h / 2.0

        # Voiles discrets, sans attirer trop l'œil.
        ...
        # Fils graphiques très légers vers la page.
        guide_y = panel_cy
        target.create_line(left_x2 - 10, guide_y, x - 18, guide_y, fill=theme.BORDER_SOFT, width=1)
        target.create_line(x + page_w + 18, guide_y, right_x1 + 10, guide_y, fill=theme.BORDER_SOFT, width=1)
        target.create_oval(x - 22, guide_y - 3, x - 16, guide_y + 3, fill=theme.ACCENT_DARK, outline="")
        target.create_oval(x + page_w + 16, guide_y - 3, x + page_w + 22, guide_y + 3, fill=self.ORANGE, outline="")

        # Bloc gauche : titre et type, plus discrets.
        target.create_text(left_x1 + 10, panel_y1 + 16, text="TITRE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 7, "bold"))
        target.create_text(left_x1 + 10, panel_y1 + 36, text=page_title, anchor="w", fill=theme.INK, font=(theme.FONT_TITLE, 12, "bold"), width=max(120, left_w - 20))
        target.create_text(left_x1 + 10, panel_y1 + 62, text="TYPE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 7, "bold"))
        target.create_text(left_x2 - 10, panel_y1 + 62, text=page_type, anchor="e", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 9, "bold"), width=max(100, left_w - 24))

        # Bloc droit : partie et avancement, même logique.
        target.create_text(right_x1 + 10, panel_y1 + 16, text="PARTIE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 7, "bold"))
        target.create_text(right_x1 + 10, panel_y1 + 36, text=part_value, anchor="w", fill=theme.INK, font=(theme.FONT_TITLE, 12, "bold"), width=max(130, right_w - 20))
        target.create_text(right_x1 + 10, panel_y1 + 62, text="AVANCEMENT", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 7, "bold"))
        target.create_text(right_x2 - 10, panel_y1 + 62, text=stage_text, anchor="e", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 9, "bold"))
        target.create_text(right_x2 - 10, panel_y1 + 78, text=f"Page {index + 1}/{len(self.items)}", anchor="e", fill=theme.MUTED, font=(theme.FONT_UI, 8))
    ...
    def render(self):
        ...
        if (
            ...
            self._draw_structure_tool_targets()
    ...
    def _on_press(self, event):
        ...
        # MODE_C_LOCK_V3 — B suit exclusivement le mode actif de C.
        if getattr(self, "_work_mode", "structure") != "structure":
            index = self._index_at(event)
