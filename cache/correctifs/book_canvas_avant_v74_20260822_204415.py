# TOMELINEA_GABARITS_OUTILS_ETALES_V73
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        def icon_strip(items, *, button=41.0, gap=8.0, y_offset=0.0, center=True, start_x=None):
            # V73 — les outils remplissent réellement la largeur disponible.
            # Au lieu d'être centrés ou tassés sur un bord, leurs centres sont
            # répartis régulièrement de gauche à droite avec une marge identique.
            count_items=len(items)
            yy=content_top+y_offset
            pad=14.0
            usable_left=fly_left+pad
            usable_right=fly_right-pad
            if count_items <= 1:
                centers=[(usable_left+usable_right)/2.0]
            else:
                first=usable_left+button/2.0
                last=usable_right-button/2.0
                step=max(0.0,(last-first)/float(count_items-1))
                centers=[first+i*step for i in range(count_items)]
        ...
        if active_band == "create":
            ...
            target.create_text(label_x,content_top+117,text="ÉGALISER",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=34.0,gap=10.0,y_offset=126.0)
            target.create_text(label_x,content_top+166,text="ASSOCIER",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=34.0,gap=12.0,y_offset=175.0)
            target.create_text(label_x,content_top+215,text="SUPERPOSITION",anchor="w",fill="#7F959A",font=(theme.FONT_UI,7,"bold"))
            icon_strip((
            ...
            ),button=34.0,gap=7.0,y_offset=224.0)
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        ...
        # V72 — le calque reste large en permanence pour que le rail ne saute plus.
        # L'inspecteur général conserve toutefois sa largeur visuelle historique et
        # reste ancré au bord droit ; la réserve transparente à gauche n'est utilisée
        # que par les flyouts contextuels.
        general_w = 210.0 if width >= 340.0 else 188.0
        panel_left = max(0.0, width - general_w)
        target.create_line(panel_left, 8.0, panel_left, max(8.0, height - 8.0), fill="#465A63", width=1)
        tx = panel_left + 16.0
        right = width - 12.0
        inner = max(100.0, right - tx)
        y = 18.0
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float, *, target=None) -> None:
        ...
        for zone in zones:
            ...
            selected_width = 2 if selected and len(selected_ids) > 1 else (3 if selected else 1)
            rect_id = target.create_polygon(
                *flat, fill="", outline=color, width=selected_width,
            )
