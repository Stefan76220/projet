...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        info_y2 = viewport_h
        # Le trait s'arrete avant les angles coupes : aucune ligne ni commande
        # ne vient mordre sur le dessin du panneau.
        ...
        # Les polices restent a la taille lisible validee en v8. On recupere
        # uniquement de la hauteur dans les respirations et dans les boutons.
        # Cela evite que « Fond perdu » sorte de B sur les ecrans a mise a
        # l'echelle Windows elevee.
        def section_label(y: float, text: str) -> float:
            self.canvas.create_text(
                tx, y, text=text, anchor="w", fill="#A8BCC0",
                font=(theme.FONT_UI, 9, "bold"),
            )
            # Grille verticale régulière : même respiration après chaque titre de section.
            return y + 24.0

        def separator(y: float) -> float:
            self.canvas.create_line(tx, y, inner_right, y, fill="#40525B", width=1)
            # Même espace après chaque séparateur pour éviter l'effet tassé.
            return y + 17.0
        ...
        y = float((bbox[3] if bbox else y + 20.0) + 7.0)
        self.canvas.create_text(
        ...
        # Espace franc entre la commande Portée et le libellé Statut.
        y += scope_h + 14.0
        ...
        self.canvas.create_oval(tx + 10, y + 13.0, tx + 22, y + 25.0, fill=status_color, outline="")
        self.canvas.create_text(tx + 30, y + status_h / 2.0, text=status_text, anchor="w", fill=status_color, font=(theme.FONT_UI, 9, "bold"))
        self.canvas.create_text(inner_right - 9, y + status_h / 2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 12, "bold"))
        y += status_h + 18.0
        y = separator(y)
        ...
        self.canvas.create_text(tx + 10, y + 29, text=compact_values(margins, ("top", "bottom", "inside", "outside"), ("H", "B", "I", "E")), anchor="w", fill="#B8C6C3", font=(theme.FONT_UI, 8))
        self.canvas.create_text(inner_right - 9, y + book_control_h / 2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += book_control_h + 10.0
        ...
        self.canvas.create_text(tx + 10, y + 29, text=compact_values(bleed, ("top", "bottom", "left", "right"), ("H", "B", "G", "D")), anchor="w", fill="#D1AAAA", font=(theme.FONT_UI, 8))
        self.canvas.create_text(inner_right - 9, y + book_control_h / 2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += book_control_h
