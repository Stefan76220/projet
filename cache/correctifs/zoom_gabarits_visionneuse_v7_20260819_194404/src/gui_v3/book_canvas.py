...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Gabarits — poste de zoom historique à gauche de B.
        # La molette reste le geste principal ; ces quatre contrôles sont un
        # secours permanent, volontairement plus discret que dans la première
        # version de Gabarits. De vrais tk.Button garantissent l'activation.
        self.gabarit_zoom_controls = tk.Frame(
            viewer, bg=theme.WINDOW_DEEP, bd=0, highlightthickness=0,
        )

        def _gabarit_zoom_button(text, command, *, compact=False):
            return tk.Button(
                self.gabarit_zoom_controls,
                text=text, command=command,
                bg="#203744", fg="#E9EFED",
                activebackground="#31515E", activeforeground="#FFFFFF",
                relief="flat", bd=0, highlightthickness=0,
                padx=3, pady=2 if compact else 4,
                font=(theme.FONT_UI, 7 if compact else 10, "bold"),
                cursor="hand2", takefocus=False,
            )

        self.gabarit_zoom_plus_btn = _gabarit_zoom_button("+", self.gabarit_zoom_in)
        self.gabarit_zoom_plus_btn.pack(fill="x", ipady=3)
        self.gabarit_zoom_label = tk.Label(
            self.gabarit_zoom_controls, textvariable=self._gabarit_zoom_text_var,
            bg=theme.WINDOW_DEEP, fg="#9FB8BC",
            font=(theme.FONT_UI, 7, "bold"), pady=4,
        )
        self.gabarit_zoom_label.pack(fill="x")
        self.gabarit_zoom_minus_btn = _gabarit_zoom_button("−", self.gabarit_zoom_out)
        self.gabarit_zoom_minus_btn.pack(fill="x", ipady=3)
        self.gabarit_zoom_fit_btn = _gabarit_zoom_button("Adapter", self.gabarit_reset_zoom, compact=True)
        self.gabarit_zoom_fit_btn.pack(fill="x", pady=(5, 0), ipady=2)
    ...
    def _gabarit_sync_zoom_widget(self) -> None:
        """Synchronise l'indication du poste de zoom de B."""
        value = int(getattr(self, "_gabarit_zoom", 100))
        try:
    ...
    def _gabarit_zoom_navigate(self, velocity: float) -> None:
        """Commande continue − / +, identique au navigateur historique Structure."""
        ...
        self.gabarit_set_zoom(
            current + delta,
            anchor=self._gabarit_button_zoom_anchor(),
        )
    ...
    def _gabarit_clamp_pan(self, viewport_w: float, viewport_h: float, page_w: float, page_h: float) -> None:
        """Limite le déplacement dans la vraie surface de travail de B.

        L’ancienne limite utilisait toute la hauteur du Canvas, inspecteur et ligne
        compris. Une page agrandie près du bas était donc recentrée de force et son
        bas disparaissait. Ici chaque bord peut réellement rejoindre le bord visible.
        """
        ...
        if hasattr(self, "canvas"):
            ...
            # Compatibilité avec les tests/outils légers qui appellent le moteur
            # de déplacement sans construire le widget Tk complet.
        ...
        # Une petite marge de débordement est volontairement admise. Sans elle,
        # un zoom effectué près d'un bord serait recadré dès que ce bord atteindrait
        # la limite de B : le point sous la souris sauterait alors vers le centre.
        edge_slack = 42.0
        max_x = max(0.0, (float(page_w) - work_w) / 2.0 + edge_slack)
        max_y = max(0.0, (float(page_h) - work_h) / 2.0 + edge_slack)
        self._gabarit_pan_x = 0.0 if max_x <= 0.0 else max(-max_x, min(max_x, float(self._gabarit_pan_x)))
        self._gabarit_pan_y = 0.0 if max_y <= 0.0 else max(-max_y, min(max_y, float(self._gabarit_pan_y)))
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # ----- zoom discret, toujours visible à gauche de B -----
        # Il est dessiné dans le Canvas lui-même : impossible qu'un widget frère
        # passe derrière B ou disparaisse suivant l'empilement Tk. La molette
        # reste le geste principal ; ces commandes sont un secours permanent.
        zoom_w = 54.0
        zoom_x1 = 14.0
        zoom_x2 = zoom_x1 + zoom_w
        zoom_total_h = 128.0
        zoom_y1 = max(work_top + 8.0, min(work_bottom - zoom_total_h - 8.0, (work_top + work_bottom - zoom_total_h) / 2.0))

        def draw_zoom_button(key: str, yb: float, hb: float, text: str, *, small: bool = False) -> None:
            hovered = str(getattr(self, "_gabarit_hover_control", "")) == key
            pressed = str(getattr(self, "_gabarit_pressed_control", "")) == key
            shift_y = 1.5 if pressed else 0.0
            bx1, by1, bx2, by2 = zoom_x1, yb + shift_y, zoom_x2, yb + hb + shift_y
            fill = "#314752" if hovered else "#203744"
            if pressed:
                fill = "#192A33"
            outline = theme.ACCENT_BRIGHT if hovered or pressed else "#42555D"
            self.canvas.create_rectangle(bx1, by1, bx2, by2, fill=fill, outline=outline, width=1)
            self.canvas.create_text(
                (bx1 + bx2) / 2.0, (by1 + by2) / 2.0, text=text,
                fill="#FFFFFF" if hovered or pressed else "#DDE8E5",
                font=(theme.FONT_UI, 7 if small else 12, "bold"), anchor="center",
            )
            self._gabarit_hitboxes[key] = (bx1 - 3, yb - 3, bx2 + 3, yb + hb + 3)

        draw_zoom_button("zoom_plus", zoom_y1, 30.0, "+")
        self.canvas.create_text(
            (zoom_x1 + zoom_x2) / 2.0, zoom_y1 + 45.0,
            text=str(getattr(self, "_gabarit_zoom_text_var", tk.StringVar(value="100 %")).get()),
            fill="#9FB8BC", font=(theme.FONT_UI, 7, "bold"), anchor="center",
        )
        draw_zoom_button("zoom_minus", zoom_y1 + 58.0, 30.0, "−")
        draw_zoom_button("zoom_fit", zoom_y1 + 96.0, 27.0, "Adapter", small=True)
        ...
        control_box("edit_format", y, 31.0)
        self.canvas.create_text(tx + 9, y + 10, text="Format fini", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(
            tx + 9, y + 23,
            text=f"{fmt['label']} · {fmt['width_mm']:g} × {fmt['height_mm']:g} mm",
            anchor="w", fill="#AEBDBA", font=(theme.FONT_UI, 6),
        )
        ...
        # L'ancien widget Tk de secours reste créé pour compatibilité interne,
        # mais il n'est plus affiché : le poste visible est désormais dessiné
        # directement dans B, ce qui garantit son apparition.
        zoom_controls = getattr(self, "gabarit_zoom_controls", None)
