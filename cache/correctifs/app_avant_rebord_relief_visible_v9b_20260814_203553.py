...
class PageMaitreV2(tk.Tk):
    ...
    def _render_header_canvas(self, _event=None) -> None:
        ...
        # Relief identique au principe des zones de l’Accueil.
        relief_high = self._mix(theme.WINDOW, "#FFFFFF", 0.16)
        relief = self._mix(theme.WINDOW, "#000000", 0.20)
        canvas.create_line(
            0, height - 4,
            width, height - 4,
            fill=relief_high,
    ...
    def _build_visualisation_vierge(self, parent: tk.Frame) -> None:
        ...
        def draw_global_panel(_event=None):
            ...
            global_panel.create_line(
                14, h - 4, w - 22, h - 4,
                fill=self._mix(PANEL, "#FFFFFF", 0.16),
                width=1,
                tags=("panel_decor",),
            )
            global_panel.create_line(
                12, h - 2, w - 20, h - 2,
                fill=self._mix(PANEL, "#000000", 0.20),
                width=3,
                tags=("panel_decor",),
            )
    ...
    def _card(
        ...
        *,
        ...
        # Relief TomeLinea identique au principe de l’Accueil :
        # trait fin clair + trait inférieur plus épais et sombre.
        tk.Frame(
            card,
            bg=self._mix(theme.PANEL, "#000000", 0.20),
            height=3,
        ).pack(side="bottom", fill="x", padx=14, pady=(0, 2))
        tk.Frame(
            card,
            bg=self._mix(theme.PANEL, "#FFFFFF", 0.16),
            height=1,
