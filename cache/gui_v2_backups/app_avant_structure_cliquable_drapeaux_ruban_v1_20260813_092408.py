...
class PageMaitreV2(tk.Tk):
    ...
    def _tomelinea_open_atelier_page(self, group_name, page_number):
        self._atelier_selected_group = str(group_name)
        self._atelier_selected_page = int(page_number)
        context = f'{self._atelier_selected_group} · page {self._atelier_selected_page}'
        if hasattr(self, "_atelier_context_var"):
            self._atelier_context_var.set(context)
        self.show_screen("atelier")
    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        ...
        def on_maquettage_motion(event):
            ...
            if current == previous and not page_changed:
                canvas.configure(cursor="hand2" if current else "arrow")
                return
        ...
        def on_maquettage_release(event):
            ...
            if maquettage_page_pressed is not None:
                ...
                release_region = maquettage_page_at(event.x, event.y)
                ...
                if was_dragging and source_slot is not None and drop_slot is not None:
                    ...
                    release_region is not None
                    and release_region["page_id"] == page_id
                ):
                    open_page = release_region["slot"] + 1
                ...
                if open_page is not None:
                    group_name = str(selected_group["name"])
                    canvas.after_idle(
                        lambda g=group_name, p=open_page:
                        self._tomelinea_open_atelier_page(g, p)
                    )
                return
    ...
    def _build_atelier(self, parent: tk.Frame) -> None:
        ...
        if not hasattr(self, "_atelier_context_var"):
            self._atelier_context_var = tk.StringVar(
                value="Aucune page ouverte depuis le Maquettage"
            )

        atelier_context = tk.Frame(
            parent,
            bg="#F4F0EA",
            highlightthickness=1,
            highlightbackground="#D7D1C9",
        )
        atelier_context.pack(fill="x", padx=22, pady=(4, 2))
        tk.Label(
            atelier_context,
            text="Page active",
            bg="#F4F0EA",
        ...
        tk.Label(
            atelier_context,
            textvariable=self._atelier_context_var,
            bg="#F4F0EA",
            fg="#111111",
            font=("Segoe UI", 9, "bold"),
