...
class MockupView:
    ...
    def _create_sequence_row(
        ...
        """Carte visuelle : la carte sert à voir, sélectionner et déplacer."""
        item_id = str(item.get("id", ""))
        ...
        card_width = 820 if not automatic_blank else 650
        ...
        thumbnail.grid(
            ...
            column=1,
            padx=(0, 10),
            pady=7 if not automatic_blank else 6,
        )
        ...
        info = tk.Frame(
            card,
            width=610 if not automatic_blank else 520,
            height=card_height - 10,
            background=self.CARD_BG,
            borderwidth=0,
        )
        ...
        title_label.grid(
            ...
            column=0,
            sticky="w",
            pady=(5 if not automatic_blank else 2, 0),
        )
    ...
    def _update_selection_controls(self) -> None:
        ...
        single_count = (
            max(1, int(single_item.get("count", 1)))
            if single_item is not None
            else 0
        )
        single_done = (
            self._sequence_progress_counts(single_item)
            if single_item is not None
            else (0, 0)
        )
        single_auto = (
            self._associated_automatic_blank_count(single_id)
            if single_id
            else 0
        )
        ...
        if count > 1:
            self._selection_label.configure(
                text=f"{count} pages sélectionnées"
            )
        ...
        if group_title:
            info_parts.append(f"Groupe : {group_title}")
        if single_auto:
            info_parts.append(
        ...
        if not automatic_blank and not single_page:
            ...
            self._context_minus_button.configure(
                ...
                command=lambda current=single_id: self._change_count_by_id(
                    current, -1
                ),
            )
        ...
        if single_index is not None:
            self._context_up_button.configure(
                state=(
                    "normal"
                    if self._can_move_item(single_index, -1)
                    else "disabled"
                ),
                command=lambda current=single_id: self._move_item_by_id(
                    current, -1
                ),
            )
