...
class BookCanvas(tk.Frame):
    ...
    def gabarit_selected_zone_info(self) -> dict:
        ...
        width_mm = 420.0 if spread and spread_position == "center" else 210.0
        return {
    ...
    def gabarit_set_selected_zone_rect_mm(self, x_mm: float, y_mm: float, w_mm: float, h_mm: float) -> bool:
        ...
        width_mm = 420.0 if spread_position == "center" else 210.0
        # Une zone libre peut volontairement dépasser légèrement sa surface ; on
        # garde toutefois des limites raisonnables pour éviter une saisie aberrante.
        if not (-30.0 <= x_mm <= width_mm + 30.0 and -30.0 <= y_mm <= 327.0 and w_mm <= width_mm + 60.0 and h_mm <= 357.0):
            return False
        zone["occupation"] = "free"
        return self._gabarit_commit_zone_rect(
            zone,
            (x_mm / width_mm, y_mm / 297.0, w_mm / width_mm, h_mm / 297.0),
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        control_box("scope_page", y, 25.0, selected=page_scope, x1=tx, x2=scope_mid)
        self.canvas.create_text((tx + scope_mid) / 2.0, y + 12.5, text="Cette page", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 7, "bold" if page_scope else "normal"))
        type_x1 = scope_mid + scope_gap
        control_box("scope_type", y, 25.0, selected=type_scope, x1=type_x1, x2=inner_right)
        self.canvas.create_text((type_x1 + inner_right) / 2.0, y + 12.5, text="Tout le type", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 7, "bold" if type_scope else "normal"))
        y += 34.0
        ...
        # Les réglages du livre sont volontairement compacts : une ligne lisible
        # chacun, sans vignette et sans voler la place du zoom en bas.
        control_box("edit_margins", y, 31.0)
        self.canvas.create_text(tx + 9, y + 10, text="Marges", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(tx + 9, y + 23, text=compact_values(margins, ("top", "bottom", "inside", "outside"), ("H", "B", "I", "E")), anchor="w", fill="#AEBDBA", font=(theme.FONT_UI, 6))
        self.canvas.create_text(inner_right - 8, y + 15.5, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 10, "bold"))
        y += 35.0
        control_box("edit_bleed", y, 31.0)
        self.canvas.create_text(tx + 9, y + 10, text="Fond perdu", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 8, "bold"))
        self.canvas.create_text(tx + 9, y + 23, text=compact_values(bleed, ("top", "bottom", "left", "right"), ("H", "B", "G", "D")), anchor="w", fill="#C49A9A", font=(theme.FONT_UI, 6))
        self.canvas.create_text(inner_right - 8, y + 15.5, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 10, "bold"))
        y += 31.0

        # Zoom secondaire : une seule rangée, toujours placée APRES les réglages.
        # Son y est calculé à partir du contenu ET du bas du panneau : les hitboxes
        # ne peuvent donc plus se superposer à Marges / Fond perdu.
        zoom_top = max(y + 8.0, info_y2 - 31.0)
        if zoom_top + 24.0 <= info_y2 - 5.0:
            self.canvas.create_line(tx, zoom_top - 6, inner_right, zoom_top - 6, fill="#40525B", width=1)
            gap = 3.0
            label_w = 32.0
            minus_w, value_w, plus_w = 24.0, 38.0, 24.0
            zx = tx
            self.canvas.create_text(zx, zoom_top + 12, text="Zoom", anchor="w", fill="#9DB0B5", font=(theme.FONT_UI, 6, "bold"))
            zx += label_w
            hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_minus"
            self.canvas.create_rectangle(zx, zoom_top, zx + minus_w, zoom_top + 24, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
            self.canvas.create_text(zx + minus_w/2, zoom_top + 12, text="−", fill="#E6EBE9", font=(theme.FONT_UI, 10, "bold"))
            self._gabarit_hitboxes["zoom_minus"] = (zx, zoom_top, zx + minus_w, zoom_top + 24)
            zx += minus_w + gap
            self.canvas.create_rectangle(zx, zoom_top, zx + value_w, zoom_top + 24, fill="#26313B", outline="#40525B")
            self.canvas.create_text(zx + value_w/2, zoom_top + 12, text=f"{int(getattr(self, '_gabarit_zoom', 100))}%", fill="#B9C6C4", font=(theme.FONT_UI, 6, "bold"))
            zx += value_w + gap
            hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_plus"
            self.canvas.create_rectangle(zx, zoom_top, zx + plus_w, zoom_top + 24, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
            self.canvas.create_text(zx + plus_w/2, zoom_top + 12, text="+", fill="#E6EBE9", font=(theme.FONT_UI, 9, "bold"))
            self._gabarit_hitboxes["zoom_plus"] = (zx, zoom_top, zx + plus_w, zoom_top + 24)
            zx += plus_w + gap
            if zx < inner_right - 3:
                hovered = str(getattr(self, "_gabarit_hover_control", "")) == "zoom_fit"
                self.canvas.create_rectangle(zx, zoom_top, inner_right, zoom_top + 24, fill="#314752" if hovered else "#202D36", outline=theme.ACCENT_BRIGHT if hovered else "#465963")
                self.canvas.create_text((zx + inner_right)/2, zoom_top + 12, text="Adapter", fill="#E6EBE9", font=(theme.FONT_UI, 6, "bold"))
                self._gabarit_hitboxes["zoom_fit"] = (zx, zoom_top, inner_right, zoom_top + 24)
