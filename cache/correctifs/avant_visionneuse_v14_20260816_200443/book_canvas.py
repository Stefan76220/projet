...
class BookCanvas(tk.Frame):
    ...
    def _draw_overlay_branding(self, target: tk.Canvas, width: float, height: float):
        ...
        if Image is not None and ImageTk is not None:
            try:
                ...
                target.create_image(width / 2.0 - 165, 30, image=photo, anchor="center")
            except Exception:
                pass
            try:
                with Image.open(title_path) as src:
                    ...
                    photo = ImageTk.PhotoImage(image)
                self._overlay_brand_refs.append(photo)
                target.create_image(width / 2.0 + 22, 32, image=photo, anchor="center")
            except Exception:
                pass
        ...
        target.create_text(width / 2.0, 82, text="LA LIGNE ÉDITORIALE JUSQU’AU LIVRE", fill=theme.MUTED_DARK, font=(theme.FONT_UI, 8, "bold"))
    ...
    def _render_page_overlay(self):
        ...
        # Position latérale des informations : elles doivent rester visibles,
        # même si la page prend beaucoup de place. On les dessine après la page
        # pour éviter qu'elles disparaissent sous le zoom.
        left_x = max(28.0, min(x - 26.0, width * 0.18))
        right_x = min(width - 28.0, max(x + page_w + 26.0, width * 0.82))
        top_y = max(118.0, min(y + 30.0, height * 0.24))
