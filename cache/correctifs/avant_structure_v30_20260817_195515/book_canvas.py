...
class BookCanvas(tk.Frame):
    ...
    def _structure_view_anchor_snapshot(self):
        """Mémorise plusieurs repères visuels avant un rendu Structure.

        On garde plusieurs pages / parties proches du centre : si l'action
        supprime l'un des repères, un autre survivant maintient la ligne
        exactement au même endroit dans la fenêtre.
        """
        ...
        for index, box in getattr(self, "_page_hitboxes", {}).items():
            ...
            if not page_id:
                continue
            world_x = (float(x1) + float(x2)) / 2.0
            page_anchors.append({
                "id": page_id,
                "screen_x": world_x - left,
                "distance": abs(world_x - center_world),
            })
        page_anchors.sort(key=lambda anchor: anchor["distance"])
        ...
        group_anchors.sort(key=lambda anchor: anchor["distance"])

        return {
            "left": left,
            "pages": page_anchors[:8],
            "groups": group_anchors[:8],
        }

    def _structure_restore_view_anchor(self, snapshot) -> None:
        """Restaure la ligne sans qu'une commande ne la recentre arbitrairement."""
        ...
        # Priorité à une page encore présente.
        for anchor in snapshot.get("pages", []):
            page_id = str(anchor.get("id") or "")
            desired_screen_x = anchor.get("screen_x")
            if not page_id or desired_screen_x is None:
                continue
            new_index = next(
                (
            ...
            if new_index is None or new_index not in self._page_hitboxes:
                continue
            x1, _y1, x2, _y2 = self._page_hitboxes[new_index]
            new_world_x = (float(x1) + float(x2)) / 2.0
            target_left = new_world_x - float(desired_screen_x)
            restored = True
            break

        # Si toutes les pages repères ont disparu, on essaie une partie survivante.
        if not restored:
            for anchor in snapshot.get("groups", []):
                group_id = str(anchor.get("id") or "")
                desired_screen_x = anchor.get("screen_x")
                box = self._group_hitboxes.get(group_id)
                if not group_id or desired_screen_x is None or box is None:
                    continue
                x1, _y1, x2, _y2 = box
                new_world_x = (float(x1) + float(x2)) / 2.0
                target_left = new_world_x - float(desired_screen_x)
    ...
    def _on_press(self, event):
        ...
        # V24.1_SELECTION_VISIBLE
        # Glisser depuis toute zone vide autour des pages = cadre de sélection multiple.
        # Une partie recouvre une grande surface de B : on ne teste donc plus
        # group_id. Seule une page réellement sous le pointeur bloque le lasso.
        if index is None:
            cx = self.canvas.canvasx(event.x)
            cy = self.canvas.canvasy(event.y)
