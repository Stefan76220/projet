...
def write_phase1_report(
    ...
    *,
    ...
    for section in sections:
        ...
        column_count = columns.get("count") or 1
        column_space = columns.get("space_mm")
        column_text = f"{column_count} colonne" if column_count == 1 else f"{column_count} colonnes"
        if column_space is not None and column_count > 1:
            column_text += f" / espace {column_space} mm"
        section_rows.append(
            "<tr>"
    ...
    for index, image in enumerate(images, start=1):
        adv = image.get("advanced", {}) or {}
        pos = []
        if image.get("position_h_offset_mm") is not None:
            pos.append(f"H {image.get('position_h_offset_mm')} mm / {image.get('position_h_relative_to') or '—'}")
        if image.get("position_v_offset_mm") is not None:
            pos.append(f"V {image.get('position_v_offset_mm')} mm / {image.get('position_v_relative_to') or '—'}")
        image_rows.append(
            "<tr>"
            ...
            f"<td>{_e(image.get('wrap'))}</td><td>{escape(' ; '.join(pos) or '—')}</td>"
            f"<td><code>{escape(json.dumps(adv, ensure_ascii=False))}</code></td>"
            "</tr>"
        )
    ...
    for table in tables:
        advanced = {
            ...
            "cell_margins": table.get("cell_margins", {}),
            "shading": table.get("shading", {}),
            "rows": [
                {
                    "row": row.get("row"),
                    "height_mm": row.get("height_mm"),
                    "height_rule": row.get("height_rule"),
                    "repeat_header": row.get("repeat_header"),
                    "cant_split": row.get("cant_split"),
                    "cells": [
                        {
                            "cell": cell.get("cell"),
                            "grid_span": cell.get("grid_span"),
                            "vertical_merge": cell.get("vertical_merge"),
        ...
        table_rows.append(
            ...
            f"<td>{_e(table.get('alignment'))}</td><td><code>{escape(json.dumps(advanced, ensure_ascii=False))}</code></td>"
            "</tr>"
        )
    ...
    hyperlink_rows = []
    for item in doc.get("hyperlinks", []):
        hyperlink_rows.append(
            "<tr>"
            f"<td>{_e(item.get('paragraph'))}</td><td>{_e(item.get('start_offset'))} → {_e(item.get('end_offset'))}</td>"
            f"<td>{escape(str(item.get('text') or ''))}</td><td>{_e(item.get('target'))}</td><td>{_e(item.get('target_mode'))}</td>"
            "</tr>"
        )
    bookmark_rows = []
    for item in doc.get("bookmarks", []):
        bookmark_rows.append(
            "<tr>"
            f"<td>{_e(item.get('name'))}</td><td>{_e(item.get('start_paragraph'))}:{_e(item.get('start_offset'))}</td>"
            f"<td>{_e(item.get('end_paragraph'))}:{_e(item.get('end_offset'))}</td><td>{_e(item.get('unclosed'))}</td>"
            "</tr>"
        )
...
<table><thead><tr><th>Section</th><th>Paragraphes</th><th>Format</th><th>Orientation</th><th>Marges</th><th>Rupture</th><th>Colonnes</th><th>Grille ligne</th></tr></thead><tbody>{''.join(section_rows)}</tbody></table>
...
<table><thead><tr><th>#</th><th>Paragraphe</th><th>Mode</th><th>Partie OOXML</th><th>Taille mm</th><th>Habillage</th><th>Position</th><th>Géométrie avancée</th></tr></thead><tbody>{''.join(image_rows)}</tbody></table>
<h2>Tableaux</h2>
<table><thead><tr><th>#</th><th>Lignes</th><th>Style</th><th>Disposition</th><th>Alignement</th><th>Propriétés avancées</th></tr></thead><tbody>{''.join(table_rows)}</tbody></table>
<h2>Hyperliens</h2>
<table><thead><tr><th>Paragraphe</th><th>Plage caractères</th><th>Texte</th><th>Cible</th><th>Mode</th></tr></thead><tbody>{''.join(hyperlink_rows)}</tbody></table>
<h2>Signets</h2>
<table><thead><tr><th>Nom</th><th>Début</th><th>Fin</th><th>Non fermé</th></tr></thead><tbody>{''.join(bookmark_rows)}</tbody></table>
...
<pre>{escape(json.dumps({
    ...
    'content_controls': doc.get('content_controls', []),
    'tracked_change_ranges': doc.get('tracked_change_ranges', []),
