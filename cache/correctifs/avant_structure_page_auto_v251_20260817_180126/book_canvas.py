...
class BookCanvas(tk.Frame):
    ...
    def _structure_open_auto_menu(self, position: str, widget) -> None:
        ...
        local_scope_label = (
            "Cette page"
            if len(sources) == 1
            else f"Les {len(sources)} pages sélectionnées"
        )
        ...
        def build_type_menu(scope: str):
            ...
            for type_key, label in options:
                ...
                submenu.add_command(
                    label=visible,
                    command=lambda key=type_key, chosen_scope=scope: choose(key, chosen_scope),
                )
            return submenu

        # La portée est visible dès l'ouverture du menu.
        local_menu = build_type_menu("local")
        menu.add_cascade(label=local_scope_label, menu=local_menu)

        if same_type and source_type:
            type_menu = build_type_menu("type")
            menu.add_cascade(
