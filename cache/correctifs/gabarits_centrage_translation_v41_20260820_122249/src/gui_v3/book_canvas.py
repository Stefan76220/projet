...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Largeur du bloc zoom figée une seule fois à la construction. Le rendu
        # Gabarits n'interroge ni ne force ensuite la géométrie Tk : on garde le
        # moteur stable de v36 et on ne change que le calcul de position.
        try:
            self._gabarit_zoom_static_width = float(self.gabarit_zoom_controls.winfo_reqwidth())
        except Exception:
            self._gabarit_zoom_static_width = 64.0
        if self._gabarit_zoom_static_width <= 1.0:
            self._gabarit_zoom_static_width = 64.0
    ...
    def _gabarit_work_rect(self) -> tuple[float, float, float, float]:
        ...
        zoom_w = float(getattr(self, "_gabarit_zoom_static_width", 64.0))
        # Frontières réelles : après outils+zoom à gauche, avant inspecteur à droite.
        # 12 px de respiration identique de chaque côté. Aucun widget n'est interrogé ici.
        left_separator_x = tools_w + 8.0 + zoom_w + 10.0
        right_separator_x = width - info_w - 8.0
        work_left = left_separator_x + 12.0
        work_right = max(work_left + 1.0, right_separator_x - 12.0)
        work_top = 82.0
    ...
    def _gabarit_fit_scale(self, viewport_w: float, viewport_h: float) -> float:
        ...
        zoom_w = float(getattr(self, "_gabarit_zoom_static_width", 64.0))
        left_separator_x = tools_w + 8.0 + zoom_w + 10.0
        right_separator_x = viewport_w - info_w - 8.0
        work_left = left_separator_x + 12.0
        work_right = max(work_left + 1.0, right_separator_x - 12.0)
        top_h = 82.0
        bottom_h = 16.0
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        zoom_controls = getattr(self, "gabarit_zoom_controls", None)
        # Même frontière statique que celle utilisée pour centrer la page.
        left_separator_x = tools_w + 8.0 + float(getattr(self, "_gabarit_zoom_static_width", 64.0)) + 10.0
