...
class PageMaitreV2(tk.Tk):
    ...
    # VUE_DU_LIVRE_DEUX_ONGLETS_DYNAMIQUES_V8_RECONSTRUCTION_INSTANTANEE
    def _tomelinea_open_book_views(self, initial_tab="global"):
        ...
        # ----------------------------------------------------------
        # Contrôleur de changement de vue V8.
        #
        # Aucun état intermédiaire visible :
        # la fenêtre est cachée, reconstruite à sa taille finale,
        # puis réaffichée.
        # ----------------------------------------------------------
        switch_state = {
            "active": False,
        ...
        def show_tab(key, *, animate=True):
            ...
            try:
                # Masque entièrement la fenêtre avant tout changement.
                win.withdraw()

                apply_mode_content(key)

                # Une seule géométrie appliquée pendant que la fenêtre est cachée.
                win.geometry(
                    f"{final_w}x{final_h}"
                    f"+{final_x}+{final_y}"
                )
                ...
                # Reconstruction complète à la taille finale.
                redraw_shell()
                draw_title_line()
                ...
                # Réapparition immédiate de la vue terminée.
                win.deiconify()
                win.lift()
                try:
                    win.focus_force()
