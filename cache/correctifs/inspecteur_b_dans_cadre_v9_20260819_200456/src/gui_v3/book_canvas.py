...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # La séparation est un simple trait vertical ; la largeur gagnée permet
        # des polices réellement lisibles sans transformer B en panneau massif.
        info_y1 = 0.0
        info_x2 = viewport_w
        info_y2 = viewport_h
        self.canvas.create_line(info_x1, info_y1, info_x1, info_y2, fill="#465A63", width=1)
        tx = info_x1 + 16.0
        ...
        control_box("scope_page", y, 31.0, selected=page_scope, x1=tx, x2=scope_mid)
        self.canvas.create_text((tx + scope_mid) / 2.0, y + 15.5, text="Cette page", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if page_scope else "normal"))
        type_x1 = scope_mid + scope_gap
        control_box("scope_type", y, 31.0, selected=type_scope, x1=type_x1, x2=inner_right)
        self.canvas.create_text((type_x1 + inner_right) / 2.0, y + 15.5, text="Tout le type", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if type_scope else "normal"))
        y += 40.0
        ...
        control_box("toggle_status", y, 34.0)
        self.canvas.create_oval(tx + 10, y + 11, tx + 22, y + 23, fill=status_color, outline="")
        self.canvas.create_text(tx + 30, y + 17, text=status_text, anchor="w", fill=status_color, font=(theme.FONT_UI, 9, "bold"))
        self.canvas.create_text(inner_right - 9, y + 17, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 12, "bold"))
        y += 47.0
        y = separator(y)

        y = section_label(y, "LIVRE")
        fmt = self.gabarit_book_format()
        control_box("edit_format", y, 39.0)
        self.canvas.create_text(tx + 10, y + 12, text="Format fini", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        self.canvas.create_text(
            tx + 10, y + 28,
            text=f"{fmt['label']} · {fmt['width_mm']:g} × {fmt['height_mm']:g} mm",
            anchor="w", fill="#B8C6C3", font=(theme.FONT_UI, 8),
        )
        self.canvas.create_text(inner_right - 9, y + 19.5, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += 44.0
        ...
        self.canvas.create_text(tx + 10, y + 12, text="Marges", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        self.canvas.create_text(tx + 10, y + 28, text=compact_values(margins, ("top", "bottom", "inside", "outside"), ("H", "B", "I", "E")), anchor="w", fill="#B8C6C3", font=(theme.FONT_UI, 8))
        self.canvas.create_text(inner_right - 9, y + 19.5, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += 44.0
        control_box("edit_bleed", y, 39.0)
        self.canvas.create_text(tx + 10, y + 12, text="Fond perdu", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        self.canvas.create_text(tx + 10, y + 28, text=compact_values(bleed, ("top", "bottom", "left", "right"), ("H", "B", "G", "D")), anchor="w", fill="#D1AAAA", font=(theme.FONT_UI, 8))
        self.canvas.create_text(inner_right - 9, y + 19.5, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += 39.0
