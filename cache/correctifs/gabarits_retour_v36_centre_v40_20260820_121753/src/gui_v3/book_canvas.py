...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Largeur mémorisée hors de la boucle de rendu. Elle sert au centrage exact
        # entre les deux zones latérales sans forcer Tk à repeindre pendant un zoom.
        try:
            self._gabarit_zoom_controls_width = float(self.gabarit_zoom_controls.winfo_reqwidth())
        except Exception:
            self._gabarit_zoom_controls_width = 0.0

        def _remember_gabarit_zoom_width(_event=None):
            try:
                value = float(self.gabarit_zoom_controls.winfo_width())
                if value > 1.0:
                    self._gabarit_zoom_controls_width = value
            except Exception:
                pass

        self.gabarit_zoom_controls.bind("<Configure>", _remember_gabarit_zoom_width, add="+")
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        ...
        target.create_text(tx+10, y+mix(10.0,12.0), text="Format fini", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        target.create_text(tx+10, y+book_control_h-mix(9.0,13.0), text=f"{fmt['label']} · {fmt['width_mm']:g} × {fmt['height_mm']:g} mm", anchor="w", fill="#B8C6C3", font=(theme.FONT_UI, 8))
        target.create_text(inner_right-9, y+book_control_h/2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += book_control_h + book_gap

        settings = self.gabarit_current_settings() or {}
        margins = dict(settings.get("margins_mm") or {})
        bleed = dict(settings.get("bleed_mm") or {})
        def compact_values(values, keys, labels):
        ...
        target.create_text(tx+10, y+mix(10.0,12.0), text="Marges", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        target.create_text(tx+10, y+book_control_h-mix(9.0,13.0), text=compact_values(margins, ("top","bottom","inside","outside"), ("H","B","I","E")), anchor="w", fill="#B8C6C3", font=(theme.FONT_UI, 8))
        target.create_text(inner_right-9, y+book_control_h/2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
        y += book_control_h + book_gap

        control_box("edit_bleed", y, book_control_h)
        target.create_text(tx+10, y+mix(10.0,12.0), text="Fond perdu", anchor="w", fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"))
        target.create_text(tx+10, y+book_control_h-mix(9.0,13.0), text=compact_values(bleed, ("top","bottom","left","right"), ("H","B","G","D")), anchor="w", fill="#D1AAAA", font=(theme.FONT_UI, 8))
        target.create_text(inner_right-9, y+book_control_h/2.0, text="›", anchor="e", fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"))
    ...
    def _gabarit_left_separator_x(self, viewport_w: float | None = None) -> float:
        """Frontière réelle entre toute la commande gauche (outils + zoom) et B.

        IMPORTANT : ce calcul doit rester purement géométrique. Ne jamais appeler
        update()/update_idletasks() ici : _render_gabarit_workspace() commence par
        canvas.delete("all") et un flush d'idle à cet instant affiche brièvement
        le Canvas vide, ce qui provoque exactement le clignotement au zoom.
        """
        width = float(viewport_w if viewport_w is not None else self.canvas.winfo_width())
        tools_w = self._gabarit_tools_reserved_width(width)
        separator_x = tools_w + 10.0
        zoom_w = float(getattr(self, "_gabarit_zoom_controls_width", 0.0) or 0.0)
        if zoom_w <= 1.0:
            zoom_controls = getattr(self, "gabarit_zoom_controls", None)
            if zoom_controls is not None:
                try:
                    # winfo_reqwidth() est une simple lecture ici : surtout aucun
                    # flush de la boucle Tk pendant le rendu.
                    zoom_w = float(zoom_controls.winfo_reqwidth())
                except Exception:
                    zoom_w = 0.0
        if zoom_w > 1.0:
            separator_x = max(separator_x, tools_w + 8.0 + zoom_w + 10.0)
        return separator_x

    def _gabarit_work_rect_for_size(
        self, viewport_w: float, viewport_h: float,
    ) -> tuple[float, float, float, float]:
        """Surface centrale strictement comprise entre les deux traits de séparation."""
        width = max(1.0, float(viewport_w))
        height = max(1.0, float(viewport_h))
        left_separator = self._gabarit_left_separator_x(width)
        right_separator = width - float(self._gabarit_inspector_width(width)) - 8.0
        # Même respiration de chaque côté : la page et la ligne ont donc exactement
        # le même centre géométrique entre la commande gauche et l'inspecteur droit.
        side_inset = 12.0
        work_left = left_separator + side_inset
        work_right = max(work_left + 1.0, right_separator - side_inset)
        work_top = 82.0
    ...
    def _gabarit_work_rect(self) -> tuple[float, float, float, float]:
        """Surface réellement disponible pour la page, hors outils et inspecteur."""
    ...
    def _gabarit_fit_scale(self, viewport_w: float, viewport_h: float) -> float:
        ...
        work_left, _work_top, work_right, _work_bottom = self._gabarit_work_rect_for_size(
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        # Double tampon léger : ne jamais vider B avant d'avoir construit la nouvelle
        # image. Au zoom à la molette, plusieurs rendus peuvent s'enchaîner très
        # vite ; canvas.delete("all") exposait alors brièvement le fond entre deux
        # images. On conserve l'ancienne image dessous, on dessine la nouvelle
        # par-dessus, puis on supprime l'ancienne à l'idle suivant.
        old_frame_tag = "_tl_gabarit_previous_frame"
        try:
            self.canvas.addtag_withtag(old_frame_tag, "all")
            cleanup_job = getattr(self, "_gabarit_frame_cleanup_job", None)
            if cleanup_job is not None:
                try:
                    self.after_cancel(cleanup_job)
                except Exception:
                    pass

            def cleanup_previous_frame():
                self._gabarit_frame_cleanup_job = None
                try:
                    self.canvas.delete(old_frame_tag)
                except tk.TclError:
                    pass

            self._gabarit_frame_cleanup_job = self.after_idle(cleanup_previous_frame)
        except tk.TclError:
            # Repli sûr si le Canvas est en cours de destruction/recréation.
        ...
        work_left, work_top, work_right, work_bottom = self._gabarit_work_rect_for_size(
            viewport_w, viewport_h,
        )
        ...
        # ----- inspecteur contextuel droit : couche indépendante -----
        # Ne jamais le dessiner dans self.canvas : le zoom de la page efface et
        # reconstruit ce Canvas, alors que les deux rails doivent rester immobiles.
        self._render_gabarit_inspector_overlay()
