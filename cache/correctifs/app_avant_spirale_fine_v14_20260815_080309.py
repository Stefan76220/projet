...
# SPIRALE_AJUSTEE_POSITION_COULEUR_V13
    ...
    def _draw_notebook_spiral_canvas(
        ...
        if zone_bg is None:
            zone_bg = getattr(theme, "PANEL", "#333845")
        image = self._get_tomelinea_spiral_image(
            max(70, int(y_bottom - y_top)),
            zone_bg=zone_bg,
        )
        x_draw = int(round(x - (image.width() * 0.60)))
        ...
        if not hasattr(canvas, "_tl_spiral_refs_v13"):
            canvas._tl_spiral_refs_v13 = []
        canvas._tl_spiral_refs_v13.append(image)
        return item


    def _attach_notebook_spiral_frame(self, frame, accent=None):
        if getattr(frame, "_tl_spiral_bound_v13", False):
            return
        ...
        frame._tl_spiral_holder_v13 = holder

        def refresh(_event=None):
            try:
                zone_bg = frame.cget("bg")
            except Exception:
                zone_bg = "#333845"

            try:
                page_bg = frame.master.cget("bg")
            except Exception:
                page_bg = zone_bg

            holder.configure(bg=page_bg)

            target_height = max(90, int(frame.winfo_height()) - 18)
            image = self._get_tomelinea_spiral_image(target_height, zone_bg=zone_bg)
            holder.configure(image=image)
            holder.image = image

            x_pos = -int(round(image.width() * 0.60))
            y_pos = max(0, (int(frame.winfo_height()) - image.height()) // 2)
        ...
        frame._tl_spiral_bound_v13 = True

    def _get_tomelinea_spiral_image(self, target_height, zone_bg="#333845"):
        if not hasattr(self, "_tl_spiral_cache_v13"):
            self._tl_spiral_cache_v13 = {}

        try:
            target_height = max(60, int(target_height))
        except Exception:
            target_height = 120

        if not isinstance(zone_bg, str) or not zone_bg.startswith("#") or len(zone_bg) != 7:
            zone_bg = "#333845"
        zone_bg = zone_bg.lower()

        cache_key = (target_height, zone_bg)
        cached = self._tl_spiral_cache_v13.get(cache_key)
        if cached is not None:
            return cached
        ...
        source = Image.open(TOMELINEA_SPIRAL_MODEL_V13).convert("RGBA")

        fixed_width = 50
        scaled_height = max(target_height, int(round(source.height * (fixed_width / float(source.width)))))
        resized = source.resize((fixed_width, scaled_height), Image.LANCZOS)

        if resized.height > target_height:
            top = max(0, (resized.height - target_height) // 2)
            work = resized.crop((0, top, resized.width, top + target_height))
        elif resized.height < target_height:
            pad = Image.new("RGBA", (resized.width, target_height), (0, 0, 0, 0))
            y_off = (target_height - resized.height) // 2
            pad.paste(resized, (0, y_off), resized)
            work = pad
        else:
            work = resized

        zone_rgb = _hex_to_rgb(zone_bg)
        band_left = int(round(work.width * 0.43))
        band_right = int(round(work.width * 0.78))
        px = work.load()

        for yy in range(work.height):
            for xx in range(band_left, min(work.width, band_right + 1)):
                r, g, b, a = px[xx, yy]
                if a == 0:
                    continue
                ...
                if sat < 0.30 and 16 < lum < 175:
                    shade = lum / 110.0
                    nr = int(zone_rgb[0] * 0.68 + zone_rgb[0] * 0.40 * shade)
                    ng = int(zone_rgb[1] * 0.68 + zone_rgb[1] * 0.40 * shade)
                    nb = int(zone_rgb[2] * 0.68 + zone_rgb[2] * 0.40 * shade)
                    nr = max(0, min(255, int(nr * 0.90 + r * 0.10)))
                    ng = max(0, min(255, int(ng * 0.90 + g * 0.10)))
                    nb = max(0, min(255, int(nb * 0.90 + b * 0.10)))
                    px[xx, yy] = (nr, ng, nb, a)

        tk_image = ImageTk.PhotoImage(work)
        self._tl_spiral_cache_v13[cache_key] = tk_image
        return tk_image
