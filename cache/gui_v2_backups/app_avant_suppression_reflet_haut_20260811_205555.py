    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton de commande : verre dépoli légèrement rugueux."""
            ...
            shadow = self._mix(color, "#6E6D68", 0.34 if state == "hover" else 0.40)
            face = self._mix(color, "#FBF6F0", 0.79 if state == "hover" else (0.82 if state == "normal" else 0.76))
            inner_face = self._mix(color, "#F7F2EB", 0.86)
            outline = self._mix(color, "#EFE5D9", 0.30)
            inner_outline = self._mix(color, "#FFFFFF", 0.84)
            highlight = self._mix(color, "#FFFFFF", 0.95)
            haze = self._mix(color, "#FFFFFF", 0.72)
            rough_light = self._mix(color, "#FFFFFF", 0.70)
            rough_dark = self._mix(color, "#E3D9CC", 0.74)
            text_color = self._mix(color, "#26342F", 0.56)
            ...
            canvas.create_line(
                x1 + 18, y1 + 8,
                x2 - 18, y1 + 8,
                fill=self._mix(face, "#FFFFFF", 0.52),
                width=1,
                tags=(tag, "home_button"),
            )

            # Voile dépoli.
            for yy, w in ((y1 + 12, 4), (y1 + 18, 3), (y2 - 9, 2)):
                canvas.create_line(
                    x1 + 16, yy,
                    x2 - 16, yy,
                    fill=haze,
            ...
            # Rugosité discrète.
            dots = [
                (x1 + 12, y1 + 11), (x1 + 18, y1 + 16), (x1 + 15, y2 - 11),
                (x2 - 20, y1 + 12), (x2 - 15, y1 + 17), (x2 - 18, y2 - 10),
                (cx - 26, cy - 3), (cx + 23, cy + 2),
            ]
            for i, (gx, gy) in enumerate(dots):
                canvas.create_oval(
                    gx, gy, gx + 1.4, gy + 1.4,
                    fill=rough_light if i % 2 == 0 else rough_dark,
                    outline="",
                    tags=(tag, "home_button"),
                )
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Carte de choix : verre dépoli un peu rugueux."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            base_mix = 0.82 if not selected else 0.75
            if hover:
                base_mix -= 0.04
            if pressed:
                base_mix += 0.01

            face = self._mix(color, "#FBF6F0", max(0.68, base_mix))
            inner_face = self._mix(color, "#F7F2EB", 0.86)
            shadow = self._mix(color, "#706D67", 0.36 if hover else 0.42)
            outline = self._mix(color, "#EFE5D9", 0.22 if selected else 0.36)
            inner_outline = self._mix(color, "#FFFFFF", 0.84)
            top_light = self._mix(color, "#FFFFFF", 0.95)
            haze = self._mix(color, "#FFFFFF", 0.70)
            rough_light = self._mix(color, "#FFFFFF", 0.72)
            rough_dark = self._mix(color, "#E3D9CC", 0.76)
            ...
            canvas.create_line(
                x1 + 24, y1 + 10,
                x2 - 24, y1 + 10,
                fill=self._mix(face, "#FFFFFF", 0.50),
                width=1,
                tags=(tag, "home_choice"),
            )

            # Voile dépoli large et doux.
            for yy, w in ((y1 + 24, 7), (y1 + 39, 5), (y2 - 18, 4)):
                canvas.create_line(
                    x1 + 18, yy,
                    x2 - 18, yy,
                    fill=haze,
            ...
            # Rugosité diffuse.
            dots = [
                (x1 + 16, y1 + 17), (x1 + 24, y1 + 25), (x1 + 20, y2 - 18),
                (x1 + 31, y2 - 13), (x2 - 21, y1 + 18), (x2 - 14, y1 + 27),
                (x2 - 26, y2 - 17), (x2 - 17, y2 - 11), (x1 + 46, y1 + 15),
                (x2 - 48, y1 + 14), (x1 + 42, y2 - 14), (x2 - 44, y2 - 15),
            ]
            for i, (gx, gy) in enumerate(dots):
                canvas.create_oval(
                    gx, gy, gx + 1.5, gy + 1.5,
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule informative : verre dépoli rugueux."""
            width = max(64, int(len(text) * 5.0) + 31)
            height = 20

            face = self._mix(fg, "#FBF6F0", 0.84)
            inner_face = self._mix(fg, "#F7F2EB", 0.88)
            outline = self._mix(fg, "#EFE5D9", 0.34)
            inner_outline = self._mix(fg, "#FFFFFF", 0.84)
            top_light = self._mix(fg, "#FFFFFF", 0.95)
            rough_light = self._mix(fg, "#FFFFFF", 0.72)
            rough_dark = self._mix(fg, "#E3D9CC", 0.76)
            ...
            canvas.create_line(
                x + 12, y + 10,
                x + width - 10, y + 10,
                fill=self._mix(face, "#FFFFFF", 0.48),
                width=2,
                capstyle="round",
                tags=tag,
            )
            for i, (gx, gy) in enumerate(((x + 9, y + 9), (x + width - 14, y + 8), (x + width - 19, y + 12))):
                canvas.create_oval(
                    gx, gy, gx + 1.3, gy + 1.3,
                    fill=rough_light if i % 2 == 0 else rough_dark,
                    outline="",
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Raccourci : verre dépoli un peu rugueux."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FBF6F0", face_mix)
            inner_face = self._mix(color, "#F7F2EB", 0.86)
            shadow = self._mix(color, "#726E68", 0.38 if state == "hover" else 0.43)
            outline = self._mix(color, "#EFE5D9", 0.36)
            inner_outline = self._mix(color, "#FFFFFF", 0.84)
            top_light = self._mix(color, "#FFFFFF", 0.95)
            haze = self._mix(color, "#FFFFFF", 0.68)
            rough_light = self._mix(color, "#FFFFFF", 0.72)
            rough_dark = self._mix(color, "#E3D9CC", 0.76)
            ...
            dots = [
                (x1 + 12, y1 + 16), (x1 + 20, y2 - 13),
                (x2 - 18, y1 + 15), (x2 - 12, y2 - 12),
                (x1 + 44, y1 + 13), (x2 - 42, y2 - 14),
            ]
            for i, (gx, gy) in enumerate(dots):
                canvas.create_oval(
                    gx, gy, gx + 1.3, gy + 1.3,
