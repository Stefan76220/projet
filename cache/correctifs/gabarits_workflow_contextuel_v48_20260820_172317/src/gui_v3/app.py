...
class TomeLineaV3(tk.Tk):
    ...
    def _build_gabarits_tab(self, parent):
        ...
        # ----------------------------------------------------------
        # Réglages globaux du livre — un seul accès, une seule fenêtre.
        # ----------------------------------------------------------
        ...
        # 2 — PLACEMENT
        # ----------------------------------------------------------
        # Règle ergonomique TomeLinea :
        #   1) sélectionner la zone dans B ;
        #   2) choisir l'outil ;
        #   3) valider directement dans le panneau local.
        # Aucun Toplevel ni sous-menu supplémentaire pour Placement.
        ...
        # Éditeur contextuel local : visible uniquement après le clic sur l'outil.
        # Il reste dans le rail gauche, donc il ne peut ni se perdre derrière B ni
        # ajouter un clic de navigation vers une palette séparée.
        placement_editor = tk.Frame(
            place_box, bg="#182936", highlightthickness=1,
            highlightbackground=CARD_BORDER, padx=8, pady=8,
        )
        placement_editor.grid(row=4, column=0, sticky="ew", pady=(8, 0))
        placement_editor.grid_remove()

        def close_placement_editor():
            placement_editor.grid_remove()
            for child in placement_editor.winfo_children():
                child.destroy()

        def show_placement_editor(title, builder):
            for child in placement_editor.winfo_children():
                child.destroy()
            header = tk.Frame(placement_editor, bg="#182936")
            header.pack(fill="x", pady=(0, 7))
            tk.Label(
                header, text=title, bg="#182936", fg=TEXT,
                font=(theme.FONT_UI, 8, "bold"),
            ).pack(side="left")
            close_label = tk.Label(
                header, text="×", bg="#182936", fg=MUTED,
                font=(theme.FONT_UI, 10, "bold"), cursor="hand2", padx=3,
            )
            close_label.pack(side="right")
            close_label.bind("<Button-1>", lambda _e: close_placement_editor())
            body = tk.Frame(placement_editor, bg="#182936")
            body.pack(fill="x")
            builder(body, close_placement_editor)
            placement_editor.grid()

        def open_occupation():
            canvas = book()
            info = canvas.gabarit_selected_zone_info() if canvas is not None else {}
            if not info or not bool(info.get("spread")):
                return

            def build(host, close):
                tk.Label(
                    host, text="Position dans la double page", bg="#182936", fg=MUTED,
                    font=(theme.FONT_UI, 7), anchor="w",
                ).pack(fill="x", pady=(0, 6))
                current = str(info.get("spread_position") or "left")
                for label, position in (("Gauche", "left"), ("Centre", "center"), ("Droite", "right")):
                    caption = f"✓  {label}" if position == current else label
                    def apply(p=position):
                        if canvas.gabarit_set_selected_zone_spread_position(p):
                            refresh_gabarit_tools()
                        close()
                    V3Button(host, caption, apply, compact=True).pack(fill="x", pady=2)

            show_placement_editor("Occupation", build)
        ...
        def open_dimensions():
            ...
            def build(host, close):
                ...
                vars_ = {}
                rows = (("x", "X"), ("y", "Y"), ("w", "Largeur"), ("h", "Hauteur"))
                first_entry = None
                for r, (key, label) in enumerate(rows):
                    tk.Label(
                        host, text=label, bg="#182936", fg=MUTED,
                        font=(theme.FONT_UI, 7),
                    ).grid(row=r, column=0, sticky="w", pady=2, padx=(0, 6))
                    var = tk.StringVar(value=f"{float(rect.get(key, 0)):g}")
                    vars_[key] = var
                    entry = tk.Entry(
                        host, textvariable=var, width=8, justify="right",
                        bg="#20313B", fg=TEXT, insertbackground=TEXT,
                        relief="flat", bd=0, highlightthickness=1,
                    ...
                    tk.Label(
                        host, text="mm", bg="#182936", fg=MUTED,
                        font=(theme.FONT_UI, 7),
                ...
                tk.Label(
                    host, textvariable=error_var, bg="#182936", fg="#D99A9A",
                    font=(theme.FONT_UI, 7), anchor="w",
                ...
                def apply():
                    try:
                        ...
                        return
                    if not canvas.gabarit_set_selected_zone_rect_mm(
                        values["x"], values["y"], values["w"], values["h"]
                    ):
                        error_var.set("Dimensions incompatibles avec la page")
                        return
                    refresh_gabarit_tools()
                    close()

                validate = V3Button(host, "Valider", apply, compact=True, primary=True)
                validate.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(7, 0))
                for entry in [w for w in host.winfo_children() if isinstance(w, tk.Entry)]:
                    entry.bind("<Return>", lambda _e: apply())
                if first_entry is not None:
                    first_entry.focus_set()
                    first_entry.selection_range(0, "end")

            show_placement_editor("Dimensions", build)
        ...
        def open_snap():
            ...
            if not info:
                return
            current = canvas.gabarit_snap_preset()

            def build(host, close):
                tk.Label(
                    host, text="Le choix s'applique immédiatement", bg="#182936", fg=MUTED,
                    font=(theme.FONT_UI, 7), anchor="w",
                ).pack(fill="x", pady=(0, 6))
                choices = (
                    ("Aucun", "none"),
                ...
                for label, preset in choices:
                    caption = f"✓  {label}" if preset == current else label
                    def apply(p=preset):
                        canvas.gabarit_set_snap_preset(p)
                        refresh_gabarit_tools()
                        close()
                    V3Button(host, caption, apply, compact=True).pack(fill="x", pady=2)

            show_placement_editor("Accrochage", build)
        ...
        def refresh_gabarit_tools(_event=None):
            ...
            for btn in (dimensions_btn, snap_btn, align_btn, delete_zone_btn):
                btn.configure(state=state, cursor="hand2" if active else "arrow")
            spread_active = active and bool(selected.get("spread"))
            occupation_btn.configure(
                state="normal" if spread_active else "disabled",
                cursor="hand2" if spread_active else "arrow",
            )
            if not active:
                close_placement_editor()
            try:
    ...
    def _show_gabarit_book_settings_dialog(self, *, reopen: bool = False) -> bool:
        ...
        adapt_frame.grid(row=4, column=0, sticky="ew", pady=(3, 7))
        ...
        note.grid(row=5, column=0, sticky="w", pady=(2, 0))
