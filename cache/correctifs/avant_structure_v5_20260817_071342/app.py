...
class TomeLineaV3(tk.Tk):
    ...
    def _build_structure_tab(self, parent):
        """Structure V4 : C fournit les briques ; B est l'unique zone de construction."""
        ...
        # Bandeau contextuel : il agit toujours sur la sélection courante de B.
        ...
        def delete_selected():
            ...
            if not messagebox.askyesno(
                "Structure du livre",
                "Supprimer l'élément sélectionné ?\n\nLes couvertures et les parties contenant encore des pages sont protégées.",
                parent=self,
            ):
                return
        ...
        palette_hint_var = tk.StringVar(value="Clic : choisir la brique  •  Glisser : déposer directement dans B")
        tk.Label(
        ...
        def create_custom_type():
            ...
            if canvas is None or not hasattr(canvas, "structure_create_custom_type"):
                return
            canvas.structure_create_custom_type(name.strip())
            select_family("Types")
        ...
        def select_family(name: str):
            ...
            if name == "Pages":
                for type_key, label, _custom in page_types():
                    make_brick(row, label, "page", (type_key, label)).pack(
                        side="left", padx=(0, 6), pady=(0, 6)
                    )
                palette_hint_var.set("Clic : choisir le type  •  puis clic dans B  •  dépôt sur une page = changer son type")

            elif name == "Parties":
                make_brick(row, "+ Partie", "group", ("partie", "+ Partie"), accent=True).pack(
                    side="left", padx=(0, 7), pady=(0, 6)
                )
                ...
                palette_hint_var.set("Posez + Partie dans B. Son ordre reste ensuite modifiable par glisser-déposer.")

            elif name == "Blancs":
                make_brick(row, "Page blanche", "page", ("page_blanche", "Page blanche")).pack(
                    side="left", padx=(0, 7), pady=(0, 6)
                )
                ...
                palette_hint_var.set("Blanc volontaire : déposez-le librement, ou ajoutez-le avant/après la page sélectionnée.")

            elif name == "Types":
                small_action(row, "+ Nouveau type", create_custom_type).pack(side="left", padx=(0, 8), pady=(0, 6))
                custom = [(key, label) for key, label, is_custom in page_types() if is_custom]
                for type_key, label in custom:
                    make_brick(row, label, "page", (type_key, label), accent=True).pack(
                        side="left", padx=(0, 6), pady=(0, 6)
                    )
                if not custom:
                    tk.Label(
                ...
                palette_hint_var.set("Créez un type une seule fois : il devient ensuite une brique disponible dans Pages.")

            elif name == "Règles":
                small_action(row, "Blanc avant", lambda: insert_blank_relative("before")).pack(side="left", padx=(0, 6), pady=(0, 6))
                small_action(row, "Blanc après", lambda: insert_blank_relative("after")).pack(side="left", padx=(0, 6), pady=(0, 6))
                tk.Label(
                    row, text="Recto / verso sera branché après validation de ce premier atelier.",
                    bg=theme.PANEL_ALT, fg=theme.MUTED_DARK,
                    font=(theme.FONT_UI, 8),
