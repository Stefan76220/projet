...
class BookCanvas(tk.Frame):
    ...
    def _selected_double_page_pair_context(self) -> dict:
        ...
        # AV/AP et les anciennes contraintes R/V restent mémorisées sur chaque page.
        # Tant que la paire est 2P, elles ne doivent pas empêcher l'association :
        # les contraintes incompatibles à l'intérieur de la paire sont simplement
        # suspendues et réapparaîtront si la double page est scindée.
        ...
        try:
            ...
            for gid in group_order:
                ...
                for pos, source in enumerate(pages):
                    ...
                    # AV/AP restent attachées à chaque page. Si la demande tombe
                    # exactement ENTRE les deux moitiés d'une 2P, elle est seulement
                    # suspendue pendant l'association ; elle réapparaît après Scinder.
                    if before_type and not internal_before:
                        add_slot_role(bkey, "AV", source, before_type)
                    if after_type and not internal_after:
