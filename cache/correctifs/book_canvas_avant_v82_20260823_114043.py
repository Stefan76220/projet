...
# TOMELINEA_GABARITS_ARCHITECTURE_PERMANENTE_V81 — réglages/ajout/historique permanents + rail contextuel stable
...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_inspector_draw_icon(self, target, icon: str, box, *, selected=False, danger=False, disabled=False, hovered=False):
        ...
        if icon in {"undo", "redo"}:
            # V81 — convention graphique la plus simple : flèches de retour standard.
            # Aucun dessin décoratif : la reconnaissance prime.
            c = teal if hovered and not disabled else fg
            glyph = "↶" if icon == "undo" else "↷"
            target.create_text(cx, cy-1, text=glyph, anchor="center", fill=c, font=(theme.FONT_UI, 22, "bold"))
            return
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict, *, multi_infos=None) -> None:
        ...
        permanent_w = 164.0
        permanent_left = max(8.0, right - permanent_w)

        # Réglages généraux — bouton permanent, distinct des outils de zone.
        settings_y1, settings_y2 = 8.0, 43.0
        self._gabarit_inspector_control_box(target, "edit_book_settings", (permanent_left, settings_y1, right, settings_y2))
        self._gabarit_inspector_draw_icon(target, "book_settings", (permanent_left+5.0, settings_y1+3.0, permanent_left+37.0, settings_y2-3.0), hovered=(self._gabarit_inspector_hover=="edit_book_settings"))
        ...
        for i,(key,icon,tip) in enumerate((
            ...
            self._gabarit_inspector_icon_button(target,key,(x1,add_y,x1+add_button,add_y+add_button),icon,tooltip=tip)
        ...
        self._gabarit_inspector_icon_button(target,"history_undo",(quick_x,quick_y,quick_x+quick_button,quick_y+quick_button),"undo",disabled=not self.can_undo(),tooltip="Annuler · Ctrl+Z")
        quick_x += quick_button + quick_gap
        self._gabarit_inspector_icon_button(target,"history_redo",(quick_x,quick_y,quick_x+quick_button,quick_y+quick_button),"redo",disabled=not self.can_redo(),tooltip="Rétablir · Ctrl+Y / Ctrl+Maj+Z")
        quick_x += quick_button + quick_gap
        self._gabarit_inspector_icon_button(target,"zone_delete",(quick_x,quick_y,quick_x+quick_button,quick_y+quick_button),"delete",danger=True,disabled=count==0,tooltip="Supprimer la sélection · Suppr")

        target.create_line(permanent_left, add_y+add_button+8.0, right, add_y+add_button+8.0, fill="#34464F", width=1)
        target.create_line(permanent_left, quick_y-8.0, right, quick_y-8.0, fill="#34464F", width=1)
        ...
        target.create_rectangle(rail_x1-2.0, y-4.0, right+1.0, min(rail_bottom+4.0,y+total_h+4.0), fill="#141E24", outline="#34464F", width=1)
        ...
        if active_band == "allowed":
            ...
            ),button=34.0,gap=10.0,y_offset=175.0)
