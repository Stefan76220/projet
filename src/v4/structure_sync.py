...
from src.v4.structure_spreads import (
    SPREAD_LEFT,
    SPREAD_RIGHT,
    restore_auto_spreads,
    structure_spread_issues,
    suspend_auto_spreads,
)
...
def _sync_av_ap(
    ...
    old_positions = {page_id: index for index, page_id in enumerate(old_order)}
    ...
    # Une page automatique qui a reçu du contenu ou une décision éditoriale
    # ne disparaît jamais silencieusement. Si sa relation AV/AP n'existe plus,
    # elle devient une page TomeLinea retenue, à la même position approximative.
    protected_unused = [
        candidate
        for candidate in candidates
        if candidate.id not in book.pages
        and auto_page_is_protected(candidate)
    ]

    for candidate in sorted(
        protected_unused,
        key=lambda page: old_positions.get(page.id, len(old_order)),
    ):
        retain_auto_page(candidate)
        book.pages[candidate.id] = candidate

        old_index = old_positions.get(candidate.id, len(old_order))
        next_id = next(
            (
                page_id
                for page_id in old_order[old_index + 1:]
                if page_id in book.pages and page_id in book.page_order
            ),
            None,
        )
        prev_id = next(
            (
                page_id
                for page_id in reversed(old_order[:old_index])
                if page_id in book.pages and page_id in book.page_order
            ),
            None,
        )
        ...
        if next_id is not None:
            insert_at = book.page_order.index(next_id)
        elif prev_id is not None:
            insert_at = book.page_order.index(prev_id) + 1
        else:
            insert_at = len(book.page_order)
        book.page_order.insert(insert_at, candidate.id)
...
def sync_structure_rules(
    ...
    suspended_auto_spreads = suspend_auto_spreads(book)
    _sync_av_ap(
        book
    )
    restore_auto_spreads(book, suspended_auto_spreads)
    ...
    from src.v4.structure_covers import cover_structure_issues
