...
class BookCanvas(tk.Frame):
    ...
    def _render_page_overlay(self):
        ...
        # La page reste la vedette. Les informations sont maintenant placées
        # dans un bandeau léger, au-dessus de la page, et ne dépendent plus
        # de sa largeur : elles restent toujours visibles pendant le zoom.
        ...
        band_y = 122.0
        band_h = 68.0
        left_x1 = max(32.0, width * 0.12)
        left_x2 = width * 0.43
        right_x1 = width * 0.57
        right_x2 = min(width - 32.0, width * 0.88)

        # Voiles très discrets : lisibles mais non bloquants.
        target.create_rectangle(left_x1, band_y - band_h / 2.0, left_x2, band_y + band_h / 2.0, fill="#1D2731", stipple="gray50", outline="")
        target.create_rectangle(right_x1, band_y - band_h / 2.0, right_x2, band_y + band_h / 2.0, fill="#1D2731", stipple="gray50", outline="")

        # Repères sobres connectés à la page, sans la masquer.
        guide_y = band_y + band_h / 2.0 + 12.0
        target.create_line(left_x2 - 24, guide_y, x - 18, guide_y, fill=theme.BORDER_SOFT, width=1)
        target.create_line(x + page_w + 18, guide_y, right_x1 + 24, guide_y, fill=theme.BORDER_SOFT, width=1)
        target.create_oval(x - 23, guide_y - 3, x - 17, guide_y + 3, fill=theme.ACCENT_DARK, outline="")
        target.create_oval(x + page_w + 17, guide_y - 3, x + page_w + 23, guide_y + 3, fill=self.ORANGE, outline="")

        # Bloc gauche : titre et type.
        target.create_text(left_x1 + 10, band_y - 20, text="TITRE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(left_x1 + 10, band_y + 0, text=page_title, anchor="w", fill=theme.INK, font=(theme.FONT_TITLE, 15, "bold"), width=max(120, left_x2 - left_x1 - 20))
        target.create_text(left_x2 - 10, band_y - 20, text="TYPE", anchor="e", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(left_x2 - 10, band_y + 0, text=page_type, anchor="e", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 10, "bold"), width=max(110, left_x2 - left_x1 - 24))

        # Bloc droit : partie et avancement.
        target.create_text(right_x1 + 10, band_y - 20, text="PARTIE", anchor="w", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right_x1 + 10, band_y + 0, text=part_value, anchor="w", fill=theme.INK, font=(theme.FONT_TITLE, 14, "bold"), width=max(140, right_x2 - right_x1 - 20))
        target.create_text(right_x2 - 10, band_y - 20, text="AVANCEMENT", anchor="e", fill=self.GOLD, font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right_x2 - 10, band_y - 1, text=stage_text, anchor="e", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 11, "bold"))
        target.create_text(right_x2 - 10, band_y + 19, text=f"Page {index + 1} sur {len(self.items)}", anchor="e", fill=theme.MUTED, font=(theme.FONT_UI, 9))
