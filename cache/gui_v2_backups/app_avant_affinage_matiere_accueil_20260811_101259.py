    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton de commande : base claire éditoriale + accent coloré discret."""
            ...
            face = self._mix(color, "#FBF8F2", 0.90 if hover else 0.93)
            inner_face = self._mix(color, "#FEFDFC", 0.95 if hover else 0.97)
            shadow = self._mix(color, "#8F877C", 0.72 if hover else 0.77)
            outline = self._mix(color, "#CFC6BB", 0.42 if hover else 0.54)
            accent = self._mix(color, "#4F5A54", 0.08 if hover else 0.18)
            text_color = "#2C342F"
            ...
            home_round_rect(
                ...
                10,
                fill=inner_face,
                outline=self._mix(face, "#FFFFFF", 0.55),
                width=1,
                tags=(tag, "home_button"),
            )

            # Léger reflet, sans aplat.
            canvas.create_line(
                x1 + 16, y1 + 5,
                x2 - 16, y1 + 5,
                fill=self._mix(face, "#FFFFFF", 0.72),
                width=2,
                tags=(tag, "home_button"),
            )

            # Accent coloré réduit à une fine ligne basse.
            canvas.create_line(
                x1 + 12, y2 - 5,
                x2 - 12, y2 - 5,
                fill=accent,
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Zone de choix : carte papier satinée, presque neutre, avec accent discret."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FBF8F3", 0.92 if hover else 0.95)
            inner_face = self._mix(color, "#FFFDFC", 0.96 if hover else 0.98)
            shadow = self._mix(color, "#8D867C", 0.76 if hover else 0.80)
            outline = (
                self._mix(color, "#BFB6AA", 0.18)
                if selected
                else self._mix(color, "#CFC7BC", 0.56)
            )
            accent = self._mix(color, "#5F6A62", 0.05 if selected else 0.16)
            texture = self._mix(color, "#E8E0D5", 0.78)
            ...
            # Texture très légère pour casser l'aplat.
            mid = (y1 + y2) / 2
            for dy in (-14, 0, 14):
                canvas.create_line(
                    x1 + 16, mid + dy,
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Grand bouton raccourci : même langage, sans aplat massif."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FBF8F3", 0.93 if hover else 0.95)
            inner_face = self._mix(color, "#FFFDFC", 0.96 if hover else 0.98)
            shadow = self._mix(color, "#8E877D", 0.77 if hover else 0.81)
            outline = self._mix(color, "#CEC5BA", 0.54)
            accent = self._mix(color, "#58645E", 0.08 if hover else 0.16)
            ...
            # Accent vertical discret à gauche.
            home_round_rect(
                canvas,
                x1 + 8, y1 + 10,
                x1 + 14, y2 - 10,
            ...
            canvas.create_line(
                x1 + 18, y2 - 6,
                x2 - 14, y2 - 6,
                fill=self._mix(accent, "#FFFFFF", 0.30),
                width=1,
                tags=("home_shortcuts", tag),
            )
