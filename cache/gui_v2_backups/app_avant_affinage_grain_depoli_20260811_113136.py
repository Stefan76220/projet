    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def home_command_button(
            ...
            *,
            ...
            """Bouton de commande : verre dépoli teinté avec légère brillance."""
            ...
            shadow = self._mix(color, "#5A6460", 0.30 if state == "hover" else 0.35)
            rim = self._mix(color, "#F4EFE8", 0.34)
            face = self._mix(
                color,
                "#FFF9F4",
                0.80 if state == "normal" else (0.74 if state == "hover" else 0.83),
            )
            inner = self._mix(color, "#FFFFFF", 0.88)
            glass = self._mix(color, "#FFFFFF", 0.94)
            veil = self._mix(color, "#F8F4EE", 0.84)
            shine = self._mix(color, "#FFFFFF", 0.98)
            text_color = self._mix(color, "#273229", 0.54)
            ...
            canvas.create_line(
                x1 + 16, y1 + 18,
                x2 - 16, y1 + 18,
                fill=self._mix(veil, "#FFFFFF", 0.50),
                width=4,
                capstyle="round",
                tags=(tag, "home_button"),
            )
            ...
            # Petit éclat perlé discret.
            canvas.create_line(
                x2 - 26, y1 + 9,
                x2 - 18, y1 + 14,
                fill=self._mix(color, "#FFFFFF", 0.78),
                width=2,
                tags=(tag, "home_button"),
            )
        ...
        def home_choice_panel(
            ...
            *,
            ...
            """Carte de choix : verre dépoli teinté, léger et lisible."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            base_mix = 0.80 if not selected else 0.73
            if hover:
                base_mix -= 0.05
            if pressed:
                base_mix += 0.02

            face = self._mix(color, "#FFF8F2", max(0.64, base_mix))
            shadow = self._mix(color, "#79736A", 0.64 if hover else 0.69)
            outline = self._mix(color, "#FFF5ED", 0.22 if selected else 0.40)
            inner = self._mix(color, "#FFFFFF", 0.84)
            veil = self._mix(color, "#FBF8F3", 0.80)
            glass = self._mix(color, "#FFFFFF", 0.94)
            shine = self._mix(color, "#FFFFFF", 0.98)
            ...
            # Voile dépoli : doux et large, sans texture ponctuée.
            canvas.create_line(
                x1 + 16, y1 + 25,
                x2 - 16, y1 + 25,
                fill=glass,
            ...
            canvas.create_line(
                x1 + 22, y2 - 20,
                x2 - 22, y2 - 20,
                fill=self._mix(veil, "#FFFFFF", 0.42),
                width=6,
                capstyle="round",
                tags=(tag, "home_choice"),
            )

            # Léger éclat dans le coin haut droit.
            canvas.create_line(
                x2 - 34, y1 + 13,
                x2 - 20, y1 + 22,
                fill=self._mix(color, "#FFFFFF", 0.76),
                width=2,
                tags=(tag, "home_choice"),
            )
        ...
        def home_chip(canvas, *, x, y, text, bg, fg, tag):
            """Capsule informative : verre dépoli teinté."""
            width = max(64, int(len(text) * 5.0) + 31)
            height = 20

            face = self._mix(fg, "#FFF8F2", 0.84)
            outline = self._mix(fg, "#FFF5ED", 0.42)
            inner = self._mix(fg, "#FFFFFF", 0.84)
            veil = self._mix(fg, "#FBF8F3", 0.82)
            shine = self._mix(fg, "#FFFFFF", 0.98)
            ...
            canvas.create_line(
                x + 10, y + 10,
                x + width - 10, y + 10,
                fill=self._mix(veil, "#FFFFFF", 0.45),
                width=3,
                capstyle="round",
                tags=tag,
            )
        ...
        def home_shortcut_button(
            ...
            *,
            ...
            """Raccourci : verre dépoli teinté cohérent avec les cartes."""
            if not hasattr(self, "_home_interaction_state"):
                self._home_interaction_state = {}
            ...
            face = self._mix(color, "#FFF8F2", face_mix)
            shadow = self._mix(color, "#7B746B", 0.65 if state == "hover" else 0.70)
            outline = self._mix(color, "#FFF5ED", 0.38)
            inner = self._mix(color, "#FFFFFF", 0.84)
            veil = self._mix(color, "#FBF8F3", 0.82)
            shine = self._mix(color, "#FFFFFF", 0.98)
            ...
            canvas.create_line(
                x1 + 14, y1 + 20,
                x2 - 14, y1 + 20,
                fill=self._mix(veil, "#FFFFFF", 0.48),
                width=6,
                capstyle="round",
                tags=("home_shortcuts", tag),
            )
