...
class BookCanvas(tk.Frame):
    ...
    def viewer_insert_blank_relative(self, physical_index: int, position: str) -> dict:
        ...
        new_index = next((i for i, candidate in enumerate(self.items) if str(candidate.get("id") or "") == new_id), None)
        if new_index is None:
            new_index = min(index + (1 if position == "after" else 0), max(0, len(self.items) - 1))
        self._set_single_page_selection(new_index)
        self.render()
        self.status_var.set("Page blanche ajoutée depuis le Visionneur.")
        return {
            "ok": True,
            "page_index": int(new_index),
    ...
    def viewer_delete_physical_page(self, physical_index: int) -> dict:
        ...
        target_index = max(0, min(index, len(self.items) - 1))
        self._set_single_page_selection(target_index)
        self.render()
