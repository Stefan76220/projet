...
class BookCanvas(tk.Frame):
    ...
    def _draw_page_auto_rule_markers(self, item: dict, x: float, y: float, page_w: float, scale: float) -> None:
        """Petit maillon sur la page source : lié ou désolidarisé de la règle."""
        if self._is_automatic_page(item) or self._is_locked_page(item):
            return
        source_type = self._type_of(item)
        if not source_type:
            return
        size = max(6.0, min(11.0, 8.0 * scale))
        cy = y + max(16.0, 23.0 * scale)
        for position in ("before", "after"):
            ...
            cx = x + max(9.0, 13.0 * scale) if position == "before" else x + page_w - max(9.0, 13.0 * scale)
            color = "#B7745C" if excluded else "#4F9A80"
            gap = size * (0.72 if excluded else 0.42)
            r = size * 0.45
            self.canvas.create_oval(cx-gap-r, cy-r, cx-gap+r, cy+r, outline=color, width=max(1, int(1.5*scale)))
            self.canvas.create_oval(cx+gap-r, cy-r, cx+gap+r, cy+r, outline=color, width=max(1, int(1.5*scale)))
            if excluded:
                self.canvas.create_line(cx-size*.22, cy+size*.50, cx+size*.22, cy-size*.50, fill=color, width=max(1, int(1.5*scale)))
            else:
                self.canvas.create_line(cx-gap+r*.55, cy, cx+gap-r*.55, cy, fill=color, width=max(1, int(1.5*scale)))
