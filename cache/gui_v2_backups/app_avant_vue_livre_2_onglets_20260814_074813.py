...
class PageMaitreV2(tk.Tk):
    ...
    def _tomelinea_open_global_view(self):
        """Vue panoramique horizontale de tout le livre.

        Réutilise exclusivement le vocabulaire visuel du Maquettage :
        - bornes et livres de la ligne Structure ;
        - trois tailles de pages de Composition ;
        - une seule ligne de lecture de gauche à droite.
        """
        ...
        # Ouverture atomique : la fenêtre apparaît directement terminée.
        ...
        width = max(1120, screen_w - 36)
        height = min(620, max(500, int(screen_h * 0.63)))

        if hasattr(self, "_accueil_center_base_window"):
            self._accueil_center_base_window(win, width, height)
        else:
        ...
        win._global_view_bg_source = None
        win._global_view_bg_photo = None
        win._global_view_structure_photos = {}
        win._global_view_page_photos = []

        try:
            if bg_path.exists():
                win._global_view_bg_source = Image.open(
                    bg_path
                ).convert("RGB")
        except Exception:
            win._global_view_bg_source = None
        ...
        # ----------------------------------------------------------
        # Panneau central à angles coupés.
        # ----------------------------------------------------------
        viewport = tk.Canvas(
            outer,
            bg=PANEL,
        ...
        def panel_points(x1, y1, x2, y2, cut=18, dx=0, dy=0):
            return [
                x1 + dx, y1 + dy,
                x2 - cut + dx, y1 + dy,
                x2 + dx, y1 + cut + dy,
        ...
        def redraw_shell(_event=None):
            w = max(outer.winfo_width(), width)
            h = max(outer.winfo_height(), height)

            outer.delete("global_bg")
            outer.delete("global_panel")

            if win._global_view_bg_source is not None:
                try:
                    image = win._global_view_bg_source.resize(
                        (w, h),
                        Image.Resampling.LANCZOS,
                    )
                    image = Image.blend(
                        image.convert("RGB"),
                        Image.new("RGB", image.size, "#D6D7D6"),
                        0.08,
                    )
                    win._global_view_bg_photo = ImageTk.PhotoImage(image)
                    outer.create_image(
                        0,
                        0,
                        image=win._global_view_bg_photo,
                        anchor="nw",
                        tags=("global_bg",),
                    )
                except Exception:
            ...
            outer.create_polygon(
                panel_points(x1, y1, x2, y2, dx=2, dy=4),
                fill="#D1CCC4",
                outline="",
                tags=("global_panel",),
            )
            outer.create_polygon(
                panel_points(x1, y1, x2, y2),
                fill=PANEL,
                outline=BORDER,
                width=1,
                tags=("global_panel",),
            ...
            outer.coords(viewport_id, x1 + 24, y1 + 18)
            outer.itemconfigure(
                viewport_id,
                width=max(900, x2 - x1 - 48),
                height=max(400, y2 - y1 - 36),
            )

            outer.tag_lower("global_panel")
            outer.tag_lower("global_bg")
        ...
        # Titre aligné comme les autres zones TomeLinea :
        # texte, respiration, trait noir et cercle terminal.
        title_row = tk.Frame(
            header,
            bg=PANEL,
            height=31,
        ...
        def draw_title_line(_event=None):
            ...
            line_w = max(52, title_line.winfo_width())
            yy = 13

            # Même respiration que les autres titres TomeLinea :
            # le cercle reste visible et la ligne ne colle jamais à la fermeture.
            end_x = max(18, line_w - 28)
        ...
        subtitle = tk.Label(
            header,
            text="Aperçu de l’enchaînement complet des pages du livre.",
            bg=PANEL,
            fg=MUTED,
            font=("Segoe UI", 8),
        )
        subtitle.place(x=10, y=42)

        # ----------------------------------------------------------
        # Fermeture : reprend exactement l’icône Fermer du ruban.
        # ----------------------------------------------------------
        ...
        close.place(relx=1.0, x=-44, y=1, anchor="ne")

        close_state = {"hover": False, "pressed": False}
        win._global_view_close_photos = {}

        def get_close_photo(state):
            state = state if state in ("normal", "survol", "actif") else "normal"

            if state in win._global_view_close_photos:
                return win._global_view_close_photos[state]

            try:
                photo = self._get_nav_photo("fermer", state, 32)
            except Exception:
                photo = None

            win._global_view_close_photos[state] = photo
            return photo
        ...
        def close_enter(_event=None):
            close_state["hover"] = True
            draw_close()

        def close_leave(_event=None):
            close_state["hover"] = False
            close_state["pressed"] = False
            draw_close()

        def close_press(_event=None):
            close_state["pressed"] = True
            draw_close()

        def close_release(_event=None):
            if close_state["pressed"]:
                win.destroy()

        close.bind("<Enter>", close_enter)
        close.bind("<Leave>", close_leave)
        close.bind("<ButtonPress-1>", close_press)
        close.bind("<ButtonRelease-1>", close_release)
        draw_close()

        # ----------------------------------------------------------
        # Zone panoramique.
        # ----------------------------------------------------------
        panorama_shell = tk.Frame(
            viewport,
            bg=PANEL,
        )
        panorama_shell.pack(
        ...
        panorama.pack(fill="both", expand=True)

        # ----------------------------------------------------------
        # Icônes Structure identiques à celles du Maquettage.
        # ----------------------------------------------------------
        structure_icon_dir = (
        ...
        def structure_photo(path, max_w, max_h, *, green_flag=False):
            key = (str(path), max_w, max_h, bool(green_flag))
            cached = win._global_view_structure_photos.get(key)
            if cached is not None:
                return cached
            ...
            try:
                ...
                photo = ImageTk.PhotoImage(image)
                win._global_view_structure_photos[key] = photo
                return photo
            except Exception:
                return None

        # ----------------------------------------------------------
        # Fonds de borne : petits dossiers de maquettage légèrement teintés.
        # Les livres reprennent discrètement la couleur de leur partie.
        # ----------------------------------------------------------
        def mix_hex(color_a, color_b, ratio):
            ...
            b = rgb(color_b)
            mixed = tuple(
                int(round(a[i] * (1.0 - ratio) + b[i] * ratio))
                for i in range(3)
            )
        ...
        def draw_structure_tile(
            ...
            *,
            ...
            tile_w = 96 if book else 84
            tile_h = 60 if book else 58
            x1 = cx - tile_w / 2
            x2 = cx + tile_w / 2
            y2 = icon_anchor_y + 7
            y1 = y2 - tile_h
            ...
            if book:
                tile_fill = mix_hex(accent, "#FFFFFF", 0.86)
                tile_outline = mix_hex(accent, "#BDB8B0", 0.62)
                grid = mix_hex(accent, "#FFFFFF", 0.91)
                crop = mix_hex(accent, "#7D8784", 0.66)
            else:
                tile_fill = "#F4F1E9"
                tile_outline = "#D3CEC5"
                grid = "#DEE5E1"
            ...
            # Traits de construction / repères de coupe très discrets.
            ...
            mark = 7
            panorama.create_line(
                x1 + 6, y1 + 6, x1 + 6 + mark, y1 + 6,
                fill=crop, width=1,
            )
            panorama.create_line(
                x1 + 6, y1 + 6, x1 + 6, y1 + 6 + mark,
                fill=crop, width=1,
            )
            panorama.create_line(
                x2 - 6 - mark, y2 - 6, x2 - 6, y2 - 6,
                fill=crop, width=1,
            )
            panorama.create_line(
                x2 - 6, y2 - 6 - mark, x2 - 6, y2 - 6,
                fill=crop, width=1,
            )

        # ----------------------------------------------------------
        # Données de pages regroupées par partie.
        # ----------------------------------------------------------
        pages_by_group = {}
        for page in pages:
        ...
        # Trois tailles exactement reprises de Composition.
        page_ratio = 1.40
        ...
        # La vue est une vue de contrôle : toute la frise est centrée
        # verticalement dans l’espace disponible, quelle que soit la taille
        # de l’écran.
        win.update_idletasks()
        panorama_h = max(390, panorama.winfo_height())
        panorama_w = max(900, panorama.winfo_width())

        marker_y = int(panorama_h * 0.23)
        guide_y = int(panorama_h * 0.33)
        page_baseline = int(panorama_h * 0.68)

        # ----------------------------------------------------------
        # Dessin d'une page selon la grammaire Composition.
        # ----------------------------------------------------------
        def draw_page(px, page):
            kind_text = str(page.get("kind", "Page courante"))
            low_kind = kind_text.strip().lower()
            ...
            if visual_kind == "head":
                ...
                if low_kind == "couverture":
                    text = "COUVERTURE"
                elif low_kind == "4e de couverture":
                    text = "4e DE\nCOUVERTURE"
                else:
                    text = "TÊTE DE\nPARTIE"
        ...
        # ----------------------------------------------------------
        # Calcul du flux complet avant dessin.
        # ----------------------------------------------------------
        groups_layout = []
        cursor_x = left_pad
        ...
        for group_index, group in enumerate(model):
            ...
            if group_pages:
                for page_index, page in enumerate(group_pages):
                    kind_text = str(page.get("kind", "Page courante")).lower()
                ...
                # Une partie vide reste visible comme borne,
                # sans inventer de page fantôme.
                end_x = start_x + 44
                cursor_x = end_x
        ...
        # Centre horizontalement la frise lorsqu’elle tient dans la fenêtre.
        # Pour les livres longs, on conserve une marge d’entrée puis le
        # défilement horizontal prend naturellement le relais.
        natural_total_w = cursor_x + right_pad
        if natural_total_w < panorama_w:
            shift_x = (panorama_w - natural_total_w) / 2
            for group_data in groups_layout:
                group_data["start_x"] += shift_x
                group_data["end_x"] += shift_x
                group_data["center_x"] += shift_x
                for page_layout in group_data["page_layouts"]:
                    page_layout["x"] += shift_x
            total_w = panorama_w
        else:
            total_w = natural_total_w

        # ----------------------------------------------------------
        # Fond de contrôle derrière les livres ET les pages.
        # Une table de montage papier légèrement grisée détache les objets
        # sans prendre le dessus sur la lecture du livre.
        # ----------------------------------------------------------
        band_x1 = 24
        band_x2 = max(band_x1 + 240, total_w - 24)
        band_y1 = max(14, marker_y - 76)
        band_y2 = min(
            panorama_h - 42,
            int(page_baseline + large_h / 2 + 50),
        )
        band_cut = 16

        def band_points(dx=0, dy=0):
            return [
                band_x1 + dx, band_y1 + dy,
                band_x2 - band_cut + dx, band_y1 + dy,
                band_x2 + dx, band_y1 + band_cut + dy,
                band_x2 + dx, band_y2 + dy,
                band_x1 + band_cut + dx, band_y2 + dy,
                band_x1 + dx, band_y2 - band_cut + dy,
            ]
        ...
        # Grille de table de montage, très espacée et presque ton sur ton.
        grid_color = "#E5E1DA"
        grid_step_x = 96
        grid_step_y = 44

        grid_x = band_x1 + 34
        while grid_x < band_x2 - 24:
            panorama.create_line(
                grid_x,
                band_y1 + 14,
                grid_x,
        ...
        # Petit repère de maquettage : rappel du rail de Composition.
        panorama.create_line(
            band_x1 + 18,
            band_y1 + 14,
            band_x1 + 82,
        ...
        # Ligne de lecture : très discrète derrière les pages.
        first_page_center = None
        last_page_center = None

        for group_data in groups_layout:
            if group_data["page_layouts"]:
                first_layout = group_data["page_layouts"][0]
                last_layout = group_data["page_layouts"][-1]

                first_page_center = (
                    first_page_center
                    if first_page_center is not None
                    else first_layout["x"] + first_layout["w"] / 2
                )
                last_page_center = (
                    last_layout["x"] + last_layout["w"] / 2
                )

        if first_page_center is not None and last_page_center is not None:
            panorama.create_line(
                first_page_center,
                page_baseline,
                last_page_center,
        ...
        # ----------------------------------------------------------
        # Bornes de parties + pages.
        # ----------------------------------------------------------
        previous_page_box = None

        for group_data in groups_layout:
            index = group_data["index"]
            group = group_data["group"]
            name = group_data["name"]
            page_layouts = group_data["page_layouts"]

            if page_layouts:
                marker_x = page_layouts[0]["x"]
            else:
                marker_x = group_data["center_x"]

            is_start = index == 0
            is_end = index == len(groups_layout) - 1

            if is_start:
                marker_center = marker_x + 12
                icon_path = (
                    ribbon_flag_path
            ...
            draw_structure_tile(
                marker_center,
                marker_y,
                book=not (is_start or is_end),
                accent=str(group.get("color", "#8D70C7")),
            )
            ...
            # Guide vertical extrêmement léger :
            # suffit à identifier le début d'une nouvelle partie.
            panorama.create_line(
                marker_center,
                guide_y + 34,
                marker_center,
                page_baseline - large_h / 2 - 14,
                fill="#E3DED7",
                width=1,
                dash=(2, 3),
            )
            ...
            # Pages du groupe.
            for page_data in page_layouts:
                box = draw_page(
                    page_data["x"],
                    page_data["page"],
                )

                if previous_page_box is not None:
                    connector_x1 = previous_page_box["x2"] + 5
                    connector_x2 = box["x1"] - 5

                    if connector_x2 > connector_x1:
                        connector_y = page_baseline
                        panorama.create_line(
                            connector_x1,
                            connector_y,
                            connector_x2,
                ...
                previous_page_box = box

        # ----------------------------------------------------------
        # Scroll horizontal TomeLinea.
        # ----------------------------------------------------------
        scrollbar_h = 30
        scrollbar = tk.Canvas(
            panorama_shell,
        ...
        def scroll_geometry():
            ...
            first = max(
                0.0,
                min(1.0, scroll_state["first"]),
            )
            last = max(
                first,
                min(1.0, scroll_state["last"]),
            )
            ...
            x1 = left + travel * first
            x2 = x1 + thumb_w

            return left, right, x1, x2, travel, thumb_w

        def draw_scrollbar():
            scrollbar.delete("all")
            left, right, x1, x2, _travel, _thumb_w = (
                scroll_geometry()
            )
            yy = 15
            ...
            if scroll_state["last"] - scroll_state["first"] < 0.999:
                round_rect(
                    scrollbar,
                    x1,
                    yy - 5,
        ...
        panorama.configure(
            xscrollcommand=update_scrollbar,
            scrollregion=(0, 0, total_w, panorama_h),
        )
        ...
        def scrollbar_press(event):
            ...
            if x1 <= event.x <= x2:
                ...
                scroll_state["offset"] = event.x - x1
                draw_scrollbar()
                return

            target = event.x - thumb_w / 2
            target = max(left, min(right - thumb_w, target))
            fraction = (
                0.0
                if travel <= 0
                else (target - left) / travel
            )
            panorama.xview_moveto(fraction)
        ...
        def scrollbar_drag(event):
            ...
            target = event.x - scroll_state["offset"]
            target = max(left, min(right - thumb_w, target))
            fraction = (
                0.0
                if travel <= 0
        ...
        def scrollbar_release(_event=None):
            scroll_state["dragging"] = False

        def scrollbar_enter(_event=None):
            scroll_state["hover"] = True
            draw_scrollbar()

        def scrollbar_leave(_event=None):
            if not scroll_state["dragging"]:
                scroll_state["hover"] = False
                draw_scrollbar()

        scrollbar.bind("<Configure>", lambda _e: draw_scrollbar())
        scrollbar.bind("<Enter>", scrollbar_enter)
        scrollbar.bind("<Leave>", scrollbar_leave)
        scrollbar.bind("<ButtonPress-1>", scrollbar_press)
        ...
        def horizontal_wheel(event):
            if getattr(event, "delta", 0):
                ...
                panorama.xview_scroll(step * 3, "units")
                return "break"
            return None

        panorama.bind("<MouseWheel>", horizontal_wheel)
        win.bind("<Shift-MouseWheel>", horizontal_wheel)

        win.bind(
            "<Left>",
            lambda _e: panorama.xview_scroll(-5, "units"),
        )
        win.bind(
            "<Right>",
            lambda _e: panorama.xview_scroll(5, "units"),
        ...
        # Le ruban reste verticalement centré dans la zone panoramique.
        panorama.configure(scrollregion=(0, 0, total_w, panorama_h))
        panorama.xview_moveto(0.0)

        # Apparition finale atomique.
        win.update_idletasks()
        redraw_shell()
        draw_title_line()
        draw_scrollbar()
