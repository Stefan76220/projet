    ...
    def _build_maquettage(self, parent: tk.Frame) -> None:
        ...
        def zone(
            ...
            *,
            ...
            shadow = self._mix(accent, "#77706A", 0.78)
            face = self._mix(fill, "#FFFDFC", 0.16)
            outline = self._mix(accent, "#FFFFFF", 0.52)
        ...
        def render(_event=None):
            ...
            margin = 44
            top_y1 = 112
            top_y2 = min(318, max(286, height * 0.38))
            ...
            zone(
                ...
                subtitle=(
                    "Définissez les groupes, leur ordre et leur nombre de pages. "
                    "Le contenu détaillé sera composé dans l’espace inférieur."
                ),
            )

            track_left = margin + 32
            track_right = width - margin - 32
            track_y = top_y1 + 103

            # Les groupes non encore dimensionnés gardent une largeur minimale.
            weights = []
            for group in structure_model:
                pages = group["pages"]
                weights.append(
                    max(4, pages if isinstance(pages, int) else 4)
                )

            total_weight = max(sum(weights), 1)
            usable = track_right - track_left
            min_segment = 108
            raw_widths = [
                max(
                    min_segment,
                    usable * weight / total_weight,
                )
                for weight in weights
            ]
            scale = usable / max(sum(raw_widths), 1)
            seg_widths = [w * scale for w in raw_widths]

            x = track_left

            for index, group in enumerate(structure_model):
                segment_w = seg_widths[index]
                x1 = x
                x2 = x + segment_w
                color = group["color"]
            ...
            # Barre de commandes intégrée à la structure.
            commands_y1 = top_y2 - 54
            commands_y2 = top_y2 - 18
            cmd_x = margin + 24

            canvas.create_text(
                cmd_x,
                (commands_y1 + commands_y2) / 2,
                text="Commandes de structure",
                fill="#5A6570",
                font=("Segoe UI", 8, "bold"),
                anchor="w",
                tags="maquettage_ui",
            ...
            # Zone de choix / création des types.
            palette_y = work_y2 - 94
            canvas.create_line(
                cx1,
                palette_y - 18,
                cx2,
            ...
            canvas.create_text(
                cx1,
                palette_y,
                text="Types de page de cette partie",
                fill=theme.INK,
                font=("Segoe UI", 9, "bold"),
                anchor="nw",
                tags="maquettage_ui",
            ...
            canvas.create_text(
                cx1,
                palette_y + 25,
                text="Aucun type de page n’est encore défini.",
                fill="#68737C",
                font=("Segoe UI", 8),
                anchor="nw",
                tags="maquettage_ui",
            ...
            ry += 90
            divider(right_x1 + 20, ry - 12, ry + 142)
            ...
            action_y = work_y2 - 105

            for row_index, pair in enumerate(
                (
                    (("Monter", "#72AFCB"), ("Descendre", "#72AFCB")),
                    (("Dupliquer", "#8D70C7"), ("Supprimer", "#E28A6D")),
                )
            ):
                ay = action_y + row_index * 44
