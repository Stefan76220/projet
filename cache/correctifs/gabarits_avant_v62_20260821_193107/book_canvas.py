...
# TOMELINEA_GABARITS_PALETTE_VISUELLE_V61
...
class BookCanvas(tk.Frame):
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        en-tête. V61 resserre légèrement les hauteurs afin de garder Portée visible
        et remplace l'aimant d'accrochage par une représentation du contact réel.
        Les clés d'action historiques restent inchangées.
        """
        ...
        def render_geometry(y0: float) -> float:
            ...
            for i,(lab,field,key) in enumerate(fields):
                row,col = divmod(i,2)
                x1=tx+col*(bw+gap); yy=y0+row*28.0
                self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+25))
                target.create_text(x1+7,yy+12.5,text=lab,anchor="w",fill="#81959B",font=(theme.FONT_UI,7,"bold"))
                target.create_text(x1+bw-7,yy+12.5,text=f"{float(rect.get(field,0)):g} mm",anchor="e",fill="#D8E0E1",font=(theme.FONT_UI,8,"bold"))
            y0 += 56.0
        ...
        def render_allowed(y0: float) -> float:
            ...
            for i,ref in enumerate(refs[:2]):
                ...
                self._gabarit_inspector_icon_button(target,f"frame_{ref}",(x1,y0,x1+bw,y0+36),icons[ref],selected=pending==ref,tooltip=tip)
            y0 += 42.0
        ...
        def render_align(y0: float) -> float:
            ...
            for i,(key,icon,tip) in enumerate(actions):
                row,col=divmod(i,3); x1=tx+col*(bw+gap); yy=y0+row*36.0
                self._gabarit_inspector_icon_button(target,key,(x1,yy,x1+bw,yy+32),icon,tooltip=tip)
            return y0 + 72.0 + 5.0
        ...
        def render_snap(y0: float) -> float:
            ...
            for i,(key,icon,preset,tip) in enumerate(items):
                row,col=divmod(i,3); x1=tx+col*(bw+gap); yy=y0+row*36.0
                self._gabarit_inspector_icon_button(target,key,(x1,yy,x1+bw,yy+32),icon,selected=current==preset,tooltip=tip)
            return y0 + 72.0 + 5.0
        ...
        def render_layer(y0: float) -> float:
            ...
            for i,(key,icon,tip) in enumerate(items):
                x1=tx+i*(bw+gap)
                self._gabarit_inspector_icon_button(target,key,(x1,y0,x1+bw,y0+34),icon,tooltip=tip)
            return y0 + 39.0
        ...
        def render_actions(y0: float) -> float:
            ...
            for i,(key,icon,selected,tip,danger) in enumerate(items):
                x1=tx+i*(bw+gap)
                self._gabarit_inspector_icon_button(target,key,(x1,y0,x1+bw,y0+35),icon,selected=selected,danger=danger,tooltip=tip)
            return y0 + 40.0
