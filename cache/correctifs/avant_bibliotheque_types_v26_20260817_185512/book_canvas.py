...
class BookCanvas(tk.Frame):
    ...
    def _structure_view_anchor_snapshot(self):
        """Mémorise un repère visuel de la ligne avant un rendu Structure."""
        ...
        try:
            ...
            return None
        if not getattr(self, "_page_hitboxes", None):
            return {"anchor_id": "", "screen_x": None, "left": left}
        center_world = left + view_w / 2.0
        candidates = []
        for index, box in self._page_hitboxes.items():
            if not 0 <= int(index) < len(self.items):
                continue
            x1, _y1, x2, _y2 = box
            ...
            candidates.append((abs(world_x - center_world), page_id, world_x - left))
        if not candidates:
            return {"anchor_id": "", "screen_x": None, "left": left}
        _dist, page_id, screen_x = min(candidates, key=lambda item: item[0])
        return {"anchor_id": page_id, "screen_x": float(screen_x), "left": left}

    def _structure_restore_view_anchor(self, snapshot) -> None:
        """Restaure la position visuelle de la ligne après un rendu Structure."""
        ...
        anchor_id = str(snapshot.get("anchor_id") or "")
        desired_screen_x = snapshot.get("screen_x")
        if anchor_id and desired_screen_x is not None:
            new_index = next(
                (i for i, item in enumerate(self.items) if str(item.get("id") or "") == anchor_id),
                None,
            )
            if new_index is not None and new_index in self._page_hitboxes:
