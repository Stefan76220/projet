...
class MockupView:
    ...
    def _create_deleted_parts_history(
        ...
        """Historique fixe des Parties et groupes libres supprimés."""
        block = ctk.CTkFrame(
            parent,
            width=width,
            height=104,
        ...
        for index, group in enumerate(deleted_parts):
            raw_label = str(group.get("deleted_label") or "").strip()
            if not raw_label:
                ...
                if title.startswith("Partie "):
                    ...
                    raw_label = (
                        f"P{number_text}" if number_text.isdigit() else "Partie"
                    )
                else:
                    raw_label = title.split("—", 1)[0].strip() or "Groupe"
