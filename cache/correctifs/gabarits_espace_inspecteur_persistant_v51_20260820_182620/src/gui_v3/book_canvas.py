...
class BookCanvas(tk.Frame):
    ...
    def _show_gabarit_tools_host(self):
        ...
        if bool(getattr(self, "_gabarit_tools_overlay", False)):
            self._schedule_gabarit_tools_overlay_sync()
            return
        try:
            host.place(
                x=0, y=0, width=int(getattr(self, "_gabarit_tools_width", 196)),
                relheight=1.0,
            )
            host.lift()
        except Exception:
    ...
    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        ...
        y += 24
        target.create_line(tx, y, right, y, fill="#40525B", width=1); y += 14
        ...
        ref_label = "PAGE · EXCEPTION" if alignment_exception else "MARGES"
        ...
        # Marges est la règle implicite du livre. On n'affiche PAS un choix
        # Marges/Page permanent : seule l'exception est proposée.
        ...
        # Exception rare : un seul bouton discret. 2e clic = préparer la règle,
        # 3e clic = choisir la portée, conformément à la règle TomeLinea.
        exception_key = "align_use_margins" if alignment_ref == "page" else "align_use_page"
        exception_text = "Revenir aux marges" if alignment_ref == "page" else "Utiliser la page"
        self._gabarit_inspector_control_box(target, exception_key, (tx,y,right,y+28), selected=bool(pending_ref))
        target.create_text((tx+right)/2,y+14,text=exception_text,anchor="center",fill="#D3DDDA",font=(theme.FONT_UI,7,"bold" if pending_ref else "normal"))
        y += 34

        if pending_ref in {"margins", "page"}:
            target.create_text(tx, y, text="APPLIQUER À", anchor="w", fill="#9FB0B3", font=(theme.FONT_UI, 7, "bold")); y += 15
    ...
    def _render_gabarit_multi_inspector(self, target, width: float, height: float, infos: list[dict]) -> None:
        ...
        target.create_line(tx,y,right,y,fill="#40525B",width=1); y+=15
        ...
        target.create_text(tx,y,text="DISTRIBUER",anchor="w",fill="#A8BCC0",font=(theme.FONT_UI,8,"bold")); y+=17
        bw=(inner-gap)/2.0
    ...
    def _gabarit_tools_reserved_width(self, viewport_w: float | None = None) -> float:
        """Largeur réservée aux outils verticaux de Gabarits dans B."""
        width = float(viewport_w if viewport_w is not None else (self.canvas.winfo_width() if hasattr(self, "canvas") else 1200))
        base = float(getattr(self, "_gabarit_tools_width", 196))
        # Sur un écran étroit on gagne quelques pixels sans rendre les boutons illisibles.
        return max(176.0, base - (12.0 if width < 1180.0 else 0.0))

    def _gabarit_left_separator_x(self, viewport_w: float | None = None) -> float:
        width = float(viewport_w if viewport_w is not None else self.canvas.winfo_width())
        tools_w = self._gabarit_tools_reserved_width(width)
        zoom_w = float(getattr(self, "_gabarit_zoom_controls_width", 68.0))
        return max(tools_w + 10.0, tools_w + 8.0 + zoom_w + 10.0)
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float, *, update_inspector: bool = True) -> None:
        ...
        if zoom_controls is not None:
            try:
                zoom_controls.place(x=tools_w + 8.0, rely=0.54, anchor="w")
                zoom_controls.lift()
            except Exception:
                pass
        self._gabarit_sync_zoom_widget()
