...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def choice_box(tag, x1, y1, x2, y2, *, accent, selected=False, hovered=False):
            if selected:
                fill = mix(accent, "#FFFFFF", 0.91)
                outline = mix(accent, "#FFFFFF", 0.35)
            elif hovered:
                fill = mix(accent, "#FFFFFF", 0.965)
                outline = mix(accent, "#FFFFFF", 0.48)
            else:
                fill = "#FFFDFC"
                outline = "#D8D5D0"
        ...
        def bind_interaction(tag, *, command=None):
            ...
            def enter(_event=None):
                self._accueil_prof_state[tag] = "hover"
                canvas.configure(cursor="hand2")
                redraw()

            def leave(_event=None):
                self._accueil_prof_state[tag] = "normal"
                canvas.configure(cursor="arrow")
                redraw()

            def press(_event=None):
                self._accueil_prof_state[tag] = "pressed"
                redraw()
        ...
        def render(_event=None):
            ...
            status_y = 690
            canvas.create_line(X(690), Y(status_y - 21), X(690), Y(status_y + 21), fill="#DCD8D2")
            canvas.create_line(X(875), Y(status_y - 21), X(875), Y(status_y + 21), fill="#DCD8D2")
            canvas.create_line(X(1060), Y(status_y - 21), X(1060), Y(status_y + 21), fill="#DCD8D2")

            status_items = (
                (720, "État", "●  En cours", "#4C936D"),
                (905, "Étape actuelle", "●  Maquettage", "#8564B1"),
                (1090, "Dernière activité", "◷  10/08/2026 15:23", "#3E607E"),
            )
