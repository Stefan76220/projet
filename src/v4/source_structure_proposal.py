...
def _suggested_kind(
    ...
    if (
        len(body_roles) >= 3
        and opening_count >= 1
        and text_count / len(body_roles) >= 0.70
    ):
        return BookKind.ROMAN
...
def build_book_proposal(
    ...
    *,
    ...
    # ==========================================================
    # Frontieres editoriales
    # ==========================================================
    #
    # Une page marquee "ouverture_partie" n'est plus suffisante
    # a elle seule.
    #
    # TomeLinea demande une confirmation dans les pages qui
    # suivent. Pour cette premiere version du moteur, la preuve
    # forte est une famille de mise en page repetee.
    #
    # Le moteur est volontairement conservateur :
    # en cas de doute, il prefere ne pas couper le livre.
    # ==========================================================

    def _forward_family_confirmation(
        opening_page: int,
    ) -> tuple[
        str | None,
        tuple[int, ...],
        ...
        for candidate in range(
            ...
            if candidate_role in FRONT_ROLES:
                continue

            if candidate_role in BACK_ROLES:
                continue
    ...
    def _boundary_decision(
        ...
        if family_id is not None:
            ...
            reasons.append(
                "famille_suivante_confirmee"
            )

        # Regle essentielle :
        # une frontière peut être prouvée de trois façons :
        # - marqueur structurel explicite ;
        # - titre court repris comme en-tête sur la page suivante ;
        # - confirmation forte par une famille de pages suivantes.
        ...
        for line in next_text.splitlines():
            candidate = line.strip()
            if candidate:
                next_first_line = candidate.lower()
                break
        ...
        explicit_structure = bool(
            ...
            or re.match(
                r"^(premi(?:e|è)re|deuxi(?:e|è)me|"
                r"troisi(?:e|è)me|quatri(?:e|è)me|"
                r"cinqui(?:e|è)me|sixi(?:e|è)me|"
                r"septi(?:e|è)me|huiti(?:e|è)me|"
                r"neuvi(?:e|è)me|dixi(?:e|è)me)\s+"
                r"(partie|part|section|livre)\b",
                first_line,
                flags=re.IGNORECASE,
            )
            or re.fullmatch(
        ...
        repeated_heading = (
            bool(first_line)
            and first_line == next_first_line
    ...
    for page_number in range(
        ...
        if boundary_accepted:
            ...
            # Les faits ayant confirme la coupure sont conserves
            # avec la partie via analysis_refs.
            for confirmed_page in (
                confirmation_pages
            ):
        ...
        # Une ancienne "ouverture_partie" non confirmee
        # reste une page normale du segment courant.
    ...
    # La 3e physique n'est plus injectée dans la proposition de contenu.
    # Son emplacement est créé par le moteur de couvertures puis reste
    # explicitement à confirmer par l'utilisateur.
    must_generate_third_cover = False
