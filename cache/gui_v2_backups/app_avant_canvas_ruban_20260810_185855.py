...
class PageMaitreV2(tk.Tk):
    ...
    def _build_shell(self) -> None:
        ...
        self.header = tk.Frame(self, bg="#F8F4EC", height=126, highlightthickness=0)
        self.header.grid(row=0, column=0, sticky="ew")
        self.header.grid_propagate(False)

        self._build_header_background()

        left = tk.Frame(self.header, bg="#F8F4EC", bd=0, highlightthickness=0)
        left.place(x=18, y=10, width=250, height=104)

        self.header_title = tk.Label(
            left, text="Accueil", bg="#F8F4EC", fg=theme.INK,
            font=("Segoe UI", 10, "bold"), anchor="w"
        )
        self.header_title.pack(anchor="w", pady=(0, 6))

        quick = tk.Frame(left, bg="#F8F4EC")
        quick.pack(anchor="w")
        ...
        self.nav = tk.Frame(self.header, bg="#F8F4EC", bd=0, highlightthickness=0)
        ...
        right = tk.Frame(self.header, bg="#F8F4EC", bd=0, highlightthickness=0)
        right.place(relx=1.0, x=-205, y=10, width=195, height=105)

        self.project_label = tk.Label(
            right, text="projet x  ·  Accueil", bg="#F8F4EC",
