...
class PageMaitreV2(tk.Tk):
    ...
    def _build_visualisation_vierge(self, parent: tk.Frame) -> None:
        """Visualisation — fond général + une seule zone Outils, directement sur le fond."""
        ...
        def render(_event=None):
            ...
            # Fond Visualisation : ivoire/gris chaud, calme au centre,
            # avec un relief diffus repoussé vers les bords.
            bg_size = (width, height)
            if self._visualisation_bg_size != bg_size:
                base = Image.new("RGBA", bg_size, "#D8D4CD")
                haze = Image.new("RGBA", bg_size, (0, 0, 0, 0))
                draw = ImageDraw.Draw(haze, "RGBA")

                # Voiles clairs en périphérie.
                draw.ellipse(
                ...
                # Quelques ombres froides très légères sur les bords.
                draw.ellipse(
                ...
                haze = haze.filter(
                    ImageFilter.GaussianBlur(max(28, int(min(width, height) * 0.045)))
                )
                base = Image.alpha_composite(base, haze)

                # Filigrane éditorial discret uniquement en bordure.
                details = Image.new("RGBA", bg_size, (0, 0, 0, 0))
                d = ImageDraw.Draw(details, "RGBA")
                accent = (63, 99, 118, 28)
                for offset in (0, 15, 30):
                    d.arc(
                        (-110 - offset, 35 + offset, 170 + offset, 315 + offset),
                ...
                self._visualisation_bg_size = bg_size
            ...
            # Même colonne droite que la zone « Propriétés de la page »
            # du Maquettage : mêmes largeur et position.
            margin = 44
            right_w = 300
            top_y2_maquettage = 188
            work_y1 = top_y2_maquettage + 12
            work_y2 = height - 52

            x1 = width - margin - right_w
            x2 = width - margin
            y1 = work_y1
            y2 = work_y2

            # Ombre puis surface : même géométrie TomeLinea que Maquettage.
            canvas.create_polygon(
                panel_points(x1, y1, x2, y2, dx=2, dy=4),
                fill="#BDB7AE",
                outline="",
                tags="visualisation_outils",
            )
            canvas.create_polygon(
