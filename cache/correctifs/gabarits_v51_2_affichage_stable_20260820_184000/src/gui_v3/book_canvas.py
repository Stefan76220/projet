...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Deux surfaces persistantes : Structure et Gabarits ne partagent plus
        # le même Canvas. Le bureau cible est rendu pendant qu'il est encore
        # caché, puis simplement remonté au premier plan. On évite ainsi le
        # démontage/reconstruction visible lors du changement de bureau.
        self.structure_canvas = self.canvas
        self.gabarit_canvas = tk.Canvas(
            viewer,
            bg=theme.WINDOW_DEEP,
            bd=0,
            highlightthickness=0,
        ...
        self.gabarit_canvas.grid(row=0, column=0, sticky="nsew")
        self.gabarit_canvas._tl_item_hover_only = True
        tk.Misc.tkraise(self.structure_canvas)
        ...
        if self._gabarit_tools_overlay:
            self.gabarit_canvas.bind("<Configure>", self._schedule_gabarit_tools_overlay_sync, add="+")
            try:
        ...
        if self._gabarit_inspector_overlay:
            self.gabarit_canvas.bind("<Configure>", self._schedule_gabarit_inspector_overlay_sync, add="+")
            try:
        ...
        # La surface Gabarits possède ses propres bindings. Les deux Canvas
        # restent mappés en permanence ; seul l'ordre d'empilement change.
        gc = self.gabarit_canvas
        gc.bind("<ButtonPress-1>", self._on_press)
        gc.bind("<B1-Motion>", self._on_drag)
        gc.bind("<ButtonRelease-1>", self._on_release)
        gc.bind("<ButtonPress-2>", self._gabarit_middle_press, add="+")
        gc.bind("<B2-Motion>", self._gabarit_middle_drag, add="+")
        gc.bind("<ButtonRelease-2>", self._gabarit_middle_release, add="+")
        gc.bind("<KeyPress-space>", self._gabarit_space_press, add="+")
        gc.bind("<KeyRelease-space>", self._gabarit_space_release, add="+")
        gc.bind("<Double-Button-1>", self._on_double_click)
        gc.bind("<MouseWheel>", self._on_mousewheel)
        gc.bind("<Shift-MouseWheel>", self._on_shift_mousewheel)
        gc.bind("<Control-MouseWheel>", self._on_ctrl_mousewheel)
        gc.bind("<Configure>", lambda e: self._schedule_render(e) if str(getattr(self, "_work_mode", "")) == "gabarits" and self.canvas is gc else None, add="+")
        gc.bind("<Motion>", self._on_hover_motion)
        gc.bind("<Leave>", self._on_hover_leave)
        gc.bind("<Delete>", self._gabarit_delete_key, add="+")
    ...
    def _gabarit_inspector_width(self, viewport_w: float | None = None) -> int:
        ...
        return 210 if width >= 1000.0 else 188
    ...
    def _gabarit_inspector_control_box(self, target, key: str, box, *, selected=False, danger=False, disabled=False):
        ...
        if disabled:
            fill, outline = "#1B252C", "#34434B"
        elif danger:
            fill, outline = ("#4A3030", "#9A5E5E") if not (hovered or pressed) else ("#593838", "#C07878")
        elif selected:
            fill, outline = "#2B4249", theme.ACCENT_BRIGHT
        elif hovered or pressed:
            fill, outline = "#314752", theme.ACCENT_BRIGHT
        else:
            fill, outline = "#202D36", "#465963"
    ...
    def _render_gabarit_create_row(self, target, tx: float, right: float, y: float) -> float:
        ...
        for i, (lab, key) in enumerate((("Texte", "create_text"), ("Image", "create_image"), ("Document", "create_document"))):
            ...
            target.create_text(x1+bw/2.0, y+14.0, text=lab, anchor="center", fill="#E7ECEA", font=(theme.FONT_UI, 6, "bold"))
        return y + 36.0

    def _render_gabarit_zone_inspector(self, target, width: float, height: float, info: dict) -> None:
        target.create_line(0, 8, 0, max(8, height-8), fill="#465A63", width=1)
        tx = 16.0; right = width - 12.0; inner = max(100.0, right-tx)
        kind = {"text":"ZONE TEXTE", "image":"ZONE IMAGE", "document":"ZONE DOCUMENT"}.get(str(info.get("kind") or ""), "ZONE")
        rect = dict(info.get("rect_mm") or {})
        y = 18.0
        target.create_text(tx, y, text=kind, anchor="w", fill="#78B8B1", font=(theme.FONT_UI, 9, "bold"))
        target.create_text(right, y, text="sélection active", anchor="e", fill="#9FB0B3", font=(theme.FONT_UI, 7))
        y += 24
        target.create_line(tx, y, right, y, fill="#40525B", width=1); y += 12
        y = self._render_gabarit_create_row(target, tx, right, y)
        target.create_line(tx, y, right, y, fill="#40525B", width=1); y += 12

        target.create_text(tx, y, text="POSITION & DIMENSIONS", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 18
        # Les quatre compteurs sont les champs d'édition eux-mêmes : clic sur la
        # valeur -> saisie -> Entrée. Aucun changement de zone de l'écran.
        gap = 4.0
        bw = (inner-gap)/2.0
        geom_fields = (("X", "x", "geom_x"), ("Y", "y", "geom_y"),
                       ("L", "w", "geom_w"), ("H", "h", "geom_h"))
        for i, (lab, field, key) in enumerate(geom_fields):
            row=i//2; col=i%2
            x1=tx+col*(bw+gap); yy=y+row*34.0
            self._gabarit_inspector_control_box(target, key, (x1,yy,x1+bw,yy+29))
            target.create_text(x1+8, yy+14.5, text=lab, anchor="w", fill="#8FA2A6", font=(theme.FONT_UI, 7, "bold"))
            target.create_text(x1+bw-8, yy+14.5, text=f"{float(rect.get(field,0)):g} mm", anchor="e", fill="#E7ECEA", font=(theme.FONT_UI, 8, "bold"))
        y += 68.0 + 12.0

        if bool(info.get("spread")):
            target.create_text(tx, y, text="OCCUPATION 2P", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 17
            current = str(info.get("spread_position") or "left")
            gap=4.0; bw=(inner-gap*2)/3.0
            for i,(lab,key,pos) in enumerate((("G","spread_left","left"),("C","spread_center","center"),("D","spread_right","right"))):
                x1=tx+i*(bw+gap); x2=x1+bw
                self._gabarit_inspector_control_box(target,key,(x1,y,x2,y+30),selected=current==pos)
                target.create_text((x1+x2)/2,y+15,text=lab,anchor="center",fill="#F0F2F0",font=(theme.FONT_UI,8,"bold"))
            y += 43

        alignment_ref = self._gabarit_alignment_reference(self._gabarit_active_item())
        alignment_exception = self._gabarit_alignment_is_exception(self._gabarit_active_item())
        pending_ref = str(getattr(self, "_gabarit_alignment_pending_reference", "") or "")
        ref_label = "PAGE · EXCEPTION" if alignment_exception else "MARGES · LIVRE"
        target.create_text(tx, y, text="ALIGNEMENT", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold"))
        target.create_text(right, y, text=ref_label, anchor="e", fill="#78B8B1" if alignment_exception else "#9FB0B3", font=(theme.FONT_UI, 7, "bold" if alignment_exception else "normal")); y += 17

        # L'exception est rare mais doit être immédiatement repérable.
        exception_key = "align_use_margins" if alignment_ref == "page" else "align_use_page"
        exception_text = "Revenir aux marges" if alignment_ref == "page" else "Exception → Page"
        self._gabarit_inspector_control_box(target, exception_key, (tx,y,right,y+29), selected=bool(pending_ref) or alignment_exception)
        target.create_text((tx+right)/2,y+14.5,text=exception_text,anchor="center",fill="#DDE8E5",font=(theme.FONT_UI,7,"bold"))
        y += 35
        if pending_ref in {"margins", "page"}:
            target.create_text(tx, y, text="PORTÉE DE L'EXCEPTION", anchor="w", fill="#9FB0B3", font=(theme.FONT_UI, 7, "bold")); y += 15
            gap=4.0; bw=(inner-gap)/2.0
            self._gabarit_inspector_control_box(target,"align_scope_page",(tx,y,tx+bw,y+28))
            target.create_text(tx+bw/2,y+14,text="Cette page",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
            self._gabarit_inspector_control_box(target,"align_scope_type",(tx+bw+gap,y,right,y+28))
            target.create_text(tx+bw+gap+bw/2,y+14,text="Toutes du type",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
            y += 35

        actions=(("←","align_left"),("↔","align_center"),("→","align_right"),("↑","align_top"),("↕","align_middle"),("↓","align_bottom"))
        gap=4.0; bw=(inner-gap*2)/3.0
        for i,(glyph,key) in enumerate(actions):
            row=i//3; col=i%3; x1=tx+col*(bw+gap); yy=y+row*34
            self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+29))
            target.create_text(x1+bw/2,yy+14.5,text=glyph,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,10,"bold"))
        y += 72

        target.create_text(tx, y, text="ACCROCHAGE", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 17
        current = self.gabarit_snap_preset() if self.gabarit_snap_enabled() else "none"
        snaps=(("Aucun","snap_none"),("Page","snap_page"),("Marges","snap_margins"),("Centre","snap_center"),("Zones","snap_zones"))
        bw=(inner-4.0)/2.0
        for i,(lab,key) in enumerate(snaps):
            row=i//2; col=i%2; x1=tx+col*(bw+4.0); yy=y+row*31
            preset=key.replace("snap_","")
            self._gabarit_inspector_control_box(target,key,(x1,yy,x1+bw,yy+27),selected=current==preset)
            target.create_text(x1+bw/2,yy+13.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold" if current==preset else "normal"))
        y += 98

        target.create_text(tx, y, text="PLAN", anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 8, "bold")); y += 17
        gap=4.0; bw4=(inner-gap*3.0)/4.0
        for i,(lab,key) in enumerate((("Fond","layer_back"),("−","layer_backward"),("+","layer_forward"),("Devant","layer_front"))):
            x1=tx+i*(bw4+gap)
            self._gabarit_inspector_control_box(target,key,(x1,y,x1+bw4,y+29))
            target.create_text(x1+bw4/2,y+14.5,text=lab,anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,6 if lab in {"Fond","Devant"} else 9,"bold"))
        y += 40

        # Actions toujours au même endroit sous les réglages fréquents.
        gap=5.0; bw=(inner-gap)/2.0
        locked=bool(info.get("locked",False))
        self._gabarit_inspector_control_box(target,"zone_lock",(tx,y,tx+bw,y+31),selected=locked)
        target.create_text(tx+bw/2,y+15.5,text="Déverrouiller" if locked else "Verrouiller",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
        self._gabarit_inspector_control_box(target,"zone_duplicate",(tx+bw+gap,y,right,y+31))
        target.create_text(tx+bw+gap+(bw/2),y+15.5,text="Dupliquer",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
        y += 38
        self._gabarit_inspector_control_box(target,"zone_delete",(tx,y,right,y+31),danger=True)
        target.create_text((tx+right)/2,y+15.5,text="Supprimer",anchor="center",fill="#F0D6D6",font=(theme.FONT_UI,7,"bold"))

    def _render_gabarit_multi_inspector(self, target, width: float, height: float, infos: list[dict]) -> None:
        target.create_line(0, 8, 0, max(8, height-8), fill="#465A63", width=1)
        tx=16.0; right=width-12.0; inner=max(100.0,right-tx); y=18.0
        count=len(infos)
        target.create_text(tx,y,text=f"{count} ZONES SÉLECTIONNÉES",anchor="w",fill="#78B8B1",font=(theme.FONT_UI,8,"bold")); y+=24
        target.create_text(tx,y,text="Maj + clic pour ajouter ou retirer",anchor="w",fill="#9FB0B3",font=(theme.FONT_UI,7)); y+=18
        target.create_line(tx,y,right,y,fill="#40525B",width=1); y+=12
        y = self._render_gabarit_create_row(target, tx, right, y)
        target.create_line(tx,y,right,y,fill="#40525B",width=1); y+=12
        ...
        alignment_exception = self._gabarit_alignment_is_exception(self._gabarit_active_item())
        ref_label = "PAGE · EXCEPTION" if alignment_exception else "MARGES · LIVRE"
        target.create_text(tx,y,text="ALIGNER",anchor="w",fill="#A8BCC0",font=(theme.FONT_UI,8,"bold"))
        target.create_text(right,y,text=ref_label,anchor="e",fill="#78B8B1" if alignment_exception else "#9FB0B3",font=(theme.FONT_UI,7,"bold" if alignment_exception else "normal")); y+=17
        actions=(("←","align_left"),("↔","align_center"),("→","align_right"),("↑","align_top"),("↕","align_middle"),("↓","align_bottom"))
        gap=4.0; bw=(inner-gap*2)/3.0
        for i,(glyph,key) in enumerate(actions):
        ...
        target.create_text(tx,y,text="RÉPARTIR LES ESPACES",anchor="w",fill="#A8BCC0",font=(theme.FONT_UI,8,"bold")); y+=17
        bw=(inner-gap)/2.0
        self._gabarit_inspector_control_box(target,"multi_distribute_h",(tx,y,tx+bw,y+31),disabled=count<3)
        target.create_text(tx+bw/2,y+15.5,text="Horizontal",anchor="center",fill="#E7ECEA" if count>=3 else "#65737A",font=(theme.FONT_UI,7,"bold"))
        self._gabarit_inspector_control_box(target,"multi_distribute_v",(tx+bw+gap,y,right,y+31),disabled=count<3)
        target.create_text(tx+bw+gap+bw/2,y+15.5,text="Vertical",anchor="center",fill="#E7ECEA" if count>=3 else "#65737A",font=(theme.FONT_UI,7,"bold")); y+=45

        target.create_text(tx,y,text="TAILLE",anchor="w",fill="#A8BCC0",font=(theme.FONT_UI,8,"bold")); y+=17
        self._gabarit_inspector_control_box(target,"multi_same_w",(tx,y,tx+bw,y+31))
        target.create_text(tx+bw/2,y+15.5,text="Même largeur",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold"))
        self._gabarit_inspector_control_box(target,"multi_same_h",(tx+bw+gap,y,right,y+31))
        target.create_text(tx+bw+gap+bw/2,y+15.5,text="Même hauteur",anchor="center",fill="#E7ECEA",font=(theme.FONT_UI,7,"bold")); y+=48

        self._gabarit_inspector_control_box(target,"multi_delete",(tx,y,right,y+32),danger=True)
        target.create_text((tx+right)/2,y+16,text="Supprimer la sélection",anchor="center",fill="#F0D6D6",font=(theme.FONT_UI,7,"bold"))
    ...
    def _render_gabarit_inspector_overlay(self, *, force: bool = False):
        """Dessine l'inspecteur sur sa propre couche, jamais sur le Canvas page."""
        if str(getattr(self, "_work_mode", "")) != "gabarits":
            return
        target = getattr(self, "gabarit_inspector_canvas", None)
        if target is None or not self.items:
            return
        # Pendant la saisie directe d'un compteur, le Canvas inspecteur ne doit
        # pas effacer le widget Entry sous l'effet d'un simple survol.
        if self._gabarit_inline_geometry_is_active():
            return
        try:
        ...
        # Même géométrie que l'ancien inspecteur Canvas, translatée à x=0.
        target.create_line(0, 8.0, 0, max(8.0, height - 8.0), fill="#465A63", width=1)
        tx = 16.0
        inner_right = width - 12.0
        inner_w = max(112.0, inner_right - tx)
        room = max(0.0, min(1.0, (height - 370.0) / 150.0))
        ...
        def section_label(y: float, label: str) -> float:
            target.create_text(tx, y, text=label, anchor="w", fill="#A8BCC0", font=(theme.FONT_UI, 9, "bold"))
            return y + section_step

        def separator(y: float) -> float:
            target.create_line(tx, y, inner_right, y, fill="#40525B", width=1)
            return y + separator_step

        def control_box(key: str, y: float, h: float, *, selected=False, pending=False, x1=None, x2=None):
            bx1 = tx if x1 is None else float(x1)
            bx2 = inner_right if x2 is None else float(x2)
            hovered = str(getattr(self, "_gabarit_inspector_hover", "")) == key
            pressed = str(getattr(self, "_gabarit_inspector_pressed", "")) == key
            if pending:
                fill = "#55442F" if not hovered and not pressed else "#655239"; outline = "#D0A458"
            elif hovered or pressed:
                fill = "#314752"; outline = theme.ACCENT_BRIGHT
            elif selected:
                fill = "#2B4249"; outline = theme.ACCENT_BRIGHT
            else:
                fill = "#202D36"; outline = "#465963"
            target.create_rectangle(bx1, y, bx2, y+h, fill=fill, outline=outline, width=2 if hovered or selected or pending else 1)
            self._gabarit_inspector_hitboxes[key] = (bx1, y, bx2, y+h)
            return bx1, y, bx2, y+h

        y = top_pad
        page_active_header_y = y
        y = section_label(y, "PAGE ACTIVE")
        if self._gabarit_is_local_exception(active_item, int(self._selected_index)):
            badge_right = inner_right
            badge_icon_x = badge_right - 65.0
            r = 6.0
            box = (badge_icon_x-r, page_active_header_y-r, badge_icon_x+r, page_active_header_y+r)
            target.create_arc(*box, start=118, extent=124, style="arc", outline="#D7DEDC", width=2)
            target.create_arc(*box, start=298, extent=124, style="arc", outline="#D7DEDC", width=2)
            target.create_text(badge_right, page_active_header_y, text="EXCEPTION", anchor="e", fill="#D7DEDC", font=(theme.FONT_UI, 7, "bold"))
        title_id = target.create_text(tx, y, text=f"{number} — {active_type}", anchor="nw", fill="#F0F2F0", font=(theme.FONT_TITLE, 10, "bold"), width=inner_w)
        bbox = target.bbox(title_id)
        y = float((bbox[3] if bbox else y + 20.0) + mix(4.0, 7.0))
        target.create_text(tx, y, text=part_text, anchor="nw", fill="#BFC9C7", font=(theme.FONT_UI, 8), width=inner_w)
        y += page_part_step
        y = separator(y)

        y = self._render_gabarit_create_row(target, tx, inner_right, y)
        y = separator(y)

        y = section_label(y, "GABARIT")
        target.create_text(tx, y, text="Portée", anchor="w", fill="#C3CDCB", font=(theme.FONT_UI, 9, "bold"))
        y += mix(16.0, 18.0)
        last_scope = self._gabarit_item_last_scope(active_item)
        scope_active = self._gabarit_scope_is_active(active_item)
        pending_scope = str(getattr(self, "_gabarit_scope_pending", ""))
        page_scope = scope_active and last_scope == "page"
        type_scope = scope_active and last_scope == "type"
        page_pending = pending_scope == "page"
        type_pending = pending_scope == "type"
        scope_gap = 4.0
        scope_mid = tx + (inner_w - scope_gap) / 2.0
        control_box("scope_page", y, scope_h, selected=page_scope, pending=page_pending, x1=tx, x2=scope_mid)
        target.create_text((tx+scope_mid)/2.0, y+scope_h/2.0, text="Confirmer" if page_pending else "Cette page", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if page_scope or page_pending else "normal"))
        type_x1 = scope_mid + scope_gap
        control_box("scope_type", y, scope_h, selected=type_scope, pending=type_pending, x1=type_x1, x2=inner_right)
        target.create_text((type_x1+inner_right)/2.0, y+scope_h/2.0, text="Confirmer" if type_pending else "Toutes du type", anchor="center", fill="#F1F3F2", font=(theme.FONT_UI, 8, "bold" if type_scope or type_pending else "normal"))
        y += scope_h + between_controls + 6.0

        target.create_text(tx, y, text="Statut", anchor="w", fill="#C3CDCB", font=(theme.FONT_UI, 9, "bold"))
        y += mix(16.0, 18.0)
        ...
        target.create_rectangle(tx, y, inner_right, y+status_h, fill="#202D36", outline="#465963", width=1)
        dot_cy = y + status_h/2.0
        target.create_oval(tx+10, dot_cy-6, tx+22, dot_cy+6, fill=status_color, outline="")
        target.create_text(tx+30, dot_cy, text=status_text, anchor="w", fill=status_color, font=(theme.FONT_UI, 9, "bold"))
        ...
        def compact_uniform(values, keys):
            vals = [float(values.get(k, 0.0)) for k in keys]
            if vals and max(vals) - min(vals) < 0.001:
                return f"{vals[0]:g} mm"
            return "variables"

        # Un seul accès pour les trois réglages globaux. Ils sont rarement modifiés
        # pendant le maquettage et ne doivent plus encombrer l'inspecteur.
        book_settings_h = mix(46.0, 56.0)
        control_box("edit_book_settings", y, book_settings_h)
        target.create_text(
            tx+10, y+mix(11.0,13.0), text="Réglages du livre", anchor="w",
            fill="#F0F2F0", font=(theme.FONT_UI, 9, "bold"),
        )
        summary = (
            f"{fmt['label']} · {fmt['width_mm']:g} × {fmt['height_mm']:g} mm"
            f"  ·  Marges {compact_uniform(margins, ('top','bottom','inside','outside'))}"
            f"  ·  Fond perdu {compact_uniform(bleed, ('top','bottom','left','right'))}"
        )
        target.create_text(
            tx+10, y+book_settings_h-mix(10.0,13.0), text=summary, anchor="w",
            fill="#B8C6C3", font=(theme.FONT_UI, 7), width=max(80.0, inner_w-28.0),
        )
        target.create_text(
            inner_right-9, y+book_settings_h/2.0, text="›", anchor="e",
            fill="#A6B7BA", font=(theme.FONT_UI, 11, "bold"),
        )
        y += book_settings_h + mix(7.0, 10.0)
        gap = 5.0
        bw = (inner_w-gap)/2.0
        control_box("copy_template", y, mix(28.0,32.0), x1=tx, x2=tx+bw)
        target.create_text(tx+bw/2.0, y+mix(14.0,16.0), text="Copier", anchor="center", fill="#E7ECEA", font=(theme.FONT_UI,7,"bold"))
        has_copy = self.gabarit_has_copied_template()
        if has_copy:
            control_box("apply_template", y, mix(28.0,32.0), x1=tx+bw+gap, x2=inner_right)
        else:
            target.create_rectangle(tx+bw+gap, y, inner_right, y+mix(28.0,32.0), fill="#1B252C", outline="#34434B", width=1)
        target.create_text(tx+bw+gap+bw/2.0, y+mix(14.0,16.0), text="Appliquer", anchor="center", fill="#E7ECEA" if has_copy else "#65737A", font=(theme.FONT_UI,7,"bold"))

    def _gabarit_inspector_key_at(self, x: float, y: float) -> str:
        for key, box in getattr(self, "_gabarit_inspector_hitboxes", {}).items():
    ...
    def _gabarit_inspector_leave(self, _event=None):
        if self._gabarit_inline_geometry_is_active():
            return "break"
        if getattr(self, "_gabarit_inspector_hover", "") or getattr(self, "_gabarit_inspector_pressed", ""):
    ...
    def _gabarit_inspector_press(self, event):
        ...
        if key == "create_text":
            ...
            self.gabarit_toggle_selected_zone_lock()
        elif key == "zone_duplicate":
            self.gabarit_duplicate_selected_zone()
        elif key == "layer_back":
            self.gabarit_move_selected_zone_layer("back")
        elif key == "layer_backward":
            self.gabarit_move_selected_zone_layer("backward")
        elif key == "layer_forward":
            self.gabarit_move_selected_zone_layer("forward")
        elif key == "layer_front":
            self.gabarit_move_selected_zone_layer("frontmost")
    ...
    def set_work_mode(self, mode: str):
        ...
        if mode == "structure":
            if bottom is not None: bottom.grid()
            if bar is not None: bar.grid()
        ...
        if mode != "structure":
            ...
            self.structure_cancel_tool()
            self._structure_reset_action()
            try: self._cancel_drag_autoscroll()
            except Exception: pass
        ...
        if mode == "gabarits":
            ...
            self._gabarit_scope = self._gabarit_item_last_scope(self._gabarit_active_item()) or "page"

            # Prépare Gabarits derrière Structure, puis échange les surfaces.
            target = getattr(self, "gabarit_canvas", None)
            if target is not None:
                self.canvas = target
                self._workspace_switch_rendering = True
                try:
                    self.render()
                finally:
                    self._workspace_switch_rendering = False
                tk.Misc.tkraise(target)
            self._hide_gabarit_tools_host()
            self._show_gabarit_inspector_host()
            if zoom_controls is not None:
                try:
                    zoom_controls.place(x=8.0, rely=0.54, anchor="w")
                    zoom_controls.lift()
                except Exception:
                    pass
        ...
        # Structure est elle aussi pré-rendue derrière Gabarits avant l'échange.
        self._hide_gabarit_tools_host()
        self._hide_gabarit_inspector_host()
        target = getattr(self, "structure_canvas", None)
        if target is not None:
            self.canvas = target
            self.render()
            tk.Misc.tkraise(target)
    ...
    # ------------------------------------------------------------------
    # Référentiel d'alignement du LIVRE
    # ------------------------------------------------------------------
    GABARIT_ALIGNMENT_TYPE_RULES_KEY = "gabarit_alignment_type_rules"

    @staticmethod
    def _gabarit_book_alignment_reference() -> str:
        # Règle fondamentale TomeLinea : le livre travaille dans ses marges.
        return "margins"
    ...
    def _gabarit_alignment_inherited_reference(self, item: dict | None) -> str:
        ...
        return "page" if str(rule or "").strip().lower() == "page" else default
    ...
    def _gabarit_alignment_reference(self, item: dict | None = None) -> str:
        ...
        if bool(item.get("gabarit_alignment_local_override")):
            local = str(item.get("gabarit_alignment_reference") or "").strip().lower()
            if local in {"margins", "page"}:
                return local
    ...
    def gabarit_prepare_alignment_reference(self, reference: str) -> bool:
        reference = "page" if str(reference or "").strip().lower() == "page" else "margins"
        self._gabarit_alignment_pending_reference = reference
        self._gabarit_inspector_signature = None
        self._render_gabarit_inspector_overlay(force=True)
        self.status_var.set(f"Référence {('Page' if reference == 'page' else 'Marges')} : choisissez la portée.")
        return True
    ...
    def gabarit_apply_alignment_reference(self, scope: str) -> bool:
        ...
        if not isinstance(active, dict):
            return False
        desired = str(getattr(self, "_gabarit_alignment_pending_reference", "") or "").strip().lower()
        if desired not in {"margins", "page"}:
            return False
        ...
        if scope == "type" and type_key:
            ...
            # « Toutes du type » devient la règle du type : les anciennes
            # exceptions locales de référence sont levées pour que toutes les
            # occurrences suivent réellement la même règle, y compris les futures.
        ...
        self._emit_gabarit_page_changed()
        self._emit_gabarit_selection_changed()
        ref_label = "la page" if desired == "page" else "les marges"
        self.status_var.set(f"Alignement sur {ref_label} · {label}")
        return True
    ...
    def _gabarit_tools_reserved_width(self, viewport_w: float | None = None) -> float:
        """Plus de rail gauche : seule une petite respiration avant le zoom."""
        return 0.0

    def _gabarit_left_separator_x(self, viewport_w: float | None = None) -> float:
        zoom_w = float(getattr(self, "_gabarit_zoom_controls_width", 68.0))
        return 8.0 + zoom_w + 10.0
    ...
    def _gabarit_work_rect_for_size(self, width: float, height: float) -> tuple[float, float, float, float]:
        ...
        info_w = float(self._gabarit_inspector_width(width))
        left_separator = self._gabarit_left_separator_x(width)
        right_separator = width - info_w - 8.0
        work_left = left_separator + 12.0
        work_right = max(work_left + 1.0, right_separator - 12.0)
        work_top = 82.0
        ...
        *, work_left: float = 12.0, work_right: float | None = None, target=None,
        ...
        for slot, x in enumerate(positions):
            ...
            if automatic:
                ...
                if self._gabarit_is_local_exception(item, idx):
                    # L'anneau d'exception ne remplace jamais la couleur de statut.
                    # Il est volontairement plus grand et très interrompu pour être
                    # lisible même sur une station inactive.
                    self._gabarit_draw_exception_ring(
                        x, 50, 11.5 if active else 10.5,
                        width=2 if active else 2, color="#E0E5E3" if active else "#BEC8C5", target=target,
                    )
                if self._gabarit_alignment_is_exception(item):
                    # Exception d'alignement : petit soulignement indépendant des
                    # parenthèses du gabarit. Les deux signes peuvent coexister.
    ...
    def gabarit_apply_book_settings(
        ...
        """Applique atomiquement Format + Marges + Fond perdu au niveau du livre."""
        try:
    ...
    def gabarit_align_selected_zones(self, mode: str) -> bool:
        ...
        if changed:
            ...
            self.status_var.set(f"{len(zones)} zones alignées sur la {label}")
        return changed

    def gabarit_distribute_selected_zones(self, axis: str) -> bool:
        zones=self._gabarit_selected_zone_dicts()
        if len(zones) < 3:
            self.status_var.set("Sélectionnez au moins trois zones à distribuer.")
            return False
        if not self._gabarit_multi_same_surface(zones):
            self.status_var.set("Distribuez ensemble des zones situées sur la même surface.")
            return False
        if any(bool(z.get("locked", False)) for z in zones): return False
        axis=str(axis or "horizontal").lower()
        if axis.startswith("h"):
            ordered=sorted(zones,key=lambda z:float(z.get("x",0)))
            first=float(ordered[0].get("x",0)); last_right=float(ordered[-1].get("x",0))+float(ordered[-1].get("w",0))
            total=sum(float(z.get("w",0)) for z in ordered); gap=(last_right-first-total)/(len(ordered)-1)
            cursor=first
            for z in ordered: z["x"]=cursor; cursor += float(z.get("w",0))+gap
        else:
            ordered=sorted(zones,key=lambda z:float(z.get("y",0)))
            first=float(ordered[0].get("y",0)); last_bottom=float(ordered[-1].get("y",0))+float(ordered[-1].get("h",0))
            total=sum(float(z.get("h",0)) for z in ordered); gap=(last_bottom-first-total)/(len(ordered)-1)
            cursor=first
            for z in ordered: z["y"]=cursor; cursor += float(z.get("h",0))+gap
        self._gabarit_mark_edited(self._gabarit_active_item())
        self._save_order(); self.render(); self._emit_gabarit_selection_changed()
        self.status_var.set(f"{len(zones)} zones distribuées")
        return True

    def gabarit_equalize_selected_zones(self, dimension: str) -> bool:
        zones=self._gabarit_selected_zone_dicts()
        if len(zones) < 2: return False
        primary=self._gabarit_find_zone(self._gabarit_active_item(), zone_id=self._gabarit_selected_zone_id)
        if not isinstance(primary,dict): primary=zones[0]
        key="h" if str(dimension).lower().startswith("h") else "w"
        value=float(primary.get(key,.2))
        for z in zones:
            if not bool(z.get("locked",False)): z[key]=value
        self._gabarit_mark_edited(self._gabarit_active_item())
        self._save_order(); self.render(); self._emit_gabarit_selection_changed()
        self.status_var.set("Même hauteur" if key=="h" else "Même largeur")
        return True
    ...
    def gabarit_align_selected_zone(self, mode: str, reference: str | None = None) -> bool:
        ...
        if rect is None:
            return False
        label = "marges" if reference == "margins" else "page"
        return self._gabarit_commit_zone_rect(zone, rect, f"Zone alignée sur la {label}")
    ...
    def _gabarit_draw_zones(self, x1: float, y1: float, total_w: float, page_h: float, *, target=None) -> None:
        ...
        for zone in zones:
            ...
            color = colors.get(kind, "#C39A4A")
            rect_id = target.create_rectangle(
                zx1, zy1, zx2, zy2, outline=color, width=3 if selected else 1,
            )
            text_id = target.create_text(
                (zx1+zx2)/2, (zy1+zy2)/2,
                text=labels.get(kind, kind.upper()), fill=color,
                font=(theme.FONT_UI, 8, "bold"), anchor="center",
                state="normal" if (zx2 - zx1) > 74 and (zy2 - zy1) > 28 else "hidden",
            )
            ...
            if selected:
                if bool(zone.get("locked", False)):
                    lock_id = target.create_text(
                        zx1 + 7, zy1 + 7, text="VERROUILLÉ", anchor="nw",
                        fill=color, font=(theme.FONT_UI, 6, "bold"),
                    )
                elif len(selected_ids) == 1 and zid == primary_id:
                    hs = 7.0
                    handle_id = target.create_rectangle(
                        zx2-hs, zy2-hs, zx2+hs, zy2+hs,
                        fill=color, outline="#F5F6F4", width=1,
                    )
                    self._gabarit_zone_handle_hitboxes[zid] = (zx2-hs-3, zy2-hs-3, zx2+hs+3, zy2+hs+3)
            self._gabarit_zone_canvas_items[zid] = {
                "rect": rect_id, "text": text_id, "handle": handle_id, "lock": lock_id,
            }

    def _gabarit_update_zone_canvas(self, zid: str, zone: dict) -> None:
        """Met à jour uniquement la zone déplacée/redimensionnée, sans render()."""
        box = self._gabarit_page_box
        items = dict(getattr(self, "_gabarit_zone_canvas_items", {}).get(str(zid), {}) or {})
        if not box or not items:
            return
        x1, y1, x2, y2 = box
        total_w = max(1.0, float(x2) - float(x1))
        page_h = max(1.0, float(y2) - float(y1))
        sx, sy, sw, sh = self._gabarit_zone_surface_box(zone, x1, y1, total_w, page_h)
        zx1 = sx + max(0.0, min(1.0, float(zone.get("x", 0.1)))) * sw
        zy1 = sy + max(0.0, min(1.0, float(zone.get("y", 0.1)))) * sh
        zw = max(0.04, min(1.0, float(zone.get("w", 0.3)))) * sw
        ...
        rect_id = items.get("rect")
        text_id = items.get("text")
        handle_id = items.get("handle")
        lock_id = items.get("lock")
        try:
            if rect_id:
                self.canvas.coords(rect_id, zx1, zy1, zx2, zy2)
            if text_id:
                self.canvas.coords(text_id, (zx1 + zx2) / 2.0, (zy1 + zy2) / 2.0)
                self.canvas.itemconfigure(
                    text_id,
                    state="normal" if (zx2 - zx1) > 74 and (zy2 - zy1) > 28 else "hidden",
                )
            if lock_id:
                self.canvas.coords(lock_id, zx1 + 7, zy1 + 7)
            if handle_id:
                hs = 7.0
                self.canvas.coords(handle_id, zx2-hs, zy2-hs, zx2+hs, zy2+hs)
                self._gabarit_zone_handle_hitboxes[str(zid)] = (
                    zx2-hs-3, zy2-hs-3, zx2+hs+3, zy2+hs+3,
                )
        except tk.TclError:
            return
        self._gabarit_zone_hitboxes[str(zid)] = (zx1, zy1, zx2, zy2)
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float, *, update_inspector: bool = True) -> None:
        ...
        right_ok = vertical_ok and right_cx - arrow_half >= work_left and right_cx + arrow_half <= info_x1 - 8.0
        ...
        # Séparateur gauche et navigation sont de l'interface : toujours au-dessus
        # de la page, comme dans le prototype v5 validé.
        target.create_line(left_separator_x, 8.0, left_separator_x, max(8.0, viewport_h - 8.0), fill="#465A63", width=1)
        ...
        zoom_controls = getattr(self, "gabarit_zoom_controls", None)
        if zoom_controls is not None and not bool(getattr(self, "_workspace_switch_rendering", False)):
            try:
                zoom_controls.place(x=8.0, rely=0.54, anchor="w")
                zoom_controls.lift()
            except Exception:
    ...
    def _gabarit_press(self, event):
        ...
        for zid, box in reversed(list(self._gabarit_zone_hitboxes.items())):
            if self._gabarit_point_in(box, x, y):
                ...
                if shift:
                    ids = self._gabarit_selected_ids()
                    if zid in ids:
                        ids.remove(zid)
                    else:
                        ids.append(zid)
                    self._gabarit_set_selected_ids(ids, primary=zid if zid in ids else (ids[-1] if ids else ""))
                    self._gabarit_zone_drag = None
                else:
                    self._gabarit_set_selected_ids([zid], primary=zid)
    ...
    def _gabarit_drag(self, event):
        if self._gabarit_zone_drag is not None and self._gabarit_page_box:
            ...
            if preset == "page":
                ...
                for other in zones if isinstance(zones, list) else []:
                    if not isinstance(other, dict) or str(other.get("id") or "") == current_id:
                        continue
                    if self._gabarit_active_is_spread() and self._gabarit_zone_spread_position(other) != current_position:
                        continue
                    tx = float(other.get("x", 0.0)); ty = float(other.get("y", 0.0))
                    tw = float(other.get("w", 0.0)); th = float(other.get("h", 0.0))
            ...
            def snap_origin(value, size, refs, threshold, release, lock_key):
                ...
                for ref in refs:
                    if center_only:
                        desired.append(ref - size / 2.0)
                    elif preset == "zones":
                        desired.extend((ref, ref - size / 2.0, ref - size))
                    else:
                        desired.extend((ref, ref - size))
                locked = state.get(lock_key)
                if locked is not None:
                    locked = float(locked)
                    if abs(value - locked) <= release:
                        return locked
                    state.pop(lock_key, None)
                best = min(desired, key=lambda candidate: abs(value - candidate))
                if abs(value - best) <= threshold:
                    state[lock_key] = float(best)
                    return best
                return value
            ...
            if state["mode"] == "resize":
                ...
                right = snap_edge(ox + nw, x_refs, threshold_x, release_x, "snap_edge_x")
                bottom = snap_edge(oy + nh, y_refs, threshold_y, release_y, "snap_edge_y")
                nw = max(0.05, min(1.0 - ox, right - ox))
                nh = max(0.05, min(1.0 - oy, bottom - oy))
                zone.update({"x": ox, "y": oy, "w": nw, "h": nh})
            else:
                nx = max(0.0, min(1.0 - ow, ox + dx))
                ny = max(0.0, min(1.0 - oh, oy + dy))
                nx = snap_origin(nx, ow, x_refs, threshold_x, release_x, "snap_origin_x")
                ny = snap_origin(ny, oh, y_refs, threshold_y, release_y, "snap_origin_y")
                nx = max(0.0, min(1.0 - ow, nx))
                ny = max(0.0, min(1.0 - oh, ny))
