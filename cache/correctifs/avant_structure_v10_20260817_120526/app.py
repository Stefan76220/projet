...
class TomeLineaV3(tk.Tk):
    ...
    def _build_workspace(self):
        ...
        def layout(event=None):
            ...
            gap = 7
            # A reste pleinement utilisable, mais C récupère la hauteur perdue dans
            # les marges et les espacements. B garde presque la même surface utile.
    ...
    def _build_structure_tab(self, parent):
        """Structure V9 : briques éditoriales compactes + règles réunies."""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=0, minsize=78)
        parent.grid_columnconfigure(1, weight=1)

        rail = tk.Frame(parent, bg=theme.PANEL_ALT, width=78, padx=4, pady=4)
        ...
        palette = tk.Frame(parent, bg=theme.PANEL_ALT, padx=8, pady=4)
        ...
        # --------------------------------------------------------------
        # Fenêtres TomeLinea intégrées : réservées aux vraies pages de travail
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # Commandes contextuelles, sans changement d'écran
        # --------------------------------------------------------------
        action_bar = tk.Frame(palette, bg=theme.PANEL_ALT)
        action_bar.grid(row=0, column=0, sticky="ew", pady=(0, 3))
        tk.Label(
            action_bar, text="Sélection B", bg=theme.PANEL_ALT, fg=theme.MUTED,
            font=(theme.FONT_UI, 7, "bold"),
        ).pack(side="left", padx=(0, 6))

        delete_state = {"armed": False, "job": None, "button": None}
        def reset_delete_state():
            delete_state["armed"] = False
            job = delete_state.get("job")
            if job is not None:
                try: self.after_cancel(job)
                except Exception: pass
            delete_state["job"] = None
            btn = delete_state.get("button")
            if btn is not None and btn.winfo_exists():
                btn.configure(text="Supprimer", bg=theme.PANEL_SOFT, fg="#E4876E")

        def command_button(parent_, label, command, *, danger=False):
            return tk.Button(
                parent_, text=label, command=command,
                bg=theme.PANEL_SOFT, fg="#E4876E" if danger else theme.INK,
                activebackground="#6A3029" if danger else theme.ACCENT_SOFT,
        ...
        def duplicate_selected():
            ...
            if canvas is not None and hasattr(canvas, "structure_duplicate_selected"):
                ...
                self.after_idle(refresh_selection_context)
        ...
        def delete_selected():
            ...
            if not delete_state["armed"]:
                ...
                btn = delete_state.get("button")
                if btn is not None:
                    btn.configure(text="Confirmer", bg="#6A3029", fg=theme.WHITE)
                delete_state["job"] = self.after(4000, reset_delete_state)
                return
        ...
        command_button(action_bar, "Nommer", rename_selected).pack(side="left", padx=(0, 4))
        command_button(action_bar, "Dupliquer", duplicate_selected).pack(side="left", padx=(0, 4))
        delete_btn = command_button(action_bar, "Supprimer", delete_selected, danger=True)
        delete_btn.pack(side="left", padx=(0, 4))
        delete_state["button"] = delete_btn
        command_button(action_bar, "Annuler outil", cancel_tool).pack(side="left", padx=(0, 4))

        # --------------------------------------------------------------
        # Règles : le niveau local est maintenant réuni dans l'onglet Règles.
        # La sélection de B ne consomme donc plus une ligne permanente dans C.
        # --------------------------------------------------------------
        current_family = {"name": "Pages"}
        rules_refresh_callback = {"fn": None}

        def refresh_selection_context():
            if current_family.get("name") != "Règles":
                return
            callback = rules_refresh_callback.get("fn")
        ...
        palette_head.grid(row=1, column=0, sticky="ew", pady=(0, 3))
        palette_head.grid_columnconfigure(2, weight=1)
        palette_title_var = tk.StringVar(value="Pages")
        palette_hint_var = tk.StringVar(value="Clic sur une brique, puis clic dans B pour la poser.")
        tk.Label(
        ...
        head_action_slot.grid(row=0, column=1, sticky="w", padx=(8, 0))
        tk.Label(
        ...
        family_buttons = {}
        active_brick = {"widget": None, "normalizer": None}
        ...
        def external_drop(kind, payload, event):
            canvas = book()
            if canvas is None or not hasattr(canvas, "structure_drop_from_root"):
                return False
            success = bool(canvas.structure_drop_from_root(kind, payload, event.x_root, event.y_root))
            if success:
                reset_active_brick()
            return success

        def make_brick(parent_, label, kind, payload, *, locked=False):
            """Brique TomeLinea V9 : petite pièce éditoriale, pas un bouton."""
            width, height = 94, 30
            base = "#262B2F"
            hover = "#30383B"
            pressed = "#1F2427"
            ...
            card = tk.Canvas(
                parent_, width=width, height=height,
            ...
            def draw(fill, outline, *, active=False):
                card.delete("brick")
                # Coins coupés : langage TomeLinea, distinct des boutons rectangulaires.
                pts = (1, 1, width-7, 1, width-1, 7, width-1, height-1,
                       7, height-1, 1, height-7)
                card.create_polygon(
                    *pts, fill=fill, outline=outline, width=2 if active else 1,
                    tags="brick",
                )
                accent = muted if locked else gold
                # Petit pictogramme de feuille, discret et identique pour toutes les briques.
                card.create_rectangle(8, 8, 16, 21, outline=accent, width=1, tags="brick")
                card.create_line(12, 8, 16, 12, fill=accent, width=1, tags="brick")
                card.create_text(
                    21, height/2, text=display, anchor="w",
                    fill=theme.MUTED if locked else text_color,
                    font=(theme.FONT_UI, 7, "bold"), tags="brick",
                )

            def normal():
                draw("#23272A" if locked else base, "#454B4E" if locked else border)
        ...
        def all_type_entries(*, include_locked=True):
            result = []
            seen = set()
            if include_locked:
                for key, label in (
                    ("couverture", "Couverture"),
                    ("deuxieme_couverture", "2e couverture"),
                    ("troisieme_couverture", "3e couverture"),
                    ("quatrieme_couverture", "4e couverture"),
                ):
                    result.append((key, label, True)); seen.add(key)
            result.append(("page_blanche", "Page blanche", False)); seen.add("page_blanche")
            for key, label, _custom in page_types():
                if key not in seen:
                    result.append((key, label, False)); seen.add(key)
            return result

        def install_scroll_bindings(root_widget, cv):
            """Molette active partout dans la palette, y compris sur une brique."""
            def wheel(event):
                box = cv.bbox("all")
                if not box or box[3] <= max(1, cv.winfo_height()):
                    return None
                delta = getattr(event, "delta", 0)
                if delta:
                    steps = -2 if delta > 0 else 2
                else:
                    steps = -2 if getattr(event, "num", 0) == 4 else 2
        ...
        def make_scroll_flow(parent_, entries, *, columns=9):
            holder = tk.Frame(parent_, bg=theme.PANEL_ALT)
            holder.pack(fill="both", expand=True)
            cv = tk.Canvas(holder, bg=theme.PANEL_ALT, bd=0, highlightthickness=0)
            sb = tk.Scrollbar(holder, orient="vertical", command=cv.yview, width=9)
            inner = tk.Frame(cv, bg=theme.PANEL_ALT)
            ...
            for factory in entries:
                widget = factory(inner)
                widget.grid(row=r, column=c, padx=3, pady=3, sticky="nw")
                c += 1
                if c >= columns:
                    c = 0
                    r += 1

            def configure_inner(_e=None):
                box = cv.bbox("all")
                if box:
            ...
            install_scroll_bindings(holder, cv)
            return holder

        # --------------------------------------------------------------
        # Création d'un nouveau type : aucune propriété "automatique"
        # --------------------------------------------------------------
        def create_custom_type():
            ...
            def library_candidates():
                ...
                labels = {
                    "type_page_titre.png": "Titre", "type_page_sommaire.png": "Sommaire",
                    "type_page_chapitre.png": "Chapitre", "type_page_texte.png": "Texte",
                    "type_page_illustration.png": "Illustration", "type_page_fiche.png": "Fiche",
                    "type_page_transition.png": "Transition", "type_page_blanche.png": "Page blanche",
                    "type_page_conclusion.png": "Conclusion", "type_page_avant_propos.png": "Avant-propos",
                }
                return [(label, folder/filename) for filename, label in labels.items() if (folder/filename).is_file()]

            def make_photo(path, size=(180, 250)):
                try:
                    with Image.open(path) as source:
                        image = source.convert("RGBA"); image.thumbnail(size, Image.Resampling.LANCZOS)
                        photo = ImageTk.PhotoImage(image)
                    image_refs.append(photo); return photo
                except Exception:
                    return None
            ...
            def build_body(body):
                ...
                def field(row, label, key):
                    ...
                    entry.grid(row=row, column=1, sticky="ew", pady=6, ipady=6); entries[key] = entry
                ...
                preview_slot.grid(row=0, column=1, sticky="nsew"); preview_slot.grid_propagate(False)
                ...
                controls = tk.Frame(image_area, bg=theme.PANEL_ALT); controls.pack(anchor="w")
                library_holder = tk.Frame(image_area, bg=theme.WINDOW_DEEP, highlightthickness=1, highlightbackground=theme.BORDER)
                def close_library(): library_holder.pack_forget()
                def choose_none():
                    state["preview_image"] = None; selected_image_var.set("Aucune image sélectionnée"); close_library(); refresh_preview()
                def choose_external():
                    filename = filedialog.askopenfilename(title="Choisir une image de base", filetypes=[("Images", "*.png *.jpg *.jpeg *.webp *.bmp")])
                    if filename:
                        state["preview_image"] = Path(filename); selected_image_var.set(Path(filename).name); close_library(); refresh_preview()
                def choose_library():
                ...
                for label, command in (("Bibliothèque", choose_library), ("Image extérieure", choose_external), ("Aucune image", choose_none)):
                    command_button(controls, label, command).pack(side="left", padx=(0, 5))
                ...
                lib_grid = tk.Frame(library_holder, bg=theme.WINDOW_DEEP); lib_grid.pack(fill="x", padx=8, pady=(0,7))
                row_i=col_i=0
                for lib_label, path in library_candidates():
                    ...
                    for target in (tile,*tile.winfo_children()): target.bind("<Button-1>",select_lib); target.configure(cursor="hand2")
                    col_i+=1
                    if col_i>=3: col_i=0; row_i+=1
                ...
                tk.Label(form, text="Les pages automatiques se règlent ensuite dans Structure : localement sur une page ou globalement dans Règles.", bg=theme.PANEL_ALT, fg=theme.MUTED_DARK, font=(theme.FONT_UI,7), anchor="w", wraplength=520, justify="left").grid(row=4,column=1,sticky="w",pady=(4,0))
            ...
            def confirm_new_type():
                ...
                if not name:
                    entries["name"].configure(highlightthickness=1, highlightbackground="#A94F43"); entries["name"].focus_set(); return True
                created=canvas.structure_create_custom_type({"name":name,"short_label":short or name,"visual":"custom","duplicable":bool(state["duplicable"].get()),"preview_image":str(state["preview_image"]) if state.get("preview_image") else ""})
                if not created: return True
                self.after_idle(lambda: select_family("Types")); return False

            show_structure_modal("Créer un nouveau type de page", confirm_label="Créer le type", on_confirm=confirm_new_type, body_builder=build_body)

        # --------------------------------------------------------------
        # Règles générales : page source -> page automatique associée
        # --------------------------------------------------------------
        def build_rules():
            """Règles V9 : niveau local + niveau général, réunis dans un seul outil."""
            canvas = book()
            section = tk.Frame(palette_body, bg=theme.PANEL_ALT)
            section.pack(fill="both", expand=True)
            section.grid_columnconfigure(0, weight=1)
            section.grid_columnconfigure(1, weight=1)
            section.grid_rowconfigure(0, weight=1)

            options = canvas.structure_auto_target_options() if canvas is not None and hasattr(canvas, "structure_auto_target_options") else []
            labels = {key: label for key, label in options}
            if not options:
                tk.Label(section, text="Aucun type disponible.", bg=theme.PANEL_ALT, fg=theme.MUTED).grid(row=0, column=0, sticky="nw")
                return

            # ---- Niveau local : la page réellement sélectionnée dans B ----
            local_box = tk.Frame(section, bg="#20272A", highlightthickness=1, highlightbackground=theme.BORDER, padx=9, pady=7)
            local_box.grid(row=0, column=0, sticky="nsew", padx=(0, 6))
            local_title_var = tk.StringVar(value="Page sélectionnée")
            tk.Label(local_box, textvariable=local_title_var, bg="#20272A", fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 8, "bold")).pack(anchor="w")
            tk.Label(local_box, text="Exception locale", bg="#20272A", fg=theme.MUTED_DARK, font=(theme.FONT_UI, 6, "bold")).pack(anchor="w", pady=(0, 5))

            local_row = tk.Frame(local_box, bg="#20272A")
            local_row.pack(fill="x")
            tk.Label(local_row, text="Après cette page", bg="#20272A", fg=theme.MUTED, font=(theme.FONT_UI, 7, "bold")).pack(side="left", padx=(0, 6))
            local_var = tk.StringVar(value="Sélectionnez une page dans B")
            local_mb = tk.Menubutton(
                local_row, textvariable=local_var,
                bg=theme.PANEL_SOFT, fg=theme.INK,
                activebackground=theme.ACCENT_SOFT, activeforeground=theme.WHITE,
                relief="flat", bd=0, padx=8, pady=3,
                font=(theme.FONT_UI, 7, "bold"), cursor="hand2",
            )
            local_menu = tk.Menu(local_mb, tearoff=False, bg=theme.PANEL_ALT, fg=theme.INK, activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE)
            local_mb.configure(menu=local_menu)
            local_mb.pack(side="left", fill="x", expand=True)
            local_info_var = tk.StringVar(value="")
            tk.Label(local_box, textvariable=local_info_var, bg="#20272A", fg=theme.MUTED_DARK, font=(theme.FONT_UI, 7), anchor="w", justify="left").pack(fill="x", pady=(5, 0))

            def apply_local(mode, target=""):
                current = book()
                if current is not None and hasattr(current, "structure_set_selected_auto_override"):
                    current.structure_set_selected_auto_override(mode, target)
                refresh_local()

            def refresh_local():
                current = book()
                local_menu.delete(0, "end")
                if current is None or not hasattr(current, "structure_selected_auto_context"):
                    local_title_var.set("Page sélectionnée")
                    local_var.set("Sélectionnez une page dans B")
                    local_info_var.set("")
                    local_mb.configure(state="disabled")
                    return
                ctx = current.structure_selected_auto_context()
                if not ctx.get("selected"):
                    local_title_var.set("Page sélectionnée")
                    local_var.set("Sélectionnez une page dans B")
                    local_info_var.set("Cliquez une page dans B pour définir une exception uniquement sur cette page.")
                    local_mb.configure(state="disabled")
                    return
                local_title_var.set(str(ctx.get("page_label") or "Page sélectionnée"))
                if not ctx.get("allowed"):
                    local_var.set("Aucune règle locale")
                    local_info_var.set("Cette page ne peut pas déclencher de page automatique.")
                    local_mb.configure(state="disabled")
                    return
                local_mb.configure(state="normal")
                general = str(ctx.get("general_target") or "")
                general_label = labels.get(general, "Aucune") if general else "Aucune"
                inherit_label = f"Suivre la règle du type — {general_label}"
                local_menu.add_command(label=inherit_label, command=lambda: apply_local("inherit"))
                local_menu.add_command(label="Aucune page automatique", command=lambda: apply_local("none"))
                local_menu.add_separator()
                for key, label in options:
                    local_menu.add_command(label=label, command=lambda k=key: apply_local("local", k))
                mode = ctx.get("mode")
                if mode == "none":
                    local_var.set("Aucune page automatique")
                elif mode == "local":
                    local_var.set(labels.get(str(ctx.get("local_target") or ""), "Page automatique"))
                else:
                    local_var.set(inherit_label)
                local_info_var.set("Le choix ici ne concerne que cette page. Il est prioritaire sur la règle générale.")

            # ---- Niveau général : volontaire, appliqué à toutes les pages d'un type ----
            general_box = tk.Frame(section, bg=theme.PANEL_ALT, highlightthickness=1, highlightbackground=theme.BORDER, padx=9, pady=7)
            general_box.grid(row=0, column=1, sticky="nsew", padx=(6, 0))
            tk.Label(general_box, text="Règle générale par type", bg=theme.PANEL_ALT, fg=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 8, "bold")).pack(anchor="w")
            tk.Label(general_box, text="Ex. après chaque Chapitre → Illustration", bg=theme.PANEL_ALT, fg=theme.MUTED_DARK, font=(theme.FONT_UI, 6)).pack(anchor="w", pady=(0, 5))

            editor = tk.Frame(general_box, bg=theme.PANEL_ALT)
            editor.pack(fill="x")
            tk.Label(editor, text="Après chaque", bg=theme.PANEL_ALT, fg=theme.MUTED, font=(theme.FONT_UI, 7, "bold")).pack(side="left", padx=(0, 4))
            source_var = tk.StringVar(value=options[0][1])
            source_key = {"value": options[0][0]}
            source_mb = tk.Menubutton(editor, textvariable=source_var, bg=theme.PANEL_SOFT, fg=theme.INK, activebackground=theme.ACCENT_SOFT, activeforeground=theme.WHITE, relief="flat", bd=0, padx=7, pady=3, font=(theme.FONT_UI, 7, "bold"), cursor="hand2")
            source_menu = tk.Menu(source_mb, tearoff=False, bg=theme.PANEL_ALT, fg=theme.INK, activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE)
            source_mb.configure(menu=source_menu)
            for key, label in options:
                source_menu.add_command(label=label, command=lambda k=key, l=label: (source_key.__setitem__("value", k), source_var.set(l), load_existing_general(k)))
            source_mb.pack(side="left", padx=(0, 5))

            tk.Label(editor, text="→", bg=theme.PANEL_ALT, fg=theme.MUTED, font=(theme.FONT_UI, 8, "bold")).pack(side="left", padx=(0, 5))
            target_var = tk.StringVar(value="Aucune")
            target_key = {"value": ""}
            target_mb = tk.Menubutton(editor, textvariable=target_var, bg=theme.PANEL_SOFT, fg=theme.INK, activebackground=theme.ACCENT_SOFT, activeforeground=theme.WHITE, relief="flat", bd=0, padx=7, pady=3, font=(theme.FONT_UI, 7, "bold"), cursor="hand2")
            target_menu = tk.Menu(target_mb, tearoff=False, bg=theme.PANEL_ALT, fg=theme.INK, activebackground=theme.ACCENT_DARK, activeforeground=theme.WHITE)
            target_mb.configure(menu=target_menu)
            target_menu.add_command(label="Aucune page automatique", command=lambda: (target_key.__setitem__("value", ""), target_var.set("Aucune")))
            target_menu.add_separator()
            for key, label in options:
                target_menu.add_command(label=label, command=lambda k=key, l=label: (target_key.__setitem__("value", k), target_var.set(l)))
            target_mb.pack(side="left", padx=(0, 5))

            rules_holder = tk.Frame(general_box, bg=theme.PANEL_ALT)
            rules_holder.pack(fill="both", expand=True, pady=(6, 0))

            def load_existing_general(source):
                current = book()
                target = current.structure_get_general_auto_rule(source) if current is not None and hasattr(current, "structure_get_general_auto_rule") else ""
                target_key["value"] = target
                target_var.set(labels.get(target, "Aucune") if target else "Aucune")

            def refresh_rules_list():
                for child in rules_holder.winfo_children():
                    child.destroy()
                current = book()
                rules = current.structure_general_auto_rules() if current is not None and hasattr(current, "structure_general_auto_rules") else {}
                if not rules:
                    tk.Label(rules_holder, text="Aucune règle générale enregistrée.", bg=theme.PANEL_ALT, fg=theme.MUTED_DARK, font=(theme.FONT_UI, 7)).pack(anchor="w")
                    return
                for source, target in rules.items():
                    row = tk.Frame(rules_holder, bg=theme.PANEL_SOFT)
                    row.pack(fill="x", pady=1)
                    s_label = labels.get(source, source.replace("_", " ").capitalize())
                    t_label = labels.get(target, target.replace("_", " ").capitalize())
                    link = tk.Label(row, text=f"Chaque {s_label}  →  {t_label}", bg=theme.PANEL_SOFT, fg=theme.INK, font=(theme.FONT_UI, 7, "bold"), anchor="w", cursor="hand2")
                    link.pack(side="left", fill="x", expand=True, padx=7, pady=3)
                    def load(_e=None, s=source, t=target, sl=s_label, tl=t_label):
                        source_key["value"] = s; source_var.set(sl)
                        target_key["value"] = t; target_var.set(tl)
                    link.bind("<Button-1>", load)
                    remove = tk.Label(row, text="×", bg=theme.PANEL_SOFT, fg="#E4876E", font=(theme.FONT_UI, 9, "bold"), cursor="hand2")
                    remove.pack(side="right", padx=6)
                    def remove_rule(_e=None, s=source):
                        current = book()
                        if current is not None and hasattr(current, "structure_remove_general_auto_rule"):
                            current.structure_remove_general_auto_rule(s)
                        refresh_rules_list(); refresh_local()
                    remove.bind("<Button-1>", remove_rule)
            ...
            def save_rule():
                ...
                if target:
                    if hasattr(current, "structure_set_general_auto_rule"):
                        current.structure_set_general_auto_rule(source, target)
                else:
                    if hasattr(current, "structure_remove_general_auto_rule"):
                        current.structure_remove_general_auto_rule(source)
                refresh_rules_list(); refresh_local()

            command_button(editor, "Appliquer", save_rule).pack(side="left")
            load_existing_general(source_key["value"])
            refresh_rules_list()
            refresh_local()
            rules_refresh_callback["fn"] = refresh_local

        # --------------------------------------------------------------
        # Familles
        # --------------------------------------------------------------
        def clear_head_actions():
            for child in head_action_slot.winfo_children(): child.destroy()
        def clear_palette():
            reset_active_brick()
            canvas=book()
            if canvas is not None and hasattr(canvas,"structure_cancel_tool"):
                canvas.structure_cancel_tool()
            for child in palette_body.winfo_children(): child.destroy()

        def select_family(name):
            current_family["name"] = name
            rules_refresh_callback["fn"] = None
            palette_title_var.set(name)
            clear_head_actions(); clear_palette()
            for family,button in family_buttons.items():
                active=family==name
                button.configure(bg=theme.ACCENT_DARK if active else theme.PANEL_SOFT, fg=theme.WHITE if active else theme.INK)
            if name=="Pages":
                entries=[lambda parent_: make_brick(parent_,"Page sans type","page",{"type":"","label":"Sans type"}),
                         lambda parent_: make_brick(parent_,"Page blanche","page",{"type":"page_blanche","label":"Page blanche"})]
                for type_key,label,_custom in page_types():
                    entries.append(lambda parent_,k=type_key,l=label: make_brick(parent_,l,"page",{"type":k,"label":l}))
                make_scroll_flow(palette_body,entries,columns=10)
                palette_hint_var.set("Brique → clic dans B. Un clic ailleurs annule.")
            elif name=="Parties":
                row=tk.Frame(palette_body,bg=theme.PANEL_ALT); row.pack(anchor="nw",fill="x")
                make_brick(row,"Nouvelle partie","group",{"type":"partie","label":"Nouvelle partie"}).pack(side="left",padx=(0,7),pady=2)
                command_button(row,"Renommer",rename_selected).pack(side="left",padx=(0,5),pady=4)
                command_button(row,"Supprimer partie vide",delete_selected,danger=True).pack(side="left",padx=(0,5),pady=4)
                palette_hint_var.set("Clic sur la brique puis clic entre les parties dans B.")
            elif name=="Types":
                command_button(head_action_slot,"+ Nouveau type",create_custom_type).pack(side="left")
                entries=[]
                for type_key,label,locked in all_type_entries(include_locked=True):
                    entries.append(lambda parent_,k=type_key,l=label,locked_=locked: make_brick(parent_,l,"page",{"type":k,"label":l},locked=locked_))
                make_scroll_flow(palette_body,entries,columns=10)
                palette_hint_var.set("Types disponibles — briques compactes. Molette si nécessaire.")
            elif name=="Règles":
                build_rules()
                palette_hint_var.set("Même outil : règle générale du type + exception sur la page sélectionnée.")

        for family in ("Pages","Parties","Types","Règles"):
            button=tk.Button(
                rail,text=family,command=lambda n=family: select_family(n),
                bg=theme.PANEL_SOFT,fg=theme.INK,
                activebackground=theme.ACCENT_SOFT,activeforeground=theme.WHITE,
                relief="flat",bd=0,padx=4,pady=5,anchor="w",
                font=(theme.FONT_UI,7,"bold"),cursor="hand2",
            )
            button.pack(fill="x",pady=1); family_buttons[family]=button
        tk.Label(rail,text="B = travail",bg=theme.PANEL_ALT,fg=theme.MUTED_DARK,font=(theme.FONT_UI,6,"bold")).pack(side="bottom",anchor="w",padx=2,pady=(5,1))

        # Une sélection dans B met à jour le niveau local uniquement dans Règles.
        try:
            self.book_canvas.canvas.bind("<ButtonRelease-1>", lambda _e: self.after_idle(refresh_selection_context), add="+")
        except Exception:
            pass
