...
# SPIRALE_MODELE_REFERENCE_V12
    ...
    def _draw_notebook_spiral_canvas(
        ...
        image = self._get_tomelinea_spiral_image(max(60, int(y_bottom - y_top)))
        x_draw = int(round(x - (image.width() * 0.84)))
        ...
        if not hasattr(canvas, "_tl_spiral_refs_v12"):
            canvas._tl_spiral_refs_v12 = []
        canvas._tl_spiral_refs_v12.append(image)
        return item


    def _attach_notebook_spiral_frame(self, frame, accent=None):
        if getattr(frame, "_tl_spiral_bound_v12", False):
            return
        ...
        frame._tl_spiral_holder_v12 = holder
        ...
        def refresh(_event=None):
            try:
                ...
                frame_bg = "#2E3440"
            holder.configure(bg=frame_bg)

            target_height = max(70, int(frame.winfo_height()) - 30)
            image = self._get_tomelinea_spiral_image(target_height)
            holder.configure(image=image)
            holder.image = image

            x_pos = -int(round(image.width() * 0.84))
        ...
        frame._tl_spiral_bound_v12 = True

    def _get_tomelinea_spiral_image(self, target_height):
        if not hasattr(self, "_tl_spiral_cache_v12"):
            self._tl_spiral_cache_v12 = {}
        target_height = max(60, int(target_height))
        cached = self._tl_spiral_cache_v12.get(target_height)
        if cached is not None:
            return cached

        source = Image.open(TOMELINEA_SPIRAL_MODEL).convert("RGB")
        src_w, src_h = source.size
        ratio = target_height / float(src_h)
        target_width = max(10, int(round(src_w * ratio)))
        resized = source.resize((target_width, target_height), Image.LANCZOS)
        tk_image = ImageTk.PhotoImage(resized)
        self._tl_spiral_cache_v12[target_height] = tk_image
        return tk_image
