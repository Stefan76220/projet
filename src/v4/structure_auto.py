...
AUTO_PROTECTED_KEY = "automatic_user_protected"
AUTO_RETAINED_KEY = "automatic_retained"
AUTO_ORIGIN_KEY = "automatic_origin"
AUTO_ORIGIN_ROLES_KEY = "automatic_origin_roles"


def is_auto_origin_page(
    page: PageV4,
) -> bool:
    """Vrai pour toute page née automatiquement, même retenue ensuite."""

    return bool(
        is_structural_auto_page(page)
        or page.metadata.get(AUTO_RETAINED_KEY, False)
        or page.metadata.get(AUTO_ORIGIN_KEY, False)
        or str(page.metadata.get("creation_kind") or "").strip() == "automatic_structure"
    )


def auto_page_is_protected(
    page: PageV4,
) -> bool:
    """Une page auto utilisée éditorialement ne peut plus disparaître silencieusement."""

    return bool(
        page.metadata.get(AUTO_PROTECTED_KEY, False)
        or page.content
        or page.modifications
    )


def protect_auto_page(
    page: PageV4,
) -> None:
    if not is_auto_origin_page(page):
        raise ValueError("La page n'est pas d'origine automatique.")
    page.metadata[AUTO_PROTECTED_KEY] = True
    page.metadata[AUTO_ORIGIN_KEY] = True


def retain_auto_page(
    page: PageV4,
) -> None:
    """Convertit une auto devenue éditoriale en page retenue, sans perdre son origine."""
    ...
    if isinstance(roles, list) and roles:
        page.metadata[AUTO_ORIGIN_ROLES_KEY] = [
            dict(role) for role in roles if isinstance(role, dict)
        ]

    page.metadata[AUTO_ORIGIN_KEY] = True
    page.metadata[AUTO_RETAINED_KEY] = True
    page.metadata[AUTO_PROTECTED_KEY] = True
...
def structure_auto_issues(
    ...
    for source in book.pages.values():
        # Une page automatique est désormais une vraie page physique : elle peut
        # appartenir à une 2P et recevoir une correction automatique avant/après.
        for position, ids in (
            (
                AUTO_BEFORE,
                source.auto_before,
