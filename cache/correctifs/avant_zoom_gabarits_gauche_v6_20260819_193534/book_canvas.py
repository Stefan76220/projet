...
class BookCanvas(tk.Frame):
    ...
    def _build_canvas(self):
        ...
        # Gabarits — zoom secondaire repris du navigateur historique de Structure.
        # On réutilise le vrai widget (et non des hitboxes dessinées dans B) :
        # le geste − / + reste fiable même lorsque le Canvas est entièrement rerendu.
        self.gabarit_zoom_controls = tk.Frame(
            viewer, bg="#26313B", bd=0, highlightthickness=0,
        )
        self.gabarit_zoom_nav = TLZoomNavigator(
            self.gabarit_zoom_controls, command=self._gabarit_zoom_navigate,
        )
        self.gabarit_zoom_nav.configure(width=108, height=46, bg="#26313B")
        self.gabarit_zoom_nav.set_caption("Zoom 100 %")
        self.gabarit_zoom_nav.pack(side="left", fill="y")
        self.gabarit_zoom_fit_btn = tk.Button(
            self.gabarit_zoom_controls,
            text="Adapter",
            command=self.gabarit_reset_zoom,
            bg="#202D36", fg="#E6EBE9",
            activebackground="#314752", activeforeground="#FFFFFF",
            relief="flat", bd=0, padx=5, pady=4,
            font=(theme.FONT_UI, 6, "bold"), cursor="hand2",
        )
        self.gabarit_zoom_fit_btn.pack(side="left", fill="y", padx=(3, 0))
    ...
    def _gabarit_sync_zoom_widget(self) -> None:
        """Synchronise le navigateur historique avec le niveau de zoom réel."""
        value = int(getattr(self, "_gabarit_zoom", 100))
        nav = getattr(self, "gabarit_zoom_nav", None)
        if nav is not None:
            try:
                nav.set_caption(f"Zoom {value} %")
            except Exception:
                pass
    ...
    def _render_gabarit_workspace(self, viewport_w: float, viewport_h: float) -> None:
        ...
        # Zoom secondaire : vrai navigateur historique de Structure, posé tout en
        # bas de l’inspecteur. La molette reste le geste principal.
        zoom_controls = getattr(self, "gabarit_zoom_controls", None)
        if zoom_controls is not None:
            try:
                control_h = 48
                control_x = int(info_x1 + 8)
                control_y = int(info_y2 - control_h - 6)
                control_w = max(130, int(info_w - 16))
                # Si B devient exceptionnellement bas, on masque ce contrôle
                # secondaire plutôt que de le superposer aux réglages du livre.
                if control_y >= int(y + 8):
                    zoom_controls.place(x=control_x, y=control_y, width=control_w, height=control_h)
                    zoom_controls.lift()
                    nav = getattr(self, "gabarit_zoom_nav", None)
                    if nav is not None:
                        nav.configure(width=max(86, control_w - 54), height=46)
                else:
                    zoom_controls.place_forget()
            except Exception:
                pass
