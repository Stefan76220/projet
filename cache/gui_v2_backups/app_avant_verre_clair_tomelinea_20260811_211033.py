...
class PageMaitreV2(tk.Tk):
    ...
    def _build_accueil(self, parent: tk.Frame) -> None:
        ...
        def _home_glass_photo(kind: str, width: int, height: int, color: str, state: str):
            ...
            key = (kind, width, height, color, state)
            ...
            back_rgb = _blend_rgb(color_rgb, (255, 255, 255), 0.14)
            glass_rgb = _blend_rgb(color_rgb, (247, 244, 239), 0.74)
            warm_white = (252, 249, 244)
            edge_rgb = _blend_rgb(color_rgb, (232, 224, 214), 0.48)
            grain_light = _blend_rgb(warm_white, glass_rgb, 0.30)
            grain_dark = _blend_rgb((222, 215, 206), color_rgb, 0.20)

            img = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            draw = ImageDraw.Draw(img)

            # Fond coloré visible à travers le verre.
            ...
            draw.rounded_rectangle(
                ...
                outline=warm_white + (110,),
                width=1,
            )

            # Dégradé très doux venant du haut, sans trait visible.
            top = inset + 1
            bottom = height - inset - 2
            usable_h = max(1, bottom - top + 1)
            for yy in range(top, bottom + 1):
                p = (yy - top) / usable_h
                alpha = int(26 * (1.0 - p) + 6)
                wash_rgb = _blend_rgb(warm_white, glass_rgb, 0.20 + 0.35 * p)
                draw.line(
            ...
            # Grain discret réparti régulièrement.
            for gx in range(10, width - 10, grain_step_x):
                for gy in range(8, height - 8, grain_step_y):
                    seed = ((gx * 17 + gy * 23 + width * 3 + height) % 7)
                    if seed in (0, 3, 5):
                        dot_fill = grain_light + (50 if seed == 0 else 36,)
                        r = 1
                    else:
                        dot_fill = grain_dark + (24 if seed == 1 else 18,)
                        r = 1
                    draw.ellipse(
                        (gx, gy, gx + r, gy + r),
                        fill=dot_fill,
            ...
            # Fin liseré intérieur légèrement plus clair.
            draw.rounded_rectangle(
        ...
        def home_command_button(
            ...
            *,
            ...
            shadow = self._mix(color, "#7A756F", 0.34 if state == "hover" else 0.40)
        ...
        def home_choice_panel(
            ...
            *,
            ...
            shadow = self._mix(color, "#7B756F", 0.34 if hover else 0.40)
            if selected:
                shadow = self._mix(color, "#6E6963", 0.30 if hover else 0.36)
