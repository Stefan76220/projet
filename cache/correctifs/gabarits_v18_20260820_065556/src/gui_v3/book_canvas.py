...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_type_identity(self, item: dict | None) -> str:
        """Identité éditoriale stable utilisée par « Toutes du type ».

        L'ancienne propagation comparait uniquement ``item["type"]``. Or TomeLinea
        accepte aussi des alias historiques et plusieurs champs de type. Deux pages
        visuellement du même type pouvaient donc ne pas être reconnues comme homologues.
        On s'appuie désormais sur la clé canonique déjà utilisée par la bibliothèque
        visuelle, avec repli sur le type brut pour les types personnalisés.
        """
        if not isinstance(item, dict):
            return ""
        try:
            key = str(self._page_type_key(item) or "").strip().lower()
        except Exception:
            key = ""
        if key:
            return key
