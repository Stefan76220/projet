...
class BookCanvas(tk.Frame):
    ...
    def _structure_bar_button(self, parent, text, command, *, danger=False):
        """Outil Structure V12 : même matière visuelle que l’atelier C."""
        base = "#31474B"
        hover = "#3A5559"
        pressed = "#243438"
        edge = "#48656A"
        fg = "#F0D8CF" if danger else "#E9ECE8"
        label = tk.Label(
            parent, text=text, bg=base, fg=fg,
            font=(theme.FONT_UI, 8, "bold"), padx=11, pady=3,
            highlightthickness=1, highlightbackground=edge,
            cursor="hand2",
        )
        def enter(_e=None):
            label.configure(bg=hover, highlightbackground="#74B6AA")
        def leave(_e=None):
            label.configure(bg=base, highlightbackground=edge)
        def press(_e=None):
    ...
    def _structure_action_button(self, mode: str):
        ...
        if mode not in {"duplicate", "delete"}:
            return
        # Si la cible est déjà choisie, le même bouton devient Confirmer.
        ...
        self.status_var.set(f"{verb}  •  cliquez sur la page ou la partie concernée dans B")
    ...
    def _structure_update_action_buttons(self):
        ...
        if duplicate is not None and duplicate.winfo_exists():
            duplicate.configure(
                ...
                bg="#385E60" if mode == "duplicate" else "#31474B",
                fg="#E9ECE8",
                highlightbackground="#C2A36A" if mode == "duplicate" else "#48656A",
            )
        if delete is not None and delete.winfo_exists():
            delete.configure(
                text="Confirmer" if mode == "delete" and target else "Supprimer",
                bg="#5A3937" if mode == "delete" else "#31474B",
                fg="#F0D8CF",
                highlightbackground="#C56B5B" if mode == "delete" else "#48656A",
            )
    ...
    def _build_canvas(self):
        ...
        # Bandeau Structure V12 : dock centré, de la longueur exacte de son contenu.
        # Le fond de B reste visible autour : ce n'est plus une barre pleine largeur.
        self.structure_command_bar_host = tk.Frame(
            viewer, bg=theme.WINDOW_DEEP, height=38,
            bd=0, highlightthickness=0,
        )
        self.structure_command_bar_host.grid(row=1, column=0, sticky="ew", pady=(0, 1))
        self.structure_command_bar_host.grid_propagate(False)

        self.structure_command_bar = tk.Frame(
            self.structure_command_bar_host, bg="#293A3D",
            highlightthickness=1, highlightbackground="#48656A",
            padx=5, pady=4,
        )
        self.structure_command_bar.pack(anchor="center", pady=2)

        self.structure_new_part_btn = self._structure_bar_button(
            self.structure_command_bar, "+ Partie", self._structure_new_part_command
        )
        self.structure_new_part_btn.pack(side="left", padx=(0, 5))
        self.structure_rename_btn = self._structure_bar_button(
            self.structure_command_bar, "Nommer", self.structure_rename_selected
        )
        self.structure_rename_btn.pack(side="left", padx=(0, 5))
        ...
        self.structure_duplicate_btn.pack(side="left", padx=(0, 5))
        ...
        self.structure_delete_btn.pack(side="left")
    ...
    def set_work_mode(self, mode: str):
        ...
        bar_host = getattr(self, "structure_command_bar_host", None)
        if bar_host is not None:
            if mode == "structure":
                bar_host.grid()
            else:
                bar_host.grid_remove()
        if mode != "structure":
    ...
    def structure_duplicate_selected(self) -> bool:
        ...
        self._selected_group_id = group_id
        self._structure_selection_kind = "page"
        self._sync_auto_follow_for_source(duplicate)
        self._save_order()
        self.render()
        self.after_idle(self.center_selected)
        return True
    ...
    def _on_press(self, event):
        if getattr(self, "_work_mode", "structure") == "structure" and getattr(self, "_structure_action_mode", None):
            return self._structure_pick_action_target(event)
        if getattr(self, "_work_mode", "structure") == "structure" and getattr(self, "_structure_pending_kind", None):
