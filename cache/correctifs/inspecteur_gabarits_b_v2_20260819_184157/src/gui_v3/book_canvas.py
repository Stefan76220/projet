...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_sheet_settings(self, item: dict | None = None) -> dict:
        """Réunit les réglages de page utilisés par le rendu de B."""
        settings = self._gabarit_default_settings()
    ...
    def _gabarit_bleed_pixels(w: float, h: float, settings: dict) -> tuple[float, float, float, float]:
        ...
        def px(mm: float, dimension: float, reference_mm: float) -> float:
            ...
            if mm <= 0.0:
                return 0.0
            return max(2.5, float(dimension) * mm / reference_mm)
    ...
    def _gabarit_draw_bleed_halo(
        ...
        *, settings: dict, fold_side: str = "",
        ...
        fill = "#9A6263"      # rose brique / saumon fumé : technique, pas alerte
        outline = "#AA7273"
        stipple = "gray25"    # transparence simulée, portable sur Tkinter
