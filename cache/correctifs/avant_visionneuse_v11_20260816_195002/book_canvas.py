...
class BookCanvas(tk.Frame):
    ...
    def _render_page_overlay(self):
        ...
        target.create_text(width / 2.0, max(26.0, y - 42.0), text="TomeLinea", fill="#36454C", font=(theme.FONT_TITLE, 18, "bold"))
        target.create_line(max(26, x - 220), max(44, y - 18), x - 20, max(44, y - 18), fill=theme.BORDER_SOFT, width=1)
        target.create_line(x + page_w + 20, max(44, y - 18), min(width - 26, x + page_w + 220), max(44, y - 18), fill=theme.BORDER_SOFT, width=1)

        left_x = max(28.0, x - 210.0)
        right_x = min(width - 28.0, x + page_w + 24.0)
        top_y = max(62.0, y + 16.0)
        target.create_text(left_x, top_y, text=page_title, anchor="nw", fill=theme.INK, font=(theme.FONT_TITLE, 15, "bold"))
        target.create_text(left_x, top_y + 26, text=page_type, anchor="nw", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 10, "bold"))
        target.create_text(left_x, top_y + 48, text=f"Partie : {part_name}", anchor="nw", fill=theme.INK_SOFT, font=(theme.FONT_UI, 9))
        target.create_text(right_x, top_y, text=part_title, anchor="ne", fill=theme.INK, font=(theme.FONT_TITLE, 14, "bold"))
        target.create_text(right_x, top_y + 24, text=f"Circuit : {stage_text}", anchor="ne", fill=theme.ACCENT_BRIGHT, font=(theme.FONT_UI, 10, "bold"))
        target.create_text(right_x, top_y + 46, text=f"Position : {index + 1}/{len(self.items)}", anchor="ne", fill=theme.INK_SOFT, font=(theme.FONT_UI, 9))
        deco_y = y + page_h * 0.5
        deco_x = width / 2.0
        self._draw_overlay_book_icon(target, deco_x, deco_y - 26, theme.BORDER_SOFT)
        target.create_line(deco_x - 160, deco_y + 34, deco_x + 160, deco_y + 34, fill="#26343B", width=1)
