...
class BookCanvas(tk.Frame):
    ...
    def _gabarit_is_local_exception(self, item: dict | None, index: int | None = None) -> bool:
        """Vrai seulement pour une vraie exception à un groupe de même type.

        ``gabarit_local_override`` signifie qu'une page a été validée localement.
        Pour éviter d'afficher « Exception » sur une page qui n'a jamais appartenu
        à un groupe, on exige aussi l'existence d'au moins une autre page manuelle
        du même type encore rattachée à la portée « Toutes du type ».
        """
