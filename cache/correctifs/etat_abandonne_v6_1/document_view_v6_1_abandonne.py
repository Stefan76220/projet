...
class DocumentView:
    ...
    def __init__(
        ...
        self._mockup_view: MockupView | None = None
    ...
    def _open_mockup(self) -> None:
        """
        Construit le Maquettage hors écran si nécessaire,
        puis l'affiche d'un seul coup.
        """
        self._hide_project_tools_for_subspace()

        view = self._ensure_mockup_view()
        ready = bool(getattr(view, "_mockup_ready", False))
        root = getattr(view, "_root", None)

        if root is not None:
            try:
                ready = ready and bool(root.winfo_exists())
            except tk.TclError:
                ready = False
        ...
        try:
            ...
            root.lift()
            view._activate_global_shortcuts()
        except tk.TclError:
            pass

    def _close_mockup(self) -> None:
        """Revient au Centre instantanément, sans reconstruction."""
        view = self._mockup_view
        if view is not None:
            view.hide()
    ...
    def _mockup_go_home(self) -> None:
        """Quitte le Maquettage puis ouvre l'accueil général."""
        view = self._mockup_view
        if view is not None:
            view.hide()
    ...
    def _open_mockup_page(self, item: dict) -> None:
        """Ouvre le Maquettage persistant directement sur une page."""
        item_id = str(item.get("id", "")).strip()
        ...
        view = self._mockup_view
        if view is None or not item_id:
            return
