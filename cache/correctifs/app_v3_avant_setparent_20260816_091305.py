...
class TomeLineaV3(tk.Tk):
    ...
    def __init__(self, *, use: str | None = None, native_embedded: bool = False) -> None:
        # INTEGRATION_HOTE_WIN32_VALIDEE_V3_V1
        # En mode natif, Tk ne gère plus la fenêtre extérieure : il est
        # simplement embarqué comme contenu d'une vraie fenêtre Win32.
        super().__init__(use=use)
        self._native_embedded = bool(native_embedded)

        if not self._native_embedded:
            # Mode historique : construction hors écran avant affichage.
            self.withdraw()

            # Le mode historique reste disponible hors de l'hôte Win32.
            try:
        ...
        # Première passe de géométrie.
        self.update_idletasks()

        if self._native_embedded:
            # La fenêtre extérieure et ses transitions appartiennent à Win32.
            # A/B/C ne changent pas : seul leur conteneur extérieur est différent.
            self._screens["home"].tkraise()
            self.update_idletasks()
        else:
            # Mode historique hors hôte natif.
            self.deiconify()
