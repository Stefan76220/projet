...
class BookCanvas(tk.Frame):
    ...
    def gabarit_set_scope(self, scope: str) -> None:
        """Choisit la portée des prochaines modifications de gabarit.

        La portée locale reste le choix sûr par défaut. Le choix "type"
        s'applique aux pages manuelles du même type et reste actif tant que
        l'utilisateur ne le change pas.
        """
        scope = str(scope or "page").strip().lower()
        new_scope = "type" if scope == "type" else "page"
        if str(getattr(self, "_gabarit_scope", "page")) == new_scope:
            return
        self._gabarit_scope = new_scope
        if getattr(self, "_work_mode", "") == "gabarits":
            self.render()
        try:
            self.status_var.set(
                "Portée : toutes les pages de ce type"
                if new_scope == "type"
                else "Portée : cette page"
            )
        except Exception:
            pass
